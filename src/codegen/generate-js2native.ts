...
function normalizeSymbolPathPrefix(input: string) {
  ...
  const funDir = path.resolve(path.join(import.meta.dir, "..", ".."));
  if (input.startsWith(funDir)) {
    input = input.slice(funDir.length);
  }
  ...
  const wrapperCallStrings = wrapperCalls.map(x => {
    if (x.wrap_kind === "new-function") {
      return [
        (x.type === "zig" &&
          externs.push(
            `FUN_DECLARE_HOST_FUNCTION(${symbol({
              type: "zig",
  ...
  return [
    ...
    ...wrapperCalls
      ...
      .flatMap(x => [
        ...
        `    return @call(fun.callmod_inline, jsc.toJSHostFn(function.${x.symbol_target}), .{global, call_frame});`,
        "}",
      ]),
    "comptime {",
    ...nativeCalls
      ...
      .flatMap(x => {
        ...
        return [
          `    @export(&fun.gen.${base}.create${cap(x.symbol)}Callback, .{ .name = ${JSON.stringify(
