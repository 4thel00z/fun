//! Closed-world enum of every us_socket_t consumer in Fun. Stamped on the
...
//! *handler* often differs (e.g. `Fun.connect` TCP vs TLS land in different
...
pub const SocketKind = enum(u8) {
    ...
    // ── Fun.connect / Fun.listen (src/runtime/api/fun/socket.zig) ──────────
    fun_socket_tcp,
    fun_socket_tls,
    /// Server-accepted socket; ext is the `*Listener` so onCreate can attach
    /// a fresh `NewSocket` before re-stamping to `fun_socket_{tcp,tls}`.
    fun_listener_tcp,
    fun_listener_tls,
    ...
    // ── Fun.spawn IPC over socketpair ─────────────────────────────────────
    spawn_ipc,

    // ── Fun.serve / uWS — handlers live in C++; dispatch calls a thunk and
...
comptime {
    ...
    fun.assert(@typeInfo(SocketKind).@"enum".fields.len <= 256);
}

/// The four kinds whose handlers live in C++ are also referenced from C++
/// (`packages/fun-uws/src/SocketKinds.h`). Export their ordinals so the C++
...
const exported_ordinals = struct {
    export const FUN_SOCKET_KIND_DYNAMIC: u8 = @intFromEnum(SocketKind.dynamic);
    export const FUN_SOCKET_KIND_UWS_HTTP: u8 = @intFromEnum(SocketKind.uws_http);
    export const FUN_SOCKET_KIND_UWS_HTTP_TLS: u8 = @intFromEnum(SocketKind.uws_http_tls);
    export const FUN_SOCKET_KIND_UWS_WS: u8 = @intFromEnum(SocketKind.uws_ws);
    export const FUN_SOCKET_KIND_UWS_WS_TLS: u8 = @intFromEnum(SocketKind.uws_ws_tls);
};
comptime {
    _ = exported_ordinals;
}
