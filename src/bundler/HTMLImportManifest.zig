//! HTMLImportManifest generates JSON manifests for HTML imports in Fun's bundler.
//!
...
//! ```
//!
//! Fun transforms this into a call to `__jsonParse()` with a JSON manifest containing
...
pub fn format(this: HTMLImportManifest, writer: *std.Io.Writer) fun.OOM!void {
...
fn writeEntryItem(
    ...
    kind: fun.jsc.API.BuildArtifact.OutputKind,
) !void {
    try writer.writeAll("{");

    if (input.len > 0) {
        try writer.writeAll("\"input\":");
        try fun.js_printer.writeJSONString(input, @TypeOf(writer), writer, .utf8);
        try writer.writeAll(",");
    }

    try writer.writeAll("\"path\":");
    try fun.js_printer.writeJSONString(path, @TypeOf(writer), writer, .utf8);
    ...
    if (hash > 0) {
        var base64_buf: [fun.base64.encodeLenFromSize(@sizeOf(@TypeOf(hash))) + 2]u8 = undefined;
        const base64 = base64_buf[0..fun.base64.encodeURLSafe(&base64_buf, &std.mem.toBytes(hash))];
        try writer.print(
            \\"etag":"{s}",
        , .{base64});
    }
...
pub fn writeEscapedJSON(index: u32, graph: *const Graph, linker_graph: *const LinkerGraph, chunks: []const Chunk, writer: anytype) !void {
    var stack = std.heap.stackFallback(4096, fun.default_allocator);
    const allocator = stack.get();
    var bytes = std.array_list.Managed(u8).init(allocator);
    defer bytes.deinit();
    try write(index, graph, linker_graph, chunks, bytes.writer());
    try fun.js_printer.writePreQuotedString(bytes.items, @TypeOf(writer), writer, '"', false, true, .utf8);
}
...
pub fn write(index: u32, graph: *const Graph, linker_graph: *const LinkerGraph, chunks: []const Chunk, writer: anytype) !void {
    ...
    const sources: []const fun.logger.Source = graph.input_files.items(.source);
    const bv2: *const BundleV2 = @alignCast(@fieldParentPtr("graph", graph));
    var entry_point_bits = try fun.bit_set.AutoBitSet.initEmpty(fun.default_allocator, graph.entry_points.items.len);
    defer entry_point_bits.deinit(fun.default_allocator);

    const root_dir = if (bv2.transpiler.options.root_dir.len > 0) bv2.transpiler.options.root_dir else fun.fs.FileSystem.instance.top_level_dir;
    ...
    var temp_buffer = std.array_list.Managed(u8).init(fun.default_allocator);
        ...
        if (ch.entry_point.source_index == browser_source_index and ch.entry_point.is_entry_point) {
            ...
            if (ch.content == .html) {
                ...
                if (inject_compiler_filesystem_prefix) {
                    ...
                    try temp_buffer.appendSlice(fun.strings.removeLeadingDotSlash(ch.final_rel_path));
                    try fun.js_printer.writeJSONString(temp_buffer.items, @TypeOf(writer), writer, .utf8);
                } else {
                    try fun.js_printer.writeJSONString(ch.final_rel_path, @TypeOf(writer), writer, .utf8);
                }
                try writer.writeAll(",");
            }
        }
    ...
    var already_visited_output_file = try fun.bit_set.AutoBitSet.initEmpty(fun.default_allocator, additional_output_files.len);
    defer already_visited_output_file.deinit(fun.default_allocator);
            ...
            (has_single_html_import and ch.flags.is_browser_chunk_from_server_build))
        {
            ...
            try writeEntryItem(
                ...
                brk: {
                    ...
                    var path_for_key = fun.path.relativeNormalized(
                        root_dir,
                    ...
                    path_for_key = fun.strings.removeLeadingDotSlash(path_for_key);
                ...
                try writeEntryItem(
                    ...
                    brk: {
                        if (inject_compiler_filesystem_prefix) {
                            temp_buffer.clearRetainingCapacity();
                            try temp_buffer.appendSlice(public_path);
                            try temp_buffer.appendSlice(fun.strings.removeLeadingDotSlash(output_file.dest_path));
                            break :brk temp_buffer.items;
                        }
