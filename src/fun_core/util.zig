...
pub fn fromEntries(
    ...
    if (comptime fun.trait.isIndexable(EntryType)) {
    ...
    } else if (comptime fun.trait.isContainer(EntryType) and std.meta.fields(EntryType).len > 0) {
        if (comptime !needsAllocator(Map.ensureUnusedCapacity)) {
        ...
        return map;
    } else if (comptime fun.trait.isConstPtr(EntryType) and std.meta.fields(std.meta.Child(EntryType)).len > 0) {
        if (comptime !needsAllocator(Map.ensureUnusedCapacity)) {
...
pub fn Of(comptime ArrayLike: type) type {
    if (fun.trait.isSlice(ArrayLike)) {
...
pub inline fn from(
    ...
    if (comptime fun.trait.isSlice(DefaultType)) {
        return fromSlice(Array, allocator, DefaultType, default);
    }

    if (comptime fun.trait.isContainer(DefaultType)) {
        if (comptime fun.trait.isContainer(Array) and @hasDecl(DefaultType, "put")) {
    ...
    if (comptime fun.trait.isContainer(Array) and @hasDecl(Array, "put")) {
        if (comptime fun.trait.isConstPtr(DefaultType) and std.meta.fields(std.meta.Child(DefaultType)).len > 0) {
...
pub fn fromSlice(
    ...
    if (comptime fun.trait.isSlice(Array)) {} else if (comptime @hasField(Array, "allocator")) {
    ...
    if (comptime !fun.trait.isSlice(Array) and @hasField(Array, "bytes")) {
        ...
        if (comptime !fun.trait.isSlice(Array)) {
