...
pub fn IntegrityFormatter(comptime style: IntegrityFormatStyle) type {
    return struct {
        ...
        pub fn format(this: @This(), writer: *std.Io.Writer) std.Io.Writer.Error!void {
            ...
            const count = fun.simdutf.base64.encode(this.bytes[0..sha.SHA512.digest], &buf, false);
...
const JSONFormatter = struct {
    ...
    pub fn format(self: JSONFormatter, writer: *std.Io.Writer) !void {
        try fun.js_printer.writeJSONString(self.input, @TypeOf(writer), writer, .latin1);
    }
};
...
const JSONFormatterUTF8 = struct {
    ...
    pub fn format(self: JSONFormatterUTF8, writer: *std.Io.Writer) !void {
        if (self.opts.quote) {
            try fun.js_printer.writeJSONString(self.input, @TypeOf(writer), writer, .utf8);
        } else {
            try fun.js_printer.writePreQuotedString(self.input, @TypeOf(writer), writer, '"', false, true, .utf8);
        }
    }
};
...
fn getSharedBuffer() []u8 {
    return std.mem.asBytes(shared_temp_buffer_ptr orelse brk: {
        shared_temp_buffer_ptr = fun.default_allocator.create(SharedTempBuffer) catch unreachable;
    ...
    defer {
        if (shared_temp_buffer_ptr) |existing| {
            if (existing != chunk.ptr) {
                fun.default_allocator.destroy(@as(*SharedTempBuffer, @ptrCast(chunk.ptr)));
            }
        } else {
...
pub const DebugUTF32PathFormatter = struct {
    ...
    pub fn format(this: @This(), writer: *std.Io.Writer) !void {
        ...
        const result = fun.simdutf.convert.utf32.to.utf8.with_errors.le(this.path, &path_buf);
...
pub fn fmtOSPath(buf: fun.OSPathSlice, options: PathFormatOptions) FormatOSPath {
    return FormatOSPath{
        .buf = buf,
...
pub fn quotedWriter(writer: *std.Io.Writer, self: string) !void {
    ...
    if (strings.containsNewlineOrNonASCIIOrQuote(remain)) {
        try fun.js_printer.writeJSONString(self, @TypeOf(writer), writer, strings.Encoding.utf8);
    } else {
        try writer.writeAll("\"");
...
pub const QuickAndDirtyJavaScriptSyntaxHighlighter = struct {
    ...
    pub const Keywords = fun.ComptimeEnumMap(Keyword);
    ...
    pub const RedactedKeywords = fun.ComptimeEnumMap(RedactedKeyword);
...
pub fn quote(self: string) fun.fmt.QuotedFormatter {
    return fun.fmt.QuotedFormatter{
        .text = self,
    };
}
...
pub const FormatDouble = struct {
    ...
    pub fn dtoa(buf: *[124]u8, number: f64) []const u8 {
        const len = fun.cpp.WTF__dtoa(&buf.ptr[0], number);
        return buf[0..len];
    }
...
fn escapePowershellImpl(str: []const u8, writer: *std.Io.Writer) !void {
    var remain = str;
    while (fun.strings.indexOfAny(remain, "\"`")) |i| {
...
// alongside fmt_jsc.bind.ts; fun_core/ stays JSC-free.
...
const FunStringOutOfRangeFormatter = NewOutOfRangeFormatter(fun.String);
...
fn OutOfRangeFormatter(comptime T: type) type {
    ...
    } else if (T == fun.String) {
        return FunStringOutOfRangeFormatter;
    }
