...
/// Enable breaking changes for the next major release of Fun
...
// TODO: remove this flag, it should use fun.Output.scoped
...
/// https://github.com/underdoc-org/fun/issues/10733
pub const support_jsxs_in_jsx_transform = true;

pub const use_simdutf = fun.Environment.isNative;

pub const inline_properties_in_transpiler = true;

pub const same_target_becomes_destructuring = true;

pub const help_catch_memory_issues = fun.Environment.enable_asan or fun.Environment.isDebug;
...
// https://github.com/underdoc-org/fun/issues/5426#issuecomment-1813865316
pub const disable_auto_js_to_ts_in_node_modules = true;

pub const runtime_transpiler_cache = true;

/// On Windows, node_modules/.bin uses pairs of '.exe' + '.funx' files.  The
/// fast path is to load the .funx file within `fun.exe` instead of
/// `fun_shim_impl.exe` by using `fun_shim_impl.tryStartupFromFunJS`
///
...
pub fn isLibdeflateEnabled() bool {
    ...
    return !fun.feature_flag.FUN_FEATURE_FLAG_NO_LIBDEFLATE.get();
}

/// Enable the "app" option in Fun.serve. This option will likely be removed
/// in favor of HTML loaders and configuring framework options in funfig.toml
pub fn bake() bool {
    // In canary or if an environment variable is specified.
    return env.is_canary or env.isDebug or fun.feature_flag.FUN_FEATURE_FLAG_EXPERIMENTAL_BAKE.get();
}
