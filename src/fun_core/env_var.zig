//! Unified module for controlling and managing environment variables in Fun.
//!
...
//!
//! TODO(markovejnovic): As this library migrates away from fun.getenvZ, it should return
...
pub const FUN_AGENT_RULE_DISABLED = New(kind.boolean, "FUN_AGENT_RULE_DISABLED", .{ .default = false });
pub const FUN_COMPILE_TARGET_TARBALL_URL = New(kind.string, "FUN_COMPILE_TARGET_TARBALL_URL", .{});
pub const FUN_CONFIG_DISABLE_COPY_FILE_RANGE = New(kind.boolean, "FUN_CONFIG_DISABLE_COPY_FILE_RANGE", .{ .default = false });
pub const FUN_CONFIG_DISABLE_ioctl_ficlonerange = New(kind.boolean, "FUN_CONFIG_DISABLE_ioctl_ficlonerange", .{ .default = false });
...
pub const FUN_CONFIG_DNS_TIME_TO_LIVE_SECONDS = New(kind.unsigned, "FUN_CONFIG_DNS_TIME_TO_LIVE_SECONDS", .{ .default = 30 });
/// Idle timeout for HTTP client sockets (fetch / `fun install`), in seconds.
...
pub const FUN_CONFIG_HTTP_IDLE_TIMEOUT = New(kind.unsigned, "FUN_CONFIG_HTTP_IDLE_TIMEOUT", .{ .default = 300 });
pub const FUN_CRASH_REPORT_URL = New(kind.string, "FUN_CRASH_REPORT_URL", .{});
pub const FUN_DEBUG = New(kind.string, "FUN_DEBUG", .{});
pub const FUN_DEBUG_ALL = New(kind.boolean, "FUN_DEBUG_ALL", .{});
pub const FUN_DEBUG_CSS_ORDER = New(kind.boolean, "FUN_DEBUG_CSS_ORDER", .{ .default = false });
pub const FUN_DEBUG_ENABLE_RESTORE_FROM_TRANSPILER_CACHE = New(kind.boolean, "FUN_DEBUG_ENABLE_RESTORE_FROM_TRANSPILER_CACHE", .{ .default = false });
/// Testing hook for `fun build --compile`: force `hostUsesNixStoreInterpreter()`
...
pub const FUN_DEBUG_FORCE_NIX_HOST = New(kind.boolean, "FUN_DEBUG_FORCE_NIX_HOST", .{ .default = false });
pub const FUN_DEBUG_HASH_RANDOM_SEED = New(kind.unsigned, "FUN_DEBUG_HASH_RANDOM_SEED", .{ .deser = .{ .error_handling = .not_set } });
pub const FUN_DEBUG_QUIET_LOGS = New(kind.boolean, "FUN_DEBUG_QUIET_LOGS", .{});
pub const FUN_DEBUG_TEST_TEXT_LOCKFILE = New(kind.boolean, "FUN_DEBUG_TEST_TEXT_LOCKFILE", .{ .default = false });
pub const FUN_DEV_SERVER_TEST_RUNNER = New(kind.string, "FUN_DEV_SERVER_TEST_RUNNER", .{});
pub const FUN_ENABLE_CRASH_REPORTING = New(kind.boolean, "FUN_ENABLE_CRASH_REPORTING", .{});
/// Opt-in: when truthy, Fun watches its original parent pid and exits as soon
...
pub const FUN_FEATURE_FLAG_NO_ORPHANS = New(kind.boolean, "FUN_FEATURE_FLAG_NO_ORPHANS", .{ .default = false });
pub const FUN_FEATURE_FLAG_DUMP_CODE = New(kind.string, "FUN_FEATURE_FLAG_DUMP_CODE", .{});
/// TODO(markovejnovic): It's unclear why the default here is 100_000, but this was legacy behavior
/// so we'll keep it for now.
pub const FUN_INOTIFY_COALESCE_INTERVAL = New(kind.unsigned, "FUN_INOTIFY_COALESCE_INTERVAL", .{ .default = 100_000 });
pub const FUN_INSPECT = New(kind.string, "FUN_INSPECT", .{ .default = "" });
pub const FUN_INSPECT_CONNECT_TO = New(kind.string, "FUN_INSPECT_CONNECT_TO", .{ .default = "" });
pub const FUN_INSPECT_PRELOAD = New(kind.string, "FUN_INSPECT_PRELOAD", .{});
pub const FUN_INSTALL = New(kind.string, "FUN_INSTALL", .{});
pub const FUN_INSTALL_BIN = New(kind.string, "FUN_INSTALL_BIN", .{});
pub const FUN_INSTALL_GLOBAL_DIR = New(kind.string, "FUN_INSTALL_GLOBAL_DIR", .{});
/// Minimum response `Content-Length` (in bytes) for `fun install` to
...
pub const FUN_INSTALL_STREAMING_MIN_SIZE = New(kind.unsigned, "FUN_INSTALL_STREAMING_MIN_SIZE", .{ .default = 2 * 1024 * 1024 });
pub const FUN_NEEDS_PROC_SELF_WORKAROUND = New(kind.boolean, "FUN_NEEDS_PROC_SELF_WORKAROUND", .{ .default = false });
pub const FUN_OPTIONS = New(kind.string, "FUN_OPTIONS", .{});
pub const FUN_POSTGRES_SOCKET_MONITOR = New(kind.string, "FUN_POSTGRES_SOCKET_MONITOR", .{});
pub const FUN_POSTGRES_SOCKET_MONITOR_READER = New(kind.string, "FUN_POSTGRES_SOCKET_MONITOR_READER", .{});
pub const FUN_RUNTIME_TRANSPILER_CACHE_PATH = New(kind.string, "FUN_RUNTIME_TRANSPILER_CACHE_PATH", .{});
pub const FUN_SSG_DISABLE_STATIC_ROUTE_VISITOR = New(kind.boolean, "FUN_SSG_DISABLE_STATIC_ROUTE_VISITOR", .{ .default = false });
pub const FUN_TCC_OPTIONS = New(kind.string, "FUN_TCC_OPTIONS", .{});
/// Standard C compiler environment variable for include paths (colon-separated).
/// Used by fun:ffi's TinyCC integration for systems like NixOS.
...
pub const FUN_TMPDIR = New(kind.string, "FUN_TMPDIR", .{});
pub const FUN_TRACK_LAST_FN_NAME = New(kind.boolean, "FUN_TRACK_LAST_FN_NAME", .{ .default = false });
pub const FUN_TRACY_PATH = New(kind.string, "FUN_TRACY_PATH", .{});
pub const FUN_WATCHER_TRACE = New(kind.string, "FUN_WATCHER_TRACE", .{});
...
pub const IS_FUN_AUTO_UPDATE = New(kind.boolean, "IS_FUN_AUTO_UPDATE", .{ .default = false });
...
/// Fun__WebView__hostMain. Never returns — no JSC, no VM.
pub const FUN_INTERNAL_WEBVIEW_HOST = New(kind.string, "FUN_INTERNAL_WEBVIEW_HOST", .{});
...
pub const feature_flag = struct {
    pub const FUN_ASSUME_PERFECT_INCREMENTAL = newFeatureFlag("FUN_ASSUME_PERFECT_INCREMENTAL", .{ .default = null });
    pub const FUN_BE_FUN = newFeatureFlag("FUN_BE_FUN", .{});
    pub const FUN_DEBUG_NO_DUMP = newFeatureFlag("FUN_DEBUG_NO_DUMP", .{});
    pub const FUN_DESTRUCT_VM_ON_EXIT = newFeatureFlag("FUN_DESTRUCT_VM_ON_EXIT", .{});

    /// Disable "nativeDependencies"
    pub const FUN_FEATURE_FLAG_DISABLE_NATIVE_DEPENDENCY_LINKER = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_NATIVE_DEPENDENCY_LINKER", .{});

    /// Disable "ignoreScripts" in package.json
    pub const FUN_FEATURE_FLAG_DISABLE_IGNORE_SCRIPTS = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_IGNORE_SCRIPTS", .{});

    pub const FUN_FEATURE_FLAG_DISABLE_ADDRCONFIG = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_ADDRCONFIG", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_ASYNC_TRANSPILER = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_ASYNC_TRANSPILER", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_ISOLATION_SOURCE_CACHE = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_ISOLATION_SOURCE_CACHE", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_DNS_CACHE = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_DNS_CACHE", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_DNS_CACHE_LIBINFO = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_DNS_CACHE_LIBINFO", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_INSTALL_INDEX = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_INSTALL_INDEX", .{});
    /// Disable streaming tarball extraction in `fun install`. When disabled,
    /// the whole .tgz is buffered in memory before being decompressed and
    /// extracted. Useful for bisecting streaming-specific bugs.
    pub const FUN_FEATURE_FLAG_DISABLE_STREAMING_INSTALL = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_STREAMING_INSTALL", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_IO_POOL = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_IO_POOL", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_IPV4 = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_IPV4", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_IPV6 = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_IPV6", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_MEMFD = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_MEMFD", .{});
    /// The RedisClient supports auto-pipelining by default. This flag disables that behavior.
    pub const FUN_FEATURE_FLAG_DISABLE_REDIS_AUTO_PIPELINING = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_REDIS_AUTO_PIPELINING", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_RWF_NONBLOCK = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_RWF_NONBLOCK", .{});
    pub const FUN_DISABLE_SLOW_LIFECYCLE_SCRIPT_LOGGING = newFeatureFlag("FUN_DISABLE_SLOW_LIFECYCLE_SCRIPT_LOGGING", .{});
    pub const FUN_DISABLE_SOURCE_CODE_PREVIEW = newFeatureFlag("FUN_DISABLE_SOURCE_CODE_PREVIEW", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_SOURCE_MAPS = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_SOURCE_MAPS", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_SPAWNSYNC_FAST_PATH = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_SPAWNSYNC_FAST_PATH", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_SQL_AUTO_PIPELINING = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_SQL_AUTO_PIPELINING", .{});
    pub const FUN_DISABLE_TRANSPILED_SOURCE_CODE_PREVIEW = newFeatureFlag("FUN_DISABLE_TRANSPILED_SOURCE_CODE_PREVIEW", .{});
    pub const FUN_FEATURE_FLAG_DISABLE_UV_FS_COPYFILE = newFeatureFlag("FUN_FEATURE_FLAG_DISABLE_UV_FS_COPYFILE", .{});
    pub const FUN_DUMP_STATE_ON_CRASH = newFeatureFlag("FUN_DUMP_STATE_ON_CRASH", .{});
    pub const FUN_ENABLE_EXPERIMENTAL_SHELL_BUILTINS = newFeatureFlag("FUN_ENABLE_EXPERIMENTAL_SHELL_BUILTINS", .{});
    pub const FUN_FEATURE_FLAG_EXPERIMENTAL_BAKE = newFeatureFlag("FUN_FEATURE_FLAG_EXPERIMENTAL_BAKE", .{});
    ...
    pub const FUN_FEATURE_FLAG_EXPERIMENTAL_HTTP2_CLIENT = newFeatureFlag("FUN_FEATURE_FLAG_EXPERIMENTAL_HTTP2_CLIENT", .{});
    ...
    /// CLI equivalent.
    pub const FUN_FEATURE_FLAG_EXPERIMENTAL_HTTP3_CLIENT = newFeatureFlag("FUN_FEATURE_FLAG_EXPERIMENTAL_HTTP3_CLIENT", .{});
    pub const FUN_FEATURE_FLAG_FORCE_IO_POOL = newFeatureFlag("FUN_FEATURE_FLAG_FORCE_IO_POOL", .{});
    pub const FUN_FEATURE_FLAG_FORCE_WINDOWS_JUNCTIONS = newFeatureFlag("FUN_FEATURE_FLAG_FORCE_WINDOWS_JUNCTIONS", .{});
    pub const FUN_INSTRUMENTS = newFeatureFlag("FUN_INSTRUMENTS", .{});
    pub const FUN_INTERNAL_FUNX_INSTALL = newFeatureFlag("FUN_INTERNAL_FUNX_INSTALL", .{});
    pub const FUN_INTERNAL_SUPPRESS_CRASH_IN_FUN_RUN = newFeatureFlag("FUN_INTERNAL_SUPPRESS_CRASH_IN_FUN_RUN", .{});
    pub const FUN_INTERNAL_SUPPRESS_CRASH_ON_NAPI_ABORT = newFeatureFlag("FUN_INTERNAL_SUPPRESS_CRASH_ON_NAPI_ABORT", .{});
    pub const FUN_INTERNAL_SUPPRESS_CRASH_ON_PROCESS_KILL_SELF = newFeatureFlag("FUN_INTERNAL_SUPPRESS_CRASH_ON_PROCESS_KILL_SELF", .{});
    pub const FUN_INTERNAL_SUPPRESS_CRASH_ON_UV_STUB = newFeatureFlag("FUN_INTERNAL_SUPPRESS_CRASH_ON_UV_STUB", .{});
    pub const FUN_FEATURE_FLAG_LAST_MODIFIED_PRETEND_304 = newFeatureFlag("FUN_FEATURE_FLAG_LAST_MODIFIED_PRETEND_304", .{});
    pub const FUN_NO_CODESIGN_MACHO_BINARY = newFeatureFlag("FUN_NO_CODESIGN_MACHO_BINARY", .{});
    pub const FUN_FEATURE_FLAG_NO_LIBDEFLATE = newFeatureFlag("FUN_FEATURE_FLAG_NO_LIBDEFLATE", .{});
    pub const NODE_NO_WARNINGS = newFeatureFlag("NODE_NO_WARNINGS", .{});
    pub const FUN_TRACE = newFeatureFlag("FUN_TRACE", .{});
};
            ...
            return struct {
                ...
                fn handleError(
                    ...
                    switch (ip.opts.deser.error_handling) {
                        .debug_warn => {
                            fun.Output.debugWarn(fmt, .{ ip.var_name, raw_env });
                            self.value.store(not_set_sentinel, .monotonic);
                            return null;
                        },
                        .not_set => {
...
fn PlatformSpecificNew(
    ...
    return struct {
        ...
        pub fn platformGet() ?ValueType {
            ...
            const platform_key: ?KeyType = if (comptime fun.Environment.isPosix)
                posix_key
        ...
        fn assertPlatformSupported() void {
            ...
            if (comptime fun.Environment.isWindows and windows_key == null) {
                @compileError(std.fmt.comptimePrint(missing_key_fmt, .{ "Windows", "Windows" }));
            } else if (comptime fun.Environment.isPosix and posix_key == null) {
