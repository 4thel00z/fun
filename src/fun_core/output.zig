...
pub export var fun_stdio_tty: [3]i32 = .{ 0, 0, 0 };
...
pub const Source = struct {
    pub const StreamType: type = brk: {
        if (Environment.isWasm) {
            ...
            // return @TypeOf(fun.deprecated.bufferedWriter(stdout.writer()));
        }
    };
    ...
    pub fn init(
        ...
        if ((comptime Environment.isDebug and use_mimalloc) and !source_set) {
            fun.mimalloc.mi_option_set(.show_errors, 1);
        }
        source_set = true;
    ...
    pub fn configureThread() void {
        ...
        fun.debugAssert(stdout_stream_set);
    ...
    pub fn isNoColor() bool {
        return fun.env_var.NO_COLOR.get();
    }

    pub fn getForceColorDepth() ?ColorDepth {
        const force_color = fun.env_var.FORCE_COLOR.get() orelse return null;
    ...
    const WindowsStdio = struct {
        ...
        pub export fn Fun__restoreWindowsStdio() callconv(.c) void {
            restore();
        }
        comptime {
            if (Environment.isWindows) {
                _ = &Fun__restoreWindowsStdio;
            }
        }
        ...
        pub fn init() void {
            ...
            if (c.GetConsoleMode(stdin, &mode) != 0) {
                ...
                //
                // In the past, Fun would set ENABLE_VIRTUAL_TERMINAL_INPUT, which was not
    ...
    pub const Stdio = struct {
        extern "c" var fun_is_stdio_null: [3]i32;

        pub fn isStderrNull() bool {
            return fun_is_stdio_null[2] == 1;
        }

        pub fn isStdoutNull() bool {
            return fun_is_stdio_null[1] == 1;
        }

        pub fn isStdinNull() bool {
            return fun_is_stdio_null[0] == 1;
        }

        pub extern "c" fn fun_initialize_process() void;
        pub fn init() void {
            fun_initialize_process();

            if (Environment.isWindows) {
                WindowsStdio.init();
            }

            const stdout = fun.sys.File.from(std.fs.File.stdout());
            const stderr = fun.sys.File.from(std.fs.File.stderr());
        ...
        pub extern "c" fn fun_restore_stdio() void;
        pub fn restore() void {
            if (Environment.isWindows) {
    ...
    fn getColorDepthOnce() void {
        ...
        const term = fun.env_var.TERM.get() orelse "";
        if (strings.eqlComptime(term, "dumb")) {
            return;
        }

        if (fun.env_var.TMUX.get() != null) {
            lazy_color_depth = .@"256";
            return;
        }

        if (fun.env_var.CI.get() != null) {
            lazy_color_depth = .@"16";
            return;
        }

        if (fun.env_var.TERM_PROGRAM.get()) |term_program| {
            const use_16m = .{
                "ghostty",
                "MacTerm",
                "WezTerm",
        ...
        if (fun.env_var.COLORTERM.get()) |color_term| {
    ...
    pub fn setInit(stdout: StreamType, stderr: StreamType) void {
        ...
        if (!stdout_stream_set) {
            ...
            if (comptime Environment.isNative) {
                const is_stdout_tty = fun_stdio_tty[1] != 0;
                if (is_stdout_tty) {
                    stdout_descriptor_type = OutputStreamDescriptor.terminal;
                }

                const is_stderr_tty = fun_stdio_tty[2] != 0;
                if (is_stderr_tty) {
...
pub inline fn isStdoutTTY() bool {
    return fun_stdio_tty[1] != 0;
}

pub inline fn isStderrTTY() bool {
    return fun_stdio_tty[2] != 0;
}

pub inline fn isStdinTTY() bool {
    return fun_stdio_tty[0] != 0;
}

pub fn isGithubAction() bool {
    if (fun.env_var.GITHUB_ACTIONS.get()) {
...
pub fn isAIAgent() bool {
    const get_is_agent = struct {
        ...
        fn evaluate() bool {
            ...
            // Claude Code.
            if (fun.env_var.CLAUDECODE.get()) {
                return true;
            }

            // Replit.
            if (fun.env_var.REPL_ID.get()) {
                return true;
            }
...
pub fn isVerbose() bool {
    ...
    return fun.env_var.RUNNER_DEBUG.get();
}
...
/// Fun automatically calls this function in Global.exit().
pub fn flush() void {
...
///   FUN_DEBUG_ALL=1
pub const LogFunction = fn (comptime fmt: string, args: anytype) callconv(fun.callconv_inline) void;
...
fn ScopedLogger(comptime tagname: []const u8, comptime visibility: Visibility) type {
    if (comptime !Environment.enable_logs) {
        return struct {
            ...
            pub fn log(comptime _: string, _: anytype) callconv(fun.callconv_inline) void {}
        };
    }
    ...
    return struct {
        ...
        fn evaluateIsVisible() void {
            if (fun.getenvZAnyCase("FUN_DEBUG_" ++ tagname)) |val| {
                really_disable.store(strings.eqlComptime(val, "0"), .monotonic);
            } else if (fun.env_var.FUN_DEBUG_ALL.get()) |val| {
                really_disable.store(!val, .monotonic);
            } else if (fun.env_var.FUN_DEBUG_QUIET_LOGS.get()) |val| {
        ...
        pub fn log(comptime fmt: string, args: anytype) void {
            ...
            if (fun.crash_handler.isPanicking()) return;
...
pub const DebugTimer = struct {
    timer: fun.DebugOnly(std.time.Timer) = undefined,
...
pub inline fn err(error_name: anytype, comptime fmt: []const u8, args: anytype) void {
    ...
    if (comptime T == fun.sys.Error or info == .pointer and info.pointer.child == fun.sys.Error) {
        const e: fun.sys.Error = error_name;
        const tag_name, const sys_errno = e.getErrorCodeTagName() orelse {
            err("unknown error", fmt, args);
            return;
        };
        if (fun.sys.coreutils_error_map.get(sys_errno)) |label| {
    ...
    const display_name, const is_comptime_name = display_name: {
        ...
        if (comptime fun.trait.isZigString(T)) {
        ...
        if (info == .enum_literal) {
            ...
            comptime fun.assert(tag.len > 0); // how?
        ...
        // enums
        if (info == .@"enum") {
            const errno: fun.sys.SystemErrno = @enumFromInt(@intFromEnum(info));
...
pub fn initScopedDebugWriterAtStartup() void {
    ...
    if (fun.env_var.FUN_DEBUG.get()) |path| {
        if (path.len > 0 and !strings.eql(path, "0") and !strings.eql(path, "false")) {
            ...
            const pid = std.fmt.allocPrint(fun.default_allocator, "{d}", .{getpid()}) catch @panic("failed to allocate path");
            defer fun.default_allocator.free(pid);

            const path_fmt = std.mem.replaceOwned(u8, fun.default_allocator, path, "{pid}", pid) catch @panic("failed to allocate path");
            defer fun.default_allocator.free(path_fmt);

            const fd: fun.FD = .fromStdFile(std.fs.cwd().createFile(path_fmt, .{
...
pub var buffered_stdin = fun.deprecated.BufferedReader(4096, File.Reader){
