...
// ── struct_hostent ─────────────────────────────────────────────────────────
pub fn hostentToJSResponse(this: *c_ares.struct_hostent, _: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime lookup_name: []const u8) fun.JSError!jsc.JSValue {
    if (comptime strings.eqlComptime(lookup_name, "cname")) {
        ...
        return fun.String.toJSArray(globalThis, &[_]fun.String{fun.String.borrowUTF8(this.h_name.?[0..fun.len(this.h_name.?)])});
    }
...
// ── hostent_with_ttls ──────────────────────────────────────────────────────
pub fn hostentWithTtlsToJSResponse(this: *c_ares.hostent_with_ttls, _: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime lookup_name: []const u8) fun.JSError!jsc.JSValue {
    if (comptime strings.eqlComptime(lookup_name, "a") or strings.eqlComptime(lookup_name, "aaaa")) {
        ...
        while (this.hostent.h_addr_list.?[count]) |addr| : (count += 1) {
            const addrString = (if (this.hostent.h_addrtype == c_ares.AF.INET6)
                fun.dns.addressToJS(&std.net.Address.initIp6(addr[0..16].*, 0, 0, 0), globalThis)
            else
                fun.dns.addressToJS(&std.net.Address.initIp4(addr[0..4].*, 0), globalThis)) catch return globalThis.throwOutOfMemoryValue();
...
// ── struct_nameinfo ────────────────────────────────────────────────────────
pub fn nameinfoToJSResponse(this: *c_ares.struct_nameinfo, _: std.mem.Allocator, globalThis: *jsc.JSGlobalObject) fun.JSError!jsc.JSValue {
    const array = try jsc.JSValue.createEmptyArray(globalThis, 2); // [node, service]

    if (this.node != null) {
        const node_len = fun.len(this.node);
    ...
    if (this.service != null) {
        const service_len = fun.len(this.service);
...
// ── AddrInfo ───────────────────────────────────────────────────────────────
pub fn addrInfoToJSArray(addr_info: *c_ares.AddrInfo, globalThis: *jsc.JSGlobalObject) fun.JSError!jsc.JSValue {
                ...
                try GetAddrInfo.Result.toJS(
                    &.{
                        .address = switch (this_node.family) {
                            c_ares.AF.INET => std.net.Address{ .in = .{ .sa = fun.cast(*const std.posix.sockaddr.in, this_node.addr.?).* } },
                            c_ares.AF.INET6 => std.net.Address{ .in6 = .{ .sa = fun.cast(*const std.posix.sockaddr.in6, this_node.addr.?).* } },
                            else => unreachable,
                        },
                        .ttl = this_node.ttl,
                    },
...
pub fn caaReplyToJSResponse(this: *c_ares.struct_ares_caa_reply, parent_allocator: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var stack = std.heap.stackFallback(2048, parent_allocator);
    var arena = fun.ArenaAllocator.init(stack.get());
    defer arena.deinit();
...
pub fn srvReplyToJSResponse(this: *c_ares.struct_ares_srv_reply, parent_allocator: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var stack = std.heap.stackFallback(2048, parent_allocator);
    var arena = fun.ArenaAllocator.init(stack.get());
    defer arena.deinit();
...
pub fn mxReplyToJSResponse(this: *c_ares.struct_ares_mx_reply, parent_allocator: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var stack = std.heap.stackFallback(2048, parent_allocator);
    var arena = fun.ArenaAllocator.init(stack.get());
    defer arena.deinit();
...
pub fn mxReplyToJS(this: *c_ares.struct_ares_mx_reply, globalThis: *jsc.JSGlobalObject, _: std.mem.Allocator) jsc.JSValue {
    ...
    const host_len = fun.len(this.host);
...
pub fn txtReplyToJSResponse(this: *c_ares.struct_ares_txt_reply, parent_allocator: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var stack = std.heap.stackFallback(2048, parent_allocator);
    var arena = fun.ArenaAllocator.init(stack.get());
    defer arena.deinit();
...
pub fn txtReplyToJS(this: *c_ares.struct_ares_txt_reply, globalThis: *jsc.JSGlobalObject, _: std.mem.Allocator) fun.JSError!jsc.JSValue {
...
pub fn txtReplyToJSForAny(this: *c_ares.struct_ares_txt_reply, _: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var count: usize = 0;
    var txt: ?*c_ares.struct_ares_txt_reply = this;
    while (txt != null) : (txt = txt.?.next) {
        count += 1;
...
pub fn naptrReplyToJSResponse(this: *c_ares.struct_ares_naptr_reply, parent_allocator: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var stack = std.heap.stackFallback(2048, parent_allocator);
    var arena = fun.ArenaAllocator.init(stack.get());
    defer arena.deinit();
...
pub fn naptrReplyToJS(this: *c_ares.struct_ares_naptr_reply, globalThis: *jsc.JSGlobalObject, _: std.mem.Allocator) jsc.JSValue {
    ...
    const flags_len = fun.len(this.flags);
    ...
    const regexp_len = fun.len(this.regexp);
    ...
    const replacement_len = fun.len(this.replacement);
...
pub fn soaReplyToJSResponse(this: *c_ares.struct_ares_soa_reply, parent_allocator: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var stack = std.heap.stackFallback(2048, parent_allocator);
    var arena = fun.ArenaAllocator.init(stack.get());
    defer arena.deinit();
...
pub fn soaReplyToJS(this: *c_ares.struct_ares_soa_reply, globalThis: *jsc.JSGlobalObject, _: std.mem.Allocator) jsc.JSValue {
    ...
    const nsname_len = fun.len(this.nsname);
    ...
    const hostmaster_len = fun.len(this.hostmaster);
...
// ── struct_any_reply ───────────────────────────────────────────────────────
pub fn anyReplyToJSResponse(this: *c_ares.struct_any_reply, parent_allocator: std.mem.Allocator, globalThis: *jsc.JSGlobalObject, comptime _: []const u8) fun.JSError!jsc.JSValue {
    var stack = std.heap.stackFallback(2048, parent_allocator);
    var arena = fun.ArenaAllocator.init(stack.get());
    defer arena.deinit();
...
fn anyReplyAppend(globalThis: *jsc.JSGlobalObject, array: jsc.JSValue, i: *u32, response: jsc.JSValue, comptime lookup_name: []const u8) fun.JSError!void {
    const transformed = if (response.isString())
        (try jsc.JSObject.create(.{
            .value = response,
        }, globalThis)).toJS()
    else blk: {
        fun.assert(response.isObject());
        break :blk response;
    };
    ...
    transformed.put(globalThis, "type", try fun.String.ascii(&upper).toJS(globalThis));
    try array.putIndex(globalThis, i.*, transformed);
    i.* += 1;
}

fn anyReplyAppendAll(globalThis: *jsc.JSGlobalObject, allocator: std.mem.Allocator, array: jsc.JSValue, i: *u32, reply: anytype, comptime lookup_name: []const u8) fun.JSError!void {
...
pub fn anyReplyToJS(this: *c_ares.struct_any_reply, globalThis: *jsc.JSGlobalObject, allocator: std.mem.Allocator) fun.JSError!jsc.JSValue {
...
pub const ErrorDeferred = struct {
    ...
    pub fn init(errno: c_ares.Error, syscall: []const u8, hostname: ?fun.String, promise: jsc.JSPromise.Strong) *ErrorDeferred {
    ...
    pub fn reject(this: *ErrorDeferred, globalThis: *jsc.JSGlobalObject) fun.JSError!void {
        const system_error = jsc.SystemError{
            .errno = @intFromEnum(this.errno),
            .code = fun.String.static(this.errno.code()),
            .message = if (this.hostname) |hostname|
                fun.handleOom(fun.String.createFormat("{s} {s} {f}", .{ this.syscall, this.errno.code()[4..], hostname }))
            else
                fun.handleOom(fun.String.createFormat("{s} {s}", .{ this.syscall, this.errno.code()[4..] })),
            .syscall = fun.String.cloneUTF8(this.syscall),
            .hostname = this.hostname orelse fun.String.empty,
        };

        const instance = system_error.toErrorInstanceWithAsyncStack(globalThis, this.promise.get());
        instance.put(globalThis, "name", try fun.String.static("DNSException").toJS(globalThis));
    ...
    pub fn rejectLater(this: *ErrorDeferred, globalThis: *jsc.JSGlobalObject) void {
        const Context = struct {
            ...
            pub fn callback(context: *@This()) fun.JSError!void {
                defer fun.default_allocator.destroy(context);
        ...
        globalThis.funVM().enqueueTask(jsc.ManagedTask.New(Context, Context.callback).init(context));
    }
...
pub fn errorToDeferred(this: c_ares.Error, syscall: []const u8, hostname: ?[]const u8, promise: *jsc.JSPromise.Strong) *ErrorDeferred {
    const host_string: ?fun.String = if (hostname) |host|
        fun.String.cloneUTF8(host)
    else
        null;
    defer promise.* = .{};
...
pub fn errorToJSWithSyscall(this: c_ares.Error, globalThis: *jsc.JSGlobalObject, comptime syscall: [:0]const u8) fun.JSError!jsc.JSValue {
    const instance = (jsc.SystemError{
        .errno = @intFromEnum(this),
        .code = fun.String.static(this.code()[4..]),
        .syscall = fun.String.static(syscall),
        .message = fun.handleOom(fun.String.createFormat("{s} {s}", .{ syscall, this.code()[4..] })),
    }).toErrorInstance(globalThis);
    instance.put(globalThis, "name", try fun.String.static("DNSException").toJS(globalThis));
    return instance;
}

pub fn errorToJSWithSyscallAndHostname(this: c_ares.Error, globalThis: *jsc.JSGlobalObject, comptime syscall: [:0]const u8, hostname: []const u8) fun.JSError!jsc.JSValue {
    const instance = (jsc.SystemError{
        .errno = @intFromEnum(this),
        .code = fun.String.static(this.code()[4..]),
        .message = fun.handleOom(fun.String.createFormat("{s} {s} {s}", .{ syscall, this.code()[4..], hostname })),
        .syscall = fun.String.static(syscall),
        .hostname = fun.String.cloneUTF8(hostname),
    }).toErrorInstance(globalThis);
    instance.put(globalThis, "name", try fun.String.static("DNSException").toJS(globalThis));
    return instance;
}
...
comptime {
    const Fun__canonicalizeIP = jsc.toJSHostFn(Fun__canonicalizeIP_);
    @export(&Fun__canonicalizeIP, .{ .name = "Fun__canonicalizeIP" });
}
pub fn Fun__canonicalizeIP_(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
    jsc.markBinding(@src());
    ...
    const INET6_ADDRSTRLEN = if (comptime fun.Environment.isWindows) 65 else 46;

    const addr_arg = try arguments[0].toSlice(globalThis, fun.default_allocator);
    ...
    const slice = fun.sliceTo(ip_addr[0..], 0);
    if (fun.strings.eql(addr_str, slice)) {
        return arguments[0];
    }
