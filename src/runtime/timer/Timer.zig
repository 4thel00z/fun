...
pub const All = struct {
    ...
    fn insertLockHeld(this: *All, timer: *EventLoopTimer) void {
        if (Environment.ci_assert) fun.assert(this.lock.tryLock() == false);
    ...
    fn removeLockHeld(this: *All, timer: *EventLoopTimer) void {
        ...
        switch (timer.in_heap) {
            .none => if (Environment.ci_assert) fun.assert(false), // can't remove a timer that was not inserted
        ...
        if (loop.shouldEnableDateHeaderTimer()) {
            if (!this.isDateTimerActive()) {
                this.date_header_timer.enable(
                    vm,
                    // Be careful to avoid adding extra calls to fun.timespec.now()
                    // when it's not needed.
                    &fun.timespec.now(.allow_mocked_time),
                );
            }
        } else {
    ...
    export fn Fun__internal_drainTimers(vm: *VirtualMachine) callconv(.c) void {
        drainTimers(&vm.timer, vm);
    }

    comptime {
        _ = &Fun__internal_drainTimers;
    }
    ...
    fn warnInvalidCountdown(globalThis: *JSGlobalObject, countdown: f64, warning_type: TimeoutWarning) void {
        ...
        var warning_string = switch (warning_type) {
            ...
            .TimeoutNegativeWarning => if (std.math.isFinite(countdown))
                ...
                fun.String.ascii("-Infinity is a negative number" ++ suffix),
            // std.fmt gives us "nan" but Node.js wants "NaN".
            .TimeoutNaNWarning => nan_warning: {
                assert(std.math.isNan(countdown));
                break :nan_warning fun.String.ascii("NaN is not a number" ++ suffix);
            },
        };
        var warning_type_string = fun.String.createAtomIfPossible(@tagName(warning_type));
    ...
    const CountdownOverflowBehavior = enum(u8) {
        ...
        one_ms,
        /// If the countdown overflows the range of int32_t, clamp to the nearest value within the range. Behavior of `Fun.sleep`.
        clamp,
    };
    ...
    pub fn sleep(
        ...
        jsc.markBinding(@src());
        fun.debugAssert(promise != .zero and countdown != .zero);
    ...
    pub fn setImmediate(
        ...
        jsc.markBinding(@src());
        fun.debugAssert(callback != .zero and arguments != .zero);
        const vm = global.funVM();
        const id = vm.timer.last_id;
        vm.timer.last_id +%= 1;
    ...
    pub fn setTimeout(
        ...
        jsc.markBinding(@src());
        fun.debugAssert(callback != .zero and arguments != .zero and countdown != .zero);
        const vm = global.funVM();
        const id = vm.timer.last_id;
        vm.timer.last_id +%= 1;

        const wrapped_callback = callback.withAsyncContextIfNeeded(global);
        const countdown_int = try global.funVM().timer.jsValueToCountdown(global, countdown, .one_ms, true);
    ...
    fn removeTimerById(this: *All, id: i32) ?*TimeoutObject {
        if (this.maps.setTimeout.fetchSwapRemove(id)) |entry| {
            fun.assert(entry.value.tag == .TimeoutObject);
    ...
    pub fn clearTimer(timer_id_value: JSValue, globalThis: *JSGlobalObject, kind: Kind) JSError!void {
        ...
        const timer: *TimerObjectInternals = brk: {
            if (timer_id_value.isInt32()) {
                ...
                const string = try timer_id_value.toFunString(globalThis);
                defer string.deref();
                ...
                const parsed = parsed: {
                    ...
                    switch (string.encoding()) {
                        ...
                        inline else => |encoding| {
                            ...
                            const slice = @field(fun.String, @tagName(encoding))(string);
                            for (slice, 0..) |c, i| {
                                if (c < '0' or c > '9') {
    ...
    comptime {
        @export(&jsc.host_fn.wrap3(setImmediate), .{ .name = "Fun__Timer__setImmediate" });
        @export(&jsc.host_fn.wrap3(sleep), .{ .name = "Fun__Timer__sleep" });
        @export(&jsc.host_fn.wrap4(setTimeout), .{ .name = "Fun__Timer__setTimeout" });
        @export(&jsc.host_fn.wrap4(setInterval), .{ .name = "Fun__Timer__setInterval" });
        @export(&jsc.host_fn.wrap2(clearImmediate), .{ .name = "Fun__Timer__clearImmediate" });
        @export(&jsc.host_fn.wrap2(clearTimeout), .{ .name = "Fun__Timer__clearTimeout" });
        @export(&jsc.host_fn.wrap2(clearInterval), .{ .name = "Fun__Timer__clearInterval" });
        @export(&getNextID, .{ .name = "Fun__Timer__getNextID" });
    }
};
...
pub const internal_bindings = struct {
    ...
    /// timers implementation uses (for Fun this is `fun.timespec.now()`), so the tests end up
    /// thinking that the timing is wrong (this also happens when I run the modified test in
    /// Node.js). So the best course of action is for Fun to also expose a function that reveals the
    /// clock that is used to schedule timers.
    pub fn timerClockMs(globalThis: *jsc.JSGlobalObject, callFrame: *jsc.CallFrame) fun.JSError!JSValue {
        _ = globalThis;
        _ = callFrame;
