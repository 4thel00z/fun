...
extern fn fun_coregraphics_encode(
    rgba: [*]const u8,
    width: u32,
    height: u32,
    format: i32,
...
pub fn decode(bytes: []const u8, max_pixels: u64) BackendError!codecs.Decoded {
    ...
    // fun.default_allocator like every other decode path.
    switch (fun_coregraphics_decode(bytes.ptr, bytes.len, max_pixels, &w, &h, null)) {
        CG_OK => {},
    ...
    switch (fun_coregraphics_decode(bytes.ptr, bytes.len, max_pixels, &w, &h, out.ptr)) {
        CG_OK => {},
        else => |rc| return mapErr(rc),
    }
...
pub fn encode(rgba: []const u8, width: u32, height: u32, opts: codecs.EncodeOptions) BackendError![]u8 {
    ...
    switch (fun_coregraphics_encode(rgba.ptr, width, height, fmt, opts.quality, null, &len)) {
        CG_OK => {},
        else => |rc| return mapErr(rc),
    }
    const out = try fun.default_allocator.alloc(u8, len);
    errdefer fun.default_allocator.free(out);
    // Phase 2: copy out and release the CFData.
    switch (fun_coregraphics_encode(rgba.ptr, width, height, fmt, opts.quality, out.ptr, &len)) {
        CG_OK => {},
        else => |rc| return mapErr(rc),
    }
    return out[0..len];
...
extern fn fun_coregraphics_scale(src: [*]const u8, sw: u32, sh: u32, dst: [*]u8, dw: u32, dh: u32) i32;
extern fn fun_coregraphics_rotate90(src: [*]const u8, w: u32, h: u32, dst: [*]u8, quarters: u32) i32;
extern fn fun_coregraphics_reflect(src: [*]const u8, w: u32, h: u32, dst: [*]u8, horizontal: i32) i32;
...
pub fn scale(src: []const u8, sw: u32, sh: u32, dw: u32, dh: u32, filter: codecs.Filter) BackendError![]u8 {
    ...
    errdefer fun.default_allocator.free(out);
    if (fun_coregraphics_scale(src.ptr, sw, sh, out.ptr, dw, dh) != CG_OK)
...
pub fn rotate(src: []const u8, w: u32, h: u32, quarters: u32) BackendError![]u8 {
    const out = try fun.default_allocator.alloc(u8, @as(usize, w) * h * 4);
    errdefer fun.default_allocator.free(out);
    if (fun_coregraphics_rotate90(src.ptr, w, h, out.ptr, quarters) != CG_OK)
        return error.BackendUnavailable;
    return out;
}
...
pub fn flip(src: []const u8, w: u32, h: u32, horizontal: bool) BackendError![]u8 {
    const out = try fun.default_allocator.alloc(u8, @as(usize, w) * h * 4);
    errdefer fun.default_allocator.free(out);
    if (fun_coregraphics_reflect(src.ptr, w, h, out.ptr, @intFromBool(horizontal)) != CG_OK)
        return error.BackendUnavailable;
    return out;
}
...
// the static `Fun.Image.fromClipboard()` accessor calls this synchronously
// before constructing the Image — the heavy decode still goes to WorkPool).

extern fn fun_coregraphics_clipboard(out: ?[*]u8, out_len: *usize, probe_only: i32) i32;

/// `null` ⇔ no image on the pasteboard. Returned bytes are an opaque container
/// (PNG/TIFF/HEIC/…); feed straight to `new Fun.Image(…)`.
pub fn clipboard() error{ BackendUnavailable, OutOfMemory }!?[]u8 {
    var len: usize = 0;
    if (fun_coregraphics_clipboard(null, &len, 0) != CG_OK) return error.BackendUnavailable;
    if (len == 0) return null;
    const out = try fun.default_allocator.alloc(u8, len);
    errdefer fun.default_allocator.free(out);
    if (fun_coregraphics_clipboard(out.ptr, &len, 0) != CG_OK) return error.BackendUnavailable;
    return out[0..len];
}

pub fn hasClipboardImage() bool {
    var len: usize = 0;
    return fun_coregraphics_clipboard(null, &len, 1) == CG_OK and len > 0;
}

extern fn fun_coregraphics_clipboard_change_count() i64;
pub const clipboardChangeCount = fun_coregraphics_clipboard_change_count;
