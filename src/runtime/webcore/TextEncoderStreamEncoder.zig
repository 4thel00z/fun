...
pub const new = fun.TrivialNew(TextEncoderStreamEncoder);

pub fn finalize(this: *TextEncoderStreamEncoder) void {
    fun.destroy(this);
}

pub fn constructor(_: *JSGlobalObject, _: *jsc.CallFrame) fun.JSError!*TextEncoderStreamEncoder {
    return TextEncoderStreamEncoder.new(.{});
}

pub fn encode(this: *TextEncoderStreamEncoder, globalObject: *jsc.JSGlobalObject, callFrame: *jsc.CallFrame) fun.JSError!JSValue {
...
fn encodeLatin1(this: *TextEncoderStreamEncoder, globalObject: *JSGlobalObject, input: []const u8) JSValue {
    ...
    //
    //
    var buffer = std.array_list.Managed(u8).initCapacity(fun.default_allocator, input.len + prepend_replacement_len) catch {
    ...
    if (comptime Environment.isDebug) {
        ...
        fun.debugAssert(buffer.items.len == (fun.simdutf.length.utf8.from.latin1(input) + prepend_replacement_len));
    }
...
fn encodeUTF16(this: *TextEncoderStreamEncoder, globalObject: *JSGlobalObject, input: []const u16) JSValue {
    log("encodeUTF16: \"{f}\"", .{fun.fmt.utf16(input)});
    ...
    const prepend: ?Prepend = prepend: {
        if (this.pending_lead_surrogate) |lead| {
            ...
            if (strings.u16IsTrail(maybe_trail)) {
                ...
                // a valid lead.
                fun.debugAssert(!converted.fail);
    ...
    const length = fun.simdutf.length.utf8.from.utf16.le(remain);
    ...
    const result = fun.simdutf.convert.utf16.to.utf8.with_errors.le(remain, buf.unusedCapacitySlice());

    switch (result.status) {
        else => {
            // Slow path: there was invalid UTF-16, so we need to convert it without simdutf.
            const lead_surrogate = strings.toUTF8ListWithTypeFun(&buf, remain, true) catch {
                buf.deinit();
                return globalObject.throwOutOfMemoryValue();
            };
...
pub fn flush(this: *TextEncoderStreamEncoder, globalObject: *JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
