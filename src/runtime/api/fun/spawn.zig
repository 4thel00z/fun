pub const FunSpawn = struct {
    ...
    pub const Actions = struct {
        ...
        pub fn deinit(self: *Actions) void {
            if (self.chdir_buf) |buf| {
                fun.default_allocator.free(fun.span(buf));
            }

            for (self.actions.items) |*action| {
                action.deinit(fun.default_allocator);
            }

            self.actions.deinit(fun.default_allocator);
        }

        pub fn open(self: *Actions, fd: fun.FD, path: []const u8, flags: u32, mode: i32) !void {
            const posix_path = try toPosixPath(path);

            return self.openZ(fd, &posix_path, flags, mode);
        }

        pub fn openZ(self: *Actions, fd: fun.FD, path: [*:0]const u8, flags: u32, mode: i32) !void {
            try self.actions.append(fun.default_allocator, .{
                .kind = .open,
                .path = (try fun.default_allocator.dupeZ(u8, fun.span(path))).ptr,
                .flags = @intCast(flags),
                .mode = @intCast(mode),
                .fds = .{ fd.native(), 0 },
            });
        ...
        pub fn close(self: *Actions, fd: fun.FD) !void {
            try self.actions.append(fun.default_allocator, .{
                .kind = .close,
                .fds = .{ fd.native(), 0 },
            });
        ...
        pub fn dup2(self: *Actions, fd: fun.FD, newfd: fun.FD) !void {
            try self.actions.append(fun.default_allocator, .{
                .kind = .dup2,
        ...
        pub fn inherit(self: *Actions, fd: fun.FD) !void {
            try self.dup2(fd, fd);
        }
        ...
        pub fn chdir(self: *Actions, path: []const u8) !void {
            ...
            self.chdir_buf = (try fun.default_allocator.dupeZ(u8, path)).ptr;
        }
    };
    ...
    pub const Attr = struct {
        ...
        pub fn set(self: *Attr, flags: u16) !void {
            self.flags = flags;
            // FreeBSD's <spawn.h> has no POSIX_SPAWN_SETSID; fun-spawn.cpp
            ...
            if (comptime @hasDecl(fun.c, "POSIX_SPAWN_SETSID")) {
                self.detached = (flags & fun.c.POSIX_SPAWN_SETSID) != 0;
            }
        }
...
pub const PosixSpawn = struct {
    ...
    pub const PosixSpawnActions = struct {
        ...
        pub fn open(self: *PosixSpawnActions, fd: fun.FD, path: []const u8, flags: u32, mode: mode_t) !void {
            const posix_path = try toPosixPath(path);
            return self.openZ(fd, &posix_path, flags, mode);
        }

        pub fn openZ(self: *PosixSpawnActions, fd: fun.FD, path: [*:0]const u8, flags: u32, mode: mode_t) !void {
        ...
        pub fn close(self: *PosixSpawnActions, fd: fun.FD) !void {
        ...
        pub fn dup2(self: *PosixSpawnActions, fd: fun.FD, newfd: fun.FD) !void {
            if (fd == newfd) {
        ...
        pub fn inherit(self: *PosixSpawnActions, fd: fun.FD) !void {
    ...
    // Use FunSpawn types on POSIX (both Linux and macOS) for PTY support via posix_spawn_fun.
    // Windows uses different spawn mechanisms.
    pub const Actions = if (Environment.isPosix) FunSpawn.Actions else PosixSpawnActions;
    pub const Attr = if (Environment.isPosix) FunSpawn.Attr else PosixSpawnAttr;

    /// Used for Linux spawns and macOS PTY spawns via posix_spawn_fun.
    const FunSpawnRequest = extern struct {
        chdir_buf: ?[*:0]u8 = null,
        detached: bool = false,
        new_process_group: bool = false,
        actions: ActionsList = .{},
        ...
        const ActionsList = extern struct {
            ptr: ?[*]const FunSpawn.Action = null,
            len: usize = 0,
        };

        extern fn posix_spawn_fun(
            pid: *c_int,
            path: [*:0]const u8,
            request: *const FunSpawnRequest,
        ...
        pub fn spawn(
            ...
            const rc = posix_spawn_fun(&pid, path, &req, argv, envp);
            if (comptime fun.Environment.allow_assert)
                fun.sys.syslog("posix_spawn_fun({s}) = {d} ({d})", .{
                    fun.span(argv[0] orelse ""),
                    rc,
                    pid,
                });
            ...
            return Maybe(pid_t){
                .err = .{
                    .errno = @as(fun.sys.Error.Int, @truncate(@intFromEnum(@as(std.c.E, @enumFromInt(rc))))),
                    .syscall = .posix_spawn,
                    .path = fun.span(argv[0] orelse ""),
                },
            };
        }
    };
    ...
    pub fn spawnZ(
        ...
        const use_fun_spawn = Environment.isLinux or Environment.isFreeBSD or (Environment.isMac and pty_slave_fd >= 0);

        if (use_fun_spawn) {
            return FunSpawnRequest.spawn(
                path,
                .{
        ...
        // Need to convert FunSpawn.Actions to PosixSpawnActions for system posix_spawn
        if (comptime Environment.isMac) {
            ...
            // Pass through all flags from the FunSpawn.Attr
            if (attr) |a| {
                var flags = a.flags;
                if (a.new_process_group) {
                    flags |= fun.c.POSIX_SPAWN_SETPGROUP;
            ...
            if (actions) |act| {
                for (act.actions.items) |action| {
                    switch (action.kind) {
                        .close => posix_actions.close(fun.FD.fromNative(action.fds[0])) catch |err| {
                            if (comptime fun.Environment.allow_assert) {
                                fun.sys.syslog("posix_spawn_file_actions_addclose({d}) failed: {s}", .{ action.fds[0], @errorName(err) });
                            }
                        },
                        .dup2 => posix_actions.dup2(fun.FD.fromNative(action.fds[0]), fun.FD.fromNative(action.fds[1])) catch |err| {
                            if (comptime fun.Environment.allow_assert) {
                                fun.sys.syslog("posix_spawn_file_actions_adddup2({d}, {d}) failed: {s}", .{ action.fds[0], action.fds[1], @errorName(err) });
                            }
                        },
                        .open => posix_actions.openZ(fun.FD.fromNative(action.fds[0]), action.path.?, @intCast(action.flags), @intCast(action.mode)) catch |err| {
                            if (comptime fun.Environment.allow_assert) {
                                fun.sys.syslog("posix_spawn_file_actions_addopen({d}, {s}, {d}, {d}) failed: {s}", .{ action.fds[0], action.path.?, action.flags, action.mode, @errorName(err) });
                            }
                        },
                        .none => {},
                    }
                ...
                if (act.chdir_buf) |chdir_path| {
                    posix_actions.chdir(fun.span(chdir_path)) catch |err| {
                        if (comptime fun.Environment.allow_assert) {
                            fun.sys.syslog("posix_spawn_file_actions_addchdir({s}) failed: {s}", .{ chdir_path, @errorName(err) });
                        }
                    };
                }
            }
            ...
            if (comptime fun.Environment.allow_assert)
                fun.sys.syslog("posix_spawn({s}) = {d} ({d})", .{
                    path,
                    rc,
                    pid,
                });
        ...
        // That is why fun.sys.getErrno() is not used here, since that checks for -1.
        if (rc == 0) {
            return Maybe(pid_t){ .result = pid };
        }
    ...
    pub fn waitpid(pid: pid_t, flags: u32) Maybe(WaitPidResult) {
        ...
        while (true) {
            ...
            switch (errno(rc)) {
                ...
                else => return fun.sys.Maybe(WaitPidResult).errnoSys(rc, .waitpid).?,
            }
        }
    }
