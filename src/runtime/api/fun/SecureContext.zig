...
/// `FunSocketContextOptions.digest()` — exactly the fields that reach
...
pub fn constructor(global: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!*SecureContext {
    const args = callframe.arguments();
    const opts = if (args.len > 0) args[0] else .js_undefined;

    var config = (try SSLConfig.fromJS(global.funVM(), global, opts)) orelse SSLConfig.zero;
    defer config.deinit();
...
pub fn create(global: *jsc.JSGlobalObject, config: *const SSLConfig) fun.JSError!*SecureContext {
    const ctx_opts = config.asUSockets();
    return createWithDigest(global, ctx_opts, ctx_opts.digest());
}

fn createWithDigest(global: *jsc.JSGlobalObject, ctx_opts: uws.SocketContext.FunSocketContextOptions, d: [32]u8) fun.JSError!*SecureContext {
    var err: uws.create_fun_socket_error_t = .none;
    const ctx = global.funVM().rareData().sslCtxCache().getOrCreateDigest(ctx_opts, d, &err) orelse {
        ...
        if (err == .none) {
            ...
            if (code != 0) return global.throwValue(fun.BoringSSL.ERR_toJS(global, code));
            return global.throw("Failed to create SSL context", .{});
        }
        return global.throwValue(err.toJS(global));
    };
    return fun.new(SecureContext, .{
        .ctx = ctx,
        .digest = d,
...
pub fn intern(global: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
    const args = callframe.arguments();
    const opts = if (args.len > 0) args[0] else .js_undefined;
    ...
    const cached = cpp.Fun__SecureContextCache__get(global, key);
    if (cached != .zero) {
        if (fromJS(cached)) |existing| {
            ...
            if (fun.strings.eqlLong(&existing.digest, &d, false)) {
                return cached;
            }
        }
    }
    ...
    cpp.Fun__SecureContextCache__set(global, key, value);
    return value;
}
...
/// Exposed via `fun:internal-for-testing` so churn tests can assert
/// `SSL_CTX_new` was called O(1) times, not O(connections).
pub fn jsLiveCount(_: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!jsc.JSValue {
...
const cpp = struct {
    pub extern fn Fun__SecureContextCache__get(*jsc.JSGlobalObject, u64) jsc.JSValue;
    pub extern fn Fun__SecureContextCache__set(*jsc.JSGlobalObject, u64, jsc.JSValue) void;
};
