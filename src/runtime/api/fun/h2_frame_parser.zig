...
const ValidResponsePseudoHeaders = fun.ComptimeStringMap(void, .{
    .{":status"},
});

const ValidRequestPseudoHeaders = fun.ComptimeStringMap(void, .{
    .{":method"},
    .{":authority"},
    .{":scheme"},
    .{":path"},
...
const SingleValueHeaders = fun.ComptimeStringMap(void, .{
    .{":status"},
    .{":method"},
    .{":authority"},
    .{":scheme"},
...
pub fn jsGetUnpackedSettings(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
    jsc.markBinding(@src());
...
pub fn jsAssertSettings(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
    const args_list = callframe.arguments_old(1);
    if (args_list.len < 1) {
...
pub fn jsGetPackedSettings(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
    var settings: FullSettingsPayload = .{};
    const args_list = callframe.arguments_old(1);
...
const Handlers = struct {
    ...
    pub fn fromJS(globalObject: *jsc.JSGlobalObject, opts: jsc.JSValue, thisValue: jsc.JSValue) fun.JSError!Handlers {
...
pub const H2FrameParser = struct {
    ...
    const H2FrameParserHiveAllocator = fun.HiveArray(H2FrameParser, 256).Fallback;
    ...
    const Stream = struct {
        ...
        pub const new = fun.TrivialNew(Stream);
        ...
        const SignalRef = struct {
            ...
            pub const new = fun.TrivialNew(SignalRef);
            ...
            pub fn abortListener(this: *SignalRef, reason: JSValue) void {
                ...
                if (stream.state != .CLOSED) {
                    this.parser.abortStream(stream, Fun__wrapAbortError(this.parser.globalThis, reason));
                }
            }
        ...
        const PendingQueue = struct {
            ...
            pub fn enqueue(self: *PendingQueue, value: PendingFrame, allocator: Allocator) void {
                fun.handleOom(self.data.append(allocator, value));
                self.len += 1;
                        ...
                        if (frame.len == 0) {
                            ...
                            if (max_size < frame_slice.len) {
                                ...
                                if (padding != 0) {
                                    ...
                                    fun.memmove(buffer[1..][0..able_to_send.len], able_to_send);
                                    buffer[0] = padding;
                                ...
                                if (padding != 0) {
                                    var buffer = shared_request_buffer[0..];
                                    fun.memmove(buffer[1..][0..frame_slice.len], frame_slice);
                                    buffer[0] = padding;
                                    break :brk (writer.write(buffer[0..payload_size]) catch 0) != 0;
                                } else {
        ...
        pub fn queueFrame(this: *Stream, client: *H2FrameParser, bytes: []const u8, callback: jsc.JSValue, end_stream: bool) void {
            ...
            if (this.dataFrameQueue.peekLast()) |last_frame| {
                ...
                if (last_frame.len == 0) {
                    ...
                    last_frame.buffer = fun.handleOom(client.allocator.alloc(u8, MAX_PAYLOAD_SIZE_WITHOUT_FRAME));
                }
            ...
            const frame: PendingFrame = .{
                ...
                .buffer = if (bytes.len == 0) "" else fun.handleOom(client.allocator.alloc(u8, MAX_PAYLOAD_SIZE_WITHOUT_FRAME)),
        ...
        if (buffer.len > 0) {
            {
                ...
                if (written < buffer.len) {
                    ...
                    _ = fun.handleOom(this.writeBuffer.write(this.allocator, bytes));
            ...
            this.writeBuffer.len = 0;
            {
                ...
                if (written < bytes.len) {
                    ...
                    _ = fun.handleOom(this.writeBuffer.write(this.allocator, pending));
    ...
    fn registerAutoFlush(this: *H2FrameParser) void {
        ...
        this.ref();
        AutoFlusher.registerDeferredMicrotaskWithTypeUnchecked(H2FrameParser, this, this.globalThis.funVM());
    }

    fn unregisterAutoFlush(this: *H2FrameParser) void {
        if (!this.auto_flusher.registered) return;
        AutoFlusher.unregisterDeferredMicrotaskWithTypeUnchecked(H2FrameParser, this, this.globalThis.funVM());
        this.deref();
    }
    ...
    pub fn handleIncommingPayload(this: *H2FrameParser, data: []const u8, streamIdentifier: u32) ?Payload {
        ...
        if (this.remainingLength > 0) {
            // buffer more data
            _ = fun.handleOom(this.readBuffer.appendSlice(payload));
    ...
    pub fn decodeHeaderBlock(this: *H2FrameParser, payload: []const u8, stream: *Stream, flags: u8) fun.JSError!?*Stream {
        ...
        while (true) {
            ...
            if (getHTTP2CommonString(globalObject, header.well_know)) |js_header_name| {
                ...
                try headers.push(globalObject, try fun.String.createUTF8ForJS(globalObject, header.value));
                ...
                const js_header_name = try fun.String.createUTF8ForJS(globalObject, header.name);
                const js_header_value = try fun.String.createUTF8ForJS(globalObject, header.value);
    ...
    fn stringOrEmptyToJS(this: *H2FrameParser, payload: []const u8) fun.JSError!jsc.JSValue {
        if (payload.len == 0) {
            return fun.String.empty.toJS(this.handlers.globalObject);
        }
        return fun.String.createUTF8ForJS(this.handlers.globalObject, payload);
    }

    pub fn handleOriginFrame(this: *H2FrameParser, frame: FrameHeader, data: []const u8, _: ?*Stream) fun.JSError!usize {
    ...
    pub fn handleAltsvcFrame(this: *H2FrameParser, frame: FrameHeader, data: []const u8, stream_: ?*Stream) fun.JSError!usize {
    ...
    pub fn handleContinuationFrame(this: *H2FrameParser, frame: FrameHeader, data: []const u8, stream_: ?*Stream) fun.JSError!usize {
    ...
    pub fn handleHeadersFrame(this: *H2FrameParser, frame: FrameHeader, data: []const u8, stream_: ?*Stream) fun.JSError!usize {
    ...
    fn handleReceivedStreamID(this: *H2FrameParser, streamIdentifier: u32) ?*Stream {
        ...
        fun.handleOom(this.streams.put(streamIdentifier, stream));
    ...
    fn readBytes(this: *H2FrameParser, bytes: []const u8) fun.JSError!usize {
        log("read {}", .{bytes.len});
        ...
        if (buffered_data > 0) {
            ...
            if (total < FrameHeader.byteSize) {
                // buffer more data
                _ = fun.handleOom(this.readBuffer.appendSlice(bytes));
                this.globalThis.vm().reportExtraMemory(bytes.len);
        ...
        if (bytes.len < FrameHeader.byteSize) {
            ...
            fun.handleOom(this.readBuffer.appendSlice(bytes));
            this.globalThis.vm().reportExtraMemory(bytes.len);
    ...
    pub fn setEncoding(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn loadSettingsFromJSValue(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, options: jsc.JSValue) fun.JSError!void {
        ...
        if (try options.get(globalObject, "customSettings")) |customSettings| {
            if (!customSettings.isUndefined()) {
                ...
                while (try iter.next()) |prop_name| {
                    ...
                    const setting_id_str = prop_name.toUTF8(fun.default_allocator);
    ...
    pub fn updateSettings(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn setLocalWindowSize(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn getCurrentState(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
    ...
    pub fn goaway(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
    ...
    pub fn ping(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn origin(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        ...
        if (origin_arg.isString()) {
            const origin_string = try origin_arg.toSlice(globalObject, fun.default_allocator);
            ...
            while (try value_iter.next()) |item| {
                ...
                const origin_string = try item.toSlice(globalObject, fun.default_allocator);
                defer origin_string.deinit();
                const slice = origin_string.slice();
    ...
    pub fn altsvc(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        var origin_slice: ?fun.String.Slice = null;
        var value_slice: ?fun.String.Slice = null;
        defer {
        ...
        if (!origin_string.isEmptyOrUndefinedOrNull()) {
            ...
            origin_slice = try origin_string.toSlice(globalObject, fun.default_allocator);
        ...
        if (!value_string.isEmptyOrUndefinedOrNull()) {
            ...
            value_slice = try value_string.toSlice(globalObject, fun.default_allocator);
    ...
    pub fn getEndAfterHeaders(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn isStreamAborted(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
            return globalObject.throw("Expected stream argument", .{});
    ...
    pub fn getStreamState(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
            return globalObject.throw("Expected stream argument", .{});
    ...
    pub fn setStreamPriority(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
    ...
    pub fn rstStream(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        log("rstStream", .{});
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(2);
        if (args_list.len < 2) {
    ...
    const MemoryWriter = struct {
        ...
        pub fn write(this: *MemoryWriter, data: []const u8) !usize {
            ...
            fun.debugAssert(pending.len >= data.len);
    ...
    // get memory in bytes
    pub fn getBufferSize(this: *H2FrameParser, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        ...
        if (payload.len == 0) {
            ...
            while (offset < payload.len) {
                ...
                if (this.hasBackpressure() or this.outboundQueueSize > 0 or is_flow_control_limited) {
                    ...
                    if (padding != 0) {
                        var buffer = shared_request_buffer[0..];
                        fun.memmove(buffer[1..][0..slice.len], slice);
                        buffer[0] = padding;
    ...
    pub fn noTrailers(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    fn toValidHeaderName(in: []const u8, out: []u8) ![]const u8 {
        ...
        fun.assert(out.len >= in.len);
        // lets validate and convert to lowercase in one pass
        begin: while (true) {
            for (in_slice, 0..) |c, i| {
                switch (c) {
                    'A'...'Z' => {
                        fun.copy(u8, out_slice, in_slice[0..i]);
            ...
            if (any) fun.copy(u8, out_slice, in_slice);
            break :begin;
        }
    ...
    pub fn sendTrailers(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(3);
        if (args_list.len < 3) {
            return globalObject.throw("Expected stream, headers and sensitiveHeaders arguments", .{});
        ...
        var buf_fallback = fun.allocators.BufferFallbackAllocator.init(&shared_request_buffer, fun.default_allocator);
        ...
        while (try iter.next()) |header_name| {
            ...
            const name_slice = header_name.toUTF8(fun.default_allocator);
            ...
            if (js_value.jsType().isArray()) {
                // https://github.com/underdoc-org/fun/issues/8940
                ...
                while (try value_iter.next()) |item| {
                    ...
                    const value_slice = value_str.toSlice(globalObject, fun.default_allocator);
    ...
    pub fn writeStream(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
    ...
    pub fn setNextStreamID(this: *H2FrameParser, _: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments();
        fun.debugAssert(args_list.len >= 1);
        const stream_id_arg = args_list.ptr[0];
        fun.debugAssert(stream_id_arg.isNumber());
    ...
    pub fn hasNativeRead(this: *H2FrameParser, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
        return jsc.JSValue.jsBoolean(this.native_socket == .tcp or this.native_socket == .tls);
    }

    pub fn getNextStream(this: *H2FrameParser, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
    ...
    pub fn getStreamContext(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn setStreamContext(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(2);
        if (args_list.len < 2) {
    ...
    pub fn forEachStream(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        jsc.markBinding(@src());
    ...
    pub fn emitAbortToAllStreams(this: *H2FrameParser, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        jsc.markBinding(@src());
        var it = StreamResumableIterator.init(this);
        while (it.next()) |stream| {
    ...
    pub fn emitErrorToAllStreams(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        jsc.markBinding(@src());
    ...
    pub fn flushFromJS(this: *H2FrameParser, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());

        return jsc.JSValue.jsNumber(this.flush());
    }

    pub fn request(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        log("request", .{});
        ...
        if (args_list.len > 4 and !args_list.ptr[4].isEmptyOrUndefinedOrNull()) {
            ...
            if (try options.get(globalObject, "signal")) |signal_arg| {
                if (signal_arg.as(jsc.WebCore.AbortSignal)) |signal_| {
                    if (signal_.aborted()) {
                        stream.state = .IDLE;
                        this.abortStream(stream, Fun__wrapAbortError(globalObject, signal_.abortReason()));
                        return jsc.JSValue.jsNumber(stream_id);
                    }
        ...
        if (encoded_size <= headers_frame_max_payload) {
            ...
            if (padding != 0) {
                ...
                fun.memmove(buffer[1..][0..encoded_size], buffer[0..encoded_size]);
                buffer[0] = padding;
    ...
    pub fn read(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn onNativeRead(this: *H2FrameParser, data: []const u8) fun.JSError!void {
        log("onNativeRead", .{});
        this.ref();
        defer this.deref();
        defer this.incrementWindowSizeIfNeeded();
    ...
    pub fn setNativeSocketFromJS(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        jsc.markBinding(@src());
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
    ...
    pub fn constructor(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame, thisValue: jsc.JSValue) fun.JSError!*H2FrameParser {
        const args_list = callframe.arguments_old(1);
        if (args_list.len < 1) {
            return globalObject.throw("Expected 1 argument", .{});
        }
        ...
        var this = brk: {
            if (ENABLE_ALLOCATOR_POOL) {
                if (H2FrameParser.pool == null) {
                    H2FrameParser.pool = fun.handleOom(fun.default_allocator.create(H2FrameParser.H2FrameParserHiveAllocator));
                    H2FrameParser.pool.?.* = H2FrameParser.H2FrameParserHiveAllocator.init(fun.default_allocator);
                }
                const self = fun.handleOom(H2FrameParser.pool.?.tryGet());
                ...
                self.* = H2FrameParser{
                    ...
                    .streams = fun.U32HashMap(*Stream).init(fun.default_allocator),
                };
                break :brk self;
            } else {
                break :brk fun.new(H2FrameParser, .{
                    .ref_count = .init(),
                    .handlers = handlers,
                    .globalThis = globalObject,
                    .allocator = fun.default_allocator,
    ...
    pub fn detachFromJS(this: *H2FrameParser, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
        jsc.markBinding(@src());
        var it = StreamResumableIterator.init(this);
        while (it.next()) |stream| {
            stream.freeResources(this, false);
    ...
    fn deinit(this: *H2FrameParser) void {
        ...
        this.streams = fun.U32HashMap(*Stream).init(fun.default_allocator);
    }
...
extern fn Fun__wrapAbortError(globalObject: *jsc.JSGlobalObject, cause: jsc.JSValue) jsc.JSValue;
...
const TCPSocket = fun.api.socket.TCPSocket;
const TLSSocket = fun.api.socket.TLSSocket;
