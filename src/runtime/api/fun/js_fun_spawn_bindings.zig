...
fn getArgv0(globalThis: *jsc.JSGlobalObject, PATH: []const u8, cwd: []const u8, pretend_argv0: ?[*:0]const u8, first_cmd: JSValue, allocator: std.mem.Allocator) fun.JSError!struct {
    argv0: [:0]const u8,
    arg0: [:0]u8,
} {
    ...
    if (strings.indexOfChar(arg0.slice(), 0) != null) {
        return globalThis.ERR(.INVALID_ARG_VALUE, "The argument 'args[0]' must be a string without null bytes. Received {f}", .{fun.fmt.quote(arg0.slice())}).throw();
    }
    // Heap allocate it to ensure we don't run out of stack space.
    const path_buf: *fun.PathBuffer = try fun.default_allocator.create(fun.PathBuffer);
    defer fun.default_allocator.destroy(path_buf);
    ...
    return .{
        .argv0 = actual_argv0,
        .arg0 = if (pretend_argv0) |p| try allocator.dupeZ(u8, fun.sliceTo(p, 0)) else try allocator.dupeZ(u8, arg0.slice()),
    };
}

/// `argv` for `Fun.spawn` & `Fun.spawnSync`
fn getArgv(globalThis: *jsc.JSGlobalObject, args: JSValue, PATH: []const u8, cwd: []const u8, argv0: *?[*:0]const u8, allocator: std.mem.Allocator, argv: *std.array_list.Managed(?[*:0]const u8)) fun.JSError!void {
    ...
    while (try cmds_array.next()) |value| {
        const arg = try value.toFunString(globalThis);
        defer arg.deref();
...
/// Fun.spawn() calls this.
pub fn spawn(globalThis: *jsc.JSGlobalObject, args: JSValue, secondaryArgsValue: ?JSValue) fun.JSError!JSValue {
    return spawnMaybeSync(globalThis, args, secondaryArgsValue, false);
}

/// Fun.spawnSync() calls this.
pub fn spawnSync(globalThis: *jsc.JSGlobalObject, args: JSValue, secondaryArgsValue: ?JSValue) fun.JSError!JSValue {
...
pub fn spawnMaybeSync(
    ...
    if (comptime is_sync) {
        ...
        if (comptime !Environment.isWindows) {
            ...
            if (!fun.StackCheck.init().isSafeToRecurse()) {
    ...
    var extra_fds = std.array_list.Managed(fun.spawn.SpawnOptions.Stdio).init(fun.default_allocator);
        ...
        if (args != .zero and args.isObject()) {
            ...
            if (comptime is_sync) {
                if (try args.getTruthy(globalThis, "terminal")) |_| {
                    return globalThis.throwInvalidArguments("terminal option is only supported for Fun.spawn, not Fun.spawnSync", .{});
                }
            }
            ...
            if (try args.get(globalThis, "killSignal")) |val| {
                killSignal = try fun.SignalCode.fromJS(val, globalThis);
            }
                ...
                if (try args.getTruthy(globalThis, "terminal")) |terminal_val| {
                    ...
                    if (Terminal.fromJS(terminal_val)) |terminal| {
                        ...
                        if (comptime Environment.isPosix) {
                            if (terminal.slave_fd == fun.invalid_fd) {
    ...
    if (!override_env and env_array.items.len == 0) {
        env_array.items = jsc_vm.transpiler.env.map.createNullDelimitedEnvMap(allocator) catch |err| return globalThis.throwError(err, "in Fun.spawn") catch return .zero;
    ...
    if (!is_sync) if (maybe_ipc_mode) |ipc_mode| {
        ...
        //
        // Fun currently only supports three fds: stdin, stdout, and stderr, which are all unidirectional
        //
        // And then one fd is assigned specifically and only for IPC. If the user dont specify it, we add one (default: 3).
        //
        // When Fun.spawn() is given an `.ipc` callback, it enables IPC as follows:
        env_array.ensureUnusedCapacity(allocator, 3) catch |err| return globalThis.throwError(err, "in Fun.spawn") catch return .zero;
    ...
    const can_block_entire_thread_to_reduce_cpu_usage_in_fast_path = (comptime Environment.isPosix and is_sync) and
        ...
        !fun.feature_flag.FUN_FEATURE_FLAG_DISABLE_SPAWNSYNC_FAST_PATH.get();
    ...
    var spawned = switch (fun.spawn.spawnProcess(
        &spawn_options,
        ...
        error.EMFILE, error.ENFILE => {
            ...
            const display_path: [:0]const u8 = if (argv.items.len > 0 and argv.items[0] != null)
                ...
                "";
            var systemerror = fun.sys.Error.fromCode(if (err == error.EMFILE) .MFILE else .NFILE, .posix_spawn).withPath(display_path).toSystemError();
            systemerror.errno = if (err == error.EMFILE) -fun.sys.UV_E.MFILE else -fun.sys.UV_E.NFILE;
            ...
            switch (err.getErrno()) {
                .ACCES, .NOENT, .PERM, .ISDIR, .NOTDIR => |errno| {
                    ...
                    if (display_path.len > 0) {
                        ...
                        if (errno == .NOENT) systemerror.errno = -fun.sys.UV_E.NOENT;
                        return globalThis.throwValue(systemerror.toErrorInstance(globalThis));
                    }
                },
                else => {},
    ...
    var subprocess = fun.new(Subprocess, .{
        .ref_count = .init(),
    ...
    if (comptime !is_sync) {
        ...
        if (timeout) |timeout_val| {
            subprocess.event_loop_timer.next = fun.timespec.msFromNow(.allow_mocked_time, timeout_val);
            globalThis.funVM().timer.insert(&subprocess.event_loop_timer);
            subprocess.setEventLoopTimerRefd(true);
        }

        fun.debugAssert(out != .zero);
    ...
    {
        var absolute_timespec = fun.timespec.epoch;
        var now = fun.timespec.now(.allow_mocked_time);
        var user_timespec: fun.timespec = if (timeout) |timeout_ms| now.addMs(timeout_ms) else absolute_timespec;
        ...
        while (subprocess.computeHasPendingActivity()) {
            ...
            const fun_test_timeout: fun.timespec = if (fun.jsc.Jest.Jest.runner) |runner| runner.getActiveTimeout() else .epoch;
            const has_fun_test_timeout = !fun_test_timeout.eql(&.epoch);

            if (has_fun_test_timeout) {
                switch (fun_test_timeout.orderIgnoreEpoch(user_timespec)) {
                    .lt => absolute_timespec = fun_test_timeout,
                    .eq => {},
            ...
            switch (sync_loop.tickWithTimeout(if (has_timespec and !did_timeout) &absolute_timespec else null)) {
                .completed => {
                    now = fun.timespec.now(.allow_mocked_time);
                },
                .timeout => {
                    now = fun.timespec.now(.allow_mocked_time);
                    ...
                    // Support fun:test timeouts AND spawnSync() timeout.
                    // There is a scenario where inside of spawnSync() a totally
                    // different test fails, and that SHOULD be okay.
                    if (has_fun_test_timeout) {
                        if (fun_test_timeout.order(&now) == .lt) {
                            var active_file_strong = fun.jsc.Jest.Jest.runner.?.fun_test_root.active_file
                            ...
                            fun.jsc.Jest.Jest.runner.?.removeActiveTimeout(jsc_vm);
                            ...
                            jsc.Jest.fun_test.FunTest.funTestTimeoutCallback(taken_active_file, &absolute_timespec, jsc_vm);
                        }
                    }
                },
            }
...
fn throwCommandNotFound(globalThis: *jsc.JSGlobalObject, command: []const u8) fun.JSError {
    const err = jsc.SystemError{
        .message = fun.handleOom(fun.String.createFormat("Executable not found in $PATH: \"{s}\"", .{command})),
        .code = fun.String.static("ENOENT"),
        .errno = -fun.sys.UV_E.NOENT,
        .path = fun.String.cloneUTF8(command),
    };
    return globalThis.throwValue(err.toErrorInstance(globalThis));
}

pub fn appendEnvpFromJS(globalThis: *jsc.JSGlobalObject, object: *jsc.JSObject, envp: *std.array_list.Managed(?[*:0]const u8), PATH: *[]const u8) fun.JSError!void {
    ...
    while (try object_iter.next()) |key| {
        ...
        const value_bunstr = try value.toFunString(globalThis);
        ...
        if (key.eqlComptime("PATH")) {
            PATH.* = fun.asByteSlice(line["PATH=".len..]);
        }
...
const Rusage = fun.spawn.Rusage;
const Stdio = fun.spawn.Stdio;
