...
const RefCount = fun.ptr.RefCount(@This(), "ref_count", PipeReader.deinit, .{});
...
pub fn start(this: *PipeReader, process: *Subprocess, event_loop: *jsc.EventLoop) fun.sys.Maybe(void) {
    this.ref();
...
pub fn toReadableStream(this: *PipeReader, globalObject: *jsc.JSGlobalObject) fun.JSError!jsc.JSValue {
    defer this.detach();
...
fn deinit(this: *PipeReader) void {
    if (comptime Environment.isPosix) {
        fun.assert(this.reader.isDone() or this.state == .err);
    }
    ...
    if (comptime Environment.isWindows) {
        ...
        fun.assert(this.reader.source == null or this.reader.source.?.isClosed());
    }
