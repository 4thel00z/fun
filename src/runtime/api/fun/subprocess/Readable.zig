pub const Readable = union(enum) {
    ...
    pub fn init(stdio: Stdio, event_loop: *jsc.EventLoop, process: *Subprocess, result: StdioResult, allocator: std.mem.Allocator, max_size: ?*MaxBuf, is_sync: bool) Readable {
        ...
        if (comptime Environment.isPosix) {
            if (stdio == .pipe) {
                _ = fun.sys.setNonblocking(result.?);
            }
        }
    ...
    pub fn onClose(this: *Readable, _: ?fun.sys.Error) void {
        this.* = .closed;
    }
    ...
    pub fn finalize(this: *Readable) void {
        switch (this.*) {
            ...
            .buffer => |*buf| {
                buf.deinit(fun.default_allocator);
            },
            else => {},
        }
    }

    pub fn toJS(this: *Readable, globalThis: *jsc.JSGlobalObject, exited: bool) fun.JSError!JSValue {
        _ = exited; // autofix
        switch (this.*) {
            ...
            .buffer => |*buffer| {
                ...
                const own = try buffer.takeSlice(fun.default_allocator);
    ...
    pub fn toBufferedValue(this: *Readable, globalThis: *jsc.JSGlobalObject) fun.JSError!JSValue {
        switch (this.*) {
            .fd => |fd| {
                return fd.toJS(globalThis);
            },
            ...
            .buffer => |*buf| {
                defer this.* = .{ .closed = {} };
                const own = buf.takeSlice(fun.default_allocator) catch {
                    return globalThis.throwOutOfMemory();
                };

                return jsc.MarkedArrayBuffer.fromBytes(own, fun.default_allocator, .Uint8Array).toNodeBuffer(globalThis);
            },
            else => {
                return .js_undefined;
            },
