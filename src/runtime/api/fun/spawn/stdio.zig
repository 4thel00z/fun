pub const Stdio = union(enum) {
    inherit,
    capture: struct { buf: *fun.ByteList },
    ignore,
    fd: fun.FD,
    dup2: struct {
        out: fun.jsc.Subprocess.StdioKind,
        to: fun.jsc.Subprocess.StdioKind,
    },
    ...
    pub const Result = union(enum) {
        result: fun.spawn.SpawnOptions.Stdio,
    ...
    pub fn useMemfd(this: *@This(), index: u32) bool {
        ...
        if (!fun.sys.canUseMemfd()) return false;
        ...
        const fd = fun.sys.memfd_create(label, .cross_process).unwrap() catch return false;

        var remain = this.byteSlice();

        if (remain.len > 0)
            // Hint at the size of the file
            _ = fun.sys.ftruncate(fd, @intCast(remain.len));

        // Dump all the bytes in there
        var written: isize = 0;
        while (remain.len > 0) {
            switch (fun.sys.pwrite(fd, remain, written)) {
                .err => |err| {
    ...
    fn toPosix(
        ...
        return .{
            .result = switch (stdio.*) {
                .blob => |blob| brk: {
                    const fd = fun.FD.Stdio.fromInt(i).?.fd();
    ...
    fn extractBodyValue(out_stdio: *Stdio, globalThis: *jsc.JSGlobalObject, i: i32, body: *jsc.WebCore.Body.Value, is_sync: bool) fun.JSError!void {
    ...
    pub fn extract(out_stdio: *Stdio, globalThis: *jsc.JSGlobalObject, i: i32, value: JSValue, is_sync: bool) fun.JSError!void {
        ...
        if (value.isString()) {
            ...
            if (str.eqlComptime("inherit")) {
                ...
                return globalThis.throwInvalidArguments("stdio must be an array of 'inherit', 'pipe', 'ignore', Fun.file(pathOrFd), number, or null", .{});
            }
            return;
        ...
        if (value.asArrayBuffer(globalThis)) |array_buffer| {
            // Change in Fun v1.0.34: don't throw for empty ArrayBuffer
    ...
    pub fn extractBlob(stdio: *Stdio, globalThis: *jsc.JSGlobalObject, blob: jsc.WebCore.Blob.Any, i: i32) fun.JSError!void {
        const fd = fun.FD.Stdio.fromInt(i).?.fd();
            ...
            if (blob.store()) |store| {
                if (store.data.file.pathlike == .fd) {
                    if (store.data.file.pathlike.fd == fd) {
                        ...
                        if (fun.FD.Stdio.fromInt(i)) |tag| switch (tag) {
                            .std_in,
                            => if (i == 1 or i == 2) {
                                return globalThis.throwInvalidArguments("stdin cannot be used for stdout or stderr", .{});
                            },
...
fn createZeroedPipe() *uv.Pipe {
    return fun.new(uv.Pipe, std.mem.zeroes(uv.Pipe));
}
