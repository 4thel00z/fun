...
const log = fun.Output.scoped(.PROCESS, .visible);
...
pub fn uv_getrusage(process: *uv.uv_process_t) win_rusage {
    ...
    if (fun.windows.GetProcessTimes(process_pid, &starttime, &exittime, &kerneltime, &usertime) == 1) {
...
// const ShellSubprocessMini = fun.shell.ShellSubprocessMini;
pub const ProcessExitHandler = struct {
    ...
    pub const TaggedPointer = fun.TaggedPointerUnion(
        .{
            Subprocess,
    ...
    pub fn call(this: *const ProcessExitHandler, process: *Process, status: Status, rusage: *const Rusage) void {
        ...
        switch (this.ptr.tag()) {
            ...
            @field(TaggedPointer.Tag, @typeName(SyncProcess)) => {
                ...
                @panic("Internal Fun error: ProcessExitHandler has an invalid tag. Please file a bug report.");
            },
        }
    }
};
...
/// If a poll callback then re-enters `us_loop_run_fun_tick` (e.g.
...
pub const Process = struct {
    ...
    pub fn signalCode(this: *const Process) ?fun.SignalCode {
    ...
    pub fn onWaitPidFromWaiterThread(this: *Process, waitpid_result: *const fun.sys.Maybe(PosixSpawn.WaitPidResult), rusage: *const Rusage) void {
    ...
    fn onWaitPid(this: *Process, waitpid_result: *const fun.sys.Maybe(PosixSpawn.WaitPidResult), rusage: *const Rusage) void {
        if (comptime !Environment.isPosix) {
            @compileError("not implemented on this platform");
        }
    ...
    pub fn watchOrReap(this: *Process) fun.sys.Maybe(bool) {
        if (this.hasExited()) {
    ...
    pub fn watch(this: *Process) fun.sys.Maybe(void) {
        if (comptime Environment.isWindows) {
            this.poller.uv.ref();
            return .success;
        }
        ...
        const poll = if (this.poller == .fd)
            ...
            fun.Async.FilePoll.init(this.event_loop, .fromNative(watchfd), .{}, Process, this);
    ...
    pub fn rewatchPosix(this: *Process) fun.sys.Maybe(void) {
        if (WaiterThread.shouldUseWaiterThread()) {
            if (this.poller != .waiter_thread)
                this.poller = .{ .waiter_thread = .{} };
            this.poller.waiter_thread.ref(this.event_loop);
        ...
        if (this.poller == .fd) {
            ...
            @panic("Internal Fun error: poll_ref in Subprocess is null unexpectedly. Please file a bug report.");
        }
    }
    ...
    fn onExitUV(process: *uv.uv_process_t, exit_status: i64, term_signal: c_int) callconv(.c) void {
        ...
        const signal_code: ?fun.SignalCode = if (term_signal > 0 and term_signal < @intFromEnum(fun.SignalCode.SIGSYS)) @enumFromInt(term_signal) else null;
        const rusage = uv_getrusage(process);

        fun.windows.libuv.log("Process.onExit({d}) code: {d}, signal: {?}", .{ process.pid, exit_code, signal_code });
        ...
        if (signal_code) |sig| {
            ...
            this.onExit(
                .{
                    .err = fun.sys.Error.fromCode(@intCast(exit_status), .waitpid),
                },
                &rusage,
            );
        }
    ...
    fn onCloseUV(uv_handle: *uv.uv_process_t) callconv(.c) void {
        ...
        var this: *Process = @fieldParentPtr("poller", poller);
        fun.windows.libuv.log("Process.onClose({d})", .{uv_handle.pid});
    ...
    pub fn close(this: *Process) void {
        ...
        if (comptime Environment.isLinux) {
            if (this.pidfd != fun.invalid_fd.value.as_system and this.pidfd > 0) {
                fun.FD.fromNative(this.pidfd).close();
                this.pidfd = fun.invalid_fd.value.as_system;
            }
        }
    }
        ...
        if (comptime Environment.isPosix) {
            switch (this.poller) {
                .waiter_thread, .fd => {
                    ...
                    if (err != 0) {
                        const errno_ = fun.sys.getErrno(err);

                        // if the process was already killed don't throw
                        if (errno_ != .SRCH)
                            return .{ .err = fun.sys.Error.fromCode(errno_, .kill) };
                    }
                },
                else => {},
            }
            ...
            switch (this.poller) {
                .uv => |*handle| {
                    if (handle.kill(signal).toError(.kill)) |err| {
                        // if the process was already killed don't throw
                        if (err.errno != @intFromEnum(fun.sys.E.SRCH)) {
                            return .{ .err = err };
                        }
                    }
...
pub const Status = union(enum) {
    ...
    pub const Exited = struct {
        code: u8 = 0,
        signal: fun.SignalCode = @enumFromInt(0),
    };
    ...
    pub fn signalCode(this: *const Status) ?fun.SignalCode {
        return switch (this.*) {
            .signaled => |sig| sig,
...
pub const PollerPosix = union(enum) {
    fd: *fun.Async.FilePoll,
    waiter_thread: fun.Async.KeepAlive,
    detached: void,
...
const WaiterThreadPosix = struct {
    ...
    eventfd: if (Environment.isLinux) fun.FD else u0 = undefined,
    ...
    fn NewQueue(comptime T: type) type {
        return struct {
            ...
            active: std.array_list.Managed(*T) = std.array_list.Managed(*T).init(fun.default_allocator),
            ...
            pub const ConcurrentQueue = fun.UnboundedQueue(TaskQueueEntry, .next);

            pub const ResultTask = struct {
                result: fun.sys.Maybe(PosixSpawn.WaitPidResult),
                subprocess: *T,
                rusage: Rusage,
    ...
    pub fn init() !void {
        fun.assert(should_use_waiter_thread);
    ...
    fn wakeup(_: c_int) callconv(.c) void {
        const one = @as([8]u8, @bitCast(@as(usize, 1)));
        _ = fun.sys.write(instance.eventfd, &one).unwrap() catch 0;
    }
    ...
    pub fn reloadHandlers() void {
        ...
        if (comptime Environment.isLinux) {
            var current_mask = fun.sys.sigemptyset();
            fun.sys.sigaddset(&current_mask, std.posix.SIG.CHLD);
            const act = fun.sys.Sigaction{
                .handler = .{ .handler = &wakeup },
                .mask = current_mask,
                .flags = std.posix.SA.NOCLDSTOP,
            };
            fun.sys.sigaction(std.posix.SIG.CHLD, &act, null);
        }
    }
    ...
    pub fn loop() void {
        ...
        outer: while (true) {
            ...
            if (comptime Environment.isLinux) {
                ...
                if (fun.sys.read(this.eventfd, &buf).unwrap() catch 0 > 0) {
                    continue :outer;
                }
...
pub const PosixSpawnOptions = struct {
    ...
    /// signal between vfork and exec in posix_spawn_fun, so the kernel kills
    ...
    pub const Stdio = union(enum) {
        ...
        dup2: struct { out: fun.jsc.Subprocess.StdioKind, to: fun.jsc.Subprocess.StdioKind },
    };
...
pub const WindowsSpawnResult = struct {
    ...
    extra_pipes: std.array_list.Managed(StdioResult) = std.array_list.Managed(StdioResult).init(fun.default_allocator),
    stream: bool = true,
    sync: bool = false,
...
pub const WindowsSpawnOptions = struct {
    ...
    pseudoconsole: ?fun.windows.HPCON = null,
...
pub const PosixSpawnResult = struct {
    ...
    stderr: ?fun.FD = null,
    ipc: ?fun.FD = null,
    extra_pipes: std.array_list.Managed(ExtraPipe) = std.array_list.Managed(ExtraPipe).init(fun.default_allocator),
    ...
    pub const ExtraPipe = union(enum) {
        ...
        pub fn fd(this: ExtraPipe) fun.FD {
            return switch (this) {
                .owned_fd, .unowned_fd => |f| f,
                .unavailable => fun.invalid_fd,
            };
        }
    };
    ...
    fn pidfdFlagsForLinux() u32 {
        const kernel = fun.analytics.GenerateHeader.GeneratePlatform.kernelVersion();
    ...
    pub fn pifdFromPid(this: *PosixSpawnResult) fun.sys.Maybe(PidFDType) {
        if (!Environment.isLinux or WaiterThread.shouldUseWaiterThread()) {
            return .{ .err = fun.sys.Error.fromCode(.NOSYS, .pidfd_open) };
        }
...
) !fun.sys.Maybe(SpawnProcessResult) {
    if (comptime Environment.isPosix) {
...
pub fn spawnProcessPosix(
    ...
    argv: [*:null]?[*:0]const u8,
    envp: [*:null]?[*:0]const u8,
) !fun.sys.Maybe(PosixSpawnResult) {
    fun.analytics.Features.spawn += 1;
    ...
    var flags: i32 = fun.c.POSIX_SPAWN_SETSIGDEF | fun.c.POSIX_SPAWN_SETSIGMASK;

    if (comptime Environment.isMac) {
        flags |= fun.c.POSIX_SPAWN_CLOEXEC_DEFAULT;

        if (options.use_execve_on_macos) {
            flags |= fun.c.POSIX_SPAWN_SETEXEC;
    ...
    if (options.detached) {
        if (comptime @hasDecl(fun.c, "POSIX_SPAWN_SETSID")) {
            flags |= fun.c.POSIX_SPAWN_SETSID;
        }
        attr.detached = true;
    }
    ...
    if (Environment.isLinux) {
        ...
        // every child to SIGKILL-on-parent-death so non-Fun descendants are
        ...
        attr.linux_pdeathsig = if (options.linux_pdeathsig) |sig|
            @intCast(sig)
        else if (fun.ParentDeathWatchdog.shouldDefaultSpawnPdeathsig())
            std.posix.SIG.KILL
        else
            0;
    }
    ...
    var extra_fds = std.array_list.Managed(PosixSpawnResult.ExtraPipe).init(fun.default_allocator);
    errdefer extra_fds.deinit();
    var stack_fallback = std.heap.stackFallback(2048, fun.default_allocator);
    const allocator = stack_fallback.get();
    var to_close_at_end = std.array_list.Managed(fun.FD).init(allocator);
    var to_set_cloexec = std.array_list.Managed(fun.FD).init(allocator);
    defer {
        for (to_set_cloexec.items) |fd| {
            _ = fun.sys.setCloseOnExec(fd);
        }
    ...
    var to_close_on_error = std.array_list.Managed(fun.FD).init(allocator);
    ...
    const stdios: [3]*?fun.FD = .{ &spawned.stdin, &spawned.stdout, &spawned.stderr };

    var dup_stdout_to_stderr: bool = false;

    for (0..3) |i| {
        const stdio = stdios[i];
        const fileno = fun.FD.fromNative(@intCast(i));
        const flag = if (i == 0) @as(u32, fun.O.RDONLY) else @as(u32, fun.O.WRONLY);
        ...
        switch (stdio_options[i]) {
            ...
            .ipc, .ignore => {
                try actions.openZ(fileno, "/dev/null", flag | fun.O.CREAT, 0o664);
            },
            .path => |path| {
                try actions.open(fileno, path, flag | fun.O.CREAT, 0o664);
            },
            .buffer => {
                if (Environment.isLinux) use_memfd: {
                    if (!options.stream and i > 0 and fun.sys.canUseMemfd()) {
                        // use memfd if we can
                        ...
                        const fd = fun.sys.memfd_create(label, .cross_process).unwrap() catch break :use_memfd;
                ...
                const fds: [2]fun.FD = brk: {
                    const pair = if (!options.no_sigpipe) try fun.sys.socketpairForShell(
                        std.posix.AF.UNIX,
                        std.posix.SOCK.STREAM,
                        0,
                        .blocking,
                    ).unwrap() else try fun.sys.socketpair(
                        std.posix.AF.UNIX,
                        std.posix.SOCK.STREAM,
                        0,
                        .blocking,
                ...
                if (!options.sync) {
                    try fun.sys.setNonblocking(fds[0]).unwrap();
                }
    ...
    for (options.extra_fds, 0..) |ipc, i| {
        const fileno = fun.FD.fromNative(@intCast(3 + i));
        ...
        switch (ipc) {
            ...
            .ignore => {
                try actions.openZ(fileno, "/dev/null", fun.O.RDWR, 0o664);
                try extra_fds.append(.unavailable);
            },

            .path => |path| {
                try actions.open(fileno, path, fun.O.RDWR | fun.O.CREAT, 0o664);
                try extra_fds.append(.unavailable);
            },
            .ipc, .buffer => {
                const fds: [2]fun.FD = try fun.sys.socketpair(
                    std.posix.AF.UNIX,
                    std.posix.SOCK.STREAM,
                    0,
    ...
    switch (spawn_result) {
        ...
        .result => |pid| {
            ...
            extra_fds = std.array_list.Managed(PosixSpawnResult.ExtraPipe).init(fun.default_allocator);
...
pub fn spawnProcessWindows(
    ...
    argv: [*:null]?[*:0]const u8,
    envp: [*:null]?[*:0]const u8,
) !fun.sys.Maybe(WindowsSpawnResult) {
    fun.markWindowsOnly();
    fun.analytics.Features.spawn += 1;
    ...
    var stack_allocator = std.heap.stackFallback(8192, fun.default_allocator);
    ...
    defer {
        for (uv_files_to_close.items) |fd| {
            fun.Async.Closer.close(.fromUV(fd), loop);
        }
        ...
        if (treat_as_dup) {
            if (fd_i == 1) {
                if (uv.uv_pipe(&dup_fds, 0, 0).errEnum()) |e| {
                    return .{ .err = fun.sys.Error.fromCode(e, .pipe) };
                }
            }
    ...
    defer {
        if (dup_src != null) {
            if (Environment.allow_assert) fun.assert(dup_src != null and dup_tgt != null);
        }

        if (failed) {
            if (dup_fds[0] != -1) {
                fun.FD.fromUV(dup_fds[0]).close();
            }
        }

        if (dup_fds[1] != -1) {
            fun.FD.fromUV(dup_fds[1]).close();
        }
    }
    ...
    fun.assert(process.poller.uv.exit_cb == &Process.onExitUV);

    var result = WindowsSpawnResult{
        .process_ = process,
        .extra_pipes = try std.array_list.Managed(WindowsSpawnResult.StdioResult).initCapacity(fun.default_allocator, options.extra_fds.len),
    };
...
pub const sync = struct {
                ...
                return switch (this.*) {
                    ...
                    .buffer => .{
                        .buffer = if (Environment.isWindows)
                            fun.new(fun.windows.libuv.Pipe, std.mem.zeroes(fun.windows.libuv.Pipe)),
                    },
                };
            }
        };
    ...
    pub const Result = struct {
        status: Status,
        stdout: std.array_list.Managed(u8) = .{ .items = &.{}, .allocator = fun.default_allocator, .capacity = 0 },
        stderr: std.array_list.Managed(u8) = .{ .items = &.{}, .allocator = fun.default_allocator, .capacity = 0 },
    ...
    const SyncWindowsPipeReader = struct {
        chunks: std.array_list.Managed([]u8) = .{ .items = &.{}, .allocator = fun.default_allocator, .capacity = 0 },
        pipe: *uv.Pipe,

        err: fun.sys.E = .SUCCESS,
        context: *SyncWindowsProcess,
        onDoneCallback: *const fn (*SyncWindowsProcess, tag: SyncWindowsProcess.OutFd, chunks: []const []u8, err: fun.sys.E) void = &SyncWindowsProcess.onReaderDone,
        tag: SyncWindowsProcess.OutFd,

        pub const new = fun.TrivialNew(@This());

        fn onAlloc(_: *SyncWindowsPipeReader, suggested_size: usize) []u8 {
            return fun.handleOom(fun.default_allocator.alloc(u8, suggested_size));
        }

        fn onRead(this: *SyncWindowsPipeReader, data: []const u8) void {
            fun.handleOom(this.chunks.append(@constCast(data)));
        }

        fn onError(this: *SyncWindowsPipeReader, err: fun.sys.E) void {
            this.err = err;
        ...
        fn onClose(pipe: *uv.Pipe) callconv(.c) void {
            ...
            fun.default_allocator.destroy(this.pipe);
    ...
    const SyncWindowsProcess = struct {
        ...
        pub fn onReaderDone(this: *SyncWindowsProcess, tag: OutFd, chunks: []const []u8, err: fun.sys.E) void {
            switch (tag) {
                .stderr => {
                    this.stderr = chunks;
                },
    ...
    fn spawnWindowsWithPipes(
        ...
        const result = Result{
            ...
            .stdout = std.array_list.Managed(u8).fromOwnedSlice(
                ...
                fun.handleOom(flattenOwnedChunks(fun.default_allocator, fun.default_allocator, this.stdout)),
            ),
            .stderr = std.array_list.Managed(u8).fromOwnedSlice(
                fun.default_allocator,
                fun.handleOom(flattenOwnedChunks(fun.default_allocator, fun.default_allocator, this.stderr)),
            ),
        };
        this.stdout = &.{};
        this.stderr = &.{};
    ...
    pub fn spawn(
        ...
        defer string_builder.deinit(fun.default_allocator);
        for (argv) |arg| {
            string_builder.countZ(arg);
        }

        try string_builder.allocate(fun.default_allocator);

        var args = fun.handleOom(std.array_list.Managed(?[*:0]u8).initCapacity(fun.default_allocator, argv.len + 1));
        defer args.deinit();
    ...
    extern "c" fn Fun__registerSignalsForForwarding() void;
    extern "c" fn Fun__unregisterSignalsForForwarding() void;

    // macOS p_puniqueid descendant tracker — see NoOrphansTracker.cpp.
    extern "c" fn Fun__noOrphans_begin(kq: c_int, root: std.c.pid_t) void;
    extern "c" fn Fun__noOrphans_releaseKq() void;
    extern "c" fn Fun__noOrphans_onFork() void;
    extern "c" fn Fun__noOrphans_onExit(pid: std.c.pid_t) void;

    /// TTY job-control bridge for `--no-orphans` `fun run`. We put the script
    /// in its own pgroup so `kill(-pgid)` reaches every descendant on cleanup,
    /// which makes `fun run` a one-job mini shell on a controlling terminal:
    ...
    const JobControl = struct {
        ...
        fn give(self: *@This(), pgid: std.c.pid_t) void {
            ...
            // `fun run --no-orphans dev &` from an interactive shell leaves
        ...
        /// back, stops `fun run` so the user's shell's `waitpid(WUNTRACED)`
        ...
        fn onChildStopped(self: *const @This()) void {
            ...
            ttouBlocked(self.prev);
            // SIGTSTP is not in `Fun__registerSignalsForForwarding`'s set, so
    ...
    extern "c" var Fun__currentSyncPID: i64;
    ...
    extern "c" fn Fun__sendPendingSignalIfNecessary() void;
    ...
    fn spawnPosix(
        ...
        // (`runBinaryWithoutBunxPath`, `funx`) set the flag unconditionally;
        // on Linux it's a spawn-side no-op so no-orphans must stay armed there.
        const no_orphans = fun.ParentDeathWatchdog.isEnabled() and
            !(Environment.isMac and options.use_execve_on_macos);

        // Snapshot pre-existing direct children so the disarm defer can tell
        // subreaper-adopted orphans (ppid==us) apart from `Fun.spawn` siblings
        // (also ppid==us). Typically empty — `fun run`/`funx` have no JS VM —
        ...
        const siblings = if (Environment.isLinux and no_orphans)
            fun.ParentDeathWatchdog.snapshotChildren(&siblings_buf)
        else
            siblings_buf[0..0];
        if (comptime Environment.isLinux) if (no_orphans) {
            ...
            // zombies in `fun foo.js` / `--filter` / `fun test`. Disarmed by
        ...
        defer if (comptime Environment.isLinux) if (no_orphans) {
            ...
            // to init.
            fun.ParentDeathWatchdog.killSubreaperAdoptees(siblings);
        ...
        var no_orphans_kq: fun.FD = fun.invalid_fd;
        if (comptime Environment.isMac) if (no_orphans) {
            if (std.posix.kqueue()) |kq| {
                no_orphans_kq = fun.FD.fromNative(kq);
            } else |_| {}
        };
        // LIFO: this runs LAST — after killSyncScriptTree() (which scans via
        // m_kq) and releaseKq().
        defer if (comptime Environment.isMac) if (no_orphans_kq != fun.invalid_fd)
            no_orphans_kq.close();
        // LIFO: runs after killSyncScriptTree() (which needs m_kq live for
        // its NOTE_FORK-drain rescan), before the close above.
        defer if (comptime Environment.isMac) if (no_orphans_kq != fun.invalid_fd)
            Fun__noOrphans_releaseKq();

        Fun__currentSyncPID = 0;
        Fun__registerSignalsForForwarding();
        defer {
            Fun__unregisterSignalsForForwarding();
            fun.crash_handler.resetOnPosix();
        }
        ...
        // a SIGTERM/SIGINT delivered to `fun run` reaches every descendant
        // that hasn't `setsid()`-escaped.
        Fun__currentSyncPID = if (no_orphans) -@as(i64, @intCast(process.pid)) else @intCast(process.pid);

        var jc: JobControl = .{};
        const pgid_pushed = no_orphans and fun.ParentDeathWatchdog.pushSyncPgid(process.pid);
        if (no_orphans) {
            ...
            // script's own knote.
            if (comptime Environment.isMac) if (no_orphans_kq != fun.invalid_fd)
                Fun__noOrphans_begin(no_orphans_kq.native(), process.pid);
        }
        defer if (no_orphans) {
            jc.restore();
            ...
            // that would SIGKILL the user's unrelated `Fun.spawn` children.
            // The full-tree walk runs from `onProcessExit` when the whole
            // process is actually exiting.
            fun.ParentDeathWatchdog.killSyncScriptTree();
            if (pgid_pushed) fun.ParentDeathWatchdog.popSyncPgid();
            if (comptime Environment.isLinux) {
        ...
        Fun__sendPendingSignalIfNecessary();

        var out = [2]std.array_list.Managed(u8){
            std.array_list.Managed(u8).init(fun.default_allocator),
            std.array_list.Managed(u8).init(fun.default_allocator),
        };
        var out_fds = [2]fun.FD{ process.stdout orelse fun.invalid_fd, process.stderr orelse fun.invalid_fd };
        var success = false;
        defer {
            ...
            if (comptime Environment.isLinux) {
                if (process.pidfd) |pidfd| {
                    fun.FD.fromNative(pidfd).close();
                }
            }
        }

        var out_fds_to_wait_for = [2]fun.FD{
            process.stdout orelse fun.invalid_fd,
            process.stderr orelse fun.invalid_fd,
        };

        if (process.memfds[1]) {
            out_fds_to_wait_for[0] = fun.invalid_fd;
        }

        if (process.memfds[2]) {
            out_fds_to_wait_for[1] = fun.invalid_fd;
        }
        ...
        const status: Status = blk: {
            if (no_orphans and (Environment.isLinux or Environment.isMac)) {
                const ppid = fun.ParentDeathWatchdog.ppidToWatch() orelse 0;
            ...
            while (out_fds_to_wait_for[0] != fun.invalid_fd or out_fds_to_wait_for[1] != fun.invalid_fd) {
                ...
                switch (fun.sys.getErrno(rc)) {
                    .SUCCESS => {},
                    .AGAIN, .INTR => continue,
                    else => |err| return .{ .err = fun.sys.Error.fromCode(err, .poll) },
                }
            }
        ...
        if (comptime Environment.isLinux) {
            for (process.memfds[1..], &out, out_fds) |memfd, *bytes, out_fd| {
                if (memfd) {
                    bytes.* = fun.sys.File.from(out_fd).readToEnd(fun.default_allocator).bytes;
                }
            }
        }
    ...
    ///     are tracked and killed via `Fun__noOrphans_killTracked()`.
    ...
    fn waitMacKqueue(
        ...
        if (kq_fd == fun.invalid_fd) return null;
        ...
        // `fun run`. EVFILT_SIGNAL coexists with the (default-ignore) SIGCHLD
        ...
        for (out_fds_to_wait_for, 0..) |fd, i| {
            if (fd != fun.invalid_fd) add(&changes, @intCast(fd.cast()), std.c.EVFILT.READ, 0, i);
        }

        var receipts: [5]std.c.Kevent = undefined;
        switch (fun.sys.kevent(kq_fd, changes, receipts[0..changes.len], null)) {
            .err => |err| return .{ .err = err },
            .result => {},
        }
        for (receipts[0..changes.len]) |r| {
            ...
            if (r.udata == TAG_PPID) {
                ...
                if (r.data == @intFromEnum(std.c.E.SRCH))
                    fun.Global.exit(fun.ParentDeathWatchdog.exit_code);
                continue;
            }
        ...
        Fun__noOrphans_onFork();

        var events: [16]std.c.Kevent = undefined;
        var child_exited = false;
        var child_status: ?Status = null;
        while (true) {
            const got = switch (fun.sys.kevent(kq_fd, &.{}, events[0..], null)) {
                .err => |err| return .{ .err = err },
                .result => |c| c,
            };
            var saw_fork = false;
            for (events[0..got]) |ev| {
                if (ev.filter == std.c.EVFILT.PROC) {
                    ...
                    if (ev.fflags & std.c.NOTE.EXIT != 0) {
                        ...
                        Fun__noOrphans_onExit(@intCast(ev.ident));
            ...
            if (saw_fork) Fun__noOrphans_onFork();
            if (child_exited) {
    ...
    fn waitLinuxSignalfd(
        ...
        const chld_fd: fun.FD = blk: {
            ...
            switch (linux.E.init(rc)) {
                .SUCCESS => break :blk fun.FD.fromNative(@intCast(rc)),
                else => break :blk fun.invalid_fd,
            }
        };
        defer if (chld_fd != fun.invalid_fd) chld_fd.close();

        // Parent-death: pidfd when available (instant wake). When not
        // (gVisor, sandboxes, pre-5.3): bound the poll at 100ms and recheck
        // `getppid()`.
        var ppid_fd = fun.invalid_fd;
        if (ppid > 1) switch (fun.sys.pidfd_open(ppid, 0)) {
            .result => |fd| ppid_fd = fun.FD.fromNative(fd),
            .err => |e| if (e.getErrno() == .SRCH)
                fun.Global.exit(fun.ParentDeathWatchdog.exit_code),
        };
        defer if (ppid_fd != fun.invalid_fd) ppid_fd.close();
        ...
        // restore on return so the next caller — or `fun run`'s own
        ...
        const need_ppid_fallback = ppid > 1 and ppid_fd == fun.invalid_fd;
        const timeout_ms: i32 = if (need_ppid_fallback or chld_fd == fun.invalid_fd) 100 else -1;
        ...
        while (true) {
            ...
            // here: spawnSync callers (`fun run`, `funx`, CLI subcommands)
            ...
            // Non-TTY callers never see stops, matching plain `fun run`.
            ...
            const push = struct {
                fn f(l: *[]std.c.pollfd, fd: fun.FD) void {
                    l.len += 1;
                    l.*[l.len - 1] = .{
                        .fd = @intCast(fd.cast()),
                        .events = std.posix.POLL.IN | std.posix.POLL.ERR | std.posix.POLL.HUP,
            ...
            for (out_fds_to_wait_for) |fd| if (fd != fun.invalid_fd) push(&pfds, fd);
            const ppid_idx = pfds.len;
            if (ppid_fd != fun.invalid_fd) push(&pfds, ppid_fd);
            const chld_idx = pfds.len;
            if (chld_fd != fun.invalid_fd) push(&pfds, chld_fd);
            ...
            if ((ppid_fd != fun.invalid_fd and pfds[ppid_idx].revents != 0) or
            ...
            if (chld_fd != fun.invalid_fd and pfds[chld_idx].revents != 0) {
                var si: linux.signalfd_siginfo = undefined;
                while (fun.sys.read(chld_fd, std.mem.asBytes(&si)).unwrapOr(0) == @sizeOf(linux.signalfd_siginfo)) {}
            }
        }
    ...
    fn drainFd(fd: *fun.FD, out_fd: *fun.FD, bytes: *std.array_list.Managed(u8)) ?fun.sys.Error {
        if (fd.* == fun.invalid_fd) return null;
        while (true) {
            bytes.ensureUnusedCapacity(16384) catch return fun.sys.Error.fromCode(.NOMEM, .recv);
            switch (fun.sys.recvNonBlock(fd.*, bytes.unusedCapacitySlice())) {
                .err => |err| {
...
const PosixSpawn = fun.spawn;
const Maybe = fun.sys.Maybe;
const ShellSubprocess = fun.shell.ShellSubprocess;
const uv = fun.windows.libuv;

const ChromeProcess = fun.api.ChromeProcess;
const WebViewHostProcess = fun.api.WebViewHostProcess;

const LifecycleScriptSubprocess = fun.install.LifecycleScriptSubprocess;
const SecurityScanSubprocess = fun.install.SecurityScanSubprocess;
