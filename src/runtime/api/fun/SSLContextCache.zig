...
//! Valkey, `Fun.connect`, `upgradeTLS`, WebSocket client) and the JS-side
...
const DigestContext = struct {
    ...
    pub fn eql(_: @This(), a: Digest, b: Digest, _: usize) bool {
        return fun.strings.eqlLong(&a, &b, false);
    }
};

pub const Entry = struct {
    /// Nulled by `fun_ssl_ctx_cache_on_free` when BoringSSL drops the last
...
pub fn getOrCreate(
    ...
    err: *uws.create_fun_socket_error_t,
) ?*BoringSSL.SSL_CTX {
    const opts = config.asUSockets();
    return self.getOrCreateDigest(opts, opts.digest(), err);
}

/// Variant for callers that already projected to `FunSocketContextOptions`
/// (e.g. via `asUSocketsForClientVerification()`).
pub fn getOrCreateOpts(
    self: *SSLContextCache,
    opts: uws.SocketContext.FunSocketContextOptions,
    err: *uws.create_fun_socket_error_t,
) ?*BoringSSL.SSL_CTX {
    return self.getOrCreateDigest(opts, opts.digest(), err);
}
...
pub fn getOrCreateDigest(
    ...
    const gop = fun.handleOom(self.map.getOrPut(fun.default_allocator, d));
    if (gop.found_existing) {
        ...
        // SSL_CTX_set_ex_data only fails on OOM (Fun crashes anyway), but if
    ...
    const entry = fun.new(Entry, .{ .ctx = ctx, .owner = self });
    gop.value_ptr.* = entry;
...
export fn fun_ssl_ctx_cache_on_free(
    parent: ?*anyopaque,
    ptr: ?*anyopaque,
