//! Fun.Terminal - Creates a pseudo-terminal (PTY) for interactive terminal sessions.
//!
...
const log = fun.Output.scoped(.Terminal, .hidden);
...
hpcon: if (Environment.isWindows) ?fun.windows.HPCON else void = if (Environment.isWindows) null else {},
...
pub const IOWriter = fun.io.StreamingWriter(@This(), struct {
...
pub const Options = struct {
    ...
    pub fn parseFromJS(globalObject: *jsc.JSGlobalObject, js_options: JSValue) fun.JSError!Options {
        var options = Options{};
...
fn initTerminal(
    ...
    const terminal = fun.new(Terminal, .{
        .ref_count = .init(),
        ...
        .term_name = term_name,
        .event_loop_handle = jsc.EventLoopHandle.init(globalObject.funVM().eventLoop()),
    ...
    switch (terminal.writer.start(pty_result.write_fd, true)) {
        ...
        .err => {
            ...
            terminal.read_fd = fun.invalid_fd;
            if (comptime Environment.isWindows) {
                terminal.write_fd.close();
                terminal.write_fd = fun.invalid_fd;
            }
...
/// Create a Terminal from Fun.spawn options (not from JS constructor)
...
pub fn getSlaveFd(this: *Terminal) fun.FD {
    return this.slave_fd;
}

/// Windows: get the ConPTY handle to pass to uv_spawn via
/// uv_process_options_t.pseudoconsole.
pub fn getPseudoconsole(this: *Terminal) ?fun.windows.HPCON {
...
pub fn closeSlaveFd(this: *Terminal) void {
    ...
    if (this.slave_fd != fun.invalid_fd) {
        this.slave_fd.close();
        this.slave_fd = fun.invalid_fd;
    }
}
...
fn closePseudoconsoleOffThread(this: *Terminal, hpcon: fun.windows.HPCON) void {
    if (comptime !Environment.isWindows) return;
    const Runner = struct {
        fn run(h: fun.windows.HPCON) void {
            fun.windows.ClosePseudoConsole(h);
        }
    };
...
const PtyResult = struct {
    ...
    write_fd: fun.FD,
    slave: fun.FD,
    hpcon: if (Environment.isWindows) fun.windows.HPCON else void,
};
...
const LibUtil = struct {
    ...
    pub fn getHandle() ?*anyopaque {
        ...
        for (lib_names) |lib_name| {
            handle = fun.sys.dlopen(lib_name, .{ .LAZY = true });
    ...
    pub fn getOpenPty() ?OpenPtyFn {
        return fun.sys.dlsymWithHandle(OpenPtyFn, "openpty", getHandle);
    }
};
...
fn createPtyPosix(cols: u16, rows: u16) CreatePtyError!PtyResult {
    ...
    const master_fd_desc = fun.FD.fromNative(master_fd);
    const slave_fd_desc = fun.FD.fromNative(slave_fd);
    ...
    if (std.posix.tcgetattr(slave_fd)) |termios| {
        ...
        if (comptime fun.Environment.allow_assert) {
            fun.sys.syslog("tcgetattr(slave_fd={d}) failed: {s}", .{ slave_fd, @errorName(err) });
        }
    }

    // Duplicate the master fd for reading and writing separately
    // This allows independent epoll registration and closing
    const read_fd = switch (fun.sys.dup(master_fd_desc)) {
        .result => |fd| fd,
        .err => {
    ...
    const write_fd = switch (fun.sys.dup(master_fd_desc)) {
        .result => |fd| fd,
        .err => {
            master_fd_desc.close();
            slave_fd_desc.close();
    ...
    _ = fun.sys.updateNonblocking(master_fd_desc, true);
    _ = fun.sys.updateNonblocking(read_fd, true);
    _ = fun.sys.updateNonblocking(write_fd, true);
    ...
    _ = fun.sys.setCloseOnExec(master_fd_desc);
    _ = fun.sys.setCloseOnExec(read_fd);
    _ = fun.sys.setCloseOnExec(write_fd);
...
fn createOverlappedPipePair(
    ...
    server_access: u32,
) CreatePtyError!struct { server: fun.windows.HANDLE, client: fun.windows.HANDLE } {
    const w = fun.windows;
    ...
    const name_w_len = fun.strings.convertUTF8toUTF16InBuffer(&name_w_buf, name).len;
...
fn createPtyWindows(cols: u16, rows: u16) CreatePtyError!PtyResult {
    ...
    const read_fd = fun.FD.fromNative(out_server.?).makeLibUVOwned() catch return error.DupFailed;
    out_server = null;
    errdefer read_fd.close();

    const write_fd = fun.FD.fromNative(in_server.?).makeLibUVOwned() catch return error.DupFailed;
    in_server = null;
    ...
    return PtyResult{
        .master = fun.invalid_fd,
        .read_fd = read_fd,
        .write_fd = write_fd,
        .slave = fun.invalid_fd,
        .hpcon = result_hpcon,
    };
}
...
fn getTermiosFlag(this: *Terminal, comptime field: enum { iflag, oflag, lflag, cflag }) JSValue {
    ...
    if (this.flags.closed or this.master_fd == fun.invalid_fd) return JSValue.jsNumber(0);
...
fn setTermiosFlag(this: *Terminal, globalObject: *jsc.JSGlobalObject, comptime field: enum { iflag, oflag, lflag, cflag }, value: JSValue) fun.JSError!void {
    if (comptime !Environment.isPosix) return;
    if (this.flags.closed or this.master_fd == fun.invalid_fd) return;
...
pub fn setInputFlags(this: *Terminal, globalObject: *jsc.JSGlobalObject, value: JSValue) fun.JSError!void {
...
pub fn setOutputFlags(this: *Terminal, globalObject: *jsc.JSGlobalObject, value: JSValue) fun.JSError!void {
...
pub fn setLocalFlags(this: *Terminal, globalObject: *jsc.JSGlobalObject, value: JSValue) fun.JSError!void {
...
pub fn setControlFlags(this: *Terminal, globalObject: *jsc.JSGlobalObject, value: JSValue) fun.JSError!void {
...
pub fn write(
    ...
    const string_or_buffer = try jsc.Node.StringOrBuffer.fromJS(globalObject, fun.default_allocator, data) orelse {
...
pub fn resize(
    ...
    if (comptime Environment.isWindows) {
        if (this.hpcon) |hpcon| {
            const size = fun.windows.COORD{ .X = clampToCoord(new_cols), .Y = clampToCoord(new_rows) };
            const hr = fun.windows.ResizePseudoConsole(hpcon, size);
            if (hr < 0) {
                return globalObject.throw("Failed to resize terminal", .{});
            }
        }
...
pub fn setRawMode(
    ...
    if (comptime Environment.isPosix) {
        ...
        const tty_result = fun.tty.setMode(this.master_fd.cast(), if (enabled) .raw else .normal);
        if (tty_result != 0) {
...
fn getTermios(fd: fun.FD) ?Termios {
...
fn setTermios(fd: fun.FD, termios_p: *const Termios) bool {
...
pub fn doRef(this: *Terminal, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
    this.updateRef(true);
    return .js_undefined;
}

/// Unreference the terminal
pub fn doUnref(this: *Terminal, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
...
pub fn closeInternal(this: *Terminal) void {
    ...
    this.read_fd = fun.invalid_fd;

    // Close master fd
    if (this.master_fd != fun.invalid_fd) {
        this.master_fd.close();
        this.master_fd = fun.invalid_fd;
    }
...
fn onWriterReady(this: *Terminal) void {
    ...
    if (js.gc.get(.drain, this_jsvalue)) |callback| {
        ...
        globalThis.funVM().eventLoop().runCallback(
            callback,
            globalThis,
            this_jsvalue,
            &.{this_jsvalue},
...
fn onWriterError(this: *Terminal, err: fun.sys.Error) void {
...
fn onWrite(this: *Terminal, amount: usize, status: fun.io.WriteStatus) void {
...
pub fn onReaderError(this: *Terminal, err: fun.sys.Error) void {
...
fn callExitCallback(this: *Terminal, exit_code: i32, signal: ?fun.SignalCode) void {
    const this_jsvalue = this.this_value.tryGet() orelse return;
...
pub fn onReadChunk(this: *Terminal, chunk: []const u8, has_more: fun.io.ReadState) bool {
    _ = has_more;
    ...
    const globalThis = this.globalThis;
    const duped = fun.default_allocator.dupe(u8, chunk) catch |err| {
...
pub fn loop(this: *Terminal) *fun.Async.Loop {
    if (comptime Environment.isWindows) {
