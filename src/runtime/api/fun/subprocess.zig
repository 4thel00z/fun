//! The Subprocess object is returned by `Fun.spawn`. This file also holds the
//! code for `Fun.spawnSync`
...
stdout: Readable,
stderr: Readable,
stdio_pipes: if (Environment.isWindows) std.ArrayListUnmanaged(StdioResult) else std.ArrayListUnmanaged(fun.spawn.PosixSpawnResult.ExtraPipe) = .{},
...
event_loop_timer: fun.api.Timer.EventLoopTimer = .{
...
pub fn createResourceUsageObject(this: *Subprocess, globalObject: *JSGlobalObject) fun.JSError!JSValue {
    ...
    switch (kind) {
        ...
        inline .stdout, .stderr => |tag| {
            ...
            switch (out.*) {
                .pipe => |pipe| {
                    if (pipe.state == .done) {
                        out.* = .{ .buffer = CowString.initOwned(pipe.state.done, fun.default_allocator) };
                        pipe.state = .{ .done = &.{} };
                    } else {
                        out.* = .{ .ignore = {} };
                    }
...
pub fn constructor(globalObject: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!*Subprocess {
...
pub fn getStderr(this: *Subprocess, globalThis: *JSGlobalObject) fun.JSError!JSValue {
...
pub fn getStdin(this: *Subprocess, globalThis: *JSGlobalObject) fun.JSError!JSValue {
...
pub fn getStdout(this: *Subprocess, globalThis: *JSGlobalObject) fun.JSError!JSValue {
...
pub fn asyncDispose(this: *Subprocess, global: *JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
    if (this.process.hasExited()) {
...
pub fn setEventLoopTimerRefd(this: *Subprocess, refd: bool) void {
    ...
    if (refd) {
        this.globalThis.funVM().timer.incrementTimerRef(1);
    } else {
        this.globalThis.funVM().timer.incrementTimerRef(-1);
    }
}
...
pub fn kill(
    ...
    // is still performed.
    const sig: SignalCode = try fun.SignalCode.fromJS(arguments.ptr[0], globalThis);
...
pub fn tryKill(this: *Subprocess, sig: SignalCode) fun.sys.Maybe(void) {
    if (this.hasExited()) {
        return .success;
    }
...
pub fn doRef(this: *Subprocess, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
    this.jsRef();
    return .js_undefined;
}

pub fn doUnref(this: *Subprocess, _: *jsc.JSGlobalObject, _: *jsc.CallFrame) fun.JSError!JSValue {
    this.jsUnref();
    return .js_undefined;
}
...
pub fn doSend(this: *Subprocess, global: *jsc.JSGlobalObject, callFrame: *jsc.CallFrame) fun.JSError!JSValue {
...
pub fn disconnect(this: *Subprocess, globalThis: *JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!JSValue {
    _ = globalThis;
    _ = callframe;
...
pub fn getStdio(this: *Subprocess, global: *JSGlobalObject) fun.JSError!JSValue {
...
pub fn onProcessExit(this: *Subprocess, process: *Process, status: fun.spawn.Status, rusage: *const Rusage) void {
    log("onProcessExit()", .{});
...
pub fn finalizeStreams(this: *Subprocess) void {
    ...
    this.stdio_pipes.clearAndFree(fun.default_allocator);
}
...
pub fn finalize(this: *Subprocess) callconv(.c) void {
    ...
    fun.assert(!this.computeHasPendingActivity() or jsc.VirtualMachine.get().isShuttingDown());
    ...
    if (this.event_loop_timer.state == .ACTIVE) {
        this.globalThis.funVM().timer.remove(&this.event_loop_timer);
    }
    this.setEventLoopTimerRefd(false);
...
pub fn handleIPCClose(this: *Subprocess) void {
    ...
    if (this_jsvalue != .zero) {
        ...
        if (consumeOnDisconnectCallback(this_jsvalue, globalThis)) |callback| {
            globalThis.funVM().eventLoop().runCallback(callback, globalThis, this_jsvalue, &.{.true});
        }
    }
}
...
pub const TestingAPIs = struct {
    ...
    pub fn injectStdioReadError(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        ...
        const kind_str = try kind_value.toFunString(globalThis);
        ...
        const fake_err = fun.sys.Error.fromCode(.BADF, .read);
...
pub const StdioResult = if (Environment.isWindows) fun.spawn.WindowsSpawnResult.StdioResult else ?fun.FD;
pub const Writable = @import("./subprocess/Writable.zig").Writable;

pub const MaxBuf = fun.io.MaxBuf;
pub const spawnSync = js_fun_spawn_bindings.spawnSync;
pub const spawn = js_fun_spawn_bindings.spawn;

const IPC = @import("../../../jsc/ipc.zig");
const Terminal = @import("./Terminal.zig");
const js_fun_spawn_bindings = @import("./js_fun_spawn_bindings.zig");
