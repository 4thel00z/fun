...
//! HTML file, and can be passed to the `static` option in `Fun.serve`. The build
...
pub fn getIndex(this: *HTMLBundle, globalObject: *JSGlobalObject) fun.JSError!JSValue {
    return fun.String.createUTF8ForJS(globalObject, this.path);
}
...
pub const Route = struct {
    ...
    const RefCount = fun.ptr.RefCount(@This(), "ref_count", Route.deinit, .{ .debug_name = "HTMLBundleRoute" });
    ...
    fn deinit(this: *Route) void {
        fun.assert(this.pending_responses.items.len == 0); // pending responses keep a ref to the route
        this.pending_responses.deinit(fun.default_allocator);
        this.bundle.deref();
        this.state.deinit();
        fun.destroy(this);
    }
    ...
    pub const State = union(enum) {
        pending,
        building: ?*fun.BundleV2.JSBundleCompletionTask,
        err: fun.logger.Log,
        html: *StaticRoute,
    ...
    fn onAnyRequest(this: *Route, req: uws.AnyRequest, resp: HTTPResponse, is_head: bool) void {
        ...
        if (server.config().isDevelopment()) {
            if (server.devServer()) |dev| {
                ...
                // but stay defensive.
                switch (req) {
                    .h1 => |h1| fun.handleOom(dev.respondForHTMLBundle(this, h1, resp)),
                    .h3 => {
        ...
        state: switch (this.state) {
            .pending => {
                ...
                fun.handleOom(this.scheduleBundle(server));
            ...
            .building => {
                ...
                const pending = fun.new(PendingResponse, .{
                    .method = fun.http.Method.which(req.method()) orelse {
                ...
                fun.handleOom(this.pending_responses.append(fun.default_allocator, pending));
    ...
    fn scheduleBundle(this: *Route, server: AnyServer) !void {
        switch (server.getOrLoadPlugins(.{ .html_bundle_route = this })) {
            .err => this.state = .{ .err = fun.logger.Log.init(fun.default_allocator) },
    ...
    pub fn onPluginsResolved(this: *Route, plugins: ?*jsc.API.JSBundler.Plugin) !void {
        ...
        errdefer config.deinit(fun.default_allocator);
        ...
        if (fun.cli.Command.get().args.serve_minify_identifiers) |minify_identifiers| {
            config.minify.identifiers = minify_identifiers;
        } else if (!is_development) {
            config.minify.identifiers = true;
        }

        if (fun.cli.Command.get().args.serve_minify_whitespace) |minify_whitespace| {
            config.minify.whitespace = minify_whitespace;
        } else if (!is_development) {
            config.minify.whitespace = true;
        }

        if (fun.cli.Command.get().args.serve_minify_syntax) |minify_syntax| {
            config.minify.syntax = minify_syntax;
        } else if (!is_development) {
            config.minify.syntax = true;
        }

        if (fun.cli.Command.get().args.serve_define) |define| {
            fun.assert(define.keys.len == define.values.len);
            ...
            for (config.define.map.values(), define.values) |*to, from| {
                to.* = fun.handleOom(config.define.map.allocator.dupe(u8, from));
            }
            try config.define.map.reIndex();
        }

        if (!is_development) {
            fun.handleOom(config.define.put("process.env.NODE_ENV", "\"production\""));
        ...
        const completion_task = try fun.BundleV2.createAndScheduleCompletionTask(
            config,
            plugins,
            global,
            vm.eventLoop(),
        ...
        completion_task.started_at_ns = fun.getRoughTickCount(.allow_mocked_time).ns();
    ...
    pub fn onPluginsRejected(this: *Route) !void {
        ...
        this.state = .{ .err = fun.logger.Log.init(fun.default_allocator) };
        this.resumePendingResponses();
    }

    pub fn onComplete(this: *Route, completion_task: *fun.BundleV2.JSBundleCompletionTask) void {
        // For the build task.
        defer this.deref();
        ...
        switch (completion_task.result) {
            .err => |err| {
                ...
                this.state = .{ .err = fun.logger.Log.init(fun.default_allocator) };
                fun.handleOom(completion_task.log.cloneToWithRecycled(&this.state.err, true));

                if (this.server) |server| {
                    if (server.config().isDevelopment()) {
                        switch (fun.Output.enable_ansi_colors_stderr) {
                            inline else => |enable_ansi_colors| {
                                const writer = fun.Output.errorWriterBuffered();
            ...
            .value => |bundle| {
                ...
                if (server.config().isDevelopment()) {
                    const now = fun.getRoughTickCount(.allow_mocked_time).ns();
                    const duration = now - completion_task.started_at_ns;
                    var duration_f64: f64 = @floatFromInt(duration);
                    duration_f64 /= std.time.ns_per_s;

                    fun.Output.printElapsed(duration_f64);
                    var byte_length: u64 = 0;
                    for (output_files) |*output_file| {
                        byte_length += output_file.size_without_sourcemap;
                    }

                    fun.Output.prettyErrorln(" <green>bundle<r> {s} <d>{d:.2} KB<r>", .{ std.fs.path.basename(this.bundle.data.path), @as(f64, @floatFromInt(byte_length)) / 1000.0 });
                    fun.Output.flush();
                }
                ...
                for (output_files) |*output_file| {
                    const blob = jsc.WebCore.Blob.Any{ .Blob = fun.handleOom(output_file.toBlob(fun.default_allocator, globalThis)) };
                    var headers = fun.http.Headers{ .allocator = fun.default_allocator };
                    const content_type = blob.Blob.contentTypeOrMimeType() orelse brk: {
                        fun.debugAssert(false); // should be populated by `output_file.toBlob`
                        break :brk output_file.loader.toMimeType(&.{}).value;
                    };
                    fun.handleOom(headers.append("Content-Type", content_type));
                    ...
                    if (output_file.loader != .html and output_file.value == .buffer) {
                        ...
                        const etag_str = std.fmt.bufPrint(
                            &hashbuf,
                            "{f}",
                            .{fun.fmt.hexIntLower(output_file.hash)},
                        ) catch |err| switch (err) {
                            error.NoSpaceLeft => unreachable,
                        };
                        fun.handleOom(headers.append("ETag", etag_str));
                        if (!server.config().isDevelopment() and (output_file.output_kind == .chunk))
                            fun.handleOom(headers.append("Cache-Control", "public, max-age=31536000"));
                    }
                    ...
                    if (server.config().isDevelopment()) {
                        if (output_file.source_map_index != std.math.maxInt(u32)) {
                            ...
                            fun.handleOom(headers.append("SourceMap", route_path));
                        }
                    }

                    const static_route = fun.new(StaticRoute, .{
                        .ref_count = .init(),
                        .blob = blob,
                        .server = server,
                        .status_code = 200,
                    ...
                    fun.handleOom(server.appendStaticRoute(route_path, .{ .static = static_route }, .any));
                }
                ...
                const html_route_clone = fun.handleOom(html_route.clone(globalThis));
                this.state = .{ .html = html_route_clone };

                if (!fun.handleOom(server.reloadStaticRoutes())) {
    ...
    pub const PendingResponse = struct {
        ...
        pub fn onAborted(this: *PendingResponse, _: HTTPResponse) void {
            fun.debugAssert(this.is_response_pending == true);
...
const debug = fun.Output.scoped(.HTMLBundle, .hidden);
...
const uws = fun.uws;
const HTTPResponse = fun.uws.AnyResponse;
