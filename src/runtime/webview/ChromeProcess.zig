...
/// The C++ side doesn't touch JS state; EVFILT_PROC → Fun__Chrome__died →
/// rejectAllAndMarkDead handles promise rejection on the next loop tick.
pub export fn Fun__Chrome__kill() void {
    if (instance) |i| {
        _ = i.process.kill(9);
    }
}

/// Lazy: first `new Fun.WebView({ backend: "chrome" })` calls this via
...
pub export fn Fun__Chrome__ensure(
    ...
    if (comptime fun.Environment.isWindows) return -1;
    if (instance != null) return -1; // C++ already holds the fd

    const extra: []const [*:0]const u8 = if (extraArgv) |a| a[0..extraArgvLen] else &.{};
    const fd = spawn(global.funVM(), userDataDir, path, extra, stdoutInherit, stderrInherit) catch |err| {
...
pub fn onProcessExit(this: *ChromeProcess, _: *fun.spawn.Process, status: fun.spawn.Status, _: *const fun.spawn.Rusage) void {
    log("chrome exited: {f}", .{status});
    const signo: i32 = if (status.signalCode()) |sig| @intFromEnum(sig) else 0;
    Fun__Chrome__died(signo);
    this.process.deref();
    fun.destroy(this);
    instance = null;
}
...
fn findChrome(alloc: std.mem.Allocator, explicitPath: ?[*:0]const u8) !?[:0]const u8 {
    // Precedence: backend.path > FUN_CHROME_PATH > $PATH > hardcoded > playwright.
    // backend.path is per-Fun.WebView call (first wins — later views reuse
    // the already-spawned Chrome); env var is per-process.
    if (explicitPath) |p| {
        return try alloc.dupeZ(u8, std.mem.span(p));
    }
    if (std.process.getEnvVarOwned(alloc, "FUN_CHROME_PATH")) |p| {
    ...
    const path = fun.env_var.PATH.get() orelse "";
    ...
    if (comptime fun.Environment.isMac) {
        ...
        for (bundles) |b| {
            ...
            const sys = fun.path.joinStringBufZ(buf, &sys_parts, .auto);
            if (fun.sys.isExecutableFilePath(sys)) return try alloc.dupeZ(u8, sys);
            if (home.len > 0) {
                const user_parts = [_][]const u8{ home, "Applications", b };
                const user = fun.path.joinStringBufZ(buf, &user_parts, .auto);
                if (fun.sys.isExecutableFilePath(user)) return try alloc.dupeZ(u8, user);
            }
        }
        ...
        for (absolute) |c| {
            if (fun.sys.isExecutableFilePath(c)) return try alloc.dupeZ(u8, c);
        }
    }
...
fn findPlaywrightShell(alloc: std.mem.Allocator) ?[:0]const u8 {
    const home = fun.env_var.HOME.get() orelse return null;

    const dir_buf = fun.path_buffer_pool.get();
    defer fun.path_buffer_pool.put(dir_buf);
    const cache_subpath = if (comptime fun.Environment.isMac)
        "Library/Caches/ms-playwright"
    else
        ".cache/ms-playwright";
    const parts = [_][]const u8{ home, cache_subpath };
    const cache_dir = fun.path.joinStringBufZ(dir_buf, &parts, .auto);

    const fd = switch (fun.sys.open(cache_dir, fun.O.RDONLY | fun.O.DIRECTORY, 0)) {
        .result => |f| f,
        .err => return null,
    };
    defer fd.close();
    ...
    var iter = fun.DirIterator.iterate(fd, .u8);
    while (iter.next().unwrap() catch return null) |entry| {
        if (entry.kind != .directory) continue;
        const name = entry.name.slice();
        if (!fun.strings.hasPrefixComptime(name, prefix)) continue;
    ...
    const arch = if (comptime fun.Environment.isAarch64) "arm64" else "x64";
    const plat = if (comptime fun.Environment.isMac) "mac" else "linux";
    ...
    const bin_buf = fun.path_buffer_pool.get();
    defer fun.path_buffer_pool.put(bin_buf);
    const bin_parts = [_][]const u8{ cache_dir, best_name[0..best_len], subdir_cft };
    const bin = fun.path.joinStringBufZ(bin_buf, &bin_parts, .auto);
    if (fun.sys.isExecutableFilePath(bin)) return alloc.dupeZ(u8, bin) catch return null;

    // Fall back to the non-cft linux arm64 layout.
    if (comptime fun.Environment.isLinux and fun.Environment.isAarch64) {
        const bin_parts2 = [_][]const u8{ cache_dir, best_name[0..best_len], "chrome-linux/headless_shell" };
        const bin2 = fun.path.joinStringBufZ(bin_buf, &bin_parts2, .auto);
        if (fun.sys.isExecutableFilePath(bin2)) return alloc.dupeZ(u8, bin2) catch return null;
    }
    return null;
}

fn spawn(vm: *jsc.VirtualMachine, userDataDir: ?[*:0]const u8, explicitPath: ?[*:0]const u8, extraArgv: []const [*:0]const u8, stdoutInherit: bool, stderrInherit: bool) !fun.FD {
    if (comptime fun.Environment.isWindows) return error.Unsupported;
    ...
    const dataDir = if (userDataDir) |d|
        ...
        // multiple Fun.WebView instances in one process share the Chrome.
        const pid: u32 = @intCast(std.c.getpid());
        break :blk try std.fmt.allocPrintSentinel(alloc, "--user-data-dir=/tmp/fun-chrome-{d}", .{pid}, 0);
    };
    ...
    // CString Vector — lives until Fun__Chrome__ensure returns, after which
    ...
    const self = fun.new(ChromeProcess, .{
    ...
    switch (self.process.watch()) {
        .result => {
            ...
            // dispatchOnExit also SIGKILLs via Fun__Chrome__kill.
...
extern fn Fun__Chrome__died(signo: i32) void;
...
fn readDevToolsActivePort(out_buf: *std.ArrayListUnmanaged(u8)) ?void {
    ...
    const root = if (comptime fun.Environment.isWindows)
        fun.getenvZ("LOCALAPPDATA") orelse return null
    else
        fun.getenvZ("HOME") orelse return null;
    const candidates: []const []const u8 = if (comptime fun.Environment.isMac) &.{
    ...
    for (candidates) |rel| {
        const path = fun.path.joinAbsStringBufZ(root, &path_buf, &.{rel}, .auto);
        ...
        out_buf.writer(fun.default_allocator).print("ws://127.0.0.1:{d}{s}", .{ port, ws_path }) catch return null;
        return;
    }
    return null;
}
...
pub export fn Fun__Chrome__autoDetect(out_buf: [*]u8, out_cap: usize) usize {
...
const log = fun.Output.scoped(.Chrome, .hidden);
