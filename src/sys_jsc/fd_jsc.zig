//! JSC bridge for `fun.FD`. Keeps `src/sys/` free of JSC types.
...
pub fn fromJSValidated(value: JSValue, global: *jsc.JSGlobalObject) fun.JSError!?FD {
    if (!value.isNumber())
        return null;
...
pub fn toJS(any_fd: FD, global: *jsc.JSGlobalObject) JSValue {
    ...
    const uv_owned_fd = any_fd.makeLibUVOwned() catch {
        ...
        const err_instance = (jsc.SystemError{
            .message = fun.String.static("EMFILE, too many open files"),
            .code = fun.String.static("EMFILE"),
