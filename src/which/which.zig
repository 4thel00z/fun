fn isValid(buf: *fun.PathBuffer, segment: []const u8, bin: []const u8) ?u16 {
    ...
    if (len_z > fun.MAX_PATH_BYTES) return null;

    fun.copy(u8, buf, segment);
    buf[segment.len] = std.fs.path.sep;
    fun.copy(u8, buf[prefix_len..], bin);
    buf[len] = 0;
    const filepath = buf[0..len :0];
    if (!fun.sys.isExecutableFilePath(filepath)) return null;
    return @intCast(filepath.len);
}

// Like /usr/bin/which but without needing to exec a child process
// Remember to resolve the symlink if necessary
pub fn which(buf: *fun.PathBuffer, path: []const u8, cwd: []const u8, bin: []const u8) ?[:0]const u8 {
    if (bin.len > fun.MAX_PATH_BYTES) return null;
    fun.Output.scoped(.which, .hidden)("path={s} cwd={s} bin={s}", .{ path, cwd, bin });

    if (fun.Environment.os == .windows) {
        const convert_buf = fun.w_path_buffer_pool.get();
        defer fun.w_path_buffer_pool.put(convert_buf);
        const result = whichWin(convert_buf, path, cwd, bin) orelse return null;
        const result_converted = fun.strings.convertUTF16toUTF8InBuffer(buf, result) catch unreachable;
        buf[result_converted.len] = 0;
        fun.assert(result_converted.ptr == buf.ptr);
    ...
    if (std.fs.path.isAbsolute(bin)) {
        ...
        if (fun.sys.isExecutableFilePath(binZ)) return binZ;
        // Do not look absolute paths in $PATH
        return null;
    }

    if (fun.strings.containsChar(bin, '/')) {
        if (cwd.len > 0) {
            if (isValid(
                buf,
                std.mem.trimRight(u8, cwd, std.fs.path.sep_str),
                fun.strings.withoutPrefixComptime(bin, "./"),
            )) |len| {
                return buf[0..len :0];
            }
        }
...
pub fn endsWithExtension(str: []const u8) bool {
    ...
    inline for (win_extensions) |ext| {
        comptime fun.assert(ext.len == 3);
        if (fun.strings.eqlCaseInsensitiveASCIIICheckLength(file_ext, ext)) return true;
    }
    return false;
}
...
fn searchBin(buf: *fun.WPathBuffer, path_size: usize, check_windows_extensions: bool) ?[:0]u16 {
    if (!check_windows_extensions)
        ...
        if (fun.sys.existsOSPath(buf[0..path_size :0], true))
    ...
    if (check_windows_extensions) {
        ...
        inline for (win_extensionsW) |ext| {
            ...
            if (fun.sys.existsOSPath(buf[0 .. path_size + 1 + ext.len :0], true))
...
fn searchBinInPath(buf: *fun.WPathBuffer, path_buf: *fun.PathBuffer, path: []const u8, bin: []const u8, check_windows_extensions: bool) ?[:0]u16 {
    ...
    const segment_utf16 = fun.strings.convertUTF8toUTF16InBuffer(buf, fun.strings.withoutTrailingSlash(segment));

    buf[segment_utf16.len] = std.fs.path.sep;

    const bin_utf16 = fun.strings.convertUTF8toUTF16InBuffer(buf[segment_utf16.len + 1 ..], bin);
...
pub fn whichWin(buf: *fun.WPathBuffer, path: []const u8, cwd: []const u8, bin: []const u8) ?[:0]const u16 {
    if (bin.len == 0) return null;
    ...
    if (std.fs.path.isAbsolute(bin)) {
        ...
        const bin_utf16 = fun.strings.convertUTF8toUTF16InBuffer(buf, normalized_bin);
        buf[bin_utf16.len] = 0;
        return searchBin(buf, bin_utf16.len, check_windows_extensions);
    }

    // check if bin is in cwd
    if (fun.strings.containsChar(bin, '/') or fun.strings.containsChar(bin, '\\')) {
        if (searchBinInPath(
            buf,
            path_buf,
            cwd,
            ...
            fun.path.posixToPlatformInPlace(u16, bin_path);
            return bin_path;
        }
        // Do not lookup paths with slashes in $PATH
        return null;
