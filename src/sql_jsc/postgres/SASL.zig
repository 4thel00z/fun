...
const nonce_base64_len = fun.base64.encodeLenFromSize(nonce_byte_len);

const server_signature_byte_len = 32;
const server_signature_base64_len = fun.base64.encodeLenFromSize(server_signature_byte_len);
...
fn hmac(password: []const u8, data: []const u8) ?[32]u8 {
    var buf = std.mem.zeroes([fun.BoringSSL.c.EVP_MAX_MD_SIZE]u8);

    // TODO: I don't think this is failable.
    const result = fun.hmac.generate(password, data, .sha256, &buf) orelse return null;
...
pub fn computeServerSignature(this: *SASL, auth_string: []const u8) !void {
    ...
    this.server_signature_len = @intCast(fun.base64.encode(&this.server_signature_base64_bytes, &server_signature_bytes));
}
...
pub fn clientKeySignature(_: *const SASL, client_key: []const u8, auth_string: []const u8) [32]u8 {
    ...
    fun.sha.SHA256.hash(client_key, &sha_digest, jsc.VirtualMachine.get().rareData().boringEngine());
...
pub fn nonce(this: *SASL) []const u8 {
    if (this.nonce_len == 0) {
        ...
        fun.csprng(&bytes);
        this.nonce_len = @intCast(fun.base64.encode(&this.nonce_base64_bytes, &bytes));
    }
