    ...
    pub fn toCss(this: *const @This(), dest: *Printer) PrintErr!void {
        switch (this.name) {
            ...
            inline .ident, .string => |name| {
                ...
                if (!this.iteration_count.eql(&AnimationIterationCount.default()) or fun.strings.eqlCaseInsensitiveASCII(name_str, "infinite")) {
                ...
                if (!this.fill_mode.eql(&AnimationFillMode.default()) or
                    (!fun.strings.eqlCaseInsensitiveASCII(name_str, "none") and css.parse_utility.parseString(dest.allocator, AnimationFillMode, name_str, AnimationFillMode.parse).isOk()))
                {
