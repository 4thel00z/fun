//! JSC bridge for `fun.install.PackageManager.UpdateRequest`.

pub fn fromJS(globalThis: *jsc.JSGlobalObject, input: jsc.JSValue) fun.JSError!jsc.JSValue {
    ...
    const update_requests = UpdateRequest.parseWithError(allocator, null, &log, all_positionals.items, &array, .add, false) catch {
        return globalThis.throwValue(try log.toJS(globalThis, fun.default_allocator, "Failed to parse dependencies"));
    };
    ...
    var name_str = fun.String.init(update_requests[0].name);
