...
#include "FunBuiltinNames+extras.h"
...
#define FUN_COMMON_PRIVATE_IDENTIFIERS_EACH_PROPERTY_NAME(macro) \
    macro(AbortSignal) \
    macro(Buffer) \
    macro(Loader) \
    ...
    macro(_debugStack) \
    macro(_debugTask) \
    FUN_ADDITIONAL_BUILTIN_NAMES(macro)
// --- END of FUN_COMMON_PRIVATE_IDENTIFIERS_EACH_PROPERTY_NAME ---

class FunBuiltinNames {
    WTF_MAKE_NONCOPYABLE(FunBuiltinNames);
    friend class JSVMClientData;
    explicit FunBuiltinNames(JSC::VM&);
    ~FunBuiltinNames();

public:
    FUN_COMMON_PRIVATE_IDENTIFIERS_EACH_PROPERTY_NAME(DECLARE_BUILTIN_IDENTIFIER_ACCESSOR)
...
private:
    ...
    FUN_COMMON_PRIVATE_IDENTIFIERS_EACH_PROPERTY_NAME(DECLARE_BUILTIN_NAMES)
};
