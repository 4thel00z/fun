...
const AllocatorInterface = struct {
    ...
    fn free(
        ...
        fun.sys.munmap(@ptrCast(@alignCast(buf))).unwrap() catch |err| {
            fun.Output.debugWarn("Failed to munmap memfd: {}", .{err});
        };
    }
...
pub fn alloc(self: *Self, len: usize, offset: usize, flags: std.posix.MAP) fun.sys.Maybe(fun.webcore.Blob.Store.Bytes) {
    var size = len;
    ...
    switch (fun.sys.mmap(
        ...
        .result => |slice| {
            return .{
                .result = fun.webcore.Blob.Store.Bytes{
...
pub fn shouldUse(bytes: []const u8) bool {
    ...
    if (fun.jsc.VirtualMachine.is_smol_mode) {
...
pub fn create(bytes: []const u8) fun.sys.Maybe(fun.webcore.Blob.Store.Bytes) {
    if (comptime !fun.Environment.isLinux) {
        unreachable;
    }
    ...
    const fd = switch (fun.sys.memfd_create(label, .non_executable)) {
        .err => |err| return .{ .err = fun.sys.Error.fromCode(err.getErrno(), .open) },
        .result => |fd| fd,
    };

    if (bytes.len > 0)
        // Hint at the size of the file
        _ = fun.sys.ftruncate(fd, @intCast(bytes.len));
    ...
    while (remain.len > 0) {
        switch (fun.sys.pwrite(fd, remain, written)) {
            .err => |err| {
                ...
                fun.Output.debugWarn("Failed to write to memfd: {f}", .{err});
                fd.close();
                return .{ .err = err };
            },
            .result => |result| {
                if (result == 0) {
                    fun.Output.debugWarn("Failed to write to memfd: EOF", .{});
                    fd.close();
                    return .{ .err = fun.sys.Error.fromCode(.NOMEM, .write) };
                }
