...
fn heapLabel(comptime T: type) [:0]const u8 {
    const base_name = if (comptime fun.meta.hasDecl(T, "heap_label"))
        T.heap_label
    else
        fun.meta.typeBaseName(@typeName(T));
    return base_name;
}
...
pub fn namedAllocator(comptime name: [:0]const u8) std.mem.Allocator {
    return getZone("Fun__" ++ name).allocator();
}
...
pub const Zone = opaque {
    ...
    pub inline fn create(zone: *Zone, comptime T: type, data: T) *T {
        return fun.handleOom(zone.tryCreate(T, data));
    }
