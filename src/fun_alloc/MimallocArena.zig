...
pub const Borrowed = struct {
    ...
    pub fn helpCatchMemoryIssues(self: Borrowed) void {
        if (comptime fun.FeatureFlags.help_catch_memory_issues) {
            self.gc();
...
const log = fun.Output.scoped(.mimalloc, .hidden);
...
pub fn getThreadLocalDefault() std.mem.Allocator {
    if (fun.Environment.enable_asan) return fun.default_allocator;
    return .{ .ptr = undefined, .vtable = &global_mimalloc_vtable };
}
...
pub fn dumpThreadStats(_: Self) void {
    const dump_fn = struct {
        pub fn dump(textZ: [*:0]const u8, _: ?*anyopaque) callconv(.c) void {
            const text = fun.span(textZ);
            fun.Output.errorWriter().writeAll(text) catch {};
        }
    }.dump;
...
pub fn init() Self {
    const mimalloc_heap = mimalloc.mi_heap_new() orelse fun.outOfMemory();
...
const safety_checks = fun.Environment.ci_assert;
