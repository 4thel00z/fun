...
/// like `AllocationScopeIn(fun.DefaultAllocator)`.
...
pub const enabled = fun.Environment.enableAllocScopes;
...
const LockedState = struct {
    ...
    fn alloc(self: Self, len: usize, alignment: std.mem.Alignment, ret_addr: usize) fun.OOM![*]u8 {
    ...
    fn free(self: Self, buf: []u8, alignment: std.mem.Alignment, ret_addr: usize) void {
        ...
        if (success or fun.Environment.enable_asan) {
    ...
    fn trackAllocation(self: Self, buf: []const u8, ret_addr: usize, extra: Extra) fun.OOM!void {
    ...
    fn trackFree(self: Self, buf: []const u8, ret_addr: usize) FreeError!void {
        const entry = self.history.allocations.fetchRemove(buf.ptr) orelse {
            ...
            if (self.history.frees.getPtr(buf.ptr)) |free_entry| {
                ...
                fun.crash_handler.dumpStackTrace(free_entry.allocated_at.trace(), trace_limits);
                Output.printErrorln("Pointer first freed here:", .{});
                fun.crash_handler.dumpStackTrace(free_entry.freed_at.trace(), free_trace_limits);
            }
...
const State = struct {
    ...
    parent: std.mem.Allocator,
    history: fun.threading.Guarded(History),
    ...
    pub fn deinit(self: *Self) void {
        ...
        Output.errGeneric("Allocation scope leaked {d} allocations ({f})", .{
            count,
            fun.fmt.size(history.total_memory_allocated, .{}),
        });
...
pub fn AllocationScopeIn(comptime Allocator: type) type {
    const BorrowedAllocator = fun.allocators.Borrowed(Allocator);
    ...
    const BorrowedScope = struct {
        ...
        pub fn allocator(self: Self) std.mem.Allocator {
            return if (comptime enabled)
                ...
                fun.allocators.asStd(self.#parent);
        }
        ...
        ///
        /// This method doesn't need to be called unless `fun.allocators.Borrowed(Allocator)` has
        /// a `deinit` method.
        pub fn deinit(self: *Self) void {
            fun.memory.deinit(&self.#parent);
            self.* = undefined;
        }
        ...
        fn downcastImpl(
            ...
            const new_std_parent = fun.allocators.asStd(new_parent);
            fun.safety.alloc.assertEqFmt(
                current_std_parent,
                new_std_parent,
        ...
        ///   In particular, `fun.allocators.asStd` must return the same value for each allocator.
        ...
        ///      `fun.memory.initDefault(fun.allocators.Borrowed(Allocator))`. This is the case
        ///      for `fun.DefaultAllocator`.
        pub fn downcast(std_alloc: std.mem.Allocator) Self {
            return downcastImpl(std_alloc, if (comptime Allocator == std.mem.Allocator)
                null
            else
                fun.memory.initDefault(BorrowedAllocator));
        }
    };
    ...
    return struct {
        ...
        pub fn init(parent_alloc: Allocator) Self {
            return .{
                ...
                .#state = if (comptime Self.enabled) .new(.init(
                    fun.allocators.asStd(parent_alloc),
                )),
            };
        }

        pub fn initDefault() Self {
            return .init(fun.memory.initDefault(Allocator));
        }
        ...
        pub fn parent(self: Self) BorrowedAllocator {
            return fun.allocators.borrow(self.#parent);
        }
...
pub const trace_limits: fun.crash_handler.WriteStackTraceLimits = .{
    .frame_count = 6,
    .stop_at_jsc_llint = true,
    .skip_stdlib = true,
};

pub const free_trace_limits: fun.crash_handler.WriteStackTraceLimits = .{
    .frame_count = 3,
    .stop_at_jsc_llint = true,
    .skip_stdlib = true,
};
