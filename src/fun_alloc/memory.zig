...
/// memory with `value`.
///
/// If `allocator` is `fun.default_allocator`, this will internally use `fun.tryNew` to benefit from
/// the added assertions.
pub fn create(comptime T: type, allocator: std.mem.Allocator, value: T) fun.OOM!*T {
    if ((comptime Environment.allow_assert) and isDefault(allocator)) {
        return fun.tryNew(T, value);
    }
...
///     calls `fun.memory.deinit` with a pointer to the payload.
/// * If `ptr_or_slice` is a slice, for each element of the slice, calls `fun.memory.deinit` with
...
const isDefault = fun.allocators.isDefault;
