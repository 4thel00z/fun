    ...
    return struct {
        ...
        const Block = struct {
            ...
            pub fn append(block: *Block, value: ValueType) *ValueType {
                if (comptime Environment.allow_assert) fun.assert(block.used < count);
        ...
        pub inline fn atIndex(this: *const This, index: IndexType) *const ValueType {
            ...
            if (comptime Environment.allow_assert) fun.assert(index.is_overflow);
            if (comptime Environment.allow_assert) fun.assert(this.list.used >= block_id);
            if (comptime Environment.allow_assert) fun.assert(this.list.ptrs[block_id].used > (index.index % count));
    ...
    return struct {
        ...
        const OverflowBlock = struct {
            ...
            pub inline fn zero(this: *OverflowBlock) void {
                ...
                // This makes Fun start about 1ms faster.
        ...
        pub fn init(allocator: std.mem.Allocator) *Self {
            if (!loaded) {
                instance = fun.handleOom(fun.default_allocator.create(Self));
                // Avoid struct initialization syntax.
                // This makes Fun start about 1ms faster.
                // https://github.com/ziglang/zig/issues/24313
        ...
        pub fn deinit(self: *Self) void {
            self.head.deinit();
            fun.default_allocator.destroy(instance);
            loaded = false;
        }
...
pub fn BSSStringList(comptime _count: usize, comptime _item_length: usize) type {
    // I experimented with string interning here and it was around...maybe 1% when generating a .fun?
    // I tried:
    // - arraybacked list
    // - hashmap list
    ...
    return struct {
        ...
        pub fn appendMutable(self: *Self, comptime AppendType: type, _value: AppendType) OOM![]u8 {
            const appended = try @call(fun.callmod_inline, append, .{ self, AppendType, _value });
        ...
        const lowercase_bufs = fun.ThreadlocalBuffers(struct { buf: fun.PathBuffer = undefined });
            ...
            if (value_len + instance.backing_buf_used < instance.backing_buf.len - 1) {
                ...
                switch (AppendType) {
                    ...
                    []const u8, []u8, [:0]const u8, [:0]u8 => {
                        fun.copy(u8, instance.backing_buf[start .. instance.backing_buf_used - 1], _value);
                        instance.backing_buf[instance.backing_buf_used - 1] = 0;
                    },
                    else => {
                        var remainder = instance.backing_buf[start..];
                        for (_value) |val| {
                            fun.copy(u8, remainder, val);
...
pub fn BSSMap(comptime ValueType: type, comptime count: anytype, comptime store_keys: bool, comptime estimated_key_length: usize, comptime remove_trailing_slashes: bool) type {
    ...
    return struct {
        ...
        pub fn get(self: *Self, key: []const u8) ?*ValueType {
            return @call(fun.callmod_inline, BSSMapType.get, .{ self.map, key });
        }

        pub fn atIndex(self: *Self, index: IndexType) ?*ValueType {
            return @call(fun.callmod_inline, BSSMapType.atIndex, .{ self.map, index });
        }
...
const basic = if (fun.use_mimalloc)
    @import("./basic.zig")
else
    @import("./fallback.zig");
