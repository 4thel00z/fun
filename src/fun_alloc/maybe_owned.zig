/// This type can be used with `fun.ptr.Owned` to model "maybe owned" pointers:
///
/// ```
/// // Either owned by the default allocator, or borrowed
/// const MaybeOwnedFoo = fun.ptr.OwnedIn(*Foo, fun.allocators.MaybeOwned(fun.DefaultAllocator));
///
...
pub fn MaybeOwned(comptime Allocator: type) type {
    return struct {
        ...
        _parent: fun.allocators.Nullable(Allocator),
        ...
        pub fn init() Self {
            return .initOwned(fun.memory.initDefault(Allocator));
        }
        ...
        pub fn deinit(self: *Self) void {
            ...
            if (maybe_parent) |*parent_alloc| {
                fun.memory.deinit(parent_alloc);
            }
        }
        ...
        const BorrowedParent = fun.allocators.Borrowed(Allocator);

        pub fn parent(self: Self) ?BorrowedParent {
            const maybe_parent = self.rawParent();
            return if (maybe_parent) |parent_alloc|
                fun.allocators.borrow(parent_alloc)
            else
                null;
        }
        ...
        /// Used by smart pointer types and allocator wrappers. See `fun.allocators.borrow`.
        pub const Borrowed = MaybeOwned(BorrowedParent);

        pub fn borrow(self: Self) Borrowed {
            return .{ ._parent = fun.allocators.initNullable(BorrowedParent, self.parent()) };
        }

        fn initRaw(parent_alloc: ?Allocator) Self {
            return .{ ._parent = fun.allocators.initNullable(Allocator, parent_alloc) };
        }

        fn rawParent(self: Self) ?Allocator {
            return fun.allocators.unpackNullable(Allocator, self._parent);
        }
    };
}
