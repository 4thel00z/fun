...
pub const BundlePackageOverride = fun.StringArrayHashMapUnmanaged(options.BundleOverride);
const LoaderMap = fun.StringArrayHashMapUnmanaged(options.Loader);

// TODO: replace api.TransformOptions with Funfig
pub const Funfig = struct {
    ...
    pub const Parser = struct {
        ...
        fn loadLogLevel(this: *Parser, expr: js_ast.Expr) !void {
            ...
            this.funfig.log_level = switch (Matcher.match(expr.asString(this.allocator).?)) {
        ...
        fn loadEnvConfig(this: *Parser, expr: js_ast.Expr) !void {
            switch (expr.data) {
                .e_null => {
                    ...
                    this.funfig.disable_default_env_files = true;
                },
                .e_boolean => |boolean| {
        ...
        pub fn parse(this: *Parser, comptime cmd: Command.Tag) !void {
            fun.analytics.Features.funfig += 1;

            const json = this.json;
            var allocator = this.allocator;

            if (json.data != .e_object) {
                try this.addError(json.loc, "funfig expects an object { } at the root");
            }
            ...
            if (json.get("define")) |expr| {
                ...
                this.funfig.define = api.StringMap{
                    .keys = keys,
                    .values = values,
                };
            }
            ...
            if (json.get("origin")) |expr| {
                try this.expectString(expr);
                this.funfig.origin = try expr.data.e_string.string(allocator);
            }
            ...
            if (comptime cmd == .RunCommand or cmd == .AutoCommand) {
                if (json.get("serve")) |expr| {
                    if (expr.get("port")) |port| {
                        ...
                        this.funfig.port = port.data.e_number.toU16();
                        if (this.funfig.port.? == 0) {
                            this.funfig.port = 3000;
                        }
                    }
                }
                ...
                if (json.get("telemetry")) |expr| {
                    ...
                    fun.analytics.enabled = if (expr.data.e_boolean.value) .yes else .no;
                }
            }
                ...
                if (json.get("test")) |test_| {
                    ...
                    if (test_.get("coverageReporter")) |expr| brk: {
                        ...
                        if (expr.data == .e_string) {
                            const item_str = expr.asString(fun.default_allocator) orelse "";
                            if (fun.strings.eqlComptime(item_str, "text")) {
                                this.ctx.test_options.coverage.reporters.text = true;
                            } else if (fun.strings.eqlComptime(item_str, "lcov")) {
                        ...
                        for (items) |item| {
                            try this.expectString(item);
                            const item_str = item.asString(fun.default_allocator) orelse "";
                            if (fun.strings.eqlComptime(item_str, "text")) {
                                this.ctx.test_options.coverage.reporters.text = true;
                            } else if (fun.strings.eqlComptime(item_str, "lcov")) {
                                this.ctx.test_options.coverage.reporters.lcov = true;
            ...
            if (comptime cmd.isNPMRelated() or cmd == .RunCommand or cmd == .AutoCommand or cmd == .TestCommand) {
                if (json.getObject("install")) |install_obj| {
                    var install: *api.FunInstall = this.ctx.install orelse brk: {
                        const install = try this.allocator.create(api.FunInstall);
                        install.* = std.mem.zeroes(api.FunInstall);
                    ...
                    if (install_obj.get("prefer")) |prefer_expr| {
                        ...
                        if (Prefer.get(prefer_expr.asString(fun.default_allocator) orelse "")) |setting| {
                    ...
                    if (install_obj.get("lockfile")) |lockfile_expr| {
                        if (lockfile_expr.get("print")) |lockfile| {
                            ...
                            if (lockfile.asString(this.allocator)) |value| {
                                if (!(strings.eqlComptime(value, "fun"))) {
                    ...
                    if (install_obj.get("publicHoistPattern")) |public_hoist_pattern_expr| {
                        install.public_hoist_pattern = fun.install.PnpmMatcher.fromExpr(
                            ...
                            error.UnexpectedExpr, error.InvalidRegExp => return error.@"Invalid Funfig",
                        };
                    }
                ...
                if (json.get("run")) |run_expr| {
                    ...
                    if (run_expr.get("shell")) |shell| {
                        if (shell.asString(allocator)) |value| {
                            if (strings.eqlComptime(value, "fun")) {
                                ...
                                try this.addError(shell.loc, "Invalid shell, only 'fun' and 'system' are supported");
                            }
                        } else {
                    ...
                    if (run_expr.get("fun")) |fun_flag| {
                        if (fun_flag.asBool()) |value| {
                            this.ctx.debug.run_in_fun = value;
                        } else {
                            try this.addError(fun_flag.loc, "Expected boolean");
                        }
                    }

                    if (run_expr.get("noOrphans")) |no_orphans| {
                        if (no_orphans.asBool()) |value| {
                            if (value) fun.ParentDeathWatchdog.enable();
                        } else {
            ...
            if (json.getObject("serve")) |serve_obj2| {
                if (serve_obj2.getObject("static")) |serve_obj| {
                    if (serve_obj.get("plugins")) |config_plugins| {
                        ...
                        this.funfig.serve_plugins = plugins;
                    }

                    if (serve_obj.get("hmr")) |hmr| {
                        if (hmr.asBool()) |value| {
                            this.funfig.serve_hmr = value;
                        }
                    }

                    if (serve_obj.get("minify")) |minify| {
                        if (minify.asBool()) |value| {
                            this.funfig.serve_minify_syntax = value;
                            this.funfig.serve_minify_whitespace = value;
                            this.funfig.serve_minify_identifiers = value;
                        } else if (minify.isObject()) {
                            if (minify.get("syntax")) |syntax| {
                                this.funfig.serve_minify_syntax = syntax.asBool() orelse false;
                            }

                            if (minify.get("whitespace")) |whitespace| {
                                this.funfig.serve_minify_whitespace = whitespace.asBool() orelse false;
                            }

                            if (minify.get("identifiers")) |identifiers| {
                                this.funfig.serve_minify_identifiers = identifiers.asBool() orelse false;
                            }
                        } else {
                    ...
                    if (serve_obj.get("define")) |expr| {
                        ...
                        this.funfig.serve_define = api.StringMap{
                            .keys = keys,
                            .values = values,
                        };
                    }
                    this.funfig.funfig_path = fun.handleOom(fun.default_allocator.dupe(u8, this.source.path.text));

                    if (serve_obj.get("publicPath")) |public_path| {
                        if (public_path.asString(allocator)) |value| {
                            this.funfig.serve_public_path = value;
                        }
                    }
                    ...
                    if (serve_obj.get("env")) |env| {
                        switch (env.data) {
                            .e_null => {
                                this.funfig.serve_env_behavior = .disable;
                            },
                            .e_boolean => |boolean| {
                                this.funfig.serve_env_behavior = if (boolean.value) .load_all else .disable;
                            },
                            .e_string => |str| {
                                if (str.eqlComptime("inline")) {
                                    this.funfig.serve_env_behavior = .load_all;
                                    ...
                                    if (strings.indexOfChar(slice, '*')) |asterisk| {
                                        if (asterisk > 0) {
                                            this.funfig.serve_env_prefix = slice[0..asterisk];
                                            this.funfig.serve_env_behavior = .prefix;
                                        } else {
                                            this.funfig.serve_env_behavior = .load_all;
                                        }
                                    } else {
            ...
            if (json.get("bundle")) |_fun| {
                if (comptime cmd == .BuildCommand or cmd == .RunCommand or cmd == .AutoCommand or cmd == .BuildCommand) {
                    if (_fun.get("outdir")) |dir| {
                        try this.expectString(dir);
                        this.funfig.output_dir = try dir.data.e_string.string(allocator);
                    }
                }

                if (comptime cmd == .BuildCommand) {
                    if (_fun.get("logLevel")) |expr2| {
                        try this.loadLogLevel(expr2);
                    }

                    if (_fun.get("entryPoints")) |entryPoints| {
                        ...
                        this.funfig.entry_points = names;
                    }

                    if (_fun.get("packages")) |expr| {
                        try this.expect(expr, .e_object);
                        var valid_count: usize = 0;
            ...
            if (this.funfig.jsx == null) {
                this.funfig.jsx = api.Jsx{
                ...
                var jsx: *api.Jsx = &this.funfig.jsx.?;
            ...
            if (json.get("external")) |expr| {
                switch (expr.data) {
                    .e_string => |str| {
                        ...
                        this.funfig.external = externals;
                    },
                    .e_array => |array| {
            ...
            if (json.get("loader")) |expr| {
                ...
                this.funfig.loaders = api.LoaderMap{
