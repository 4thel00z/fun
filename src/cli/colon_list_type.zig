    ...
    return struct {
        ...
        pub fn load(self: *@This(), input: []const string) !void {
            for (input, 0..) |str, i| {
                ...
                if (comptime t == fun.schema.api.Loader) {
                    if (str[0..midpoint].len > 0 and str[0] != '.') {
                        Output.prettyErrorln("<r><red>error<r><d>:<r> <b>file extension must start with a '.'<r> <d>(while mapping loader {f})<r>", .{fun.fmt.quote(str)});
                        Global.exit(1);
                    }
                }
                ...
                self.values[i] = value_resolver(str[midpoint + 1 .. str.len]) catch |err| {
                    if (err == error.InvalidLoader) {
                        Output.prettyErrorln("<r><red>error<r><d>:<r> <b>invalid loader {f}<r>, expected one of:{f}", .{ fun.fmt.quote(str[midpoint + 1 .. str.len]), fun.fmt.enumTagList(fun.options.Loader, .dash) });
                        Global.exit(1);
                    }
                    return err;
                };
