pub fn cloneActiveStrong() ?FunTestPtr {
    const runner = fun.jsc.Jest.Jest.runner orelse return null;
    return runner.fun_test_root.cloneActiveFile();
}
...
pub const js_fns = struct {
    ...
    fn getActiveTestRoot(globalThis: *jsc.JSGlobalObject, cfg: GetActiveCfg) fun.JSError!*FunTestRoot {
        if (fun.jsc.Jest.Jest.runner == null) {
            return globalThis.throw("Cannot use {f} outside of the test runner. Run \"fun test\" to run tests.", .{cfg.signature});
        }
        const funTestRoot = &fun.jsc.Jest.Jest.runner.?.fun_test_root;
    ...
    pub fn cloneActiveStrong(globalThis: *jsc.JSGlobalObject, cfg: GetActiveCfg) fun.JSError!FunTestPtr {
        const funTestRoot = try getActiveTestRoot(globalThis, cfg);
        const funTest = funTestRoot.cloneActiveFile() orelse {
    ...
    pub fn genericHook(comptime tag: @Type(.enum_literal)) type {
        return struct {
            pub fn hookFn(globalThis: *jsc.JSGlobalObject, callFrame: *jsc.CallFrame) fun.JSError!jsc.JSValue {
                group.begin(@src());
                defer group.end();
                errdefer group.log("ended in error", .{});

                var args = try ScopeFunctions.parseArguments(globalThis, callFrame, .{ .str = @tagName(tag) ++ "()" }, fun.default_allocator, .{ .callback = .require, .kind = .hook });
                defer args.deinit(fun.default_allocator);

                const has_done_parameter = if (args.callback) |callback| try callback.getLength(globalThis) > 0 else false;

                const funTestRoot = try getActiveTestRoot(globalThis, .{ .signature = .{ .str = @tagName(tag) ++ "()" }, .allow_in_preload = true });

                const cfg: ExecutionEntryCfg = .{
                    .has_done_parameter = has_done_parameter,
                    .timeout = args.options.timeout,
                };
                const funTest = funTestRoot.getActiveFileUnlessInPreload(globalThis.funVM()) orelse {
                    ...
                    _ = try funTestRoot.hook_scope.appendHook(funTestRoot.gpa, tag, args.callback, cfg, .{}, .preload);
                    return .js_undefined;
                };

                switch (funTest.phase) {
                    .collection => {
                        if (tag == .onTestFinished) {
                            return globalThis.throw("Cannot call {s}() outside of a test. It can only be called inside a test.", .{@tagName(tag)});
                        }
                        _ = try funTest.collection.active_scope.appendHook(funTest.gpa, tag, args.callback, cfg, .{}, .collection);

                        return .js_undefined;
                    },
                    .execution => {
                        const active = funTest.getCurrentStateData();
                        const sequence, _ = funTest.execution.getCurrentAndValidExecutionSequence(active) orelse {
                        ...
                        const new_item = ExecutionEntry.create(funTest.gpa, null, args.callback, cfg, null, .{}, .execution);
                        new_item.next = append_point.next;
                        append_point.next = new_item;
                        fun.handleOom(funTest.extra_execution_entries.append(new_item));
...
pub const FunTestPtr = fun.ptr.shared.WithOptions(*FunTest, .{
    .allow_weak = true,
    .Allocator = fun.DefaultAllocator,
});
pub const FunTestRoot = struct {
    gpa: std.mem.Allocator,
    active_file: FunTestPtr.Optional,

    hook_scope: *DescribeScope,

    pub fn init(outer_gpa: std.mem.Allocator) FunTestRoot {
        const gpa = outer_gpa;
    ...
    pub fn deinit(this: *FunTestRoot) void {
        fun.assert(this.hook_scope.entries.items.len == 0); // entries must not be appended to the hook_scope
        this.hook_scope.destroy(this.gpa);
        fun.assert(this.active_file == null);
    }
    ...
    pub fn resetHookScopeForTestIsolation(this: *FunTestRoot) void {
        fun.assert(this.hook_scope.entries.items.len == 0);
        this.hook_scope.destroy(this.gpa);
    ...
    pub fn enterFile(this: *FunTestRoot, file_id: jsc.Jest.TestRunner.File.ID, reporter: *test_command.CommandLineReporter, default_concurrent: bool, first_last: FirstLast) void {
        group.begin(@src());
        defer group.end();

        fun.assert(this.active_file.get() == null);

        this.active_file = .new(undefined);
        this.active_file.get().?.init(this.gpa, this, file_id, reporter, default_concurrent, first_last);
    }
    pub fn exitFile(this: *FunTestRoot) void {
        group.begin(@src());
        defer group.end();
    ...
    pub fn getActiveFileUnlessInPreload(this: *FunTestRoot, vm: *jsc.VirtualMachine) ?*FunTest {
        if (vm.is_in_preload) {
            return null;
        }
        return this.active_file.get();
    }
    pub fn cloneActiveFile(this: *FunTestRoot) ?FunTestPtr {
    ...
    pub fn onBeforePrint(this: *FunTestRoot) void {
...
pub const FunTest = struct {
    fun_test_root: *FunTestRoot,
    in_run_loop: bool,
    allocation_scope: fun.AllocationScope,
    gpa: std.mem.Allocator,
    arena_allocator: std.heap.ArenaAllocator,
    arena: std.mem.Allocator,
    file_id: jsc.Jest.TestRunner.File.ID,
    /// null if the runner has moved on to the next file but a strong reference to FunTest is stll keeping it alive
    reporter: ?*test_command.CommandLineReporter,
    timer: fun.api.Timer.EventLoopTimer = .{ .next = .epoch, .tag = .FunTest },
    result_queue: ResultQueue,
    /// Whether tests in this file should default to concurrent execution
    default_concurrent: bool,
    first_last: FunTestRoot.FirstLast,
    ...
    pub fn init(this: *FunTest, outer_gpa: std.mem.Allocator, funTest: *FunTestRoot, file_id: jsc.Jest.TestRunner.File.ID, reporter: *test_command.CommandLineReporter, default_concurrent: bool, first_last: FunTestRoot.FirstLast) void {
        group.begin(@src());
        defer group.end();
        ...
        this.* = .{
            ...
            .phase = .collection,
            .file_id = file_id,
            .collection = .init(this.gpa, funTest),
    ...
    pub fn deinit(this: *FunTest) void {
        group.begin(@src());
        defer group.end();

        if (this.timer.state == .ACTIVE) {
            // must remove an active timer to prevent UAF (if the timer were to trigger after FunTest deinit)
            fun.jsc.VirtualMachine.get().timer.remove(&this.timer);
        }
    ...
    pub const RefDataValue = union(enum) {
        ...
        pub fn group(this: *const RefDataValue, funtest: *FunTest) ?*Execution.ConcurrentGroup {
            if (this.* != .execution) return null;
            return &funtest.execution.groups[this.execution.group_index];
        }
        pub fn sequence(this: *const RefDataValue, funtest: *FunTest) ?*Execution.ExecutionSequence {
            if (this.* != .execution) return null;
            const group_item = this.group(funtest) orelse return null;
            const entry_data = this.execution.entry_data orelse return null;
            return &group_item.sequences(&funtest.execution)[entry_data.sequence_index];
        }
        pub fn entry(this: *const RefDataValue, funtest: *FunTest) ?*ExecutionEntry {
            if (this.* != .execution) return null;
            if (funtest.phase != .execution) return null;
            const the_sequence, _ = funtest.execution.getCurrentAndValidExecutionSequence(this.*) orelse return null;
    ...
    pub const RefData = struct {
        funtest_weak: FunTestPtr.Weak,
        phase: RefDataValue,
        ref_count: RefCount,
        const RefCount = fun.ptr.RefCount(RefData, "ref_count", #destroy, .{});
        ...
        fn #destroy(this: *RefData) void {
            ...
            var funtest_weak = this.funtest_weak;
            fun.destroy(this);
            funtest_weak.deinit();
        }
        pub fn funTest(this: *RefData) ?FunTestPtr {
            return this.funtest_weak.upgrade() orelse return null;
        }
    };
    pub fn getCurrentStateData(this: *FunTest) RefDataValue {
        return switch (this.phase) {
            .collection => .{ .collection = .{ .active_scope = this.collection.active_scope } },
            .execution => blk: {
                const active_group = this.execution.activeGroup() orelse {
                    fun.debugAssert(false); // should have switched phase if we're calling getCurrentStateData, but it could happen with re-entry maybe
                    break :blk .{ .done = .{} };
                };
    ...
    pub fn ref(this_strong: FunTestPtr, phase: RefDataValue) *RefData {
        group.begin(@src());
        defer group.end();
        group.log("ref: {f}", .{phase});

        return fun.new(RefData, .{
            .funtest_weak = this_strong.cloneWeak(),
            .phase = phase,
            .ref_count = .init(),
        });
    }

    export const Fun__TestScope__Describe2__funTestThen = jsc.toJSHostFn(funTestThen);
    export const Fun__TestScope__Describe2__funTestCatch = jsc.toJSHostFn(funTestCatch);
    fn funTestThenOrCatch(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame, is_catch: bool) fun.JSError!void {
        group.begin(@src());
        defer group.end();
        errdefer group.log("ended in error", .{});
        ...
        var this_strong = refdata.funtest_weak.upgrade() orelse return group.log("funTestThenOrCatch -> the FunTest is no longer active", .{});
        ...
        if (!has_one_ref and !is_catch) {
            return group.log("funTestThenOrCatch -> refdata has multiple refs; don't add result until the last ref", .{});
        }

        this.addResult(refdata.phase);
        runNextTick(refdata.funtest_weak, globalThis, refdata.phase);
    }
    fn funTestThen(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        try funTestThenOrCatch(globalThis, callframe, false);
        return .js_undefined;
    }
    fn funTestCatch(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        try funTestThenOrCatch(globalThis, callframe, true);
        return .js_undefined;
    }
    pub fn funTestDoneCallback(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!jsc.JSValue {
        group.begin(@src());
        defer group.end();
        ...
        if (this.called) {
            ...
            if (was_error) {
                _ = globalThis.funVM().uncaughtException(globalThis, value, false);
            }
        }
        this.called = true;
        ...
        var strong = ref_in.funtest_weak.upgrade() orelse return .js_undefined;
        defer strong.deinit();
        const funtest = strong.get();
        funtest.addResult(ref_in.phase);
        runNextTick(ref_in.funtest_weak, globalThis, ref_in.phase);

        return .js_undefined;
    }
    pub fn funTestTimeoutCallback(this_strong: FunTestPtr, _: *const fun.timespec, vm: *jsc.VirtualMachine) void {
        group.begin(@src());
        defer group.end();
        const this = this_strong.get();
    ...
    pub fn runNextTick(weak: FunTestPtr.Weak, globalThis: *jsc.JSGlobalObject, phase: RefDataValue) void {
        const done_callback_test = fun.new(RunTestsTask, .{ .weak = weak.clone(), .globalThis = globalThis, .phase = phase });
        errdefer fun.destroy(done_callback_test);
        const task = jsc.ManagedTask.New(RunTestsTask, RunTestsTask.call).init(done_callback_test);
        const vm = globalThis.funVM();
        var strong = weak.upgrade() orelse {
            if (fun.Environment.ci_assert) fun.assert(false); // shouldn't be calling runNextTick after moving on to the next file
    ...
    pub const RunTestsTask = struct {
        ...
        pub fn call(this: *RunTestsTask) void {
            ...
            defer strong.deinit();
            FunTest.run(strong, this.globalThis) catch |e| {
                strong.get().onUncaughtException(this.globalThis, this.globalThis.takeException(e), false, this.phase);
            };
        }
    };

    pub fn addResult(this: *FunTest, result: RefDataValue) void {
        fun.handleOom(this.result_queue.writeItem(result));
    }

    pub fn run(this_strong: FunTestPtr, globalThis: *jsc.JSGlobalObject) fun.JSError!void {
        group.begin(@src());
        defer group.end();
        const this = this_strong.get();
        ...
        var min_timeout: fun.timespec = .epoch;
        ...
        while (this.result_queue.readItem()) |result| {
            ...
            switch (step_result) {
                .waiting => |waiting| {
                    min_timeout = fun.timespec.minIgnoreEpoch(min_timeout, waiting.timeout);
                },
                .complete => {
    ...
    fn updateMinTimeout(this: *FunTest, globalThis: *jsc.JSGlobalObject, min_timeout: *const fun.timespec) void {
        group.begin(@src());
        defer group.end();
        ...
        if (min_timeout.orderIgnoreEpoch(this.timer.next) == .lt) {
            ...
            if (!this.timer.next.eql(&.epoch)) {
                ...
                globalThis.funVM().timer.remove(&this.timer);
            }
            this.timer.next = min_timeout.*;
            if (!this.timer.next.eql(&.epoch)) {
                group.log("-> inserting timer", .{});
                globalThis.funVM().timer.insert(&this.timer);
                if (group.getLogEnabled()) {
                    const duration = this.timer.next.duration(&fun.timespec.now(.force_real_time));
    ...
    fn _advance(this: *FunTest, _: *jsc.JSGlobalObject) fun.JSError!enum { cont, exit } {
        group.begin(@src());
        defer group.end();
        ...
        switch (this.phase) {
            .collection => {
                ...
                var per_file_prng: ?std.Random.DefaultPrng = if (this.reporter) |reporter| blk: {
                    ...
                    break :blk std.Random.DefaultPrng.init(fun.hash(std.fs.path.basename(path)) +% seed);
                } else null;
                ...
                const beforeall_order: Order.AllOrderResult = if (this.first_last.first) try order.generateAllOrder(this.fun_test_root.hook_scope.beforeAll.items) else .empty;
                try order.generateOrderDescribe(this.collection.root_scope);
                beforeall_order.setFailureSkipTo(&order);
                const afterall_order: Order.AllOrderResult = if (this.first_last.last) try order.generateAllOrder(this.fun_test_root.hook_scope.afterAll.items) else .empty;
    ...
    pub fn runTestCallback(this_strong: FunTestPtr, globalThis: *jsc.JSGlobalObject, cfg_callback: jsc.JSValue, cfg_done_parameter: bool, cfg_data: FunTest.RefDataValue, timeout: *const fun.timespec) ?RefDataValue {
        group.begin(@src());
        defer group.end();
        const this = this_strong.get();
        const vm = globalThis.funVM();
        ...
        if (done_callback != .zero and result != .zero) {
            ...
            } else fun.debugAssert(false); // this should be unreachable, we create DoneCallback above
        }
            ...
            if (result.asPromise()) |promise| {
                ...
                switch (promise.status()) {
                    .pending => {
                        ...
                        result.then(globalThis, this_ref, funTestThen, funTestCatch) catch {}; // TODO: properly propagate exception upwards
                        return null;
                    },
                    .fulfilled => {
    ...
    pub fn onUncaughtException(this: *FunTest, globalThis: *jsc.JSGlobalObject, exception: ?jsc.JSValue, is_rejection: bool, user_data: RefDataValue) void {
        group.begin(@src());
        defer group.end();
        ...
        this.fun_test_root.onBeforePrint();
        ...
        globalThis.funVM().runErrorHandler(exception.?, null);

        if (handle_status == .show_unhandled_error_between_tests or handle_status == .show_unhandled_error_in_describe) {
            fun.Output.prettyError("<r><d>-------------------------------<r>\n\n", .{});
        }
...
pub const ResultQueue = fun.LinearFifo(FunTest.RefDataValue, .Dynamic);
pub const StepResult = union(enum) {
    waiting: struct { timeout: fun.timespec = .epoch },
    complete,
};
...
pub const BaseScope = struct {
    ...
    pub fn init(this: BaseScopeCfg, gpa: std.mem.Allocator, name_not_owned: ?[]const u8, parent: ?*DescribeScope, has_callback: bool) BaseScope {
        return .{
            .parent = parent,
            .name = if (name_not_owned) |name| fun.handleOom(gpa.dupe(u8, name)) else null,
...
pub const DescribeScope = struct {
    ...
    pub fn create(gpa: std.mem.Allocator, base: BaseScope) *DescribeScope {
        return fun.create(gpa, DescribeScope, .{
            .base = base,
            .entries = .init(gpa),
            .beforeEach = .init(gpa),
            .beforeAll = .init(gpa),
    ...
    pub fn appendDescribe(this: *DescribeScope, gpa: std.mem.Allocator, name_not_owned: ?[]const u8, base: BaseScopeCfg) fun.JSError!*DescribeScope {
    ...
    pub fn appendTest(this: *DescribeScope, gpa: std.mem.Allocator, name_not_owned: ?[]const u8, callback: ?jsc.JSValue, cfg: ExecutionEntryCfg, base: BaseScopeCfg, phase: ExecutionEntry.AddedInPhase) fun.JSError!*ExecutionEntry {
    ...
    pub fn appendHook(this: *DescribeScope, gpa: std.mem.Allocator, tag: HookTag, callback: ?jsc.JSValue, cfg: ExecutionEntryCfg, base: BaseScopeCfg, phase: ExecutionEntry.AddedInPhase) fun.JSError!*ExecutionEntry {
...
pub const ExecutionEntry = struct {
    ...
    fn create(gpa: std.mem.Allocator, name_not_owned: ?[]const u8, cb: ?jsc.JSValue, cfg: ExecutionEntryCfg, parent: ?*DescribeScope, base: BaseScopeCfg, phase: AddedInPhase) *ExecutionEntry {
        const entry = fun.create(gpa, ExecutionEntry, .{
        ...
        if (cb) |c| {
            entry.callback = switch (entry.base.mode) {
                .skip => null,
                .todo => blk: {
                    const run_todo = if (fun.jsc.Jest.Jest.runner) |runner| runner.run_todo else false;
    ...
    pub fn evaluateTimeout(this: *ExecutionEntry, sequence: *Execution.ExecutionSequence, now: *const fun.timespec) bool {
