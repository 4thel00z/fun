...
pub const JestPrettyFormat = struct {
    ...
    pub const Formatter = struct {
            ...
            switch (comptime Format) {
                ...
                .String => {
                    ...
                    if (str.is16Bit()) {
                        ...
                        // slow path
                        const buf = strings.allocateLatin1IntoUTF8(fun.default_allocator, str.slice()) catch &[_]u8{};
                        if (buf.len > 0) {
                ...
                .Error => {
                    ...
                    var message_string = fun.String.empty;
                    defer message_string.deref();

                    if (try value.fastGet(this.globalThis, .message)) |message_prop| {
                        message_string = try message_prop.toFunString(this.globalThis);
                    }
                ...
                .TypedArray => {
                    ...
                    if (jsType == .Uint8Array) {
                        ...
                        writer.writeAll(fun.asByteSlice(@tagName(arrayBuffer.typed_array_type)));
                    } else {
                        writer.writeAll(fun.asByteSlice(@tagName(arrayBuffer.typed_array_type)));
                    }
        ...
        pub fn format(this: *JestPrettyFormat.Formatter, result: Tag.Result, comptime Writer: type, writer: *std.Io.Writer, value: JSValue, globalThis: *JSGlobalObject, comptime enable_ansi_colors: bool) fun.JSError!void {
