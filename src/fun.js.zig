...
pub fn applyStandaloneRuntimeFlags(b: *fun.Transpiler, graph: *const fun.StandaloneModuleGraph) void {
...
pub const Run = struct {
    ...
    pub fn bootStandalone(ctx: Command.Context, entry_path: string, graph_ptr: *fun.StandaloneModuleGraph) !void {
        jsc.markBinding(@src());
        fun.jsc.initialize(false);
        fun.analytics.Features.standalone_executable += 1;

        js_ast.Expr.Data.Store.create();
        js_ast.Stmt.Data.Store.create();
        const arena = Arena.init();

        // Load funfig.toml unless disabled by compile flags
        ...
        b.options.funfig_path = ctx.args.funfig_path;
    ...
    fn doPreconnect(preconnect: []const string) void {
        ...
        for (preconnect) |url_str| {
            const url = fun.URL.parse(url_str);

            if (!url.isHTTP() and !url.isHTTPS()) {
                Output.errGeneric("preconnect URL must be HTTP or HTTPS: {f}", .{fun.fmt.quote(url_str)});
                Global.exit(1);
            }

            if (url.hostname.len == 0) {
                Output.errGeneric("preconnect URL must have a hostname: {f}", .{fun.fmt.quote(url_str)});
                Global.exit(1);
            }

            if (!url.hasValidPort()) {
                Output.errGeneric("preconnect URL must have a valid port: {f}", .{fun.fmt.quote(url_str)});
                Global.exit(1);
            }
    ...
    fn bootFunShell(ctx: Command.Context, entry_path: []const u8) !fun.shell.ExitCode {
        @branchHint(.cold);
        ...
        return fun.shell.Interpreter.initAndRunFromFile(ctx, mini, entry_path);
    }

    pub fn boot(ctx: Command.Context, entry_path: string, loader: ?fun.options.Loader) !void {
        jsc.markBinding(@src());
        ...
        if (strings.endsWithComptime(entry_path, ".sh")) {
            const exit_code = try bootFunShell(ctx, entry_path);
            Global.exit(exit_code);
            return;
        }

        fun.jsc.initialize(ctx.runtime_options.eval.eval_and_print);
        ...
        if (ctx.runtime_options.eval.script.len > 0) {
            const script_source = try fun.default_allocator.create(logger.Source);
            ...
            const escaped_path = try escapeForJSString(fun.default_allocator, entry_path);
            defer fun.default_allocator.free(escaped_path);
            const escaped_period = try escapeForJSString(fun.default_allocator, ctx.runtime_options.cron_period);
            defer fun.default_allocator.free(escaped_period);
            const cron_script = try std.fmt.allocPrint(fun.default_allocator,
            ...
            const cwd_slice = switch (fun.sys.getcwd(&cwd_buf)) {
                .result => |cwd| cwd,
                .err => return error.SystemResources,
            };
            var eval_path_buf: [fun.MAX_PATH_BYTES + trigger.len]u8 = undefined;
            ...
            const heap_entry_path = try fun.default_allocator.dupe(u8, eval_entry_path);
            const script_source = try fun.default_allocator.create(logger.Source);
    ...
    pub fn start(this: *Run) void {
        ...
        if (this.ctx.runtime_options.cpu_prof.enabled) {
            ...
            fun.analytics.Features.cpu_profile += 1;
        }
        ...
        if (this.ctx.runtime_options.heap_prof.enabled) {
            ...
            fun.analytics.Features.heap_snapshot += 1;
        }
        ...
        do_redis_preconnect: {
            ...
            if (this.ctx.runtime_options.redis_preconnect) {
                // Go through the global object's getter because Fun.redis is a
                // PropertyCallback which means we don't have a WriteBarrier we can access
                const global = vm.global;
                const fun_object = vm.global.toJSValue().get(global, "Fun") catch |err| {
                    vm.global.reportActiveExceptionAsUnhandled(err);
                    break :do_redis_preconnect;
                } orelse break :do_redis_preconnect;
                const redis = fun_object.get(global, "redis") catch |err| {
                    vm.global.reportActiveExceptionAsUnhandled(err);
                    break :do_redis_preconnect;
                } orelse break :do_redis_preconnect;
                const client = redis.as(fun.valkey.JSValkeyClient) orelse break :do_redis_preconnect;
        ...
        do_postgres_preconnect: {
            if (this.ctx.runtime_options.sql_preconnect) {
                ...
                const sql_object = fun_object.get(global, "sql") catch |err| {
                    global.reportActiveExceptionAsUnhandled(err);
                    break :do_postgres_preconnect;
                } orelse break :do_postgres_preconnect;
            ...
            if (promise.status() == .rejected) {
                ...
                if (vm.hot_reload != .none or handled) {
                    ...
                    if (run.any_unhandled) {
                        ...
                        fun.jsc.SavedSourceMap.MissingSourceMapNoteInfo.print();

                        Output.prettyErrorln(
                            "<r>\n<d>{s}<r>",
                            .{Global.unhandled_error_fun_version_string},
                        );
                    }
                    vm.globalExit();
                }
        ...
        if (!this.vm.isWatcherEnabled()) {
            fun.StandaloneModuleGraph.hintSourcePagesDontNeed();
        }
                    ...
                    const to_print = brk: {
                        ...
                        if (result.asAnyPromise()) |promise| {
                            switch (promise.status()) {
                                .pending => {
                                    result.then2(vm.global, .js_undefined, Fun__onResolveEntryPointResult, Fun__onRejectEntryPointResult) catch {}; // TODO: properly propagate exception upwards
        ...
        fun.api.napi.fixDeadCodeElimination();
        fun.webcore.BakeResponse.fixDeadCodeElimination();
        fun.crash_handler.fixDeadCodeElimination();
    ...
    fn addConditionalGlobals(this: *Run) void {
        ...
        if (runtime_options.expose_gc) {
            fun.cpp.JSC__JSGlobalObject__addGc(vm.global);
        }
    }
};

pub export fn Fun__onResolveEntryPointResult(global: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) callconv(jsc.conv) noreturn {
    const arguments = callframe.arguments_old(1).slice();
    const result = arguments[0];
    result.print(global, .Log, .Log);
    Global.exit(global.funVM().exit_handler.exit_code);
    return .js_undefined;
}

pub export fn Fun__onRejectEntryPointResult(global: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) callconv(jsc.conv) noreturn {
    const arguments = callframe.arguments_old(1).slice();
    const result = arguments[0];
    result.print(global, .Log, .Log);
    Global.exit(global.funVM().exit_handler.exit_code);
...
const CPUProfiler = @import("./jsc/FunCPUProfiler.zig");
const HeapProfiler = @import("./jsc/FunHeapProfiler.zig");
