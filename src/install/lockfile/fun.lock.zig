...
pub const Stringifier = struct {
    ...
    pub fn saveFromBinary(allocator: std.mem.Allocator, lockfile: *BinaryLockfile, load_result: *const LoadResult, options: *const PackageManager.Options, writer: *std.Io.Writer) std.Io.Writer.Error!void {
        return fun.handleOom(saveFromBinary_inner(allocator, lockfile, load_result, options, writer));
    }
    pub fn saveFromBinary_inner(allocator: std.mem.Allocator, lockfile: *BinaryLockfile, load_result: *const LoadResult, options: *const PackageManager.Options, writer: *std.Io.Writer) !void {
        ...
        try incIndent(writer, indent);
        {
            ...
            const config_version: fun.ConfigVersion = options.config_version orelse .current;
            ...
            if (found_patched_dependencies.count() > 0) {
                ...
                while (values_iter.next()) |value| {
                    ...
                    try writer.print(
                        \\{f}: {f},
                        \\
                    , .{ fun.fmt.formatJSONStringUTF8(name_and_version, .{}), patch_path.fmtJson(buf, .{}) });
                }
            ...
            for (tree_sort_buf.items) |item| {
                ...
                for (tree_deps_sort_buf.items) |dep_id| {
                    ...
                    try writer.print("{f}", .{
                        fun.fmt.formatJSONStringUTF8(relative_path, .{ .quote = false }),
                    });
                    ...
                    try writer.print("{f}\": ", .{
                        fun.fmt.formatJSONStringUTF8(dep_name, .{ .quote = false }),
                    });
    ...
    fn writePackageInfoObject(
        ...
        inline for (workspace_dependency_groups) |group| {
            ...
            for (pkg_dep_ids) |dep_id| {
                ...
                try writer.print("{f}: {f}", .{
                    fun.fmt.formatJSONStringUTF8(dep.name.slice(buf), .{}),
                    fun.fmt.formatJSONStringUTF8(dep.version.literal.slice(buf), .{}),
                });
        ...
        if (optional_peers_buf.items.len > 0) {
            ...
            for (optional_peers_buf.items, 0..) |optional_peer, i| {
                ...
                , .{
                    if (i != 0) " " else "",
                    fun.fmt.formatJSONStringUTF8(optional_peer.slice(buf), .{}),
    ...
    fn writeWorkspaceDeps(
        ...
        if (pkg_id == 0) {
            ...
            if (root_name.len > 0) {
                ...
                try writer.print("\"name\": {f}", .{
                    fun.fmt.formatJSONStringUTF8(root_name, .{}),
                });
            ...
            try writer.print("{f}: {{", .{
                fun.fmt.formatJSONStringUTF8(res.slice(buf), .{}),
            });
            try writer.writeByte('\n');
            try incIndent(writer, indent);
            try writer.print("\"name\": {f}", .{
                fun.fmt.formatJSONStringUTF8(pkg_names[pkg_id].slice(buf), .{}),
            });
        ...
        inline for (workspace_dependency_groups) |group| {
            ...
            for (pkg_deps[pkg_id].get(deps_buf)) |*dep| {
                ...
                try writer.print("{f}: {f}", .{
                    fun.fmt.formatJSONStringUTF8(name, .{}),
                    fun.fmt.formatJSONStringUTF8(version, .{}),
                });
...
fn PkgMap(comptime T: type) type {
    return struct {
        map: fun.StringHashMap(T),

        pub const Entry = T;

        pub fn init(allocator: std.mem.Allocator) @This() {
            return .{
                .map = fun.StringHashMap(T).init(allocator),
            };
        }
...
pub fn parseIntoBinaryLockfile(
    ...
    if (root.get("configVersion")) |config_version_expr| {
        lockfile.saved_config_version = fun.ConfigVersion.fromExpr(config_version_expr) orelse {
...

    {
        ...
        // fun.lock marks package keys with { bundled: true } if they originate
        // from a bundled dependency. Transitive dependencies of bundled dependencies
        // will not have a bundled property, and `fun install` expects them to not
        ...
        next_pkg_key: for (pkgs_expr.data.e_object.properties.slice()) |prop| {
                    ...
                    for (workspace_pkgs_off..workspace_pkgs_off + workspace_pkgs_len) |_workspace_pkg_id| {
                        ...
                        if (res.eql(&pkg_resolutions[workspace_pkg_id], string_buf.bytes.items, string_buf.bytes.items)) {
                            if (comptime Environment.isDebug) {
                                fun.assertWithLocation(!strings.eqlLong(pkg_path, pkg_names[workspace_pkg_id].slice(string_buf.bytes.items), true), @src());
                            }
            ...
            switch (res.tag) {
                ...
                inline .git, .github => |tag| {
                    ...
                    const fun_tag = pkg_info.at(i);
                    i += 1;

                    const fun_tag_str = fun_tag.asString(allocator) orelse {
                        try log.addError(source, fun_tag.loc, "Expected a string");
                        return error.InvalidPackageInfo;
                    };

                    @field(res.value, @tagName(tag)).resolved = try string_buf.append(fun_tag_str);
        ...
        var seen_deps: fun.StringArrayHashMapUnmanaged(void) = .empty;
...
fn parseAppendDependencies(
    ...
    var path_buf: if (check_for_bundled) fun.PathBuffer else void = undefined;
...
const BinaryLockfile = fun.install.Lockfile;
