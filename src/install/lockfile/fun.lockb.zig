...
pub const version = "fun-lockfile-format-v0\n";
...
pub fn save(this: *Lockfile, options: *const PackageManager.Options, bytes: *std.array_list.Managed(u8), total_size: *usize, end_pos: *usize) !void {
    ...
    // < Fun v1.0.4 stopped right here when reading the lockfile
    ...
    if (this.patched_dependencies.entries.len > 0) {
        for (this.patched_dependencies.values()) |patched_dep| fun.assert(!patched_dep.patchfile_hash_is_null);
...

    {
        ...
        if (remaining_in_buffer > 8 and total_buffer_size <= stream.buffer.len) {
            ...
            if (next_num == has_config_version_tag) {
                const config_version = fun.ConfigVersion.fromInt(try reader.readInt(u64, .little)) orelse {
                    return error.InvalidLockfile;
                };
