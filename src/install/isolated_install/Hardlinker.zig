const Hardlinker = @This();

src_dir: FD,
src: fun.AbsPath(.{ .sep = .auto, .unit = .os }),
dest: fun.Path(.{ .sep = .auto, .unit = .os }),
walker: Walker,

pub fn init(
    folder_dir: FD,
    src: fun.AbsPath(.{ .sep = .auto, .unit = .os }),
    dest: fun.Path(.{ .sep = .auto, .unit = .os }),
    skip_dirnames: []const fun.OSPathSlice,
) OOM!Hardlinker {
    return .{
        .src_dir = folder_dir,
        .src = src,
        .dest = dest,
        .walker = walker: {
            var w = try Walker.walk(
                folder_dir,
                fun.default_allocator,
                &.{},
                skip_dirnames,
            );
            w.resolve_unknown_entry_types = true;
            break :walker w;
        },
    };
}

pub fn deinit(this: *Hardlinker) void {
    this.walker.deinit();
}

pub fn link(this: *Hardlinker) OOM!sys.Maybe(void) {
    if (fun.install.PackageManager.verbose_install) {
        fun.Output.prettyErrorln(
            \\Hardlinking {f} to {f}
        ,
            .{
                fun.fmt.fmtOSPath(this.src.slice(), .{ .path_sep = .auto }),
                fun.fmt.fmtOSPath(this.dest.slice(), .{ .path_sep = .auto }),
            },
        );
        fun.Output.flush();
    }

    if (comptime Environment.isWindows) {
        const cwd_buf = fun.w_path_buffer_pool.get();
        defer fun.w_path_buffer_pool.put(cwd_buf);
        const dest_cwd = FD.cwd().getFdPathW(cwd_buf) catch {
            return .initErr(fun.sys.Error.fromCode(fun.sys.E.ACCES, .link));
        };

        while (switch (this.walker.next()) {
            .result => |res| res,
            .err => |err| return .initErr(err),
        }) |entry| {
            var src_save = this.src.save();
            defer src_save.restore();

            this.src.append(entry.path);

            var dest_save = this.dest.save();
            defer dest_save.restore();

            this.dest.append(entry.path);

            switch (entry.kind) {
                .directory => {
                    FD.cwd().makePath(u16, this.dest.slice()) catch {};
                },
                .file => {
                    const destfile_path_buf = fun.w_path_buffer_pool.get();
                    const destfile_path_buf2 = fun.w_path_buffer_pool.get();
                    defer fun.w_path_buffer_pool.put(destfile_path_buf2);
                    defer fun.w_path_buffer_pool.put(destfile_path_buf);
                    // `dest` may already be absolute (global virtual store
                    // entries live under the cache, not cwd); only prefix the
                    // working-directory path when it's project-relative.
                    const dest_parts: []const []const u16 = if (this.dest.len() > 0 and fun.path.Platform.windows.isAbsoluteT(u16, this.dest.slice()))
                        &.{this.dest.slice()}
                    else
                        &.{ dest_cwd, this.dest.slice() };
                    const destfile_path = fun.strings.addNTPathPrefixIfNeeded(destfile_path_buf2, fun.path.joinStringBufWZ(destfile_path_buf, dest_parts, .windows));

                    const srcfile_path_buf = fun.w_path_buffer_pool.get();
                    defer fun.w_path_buffer_pool.put(srcfile_path_buf);

                    switch (sys.link(u16, this.src.sliceZ(), destfile_path)) {
                        .result => {},
                        .err => |link_err1| switch (link_err1.getErrno()) {
                            .UV_EEXIST,
                            .EXIST,
                            => {
                                if (fun.install.PackageManager.verbose_install) {
                                    fun.Output.prettyErrorln(
                                        \\Hardlinking {f} to a path that already exists: {f}
                                    ,
                                        .{
                                            fun.fmt.fmtOSPath(this.src.slice(), .{ .path_sep = .auto }),
                                            fun.fmt.fmtOSPath(destfile_path, .{ .path_sep = .auto }),
                                        },
                                    );
                                }

                                try_delete: {
                                    const delete_tree_buf = fun.path_buffer_pool.get();
                                    defer fun.path_buffer_pool.put(delete_tree_buf);

                                    const delete_tree_path = fun.strings.convertUTF16toUTF8InBuffer(delete_tree_buf, this.dest.slice()) catch {
                                        break :try_delete;
                                    };
                                    FD.cwd().deleteTree(delete_tree_path) catch {};
                                }
                                switch (sys.link(u16, this.src.sliceZ(), destfile_path)) {
                                    .result => {},
                                    .err => |link_err2| return .initErr(link_err2),
                                }
                            },
                            .UV_ENOENT,
                            .NOENT,
                            => {
                                if (fun.install.PackageManager.verbose_install) {
                                    fun.Output.prettyErrorln(
                                        \\Hardlinking {f} to a path that doesn't exist: {f}
                                    ,
                                        .{
                                            fun.fmt.fmtOSPath(this.src.slice(), .{ .path_sep = .auto }),
                                            fun.fmt.fmtOSPath(destfile_path, .{ .path_sep = .auto }),
                                        },
                                    );
                                }
                                const dest_parent = this.dest.dirname() orelse {
                                    return .initErr(link_err1);
                                };

                                FD.cwd().makePath(u16, dest_parent) catch {};

                                switch (sys.link(u16, this.src.sliceZ(), destfile_path)) {
                                    .result => {},
                                    .err => |link_err2| return .initErr(link_err2),
                                }
                            },
                            else => return .initErr(link_err1),
                        },
                    }
                },
                else => {},
            }
        }

        return .success;
    }

    while (switch (this.walker.next()) {
        .result => |res| res,
        .err => |err| return .initErr(err),
    }) |entry| {
        var dest_save = this.dest.save();
        defer dest_save.restore();

        this.dest.append(entry.path);

        switch (entry.kind) {
            .directory => {
                FD.cwd().makePath(u8, this.dest.sliceZ()) catch {};
            },
            .file => {
                switch (sys.linkatZ(entry.dir, entry.basename, FD.cwd(), this.dest.sliceZ())) {
                    .result => {},
                    .err => |link_err1| {
                        switch (link_err1.getErrno()) {
                            .EXIST => {
                                FD.cwd().deleteTree(this.dest.slice()) catch {};
                                switch (sys.linkatZ(entry.dir, entry.basename, FD.cwd(), this.dest.sliceZ())) {
                                    .result => {},
                                    .err => |link_err2| return .initErr(link_err2),
                                }
                            },
                            .NOENT => {
                                const dest_parent = this.dest.dirname() orelse {
                                    return .initErr(link_err1);
                                };

                                FD.cwd().makePath(u8, dest_parent) catch {};
                                switch (sys.linkatZ(entry.dir, entry.basename, FD.cwd(), this.dest.sliceZ())) {
                                    .result => {},
                                    .err => |link_err2| return .initErr(link_err2),
                                }
                            },
                            else => return .initErr(link_err1),
                        }
                    },
                }
            },
            else => {},
        }
    }

    return .success;
}

const Walker = @import("../../sys/walker_skippable.zig");

const fun = @import("fun");
const Environment = fun.Environment;
const FD = fun.FD;
const OOM = fun.OOM;
const sys = fun.sys;
