...
//!
//! The approach implemented is a `.funx` file which sits right next to the renamed
...
//!
//! The compiled binary is 13312 bytes and is `@embedFile`d into Fun itself.
...
const fun = if (!is_standalone) @import("fun") else @compileError("cannot use 'fun' in standalone build of fun_shim_impl");
const funDebugMessage = fun.Output.scoped(.fun_shim_impl, .hidden);
const callmod_inline = if (is_standalone) std.builtin.CallModifier.always_inline else fun.callmod_inline;
...
const FailReason = enum {
    ...
    pub fn getFormatTemplate(reason: FailReason) []const u8 {
        return switch (reason) {
            ...
            // like node or fun is not in %path%, vs the actual executable was not installed in node_modules.
            .InterpreterNotFound => "interpreter executable \"{s}\" not found in %PATH%",
            .InterpreterNotFoundFun => "fun is not installed in %PATH%",
    ...
    pub fn format(reason: FailReason, writer: anytype) !void {
        if (!is_standalone and fun.Environment.allow_assert and reason == .InvalidShimValidation) {
        ...
        switch (reason) {
            inline else => |r| {
                ...
                const rest = switch (r) {
                    ...
                    \\Fun failed to remap this bin to its proper location within node_modules.
                    \\This is an indication of a corrupted node_modules directory.
                    \\
                    \\Please run 'fun install --force' in the project root and try
...
pub const LauncherMode = enum {
    ...
    fn RetType(comptime mode: LauncherMode) type {
        return switch (mode) {
            // See `tryStartupFromFunJS` for why this is `void` outside of standalone.
...
fn launcher(comptime mode: LauncherMode, fun_ctx: anytype) mode.RetType() {
    ...
    const image_path_b_len = if (is_standalone) ImagePathName.Length else fun_ctx.base_path.len * 2;
    const image_path_u16 = (if (is_standalone) ImagePathName.Buffer.? else fun_ctx.base_path.ptr)[0 .. image_path_b_len / 2];
    const image_path_u8 = @as([*]u8, @ptrCast(if (is_standalone) ImagePathName.Buffer.? else fun_ctx.base_path.ptr))[0..image_path_b_len];
    ...
    const suffix = comptime (if (is_standalone) wliteral("exe") else wliteral("funx"));
    ...
    var io: w.IO_STATUS_BLOCK = undefined;
    if (is_standalone) {
        // BUF1: '\??\C:\Users\chloe\project\node_modules\.bin\hello.funx!!!!!!!!!!!!!!!!!!!!!!'
        @as(*align(1) u64, @ptrCast(&buf1_u8[image_path_b_len + 2 * (nt_object_prefix.len - "exe".len)])).* = @as(u64, @bitCast([4]u16{ 'b', 'u', 'n', 'x' }));

        const path_len_bytes: c_ushort = image_path_b_len + @as(c_ushort, 2 * (nt_object_prefix.len - "exe".len + "funx".len));
        ...
        if (dbg) {
            ...
            assert(std.mem.endsWith(u16, unicodeStringToU16(nt_name), comptime wliteral(".funx")));
        }
        ...
        metadata_handle = fun_ctx.handle;
    }
    ...
    const user_arguments_u8: []const u8 = if (!is_standalone)
        std.mem.sliceAsBytes(fun_ctx.arguments)
    else find_args: {
    ...
    // without needing to store the absolute path in the '.funx' file
    //
    ...
    var read_ptr: [*]u16 = brk: {
        var left = image_path_b_len / 2 - (if (is_standalone) ".exe".len else ".funx".len) - 1;
        ...
        // we start our search right before the . as we know the extension is '.funx'
        assert(ptr[1] == '.');
    ...
    if (!flags.isValid()) {
        // We want to return control flow back into fun.exe's main code, so that it can fall
        // back to the slow path. For more explanation, see the comment on top of `tryStartupFromFunJS`.
    ...
    var spawn_command_line: [*:0]u16 = switch (flags.has_shebang) {
        ...
        true => spawn_command_line: {
            ...
            if (!is_standalone and flags.is_node_or_fun and fun_ctx.force_use_fun) {
                // If we are running `fun --fun ...` and the script is already set to run
                // in node.exe or fun.exe, we can just directly launch it by calling Run.boot
                //
                // This can only be done in non-standalone as standalone doesn't have the JS runtime.
                // And if --fun was passed to any parent fun process, then %PATH% is already setup
                // to redirect a call to node.exe -> fun.exe. So we need not check.
                //
                // This optimization can save an additional ~10-20ms depending on the machine
                // as we do not have to launch a second process.
                if (dbg) debug("direct_launch_with_fun_js", .{});
                // BUF1: '\??\C:\Users\chloe\project\node_modules\my-cli\src\app.js"#node #####!!!!!!!!!!'
                //            ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^  ^ read_ptr
                ...
                fun_ctx.direct_launch_with_fun_js(
                    launch_slice,
                    fun_ctx.cli_context,
                );
            ...
            // The filename must end with a quote character as per the funx file format.
    ...
    if (!is_standalone) {
        ...
        fun.Output.Source.Stdio.restore();
        fun.windows.windows_enable_stdio_inheritance();
    }
    ...
    var startup_info = w.STARTUPINFOW{
        ...
        .hStdInput = if (is_standalone) ProcessParameters.hStdInput else fun.FD.stdin().native(),
        .hStdOutput = if (is_standalone) ProcessParameters.hStdOutput else fun.FD.stdout().native(),
        .hStdError = if (is_standalone) ProcessParameters.hStdError else fun.FD.stderr().native(),
    };
    ...
    inline for (.{ 0, 1 }) |attempt_number| iteration: {
        ...
        const did_process_spawn = k32.CreateProcessW(
            ...
            if (is_standalone) null else @constCast(fun_ctx.environment),
            null,
            &startup_info,
            &process,
        );
            ...
            return switch (spawn_err) {
                .FILE_NOT_FOUND => if (flags.has_shebang) {
                    if (attempt_number == 0) {
                        if (flags.is_node) {
                            if (dbg)
                                debug("node is not found, changing to fun", .{});

                            if (!is_standalone) {
                                // TODO: this is another place that direct_launch_with_fun_js should be used
                            }

                            // There are many packages that specifically call for node.exe, and Fun will respect that
                            // but if node installed, this means the binary is unlaunchable. So before we fail,
                            // we will try to launch it with fun.exe
                            //
                            // This is not an issue when using 'funx' or 'fun run', because node.exe is already
                            ...
                            // To go from node -> fun, it is a matter of writing three chars, and incrementing a pointer.
                            //
                            // lpCommandLine: 'node "C:\Users\chloe\project\node_modules\my-cli\src\app.js" --flags#!!!!!!!!!!'
                            //                  ^~~ replace these three bytes with 'fun'
                            @memcpy(spawn_command_line[1..][0..3], comptime wliteral("fun"));

                            // lpCommandLine: 'nfun "C:\Users\chloe\project\node_modules\my-cli\src\app.js" --flags#!!!!!!!!!!'
                        ...
                        if (flags.is_node_or_fun) {
                            // This script calls for 'fun', but it was not found.
                            if (dbg)
                                assert(std.mem.startsWith(u16, std.mem.span(spawn_command_line), comptime wliteral("fun ")));
                            return mode.fail(.InterpreterNotFoundFun);
                        }
                    }

                    // if attempt_number == 1, we already tried rewriting this to fun, and will now fail for real
                    if (attempt_number == 1) {
                        if (dbg)
                            assert(std.mem.startsWith(u16, std.mem.span(spawn_command_line), comptime wliteral("fun ")));
                        return mode.fail(.InterpreterNotFoundFun);
...
pub const FromFunRunContext = struct {
    const CommandContext = fun.cli.Command.Context;

    /// Path like 'C:\Users\chloe\project\node_modules\.bin\foo.funx'
    base_path: []u16,
    ...
    direct_launch_with_fun_js: *const fn (wpath: []u16, args: CommandContext) void,
    /// Command.Context
...
/// This is called from run_command.zig in fun.exe which allows us to skip the CreateProcessW
/// call to create fun_shim_impl.exe. Instead we invoke the logic it has from an open file handle.
///
...
/// is to handle version mismatches where fun.exe's decoder is too new than the .funx file.
pub fn tryStartupFromFunJS(context: FromFunRunContext) void {
    assert(!std.mem.startsWith(u16, context.base_path, &nt_object_prefix));
    comptime assert(!is_standalone);
    comptime assert(fun.FeatureFlags.windows_bunx_fast_path);
    launcher(.launch, context);
}

pub const FromFunShellContext = struct {
    /// Path like 'C:\Users\chloe\project\node_modules\.bin\foo.funx'
    base_path: []u16,
    /// Command line arguments which does NOT include the bin name:
    /// like '--port 3000 --config ./config.json'
...
/// Given the path and handle to a .funx file, do everything needed to execute it,
/// *except* for spawning it. This is used by the Fun shell to skip spawning the
/// fun_shim_impl.exe executable. The returned command line is fed into the shell's
...
pub fn readWithoutLaunch(context: FromFunShellContext) ReadWithoutLaunchResult {
    assert(!std.mem.startsWith(u16, context.base_path, &nt_object_prefix));
    comptime assert(!is_standalone);
    comptime assert(fun.FeatureFlags.windows_bunx_fast_path);
...
/// Main function for `fun_shim_impl.exe`
