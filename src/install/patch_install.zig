pub const PackageID = fun.install.PackageID;
pub const DependencyID = fun.install.DependencyID;
...
pub const PackageInstall = fun.install.PackageInstall;

pub const fun_hash_tag = fun.install.fun_hash_tag;
...
pub const PatchTask = struct {
    ...
    const debug = fun.Output.scoped(.InstallPatch, .visible);
    ...
    fn runFromMainThreadCalcHash(
        ...
        var gop = fun.handleOom(manager.lockfile.patched_dependencies.getOrPut(manager.allocator, calc_hash.name_and_version_hash));
        if (gop.found_existing) {
            gop.value_ptr.setPatchfileHash(hash);
        } else @panic("No entry for patched dependency, this is a bug in Fun.");
        ...
        if (calc_hash.state) |state| {
            ...
            switch (manager.determinePreinstallState(pkg, manager.lockfile, &out_name_and_version_hash, &out_patchfile_hash)) {
                ...
                .extract => {
                    ...
                    fun.debugAssert(!manager.network_dedupe_map.contains(task_id));
    ...
    // 5. Add fun tag for patch hash
    // 6. rename() newly patched pkg to cache
    pub fn apply(this: *PatchTask) fun.OOM!void {
        var log = &this.callback.apply.logger;
        debug("apply patch task", .{});
        fun.assert(this.callback == .apply);

        const patch: *const ApplyPatch = &this.callback.apply;
        const dir = this.project_dir;
        const patchfile_path = patch.patchfilepath;

        var absolute_patchfile_path_buf: fun.PathBuffer = undefined;
        // 1. Parse the patch file
        const absolute_patchfile_path = fun.path.joinZBuf(&absolute_patchfile_path_buf, &[_][]const u8{
            dir,
            patchfile_path,
        }, .auto);
        // TODO: can the patch file be anything other than utf-8?

        const patchfile_txt = switch (fun.sys.File.readFrom(
            fun.FD.cwd(),
        ...
        var patchfile = fun.patch.parsePatchFile(patchfile_txt) catch |e| {
        ...
        defer patchfile.deinit(fun.default_allocator);

        // 2. Create temp dir to do all the modifications
        var tmpname_buf: [1024]u8 = undefined;
        const tempdir_name = fun.fs.FileSystem.tmpname("tmp", &tmpname_buf, fun.fastRandom()) catch |err| switch (err) {
        ...
        const resolution_label, const resolution_tag = brk: {
            ...
            break :brk .{ fun.handleOom(std.fmt.allocPrint(fun.default_allocator, "{f}", .{resolution.fmt(this.manager.lockfile.buffers.string_bytes.items, .posix)})), resolution.tag };
        };
        ...
        {
            const patch_pkg_dir = switch (fun.sys.openat(
                .fromStdDir(system_tmpdir),
                tempdir_name,
                fun.O.RDONLY | fun.O.DIRECTORY,
                0,
            )) {
                .result => |fd| fd,
            ...
            // 5. Add fun tag
            const fun_tag_prefix = fun_hash_tag;
            var funtagbuf: FuntagHashBuf = undefined;
            @memcpy(funtagbuf[0..fun_tag_prefix.len], fun_tag_prefix);
            const hashlen = (std.fmt.bufPrint(funtagbuf[fun_tag_prefix.len..], "{x}", .{this.callback.apply.patch_hash}) catch unreachable).len;
            funtagbuf[fun_tag_prefix.len + hashlen] = 0;
            const funtagfd = switch (fun.sys.openat(
                patch_pkg_dir,
                funtagbuf[0 .. fun_tag_prefix.len + hashlen :0],
                ...
                .err => |e| {
                    return try log.addErrorFmtOpts(
                        this.manager.allocator,
                        "failed adding fun tag: {f}",
                        .{e.withPath(funtagbuf[0 .. fun_tag_prefix.len + hashlen :0])},
                        .{},
                    );
                },
            };
        ...
        var path_in_tmpdir_buf: fun.PathBuffer = undefined;
        const path_in_tmpdir = fun.path.joinZBuf(
            &path_in_tmpdir_buf,
            &[_][]const u8{
                tempdir_name,
                // tempdir_name,
    ...
    pub fn calcHash(this: *PatchTask) ?u64 {
        fun.assert(this.callback == .calc_hash);
        ...
        const absolute_patchfile_path = fun.path.joinZBuf(
        ...
        const stat: fun.Stat = switch (fun.sys.stat(absolute_patchfile_path)) {
            .err => |e| {
                if (e.getErrno() == .NOENT) {
                    fun.handleOom(log.addErrorFmt(null, Loc.Empty, this.manager.allocator, "Couldn't find patch file: '{s}'\n\nTo create a new patch file run:\n\n  <cyan>fun patch {s}<r>", .{
        ...
        const fd = switch (fun.sys.open(absolute_patchfile_path, fun.O.RDONLY, 0)) {
            .err => |e| {
                log.addErrorFmt(
                    null,
                    Loc.Empty,
        ...
        var file = fun.sys.File{ .handle = fd };
    ...
    pub fn newCalcPatchHash(
        ...
        const patchfile_path = fun.handleOom(manager.allocator.dupeZ(u8, patchdep.path.slice(manager.lockfile.buffers.string_bytes.items)));

        const pt = fun.new(PatchTask, .{
    ...
    pub fn newApplyPatchHash(
        ...
        const patchfilepath = fun.handleOom(pkg_manager.allocator.dupe(u8, pkg_manager.lockfile.patched_dependencies.get(name_and_version_hash).?.path.slice(pkg_manager.lockfile.buffers.string_bytes.items)));
        ...
        const pt = fun.new(PatchTask, .{
            ...
            .callback = .{
                .apply = .{
                    ...
                    // modified
                    .cache_dir_subpath = fun.handleOom(pkg_manager.allocator.dupeZ(u8, stuff.cache_dir_subpath)),
                    .cache_dir_subpath_without_patch_hash = fun.handleOom(pkg_manager.allocator.dupeZ(u8, stuff.cache_dir_subpath[0 .. std.mem.indexOf(u8, stuff.cache_dir_subpath, "_patch_hash=") orelse @panic("This is a bug in Fun.")])),
                },
            },
            .manager = pkg_manager,
            .project_dir = FileSystem.instance.top_level_dir,
...
const PackageManager = fun.PackageManager;
