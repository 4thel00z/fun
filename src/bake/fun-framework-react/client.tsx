...
/// <reference lib="dom" />
import { onServerSideReload } from "fun:bake/client";
import * as React from "react";
import { flushSync } from "react-dom";
import { hydrateRoot } from "react-dom/client";
import { createFromReadableStream } from "react-server-dom-fun/client.browser";
...
// `(self.__fun_f ??= []).push(chunk)`, which is converted into a ReadableStream
// here for React hydration. Since inline scripts are executed immediately, and
// this file is loaded asynchronously, the `__fun_f` becomes a clever way to
...
let rscPayload: any = createFromReadableStream(
  new ReadableStream({
    start(controller) {
      ...
      (self.__fun_f ||= []).forEach((__fun_f.push = handleChunk));
...
async function readCssMetadata(stream: ReadableStream<Uint8Array>) {
  ...
  if (!header) {
    if (import.meta.env.DEV) {
      throw new Error("Did not read all bytes! This is a bug in fun-framework-react");
    } else {
      location.reload();
    }
  }
