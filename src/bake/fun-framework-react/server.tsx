...
import { renderToHtml, renderToStaticHtml } from "fun-framework-react/ssr.tsx" with { funBakeGraph: "ssr" };
import { serverManifest } from "fun:bake/server";
import type { AsyncLocalStorage } from "node:async_hooks";
import { PassThrough } from "node:stream";
import { renderToPipeableStream } from "react-server-dom-fun/server.node.unbundled.js";
  ...
  return (
    <html lang="en">
      <head>
        ...
        <title>Fun + React Server Components</title>
        {styles.map(url => (
...
export async function prerender(meta: Bake.RouteMetadata) {
  ...
  return {
    ...
    // files relative to the route path, and values are anything `Fun.write`
    ...
    // pre-rendered page instead of a fully rendered route. Fun might also
...
export async function getParams(meta: Bake.ParamsMetadata): Promise<Bake.GetParamIterator> {
  ...
  if (getStaticPaths == null) {
    if (import.meta.env.STATIC) {
      throw new Error(
        "In files with dynamic params, a `getStaticPaths` function must be exported to tell Fun what files to render.",
      );
    } else {
  ...
  // Remap the Next.js pagess paradigm to Fun's format
  if (result.paths) {
    return {
