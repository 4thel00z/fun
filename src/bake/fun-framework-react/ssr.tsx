...
import { ssrManifest } from "fun:bake/server";
import { EventEmitter } from "node:events";
import type { Readable } from "node:stream";
import * as React from "react";
import { renderToPipeableStream } from "react-dom/server.node";
import { createFromNodeStream, type Manifest } from "react-server-dom-fun/client.node.unbundled.js";
import type { MiniAbortSignal } from "./server";

// Verify that React 19 is being used.
if (!React.use) {
  throw new Error("Fun's React integration requires React 19");
}
...
// payload in the `__fun_f` ('f' meaning flight) global. `client.tsx` can
...
export function renderToHtml(
  ...
  // Fun supports a special type of readable stream type called "direct",
  ...
  } as Fun.DirectUnderlyingSource as any);
}
...
const startScriptTag = "<script>(self.__fun_f||=[]).push(";
const continueScriptTag = "<script>__fun_f.push(";
...
class RscInjectionStream extends EventEmitter {
  ...
  flush() {
    // Ignore flush requests from React. Fun will automatically flush when reasonable.
  }
...
function writeManyFlightScriptData(
  ...
  try {
    ...
    if (i > 0) controller.write("');__fun_f.push(");
