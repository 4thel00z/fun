//! Instance of the development server. Attaches to an instance of `Fun.serve`,
...
pub const debug = fun.Output.Scoped(.DevServer, .visible);
pub const igLog = fun.Output.scoped(.IncrementalGraph, .visible);
pub const mapLog = fun.Output.scoped(.SourceMapStore, .visible);
...
/// Hex string generated by hashing the framework config and fun revision.
...
graph_safety_lock: fun.safety.ThreadLock,
...
barrel_files_with_deferrals: fun.StringArrayHashMapUnmanaged(void) = .{},
...
barrel_needed_exports: fun.StringArrayHashMapUnmanaged(fun.StringHashMapUnmanaged(void)) = .{},
...
/// Assets are accessible via `/_fun/asset/<key>`
...
/// This special case makes `fun-plugin-tailwind` work, which is a requirement
/// to ship initial incremental bundling support for HTML files.
has_tailwind_plugin_hack: ?fun.StringArrayHashMapUnmanaged(void) = null,
...
current_bundle: ?struct {
    ...
    start_data: fun.bundle_v2.DevServerInput,
    ...
    resolution_failure_entries: AutoArrayHashMapUnmanaged(SerializedFailure.Owner.Packed, fun.logger.Log),
...
deferred_request_pool: fun.HiveArray(DeferredRequest.Node, DeferredRequest.max_preallocated).Fallback,
...
dump_dir: if (fun.FeatureFlags.bake_debugging_features) ?std.fs.Dir else void,
...
/// Can be enabled with env var `FUN_ASSUME_PERFECT_INCREMENTAL=1`
...
pub const internal_prefix = "/_fun";
/// Assets which are routed to the `Assets` storage.
pub const asset_prefix = internal_prefix ++ "/asset";
/// Client scripts are available at `/_fun/client/{name}-{rbi}{generation}.js`
/// where:
...
///
/// Example: `/_fun/client/index-00000000f209a20e.js`
...
const DeferredPromise = struct {
    ...
    pub fn deinitIdempotently(self: *DeferredPromise) void {
        self.strong.deinit();
        self.route_bundle_indices.deinit(fun.default_allocator);
        self.route_bundle_indices = .{};
    }
};

/// DevServer is stored on the heap, storing its allocator.
pub fn init(options: Options) fun.JSOOM!*DevServer {
    fun.analytics.Features.dev_server +|= 1;

    var dump_dir = if (fun.FeatureFlags.bake_debugging_features)
        if (options.dump_sources) |dir|
            ...
            null;
    errdefer if (fun.FeatureFlags.bake_debugging_features) if (dump_dir) |*dir| dir.close();

    const separate_ssr_graph = if (options.framework.server_components) |sc| sc.separate_ssr_graph else false;

    const dev = fun.new(DevServer, .{
        ...
        .has_pre_crash_handler = fun.FeatureFlags.bake_debugging_features and
            options.dump_state_on_crash orelse
                fun.feature_flag.FUN_DUMP_STATE_ON_CRASH.get(),
        ...
        .bundling_failures = .{},
        .assume_perfect_incremental_bundling = fun.feature_flag.FUN_ASSUME_PERFECT_INCREMENTAL.get() orelse fun.Environment.isDebug,
    ...
    const fs = fun.fs.FileSystem.init(options.root) catch |err|
        return global.throwError(err, generic_action);

    dev.fun_watcher = Watcher.init(DevServer, dev, fs, fun.default_allocator) catch |err|
        return global.throwError(err, "while initializing file watcher for development server");

    errdefer dev.fun_watcher.deinit(false);
    dev.fun_watcher.start() catch |err|
    ...
    const transpiler_allocator = fun.default_allocator;
    ...
    dev.server_transpiler.resolver.watcher = dev.fun_watcher.getResolveWatcher();
    dev.client_transpiler.resolver.watcher = dev.fun_watcher.getResolveWatcher();
    ...
    if (separate_ssr_graph) {
        ...
        dev.ssr_transpiler.resolver.watcher = dev.fun_watcher.getResolveWatcher();
    }
    ...
    dev.framework = dev.framework.resolve(&dev.server_transpiler.resolver, &dev.client_transpiler.resolver, options.arena) catch {
        ...
        return global.throwValue(try dev.log.toJSAggregateError(global, fun.String.static("Framework is missing required files!")));
    };
    ...
    dev.configuration_hash_key = hash_key: {
        ...
        if (fun.Environment.isDebug) {
            const stat = fun.sys.stat(fun.selfExePath() catch |e|
                Output.panic("unhandled {}", .{e})).unwrap() catch |e|
                Output.panic("unhandled {}", .{e});
            fun.writeAnyToHasher(&hash, stat.mtime());
            hash.update(bake.getHmrRuntime(.client).code);
            hash.update(bake.getHmrRuntime(.server).code);
        } else {
            hash.update(fun.Environment.git_sha_short);
        }

        for (dev.framework.file_system_router_types) |fsr| {
            fun.writeAnyToHasher(&hash, fsr.allow_layouts);
            fun.writeAnyToHasher(&hash, fsr.ignore_underscores);
        ...
        if (dev.framework.server_components) |sc| {
            fun.writeAnyToHasher(&hash, true);
            fun.writeAnyToHasher(&hash, sc.separate_ssr_graph);
            ...
            fun.writeAnyToHasher(&hash, false);
        }
        ...
        for (dev.framework.built_in_modules.keys(), dev.framework.built_in_modules.values()) |k, v| {
            hash.update(k);
            hash.update(&.{0});
            fun.writeAnyToHasher(&hash, std.meta.activeTag(v));
    ...
    dev.router = router: {
        ...
        for (options.framework.file_system_router_types, 0..) |fsr, i| {
            ...
            const joined_root = fun.path.joinAbsStringBuf(dev.root, buf, &.{fsr.root}, .auto);
            ...
            try types.append(alloc, .{
                .abs_root = fun.strings.withoutTrailingSlash(entry.abs_path),
                .prefix = fsr.prefix,
    ...
    if (fun.FeatureFlags.bake_debugging_features and dev.has_pre_crash_handler)
        try fun.crash_handler.appendPreCrashHandler(DevServer, dev, dumpStateDueToCrash);

    fun.assert(dev.magic == .valid);
...
pub fn deinit(dev: *DevServer) void {
    ...
    useAllFields(DevServer, .{
        ...
        .active_websocket_connections = {
            ...
            if (count > 0) {
                const sockets = fun.handleOom(alloc.alloc(*HmrSocket, count));
            ...
            fun.debugAssert(dev.active_websocket_connections.count() == 0);
        ...
        .fun_watcher = dev.fun_watcher.deinit(true),
        .dump_dir = if (fun.FeatureFlags.bake_debugging_features) if (dev.dump_dir) |*dir| dir.close(),
        .log = dev.log.deinit(),
        ...
        .has_pre_crash_handler = if (dev.has_pre_crash_handler)
            fun.crash_handler.removePreCrashHandler(dev),
        .router = {
        ...
        .current_bundle = {
            if (dev.current_bundle) |_| {
                fun.debugAssert(false); // impossible to de-initialize this state correctly.
            }
        },
        .next_bundle = {
            ...
            while (r) |request| {
                ...
                fun.debugAssert(request.data.handler != .server_handler);
        ...
        .magic = {
            fun.debugAssert(dev.magic == .valid);
            dev.magic = undefined;
        },
    });
...
const AllocationScope = fun.allocators.AllocationScopeIn(fun.DefaultAllocator);
...
fn initServerRuntime(dev: *DevServer) void {
    const runtime = fun.String.static(fun.bake.getHmrRuntime(.server).code);
    ...
    const interface = c.BakeLoadInitialServerCode(
        ...
        @panic("Server runtime failed to start. The above error is always a bug in Fun");
    };
    ...
    fun.assert(fetch_function.isCallable());
...
pub fn setRoutes(dev: *DevServer, server: anytype) !bool {
    ...
    dev.server = fun.jsc.API.AnyServer.from(server);
    ...
    // types. Otherwise, this can just be Fun.serve's default handler.
...
fn onJsRequest(dev: *DevServer, req: *Request, resp: AnyResponse) void {
    ...
    const is_map = fun.strings.hasSuffixComptime(route_id, ".js.map");
    if (!is_map and !fun.strings.hasSuffixComptime(route_id, ".js"))
    ...
    dev.onJsRequestWithBundle(route_bundle_index, resp, fun.http.Method.which(req.method()) orelse .POST);
}
...
fn onIncrementalVisualizerCorked(resp: AnyResponse) void {
    const code = if (Environment.codegen_embed)
        ...
        fun.runtimeEmbedFile(.src_eager, "bake/incremental_visualizer.html");
    resp.end(code, false);
}
...
fn onMemoryVisualizerCorked(resp: AnyResponse) void {
    const code = if (Environment.codegen_embed)
        ...
        fun.runtimeEmbedFile(.src_eager, "bake/memory_visualizer.html");
    resp.end(code, false);
}
...
const RequestEnsureRouteBundledCtx = struct {
    ...
    fn onDefer(this: *RequestEnsureRouteBundledCtx, bundle_field: BundleQueueType) fun.JSError!void {
    ...
    fn onLoaded(this: *RequestEnsureRouteBundledCtx) fun.JSError!void {
        switch (this.kind) {
    ...
    fn onFailure(this: *RequestEnsureRouteBundledCtx) fun.JSError!void {
    ...
    fn onPluginError(this: *RequestEnsureRouteBundledCtx) fun.JSError!void {
...
fn ensureRouteIsBundled(
    ...
    const defer_function: *const fn (*Ctx, BundleQueueType) fun.JSError!void = @field(Ctx, "onDefer");
    const on_loaded_function: *const fn (*Ctx) fun.JSError!void = @field(Ctx, "onLoaded");
    const on_failure_function: *const fn (*Ctx) fun.JSError!void = @field(Ctx, "onFailure");
    const on_plugin_error: *const fn (*Ctx) fun.JSError!void = @field(Ctx, "onPluginError");
    ...
    sw: switch (state) {
                ...
                .unknown => if (dev.bundler_options.plugin != null) {
                    ...
                    dev.has_tailwind_plugin_hack = if (dev.vm.transpiler.options.serve_plugins) |serve_plugins|
                        for (serve_plugins) |plugin| {
                            if (fun.strings.includes(plugin, "tailwind")) break .empty;
                        } else null
                    else
                        null;
        ...
        .deferred_to_next_bundle => {
            fun.assert(dev.next_bundle.route_queue.get(route_bundle_index) != null);
            try defer_function(ctx, .next_bundle);
        },
        .bundling => {
            fun.assert(dev.current_bundle != null);
...
const ReqOrSaved = union(enum) {
    ...
    pub fn method(this: *const @This()) fun.http.Method {
        return switch (this.*) {
            .req => |req| fun.http.Method.which(req.method()) orelse .POST,
...
fn deferRequest(
    ...
    const method = switch (req) {
        .req => |r| fun.http.Method.which(r.method()) orelse .GET,
        .saved => |saved| saved.request.method,
    };
...
fn appendRouteEntryPointsIfNotStale(dev: *DevServer, entry_points: *EntryPointList, alloc: Allocator, rbi: RouteBundle.Index) fun.OOM!void {
    ...
    if (dev.has_tailwind_plugin_hack) |*map| {
        for (map.keys()) |abs_path| {
            ...
            if (file.kind() == .css)
                fun.handleOom(entry_points.appendCss(alloc, abs_path));
        }
    }
}

extern "C" fn Bake__getEnsureAsyncLocalStorageInstanceJSFunction(global: *fun.jsc.JSGlobalObject) callconv(jsc.conv) fun.jsc.JSValue;
extern "C" fn Bake__getBundleNewRouteJSFunction(global: *fun.jsc.JSGlobalObject) callconv(jsc.conv) fun.jsc.JSValue;
extern "C" fn Bake__getNewRouteParamsJSFunction(global: *fun.jsc.JSGlobalObject) callconv(jsc.conv) fun.jsc.JSValue;
...
fn computeArgumentsForFrameworkRequest(
    ...
    return .{
        ...
        .router_type_main = router_type.server_file_string.get() orelse str: {
            ...
            const str = try fun.String.createUTF8ForJS(dev.vm.global, dev.relativePath(relative_path_buf, name));
        ...
        .route_modules = framework_bundle.cached_module_list.get() orelse arr: {
            ...
            route = dev.router.routePtr(framework_bundle.route_index);
            {
                const relative_path_buf = fun.path_buffer_pool.get();
                defer fun.path_buffer_pool.put(relative_path_buf);
                var route_name = fun.String.cloneUTF8(dev.relativePath(relative_path_buf, keys[fromOpaqueFileId(.server, route.file_page.unwrap().?).get()]));
            ...
            while (true) {
                if (route.file_layout.unwrap()) |layout| {
                    const relative_path_buf = fun.path_buffer_pool.get();
                    defer fun.path_buffer_pool.put(relative_path_buf);
                    var layout_name = fun.String.cloneUTF8(dev.relativePath(
                        relative_path_buf,
        ...
        .client_id = framework_bundle.cached_client_bundle_url.get() orelse str: {
            ...
            const str = fun.String.createFormat(client_prefix ++ "/route-{x}{x}.js", .{
...
fn onFrameworkRequestWithBundle(
    dev: *DevServer,
    route_bundle_index: RouteBundle.Index,
    req: fun.jsc.API.SavedRequest.Union,
    resp: AnyResponse,
) fun.JSError!void {
    const route_bundle = dev.routeBundlePtr(route_bundle_index);
    ...
    const url_bunstr = switch (req) {
        .stack => |r| fun.String{
            .tag = .ZigString,
            .value = .{ .ZigString = fun.ZigString.fromUTF8(r.url()) },
        },
        .saved => |data| brk: {
    ...
    const url = url_bunstr.toUTF8(fun.default_allocator);
    defer url.deinit();
...
fn onHtmlRequestWithBundle(dev: *DevServer, route_bundle_index: RouteBundle.Index, resp: AnyResponse, method: fun.http.Method) void {
    const route_bundle = dev.routeBundlePtr(route_bundle_index);
    assert(route_bundle.data == .html);
    const html = &route_bundle.data.html;

    const blob = html.cached_response orelse generate: {
        const payload = fun.handleOom(generateHTMLPayload(dev, route_bundle_index, route_bundle, html));
...
const script_unref_payload = "<script>" ++
    ...
    "globalThis[Symbol.for('fun:loadData')]=()=>" ++
    "document.visibilityState==='hidden'&&" ++
    "navigator.sendBeacon('/_fun/unref',a)" ++
    ");" ++
    "})(document.querySelector('[data-fun-dev-server-script]').src.slice(-11,-3))" ++
    "</script>";

fn generateHTMLPayload(dev: *DevServer, route_bundle_index: RouteBundle.Index, route_bundle: *RouteBundle, html: *RouteBundle.HTML) fun.OOM![]u8 {
    ...
    var display_name = fun.strings.withoutSuffixComptime(fun.path.basename(html.html_bundle.data.bundle.data.path), ".html");
    // TODO: function for URL safe chars
    if (!fun.strings.isAllASCII(display_name)) display_name = "page";
    ...
    const payload_size = bundled_html.len +
        ...
        "<script type=\"module\" crossorigin src=\"\" data-fun-dev-server-script></script>".len +
    ...
    array.appendSliceAssumeCapacity(".js\" data-fun-dev-server-script></script>");
...
fn generateJavaScriptCodeForHTMLFile(
    ...
    input_file_sources: []fun.logger.Source,
    loaders: []fun.options.Loader,
) fun.OOM![]const u8 {
    var sfa_state = std.heap.stackFallback(65536, dev.allocator());
    const sfa = sfa_state.get();
    var array = fun.handleOom(std.ArrayListUnmanaged(u8).initCapacity(sfa, 65536));
    defer array.deinit(sfa);
    const w = array.writer(sfa);

    try w.writeAll("  ");
    try fun.js_printer.writeJSONString(input_file_sources[index.get()].path.pretty, @TypeOf(w), w, .utf8);
    try w.writeAll(": [ [");
    var any = false;
    for (import_records[index.get()].slice()) |import| {
        ...
        try w.writeAll("    ");
        try fun.js_printer.writeJSONString(import.path.pretty, @TypeOf(w), w, .utf8);
        try w.writeAll(", 0,\n");
    }
    if (any) {
        try w.writeAll("  ");
...
pub fn onJsRequestWithBundle(dev: *DevServer, bundle_index: RouteBundle.Index, resp: AnyResponse, method: fun.http.Method) void {
    const route_bundle = dev.routeBundlePtr(bundle_index);
    const client_bundle = route_bundle.client_bundle orelse generate: {
        const payload = fun.handleOom(dev.generateClientBundle(route_bundle));
        errdefer dev.allocator().free(payload);
...
pub fn onSrcRequest(dev: *DevServer, req: *uws.Request, resp: anytype) void {
    ...
    //     if (fun.strings.indexOfChar(url, ':')) |colon| {
    //         url = url[0..colon];
    //     }
    ...
    //     resp.end("Please set your editor in funfig.toml", false);
    // }
}
...
pub const DeferredRequest = struct {
    ...
    pub const List = fun.deprecated.SinglyLinkedList(DeferredRequest);
    pub const Node = List.Node;

    const debugLog = fun.Output.Scoped("DlogeferredRequest", .hidden).log;
    ...
    pub fn weakRef(this: *DeferredRequest) void {
        fun.assert(!this.weakly_referenced_by_requestcontext);
...
pub fn startAsyncBundle(
    ...
    if (dev.inspector()) |agent| {
        ...
        const sfa = sfa_state.get();
        var trigger_files = try std.array_list.Managed(fun.String).initCapacity(sfa, entry_points.set.count());
        defer trigger_files.deinit();
        defer for (trigger_files.items) |*str| {
            str.deref();
        };
        for (entry_points.set.keys()) |key| {
            try trigger_files.append(fun.String.cloneUTF8(key));
        }
    ...
    const ast_memory_allocator = try alloc.create(fun.ast.ASTMemoryAllocator);
    ...
    bv2.fun_watcher = dev.fun_watcher;
...
fn generateClientBundle(dev: *DevServer, route_bundle: *RouteBundle) fun.OOM![]u8 {
...
fn generateCssJSArray(dev: *DevServer, route_bundle: *RouteBundle) fun.JSError!jsc.JSValue {
    ...
    for (names, 0..) |item, i| {
        ...
        const str = fun.String.cloneUTF8(path);
        defer str.deref();
...
fn makeArrayForServerComponentsPatch(dev: *DevServer, global: *jsc.JSGlobalObject, items: []const IncrementalGraph(.server).FileIndex) fun.JSError!JSValue {
    ...
    for (items, 0..) |item, i| {
        const relative_path_buf = fun.path_buffer_pool.get();
        defer fun.path_buffer_pool.put(relative_path_buf);
        const str = fun.String.cloneUTF8(dev.relativePath(relative_path_buf, names[item.get()]));
        defer str.deref();
...
pub const HotUpdateContext = struct {
    ...
    sources: []fun.logger.Source,
    /// bundle_v2.Graph.ast.items(.import_records)
    import_records: []fun.ImportRecord.List,
    /// bundle_v2.Graph.server_component_boundaries.slice()
    scbs: fun.ast.ServerComponentBoundary.List.Slice,
...
pub fn finalizeBundle(
    dev: *DevServer,
    bv2: *fun.bundle_v2.BundleV2,
    result: *const fun.bundle_v2.DevServerOutput,
) fun.JSError!void {
    assert(dev.magic == .valid);
    ...
    var scb_bitset = try fun.bit_set.DynamicBitSetUnmanaged.initEmpty(stack_alloc, input_file_sources.len);
    for (
    ...
    var ctx: fun.bake.DevServer.HotUpdateContext = .{
    ...
    for (
        ...
        const source_map: SourceMap.Chunk = compile_result.sourceMapChunk() orelse brk: {
            ...
            fun.assert(compile_result.javascript.result == .result);
            fun.assert(dev.server_transpiler.options.source_map != .none);
            fun.assert(!part_range.source_index.isRuntime());
            break :brk .initEmpty();
        };
    ...
    for (result.cssChunks(), result.css_file_list.values()) |*chunk, metadata| {
        ...
        const index = fun.ast.Index.init(chunk.entry_point.source_index);
    ...
    ctx.gts = &gts;
    ctx.server_seen_bit_set = try fun.bit_set.DynamicBitSetUnmanaged.initEmpty(bv2.allocator(), dev.server_graph.bundled_files.count());
    ...
    for (result.cssChunks()) |*chunk| {
        const entry_index = fun.ast.Index.init(chunk.entry_point.source_index);
    ...
    if (!dev.frontend_only and dev.server_graph.current_chunk_len > 0) {
        ...
        const server_modules = if (fun.take(&source_map_json)) |json| blk: {
            ...
            break :blk c.BakeLoadServerHmrPatchWithSourceMap(
                @ptrCast(dev.vm.global),
                fun.String.cloneUTF8(server_bundle),
                json.ptr,
                json.len,
            ) catch |err| {
        ...
        } else c.BakeLoadServerHmrPatch(@ptrCast(dev.vm.global), fun.String.cloneLatin1(server_bundle)) catch |err| {
            dev.vm.printErrorLikeObjectToConsole(dev.vm.global.takeException(err));
            @panic("Error thrown while evaluating server code. This is always a bug in the bundler.");
        };
    ...
    if (will_hear_hot_update) {
        if (dev.client_graph.current_chunk_len > 0 or css_chunks.len > 0) {
            ...
            for (css_chunks) |chunk| {
                ...
                try w.writeAll(&std.fmt.bytesToHex(std.mem.asBytes(&fun.hash(key)), .lower));
            ...
            if (dev.client_graph.current_chunk_len > 0) {
                const script_id = hash: {
                    var source_map_hash: fun.bundle_v2.ContentHasher.Hash = .init(0x4b12); // arbitrarily different seed than what .initial_response uses
                ...
                while (it.next()) |socket_ptr_ptr| {
                    ...
                    if (socket.subscriptions.hot_update) {
                        const entry = fun.handleOom(socket.referenced_source_maps.getOrPut(dev.allocator(), script_id));
                        if (!entry.found_existing) {
                            sockets += 1;
                        } else {
...
fn startNextBundleIfPresent(dev: *DevServer) void {
    ...
    if (dev.next_bundle.reload_event != null or dev.next_bundle.requests.first != null or dev.next_bundle.promise.strong.hasValue()) {
        ...
        for (dev.next_bundle.route_queue.keys()) |route_bundle_index| {
            ...
            fun.handleOom(dev.appendRouteEntryPointsIfNotStale(&entry_points, temp_alloc, route_bundle_index));
        }

        if (entry_points.set.count() > 0) {
            fun.handleOom(dev.startAsyncBundle(entry_points, is_reload, timer));
        }
...
pub fn getLogForResolutionFailures(dev: *DevServer, abs_path: []const u8, graph: bake.Graph) !*fun.logger.Log {
    assert(dev.current_bundle != null);
    const current_bundle = &dev.current_bundle.?;
    ...
    if (!gop.found_existing) {
        gop.value_ptr.* = fun.logger.Log.init(current_bundle.bv2.allocator());
    }
...
fn onRequest(dev: *DevServer, req: *Request, resp: anytype) void {
    ...
    if (dev.router.matchSlow(req.url(), &params)) |route_index| {
        var ctx = RequestEnsureRouteBundledCtx{
            ...
            .kind = .server_handler,
            .route_bundle_index = fun.handleOom(dev.getOrPutRouteBundle(.{ .framework = route_index })),
        };
...
pub fn handleRenderRedirect(
    dev: *DevServer,
    saved_request: fun.jsc.API.SavedRequest,
...
pub fn respondForHTMLBundle(dev: *DevServer, html: *HTMLBundle.HTMLBundleRoute, req: *uws.Request, resp: AnyResponse) fun.OOM!void {
    var ctx = RequestEnsureRouteBundledCtx{
        .dev = dev,
        .req = .{ .req = req },
        .resp = resp,
...
fn encodeSerializedFailures(
    ...
    const len = fun.base64.encodeLen(all_failures.items);
    try buf.ensureUnusedCapacity(len);
    const to_write_into = buf.unusedCapacitySlice();
    buf.items.len += fun.base64.encode(to_write_into, all_failures.items);
    ...
    if (inspector_agent) |agent| {
        ...
        var str = fun.String.initLatin1OrASCIIView(failures_encoded);
        defer str.deref();
...
fn sendSerializedFailures(
    ...
    try buf.appendSlice(switch (kind) {
        inline else => |k| std.fmt.comptimePrint(
            ...
            \\<title>Fun - {[page_title]s}</title>
            \\<style>:root{{color-scheme:light dark}}body{{background:light-dark(white,black)}}</style>
            \\</head>
            \\<body>
            \\<noscript><h1 style="font:28px sans-serif;">{[page_title]s}</h1><p style="font:20px sans-serif;">Fun requires JavaScript enabled in the browser to render this error screen, as well as receive hot reloading events.</p></noscript>
    ...
    const pre = "\"),c=>c.charCodeAt(0));let config={fun:\"" ++ fun.Global.package_json_version_with_canary ++ "\"};";
    ...
    if (Environment.codegen_embed) {
        ...
        try buf.appendSlice(fun.runtimeEmbedFile(.codegen_eager, "bake.error.js"));
    ...
    switch (resp) {
        ...
        .promise => |_r| {
            var r = _r;
            var any_blob = fun.webcore.Blob.Any.fromArrayList(buf);
            var headers = try fun.http.Headers.from(null, fun.default_allocator, .{ .body = &any_blob });
            defer headers.deinit();
            fun.handleOom(headers.append("Content-Type", MimeType.html.value));
            if (headers.get("etag") == null) {
                if (any_blob.slice().len > 0) {
                    fun.handleOom(fun.http.ETag.appendToHeaders(any_blob.slice(), &headers));
                }
            }
...
fn printMemoryLine(dev: *DevServer) void {
    ...
    Output.prettyErrorln("<d>DevServer tracked {f}, measured: {} ({f}), process: {f}<r>", .{
        fun.fmt.size(dev.memoryCost(), .{}),
        stats.num_allocations,
        fun.fmt.size(stats.total_memory_allocated, .{}),
        fun.fmt.size(fun.sys.selfProcessMemoryUsage() orelse 0, .{}),
    });
}
...
pub fn dumpBundle(dump_dir: std.fs.Dir, graph: bake.Graph, rel_path: []const u8, chunk: []const u8, wrap: bool) !void {
    const buf = fun.path_buffer_pool.get();
    defer fun.path_buffer_pool.put(buf);
    const name = fun.path.joinAbsStringBuf("/", buf, &.{
        @tagName(graph),
        rel_path,
    }, .auto)[1..];
    var inner_dir = try dump_dir.makeOpenPath(fun.Dirname.dirname(u8, name).?, .{});
    defer inner_dir.close();

    const file = try inner_dir.createFile(fun.path.basename(name), .{});
    defer file.close();
    var file_buffer: [1024]u8 = undefined;
    var file_writer = file.writerStreaming(&file_buffer);
    const bufw = &file_writer.interface;

    if (!fun.strings.hasSuffixComptime(rel_path, ".map")) {
        try bufw.print("// {f} bundled for {s}\n", .{
            fun.fmt.quote(rel_path),
            @tagName(graph),
        });
        try bufw.print("// Bundled at {d}, Fun " ++ fun.Global.package_json_version_with_canary ++ "\n", .{
...
pub noinline fn dumpBundleForChunk(dev: *DevServer, dump_dir: std.fs.Dir, side: bake.Side, key: []const u8, code: []const u8, wrap: bool, is_ssr_graph: bool) void {
    const cwd = dev.root;
    var a: fun.PathBuffer = undefined;
    var b: [fun.MAX_PATH_BYTES * 2]u8 = undefined;
    const rel_path = fun.path.relativeBufZ(&a, cwd, key);
...
pub fn emitVisualizerMessageIfNeeded(dev: *DevServer) void {
    if (!fun.FeatureFlags.bake_debugging_features) return;
...
pub fn emitMemoryVisualizerMessageTimer(timer: *EventLoopTimer, _: *const fun.timespec) void {
    if (!fun.FeatureFlags.bake_debugging_features) return;
    const dev: *DevServer = @alignCast(@fieldParentPtr("memory_visualizer_timer", timer));
    assert(dev.magic == .valid);
    dev.emitMemoryVisualizerMessage();
    timer.state = .FIRED;
    dev.vm.timer.update(timer, &fun.timespec.msFromNow(.allow_mocked_time, 1000));
}
...
pub fn emitMemoryVisualizerMessage(dev: *DevServer) void {
    comptime assert(fun.FeatureFlags.bake_debugging_features);
    fun.debugAssert(dev.emit_memory_visualizer_events > 0);
...
pub fn writeMemoryVisualizerMessage(dev: *DevServer, payload: *std.array_list.Managed(u8)) !void {
    ...
    const system_total = fun.api.node.os.totalmem();
    try w.writeStruct(Fields{
        ...
        .devserver_tracked = if (comptime AllocationScope.enabled)
            ...
            0,
        .process_used = @truncate(fun.sys.selfProcessMemoryUsage() orelse 0),
        .system_used = @truncate(system_total -| fun.api.node.os.freemem()),
...
pub fn onWebSocketUpgrade(
    ...
    fun.handleOom(dev.active_websocket_connections.put(dev.allocator(), dw, {}));
    _ = res.upgrade(
        *HmrSocket,
        dw,
...
/// versioned alongside Fun; breaking changes may occur at any point.
///
...
pub const MessageId = enum(u8) {
    ...
    /// This can be accessed via `/_fun/incremental_visualizer`.
    ///
    ...
    /// This can be accessed via `/_fun/memory_visualizer`.
    ///
...
pub fn routeToBundleIndexSlow(dev: *DevServer, pattern: []const u8) ?RouteBundle.Index {
    ...
    if (dev.router.matchSlow(pattern, &params)) |route_index| {
        return fun.handleOom(dev.getOrPutRouteBundle(.{ .framework = route_index }));
    }
    if (dev.html_router.get(pattern)) |html| {
        return fun.handleOom(dev.getOrPutRouteBundle(.{ .html = html }));
    }
    return null;
}
...
const c = struct {
    ...
    fn BakeLoadServerHmrPatch(global: *jsc.JSGlobalObject, code: fun.String) fun.JSError!JSValue {
        const f = @extern(*const fn (*jsc.JSGlobalObject, fun.String) callconv(.c) JSValue, .{ .name = "BakeLoadServerHmrPatch" }).*;
        return fun.jsc.fromJSHostCall(global, @src(), f, .{ global, code });
    }

    fn BakeLoadServerHmrPatchWithSourceMap(global: *jsc.JSGlobalObject, code: fun.String, source_map_json_ptr: [*]const u8, source_map_json_len: usize) fun.JSError!JSValue {
        const f = @extern(*const fn (*jsc.JSGlobalObject, fun.String, [*]const u8, usize) callconv(.c) JSValue, .{ .name = "BakeLoadServerHmrPatchWithSourceMap" }).*;
        return fun.jsc.fromJSHostCall(global, @src(), f, .{ global, code, source_map_json_ptr, source_map_json_len });
    }

    fn BakeLoadInitialServerCode(global: *jsc.JSGlobalObject, code: fun.String, separate_ssr_graph: bool) fun.JSError!JSValue {
        const f = @extern(*const fn (*jsc.JSGlobalObject, fun.String, bool) callconv(.c) JSValue, .{ .name = "BakeLoadInitialServerCode" }).*;
        return fun.jsc.fromJSHostCall(global, @src(), f, .{ global, code, separate_ssr_graph });
    }
};
...
pub fn onFileUpdate(dev: *DevServer, events: []Watcher.Event, changed_files: []?[:0]u8, watchlist: Watcher.ItemList) void {
    ...
    defer dev.fun_watcher.flushEvictions();
        ...
        switch (kind) {
            .file => {
                if (event.op.delete or event.op.rename) {
                    ...
                    dev.fun_watcher.removeAtIndex(event.index, 0, &.{}, .file);
                }
...
pub fn onWatchError(_: *DevServer, err: fun.sys.Error) void {
    if (err.path.len > 0) {
        Output.err(err, "failed to watch {f} for hot-reloading", .{fun.fmt.quote(err.path)});
    } else {
...
pub fn onRouterSyntaxError(dev: *DevServer, rel_path: []const u8, log: FrameworkRouter.TinyLog) fun.OOM!void {
    _ = dev; // TODO: maybe this should track the error, send over HmrSocket?
    log.print(rel_path);
}

pub fn onRouterCollisionError(dev: *DevServer, rel_path: []const u8, other_id: OpaqueFileId, ty: Route.FileKind) fun.OOM!void {
...
pub fn relativePath(dev: *DevServer, relative_path_buf: *fun.PathBuffer, path: []const u8) []const u8 {
    fun.assert(dev.root[dev.root.len - 1] != '/');
    ...
    if (path.len >= dev.root.len + 1 and
        ...
        fun.strings.startsWith(path, dev.root))
    {
        return path[dev.root.len + 1 ..];
    }

    const rel = fun.path.relativePlatformBuf(relative_path_buf, dev.root, path, .auto, true);
    // @constCast: `rel` is owned by a buffer on `dev`, which is mutable
    fun.path.platformToPosixInPlace(u8, @constCast(rel));
    return rel;
}
...
fn dumpStateDueToCrash(dev: *DevServer) !void {
    ...
    var filepath_buf: [@min(4096, fun.MAX_PATH_BYTES)]u8 = undefined;
    ...
    var buf: [fun.base64.encodeLenFromSize(4096)]u8 = undefined;
    var it = std.mem.window(u8, payload.items, 4096, 4096);
    while (it.next()) |chunk| {
        try file.writeAll(buf[0..fun.base64.encode(&buf, chunk)]);
    }

    try file.writeAll("\"), c => c.charCodeAt(0));\n");
    try file.writeAll(end);

    Output.note("Dumped incremental bundler graph to {f}", .{fun.fmt.quote(filepath)});
}
...
pub const EntryPointList = struct {
    set: fun.StringArrayHashMapUnmanaged(Flags),
...
/// the lifetime of them are all tied to the underling Fun.serve instance.
const HTMLRouter = struct {
    ...
    pub const Map = fun.StringHashMapUnmanaged(*HTMLBundle.HTMLBundleRoute);
    ...
    pub fn put(router: *HTMLRouter, alloc: Allocator, path: []const u8, route: *HTMLBundle.HTMLBundleRoute) !void {
        if (fun.strings.eqlComptime(path, "/*")) {
...
const UnrefSourceMapRequest = struct {
    ...
    fn run(dev: *DevServer, _: *Request, resp: anytype) void {
        const ctx = fun.new(UnrefSourceMapRequest, .{
            .dev = dev,
...
const PromiseEnsureRouteBundledCtx = struct {
    ...
    fn onDefer(this: *PromiseEnsureRouteBundledCtx, bundle_field: BundleQueueType) fun.JSError!void {
        switch (bundle_field) {
            .current_bundle => {
                if (this.dev.current_bundle.?.promise.strong.hasValue()) {
                    fun.handleOom(this.dev.current_bundle.?.promise.route_bundle_indices.put(fun.default_allocator, this.route_bundle_index, {}));
            ...
            .next_bundle => {
                if (this.dev.next_bundle.promise.strong.hasValue()) {
                    fun.handleOom(this.dev.next_bundle.promise.route_bundle_indices.put(fun.default_allocator, this.route_bundle_index, {}));
    ...
    fn onLoaded(this: *PromiseEnsureRouteBundledCtx) fun.JSError!void {
        _ = this.ensurePromise();
        try this.p.?.resolve(this.global, .true);
        this.dev.vm.drainMicrotasks();
    }

    fn onFailure(this: *PromiseEnsureRouteBundledCtx) fun.JSError!void {
    ...
    fn onPluginError(this: *PromiseEnsureRouteBundledCtx) fun.JSError!void {
        _ = this.ensurePromise();
        try this.p.?.reject(this.global, fun.String.static("Plugin error").toJS(this.global));
        this.dev.vm.drainMicrotasks();
    }
...
export fn Bake__bundleNewRouteJSFunctionImpl(global: *fun.jsc.JSGlobalObject, request_ptr: *anyopaque, url: fun.String) callconv(jsc.conv) fun.jsc.JSValue {
    return fun.jsc.toJSHostCall(global, @src(), bundleNewRouteJSFunctionImpl, .{ global, request_ptr, url });
}

fn bundleNewRouteJSFunctionImpl(global: *fun.jsc.JSGlobalObject, request_ptr: *anyopaque, url_bunstr: fun.String) fun.JSError!fun.jsc.JSValue {
    const url = url_bunstr.toUTF8(fun.default_allocator);
    defer url.deinit();

    const request: *fun.webcore.Request = @ptrCast(@alignCast(request_ptr));
...
extern "C" fn Bake__createDevServerFrameworkRequestArgsObject(
    global: *fun.jsc.JSGlobalObject,
    routerTypeMain: fun.jsc.JSValue,
    routeModules: fun.jsc.JSValue,
    clientEntryUrl: fun.jsc.JSValue,
    styles: fun.jsc.JSValue,
    params: fun.jsc.JSValue,
) callconv(jsc.conv) fun.jsc.JSValue;
...
export fn Bake__getNewRouteParamsJSFunctionImpl(global: *fun.jsc.JSGlobalObject, callframe: *jsc.CallFrame) callconv(jsc.conv) fun.jsc.JSValue {
    return fun.jsc.toJSHostCall(global, @src(), newRouteParamsForBundlePromiseForJS, .{ global, callframe });
}

fn newRouteParamsForBundlePromiseForJS(global: *fun.jsc.JSGlobalObject, callframe: *jsc.CallFrame) fun.JSError!fun.jsc.JSValue {
    ...
    const request = request_js.as(fun.webcore.Request) orelse return global.throw("Request must be a Request object", .{});
    ...
    const url = try url_js.toFunString(global);
    defer url.deref();
    const url_utf8 = url.toUTF8(fun.default_allocator);
