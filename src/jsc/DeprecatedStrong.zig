...
pub fn init(safety_gpa: std.mem.Allocator, value: jsc.JSValue) Strong {
    value.protect();
    const safety: Safety = if (enable_safety) .{ .ptr = fun.create(safety_gpa, Strong, .{ .#raw = @enumFromInt(0xAEBCFA), .#safety = null }), .gpa = safety_gpa, .ref_count = 1 };
    return .{ .#raw = value, .#safety = safety };
}
pub fn deinit(this: *Strong) void {
    this.#raw.unprotect();
    if (enable_safety) if (this.#safety) |safety| {
        fun.assert(@intFromEnum(safety.ptr.*.#raw) == 0xAEBCFA);
        safety.ptr.*.#raw = @enumFromInt(0xFFFFFF);
        fun.assert(safety.ref_count == 1);
...
const jsc = fun.jsc;
const enable_safety = fun.Environment.ci_assert;
