...
typedef void* (*FunMacroFunction)();

// thread_local JSC::JSGlobalObject* globalObjectToUse;
// static void Fun__enterMacro(mco_coro* coro)
// {
//     JSC::VM& vm = globalObjectToUse->vm();
//     JSC::JSLockHolder lock(vm);
//     reinterpret_cast<FunMacroFunction>(coro->user_data)();
...
// I don't want to block the release on this seldom-used feature of Fun
// we will just have stack overflow-risky macros for now.
extern "C" void Fun__startMacro(FunMacroFunction ctx, JSC::JSGlobalObject* globalObject)
{
    // globalObjectToUse = globalObject;
    // JSC::JSLockHolder lock(globalObject->vm());
    ctx();
    // mco_coro* co;
    // mco_desc desc = mco_desc_init(Fun__enterMacro, 1024 * 1024 * 2);
    // desc.user_data = ctx;
