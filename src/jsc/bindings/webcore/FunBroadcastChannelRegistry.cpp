...
FunBroadcastChannelRegistry& FunBroadcastChannelRegistry::singleton()
{
    static NeverDestroyed<FunBroadcastChannelRegistry> registry;
    return registry.get();
}

void FunBroadcastChannelRegistry::subscribe(const String& name, ScriptExecutionContextIdentifier ctxId, BroadcastChannel& channel)
{
...
void FunBroadcastChannelRegistry::unsubscribe(const String& name, BroadcastChannel& channel)
{
    Locker locker { m_lock };
...
void FunBroadcastChannelRegistry::post(const String& name, BroadcastChannel& source, Ref<SerializedScriptValue>&& message)
{
