...
#include "FunGCOutputConstraint.h"
...
JSHeapData::JSHeapData(Heap& heap)
    ...
    , m_heapCellTypeForNodeVMGlobalObject(JSC::IsoHeapCellType::Args<Fun::NodeVMGlobalObject>())
    , m_heapCellTypeForBakeGlobalObject(JSC::IsoHeapCellType::Args<Bake::GlobalObject>())
    , m_heapCellTypeForNapiHandleScopeImpl(JSC::IsoHeapCellType::Args<Fun::NapiHandleScopeImpl>())
    , m_heapCellTypeForNativePromiseContext(JSC::IsoHeapCellType::Args<Fun::NativePromiseContext>())
...
void JSVMClientData::create(VM* vm, void* funVM)
{
    ...
    vm->deferredWorkTimer->onAddPendingWork = [clientData](Ref<JSC::DeferredWorkTimer::TicketData>&& ticket, JSC::DeferredWorkTimer::WorkType kind) -> void {
        Fun::JSCTaskScheduler::onAddPendingWork(clientData, WTF::move(ticket), kind);
    };
    vm->deferredWorkTimer->onScheduleWorkSoon = [clientData](JSC::DeferredWorkTimer::Ticket ticket, JSC::DeferredWorkTimer::Task&& task) -> void {
        Fun::JSCTaskScheduler::onScheduleWorkSoon(clientData, ticket, WTF::move(task));
    };
    vm->deferredWorkTimer->onCancelPendingWork = [clientData](JSC::DeferredWorkTimer::Ticket ticket) -> void {
        Fun::JSCTaskScheduler::onCancelPendingWork(clientData, ticket);
    };
