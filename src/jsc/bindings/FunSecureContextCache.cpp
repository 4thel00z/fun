...
extern "C" JSC::EncodedJSValue Fun__SecureContextCache__get(Zig::GlobalObject* global, uint64_t key)
{
...
extern "C" void Fun__SecureContextCache__set(Zig::GlobalObject* global, uint64_t key, JSC::EncodedJSValue value)
{
    auto& slot = global->m_secureContextCache;
    if (!slot) slot = makeUnique<Fun::SecureContextCache>(global->vm());
