...
struct WebCore::Converter<Fun::IDLStrictInteger<T>>
    : Fun::DefaultTryConverter<Fun::IDLStrictInteger<T>> {
        ...
        Ctx& ctx)
    {
        ...
        if (value.isBigInt()) {
            if (auto result = Fun::Detail::tryBigIntToInt<T>(value)) {
                return *result;
            }
            if constexpr (maxInt < std::numeric_limits<Largest>::max()) {
                if (auto result = Fun::Detail::tryBigIntToInt<Largest>(value)) {
...
template<>
struct WebCore::Converter<Fun::IDLStrictDouble> : Fun::DefaultTryConverter<Fun::IDLStrictDouble> {
    static constexpr bool conversionHasSideEffects = false;
...
template<>
struct WebCore::Converter<Fun::IDLFiniteDouble> : Fun::DefaultTryConverter<Fun::IDLFiniteDouble> {
    static constexpr bool conversionHasSideEffects = false;
...
template<std::integral T>
struct WebCore::Converter<Fun::IDLLooseInteger<T>>
    : Fun::DefaultContextConverter<Fun::IDLLooseInteger<T>> {
    ...
    static T convert(JSC::JSGlobalObject& globalObject, JSC::JSValue value, Ctx& ctx)
    {
        ...
        RELEASE_AND_RETURN(
            scope,
            Fun::convertIDL<Fun::IDLStrictInteger<T>>(globalObject, numeric, ctx));
    }
};
