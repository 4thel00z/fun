...
extern "C" JSModuleRecord* zig__ModuleInfoDeserialized__toJSModuleRecord(JSGlobalObject* globalObject, VM& vm, const Identifier& module_key, const SourceCode& source_code, VariableEnvironment& declared_variables, VariableEnvironment& lexical_variables, fun_ModuleInfoDeserialized* module_info);
...
extern "C" EncodedJSValue Fun__analyzeTranspiledModule(JSGlobalObject* globalObject, const Identifier& moduleKey, const SourceCode& sourceCode, JSPromise* promise)
{
    ...
    auto* moduleInfo = static_cast<fun_ModuleInfoDeserialized*>(provider->m_resolvedSource.module_info);
    ...
    if (!Fun::IsolatedModuleCache::canUse(vm, uncheckedDowncast<Zig::GlobalObject>(globalObject)->funVM())) {
...
static EncodedJSValue fallbackParse(JSGlobalObject* globalObject, const Identifier& moduleKey, const SourceCode& sourceCode, JSPromise* promise, JSModuleRecord* resultValue)
{
    ...
    if (resultValue != nullptr) {
        ...
        if (actual != expected) {
            ...
            dataLog("  ------", "\n");
            dataLog("  FunAnalyzeTranspiledModule:", "\n");
