...
static inline void closeRangeOrLoop(int start, int end, bool cloexec_only)
{
    ...
    if (fun_close_range(start, end, flags) == 0) {
        return;
    }
...
typedef struct fun_spawn_request_file_action_t {
    FileActionType type;
    const char* path;
    int fds[2];
    int flags;
    int mode;
} fun_spawn_request_file_action_t;

typedef struct fun_spawn_file_action_list_t {
    const fun_spawn_request_file_action_t* ptr;
    size_t len;
} fun_spawn_file_action_list_t;

typedef struct fun_spawn_request_t {
    const char* chdir;
    bool detached;
    bool new_process_group; // setpgid(0, 0) so kill(-pid, sig) reaches descendants
    fun_spawn_file_action_list_t actions;
...
extern "C" ssize_t posix_spawn_fun(
    int* pid,
    const char* path,
    const fun_spawn_request_t* request,
    char* const argv[],
    char* const envp[])
{
    ...
    const auto startChild = [&]() -> ssize_t {
        ...
        for (size_t i = 0; i < actions.len; i++) {
            const fun_spawn_request_file_action_t& action = actions.ptr[i];
            switch (action.type) {
