...
extern "C" void Fun__startCPUProfiler(JSC::VM* vm);
extern "C" void Fun__stopCPUProfiler(JSC::VM* vm, FunString* outJSON, FunString* outText);
extern "C" void Fun__setSamplingInterval(int intervalMicroseconds);

void Fun__setSamplingInterval(int intervalMicroseconds)
{
    Fun::setSamplingInterval(intervalMicroseconds);
}
            ...
            for (int i = stackTrace.frames.size() - 1; i >= 0; i--) {
                ...
                if (frame.frameType == JSC::SamplingProfiler::FrameType::Executable && frame.executable) {
                    ...
                    if (frame.hasExpressionInfo()) {
                        ...
                        // sourcemap callback is a no-op — FUN_JSC_ADDITIONS
...
extern "C" void Fun__stopCPUProfiler(JSC::VM* vm, FunString* outJSON, FunString* outText)
{
    ...
    Fun::stopCPUProfiler(*vm, outJSON ? &jsonResult : nullptr, outText ? &textResult : nullptr);
    if (outJSON)
        *outJSON = Fun::toStringRef(jsonResult);
    if (outText)
        *outText = Fun::toStringRef(textResult);
}
