...
struct IDLUnionForDiagnostic<IDLStrictNull, IDL...> {
    using Type = IDLOrderedUnion<IDL..., Fun::IDLStrictNull>;
};

template<typename... IDL>
struct IDLUnionForDiagnostic<IDLStrictUndefined, IDL...> {
    using Type = IDLOrderedUnion<IDL..., Fun::IDLStrictUndefined>;
};
}
...
// "value must be ...". See also Bindgen::LiteralConversionContext, which uses Fun::throwError.
