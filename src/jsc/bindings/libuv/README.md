...
These are copied headers from libuv which are used by `fun uv-posix-stubs` to generate stubs which crash with a helpful error message when a NAPI
module tries to access a libuv function which is not supported in Fun.
