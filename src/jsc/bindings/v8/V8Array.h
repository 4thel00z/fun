...
public:
    ...
    FUN_EXPORT uint32_t Length() const;

    // Creates a JavaScript array with the given length. If the length
    // is negative the returned array will have length 0.
    FUN_EXPORT static Local<Array> New(Isolate* isolate, int length = 0);

    // Creates a JavaScript array out of a Local<Value> array in C++
    // with a known length.
    FUN_EXPORT static Local<Array> New(Isolate* isolate, Local<Value>* elements, size_t length);

    // Creates a JavaScript array from a provided callback.
    FUN_EXPORT static MaybeLocal<Array> New(
    ...
    FUN_EXPORT Maybe<void> Iterate(Local<Context> context, IterationCallback callback,
        void* callback_data);

private:
    Array();
    FUN_EXPORT static void CheckCast(Value* obj);
};
