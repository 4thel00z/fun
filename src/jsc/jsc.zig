...
//!
//! Web and runtime-specific APIs should go in `fun.webcore` and `fun.api`.
//!
//! TODO: Remove remaining aliases to `webcore` and `api`

/// The calling convention used for JavaScript functions <> Native
pub const conv: std.builtin.CallingConvention = if (fun.Environment.isWindows and fun.Environment.isX64)
    .{ .x86_64_sysv = .{} }
else
    .c;
...
// for error handling, Fun provides toJSHostFn to wrap JSHostFnZig into JSHostFn.
...
pub const ErrorableString = Errorable(fun.String);
...
pub const PlatformEventLoop = if (fun.Environment.isPosix) fun.uws.Loop else fun.Async.Loop;
...
/// Deprecated: Use `fun.String`
pub const ZigString = @import("./ZigString.zig").ZigString;
/// Deprecated: Use `fun.webcore`
pub const WebCore = fun.webcore;
/// Deprecated: Use `fun.api`
pub const API = fun.api;
/// Deprecated: Use `fun.api.node`
pub const Node = fun.api.node;

const log = fun.Output.scoped(.JSC, .hidden);
...
pub inline fn markMemberBinding(comptime class: anytype, src: std.builtin.SourceLocation) void {
    if (!fun.Environment.enable_logs) return;
...
pub const Subprocess = fun.api.Subprocess;
...
fn onJSCInvalidEnvVar(name: [*]const u8, len: usize) callconv(.c) void {
    fun.Output.errGeneric(
        ...
        \\
        \\Environment variables must be prefixed with "FUN_JSC_". This code runs before .env files are loaded, so those won't work here.
        \\
        \\Warning: options change between releases of Fun and WebKit without notice. This is not a stable API, you should not rely on it beyond debugging something, and it may be removed entirely in a future version of Fun.
    ,
        .{name[0..len]},
    );
    fun.Global.exit(1);
...
pub const math = struct {
    extern "c" fn Fun__JSC__operationMathPow(f64, f64) f64;
    pub fn pow(x: f64, y: f64) f64 {
        return Fun__JSC__operationMathPow(x, y);
    }
};
