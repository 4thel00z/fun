        ...
        CallFrame* callFrame))
{
    ...
    throwVMError(globalObject, scope,
        createTypeError(
            globalObject,
            "Remote inspector is not enabled in this build of Fun"_s));
    return JSC::JSValue::encode(JSC::jsUndefined());
#endif
}
...
extern "C" bool Fun__mkdirp(JSC::JSGlobalObject*, const char*);
        ...
        JSC::CallFrame* callFrame))
{
    ...
    if (directoryValue.isString()) {
        ...
        if (!path.isEmpty()) {
            ...
            if (!Fun__mkdirp(globalObject, pathCString.span().data())) {
                throwVMError(
                    globalObject, scope,
...
extern "C" void Fun__drainMicrotasks();
...
extern "C" JSC::EncodedJSValue ByteRangeMapping__findExecutedLines(
    JSC::JSGlobalObject*, FunString sourceURL, BasicBlockRange* ranges,
        ...
        CallFrame* callFrame))
{
    ...
    return ByteRangeMapping__findExecutedLines(
        globalObject, Fun::toString(fileName), basicBlocks.begin(),
...
// clang-format off
/* Source for FunJSCModuleTable.lut.h
@begin FunJSCModuleTable
