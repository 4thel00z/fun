//! This is the root source file of Fun's zig module. It can be imported using
//! `@import("fun")`, and should be able to reach all code via `.` syntax.
//!
...
pub const Environment = @import("./fun_core/env.zig");
pub const env_var = @import("./fun_core/env_var.zig");
...
pub const tty = @import("./fun_core/tty.zig");

pub const sha = @import("./sha_hmac/sha.zig");
pub const FeatureFlags = @import("./fun_core/feature_flags.zig");
...
pub const fmt = @import("./fun_core/fmt.zig");
...
/// All functions and interfaces provided from Fun's `bindgen` utility.
...
pub const Progress = @import("./fun_core/Progress.zig");
...
pub const Output = @import("./fun_core/output.zig");
pub const Global = @import("./fun_core/Global.zig");
...
/// large (`fun.PathBuffer`, fixed arrays, structs of buffers). PE/COFF has
/// no TLS-BSS equivalent, so on Windows every zero-initialized threadlocal
/// is written into fun.exe's `.tls` section as raw zeros — with ~50
/// `fun.PathBuffer` threadlocals (96 KB each on Windows vs 4 KB on POSIX)
...
pub const MultiArrayList = fun.collections.MultiArrayList;
            ...
            pub fn get() u64 {
                ...
                while (value == 0) : (value = seed_value.load(.monotonic)) {
                    if (comptime Environment.isDebug or Environment.is_canary) {
                        if (fun.env_var.FUN_DEBUG_HASH_RANDOM_SEED.get()) |v| {
...
pub const Funfig = @import("./cli/funfig.zig").Funfig;
...
pub const memory = @import("./fun_alloc/memory.zig");
pub const allocators = @import("./fun_alloc/fun_alloc.zig");
...
// /proc/self/fd
fn getFdPathViaCWD(fd: std.posix.fd_t, buf: *fun.PathBuffer) ![]u8 {
...
/// TODO: move to fun.sys and add a method onto FD
...
pub fn getFdPath(fd: FD, buf: *fun.PathBuffer) ![]u8 {
    if (comptime Environment.isWindows) {
    ...
    if (comptime Environment.allow_assert) {
        ...
        if (!ProcSelfWorkAroundForDebugging.has_checked) {
            ...
            needs_proc_self_workaround = fun.env_var.FUN_NEEDS_PROC_SELF_WORKAROUND.get();
        }
...
pub fn concat(comptime T: type, dest: []T, src: []const []const T) void {
    var remain = dest;
    for (src) |group| {
        fun.copy(T, remain[0..group.len], group);
...
/// Reload Fun's process. This clones envp, argv, and gets the current
/// executable path.
///
...
pub fn reloadProcess(
    ...
    const dupe_argv = allocator.allocSentinel(?[*:0]const u8, fun.argv.len, null) catch unreachable;
    for (fun.argv, dupe_argv) |src, *dest| {
    ...
    const exec_path = (fun.selfExePath() catch unreachable).ptr;
    ...
    while (i < env.len) {
        ...
        if (is_option) {
            ...
            const arg = switch (ArgType) {
                fun.String => fun.String.cloneUTF8(env[start..j]),
                [:0]const u8 => arg: {
                    const arg = try fun.default_allocator.allocSentinel(u8, arg_len, 0);
        ...
        switch (ArgType) {
            fun.String => {
                defer buf.deinit();
                try args.insert(offset_in_args, fun.String.cloneUTF8(buf.items));
            },
            [:0]const u8 => {
                try buf.append(0);
...
pub fn initArgv() !void {
    if (comptime Environment.isPosix) {
        argv = try fun.default_allocator.alloc([:0]const u8, std.os.argv.len);
        for (0..argv.len) |i| {
        ...
        const cmdline_ptr = fun.windows.GetCommandLineW();
        var length: c_int = 0;
        ...
        const out_argv = try fun.default_allocator.alloc([:0]const u8, @intCast(length));
        ...
        for (argvu16, out_argv) |argraw, *out| {
            ...
            // ...but sometimes, it's not valid. https://github.com/underdoc-org/fun/issues/11610
            out.* = string_builder.append16(arg, default_allocator) orelse @panic("Failed to allocate memory for argv");
        }

        argv = out_argv;
    } else {
        argv = try std.process.argsAlloc(fun.default_allocator);
    }

    if (fun.env_var.FUN_OPTIONS.get()) |opts| {
        const original_len = argv.len;
        var argv_list = std.array_list.Managed([:0]const u8).fromOwnedSlice(fun.default_allocator, argv);
        try appendOptionsEnv(opts, [:0]const u8, &argv_list);
        argv = argv_list.items;
        fun_options_argc = argv.len - original_len;
    }
}
...
/// `std.mem.Allocator.create`, but prefer using `fun.new` over `create(default_allocator, T, t)`
...
pub const heap_breakdown = @import("./fun_alloc/heap_breakdown.zig");

/// Globally-allocate a value on the heap. Must free with `fun.destroy`.
...
///
/// On macOS, you can use `Fun.unsafe.mimallocDump()` to dump the heap.
...
/// Free a globally-allocated a value from `fun.new()`.
/// For single-item heap pointers, prefer fun.new/destroy over default_allocator
///
...
pub fn deleteAllPoolsForThreadExit() void {
    const pools_to_delete = .{
        ...
        fun.w_path_buffer_pool,
        fun.path_buffer_pool,
        jsc.ConsoleObject.Formatter.Visited.Pool,
        fun.js_parser.StringVoidMap.Pool,
    };
...
/// if (comptime fun.Environment.allow_assert) {
///   const expensive = doExpensiveCheck();
///   fun.assert(expensive, "Something happened: {}", .{ expensive });
/// }
/// ```
///
/// Use `releaseAssert` for assertions that should not be stripped in release builds.
...
pub inline fn assert_eql(a: anytype, b: anytype) void {
    ...
    // Output.panic("Assertion failure: " ++ fun.deprecated.autoFormatLabelFallback(@TypeOf(a), "{f}") ++ " != " ++ fun.deprecated.autoFormatLabelFallback(@TypeOf(b), "{f}"), .{ a, b });
    // TODO
...
pub fn getRoughTickCount(comptime mock_mode: timespec.MockMode) timespec {
    if (mock_mode == .allow_mocked_time) {
        if (fun.jsc.Jest.fun_test.FakeTimers.current_time.getTimespecNow()) |fake_time| {
            return fake_time;
        }
    }
...
///
/// const Thing = struct {};
/// const Index = fun.GenericIndex(u32, Thing)
///
...
pub fn GenericIndex(backing_int: type, uid: anytype) type {
    ...
    return enum(backing_int) {
        ...
        pub inline fn init(int: backing_int) Index {
            fun.assert(int != null_value); // would be confused for null
            return @enumFromInt(int);
        }

        /// Prefer this over @intFromEnum because of type confusion with `.Optional`
        pub inline fn get(i: @This()) backing_int {
            fun.assert(@intFromEnum(i) != null_value); // memory corruption
...
pub fn indexOfPointerInSlice(comptime T: type, slice: []const T, item: *const T) usize {
    fun.assert(isSliceInBufferT(T, item[0..1], slice));
                ...
                if (getenvZ(envname)) |env| {
                    if (std.fmt.parseInt(u16, env, 10) catch null) |parsed| {
                        if (parsed >= min_threads) {
                            if (fun.logger.Log.default_log_level.atLeast(.debug)) {
...
pub const StackCheck = struct {
    ...
    pub fn configureThread() void {
        cpp.Fun__StackCheck__initialize();
    }

    fn getStackEnd() usize {
        return @intFromPtr(cpp.Fun__StackCheck__getMaxStack());
    }
...
pub const deprecated = @import("./fun_core/deprecated.zig");
...
pub fn getUseSystemCA(globalObject: *jsc.JSGlobalObject, callFrame: *jsc.CallFrame) error{ JSError, OutOfMemory }!jsc.JSValue {
    ...
    return jsc.JSValue.jsBoolean(Arguments.Fun__Node__UseSystemCA);
}

// Claude thinks its fun.JSC when we renamed it to fun.jsc months ago.
pub const JSC = @compileError("Deprecated: Use @import(\"fun\").jsc instead");
