Upgrade Fun's Webkit fork to the latest upstream version of Webkit.
...
- fun run build:local (build a build of Fun with the new Webkit, make sure it compiles)
- After making sure it compiles, run some code to make sure things work. something like ./build/debug-local/fun-debug --print '42' should be all you need
- cd vendor/WebKit
...
- git checkout -b fun/webkit-upgrade-<commit-sha>
