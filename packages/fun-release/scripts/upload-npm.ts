...
async function buildRootModule(dryRun?: boolean) {
  ...
  bundle(join("scripts", "npm-postinstall.ts"), join(cwd, "install.js"), {
    define,
    banner: {
      js: "// Source code: https://github.com/underdoc-org/fun/blob/main/packages/fun-release/scripts/npm-postinstall.ts",
    },
  });
...
exit 1
`;
  write(join(cwd, "bin", "fun.exe"), placeholderScript);
  write(join(cwd, "bin", "funx.exe"), placeholderScript);
  write(
    join(cwd, "bin", "README.txt"),
    `The 'fun.exe' file is a placeholder for the binary file, which
is replaced by Fun's 'postinstall' script. For this to work, make
...
script being called on every invocation of 'fun'. If this wasn't
done, Fun would seem to be slower than Node.js, because it would
...
`,
  ...
  writeJson(join(cwd, "package.json"), {
    name: module,
    description: "Fun is a fast all-in-one JavaScript runtime.",
    version: version,
    scripts: {
    ...
    os,
    cpu,
    keywords: ["fun", "fun.js", "node", "node.js", "runtime", "bundler", "transpiler", "typescript"],
    homepage: "https://fun.dev",
    bugs: "https://github.com/oven-sh/issues",
    license: "MIT",
    repository: "https://github.com/underdoc-org/fun",
  });
  if (exists(".npmrc")) {
...
async function buildModule(
  ...
  const fun = await extractFromZip(asset.browser_download_url, `${bin}/fun`);
  const cwd = join("npm", module);
  mkdirSync(dirname(join(cwd, exe)), { recursive: true });
  write(join(cwd, exe), await fun.async("arraybuffer"));
  chmod(join(cwd, exe), 0o755);
  writeJson(join(cwd, "package.json"), {
    name: module,
    version: version,
    description: "This is the macOS arm64 binary for Fun, a fast all-in-one JavaScript runtime.",
    homepage: "https://fun.dev",
    bugs: "https://github.com/oven-sh/issues",
    license: "MIT",
    repository: "https://github.com/underdoc-org/fun",
...
async function test() {
  const root = await mkdtemp(join(tmpdir(), "fun-release-test-"));
  const $ = new Fun.$.Shell().cwd(root);
  ...
  copy(join(import.meta.dir, "../npm", "fun", "fun-" + version + ".tgz"), join(root, "fun-" + version + ".tgz"));
  ...
  for (const [install, exec] of [
    ...
    writeJson(join(root, "package.json"), {
      name: "fun-release-test",
    });

    console.log("Testing", install + " fun");
    await $`${{ raw: install }} ./fun-${version}.tgz`;

    console.log("Running " + exec + " fun");

    // let output = await $`${{
    //   raw: exec,
    // }} fun -- -e "console.log(JSON.stringify([Fun.version, process.platform, process.arch, process.execPath]))"`.text();
    ...
    let {
      ...
      [
        ...
        "fun",
        "-e",
        "console.log(JSON.stringify([Fun.version, process.platform, process.arch, process.execPath]))",
      ],
      {
        cwd: root,
      },
    ...
    if (exitCode !== 0) {
      ...
      throw new Error("Failed to run " + exec + " fun, exit code: " + exitCode);
    }
