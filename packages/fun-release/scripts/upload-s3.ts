...
try {
  ...
  bucketUrl = new URL(`fun/`, "https://s3.amazonaws.com");
...
async function getFeaturesJSON(body: ArrayBuffer) {
  ...
  const temp = tmp();
  await Fun.write(join(temp, "fun.zip"), body);
  let unzip = Fun.spawnSync({
    cmd: ["unzip", join(temp, "fun.zip")],
    cwd: temp,
  });
  if (!unzip.success) throw new Error("Failed to unzip");
  let data = Fun.spawnSync({
    cmd: [
      join(temp, local.replace(".zip", ""), "fun"),
      "--print",
      'JSON.stringify(require("fun:internal-for-testing").crash_handler.getFeatureData())',
    ],
    cwd: temp,
    env: {
      ...process.env,
...
if (!dryRun && process.env.FUN_REPORT_TOKEN) {
  await fetch(`https://fun.report/purge-cache/${full_commit_hash}`, {
    method: "POST",
    headers: {
      Authorization: process.env.FUN_REPORT_TOKEN,
    },
  });
}
