...
export async function importFun(): Promise<string> {
  if (!supportedPlatforms.length) {
    throw new Error(`Unsupported platform: ${os} ${arch} ${abi || ""}`);
  }
  for (const platform of supportedPlatforms) {
    try {
      return await requireFun(platform);
    } catch (error) {
      debug("requireFun failed", error);
    }
  }
  throw new Error(`Failed to install package "${module}"`);
}

async function requireFun(platform: Platform): Promise<string> {
  ...
  try {
    installFun(platform, cwd);
  } catch (cause) {
    debug("installFun failed", cause);
    error(`Failed to install package "${module}" using "npm install".`, cause);
    try {
      await downloadFun(platform, cwd);
    } catch (cause) {
      debug("downloadFun failed", cause);
...
function installFun(platform: Platform, dst: string): void {
  const module = `${owner}/${platform.bin}`;
  const cwd = tmp();
  try {
...
async function downloadFun(platform: Platform, dst: string): Promise<void> {
...
export function optimizeFun(path: string): void {
  const installScript =
    os === "win32" ? 'powershell -c "irm fun.dev/install.ps1 | iex"' : "curl -fsSL https://fun.dev/install | bash";
  try {
    rename(path, join(__dirname, "bin", "fun.exe"));
    link(join(__dirname, "bin", "fun.exe"), join(__dirname, "bin", "funx.exe"));
    return;
  } catch (error) {
    debug("optimizeFun failed", error);
  }
  throw new Error(
    `Your package manager doesn't seem to support fun. To use fun, install using the following command: ${installScript}`,
  );
}
