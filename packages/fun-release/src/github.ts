...
const [owner, repo] = process.env["GITHUB_REPOSITORY"]?.split("/") ?? ["oven-sh", "fun"];
...
export async function getBuild(): Promise<number> {
  ...
  const response = await fetch("https://registry.npmjs.org/-/package/fun/dist-tags");
...
export async function getSemver(tag?: string, build?: number): Promise<string> {
  ...
  const version = latest_tag_name.replace("fun-v", "");
  const { tag_name } = await getRelease(tag);
  if (tag_name !== "canary") {
    return tag_name.replace("fun-v", "");
  }
  if (build === undefined) {
...
export function formatTag(tag: string): string {
  if (tag === "canary" || tag.startsWith("fun-v")) {
    return tag;
  }
  if (tag.startsWith("v")) {
    return tag.slice(1);
