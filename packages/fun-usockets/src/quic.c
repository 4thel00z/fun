 ...
 *      loop->data.quic_next_tick_us; Fun's getTimeout() folds it into the
...
us_quic_socket_context_t *us_create_quic_socket_context(
    struct us_loop_t *loop, struct us_fun_socket_context_options_t options,
    unsigned int ext_size, unsigned int idle_timeout_s)
{
    enum create_fun_socket_error_t ssl_err = 0;
...
int us_quic_socket_context_add_server_name(us_quic_socket_context_t *ctx,
    const char *hostname, struct us_fun_socket_context_options_t options)
{
    enum create_fun_socket_error_t ssl_err = 0;
    SSL_CTX *ssl = us_ssl_ctx_build_raw(options, &ssl_err);
    if (!ssl) return -1;
    ...
    struct us_quic_pending_connect_s **out_pending, void *user)
{
    ...
    *out_pending = NULL;
    ...
    int cached = Fun__addrinfo_get(ctx->loop, host, (uint16_t) port, &ai_req) == 0;
    if (cached) {
        struct addrinfo_result *res = Fun__addrinfo_getRequestResult(ai_req);
        if (res->error || !res->entries) {
            Fun__addrinfo_freeRequest(ai_req, 1);
            return -1;
        }
        *out_qs = us_quic_connect_result(ctx, res, port, sni, reject_unauthorized);
        Fun__addrinfo_freeRequest(ai_req, *out_qs == NULL);
        return *out_qs ? 1 : -1;
    }

    struct us_quic_pending_connect_s *pc = calloc(1, sizeof(*pc));
    if (!pc) { Fun__addrinfo_freeRequest(ai_req, 1); return -1; }
    pc->ctx = ctx;
    ...
    struct us_quic_pending_connect_s *pc)
{
    ...
    struct addrinfo_result *res = Fun__addrinfo_getRequestResult(pc->ai_req);
    if (!res->error && res->entries) {
        qs = us_quic_connect_result(pc->ctx, res, pc->port, pc->sni,
            pc->reject_unauthorized);
    }
    Fun__addrinfo_freeRequest(pc->ai_req, qs == NULL);
    free(pc->sni);
    free(pc);
    return qs;
}
...
void us_quic_pending_connect_cancel(struct us_quic_pending_connect_s *pc) {
    Fun__addrinfo_freeRequest(pc->ai_req, 1);
    free(pc->sni);
    free(pc);
}
