...
extern void __attribute__((__noreturn__)) Fun__panic(const char *message, size_t length);
...
/* Fun APIs implemented in Zig */
void Fun__lock(zig_mutex_t *lock);
void Fun__unlock(zig_mutex_t *lock);
...
extern void us_dispatch_handshake(us_socket_r s, int success, struct us_fun_verify_error_t err);
extern struct us_socket_t *us_dispatch_ssl_raw_tap(us_socket_r s, char *data, int length);

extern int Fun__addrinfo_get(struct us_loop_t* loop, const char* host, uint16_t port,  struct addrinfo_request** ptr);
extern int Fun__addrinfo_set(struct addrinfo_request* ptr, struct us_connecting_socket_t* socket);
extern int Fun__addrinfo_cancel(struct addrinfo_request* ptr, struct us_connecting_socket_t* socket);
extern void Fun__addrinfo_freeRequest(struct addrinfo_request* addrinfo_req, int error);
extern struct addrinfo_result *Fun__addrinfo_getRequestResult(struct addrinfo_request* addrinfo_req);
...
struct us_fun_verify_error_t us_internal_ssl_verify_error(us_socket_r s);
...
struct us_socket_t {
   ...
   * Used by Fun's `socket.upgradeTLS()` so the returned [raw, tls] pair's
