...
void Fun__internal_dispatch_ready_poll(void* loop, void* poll);
// void Fun__internal_dispatch_ready_poll(void* loop, void* poll) {}
...
void us_loop_run_fun_tick(struct us_loop_t *loop, const struct timespec* timeout);

/* Pointer tags are used to indicate a Fun pointer versus a uSockets pointer */
...
static int fun_epoll_pwait2(int epfd, struct epoll_event *events, int maxevents, const struct timespec *timeout) {
    int ret;
    sigset_t mask;
    sigemptyset(&mask);
...
extern int Fun__isEpollPwait2SupportedOnLinuxKernel();
...
struct us_loop_t *us_create_loop(void *hint, void (*wakeup_cb)(struct us_loop_t *loop), void (*pre_cb)(struct us_loop_t *loop), void (*post_cb)(struct us_loop_t *loop), unsigned int ext_size) {
    ...
    if (has_epoll_pwait2 == -1) {
        if (Fun__isEpollPwait2SupportedOnLinuxKernel() == 0) {
            has_epoll_pwait2 = 0;
        }
    }
...
/* Shared dispatch loop for both us_loop_run and us_loop_run_fun_tick */
static void us_internal_dispatch_ready_polls(struct us_loop_t *loop) {
    ...
    for (loop->current_ready_poll = 0; loop->current_ready_poll < loop->num_ready_polls; loop->current_ready_poll++) {
        ...
        if (LIKELY(poll)) {
            if (CLEAR_POINTER_TAG(poll) != poll) {
                Fun__internal_dispatch_ready_poll(loop, poll);
                continue;
            }
    ...
    for (loop->current_ready_poll = 0; loop->current_ready_poll < loop->num_ready_polls; loop->current_ready_poll++) {
        ...
        /* Tagged pointers (FilePoll) go through Fun's own dispatch */
        if (CLEAR_POINTER_TAG(poll) != poll) {
            Fun__internal_dispatch_ready_poll(loop, poll);
            continue;
        }
...
static void us_internal_drain_ready_polls(struct us_loop_t *loop) {
    ...
    while (UNLIKELY(loop->num_ready_polls == LIBUS_MAX_READY_POLLS) && --drain_count != 0 && loop->num_polls > 0) {
        ...
        loop->num_ready_polls = fun_epoll_pwait2(loop->fd, loop->ready_polls, LIBUS_MAX_READY_POLLS, &zero);
#else
        do {
...
void us_loop_run(struct us_loop_t *loop) {
    ...
    while (loop->num_polls) {
        ...
        /* Fetch ready polls */
#ifdef LIBUS_USE_EPOLL
        loop->num_ready_polls = fun_epoll_pwait2(loop->fd, loop->ready_polls, LIBUS_MAX_READY_POLLS, NULL);
#else
        do {
...
extern void Fun__JSC_onBeforeWait(void * _Nonnull jsc_vm);
...
void us_loop_run_fun_tick(struct us_loop_t *loop, const struct timespec* timeout) {
     ...
     * callers of us_loop_run_fun_tick (HTTP thread) pass NULL, so fold it
    ...
    if (will_idle_inside_event_loop && loop->data.jsc_vm)
        Fun__JSC_onBeforeWait(loop->data.jsc_vm);
    ...
    loop->num_ready_polls = fun_epoll_pwait2(loop->fd, loop->ready_polls, LIBUS_MAX_READY_POLLS, timeout);
#else
    do {
...
struct us_internal_async *us_internal_create_async(struct us_loop_t *loop, int fallthrough, unsigned int ext_size) {
    ...
    if (efd == -1) {
        ...
        FUN_PANIC("eventfd() failed during loop init (out of file descriptors?)");
    }
