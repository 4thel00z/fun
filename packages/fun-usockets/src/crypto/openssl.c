 ...
 *     on multi-listen).
 *
 * SSL_CTX creation runs from both the JS thread (SecureContext, Fun.connect/
...
extern void fun_ssl_ctx_cache_on_free(void *parent, void *ptr, CRYPTO_EX_DATA *ad,
                                      int index, long argl, void *argp);
...
static void us_ex_idx_init(void) {
  ...
  us_ctx_cache_ex_idx = SSL_CTX_get_ex_new_index(0, NULL, NULL, NULL, fun_ssl_ctx_cache_on_free);
...
SSL_CTX *us_ssl_ctx_build_raw(struct us_fun_socket_context_options_t options,
                              enum create_fun_socket_error_t *err) {
  ERR_clear_error();
  ...
  if (options.ca_file_name) {
    ...
    for (unsigned int i = 0; i < options.ca_count; i++) {
      ...
      if (!add_ca_cert_to_ctx_store(ssl_context, options.ca[i], cert_store)) {
        *err = CREATE_FUN_SOCKET_ERROR_INVALID_CA;
        ssl_ctx_build_fail(ssl_context);
        return NULL;
      }
      ERR_clear_error();
  ...
  if (options.ssl_ciphers) {
    if (!SSL_CTX_set_cipher_list(ssl_context, options.ssl_ciphers)) {
      ...
      if (!(strlen(options.ssl_ciphers) == 0 && ERR_GET_REASON(ssl_err) == SSL_R_NO_CIPHER_MATCH)) {
        *err = CREATE_FUN_SOCKET_ERROR_INVALID_CIPHERS;
        ssl_ctx_build_fail(ssl_context);
        return NULL;
      }
      ERR_clear_error();
...
SSL_CTX *us_ssl_ctx_from_options(struct us_fun_socket_context_options_t options,
                                 enum create_fun_socket_error_t *err) {
  ...
  /* Reneg policy is the only Fun-specific config BoringSSL has nowhere to
...
struct us_fun_verify_error_t us_ssl_socket_verify_error_from_ssl(SSL *ssl) {
  long x509_verify_error =
      us_internal_verify_peer_certificate(ssl, X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT);
  if (x509_verify_error == X509_V_OK)
    return (struct us_fun_verify_error_t){.error = 0, .code = NULL, .reason = NULL};
  const char *reason = X509_verify_cert_error_string(x509_verify_error);
  const char *code = us_X509_error_code(x509_verify_error);
  return (struct us_fun_verify_error_t){.error = x509_verify_error, .code = code, .reason = reason};
}

struct us_fun_verify_error_t us_internal_ssl_verify_error(struct us_socket_t *s) {
...
static void ssl_trigger_handshake(struct us_socket_t *s, int success) {
  ...
  struct us_fun_verify_error_t verify_error = us_internal_ssl_verify_error(s);
  us_dispatch_handshake(s, success, verify_error);
}

static void ssl_trigger_handshake_econnreset(struct us_socket_t *s) {
  s->ssl_handshake_state = HANDSHAKE_COMPLETED;
  struct us_fun_verify_error_t verify_error = {
...
/* ── Adopt-TLS (STARTTLS / Fun.connect upgrade) ──────────────────────────── */
