...
extern "C" void FUN__warn__extra_ca_load_failed(const char* filename, const char* error_msg);
...
extern "C" bool Fun__Node__UseSystemCA;
...
static STACK_OF(X509) *us_ssl_ctx_load_all_certs_from_file(const char *filename) {
  ...
  FUN__warn__extra_ca_load_failed(filename, error_msg);
  ERR_clear_error();
...
// is what kept Fun.connect({tls:true}) under the node-tls-server.test.ts
