...
static void init_debug_logging() {
    ...
    const char *recv_path = getenv("FUN_RECV");
    const char *send_path = getenv("FUN_SEND");
...
extern int Fun__doesMacOSVersionSupportSendRecvMsgX();
#endif
...
int bsd_sendmmsg(LIBUS_SOCKET_DESCRIPTOR fd, struct udp_sendbuf* sendbuf, int flags) {
    ...
    if (!sendbuf->has_empty && !sendbuf->has_addresses && Fun__doesMacOSVersionSupportSendRecvMsgX()) {
        while (1) {
...
#elif defined(__APPLE__)
    if (Fun__doesMacOSVersionSupportSendRecvMsgX()) {
        while (1) {
