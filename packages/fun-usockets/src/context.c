...
#if defined(FUN_DEBUG) || (defined(__has_feature) && __has_feature(address_sanitizer)) || defined(__SANITIZE_ADDRESS__)
#include <assert.h>
...
void *us_socket_group_connect(struct us_socket_group_t *group, unsigned char kind,
    ...
    if (Fun__addrinfo_get(loop, host, (uint16_t)port, &ai_req) == 0) {
        struct addrinfo_result *result = Fun__addrinfo_getRequestResult(ai_req);
        if (result->error) {
            errno = result->error;
        ...
        if (entries && entries->info.ai_next == NULL) {
            ...
            *has_dns_resolved = 1;
            ...
            Fun__addrinfo_freeRequest(ai_req, s == NULL);
            return s;
        }
    }
...
void us_internal_socket_after_resolve(struct us_connecting_socket_t *c) {
     ...
     * path's Fun__addrinfo_cancel branch. */
    c->pending_resolve_callback = 0;
    if (c->closed) {
        if (c->addrinfo_req) {
            Fun__addrinfo_freeRequest(c->addrinfo_req, 0);
            c->addrinfo_req = 0;
        }
...
#endif
    struct addrinfo_result *result = Fun__addrinfo_getRequestResult(c->addrinfo_req);
    if (result->error) {
        us_connecting_socket_close(c);
        return;
    }
...
struct us_fun_verify_error_t us_socket_verify_error(struct us_socket_t *s) {
    if (s->ssl) {
