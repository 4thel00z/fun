...
#if ASSERT_ENABLED
extern const size_t Fun__lock__size;
#endif

extern void Fun__internal_ensureDateHeaderTimerIsEnabled(struct us_loop_t *loop);
...
void us_internal_enable_sweep_timer(struct us_loop_t *loop) {
    ...
    if (loop->data.sweep_timer_count == 1) {
        ...
        Fun__internal_ensureDateHeaderTimerIsEnabled(loop);
    }
}
...
#if ASSERT_ENABLED
    if (Fun__lock__size != sizeof(loop->data.mutex)) {
        FUN_PANIC("The size of the mutex must match the size of the lock");
    }
#endif
}
...
void us_internal_loop_post(struct us_loop_t *loop) {
     ...
     * waitForPromise → us_loop_run_fun_tick). The inner tick must not free
