...
import { messagesToMarkdown, problemsToMarkdown, withFunInfo } from "./markdown";
...
const FUN_ERROR_CONTAINER_ID = "__fun-error__";
...
const ErrorTag = ({ type }: { type: ErrorTagType }) => (
  <div className={`FunError-ErrorTag FunError-ErrorTag--${ErrorTagType[type]}`}>{ErrorTagType[type]}</div>
);
...
function getAssetPrefixPath() {
  return globalThis["__FUN_HMR"]?.assetPrefixPath || "";
}
...
export const clientURL = filename => {
  ...
  // Since fun has source maps now, we assume that it will we are dealing with a src url
...
const AsyncSourceLines = ({
  ...
  switch (loadState) {
    case LoadState.pending: {
      return (
        <div className="FunError-SourceLines">
          <div className="FunError-SourceLines-numbers">
            <div></div>
            <div></div>
            <div></div>
            <div></div>
          ...
          <div className="FunError-SourceLines-lines">
            <div></div>
            <div></div>
            <div></div>
            <div></div>
...
const SourceLines = ({
  ...
  for (let i = 0; i < _sourceLines.length; i++) {
    ...
    lines[i] = (
      <div className="FunError-SourceLine" key={"line-" + i}>
        <a
          data-line={line}
          ...
          className={`FunError-SourceLine-number ${classes.empty ? "FunError-SourceLine-number--empty" : ""} ${
            classes.highlight ? "FunError-SourceLine-number--highlight" : ""
          }`}
        >
        ...
        <div
          tabIndex={i}
          className={`FunError-SourceLine-text ${classes.empty ? "FunError-SourceLine-text--empty" : ""} ${
            classes.highlight ? "FunError-SourceLine-text--highlight" : ""
          }`}
        >
          {_text}
        </div>
  ...
  return (
    <IndentationContext.Provider value={dedent}>
      <div className="FunError-SourceLines">
        <div className={`FunError-SourceLines-highlighter--${highlightI}`}></div>
...
const BuildErrorStackTrace = ({ location }: { location: Location }) => {
  ...
  return (
    <div className={`FunError-NativeStackTrace`}>
      <a
        href={srcFileURL(filename, line, column)}
        target="_blank"
        onClick={openWithoutFlashOfNewTab}
        className="FunError-NativeStackTrace-filename"
      >
...
const NativeStackFrame = ({
  ...
  return (
    <div className={`FunError-StackFrame ${fileName.endsWith(".fun") ? "FunError-StackFrame--muted" : ""}`}>
      <div
        title={StackFrameScope[scope]}
        className="FunError-StackFrame-identifier"
      ...
      <a
        ...
        data-column={column}
        onClick={openWithoutFlashOfNewTab}
        className="FunError-StackFrame-link"
        title="Open in editor"
        draggable={false}
      >
        <div className="FunError-StackFrame-link-content">
          <div className={`FunError-StackFrame-file`}>{fileName}</div>
          {line > -1 && <div className="FunError-StackFrame-line">:{line}</div>}
          {column > -1 && <div className="FunError-StackFrame-column">:{column}</div>}
        </div>
      </a>
    </div>
  );
...
const NativeStackFrames = ({ frames, urlBuilder }) => {
  ...
  return (
    <div className="FunError-StackFrames-container">
      <div className="FunError-StackFrames">{items}</div>
    </div>
  );
};
...
const NativeStackTrace = ({
  ...
  return (
    <div ref={ref} className={`FunError-NativeStackTrace`}>
      <a
  ...
  switch (value.code) {
    case JSErrorCode.TypeError: {
      ...
      if (fancyTypeError.runtimeType !== RuntimeType.Nothing) {
        return (
          <div className={`FunError-JSException FunError-JSException--TypeError`}>
            <div className="FunError-error-header">
              <div className={`FunError-error-code`}>TypeError</div>
              {errorTags[tag]}
            </div>

            <div className={`FunError-error-message`}>{fancyTypeError.message}</div>

            {fancyTypeError.runtimeTypeName.length && (
              <div className={`FunError-error-subtitle`}>
                It's <span className="FunError-error-typename">{fancyTypeError.runtimeTypeName}</span>.
              </div>
            )}
            ...
            {value.stack && (
              ...
              >
                <Indent by={value.stack.frames[0].position.column}>
                  <span className="FunError-error-typename">{fancyTypeError.runtimeTypeName}</span>
                </Indent>
              </NativeStackTrace>
            )}
          </div>
    ...
    default: {
      ...
      if (newline > -1) {
        ...
        if (subtitle.length) {
          return (
            <div className={`FunError-JSException`}>
              <div className="FunError-error-header">
                <div className={`FunError-error-code`}>{name}</div>
                {errorTags[tag]}
              </div>

              <div className={`FunError-error-message`}>{message}</div>
              <div className={`FunError-error-subtitle`}>{subtitle}</div>
...
const Summary = ({ errorCount, onClose }: { errorCount: number; onClose: () => void }) => {
  return (
    <div className="FunError-Summary">
      <div className="FunError-Summary-ErrorIcon"></div>
      <div className="FunError-Summary-Title">
        {errorCount}&nbsp;error{errorCount > 1 ? "s" : ""}&nbsp;on this page
      </div>

      <a href="https://fun.dev/discord" target="_blank" className="FunError-Summary-help">
      ...
      <div onClick={onClose} className="FunError-Summary-CloseButton">
        <div className="FunError-Summary-CloseIcon"></div>
      </div>
    </div>
  );
};
...
const BuildError = ({ message }: { message: Message }) => {
  ...
  return (
    <div className={`FunError-BuildError FunError-BuildError--build`}>
      <div className="FunError-error-header">
        <div className={`FunError-error-code`}>BuildError</div>
      </div>

      <div className={`FunError-error-message`}>{title}</div>

      {subtitle.length > 0 && <div className={`FunError-error-subtitle`}>{subtitle}</div>}
...
const ResolveError = ({ message }: { message: Message }) => {
  ...
  return (
    <div className={`FunError-BuildError FunError-BuildError--resolve`}>
      <div className="FunError-error-header">
        <div className={`FunError-error-code`}>ResolveError</div>
      </div>

      <div className={`FunError-error-message`}>
        Can't import{" "}
        <span className="FunError-error-message--mono FunError-error-message--quoted">"{message.on.resolve}"</span>
      </div>

      {subtitle && <div className={`FunError-error-subtitle`}>{subtitle}</div>}
...
const OverlayMessageContainer = ({
  ...
  return (
    <div id="FunErrorOverlay-container">
      <div className="FunError-content">
        <div className="FunError-header">
          <Summary errorCount={errorCount} onClose={dismissError} />
        </div>
        <div className={`FunError-list`}>
...
const Footer = ({ toMarkdown, data }) => (
  <div className="FunError-footer">
    <div
      title="Copy error as markdown so it can be pasted into a bug report or slack/discord"
      aria-label="Copy as markdown button"
      className="FunErrror-footerItem FunError-CopyButton"
      onClick={() => copyToClipboard(withFunInfo(String(toMarkdown(data))))}
    >
    ...
    <div className="FunErrror-footerItem" id="FunError-poweredBy"></div>
  </div>
);
...
function renderWithFunc(func) {
  if (!reactRoot) {
    ...
    root.id = "__fun__error-root";

    reactRoot = document.createElement("div");
    reactRoot.id = FUN_ERROR_CONTAINER_ID;

    const fallbackStyleSheet = document.querySelector("style[data-has-fun-fallback-style]");
    ...
    if (!fallbackStyleSheet) {
      ...
      link.rel = "stylesheet";
      link.href = new URL("/fun:erro.css", document.baseURI).href;
      link.onload = () => {
      ...
      reactRoot.classList.add("FunErrorRoot--FullPage");

      const page = document.querySelector("style[data-fun-error-page-style]");
      if (page) {
        page.remove();
...
globalThis[Symbol.for("Fun__renderFallbackError")] = renderFallbackError;
...
export function dismissError() {
  if (reactRoot) {
    ...
    const root = document.getElementById("__fun__error-root");
    if (root) root.remove();
    reactRoot = null;
...
globalThis.__FunClearBuildFailure = dismissError;
