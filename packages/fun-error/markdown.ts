...
export const withFunInfo = text => {
  const funInfo = getFunInfo();

  const trimmed = text.trim();

  if (funInfo && "then" in funInfo) {
    return funInfo.then(
      info => {
        const markdown = funInfoToMarkdown(info).trim();
        return trimmed + "\n" + markdown + "\n";
      },
      () => trimmed + "\n",
    );
  ...
  if (funInfo) {
    const markdown = funInfoToMarkdown(funInfo).trim();
...
function funInfoToMarkdown(_info) {
  if (!_info) return;
...
var funInfoMemoized;
function getFunInfo() {
  if (funInfoMemoized) return funInfoMemoized;
  if ("sessionStorage" in globalThis) {
    try {
      const funInfoMemoizedString = sessionStorage.getItem("__funInfo");
      if (funInfoMemoizedString) {
        funInfoMemoized = JSON.parse(funInfoMemoized);
        return funInfoMemoized;
      }
    } catch (exception) {}
  }
  ...
  return fetch("/fun:info", {
    ...
    .then(funInfo => {
      clearTimeout(id);
      funInfoMemoized = funInfo;
      if ("sessionStorage" in globalThis) {
        try {
          sessionStorage.setItem("__funInfo", JSON.stringify(funInfo));
        } catch (exception) {}
      }
