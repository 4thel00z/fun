...
export function registerDebugger(context: vscode.ExtensionContext, factory?: vscode.DebugAdapterDescriptorFactory) {
  context.subscriptions.push(
    vscode.languages.registerCodeLensProvider(
      ...
      new FunCodeLensProvider(),
    ),
    vscode.commands.registerCommand("extension.fun.runFile", runFileCommand),
    vscode.commands.registerCommand("extension.fun.debugFile", debugFileCommand),
    ...
    vscode.debug.registerDebugAdapterDescriptorFactory("fun", factory ?? new InlineDebugAdapterFactory()),
  );
...
async function injectDebugTerminal(terminal: vscode.Terminal): Promise<void> {
  ...
  const debug = vscode.window.createTerminal({
    ...
    env: {
      ...env,
      "FUN_INSPECT": `${adapter.url}?${query}`,
      "FUN_INSPECT_NOTIFY": signal.url,
      FUN_INSPECT_CONNECT_TO: "",
    },
  });
...
class FileDebugSession extends DebugSession {
  ...
  constructor(sessionId?: string, untitledDocPath?: string, mapping?: PathMapping) {
    ...
    if (untitledDocPath) {
      ...
      this.funEvalPath = join(cwd, "[eval]");
    }
  }
  ...
  async initialize() {
    ...
    const { untitledDocPath, funEvalPath } = this;
    this.adapter = new WebSocketDebugAdapter(url, untitledDocPath, funEvalPath);

    if (untitledDocPath) {
      this.adapter.on("Adapter.response", (response: DebugProtocolResponse) => {
        if (response.body?.source?.path) {
          if (response.body.source.path === funEvalPath) {
      ...
      this.adapter.on("Adapter.event", (event: DebugProtocolEvent) => {
        if (event.body?.source?.path) {
          if (event.body.source.path === funEvalPath) {
...
class TerminalDebugSession extends FileDebugSession {
  ...
  get terminalProfile(): vscode.TerminalProfile {
    return new vscode.TerminalProfile({
      name: "Fun Terminal",
      env: {
        "FUN_INSPECT": `${this.adapter.url}?wait=1`,
        "FUN_INSPECT_NOTIFY": this.signal.url,
        FUN_INSPECT_CONNECT_TO: "",
      },
      isTransient: true,
  ...
  async provideCodeLenses(document: vscode.TextDocument): Promise<vscode.CodeLens[]> {
    ...
    return [
      new vscode.CodeLens(range, {
        title: "eval with fun",
        command: "extension.fun.runUnsavedCode",
        tooltip: "Run this unsaved, scratch file with Fun",
      }),
    ];
  }
}
