...
const output = vscode.window.createOutputChannel("Fun - Diagnostics");
...
class EditorStateManager {
  ...
  public constructor() {
    this.diagnosticCollection = vscode.languages.createDiagnosticCollection("FunDiagnostics");
  }
...
class FunDiagnosticsManager {
  ...
  private static async getOrRecreateSignal(context: vscode.ExtensionContext) {
    ...
    const existing = globalState.get("FUN_INSPECT_CONNECT_TO");
    ...
    if (isWin) {
      ...
      await globalState.update("FUN_INSPECT_CONNECT_TO", {
        type: "tcp",
        port,
      });
  ...
  public static async initialize(context: vscode.ExtensionContext) {
    const signal = await FunDiagnosticsManager.getOrRecreateSignal(context);

    return new FunDiagnosticsManager(context, signal);
  }

  /**
   * Called when Fun pings FUN_INSPECT_NOTIFY (indicating a program has started).
   */
  private async handleSocketConnection(socket: Socket) {
    ...
    debugAdapter.initialize({
      adapterID: "fun-vsc-terminal-debug-adapter",
...
const description = new vscode.MarkdownString(
  "Fun's VSCode extension communicates with Fun over a socket. We set the url in your terminal with the `FUN_INSPECT_NOTIFY` environment variable",
);
...
export async function registerDiagnosticsSocket(context: vscode.ExtensionContext) {
  ...
  const manager = await FunDiagnosticsManager.initialize(context);

  context.environmentVariableCollection.replace("FUN_INSPECT_CONNECT_TO", manager.signalUrl);
