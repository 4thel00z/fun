...
import {
  ...
  UnixSignal,
  getAvailablePort,
} from "../../../../../fun-debug-adapter-protocol";
...
describe("Socket Integration - Event Handling", () => {
  test("test reporter events in correct sequence", async () => {
    ...
    // messages from Fun through the socket. This tests the event handling logic without
  ...
  test("complex test event scenarios", async () => {
    ...
    // Simulate events that the adapter would emit after receiving messages from Fun
  ...
  test("inspector connection lifecycle", async () => {
    ...
    // connection state changes from Fun
...
describe("Socket Integration - Real Fun Process", () => {
  test("real Fun test runner with socket communication", async () => {
    // Create a temporary directory for our test
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-test-"));
...
test("test with $variable and %s format", () => {
  expect(true).toBe(true);
      ...
      // Create a signal for Fun to connect to
      ...
      // Spawn Fun with the test file
      const funProcess = spawn("fun", ["test", testFile, `--inspect-wait=${signal.url}`], {
        cwd: tmpDir,
        env: {
          ...process.env,
      ...
      // Verify that % formatters ARE expanded by Fun during discovery
  ...
  test.skip("debug: minimal socket test", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-minimal-"));
...
`,
      ...
      console.log("Spawning Fun with:", ["test", testFile, `--inspect-wait=${signal.url}`]);
      const funProcess = spawn("fun", ["test", testFile, `--inspect-wait=${signal.url}`], {
      ...
      // Log stdout/stderr
      funProcess.stdout?.on("data", data => {
        console.log("Fun stdout:", data.toString());
      });

      funProcess.stderr?.on("data", data => {
        console.log("Fun stderr:", data.toString());
      });

      // Wait for process to exit or timeout
      const exitCode = await Promise.race([
        new Promise<number>(resolve => {
          funProcess.on("exit", code => {
            console.log("Fun exited with code:", code);
            resolve(code ?? -1);
          });
        }),
  ...
  test.skip("debug: write test file to see what's wrong", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-debug-"));

    try {
      // Generate a simple test file
      let content = 'import { describe, test, expect } from "fun:test";\n\n';
      ...
      const { stdout, stderr, exitCode } = await Fun.$`fun test ${testFile}`.quiet();
  ...
  test("performance test with many regular tests", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-perf-"));
    ...
    try {
      ...
      // Spawn Fun
      const funProcess = spawn("fun", ["test", testFile, `--inspect-wait=${signal.url}`, "--timeout=100"]);
  ...
  test("stress test with deeply nested describes", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-stress-"));
...
describe("Socket Integration - Complex Edge Cases", () => {
  test("test.each with deeply nested and complex data structures", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-complex-"));
    let funProcess: ReturnType<typeof spawn> | null = null;
...
`,
      ...
      funProcess = spawn("fun", ["test", testFile, `--inspect-wait=${signal.url}`]);
  ...
  test("unicode and special characters in test names", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-unicode-"));
    let funProcess: ReturnType<typeof spawn> | null = null;
...
`,
  ...
  test("malformed test files and error scenarios", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-errors-"));
...
`,
      ...
      for (const [name, file] of [
        ...
        const funProcess = spawn("fun", ["test", file as string, `--inspect-wait=${signal.url}`], {
          stdio: ["ignore", "pipe", "pipe"],
        });
        ...
        const exitCode = await new Promise<number>(resolve => {
          funProcess.on("exit", code => resolve(code ?? -1));
        });
        ...
        if (name === "syntax error") {
          ...
          // Should handle large arrays - Fun might not find all 1000 immediately
  ...
  test.skip("race conditions and concurrent operations", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-race-"));
...
`;
      ...
      const funProcess1 = spawn("fun", ["test", testFile, `--inspect-wait=${signal1.url}`]);
...
`;
      ...
      const funProcess2 = spawn("fun", ["test", testFile, `--inspect-wait=${signal2.url}`]);

      await connection2Promise;

      // Wait for both processes
      await Promise.all([
        new Promise(resolve => funProcess1.on("exit", resolve)),
        new Promise(resolve => funProcess2.on("exit", resolve)),
      ]);
  ...
  test("memory stress test with thousands of tests", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-memory-"));

    try {
      // Generate a file with 5000 tests
      let content = 'import { test, expect } from "fun:test";\n\n';
      ...
      // Verify performance
      const foundEvents = events.filter(e => e.type === "found");
      // Fun might not discover all 5000 tests within timeout
  ...
  test("test discovery with .gitignore patterns", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-gitignore-"));
...
`,
  ...
  test("test name escaping and special regex characters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "fun-vscode-escaping-"));
    let funProcess: ReturnType<typeof spawn> | null = null;
...
`,
      ...
      if (funProcess && !funProcess.killed) {
        funProcess.kill("SIGKILL");
      }
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
