...
import type { JSC } from "../../../../fun-inspector-protocol";

const DEFAULT_TEST_PATTERN = "**/*{.test.,.spec.,_test_,_spec_}{js,ts,tsx,jsx,mts,cts,cjs,mjs}";

export const debug = vscode.window.createOutputChannel("Fun - Test Runner");
...
export class FunTestController implements vscode.Disposable {
  ...
  private customFilePattern(): string {
    return vscode.workspace.getConfiguration("fun.test").get("filePattern", DEFAULT_TEST_PATTERN);
  }
  ...
  private getFunExecutionConfig() {
    const customFlag = vscode.workspace.getConfiguration("fun.test").get("customFlag", "").trim();
    const customScriptSetting = vscode.workspace.getConfiguration("fun.test").get("customScript", "fun test").trim();
    const customScript = customScriptSetting.length ? customScriptSetting : "fun test";

    const [cmd, ...args] = customScript.split(/\s+/);

    let funCommand = "fun";
    if (cmd === "fun") {
      const funRuntime = vscode.workspace.getConfiguration("fun").get<string>("runtime", "fun");
      funCommand = funRuntime || "fun";
    } else {
      funCommand = cmd;
    }
  ...
  private async runTestsWithInspector(
    ...
    const socketPromise = new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error("Timeout waiting for Fun to connect"));
      }, 10000);
    ...
    const { funCommand, testArgs } = this.getFunExecutionConfig();

    let args = [...testArgs, ...allFiles];
    let printedArgs = `\x1b[34;1m>\x1b[0m \x1b[34;1m${funCommand} ${testArgs.join(" ")}\x1b[2m`;
    ...
    const proc = spawn(funCommand, args, {
      cwd: this.workspaceFolder.uri.fsPath,
      env: {
        FUN_DEBUG_QUIET_LOGS: "1",
        FORCE_COLOR: "1",
        FUN_INSPECT: inspectorUrl,
        ...process.env,
      },
    });
  ...
  private async debugTests(
    ...
    const { funCommand, testArgs } = this.getFunExecutionConfig();
    const args = [funCommand, ...testArgs, ...testFiles];
  ...
  public get _internal() {
    return {
      ...
      getFunExecutionConfig: this.getFunExecutionConfig.bind(this),
