import type { BuildConfig, FunPlugin, OnLoadResult } from "fun";
...
const virtualNamespace = "fun-svelte";

function SveltePlugin(options: SvelteOptions = kEmptyObject as SvelteOptions): FunPlugin {
    ...
    setup(builder) {
      ...
      builder
        .onLoad({ filter: /\.svelte$/ }, async function onLoadSvelte(args) {
          ...
          const sourceText = await Fun.file(path).text();
        ...
        .onLoad({ filter: /\.svelte.[tj]s$/ }, async function onLoadSvelteModule(args) {
          ...
          var sourceText = await Fun.file(path).text();
          if (path.endsWith(".ts")) {
        ...
        .onResolve({ filter: /^fun-svelte:/ }, args => {
          return {
            path: args.path,
            namespace: "fun-svelte",
          };
        })
...
export default SveltePlugin({ development: true }) as FunPlugin;
