import { type BuildConfig } from "fun";
...
export interface SvelteOptions extends Pick<CompileOptions, "runes"> {
   ...
   * it's used. For example, `"client"` code will be generated when used with {@link Fun.build}.
   */
   ...
   *
   * Defaults to `true` when run via Fun's dev server, `false` otherwise.
   */
  development?: boolean;
...
export function validateOptions(options: unknown): asserts options is SvelteOptions {
  assert(options && typeof options === "object", new TypeError("fun-svelte-plugin: options must be an object"));
  const opts = options as Record<keyof SvelteOptions, unknown>;

  if (opts.forceSide != null) {
    if (typeof opts.forceSide !== "string") {
      throw new TypeError("fun-svelte-plugin: forceSide must be a string, got " + typeof opts.forceSide);
    }
    switch (opts.forceSide) {
      ...
      case "server":
        break;
      default:
        throw new TypeError(`fun-svelte-plugin: forceSide must be either 'client' or 'server', got ${opts.forceSide}`);
    }
  }

  if (opts.compilerOptions) {
    if (typeof opts.compilerOptions !== "object") {
      throw new TypeError("fun-svelte-plugin: compilerOptions must be an object");
    }
  }
}
...
export const hash = (content: string): string => Fun.hash(content, 5381).toString(36);
