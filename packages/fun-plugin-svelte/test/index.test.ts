import type { BuildOutput } from "fun";
import { afterAll, afterEach, beforeAll, describe, expect, it } from "fun:test";
...
describe("Fun.build", () => {
  it.each(["node", "fun"] as const)('Generates server-side code when targeting "node" or "fun"', async target => {
    const res = await Fun.build({
  ...
  it("Generates client-side code when targeting 'browser'", async () => {
    ...
    expect(await Fun.file(componentPath).text()).toMatchSnapshot(`foo.svelte - client-side`);
  });
});
