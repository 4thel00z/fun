...
Now, inside the `lib.rs` file, we'll use the `fun_native_plugin::fun` proc macro to define a function which
will implement our native plugin.

Here's an example implementing the `onBeforeParse` hook:

```rs
use fun_native_plugin::{define_fun_plugin, OnBeforeParse, fun, Result, anyhow, FunLoader};
use napi_derive::napi;

/// Define the plugin and its name
define_fun_plugin!("replace-foo-with-bar");

/// Here we'll implement `onBeforeParse` with code that replaces all occurrences of
/// `foo` with `bar`.
///
/// We use the #[fun] macro to generate some of the boilerplate code.
///
...
pub fn replace_foo_with_bar(handle: &mut OnBeforeParse) -> Result<()> {
  ...
  handle.set_output_source_code(output_source_code, FunLoader::FUN_LOADER_JSX);

  Ok(())
}
```

Internally, the `#[fun]` macro wraps your code and declares a C ABI function which implements
the function signature of `onBeforeParse` plugins in Fun's C API for bundler plugins.
...
In the case that the value of the `Result` your plugin function returns is an `Err(...)`, the error will be logged to Fun's bundler.

It is highly advised that you return all errors and avoid `.unwrap()`'ing or `.expecting()`'ing results.

The `#[fun]` wrapper macro actually runs your code inside of a [`panic::catch_unwind`](https://doc.rust-lang.org/std/panic/fn.catch_unwind.html),
