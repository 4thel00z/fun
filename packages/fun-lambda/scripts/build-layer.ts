// HACK: https://github.com/underdoc-org/fun/issues/2081
...
export class BuildCommand extends Command {
  static summary = "Build a custom Lambda layer for Fun.";
  ...
  static flags = {
    ...
    release: Flags.string({
      description: "The release of Fun to install.",
      default: "latest",
    }),
    url: Flags.string({
      description: "A custom URL to download Fun.",
      exclusive: ["release"],
    }),
    output: Flags.file({
      exists: false,
      default: async () => "fun-lambda-layer.zip",
    }),
    layer: Flags.string({
  ...
  async run() {
    ...
    const { href } = new URL(url ?? `https://fun.dev/download/${release}/linux/${arch}?avx2=true`);
    this.log("Downloading...", href);
    const response = await fetch(href, {
      headers: {
        "User-Agent": "fun-lambda",
      },
    });
    ...
    const fun = archive.filter((_, { dir, name }) => !dir && name.endsWith("fun"))[0];
    if (!fun) {
      this.error("Failed to find executable in zip", { exit: 1 });
    }
    const cwd = fun.name.split("/")[0];
