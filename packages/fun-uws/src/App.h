...
namespace uWS {
    ...
    struct SocketContextOptions {
        ...
        operator struct us_fun_socket_context_options_t() const {
            struct us_fun_socket_context_options_t socket_context_options;
            memcpy(&socket_context_options, this, sizeof(SocketContextOptions));
            return socket_context_options;
        }
    };

    static_assert(sizeof(struct us_fun_socket_context_options_t) == sizeof(SocketContextOptions), "Mismatching uSockets/uWebSockets ABI");
...
public:
    ...
    TemplatedApp &&addServerName(const std::string &hostname_pattern, SocketContextOptions options = {}, bool *success = nullptr) {
        ...
        if constexpr (SSL) {
            enum create_fun_socket_error_t err = CREATE_FUN_SOCKET_ERROR_NONE;
     ...
     * would dangle. Fun always uses TemplatedApp::create() anyway. */
