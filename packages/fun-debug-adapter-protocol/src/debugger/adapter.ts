...
import { remoteObjectToString, WebSocketInspector } from "../../../fun-inspector-protocol/index.ts";
import type { Inspector, InspectorEventMap } from "../../../fun-inspector-protocol/src/inspector/index.d.ts";
import { NodeSocketInspector } from "../../../fun-inspector-protocol/src/inspector/node-socket.ts";
import type { JSC } from "../../../fun-inspector-protocol/src/protocol/index.d.ts";
...
function normalizeSourcePath(sourcePath: string, untitledDocPath?: string, funEvalPath?: string): string {
  if (!sourcePath) return sourcePath;

  // Handle eval source paths
  if (sourcePath === funEvalPath) {
    return funEvalPath!;
  }
  ...
  implements IDebugAdapter
{
  ...
  protected constructor(inspector: T, untitledDocPath?: string, funEvalPath?: string) {
    super();
    this.#untitledDocPath = untitledDocPath;
    this.#funEvalPath = funEvalPath;
  ...
  #getBreakpointByLocation(source: Source, location: DAP.SourceBreakpoint): Breakpoint | undefined {
    ...
    if (sourceId === untitledDocPath && this.#funEvalPath) {
      sourceId = this.#funEvalPath;
    }
  ...
  #addSource(source: Source): Source {
    ...
    if (path) {
      path = source.path = normalizeSourcePath(path, this.#untitledDocPath, this.#funEvalPath);
    }
  ...
  async #getSource(sourceId: string | number): Promise<Source> {
    ...
    if (typeof sourceId === "string") {
      sourceId = normalizeSourcePath(sourceId, this.#untitledDocPath, this.#funEvalPath);
    }
...
export class NodeSocketDebugAdapter extends BaseDebugAdapter<NodeSocketInspector> {
  public constructor(socket: Socket, untitledDocPath?: string, funEvalPath?: string) {
    super(new NodeSocketInspector(socket), untitledDocPath, funEvalPath);
...
export class WebSocketDebugAdapter extends BaseDebugAdapter<WebSocketInspector> {
  ...
  public constructor(url?: string | URL, untitledDocPath?: string, funEvalPath?: string) {
    super(new WebSocketInspector(url), untitledDocPath, funEvalPath);
  }
  ...
  async #launch(request: LaunchRequest): Promise<void> {
    ...
    if (process.platform !== "win32") {
      ...
      processEnv["FUN_INSPECT"] = `${url}?${query}`;
      processEnv["FUN_INSPECT_NOTIFY"] = signal.url;

      // This is probably not correct, but it's the best we can do for now.
      processEnv["FORCE_COLOR"] = "1";
      processEnv["FUN_QUIET_DEBUG_LOGS"] = "1";
      processEnv["FUN_DEBUG_QUIET_LOGS"] = "1";
      ...
      const query = stopOnEntry ? "break=1" : "wait=1";
      processEnv["FUN_INSPECT"] = `${url}?${query}`;
      processEnv["FUN_INSPECT_NOTIFY"] = signal.url; // 127.0.0.1 so it resolves correctly on windows
