...
async function getJSC(): Promise<Protocol> {
  let funExecutable = Fun.which("fun-debug") || process.execPath;
  if (!funExecutable) {
    throw new Error("fun-debug not found");
  }
  funExecutable = realpathSync(funExecutable);
}
...
const runner = "Fun" in globalThis ? "funx" : "npx";
...
let jscJSONFile;
try {
  jscJSONFile = await Fun.file(jscJsonFile).json();
} catch (error) {
  console.warn("Failed to read CombinedDomains.json from WebKit build. Is this a WebKit build from Fun?");
  console.error(error);
  process.exit(1);
}
