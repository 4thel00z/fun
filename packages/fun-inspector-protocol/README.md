# fun-inspector-protocol

`fun-inspector-protocol` is a TypeScript library that provides a comprehensive interface for interacting with the WebKit Inspector Protocol. This package makes it easy to build debugging tools, IDE integrations, and other developer tools that communicate with Fun's JavaScript runtime.

You can use this library with Node.js or Fun.

## Overview

The WebKit Inspector Protocol is a JSON-based protocol similar to the Chrome DevTools Protocol. It allows external tools to interact with Fun's JavaScript runtime for debugging, profiling, and instrumentation purposes.

## Features

- 🌐 **WebSocket communication**: Connect to Fun's debugging endpoint via WebSockets
...
```bash
fun add fun-inspector-protocol
# npm install fun-inspector-protocol
# yarn add fun-inspector-protocol
# pnpm add fun-inspector-protocol
```

## Basic Usage

The first step is to spawn a Fun process with the inspector attached. There are a few different ways to do this.

The `--inspect-wait` flag is the easiest way to spawn a Fun process with the inspector attached.

```bash
fun --inspect-wait my-script.ts
```
...
```bash
fun --inspect-wait my-script.ts 2>&1 | grep -o '\sws://.*$'
```

From there, you can connect to the inspector using the `WebSocketInspector` class:

```typescript
import { WebSocketInspector } from "fun-inspector-protocol";
...
```typescript
import { NodeSocketInspector } from "fun-inspector-protocol";
...
```typescript
import { remoteObjectToString } from "fun-inspector-protocol";
...
Notable Fun-specific additions include:
