declare module "fun" {
  /**
   * `fun install` security related declarations
   */
  export namespace Security {
    export interface Package {
      ...
      /**
       * The URL of the tgz of this package that Fun will download
       */
      tarball: string;
    ...
    export interface Advisory {
       ...
       *
       * Fun behaves differently depending on the values returned from the
       * {@link Scanner.scan `scan()`} hook:
       *
       * > In any case, Fun *always* pretty prints *all* the advisories,
       * > but...
       * >
       * > → if any **fatal**, Fun will immediately cancel the installation
       * > and quit with a non-zero exit code
       * >
       * > → else if any **warn**, Fun will either ask the user if they'd like
      ...
      /**
       * If available, this is a brief description of the advisory that Fun
    ...
    export interface Scanner {
       ...
       * are inherently unrelated to Fun versions
       */
      version: "1";

      /**
       * Perform an advisory check when a user ran `fun add <package>
       * [...packages]` or other related/similar commands.
       *
       * If this function throws an error, Fun will immediately stop the
