declare module "fun" {
  ...
  interface FileSink {
    ...
    /**
     * For FIFOs & pipes, this lets you decide whether Fun's process should
     ...
     * remain alive until the pipe is closed.
     *
     * If you want to allow Fun's process to terminate while the stream is open,
     * call this.
     *
     * If the file is not a FIFO or pipe, {@link ref} and {@link unref} do
     * nothing. If the pipe is already closed, this does nothing.
  ...
  class S3Client {
     ...
     *     await bucket.write("my-file.txt", "Hello Fun!");
     *
     */
   ...
   *
   * Pulls credentials from environment variables. Use `new Fun.S3Client()` if you need to explicitly set credentials.
   *
   * @category Cloud Storage
   */
  var s3: S3Client;
