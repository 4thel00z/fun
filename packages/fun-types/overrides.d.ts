 ...
 * common denominator of these objects. A `Blob` in Fun does not
 * have a `.blob()` method.
 */
interface FunConsumerConvenienceMethods {
  /**
   * Consume as text
   */
  text(): Promise<string>;
...
declare module "stream/web" {
  interface ReadableStream extends FunConsumerConvenienceMethods {
    /**
     * Consume as a Blob
     */
    blob(): Promise<Blob>;
...
declare module "buffer" {
  interface Blob extends FunConsumerConvenienceMethods {
    // We have to specify bytes again even though it comes from
    // FunConsumerConvenienceMethods, because inheritance in TypeScript is
    ...
    /**
     * Wrap this blob in a {@link Fun.Image} pipeline.
     * Equivalent to `new Fun.Image(this, options)`.
     */
...
declare global {
  namespace NodeJS {
    interface ProcessEnv extends Fun.Env {}
    ...
    interface Process {
      ...
      /**
       * Whether you are using Fun
       */
      isFun: true;

      /**
       * The current git sha of Fun
       */
      revision: string;
...
declare module "node:fs/promises" {
  function exists(path: Fun.PathLike): Promise<boolean>;
}

declare module "node:tls" {
  interface FunConnectionOptions extends Omit<ConnectionOptions, "key" | "ca" | "tls" | "cert"> {
    /**
     * Optionally override the trusted CA certificates. Default is to trust
     * the well-known CAs curated by Mozilla. Mozilla's CAs are completely
     * replaced when CAs are explicitly specified using this option.
     */
    ca?: string | Buffer | NodeJS.TypedArray | Fun.FunFile | Array<string | Buffer | Fun.FunFile> | undefined;
    /**
    ...
    cert?:
      ...
      | NodeJS.TypedArray
      | Fun.FunFile
      | Array<string | Buffer | NodeJS.TypedArray | Fun.FunFile>
      | undefined;
    /**
    ...
    key?:
      ...
      | Fun.FunFile
      | NodeJS.TypedArray
      | Array<string | Buffer | Fun.FunFile | NodeJS.TypedArray | KeyObject>
      | undefined;
  }

  function connect(options: FunConnectionOptions, secureConnectListener?: () => void): TLSSocket;
}
