declare module "fun" {
   ...
   *
   * This is slightly different than the browser {@link WebSocket} which Fun supports for clients.
   *
  ...
  /**
   * Create a server-side {@link ServerWebSocket} handler for use with {@link Fun.serve}
   *
   * @example
   * ```ts
   * import { websocket, serve } from "fun";
   *
   * serve<{name: string}>({
   *   port: 3000,
   *   websocket: {
  ...
  interface WebSocketHandler<T> {
     ...
     *
     * This pattern exists in Fun due to a [TypeScript limitation (#26242)](https://github.com/microsoft/TypeScript/issues/26242)
     *
     * @example
     * ```ts
     * Fun.serve({
  ...
  namespace Serve {
    ...
    /**
     * Development configuration for {@link Fun.serve}
     */
    type Development =
      | boolean
      | {
    ...
    type BaseRouteValue = Response | false | HTMLBundle | FunFile;

    type Routes<WebSocketData, R extends string> = {
      [Path in R]:
        | BaseRouteValue
        | Handler<FunRequest<Path>, Server<WebSocketData>, Response>
        | Partial<Record<HTTPMethod, Handler<FunRequest<Path>, Server<WebSocketData>, Response> | Response>>;
    };

    type RoutesWithUpgrade<WebSocketData, R extends string> = {
      [Path in R]:
        | BaseRouteValue
        | Handler<FunRequest<Path>, Server<WebSocketData>, Response | undefined | void>
        | Partial<
            Record<HTTPMethod, Handler<FunRequest<Path>, Server<WebSocketData>, Response | undefined | void> | Response>
          >;
    };
    ...
    type FetchOrRoutesWithWebSocket<WebSocketData, R extends string> = {
      /**
       * Enable websockets with {@link Fun.serve}
       *
       ...
       * @example
       * ```js
       * const server: Fun.Server = Fun.serve({
       *  websocket: {
       *    open: (ws) => {
       *      console.log("Client connected");
       *    },
    ...
    interface BaseServeOptions<WebSocketData> {
      ...
      /**
       * Render contextual errors? This enables fun's error page
       ...
       * ---
       *
       * **When fun is started with the `--hot` flag**:
       *
       ...
       *
       * **When fun is not started with the `--hot` flag**:
       *
     ...
     *     },
     *   },
     * } satisfies Fun.Serve.Options<{ name: string }>;
     * ```
     */
    type Options<WebSocketData, R extends string = string> = Fun.__internal.XOR<
      HostnamePortServeOptions<WebSocketData>,
      UnixServeOptions<WebSocketData>
    > &
      Fun.__internal.XOR<FetchOrRoutes<WebSocketData, R>, FetchOrRoutesWithWebSocket<WebSocketData, R>>;
  }

  interface FunRequest<T extends string = string> extends Request {
    readonly params: {
   ...
   *
   * For performance, Fun pre-allocates most of the data for 2048 concurrent requests.
   * That means starting a new server allocates about 500 KB of memory. Try to
   * avoid starting and stopping the server often (unless it's a new instance of fun).
   *
  ...
  interface Server<WebSocketData> extends Disposable {
     ...
     * ```js
     * import { serve } from "fun";
     * const server: Fun.Server<{ user: string }> = serve({
     *   websocket: {
     *     open: (ws) => {
     *       console.log("Client connected");
     *     },
      ...
      ...options: [WebSocketData] extends [undefined]
        ? [
            options?: {
               ...
               *
               * Tell TypeScript about the WebSocket data by using `Fun.Server<MyWebSocketData>`
               *
               * ```ts
               * const server: Fun.Server<MyWebSocketData> = Fun.serve({
               *      fetch: (req, server) => {
     ...
     *
     * In development mode, `Fun.serve()` returns rendered error messages with
     ...
     *
     * When fun is started with the `--hot` flag, this ID is used to hot reload the server without interrupting pending requests or websockets.
     *
     * When fun is not started with the `--hot` flag, this ID is currently unused.
     */
    readonly id: string;
  }

  /**
   * Fun.serve provides a high-performance HTTP server with built-in routing support.
   ...
   *   routes: {
   *     // Single parameter
   *     "/users/:id": (req: FunRequest<"/users/:id">) => {
   *       return new Response(`User ID: ${req.params.id}`);
   *     },
   *
   *     // Multiple parameters
   *     "/posts/:postId/comments/:commentId": (
   *       req: FunRequest<"/posts/:postId/comments/:commentId">
   *     ) => {
