 ...
 * ```
 *
 * This module aliases `globalThis.Fun`.
 */
declare module "fun" {
  ...
  type MessageEventSource = Fun.__internal.UseLibDomIfAvailable<"MessageEventSource", undefined>;
  ...
  namespace __internal {
     ...
     *
     * This is to avoid type conflicts between lib.dom.d.ts and \@types/fun.
     *
     * Unfortunately some symbols cannot be defined when both Fun types and lib.dom.d.ts types are loaded,
     ...
     * properties from union options objects, like {@link Fun.SQL.Options}
     *
     * @example
     * ```ts
     * type X = Fun.DistributedOmit<{type?: 'a', url?: string} | {type?: 'b', flag?: boolean}, "url">
  ...
  interface MessageEventInit<T = any> extends EventInit {
    ...
    lastEventId?: string;
    origin?: string;
    source?: Fun.MessageEventSource | null;
  }
  ...
  interface CustomEventInit<T = any> extends Fun.EventInit {
    detail?: T;
  }

  /** A message received by a target object. */
  interface FunMessageEvent<T = any> extends Event {
    ...
    readonly source: Fun.MessageEventSource | null;
  }

  type MessageEvent<T = any> = Fun.__internal.UseLibDomIfAvailable<"MessageEvent", FunMessageEvent<T>>;
  ...
  /**
   * Fun's Web Worker constructor supports some extra options on top of the API browsers have.
   */
  interface WorkerOptions {
    ...
    /**
     * In Fun, this does nothing.
     */
    type?: Fun.WorkerType | undefined;

    /**
     * List of arguments which would be stringified and appended to
     * `Fun.argv` / `process.argv` in the worker. This is mostly similar to the `data`
     * but the values will be available on the global `Fun.argv` as if they
   ...
   *
   * Defaults to `process.env` as it was when the current Fun process launched.
   *
  ...
  /**
   * The raw arguments passed to the process, including flags passed to Fun. If you want to easily read flags passed to your script, consider using `process.argv` instead.
   */
  const argv: string[];
   ...
   * existing code can be easily ported to Fun and vice versa.
   *
   * @returns The width of the string in columns
   *
   * @example
   * ```ts
   * import { stringWidth } from "fun";
   *
  ...
  namespace JSONL {
    /**
     * The result of `Fun.JSONL.parseChunk`.
     */
     ...
     *   const { values, read, error } = Fun.JSONL.parseChunk(buffer);
   ...
   * const html = Fun.markdown.html("# Hello **world**");
   * // "<h1>Hello <strong>world</strong></h1>\n"
   *
   * // Render with custom callbacks
   * const ansi = Fun.markdown.render("# Hello **world**", {
   ...
   *   return Fun.markdown.react(text);
   * }
   *
   * // With component overrides
   * const element = Fun.markdown.react("# Hello", { h1: MyHeadingComponent });
   * ```
   */
  namespace markdown {
    /**
     ...
     * }
     * Fun.markdown.react(text, { pre: Code });
     * ```
     */
     ...
     *
     * // With options
     * const html = Fun.markdown.html("## Hello", { headings: { ids: true } });
     ...
     * @example
     * ```ts
     * const out = Fun.markdown.ansi("# Hello\n\n**bold** and *italic*\n");
     * process.stdout.write(out);
     *
     * // Plain text, no escape codes
     * const plain = Fun.markdown.ansi("# Hello", { colors: false });
     *
     * // Enable clickable OSC 8 hyperlinks
     * const linked = Fun.markdown.ansi("[docs](https://fun.dev)", {
     *   hyperlinks: true,
     * });
     *
     * // Inline images via Kitty Graphics Protocol
     * const withImg = Fun.markdown.ansi("![alt](./logo.png)", {
     *   kittyGraphics: true,
     * });
     *
     * // Custom width
     * const wrapped = Fun.markdown.ansi(longText, { columns: 60 });
     * ```
     */
     ...
     * const html = Fun.markdown.render("# Title\n\nHello **world**", {
     ...
     * const ansi = Fun.markdown.render("# Hello\n\n**bold**", {
     ...
     * const text = Fun.markdown.render("Visit www.example.com", {
     ...
     * const html = renderToString(Fun.markdown.react("# Hello **world**"));
     *
     ...
     * }
     * const el = Fun.markdown.react(text, { pre: Code, a: Link });
     *
     * // For React 18 and older
     * const el18 = Fun.markdown.react(text, undefined, { reactVersion: 18 });
     * ```
     */
   ...
   */
  function write(
    destination: FunFile | S3File | PathLike,
  ...
  function write(
    ...
    options?: {
       ...
       * @example
       * ```ts
       * await Fun.write(Fun.file("./secret.txt"), Fun.file("./source.txt"), { mode: 0o600 });
       * ```
       */
      mode?: number;
      /**
  ...
  function write(
    ...
    options?: {
       ...
       * @example
       * ```ts
       * await Fun.write("./secret.txt", Fun.file("./source.txt"), { mode: 0o600 });
       * ```
       */
      mode?: number;
      /**
   ...
   * const formData = await Fun.readableStreamToFormData(stream, boundary);
   ...
   * @example
   * ```js
   * const url = Fun.pathToFileURL("/foo/bar.txt");
   ...
   * @example
   * ```js
   * const path = Fun.fileURLToPath(new URL("file:///foo/bar.txt"));
  ...
  namespace dns {
     ...
     * ## Basic usage
     * ```js
     * const [{ address }] = await Fun.dns.lookup('example.com');
     * ```
     *
     ...
     * ## DNS resolver client
     *
     * Fun supports three DNS resolvers:
    ...
    function lookup(
      ...
      options?: {
         ...
         * may change in a future version of Fun if c-ares is not reliable
         * enough.
         *
         ...
         * hostname ends with `.local` or `.localhost`, Fun will automatically
         * use `system` instead of `c-ares`.
         *
         * [`getaddrinfo`](https://man7.org/linux/man-pages/man3/getaddrinfo.3.html)
         * is the POSIX standard function for blocking DNS resolution. Fun runs
         * it in Fun's thread pool, which is limited to `cpus / 2`. That means
     ...
     * Prefetch a hostname.
     *
     * This will be used by fetch() and Fun.connect() to avoid DNS lookups.
     *
  ...
  interface FileBlob extends FunFile {}
  /**
   ...
   * @example
   * ```js
   * const file = Fun.file("./hello.json");
   ...
   * ```
   */
  interface FunFile extends Blob {
    /**
     * Offset any operation on the file starting at `begin` and ending at `end`. `end` is relative to 0
     *
     * Similar to [`TypedArray.subarray`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypedArray/subarray). Does not copy the file, open the file, or modify the file.
     *
     * If `begin` > 0, {@link Fun.write()} will be slower on macOS
     *
     * @param begin - start offset in bytes
     * @param end - absolute offset in bytes (relative to 0)
     * @param contentType - MIME type for the new FunFile
     */
    slice(begin?: number, end?: number, contentType?: string): FunFile;
     ...
     * @param contentType - MIME type for the new FunFile
     */
    slice(begin?: number, contentType?: string): FunFile;
     ...
     * @param contentType - MIME type for the new FunFile
     */
    slice(contentType?: string): FunFile;
     ...
     * Response(Fun.file(path))` and then an `error` handler to handle
     * exceptions.
     *
    ...
    /**
     * Write data to the file. This is equivalent to using {@link Fun.write} with a {@link FunFile}.
     * @param data - The data to write.
     * @param options - The options to use for the write.
     */
    write(
      data: string | ArrayBufferView | ArrayBuffer | SharedArrayBuffer | Request | Response | FunFile,
   ...
   *   {
   *     "react-relay": {
   *       "graphql": "fun-macro-relay/fun-macro-relay.tsx"
   *     }
   *   }
   *  ```
   */
   ...
   *
   * This also powers expect().toEqual in `fun:test`
   */
  function deepEquals(
    a: any,
    b: any,
   ...
   *
   * This also powers expect().toMatchObject in `fun:test`
   */
  function deepMatch(subset: unknown, a: unknown): boolean;

  /**
   * tsconfig.json options supported by Fun
   */
  interface TSConfig {
    extends?: string;
    compilerOptions?: {
  ...
  class Transpiler {
     ...
     */
    transform(code: Fun.StringOrBuffer, loader?: JavaScriptLoader): Promise<string>;
    /**
     * Transpile code from TypeScript or JSX into valid JavaScript.
     * This function does not resolve imports.
     * @param code The code to transpile
     */
    transformSync(code: Fun.StringOrBuffer, loader: JavaScriptLoader, ctx: object): string;
    /**
     * Transpile code from TypeScript or JSX into valid JavaScript.
     * This function does not resolve imports.
     * @param code The code to transpile
     ...
     */
    transformSync(code: Fun.StringOrBuffer, ctx: object): string;
     ...
     * @param code The code to transpile
     */
    transformSync(code: Fun.StringOrBuffer, loader?: JavaScriptLoader): string;
     ...
     * ```
     */
    scan(code: Fun.StringOrBuffer): { exports: string[]; imports: Import[] };
     ...
     */
    scanImports(code: Fun.StringOrBuffer): Import[];
  }
  ...
  namespace Build {
    ...
    type CompileTarget =
      | `fun-darwin-${Architecture}`
      | `fun-darwin-${Architecture}-${SIMD}`
      | `fun-linux-${Architecture}`
      | `fun-linux-${Architecture}-${Libc}`
      | `fun-linux-${Architecture}-${SIMD}`
      | `fun-linux-${Architecture}-${SIMD}-${Libc}`
      | `fun-windows-${Architecture}`
      | `fun-windows-x64-${SIMD}`;
  }

  /**
   * @see [Fun.build API docs](https://fun.dev/docs/bundler#api)
   */
  interface BuildConfig {
     ...
     *
     * Equivalent to `--conditions` in `fun build` or `fun run`.
     *
    ...
    /**
     * Enable feature flags for dead-code elimination via `import { feature } from "fun:bundle"`.
     *
     ...
     * if (result.metafile) {
     *   await Fun.write('./dist/meta.json', result.metafile);
     * }
     *
     ...
     * ```
     */
    compile?: boolean | Fun.Build.CompileTarget | CompileBuildOptions;
  }

  interface CompileBuildOptions {
    target?: Fun.Build.CompileTarget;
    execArgv?: string[];
    ...
    /**
     * Whether to autoload funfig.toml when the standalone executable runs
     *
     * Standalone-only: applies only when building/running the standalone executable.
     *
     * Equivalent CLI flags: `--compile-autoload-funfig`, `--no-compile-autoload-funfig`
     *
     * @default true
     */
   ...
   *
   * @see [Fun.password API docs](https://fun.dev/guides/util/hash-a-password)
   *
   * @category Security
   */
  namespace Password {
  ...
  const password: {
    ...
    verify(
       ...
       */
      password: Fun.StringOrBuffer,
      /**
       * Previously hashed password.
       * If empty, always returns false
       */
      hash: Fun.StringOrBuffer,
      /**
     ...
     * Warning: password hashing is slow, consider using {@link Fun.password.verify}
    ...
    /**
     * Synchronously hash and verify passwords using argon2 or bcrypt. The default is argon2.
     * Warning: password hashing is slow, consider using {@link Fun.password.hash}
     * instead which runs in a worker thread.
     *
     * The underlying implementation of these functions are provided by the Zig
     * Standard Library. Thanks to \@jedisct1 and other Zig contributors for their
  ...
  const secrets: {
     ...
     * @example
     * ```ts
     * const password = await Fun.secrets.get({
     *   service: "my-database",
     *   name: "admin"
     * });
     *
     ...
     * const token =
     *   await Fun.secrets.get({ service: "github", name: "token" }) ||
     *   await Fun.secrets.get({ service: "gh-cli", name: "github.com" }) ||
     ...
     * const deleted = await Fun.secrets.delete({
     *   service: "my-app",
     *   name: "api-key"
     * });
     *
  ...
  /**
   * A build artifact represents a file that was generated by the bundler @see {@link Fun.build}
   *
   * @category Bundler
   */
  ...
  interface BuildOutput {
     ...
     *   await Fun.write('./dist/meta.json', JSON.stringify(result.metafile));
     * }
     * ```
     */
   ...
   * @throws {AggregateError} When build fails and config.throw is true (default in Fun 1.2+)
   *
   * @category Bundler
   *
   * @example
   ...
   *   await Fun.write(artifact.path, artifact);
   * }
   *```
   *
   * @example
   ...
   *           const content = await Fun.file(args.path).text();
   *           return {
  ...
  interface TLSOptions {
     ...
     */
    ca?: string | BufferSource | FunFile | Array<string | BufferSource | FunFile> | undefined;
    /**
     ...
     */
    cert?: string | BufferSource | FunFile | Array<string | BufferSource | FunFile> | undefined;
    /**
     ...
     */
    key?: string | BufferSource | FunFile | Array<string | BufferSource | FunFile> | undefined;
    /**
   ...
   */
  function file(path: string | URL, options?: BlobPropertyBag): FunFile;
   ...
   * @example
   * ```js
   * const file = Fun.file(new TextEncoder.encode("./hello.json"));
   * console.log(file.type); // "application/json"
   * ```
   *
   ...
   */
  function file(path: ArrayBufferLike | Uint8Array<ArrayBuffer>, options?: BlobPropertyBag): FunFile;
   ...
   * @example
   * ```js
   * const file = Fun.file(fd);
   * ```
   *
   * @param fileDescriptor The file descriptor of the file
   */
  function file(fileDescriptor: number, options?: BlobPropertyBag): FunFile;
  ...
  /**
   * Options for `Fun.inspect`
   */
  interface FunInspectOptions {
    /**
  ...
  type WebSocketOptionsTLS = {
     ...
     * @example
     * ```ts
     * // Using FunFile for certificates
  ...
  /**
   * Constructor options for the `Fun.WebSocket` client
   */
   ...
   */
  function inspect(arg: any, options?: FunInspectOptions): string;
  namespace inspect {
    /**
   ...
   * - If you delete or truncate the file, that will crash fun. This is called a segmentation fault.
   *
   * ---
   *
   ...
   * Write to stdout
   */
  const stdout: FunFile;

  /**
   * Write to stderr
   */
  const stderr: FunFile;

  /**
   * Read from stdin
   *
   * This is a read-only FunFile
   */
  const stdin: FunFile;
  ...
  /**
   * Fun.semver provides a fast way to parse and compare version numbers.
   */
  namespace semver {
    /**
  ...
  namespace unsafe {
     ...
     *
     * `FUN_GARBAGE_COLLECTOR_LEVEL` environment variable is also supported.
     *
     * @param level
   ...
   *
   * If you have any ideas, please file an issue https://github.com/underdoc-org/fun
   */
   ...
   *
   * Generate a heap snapshot in JavaScriptCore's format that can be viewed with `fun --inspect` or Safari's Web Inspector
   */
   ...
   * ```ts
   * const snapshot = Fun.generateHeapSnapshot("v8");
   * await Fun.write("heap.heapsnapshot", snapshot);
   * ```
   */
   ...
   * ```ts
   * const snapshot = Fun.generateHeapSnapshot("v8", "arraybuffer");
   * await Fun.write("heap.heapsnapshot", snapshot);
   * ```
   */
  ...
  abstract class CryptoHashInterface<T> {
     ...
     * @param data
     */
    update(data: Fun.BlobOrStringOrBuffer): T;
     ...
     * @param hashInto `TypedArray` to write the hash into. Faster than creating a new one each time
     */
    static hash(input: Fun.BlobOrStringOrBuffer, hashInto?: NodeJS.TypedArray): NodeJS.TypedArray;
     ...
     */
    static hash(input: Fun.BlobOrStringOrBuffer, encoding: DigestEncoding): string;
  }
  ...
  class CryptoHasher {
     ...
     * @param input
     */
    update(input: Fun.BlobOrStringOrBuffer, inputEncoding?: import("crypto").Encoding): CryptoHasher;
     ...
     * @param input `string`, `Uint8Array`, or `ArrayBuffer` to hash. `Uint8Array` or `ArrayBuffer` is faster.
     */
    static hash(algorithm: SupportedCryptoAlgorithms, input: Fun.BlobOrStringOrBuffer): Buffer;
    ...
    static hash(
      ...
      input: Fun.BlobOrStringOrBuffer,
   ...
   * await Fun.sleep(target);
   * ```
   * Internally, `Fun.sleep` is the equivalent of
   * ```ts
   * await new Promise((resolve) => setTimeout(resolve, ms));
   * ```
   * As always, you can use `Fun.sleep` or the imported `sleep` function interchangeably.
   */
   ...
   * ```
   */
  function sha(input: Fun.StringOrBuffer, hashInto?: NodeJS.TypedArray): NodeJS.TypedArray;
   ...
   * ```
   */
  function sha(input: Fun.StringOrBuffer, encoding: DigestEncoding): string;
  ...
  /** Compression options for `Fun.deflateSync` and `Fun.gzipSync` */
  ...
  type Target =
    /**
     * For generating bundles that are intended to be run by the Fun runtime. In many cases,
     ...
     *
     * All bundles generated with `target: "fun"` are marked with a special `// @fun` pragma, which
     * indicates to the Fun runtime that there's no need to re-transpile the file before execution.
     */
    | "fun"
    /**
  ...
  /** https://fun.dev/docs/bundler/loaders */
  type Loader =
    | "js"
    | "jsx"
    | "ts"
  ...
  interface PluginConstraints {
     ...
     * this string
     *
     * Namespaces are prefixes in import specifiers. For example, `"fun:ffi"`
     * has the namespace `"fun"`.
     *
  ...
  interface OnLoadResultSourceCode {
     ...
     *
     * "css" will be added in a future version of Fun.
     */
    loader?: Loader;
  }
  ...
  type FFIFunctionCallable = Function & {
    ...
    readonly __ffi_function_callable: typeof import("fun:ffi").FFIFunctionCallableSymbol;
  };

  /**
   * The builder object passed to `Fun.plugin`
   *
   * @category Bundler
   */
  interface PluginBuilder {
    ...
    /**
     * The config object passed to `Fun.build` as is. Can be mutated.
     */
    config: BuildConfig & { plugins: FunPlugin[] };
  ...
  /**
   * A Fun plugin. Used for extending Fun's behavior at runtime, or with {@link Fun.build}
   *
   * @category Bundler
   */
  interface FunPlugin {
     ...
     *
     * In a future version of Fun, this will be used in error messages.
     */
    name: string;

    /**
     * The target JavaScript environment the plugin should be applied to.
     * - `fun`: The default environment when using `fun run` or `fun` to load a script
     ...
     *
     * This field is not read by {@link Fun.plugin}, only {@link Fun.build} and `fun build`
     */
    target?: Target;
  ...
  /**
   * Extend Fun's module resolution and loading behavior
   *
   ...
   * application. A future version of Fun may also support specifying plugins
   * via `funfig.toml`.
   *
   * @example
   ...
   * ```
   */
  interface FunRegisterPlugin {
    <T extends FunPlugin>(options: T): ReturnType<T["setup"]>;
  ...
  const plugin: FunRegisterPlugin;
  ...
  interface HTMLBundle {
    ...
    /** Array of generated output files with metadata. This only exists when built ahead of time with `Fun.build` or `fun build` */
    files?: Array<{
   ...
   *
   * Sockets are created using `Fun.connect()` or accepted by a `Fun.listen()` server.
   *
  ...
  interface Socket<Data = undefined> extends Disposable {
     ...
     * This can be set when the socket is created via `Fun.connect({ data: ... })`.
    ...
    /**
     * Keep Fun's process alive at least until this socket is closed
     *
    ...
    /**
     * Allow Fun's process to exit even if this socket is still open
     *
    ...
    /**
     * Reset the socket's callbacks. This is useful with `fun --hot` to facilitate hot reloading.
     *
     * This will apply to all sockets from the same {@link Listener}. it is per socket only for {@link Fun.connect}.
     */
     ...
     *
     * This will return undefined if the socket was created by {@link Fun.connect} or if the listener has already closed.
     */
  ...
  interface SocketHandler<Data = unknown, DataBinaryType extends BinaryType = "buffer"> {
     ...
     *
     * The promise returned by `Fun.connect` rejects **after** this function is
     * called.
     *
     ...
     *
     * Fun originally defaulted to `Uint8Array` but when dealing with network
  ...
  /**
   * @deprecated use {@link Fun.Spawn} instead
   */
  ...
  namespace Spawn {
    ...
    interface BaseOptions<In extends Writable, Out extends Readable, Err extends Readable> {
       ...
       * - `ArrayBufferView`, `Blob`, `Fun.file()`, `Response`, `Request`: The process will read from buffer/stream.
       ...
       *
       * Warning: this may run before the `Fun.spawn` function returns.
       *
       ...
       *   `SIGILL`, `SIGKILL`, etc.).
       * - The child replaced itself with a program that does not support Fun
       *   IPC.
       *
       * Notes:
      ...
      /**
       * When specified, Fun will open an IPC channel to the subprocess. The passed callback is called for
       ...
       *
       * Currently, this is only compatible with processes that are other `fun` instances.
       */
      ipc?(
        message: any,
        /**
    ...
    interface SpawnOptions<In extends Writable, Out extends Readable, Err extends Readable>
       ...
       * ```ts
       * const terminal = new Fun.Terminal({ ... });
       * const proc1 = Fun.spawn(["echo", "first"], { terminal });
       * await proc1.exited;
       * const proc2 = Fun.spawn(["echo", "second"], { terminal });
       * await proc2.exited;
       * terminal.close();
       * ```
       */
    ...
    type ReadableToIO<X extends Readable> = X extends "pipe" | undefined
      ...
      : X extends FunFile | ArrayBufferView | number
        ? number
        : undefined;
    ...
    type WritableToIO<X extends Writable> = X extends "pipe"
      ? FileSink
      : X extends FunFile | ArrayBufferView | Blob | Request | Response | number
        ? number
        : undefined;
  }
  ...
  /**
   * A process created by {@link Fun.spawn}.
   *
  ...
  > extends AsyncDisposable {
     ...
     * @example
     * ```ts
     * const { pid } = Fun.spawn({ cmd: ["echo", "hello"] });
    ...
    /**
     * This method will tell Fun to wait for this process to exit after you already
     * called `unref()`.
     *
     * Before shutting down, Fun will wait for all subprocesses to exit by default
     */
    ref(): void;
     ...
     * Before shutting down, Fun will wait for all subprocesses to exit by default
     *
     * This method will tell Fun to not wait for this process to exit before shutting down.
     */
    unref(): void;

    /**
     * Send a message to the subprocess. This is only supported if the subprocess
     * was created with the `ipc` option, and is another instance of `fun`.
     *
  ...
  /**
   * A process created by {@link Fun.spawnSync}.
   *
  ...
  >(
    options: SpawnOptions.SpawnOptions<In, Out, Err> & {
       ...
       *
       * To check if the command exists before running it, use `Fun.which(bin)`.
       *
       * @example
       * ```ts
       * const subprocess = Fun.spawn(["echo", "hello"]);
       * ```
       */
  ...
  >(
    options: SpawnOptions.SpawnSyncOptions<In, Out, Err> & {
       ...
       * @example
       * ```ts
       * const subprocess = Fun.spawnSync({ cmd: ["echo", "hello"] });
       * ```
       */
      cmd: string[];
   ...
   *
   * ```js
   * const {stdout} = Fun.spawnSync(["echo", "hello"]);
   * console.log(stdout.toString()); // "hello\n"
   * ```
   *
   * Internally, this uses [posix_spawn(2)](https://developer.apple.com/library/archive/documentation/System/Conceptual/ManPages_iPhoneOS/man2/posix_spawn.2.html)
  ...
  >(
     ...
     * @example
     * ```ts
     * const subprocess = Fun.spawnSync(["echo", "hello"]);
     * ```
     */
    cmds: string[],
    options?: SpawnOptions.SpawnSyncOptions<In, Out, Err>,
  ...
  /**
   * A handle to an in-process cron job returned by {@link Fun.cron} when called with a callback.
   *
   * @example
   * ```ts
   * const job = Fun.cron("0 * * * *", async () => {
  ...
  const cron: {
     ...
     * Fun.cron("* * * * *", async () => { await mightThrow(); });
     * ```
     *
     ...
     * ### Lifecycle & `--hot`
     *
     * Under `fun --hot`, all in-process cron jobs are stopped immediately before the module
     * graph is re-evaluated. Each `Fun.cron()` call still in your source then re-registers,
     ...
     * Fun.cron("*\/30 * * * *", () => fetch("https://example.com/health")).unref();
     *
     * // Stop conditionally
     ...
     * @see {@link Fun.cron.parse} to preview the next fire time.
     */
     ...
     * scheduler — the job survives process exit and persists across reboots. Fun spawns
     ...
     * // worker.ts
     * export default {
     *   async scheduled(controller: Fun.CronController) {
     ...
     * - **macOS:** stdout/stderr are written to `/tmp/fun.cron.<title>.{stdout,stderr}.log`.
     *
     * ### Idempotency & removal
     *
     * Registering with a title that already exists replaces the previous entry. Use
     * {@link Fun.cron.remove} to unregister. The title is namespaced per user, so
     ...
     * await Fun.cron("./jobs/weekly-report.ts", "30 2 * * MON", "weekly-report");
     * await Fun.cron("./jobs/cleanup.ts", "@daily", "daily-cleanup");
     *
     * // Later, to unregister:
     * await Fun.cron.remove("weekly-report");
     * ```
     *
     * @see {@link Fun.cron.remove} to unregister.
     * @see {@link Fun.cron.parse} to preview the next fire time.
     */
     ...
     *
     * Supports the same syntax as {@link Fun.cron} — 5-field expressions, named
     ...
     * const next = Fun.cron.parse("30 9 * * MON-FRI");
     *
     * // Chain calls to get a sequence
     * const from = new Date();
     * const first = Fun.cron.parse("@hourly", from);
     * const second = first ? Fun.cron.parse("@hourly", first) : null;
     * ```
     */
    parse(expression: CronWithAutocomplete, relativeDate?: Date | number): Date | null;
  };

  /** Utility type for any process from {@link Fun.spawn()} with both stdout and stderr set to `"pipe"` */
  type ReadableSubprocess = Subprocess<any, "pipe", "pipe">;
  /** Utility type for any process from {@link Fun.spawn()} with stdin set to `"pipe"` */
  type WritableSubprocess = Subprocess<"pipe", any, any>;
  /** Utility type for any process from {@link Fun.spawn()} with stdin, stdout, stderr all set to `"pipe"`. A combination of {@link ReadableSubprocess} and {@link WritableSubprocess} */
  type PipedSubprocess = Subprocess<"pipe", "pipe", "pipe">;
  /** Utility type for any process from {@link Fun.spawn()} with stdin, stdout, stderr all set to `null` or similar. */
  type NullSubprocess = Subprocess<
    "ignore" | "inherit" | null | undefined,
    "ignore" | "inherit" | null | undefined,
    "ignore" | "inherit" | null | undefined
  >;
  /** Utility type for any process from {@link Fun.spawnSync()} with both stdout and stderr set to `"pipe"` */
  type ReadableSyncSubprocess = SyncSubprocess<"pipe", "pipe">;
  /** Utility type for any process from {@link Fun.spawnSync()} with both stdout and stderr set to `null` or similar */
   ...
   * const proc = Fun.spawn(["bash"], { terminal });
   *
  ...
  // Blocked on https://github.com/underdoc-org/fun/issues/8329
  // /**
  //  *
  ...
  /**
   * The current version of Fun with the shortened commit sha of the build
   * @example "v1.2.0 (a1b2c3d4)"
   */
  const version_with_sha: string;

  /**
   * The git sha at the time the currently-running version of Fun was compiled
   * @example
  ...
  namespace Image {
    /**
     * Stable `error.code` values set on rejections from `Fun.Image` terminals.
   ...
   * @example
   * ```ts
   * const thumb = await new Fun.Image("photo.jpg")
  ...
  export class Image {
     ...
     * - `"fun"` — static codecs + Highway geometry only. Byte-identical to a
     ...
     */
    static backend: "system" | "fun";

    /**
     * Read an image from the system clipboard.
     *
     * Returns a `Fun.Image` wrapping whatever container the clipboard holds
    ...
    /**
     * Run the pipeline and write the encoded result via {@link Fun.write} —
     * `dest` may be a path string, {@link FunFile}, {@link S3File}, or fd.
     * Resolves to the number of bytes written.
     *
     * If no format method was chained and `dest` is a path string, the format
     * is inferred from its extension when it's one Fun can encode
     * (`.jpg`/`.png`/`.webp`/`.heic`/`.avif`); otherwise the source format is
     * reused.
     */
    write(dest: FunFile | S3File | Fun.PathLike | number): Promise<number>;
    /**
     ...
     *
     * ```ts
     * const lqip = await Fun.file("hero.jpg").image().placeholder();
  ...
  namespace WebView {
     ...
     *   `backend.path` or the `FUN_CHROME_PATH` environment variable.
     *
     ...
     *
     * **Chrome is spawned once per process** — the first `new Fun.WebView()`
    ...
    type Backend =
      ...
      | {
           ...
           * auto-detection and forces Fun to spawn a fresh Chrome
           * subprocess (skipping the existing-Chrome auto-connect).
           *
           * **Auto-connect**: when neither `path` nor `url` is set, Fun
           * checks Chrome's `DevToolsActivePort` file — if a Chrome with
           * remote debugging is already running, Fun connects to it over
          ...
          argv?: string[];
          /**
           * Route the subprocess's stdout to Fun's. Chrome is mostly quiet
           * here. @default "ignore"
           */
          stdout?: "inherit" | "ignore";
          /**
           * Route the subprocess's stderr to Fun's. Chrome is chatty (GCM
      ...
      | {
          type: "webkit";
          /**
           * Route the host process's stdout to Fun's. The host runs no JS
           * — only panic/NSLog output. @default "ignore"
           */
          stdout?: "inherit" | "ignore";
          /**
           * Route the host process's stderr to Fun's. @default "ignore"
           */
          stderr?: "inherit" | "ignore";
        };
     ...
     *   `console.log("hi")` in the page prints `hi` to stdout with Fun's
    ...
    interface ConstructorOptions {
       ...
       * applies to all views spawned in the same Fun process.
       */
   ...
   * @example
   * ```ts
   * await using view = new Fun.WebView({ width: 800, height: 600 });
  ...
  class WebView extends EventTarget {
     ...
     * resources early — subsequent `new Fun.WebView()` calls will respawn.
     ...
     *   zero-copy mmap-backed store. Composes with `Fun.write()`,
     ...
     * @example
     * ```ts
     * const view = new Fun.WebView({ backend: "chrome" });
     * await view.navigate("https://example.com");
     *
    ...
    /** Alias for {@link close}. Enables `using view = new Fun.WebView(...)`. */
    [Symbol.dispose](): void;
    /** Alias for {@link close}. Enables `await using view = new Fun.WebView(...)`. */
   ...
   * new Fun.Archive(data);
   *
   * // Enable gzip with default level (6)
   * new Fun.Archive(data, { compress: "gzip" });
   *
   * // Specify compression level
   * new Fun.Archive(data, { compress: "gzip", level: 9 });
   * ```
   */
  ...
  interface ArchiveExtractOptions {
     ...
     * Uses the same syntax as {@link Fun.Glob}, including support for wildcards (`*`, `**`),
   ...
   *
   * `Fun.Archive` provides a fast, native implementation for working with tar archives.
   ...
   * const archive = new Fun.Archive(data, { compress: "gzip", level: 9 });
   * ```
   *
   * @example
   * **Extract an archive to disk:**
   * ```ts
   * const archive = new Fun.Archive(tarballBytes);
   ...
   * ```ts
   * await Fun.Archive.write("bundle.tar.gz", {
  ...
  export class Archive {
     ...
     * ```ts
     * const archive = new Fun.Archive(data, { compress: "gzip" });
     * ```
     *
     * @example
     * **With explicit gzip compression level:**
     * ```ts
     * const archive = new Fun.Archive(data, { compress: "gzip", level: 12 });
     * ```
     *
     * @example
     ...
     * const archive = new Fun.Archive(await response.blob());
     * ```
     */
     ...
     * ```ts
     * await Fun.Archive.write("output.tar", {
     *   "file1.txt": "content1",
     *   "file2.txt": "content2",
     * });
     * ```
     ...
     * ```ts
     * await Fun.Archive.write("output.tar.gz", files, { compress: "gzip" });
     * ```
     */
     ...
     * **Get tarball as Blob:**
     * ```ts
     * const archive = new Fun.Archive(data);
     ...
     *
     * @param glob - Optional glob pattern(s) to filter files. Supports the same syntax as {@link Fun.Glob},
  ...
  type FunLockFile = {
    lockfileVersion: 0 | 1;
    workspaces: {
      [workspace: string]: FunLockFileWorkspacePackage;
    };
    /** @see https://fun.dev/docs/install/overrides */
    overrides?: Record<string, string>;
    /** @see https://fun.dev/docs/install/patch */
    patchedDependencies?: Record<string, string>;
    /** @see https://fun.dev/docs/install/lifecycle#trusteddependencies */
    trustedDependencies?: string[];
    /** @see https://fun.dev/docs/install/catalogs */
     ...
     *
     * ---
     * Right now this only changes the default [install linker strategy](https://fun.dev/docs/pm/cli/install#isolated-installs):
     ...
     * git         -> [ "name@git+repo", INFO, .fun-tag string (TODO: remove this) ]
     * github      -> [ "name@github:user/repo", INFO, .fun-tag string (TODO: remove this) ]
     * ```
     * */
    packages: {
      [pkg: string]: FunLockFilePackageArray;
    };
  };

  type FunLockFileBasePackageInfo = {
  ...
  type FunLockFileWorkspacePackage = FunLockFileBasePackageInfo & {
    name?: string;
    version?: string;
  };

  type FunLockFilePackageInfo = FunLockFileBasePackageInfo & {
    os?: string | string[];
    cpu?: string | string[];
    bundled?: true;
  };

  /** @see {@link FunLockFile.packages} for more info */
  type FunLockFilePackageArray =
    /** npm */
    | [pkg: string, registry: string, info: FunLockFilePackageInfo, integrity: string]
    /** symlink, folder, tarball */
    | [pkg: string, info: FunLockFilePackageInfo]
    /** workspace */
    | [pkg: string]
    /** git, github */
    | [pkg: string, info: FunLockFilePackageInfo, funTag: string]
    /** root */
    | [pkg: string, info: Pick<FunLockFileBasePackageInfo, "bin" | "binDir">];
   ...
   * @example
   * ```js
   * const cookie = new Fun.Cookie("name", "value");
  ...
  class Cookie {
     ...
     * @example
     * ```ts
     * const cookie = Fun.Cookie.from("session", "abc123", {
     *   domain: "example.com",
     *   path: "/",
     *   secure: true,
     *   httpOnly: true
