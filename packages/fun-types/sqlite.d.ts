/**
 * Fast SQLite3 driver for Fun.js
 * @since v0.0.83
 *
 * @example
 * ```ts
...
declare module "fun:sqlite" {
  ...
  export class Database implements Disposable {
    ...
    close(
       ...
       *
       * Fun will automatically call close by default when the database instance is garbage collected.
       * In The future, Fun may default `throwOnError` to be true but for backwards compatibility, it is false by default.
       */
     ...
     *
     * Fun chooses the Apple build of SQLite on macOS because it brings a ~50% performance improvement.
     *
  ...
  export class Statement<ReturnType = unknown, ParamsType extends SQLQueryBindings[] = any[]> implements Disposable {
     ...
     *
     * In Fun v0.6.7 and earlier, this method returned `null` if there were no
     ...
     *
     * // For expressions:
     * const exprStmt = db.prepare("SELECT length('fun') AS str_length");
  ...
  export namespace constants {
     ...
     * ```ts
     * import { Database, constants } from "fun:sqlite";
     *
