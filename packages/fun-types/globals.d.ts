declare module "fun" {
  namespace __internal {
    ...
    type LibWorkerOrFunWorker = LibDomIsLoaded extends true ? {} : Fun.Worker;
    type LibEmptyOrFunWebSocket = LibDomIsLoaded extends true ? {} : Fun.WebSocket;
...
interface ReadableStream<R = any> extends Fun.__internal.LibEmptyOrNodeReadableStream<R> {}
declare var ReadableStream: Fun.__internal.UseLibDomIfAvailable<
  "ReadableStream",
  {
    prototype: ReadableStream;
    new <R = any>(underlyingSource?: Fun.UnderlyingSource<R>, strategy?: QueuingStrategy<R>): ReadableStream<R>;
    new <R = any>(underlyingSource?: Fun.DirectUnderlyingSource<R>, strategy?: QueuingStrategy<R>): ReadableStream<R>;
  }
>;

interface WritableStream<W = any> extends Fun.__internal.LibEmptyOrNodeWritableStream<W> {}
declare var WritableStream: Fun.__internal.UseLibDomIfAvailable<
  "WritableStream",
  {
    prototype: WritableStream;
    new <W = any>(underlyingSink?: Fun.UnderlyingSink<W>, strategy?: QueuingStrategy<W>): WritableStream<W>;
  }
>;

interface CompressionStream extends Fun.__internal.LibEmptyOrNodeStreamWebCompressionStream {}
declare var CompressionStream: Fun.__internal.UseLibDomIfAvailable<
  "CompressionStream",
  {
    prototype: CompressionStream;
    new (format: Fun.CompressionFormat): CompressionStream;
  }
>;

interface DecompressionStream extends Fun.__internal.LibEmptyOrNodeStreamWebDecompressionStream {}
declare var DecompressionStream: Fun.__internal.UseLibDomIfAvailable<
  "DecompressionStream",
  {
    prototype: DecompressionStream;
    new (format: Fun.CompressionFormat): DecompressionStream;
  }
>;

interface Worker extends Fun.__internal.LibWorkerOrFunWorker {}
declare var Worker: Fun.__internal.UseLibDomIfAvailable<
  "Worker",
  {
    prototype: Worker;
    new (scriptURL: string | URL, options?: Fun.WorkerOptions | undefined): Worker;
    /**
     * This is the cloned value of the `data` property passed to `new Worker()`
     *
     * This is Fun's equivalent of `workerData` in Node.js.
     */
    data: any;
  }
>;
 ...
 */
interface WebSocket extends Fun.__internal.LibEmptyOrFunWebSocket {}
/**
 * A WebSocket client implementation
 */
declare var WebSocket: Fun.__internal.UseLibDomIfAvailable<
  "WebSocket",
  {
    prototype: WebSocket;
     ...
     * ```
     */
    new (url: string | URL, options?: Fun.WebSocketOptions): WebSocket;
 ...
 * ```
 */
interface TextEncoder extends Fun.__internal.LibEmptyOrNodeUtilTextEncoder {
  /**
   ...
   */
  encodeInto(src?: string, dest?: Fun.BufferSource): import("node:util").TextEncoderEncodeIntoResult;
}
declare var TextEncoder: Fun.__internal.UseLibDomIfAvailable<
  "TextEncoder",
  {
    prototype: TextEncoder;
    new (encoding?: Fun.Encoding, options?: { fatal?: boolean; ignoreBOM?: boolean }): TextEncoder;
  }
>;
 ...
 */
interface TextDecoder extends Fun.__internal.LibEmptyOrNodeUtilTextDecoder {}
declare var TextDecoder: Fun.__internal.UseLibDomIfAvailable<
  "TextDecoder",
  {
    prototype: TextDecoder;
    new (encoding?: Fun.Encoding, options?: { fatal?: boolean; ignoreBOM?: boolean }): TextDecoder;
  }
>;
...
declare var Event: {
  ...
  new (type: string, eventInitDict?: Fun.EventInit): Event;
};
...
interface EventTarget {
  ...
  removeEventListener(
    type: string,
    listener: EventListener | EventListenerObject,
    options?: Fun.EventListenerOptions | boolean,
  ): void;
}
...
declare var File: Fun.__internal.UseLibDomIfAvailable<
  "File",
  {
    prototype: File;
    /**
 ...
 */
declare function setImmediate(handler: Fun.TimerHandler, ...arguments: any[]): Timer;
/**
 * Run a function every `interval` milliseconds
 * @param handler function to call
 * @param interval milliseconds to wait between calls
 */
declare function setInterval(handler: Fun.TimerHandler, interval?: number, ...arguments: any[]): Timer;
/**
 * Run a function after `timeout` (milliseconds)
 * @param handler function to call
 * @param timeout milliseconds to wait between calls
 */
declare function setTimeout(handler: Fun.TimerHandler, timeout?: number, ...arguments: any[]): Timer;
...
declare function addEventListener(
  type: string,
  listener: Fun.EventListenerOrEventListenerObject,
  options?: boolean | AddEventListenerOptions,
): void;
declare function removeEventListener<K extends keyof EventMap>(
  type: K,
  listener: (this: object, ev: EventMap[K]) => any,
  options?: boolean | Fun.EventListenerOptions,
): void;
...
declare var ErrorEvent: {
  ...
  new (type: string, eventInitDict?: Fun.ErrorEventInit): ErrorEvent;
};
...
declare var CloseEvent: {
  ...
  new (type: string, eventInitDict?: Fun.CloseEventInit): CloseEvent;
};

interface MessageEvent<T = any> extends Fun.MessageEvent<T> {}
declare var MessageEvent: Fun.__internal.UseLibDomIfAvailable<
  "MessageEvent",
  {
    prototype: MessageEvent;
    new <T>(type: string, eventInitDict?: Fun.MessageEventInit<T>): MessageEvent<any>;
  }
>;
...
declare var CustomEvent: {
  ...
  new <T>(type: string, eventInitDict?: Fun.CustomEventInit<T>): CustomEvent<T>;
};
...
interface AddEventListenerOptions extends Fun.EventListenerOptions {
  once?: boolean;
  passive?: boolean;
  signal?: AbortSignal;
}
...
/**
 * Low-level JavaScriptCore API for accessing the native ES Module loader (not a Fun API)
 *
 ...
 *
 * Fun may rewrite ESM import specifiers to point to bundled code. This will
 * be confusing when using this API, as it will return a string like
 * "/node_modules.server.fun".
 *
 * Fun may inject additional imports into your code. This usually has a `fun:` prefix.
 */
declare var Loader: {
  /**
   * ESM module registry
   *
   * This lets you implement live reload in Fun. If you
   ...
   *
   * The keys are an implementation detail for Fun that will change between
   * versions.
   *
   * - Userland modules are an absolute file path
   * - Virtual modules have a `fun:` prefix or `node:` prefix
   * - JS polyfills start with `"/fun-vfs/"`. `"buffer"` is an example of a JS polyfill
   * - If you have a `node_modules.fun` file, many modules will point to that file
   *
   * Virtual modules and JS polyfills are embedded in fun's binary. They don't
   ...
   * ["fun:wrap", "/path/to/node_modules/classnames/index.js", "/path/to/node_modules/react/index.js"]
   * ```
   *
   ...
   *
   * This may return a path to `node_modules.server.fun`, which will be confusing.
   *
   * Consider {@link Fun.resolve} or {@link ImportMeta.resolve}
   * instead.
   *
   * @param specifier - module specifier as it appears in transpiled source code
...
interface ReadableStreamDirectController {
  ...
  write(data: Fun.BufferSource | ArrayBuffer | string): number | Promise<number>;
...
interface ReadableStreamDefaultReader<R = any> extends ReadableStreamGenericReader {
  read(): Promise<Fun.ReadableStreamDefaultReadResult<R>>;
  /**
   * Only available in Fun. If there are multiple chunks in the queue, this will return all of them at the same time.
   * Will only return a promise if the data is not immediately available.
   */
  readMany(): Promise<Fun.ReadableStreamDefaultReadManyResult<R>> | Fun.ReadableStreamDefaultReadManyResult<R>;
  releaseLock(): void;
}
...
interface Transformer<I = any, O = any> {
  flush?: Fun.TransformerFlushCallback<O>;
  readableType?: undefined;
  start?: Fun.TransformerStartCallback<O>;
  transform?: Fun.TransformerTransformCallback<I, O>;
...
interface SubtleCrypto extends Fun.__internal.LibEmptyOrNodeCryptoWebcryptoSubtleCrypto {}
declare var SubtleCrypto: {
  prototype: SubtleCrypto;
  new (): SubtleCrypto;
};

interface CryptoKey extends Fun.__internal.NodeCryptoWebcryptoCryptoKey {}
declare var CryptoKey: {
  prototype: CryptoKey;
  new (): CryptoKey;
};

interface CryptoKeyPair extends Fun.__internal.NodeCryptoWebcryptoCryptoKeyPair {}
...
interface ImportMeta {
   ...
   * ```
   */
  readonly env: Fun.Env & NodeJS.ProcessEnv & ImportMetaEnv;

  /**
   * @deprecated Use `require.resolve` or `Fun.resolveSync(moduleId, path.dirname(parent))` instead
   *
  ...
  /**
   * Load a CommonJS module within an ES Module. Fun's transpiler rewrites all
   ...
   * ```ts
   * console.log(
   *   import.meta.main === (import.meta.path === Fun.main)
   * )
   * ```
   */
  main: boolean;
 ...
 */
declare function structuredClone<T>(value: T, options?: Fun.StructuredSerializeOptions): T;
 ...
 */
declare function postMessage(message: any, transfer?: Fun.Transferable[]): void;
...
interface BlobPropertyBag {
  ...
  type?: string;
  /** Not implemented in Fun yet. */
  // endings?: "transparent" | "native";
}

interface WorkerOptions extends Fun.WorkerOptions {}
...
interface Blob {
  ...
  /**
   * Wrap this blob in a {@link Fun.Image} pipeline. Equivalent to
   * `new Fun.Image(this, options)` — the constructor is synchronous (the
   * underlying read happens lazily when an Image terminal is awaited), so
   * this works on `Fun.file()`, `Fun.s3()`, fd-backed and in-memory blobs
   * alike:
   *
   * ```ts
   * await Fun.file("photo.jpg").image().resize(400).webp().write("thumb.webp");
   * ```
   */
  image(options?: Fun.Image.ConstructorOptions): Fun.Image;
...
declare var Blob: Fun.__internal.UseLibDomIfAvailable<
  "Blob",
  {
    prototype: Blob;
    new (blobParts?: Fun.BlobPart[], options?: BlobPropertyBag): Blob;
  }
>;
...
interface BroadcastChannel extends Fun.__internal.LibEmptyOrBroadcastChannel {}
declare var BroadcastChannel: Fun.__internal.UseLibDomIfAvailable<
  "BroadcastChannel",
  typeof import("node:worker_threads").BroadcastChannel
>;

declare var URL: Fun.__internal.UseLibDomIfAvailable<
  "URL",
  {
    prototype: URL;
...
declare var AbortController: Fun.__internal.UseLibDomIfAvailable<
  "AbortController",
  {
...
declare var AbortSignal: Fun.__internal.UseLibDomIfAvailable<
  "AbortSignal",
  {
...
interface FormData {
  ...
  get(name: string): Fun.FormDataEntryValue | null;
  /** [MDN Reference](https://developer.mozilla.org/docs/Web/API/FormData/getAll) */
  getAll(name: string): Fun.FormDataEntryValue[];
  ...
  forEach(callbackfn: (value: Fun.FormDataEntryValue, key: string, parent: FormData) => void, thisArg?: any): void;
  keys(): IterableIterator<string>;
  values(): IterableIterator<string>;
  entries(): IterableIterator<[string, string]>;
}
declare var FormData: Fun.__internal.UseLibDomIfAvailable<"FormData", { prototype: FormData; new (): FormData }>;

interface EventSource extends Fun.__internal.LibEmptyOrEventSource {}
declare var EventSource: Fun.__internal.UseLibDomIfAvailable<
  "EventSource",
  { prototype: EventSource; new (): EventSource }
>;

interface Performance extends Fun.__internal.LibPerformanceOrNodePerfHooksPerformance {}
declare var performance: Fun.__internal.UseLibDomIfAvailable<"performance", Performance>;
declare var Performance: Fun.__internal.UseLibDomIfAvailable<
  "Performance",
  { new (): Performance; prototype: Performance }
>;

interface PerformanceEntry extends Fun.__internal.LibEmptyOrPerformanceEntry {}
declare var PerformanceEntry: Fun.__internal.UseLibDomIfAvailable<
  "PerformanceEntry",
  { prototype: PerformanceEntry; new (): PerformanceEntry }
>;

interface PerformanceMark extends Fun.__internal.LibEmptyOrPerformanceMark {}
declare var PerformanceMark: Fun.__internal.UseLibDomIfAvailable<
  "PerformanceMark",
  { prototype: PerformanceMark; new (): PerformanceMark }
>;

interface PerformanceMeasure extends Fun.__internal.LibEmptyOrPerformanceMeasure {}
declare var PerformanceMeasure: Fun.__internal.UseLibDomIfAvailable<
  "PerformanceMeasure",
  { prototype: PerformanceMeasure; new (): PerformanceMeasure }
>;

interface PerformanceObserver extends Fun.__internal.LibEmptyOrPerformanceObserver {}
declare var PerformanceObserver: Fun.__internal.UseLibDomIfAvailable<
  "PerformanceObserver",
  { prototype: PerformanceObserver; new (): PerformanceObserver }
>;

interface PerformanceObserverEntryList extends Fun.__internal.LibEmptyOrPerformanceObserverEntryList {}
declare var PerformanceObserverEntryList: Fun.__internal.UseLibDomIfAvailable<
  "PerformanceObserverEntryList",
  { prototype: PerformanceObserverEntryList; new (): PerformanceObserverEntryList }
>;

interface PerformanceResourceTiming extends Fun.__internal.LibEmptyOrPerformanceResourceTiming {}
declare var PerformanceResourceTiming: Fun.__internal.UseLibDomIfAvailable<
  "PerformanceResourceTiming",
  { prototype: PerformanceResourceTiming; new (): PerformanceResourceTiming }
>;

interface ReadableByteStreamController extends Fun.__internal.LibEmptyOrReadableByteStreamController {}
declare var ReadableByteStreamController: Fun.__internal.UseLibDomIfAvailable<
  "ReadableByteStreamController",
  { prototype: ReadableByteStreamController; new (): ReadableByteStreamController }
>;

interface ReadableStreamBYOBReader extends Fun.__internal.LibEmptyOrReadableStreamBYOBReader {}
declare var ReadableStreamBYOBReader: Fun.__internal.UseLibDomIfAvailable<
  "ReadableStreamBYOBReader",
  { prototype: ReadableStreamBYOBReader; new (): ReadableStreamBYOBReader }
>;

interface ReadableStreamBYOBRequest extends Fun.__internal.LibEmptyOrReadableStreamBYOBRequest {}
declare var ReadableStreamBYOBRequest: Fun.__internal.UseLibDomIfAvailable<
  "ReadableStreamBYOBRequest",
  { prototype: ReadableStreamBYOBRequest; new (): ReadableStreamBYOBRequest }
>;

interface TextDecoderStream extends Fun.__internal.LibEmptyOrNodeStreamWebTextDecoderStream {}
declare var TextDecoderStream: Fun.__internal.UseLibDomIfAvailable<
  "TextDecoderStream",
  { prototype: TextDecoderStream; new (): TextDecoderStream }
>;

interface TextEncoderStream extends Fun.__internal.LibEmptyOrNodeStreamWebTextEncoderStream {}
declare var TextEncoderStream: Fun.__internal.UseLibDomIfAvailable<
  "TextEncoderStream",
  { prototype: TextEncoderStream; new (): TextEncoderStream }
>;

interface URLSearchParams {}
declare var URLSearchParams: Fun.__internal.UseLibDomIfAvailable<
  "URLSearchParams",
  {
...
declare var MessageChannel: Fun.__internal.UseLibDomIfAvailable<
  "MessageChannel",
  { prototype: MessageChannel; new (): MessageChannel }
>;

interface MessagePort extends Fun.__internal.LibEmptyOrNodeMessagePort {}
declare var MessagePort: Fun.__internal.UseLibDomIfAvailable<
  "MessagePort",
  {
...
//#region Fetch
interface RequestInit extends Fun.__internal.LibOrFallbackRequestInit {}
interface ResponseInit extends Fun.__internal.LibOrFallbackResponseInit {}

interface Headers extends Fun.__internal.FunHeadersOverride {}
declare var Headers: Fun.__internal.UseLibDomIfAvailable<
  "Headers",
  {
    prototype: Headers;
    new (init?: Fun.HeadersInit): Headers;
  }
>;

interface Request extends Fun.__internal.FunRequestOverride {}
declare var Request: Fun.__internal.UseLibDomIfAvailable<
  "Request",
  {
    prototype: Request;
...
interface Response extends Fun.__internal.FunResponseOverride {}
declare var Response: Fun.__internal.UseLibDomIfAvailable<
  "Response",
  {
    new (body?: Fun.BodyInit | null | undefined, init?: ResponseInit | undefined): Response;
    /**
...
/**
 * Extends Fun.TLSOptions with extra properties that are only supported in `fetch(url, {tls: ...})`
 */
interface FunFetchRequestInitTLS extends Fun.TLSOptions {
  /**
...
/**
 * FunFetchRequestInit represents additional options that Fun supports in `fetch()` only.
 *
 * Fun extends the `fetch` API with some additional options, except
 ...
 */
interface FunFetchRequestInit extends RequestInit {
  /**
   * Override the default TLS options
   */
  tls?: FunFetchRequestInitTLS;

  /**
   * Log the raw HTTP request & response to stdout. This API may be
   * removed in a future version of Fun without notice.
   ...
   * `FUN_FEATURE_FLAG_EXPERIMENTAL_HTTP2_CLIENT` if set. Omit to use the
 ...
 */
declare function fetch(input: string | URL | Request, init?: FunFetchRequestInit): Promise<Response>;

/**
 * Fun's extensions of the `fetch` API
 *
