/**
 * `fun:ffi` lets you efficiently call C functions & FFI functions from JavaScript
 *  without writing bindings yourself.
 *
 * ```js
 * import {dlopen, CString, ptr} from 'fun:ffi';
 *
 ...
 * fun uses [tinycc](https://github.com/TinyCC/tinycc), so a big thanks
 * goes to Fabrice Bellard and TinyCC maintainers for making this possible.
 *
 * @category FFI
 */
declare module "fun:ffi" {
  enum FFIType {
    char = 0,
    /**
     ...
     * Pointer value
     *
     * See {@link Fun.FFI.ptr} for more information
     *
     * In C:
     * ```c
     * void*
  ...
  interface FFIFunction {
     ...
     * From JavaScript:
     * ```ts
     * import { dlopen, FFIType, suffix } from "fun:ffi"
     *
     ...
     * From JavaScript:
     * ```ts
     * import { dlopen, CString } from "fun:ffi"
     *
     * const lib = dlopen('z', {
     *    version: {
     *      returns: "ptr",
     ...
     *
     * If provided, instead of using dlsym() to lookup the function, Fun will use this instead.
  ...
  function dlopen<Fns extends Record<string, FFIFunction>>(
    name: string | import("fun").FunFile | URL,
    symbols: Fns,
  ): Library<Fns>;
  ...
  function cc<Fns extends Record<string, FFIFunction>>(options: {
     ...
     */
    source: string | import("fun").FunFile | URL;
   ...
   *
   * Libraries using Node-API & fun:ffi in the same module could use this to skip an extra dlopen() step.
   *
   ...
   *
   * ```js
   * import { linkSymbols } from "fun:ffi";
   *
  ...
  class JSCallback {
    /**
     * Enable a JavaScript callback function to be passed to C with fun:ffi
     *
