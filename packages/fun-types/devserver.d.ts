declare module "fun" {
   ...
   */
  type HMREvent = `fun:${HMREventNames}` | (string & {});
}
...
interface ImportMeta {
   ...
   * ```
   *
   * However, this check is usually not needed as Fun will dead-code-eliminate
   * calls to all of the HMR APIs in production builds.
   *
   * https://fun.dev/docs/bundler/hmr
   */
  hot: {
    /**
     ...
     * When `import.meta.hot.data` is written to, Fun will mark this module as
     ...
     * ```
     *
     * In production, `data` is inlined to be `{}`. This is handy because Fun
     ...
     *
     * For compatibility with Vite, event names are also available via vite:* prefix instead of fun:*.
     *
     * https://fun.dev/docs/bundler/hmr#import-meta-hot-on-and-off
     * @param event The event to listen to
     * @param callback The callback to call when the event is emitted
     */
    on(event: Fun.HMREvent, callback: () => void): void;
     ...
     */
    off(event: Fun.HMREvent, callback: () => void): void;
  };
}
