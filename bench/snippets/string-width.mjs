...
const funStringWidth = globalThis?.Fun?.stringWidth;
...
const URL = "https://github.com/underdoc-org/fun/blob/main/src/jsc/bindings/ANSIHelpers.h";

// Each input: [content, label, opts?]. opts.skipMismatchCheck=true skips ONLY
// the cross-impl correctness check (still benchmarks both fun and npm) — for
...
const inputs = [
  ...
  //   BEL (0x07): single-byte fast path, both fun and npm handle it.
  ...
  [`${ESC}]8;;${URL}\x07Fun${ESC}]8;;\x07`, "hyperlink-bel"],
  [`${ESC}]8;;${URL}${ST}Fun${ESC}]8;;${ST}`, "hyperlink-st", { skipMismatchCheck: true }],
  [`${ESC}]8;;${URL}${C1_ST}Fun${ESC}]8;;${C1_ST}`, "hyperlink-c1st", { skipMismatchCheck: true }],
...
for (const [input, textLabel, opts = {}] of inputs) {
  for (let repeatCount of repeatCounts) {
    ...
    if (funStringWidth) {
      ...
      if (!opts.skipMismatchCheck && funStringWidth(str) !== npmStringWidth(str)) {
        throw new Error(
          `string-width mismatch (${textLabel}, repeat=${repeatCount}): fun=${funStringWidth(str)} npm=${npmStringWidth(str)}`,
        );
      }
    }
