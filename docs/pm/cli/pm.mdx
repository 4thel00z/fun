...
The `fun pm` command group provides a set of utilities for working with Fun's package manager.
...
Print your npm username. Requires you to be logged in (`funx npm login`) with credentials in either `funfig.toml` or `.npmrc`:
...
To print the path to Fun's global module cache:

```bash terminal icon="terminal"
fun pm cache
```

To clear Fun's global module cache:
...
```bash terminal icon="terminal"
fun pm default-trusted
```

see the current list on GitHub [here](https://github.com/underdoc-org/fun/blob/main/src/install/default-trusted-dependencies.txt)
...
```txt
fun pm version v1.3.3 (ca7428e9)
...
Examples:
  fun pm version patch
  fun pm version 1.2.3 --no-git-tag-version
  fun pm version prerelease --preid beta --message "Release beta: %s"
```
...
```bash terminal icon="terminal"
# get
fun pm pkg get name                               # single property
fun pm pkg get name version                       # multiple properties
fun pm pkg get                                    # entire package.json
fun pm pkg get scripts.build                      # nested property

# set
fun pm pkg set name="my-package"                  # simple property
fun pm pkg set scripts.test="jest" version=2.0.0  # multiple properties
fun pm pkg set {"private":"true"} --json          # JSON values with --json flag

# delete
fun pm pkg delete description                     # single property
fun pm pkg delete scripts.test contributors[0]    # multiple/nested

# fix
fun pm pkg fix                                    # auto-fix common issues
```
