...
<Note>To upgrade your Fun CLI version, see [`fun upgrade`](/installation#upgrading).</Note>
...
```sh terminal icon="terminal"
fun update --interactive --recursive
fun update -i -r
```
...
By default, `fun update` will update to the latest version of a dependency that satisfies the version range specified in your `package.json`.
...
- `fun update` would update to a version that matches `17.x`.
- `fun update --latest` would update to a version that matches `18.x` or later.
