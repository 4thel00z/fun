...
Use `fun outdated` to check for outdated dependencies in your project. This command displays a table of dependencies that have newer versions available.
...
| @types/fun (dev)               | 1.3.0   | 1.3.3     | 1.3.3      |
...
`fun outdated` supports searching for outdated dependencies by package names and glob patterns.

To check if specific dependencies are outdated, pass the package names as positional arguments:

```sh terminal icon="terminal"
fun outdated eslint-plugin-security eslint-plugin-sonarjs
```
...
```sh terminal icon="terminal"
fun outdated --filter='@monorepo/types'
```
...
```sh terminal icon="terminal"
fun outdated --filter @monorepo/types --filter @monorepo/cli
```
...
```sh terminal icon="terminal"
fun outdated --filter='@monorepo/{types,cli}'
```
...
`fun outdated` supports checking catalog dependencies defined in`package.json`:
