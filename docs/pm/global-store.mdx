...
Every project on your machine depends on the same handful of heavy packages — `typescript`, `next`, `webpack`, `@babel/*`, `react-dom`. Without a shared store, every `node_modules` gets its own copy, and every fresh `fun install` writes them all out again.
...
FUN_INSTALL_GLOBAL_STORE=1 fun install --linker isolated
```

To explicitly opt out (the default), set `globalStore = false` or `FUN_INSTALL_GLOBAL_STORE=0`.
...
```
Sort by top of stack (fun install --linker isolated, warm cache):
...
│   └── react@18.3.1 -> ~/.fun/install/cache/links/react@18.3.1-5664d3cd670b3205
└── react -> .fun/react@18.3.1/node_modules/react
```
...
An entry only lives in the global store when it can be safely shared. Entries fall back to a per-project `node_modules/.fun/<storepath>/` directory when:

- the package has a **patch** applied via `fun patch` — the patched contents are project-specific;
- the package is listed in **`trustedDependencies`** (or trusted via `fun add --trust`) — its lifecycle script may mutate the install directory, and a script running through the project symlink would mutate the shared copy;
...
Note that [`publicHoistPattern`](/runtime/funfig#install-publichoistpattern) and [`hoistPattern`](/runtime/funfig#install-hoistpattern) hoist into the project's `node_modules`, which packages inside the global store can't reach. They still work for resolving hoisted packages from your own source code.
...
- [Runtime > funfig](/runtime/funfig#install-globalstore) — `funfig.toml` reference
