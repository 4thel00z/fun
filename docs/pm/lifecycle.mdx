...
description: "How Fun handles package lifecycle scripts securely"
---
...
These scripts are arbitrary shell commands that the package manager is expected to read and execute at the appropriate time. But executing arbitrary scripts represents a potential security risk, so—unlike other `npm` clients—Fun does not execute arbitrary lifecycle scripts by default.
...
Instead of executing arbitrary scripts, Fun uses a "default-secure" approach. You can add certain packages to an allow list, and Fun will execute lifecycle scripts for those packages. To tell Fun to allow lifecycle scripts for a particular package, add the package name to `trustedDependencies` array in your `package.json`.
...
Once added to `trustedDependencies`, install/re-install the package. Fun will read this field and run lifecycle scripts for `my-trusted-package`.

The top 500 npm packages with lifecycle scripts are allowed by default. You can see the full list [here](https://github.com/underdoc-org/fun/blob/main/src/install/default-trusted-dependencies.txt).
...
fun install --ignore-scripts
```

To make this the default for a project, set [`install.ignoreScripts`](/runtime/funfig#install-ignorescripts) in `funfig.toml`:
