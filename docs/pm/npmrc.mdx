...
Fun supports loading configuration options from [`.npmrc`](https://docs.npmjs.com/cli/v10/configuring-npm/npmrc) files, allowing you to reuse existing registry/scope configurations.

<Note>
  We recommend migrating your `.npmrc` file to Fun's [`funfig.toml`](/runtime/funfig) format, as it provides more
  flexible options and can let you configure Fun-specific options.
</Note>
...
The equivalent `funfig.toml` option is [`install.registry`](/runtime/funfig#install-registry):
...
The equivalent `funfig.toml` option is to add a key in [`install.scopes`](/runtime/funfig#install-registry):
...
The equivalent `funfig.toml` option is [`install.linkWorkspacePackages`](/runtime/funfig#install-linkworkspacepackages):
...
The equivalent `funfig.toml` option is [`install.exact`](/runtime/funfig#install-exact):
...
This is equivalent to using the `--ignore-scripts` flag with `fun install`.
...
The equivalent `funfig.toml` option is [`install.dryRun`](/runtime/funfig#install-dryrun):
...
The equivalent `funfig.toml` option is [`install.cache`](/runtime/funfig#install-cache):
...
Control how packages are installed in `node_modules`. Fun supports two different configuration options for compatibility with different package managers.
...
The `node-linker` option controls the installation mode. Fun supports values from both pnpm and yarn:
