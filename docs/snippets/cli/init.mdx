...
<ParamField path="--react" type="string|boolean">
  ...
  Examples:{" "}
  <pre>
    <code>fun init --react fun init --react=tailwind fun init --react=shadcn</code>
  </pre>{" "}
</ParamField>
...
<ParamField path="--cwd" type="string">
  {" "}
  Run <code>fun init</code> as if started in a different working directory (useful in scripts).{" "}
</ParamField>
...
- React + Tailwind CSS
  ...
  ```bash terminal icon="terminal"
  fun init --react=tailwind
  ```

- React + @shadcn/ui
  ```bash terminal icon="terminal"
  fun init --react=shadcn
  ```
