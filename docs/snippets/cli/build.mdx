...
<ParamField path="--target" type="string" default="browser">
  Intended execution environment for the bundle. One of <code>browser</code>, <code>fun</code>, or <code>node</code>
</ParamField>
...
<ParamField path="--footer" type="string">
  Add a footer to the output (e.g. <code>// built with fun!</code>)
</ParamField>
...
<ParamField path="--compile" type="boolean">
  Generate a standalone Fun executable containing the bundle. Implies <code>--production</code>
</ParamField>
...
<ParamField path="--app" type="boolean">
  <b>(EXPERIMENTAL)</b> Build a web app for production using Fun Bake
</ParamField>
