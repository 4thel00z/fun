...
<ParamField path="--gzip-level" type="string" default="9">
  Specify the level of gzip compression to use when packing the package. Only applies to `fun publish` without a tarball
...
<ParamField path="--registry" type="string">
  Specify registry URL, overriding .npmrc and funfig.toml
</ParamField>

```bash
fun publish --registry https://my-private-registry.com
```
...
fun publish --ca "-----BEGIN CERTIFICATE-----..."
```

```bash Certificate File
fun publish --cafile ./ca-cert.pem
```
...
<Note>
  **Lifecycle Scripts** — When providing a pre-built tarball, lifecycle scripts (prepublishOnly, prepack, etc.) are not
  executed. Scripts only run when Fun packs the package itself.
</Note>
