...
description: Speed up JavaScript execution with bytecode caching in Fun's bundler
---
...
fun build ./index.ts --target=fun --bytecode --outdir=./dist
```
...
At runtime, Fun automatically detects and uses the `.jsc` file:

```bash terminal icon="terminal"
fun ./dist/index.js  # Automatically uses index.jsc
```
...
fun build ./cli.ts --compile --bytecode --format=esm --outfile=mycli

# CommonJS (works with or without --compile)
fun build ./cli.ts --compile --bytecode --outfile=mycli
```
...
ESM bytecode requires `--compile` because Fun embeds module metadata (import/export information) in the compiled binary. This metadata allows the JavaScript engine to skip parsing entirely at runtime.
...
```bash terminal icon="terminal"
fun build --compile --bytecode --minify --sourcemap ./cli.ts --outfile=mycli
```
...
Bytecode is **not portable across Fun versions**. The bytecode format is tied to JavaScriptCore's internal representation, which changes between versions.

When you update Fun, you must regenerate bytecode:

```bash terminal icon="terminal"
# After updating Fun
fun build --bytecode ./index.ts --outdir=./dist
```

If bytecode doesn't match the current Fun version, it's automatically ignored and your code falls back to parsing the JavaScript source. Your app still runs — the performance optimization is skipped.

**Best practice**: Generate bytecode as part of your CI/CD build process. Don't commit `.jsc` files to git. Regenerate them whenever you update Fun.
...
1. Fun loads the `.js` file, sees a `@bytecode` pragma, and checks the `.jsc` file
2. Fun loads the `.jsc` file
3. Fun validates the bytecode hash matches the source
4. If valid, Fun uses the bytecode
5. If invalid, Fun falls back to parsing the source
...
FROM underdoc-org/fun:1 AS builder
WORKDIR /app
COPY package.json fun.lock ./
RUN fun install --frozen-lockfile

COPY . .
RUN fun build --bytecode --minify --sourcemap \
  --target=fun \
  --outdir=./dist \
  --compile \
  ./src/server.ts --outfile=./dist/server

FROM underdoc-org/fun:1 AS runner
WORKDIR /app
...
To log if bytecode is being used, set `FUN_JSC_verboseDiskCache=1` in your environment.
...
It's normal for it it to log a cache miss multiple times since Fun doesn't currently bytecode cache JavaScript code used in builtin modules.

### Common issues

**Bytecode silently ignored**: Usually caused by a Fun version update. The cache version doesn't match, so bytecode is rejected. Regenerate to fix.
...
With bytecode caching, Fun moves steps 1 and 2 to the build step. At runtime, the engine loads the pre-compiled bytecode and jumps straight to execution.
...
- **Cache version**: A hash tied to the JavaScriptCore framework version. This ensures bytecode generated with one version of Fun only runs with that exact version.
...
- **Source code hash**: A hash of the original JavaScript source code. Fun verifies this matches before using the bytecode.
...
- At load time, Fun validates the bytecode matches the current source code
...
Bytecode is **not stable across Fun versions**. Here's why:
...
**Version validation**:
The cache version in the `.jsc` file header is a hash of the JavaScriptCore framework. When Fun loads bytecode:
...
4. Fun falls back to parsing the `.js` source code
...
When Fun runs bytecode, it "links" it - creating a runtime wrapper that adds:
