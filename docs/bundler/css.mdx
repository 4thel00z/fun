---
title: CSS
description: Fun's bundler has built-in support for CSS with modern features
---

Fun's bundler has built-in support for CSS with the following features:
...
Fun's CSS bundler lets you use modern/future CSS features without having to worry about browser compatibility — all thanks to its transpiling and vendor prefixing features which are enabled by default.

Fun's CSS parser and bundler is a direct Rust → Zig port of LightningCSS, with a bundling approach inspired by esbuild. The transpiler converts modern CSS syntax into backwards-compatible equivalents that work across browsers.

<Note>A huge thanks goes to the amazing work from the authors of LightningCSS and esbuild.</Note>

## Browser Compatibility

By default, Fun's CSS bundler targets the following browsers:
...
Fun's CSS bundler automatically converts this nested syntax into traditional flat CSS that works in all browsers:
...
Fun's CSS bundler evaluates these color mixes at build time when all color values are known (not CSS variables), generating static color values that work in all browsers:
...
Fun's CSS bundler computes these relative color modifications at build time (when not using CSS variables) and generates static color values for browser compatibility:
...
Fun's CSS bundler automatically converts these advanced color formats to backwards-compatible alternatives for browsers that don't yet support them:
...
For browsers that don't support these advanced color functions yet, Fun's CSS bundler provides appropriate RGB fallbacks:
...
Fun's CSS bundler automatically converts HWB colors to RGB for compatibility with all browsers:
...
Fun's CSS bundler automatically converts these modern color formats to ensure compatibility with older browsers:
...
For browsers that don't support this feature yet, Fun's CSS bundler converts it to use CSS variables with proper fallbacks:
...
For browsers that don't fully support logical properties, Fun's CSS bundler compiles them to physical properties with appropriate directional adjustments:
...
For browsers that don't support the `:dir()` selector yet, Fun's CSS bundler converts it to the more widely supported `:lang()` selector with appropriate language mappings:
...
For browsers that don't support multiple arguments in the `:lang()` selector, Fun's CSS bundler converts this syntax to use the `:is()` selector to maintain the same behavior:
...
If needed, Fun can provide additional fallbacks for `:is()` as well, ensuring your language-specific styles work across all browsers. This approach simplifies creating internationalized designs with distinct typographic and styling rules for different language groups.
...
For browsers that don't support `:is()`, Fun's CSS bundler provides fallbacks using vendor-prefixed alternatives:
...
<Warning>
  ...
  complex selectors. Fun handles these limitations intelligently, only using prefixed versions when they'll work
  correctly.
</Warning>
...
For browsers that don't support multiple arguments in `:not()`, Fun's CSS bundler converts this syntax to a more compatible form while preserving the same behavior:
...
And if `:is()` isn't supported, Fun can generate further fallbacks:
...
Fun's CSS bundler evaluates these mathematical expressions at build time when all values are known constants (not variables), resulting in optimized output:
...
Fun's CSS bundler converts these modern range queries to traditional media query syntax for compatibility with all browsers:
...
CSS has introduced several modern shorthand properties that improve code readability and maintainability. Fun's CSS bundler ensures these convenient shorthands work on all browsers by converting them to their longhand equivalents when needed.
...
For browsers that don't support these modern shorthands, Fun converts them to their component longhand properties:
...
For browsers that don't support this syntax, Fun's CSS bundler automatically converts it to the traditional format by duplicating color stops:
...
For browsers that don't support `system-ui`, Fun's CSS bundler automatically expands it to a comprehensive cross-platform font stack:
...
Fun's bundler also supports bundling CSS modules in addition to regular CSS with support for the following features:
...
Under the hood, Fun's bundler transforms locally scoped class names into unique identifiers.
