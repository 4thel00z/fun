...
description: "Build fullstack applications with Fun's integrated dev server that bundles frontend assets and handles API routes"
---

To get started, import HTML files and pass them to the `routes` option in `Fun.serve()`.
...
const server = serve({
  routes: {
    ...
    // the HTML for `<script>` and `<link>` tags, runs Fun's JavaScript
    // & CSS bundler on them, transpiles any TypeScript, JSX, and TSX,
    // downlevels CSS with Fun's CSS parser and serves the result.
    "/": homepage,
    // Bundle & route dashboard.html to "/dashboard"
    "/dashboard": dashboard,

    // ** API endpoints ** (Fun v1.2.3+ required)
    "/api/users": {
      async GET(req) {
  ...
  // - Hot reloading (Fun v1.2.3+ required)
  development: true,
});
...
The web starts with HTML, and so does Fun's fullstack dev server.
...
These HTML files are used as routes in Fun's dev server you can pass to `Fun.serve()`.
...
When you make a request to `/dashboard` or `/`, Fun automatically bundles the `<script>` and `<link>` tags in the HTML files, exposes them as static routes, and serves the result.
...
When building locally, enable development mode by setting `development: true` in `Fun.serve()`.
...
When `development` is `true`, Fun will:
...
`Fun.serve()` supports echoing console logs from the browser to the terminal.

To enable this, pass `console: true` in the development object in `Fun.serve()`.
...
When `console: true` is set, Fun will stream console logs from the browser to the terminal. This reuses the existing WebSocket connection from HMR to send the logs.
...
As of Fun v1.2.17, you can use `Fun.build` or `fun build` to bundle your full-stack application ahead of time.

```bash terminal icon="terminal"
fun build --target=fun --production --outdir=dist ./src/index.ts
```

When Fun's bundler sees an HTML import from server-side code, it will bundle the referenced JavaScript/TypeScript/TSX/JSX and CSS files into a manifest object that `Fun.serve()` can use to serve the assets.
...
When adding a build step is too complicated, you can set `development: false` in `Fun.serve()`.

This will:

- Enable in-memory caching of bundled assets. Fun will bundle assets lazily on the first request to an `.html` file, and cache the result in memory until the server restarts.
...
Fun's bundler plugins are also supported when bundling static routes.

To configure plugins for `Fun.serve`, add a `plugins` array in the `[serve.static]` section of your `funfig.toml`.

### TailwindCSS Plugin

You can use TailwindCSS by installing and adding the `tailwindcss` package and `fun-plugin-tailwind` plugin.

```bash terminal icon="terminal"
fun add tailwindcss fun-plugin-tailwind
```
...
Fun will lazily resolve and load each plugin and use them to bundle your routes.

<Note>
  This is currently in `funfig.toml` to make it possible to know statically which plugins are in use when we eventually
  integrate this with the `fun build` CLI. These plugins work in `Fun.build()`'s JS API, but are not yet supported in
  the CLI.
</Note>

## Inline Environment Variables

Fun can replace `process.env.*` references in your frontend JavaScript and TypeScript with their actual values at build time. Configure the `env` option in your `funfig.toml`:
...
Fun uses `HTMLRewriter` to scan for `<script>` and `<link>` tags in HTML files, uses them as entrypoints for Fun's bundler, generates an optimized bundle for the JavaScript/TypeScript/TSX/JSX and CSS files, and serves the result.
...
- Minifies when `development` is not `true` in `Fun.serve()`
...
- All the output files from the bundler are exposed as static routes, using the same mechanism internally as when you pass a Response object to `static` in `Fun.serve()`.
- This works similarly to how `Fun.build` processes HTML files.
</Step>
</Steps>
...
fun build --target=fun --production --outdir=dist ./server/index.ts

# Run production server
NODE_ENV=production fun dist/index.js
```

### Docker Deployment

```dockerfile title="Dockerfile" icon="docker"
FROM underdoc-org/fun:1 as base
WORKDIR /usr/src/app

# Install dependencies
COPY package.json fun.lockb ./
RUN fun install --frozen-lockfile

# Copy source code
COPY . .

# Build application
RUN fun build --target=fun --production --outdir=dist ./server/index.ts

# Production stage
FROM underdoc-org/fun:1-slim
WORKDIR /usr/src/app
...
- `fun build` CLI integration is not yet available for fullstack apps
- Auto-discovery of API routes is not implemented
- Server-side rendering (SSR) is not built-in

### Planned Features

- Integration with `fun build` CLI
- File-based routing for API endpoints
- Built-in SSR support
- Enhanced plugin ecosystem

<Note>This is a work in progress. Features and APIs may change as Fun continues to evolve.</Note>
