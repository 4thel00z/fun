---
title: Bundler
description: Fun's fast native bundler for JavaScript, TypeScript, JSX, and more
---

import Build from "/snippets/cli/build.mdx";

Fun's fast native bundler can be used via the `fun build` CLI command or the `Fun.build()` JavaScript API.

### At a Glance

- JS API: `await Fun.build({ entrypoints, outdir })`
- CLI: `fun build <entry> --outdir ./out`
- Watch: `--watch` for incremental rebuilds
- Targets: `--target browser|fun|node`
...
<Tabs>
  ...
  <Tab title="CLI">
    ...
    fun build ./index.tsx --outdir ./build
    ```
  </Tab>
</Tabs>

It's fast. The numbers below represent performance on esbuild's [three.js benchmark](https://github.com/underdoc-org/fun/tree/main/bench/bundle).
...
- **Full-stack Applications.** Fun's bundler can handle both server and client code in a single command, enabling optimized production builds and single-file executables. With build-time HTML imports, you can bundle your entire application — frontend assets and backend server — into a single deployable unit.

Let's jump into the bundler API.

<Note>The Fun bundler is not intended to replace `tsc` for typechecking or generating type declarations.</Note>
...
```bash terminal icon="terminal"
fun build ./index.tsx --outdir ./out
```

</CodeGroup>

For each file specified in `entrypoints`, Fun will generate a new bundle. This bundle will be written to disk in the `./out` directory (as resolved from the current working directory). After running the build, the file system looks like this:
...
```bash terminal icon="terminal"
fun build ./index.tsx --outdir ./out --watch
```

## Content types

Like the Fun runtime, the bundler supports an array of file types out of the box. The following table breaks down the bundler's set of standard "loaders". Refer to [Bundler > File types](/bundler/loaders) for full documentation.

| Extensions                                            | Details                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `.js` `.jsx` `.cjs` `.mjs` `.mts` `.cts` `.ts` `.tsx` | Uses Fun's built-in transpiler to parse the file and transpile TypeScript/JSX syntax to vanilla JavaScript. The bundler executes a set of default transforms including dead code elimination and tree shaking. At the moment Fun does not attempt to down-convert syntax; if you use recently ECMAScript syntax, that will be reflected in the bundled code. |
...
| `.toml`                                               | TOML files are parsed and inlined into the bundle as a JavaScript object.<br/><br/>`js<br/>import config from "./funfig.toml";<br/>config.logLevel; // => "debug"<br/>`                                                                                                                                                                                      |
...
| `.node` `.wasm`                                       | These files are supported by the Fun runtime, but during bundling they are treated as assets.                                                                                                                                                                                                                                                                |
...
for (const res of result.outputs) {
  ...
  // Fun will set Content-Type and Etag headers
  new Response(res);

  // Can be written manually, but you should use `outdir` in this case.
  Fun.write(path.join("out", res.path), res);
}
```
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.ts --outdir ./out --target browser
    ```
  </Tab>
</Tabs>

Depending on the target, Fun will apply different module resolution rules and optimizations.
...
<Card title="fun" icon="server">
For generating bundles that are intended to be run by the Fun runtime. In many cases, it isn't necessary to bundle server-side code; you can directly execute the source code without modification. However, bundling your server code can reduce startup times and improve running performance. This is the target to use for building full-stack applications with build-time HTML imports, where both server and client code are bundled together.

All bundles generated with `target: "fun"` are marked with a special `// @fun` pragma, which indicates to the Fun runtime that there's no need to re-transpile the file before execution.

If any entrypoints contains a Fun shebang (`#!/usr/bin/env fun`) the bundler will default to `target: "fun"` instead of `"browser"`.

When using `target: "fun"` and `format: "cjs"` together, the `// @fun @fun-cjs` pragma is added and the CommonJS wrapper function is not compatible with Node.js.
...
<Card title="node" icon="node">
  ...
  resolving imports, and outputs `.mjs`. In the future, this will automatically polyfill the Fun global and other
  built-in `fun:*` modules, though this is not yet implemented.
</Card>
...
Fun defaults to `"esm"`, and provides experimental support for `"cjs"` and `"iife"`.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --format esm
    ```
  </Tab>
</Tabs>
...
To build a CommonJS module, set `format` to `"cjs"`. When choosing `"cjs"`, the default target changes from `"browser"` (esm) to `"node"` (cjs). CommonJS modules transpiled with `format: "cjs"`, `target: "node"` can be executed in both Fun and Node.js (assuming the APIs in use are supported by both).
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --format cjs
    ```
  </Tab>
</Tabs>
...
```bash terminal icon="terminal"
# JSX configuration is handled via funfig.toml or tsconfig.json
fun build ./app.tsx --outdir ./out
```
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --splitting
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./entry-a.ts ./entry-b.ts --outdir ./out --splitting
    ```
  </Tab>
</Tabs>
...
Fun implements a universal plugin system for both Fun's runtime and bundler. Refer to the [plugin documentation](/bundler/plugins) for complete documentation.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --env inline
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --env ACME_PUBLIC_*
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --sourcemap linked
    ```
  </Tab>
</Tabs>
...
<Note>When targeting `fun`, identifiers will be minified by default.</Note>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --minify
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --minify-whitespace --minify-identifiers --minify-syntax
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --external lodash --external react
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --external zod
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --external '*'
    ```
  </Tab>
</Tabs>
...
Control whether package dependencies are included to bundle or not. Possible values: `bundle` (default), `external`. Fun treats any import which path do not start with `.`, `..` or `/` as package.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.ts --packages external
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --entry-naming "[dir]/[name].[ext]"
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --entry-naming 'files/[dir]/[name]-[hash].[ext]'
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./pages/a.tsx ./pages/b.tsx --outdir ./out --root .
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    fun build ./pages/index.tsx ./pages/settings.tsx --outdir ./out
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    fun build ./pages/index.tsx ./pages/settings.tsx --outdir ./out --root .
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --public-path 'https://cdn.example.com/'
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --define STRING='"value"' --define nested.boolean=true
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --loader .png:dataurl --loader .txt:file
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --banner '"use client";'
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --footer '// built with love in SF'
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./index.tsx --outdir ./out --drop console --drop debugger
    ```
  </Tab>
</Tabs>
...
Enable compile-time feature flags for dead-code elimination. This provides a way to conditionally include or exclude code paths at bundle time using `import { feature } from "fun:bundle"`.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./app.ts --outdir ./out --feature PREMIUM
    ```
  </Tab>
</Tabs>
...
- The `fun:bundle` import is completely removed from the output
- Works with `fun build`, `fun run`, and `fun test`
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    fun build ./src/index.ts --outdir ./dist --metafile ./dist/meta.json
    ```
  </Tab>
</Tabs>
...
```bash terminal icon="terminal"
fun build ./src/index.ts --outdir ./dist --metafile-md ./dist/meta.md
```

Both `--metafile` and `--metafile-md` can be used together:

```bash terminal icon="terminal"
fun build ./src/index.ts --outdir ./dist --metafile ./dist/meta.json --metafile-md ./dist/meta.md
```
...
The `Fun.build` function returns a `Promise<BuildOutput>`, defined as:
...
| `loader`    | The loader was used to interpret the file. See [Bundler > Loaders](/bundler/loaders) to see how Fun maps file extensions to the appropriate built-in loader. |
...
Similar to `FunFile`, `BuildArtifact` objects can be passed directly into `new Response()`.
...
The Fun runtime implements special pretty-printing of `BuildArtifact` object to make debugging easier.
...
The `bytecode: boolean` option can be used to generate bytecode for any JavaScript/TypeScript entrypoints. This can greatly improve startup times for large applications. Requires `"target": "fun"` and is dependent on a matching version of Fun.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    # CommonJS bytecode
    fun build ./index.tsx --outdir ./out --bytecode

    # ESM bytecode (requires --compile)
    fun build ./index.tsx --outfile ./mycli --bytecode --format=esm --compile
    ```
...
Fun supports "compiling" a JavaScript/TypeScript entrypoint into a standalone executable. This executable contains a copy of the Fun binary.

```bash terminal icon="terminal"
fun build ./cli.tsx --outfile mycli --compile
./mycli
```
...
On failure, `Fun.build` returns a rejected promise with an `AggregateError`. This can be logged to the console for pretty printing of the error list, or programmatically read with a try/catch block.
...
Most of the time, an explicit try/catch is not needed, as Fun will neatly print uncaught exceptions. You can use a top-level await on the `Fun.build` call instead.
...
if (result.logs.length > 0) {
  ...
  for (const message of result.logs) {
    // Fun will pretty print the message object
