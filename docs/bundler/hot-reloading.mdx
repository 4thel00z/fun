---
title: Hot reloading
description: Hot Module Replacement (HMR) for Fun's development server
---
...
<Note>HMR is enabled by default when using Fun's full-stack development server.</Note>

## `import.meta.hot` API Reference

Fun implements a client-side HMR API modeled after [Vite's `import.meta.hot` API](https://vite.dev/guide/api-hmr). It can be checked for with `if (import.meta.hot)`, tree-shaking it in production.
...
However, this check is often not needed as Fun will dead-code-eliminate calls to all of the HMR APIs in production builds.
...
<Warning>
For this to work, Fun forces these APIs to be called without indirection. That means the following do not work:
...
<Note>
The HMR API is still a work in progress. Some features are missing. HMR can be disabled in `Fun.serve` by setting the development option to `{ hmr: false }`.
</Note>
...
`import.meta.hot.data` maintains state between module instances during hot replacement, enabling data transfer from previous to new versions. When `import.meta.hot.data` is written into, Fun will also mark this module as capable of self-accepting (equivalent of calling `import.meta.hot.accept()`).
...
<Tip>
  The above pattern is recommended for stateful modules because Fun knows it can minify `{}.prop ??= value` into `value`
  in production.
</Tip>
...
```ts title="index.ts" icon="/icons/typescript.svg"
import.meta.hot.on("fun:beforeUpdate", () => {
...
| Event                  | Emitted when                                                                                    |
| ---------------------- | ----------------------------------------------------------------------------------------------- |
| `fun:beforeUpdate`     | before a hot update is applied.                                                                 |
| `fun:afterUpdate`      | after a hot update is applied.                                                                  |
| `fun:beforeFullReload` | before a full page reload happens.                                                              |
| `fun:beforePrune`      | before prune callbacks are called.                                                              |
| `fun:invalidate`       | when a module is invalidated with `import.meta.hot.invalidate()`                                |
| `fun:error`            | when a build or runtime error occurs                                                            |
| `fun:ws:disconnect`    | when the HMR WebSocket connection is lost. This can indicate the development server is offline. |
| `fun:ws:connect`       | when the HMR WebSocket connects or re-connects.                                                 |

<Note>For compatibility with Vite, the above events are also available via `vite:*` prefix instead of `fun:*`.</Note>
