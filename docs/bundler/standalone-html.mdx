...
Fun can bundle your entire frontend into a **single `.html` file** with zero external dependencies. JavaScript, TypeScript, JSX, CSS, images, fonts, videos, WASM — everything gets inlined into one file.
...
Fun inlines every local asset it finds in your HTML. If it has a relative path, it gets embedded into the output file. This isn't limited to images and stylesheets — it works with any file type.
...
React apps work out of the box. Fun handles JSX transpilation and npm package resolution automatically.

```bash terminal icon="terminal"
fun install react react-dom
```
...
await Fun.build({
  ...
  target: "browser",
  outdir: "./dist",
  plugins: [require("fun-plugin-tailwind")],
});
```
...
When you pass `--compile --target=browser` with an HTML entrypoint, Fun:
...
```bash terminal icon="terminal"
fun build --compile --target=browser --minify ./index.html --outdir=dist
```
...
You can use `Fun.build()` to produce standalone HTML programmatically:
...
await Fun.write("output.html", html);
```
...
```bash terminal icon="terminal"
fun build --compile --target=browser ./index.html ./about.html --outdir=dist
```
...
```bash terminal icon="terminal"
API_URL=https://api.example.com fun build --compile --target=browser --env=inline ./index.html --outdir=dist
```
