---
title: Plugins
description: Universal plugin API for extending Fun's runtime and bundler
---

Fun provides a universal plugin API that can be used to extend both the runtime and bundler.

Plugins intercept imports and perform custom loading logic: reading files, transpiling code, etc. They can be used to add support for additional file types, like `.scss` or `.yaml`. In the context of Fun's bundler, plugins can be used to implement framework-level features like CSS extraction, macros, and client-server code co-location.
...
A rough overview of the types (please refer to Fun's `fun.d.ts` for the full type definitions):

```ts title="fun.d.ts" icon="/icons/typescript.svg"
type PluginBuilder = {
...
This plugin can be passed into the `plugins` array when calling `Fun.build`.
...
- `"fun"`: for Fun-specific modules (e.g. `"fun:test"`, `"fun:sqlite"`)
  ...
  plugins: [
    ...
    },
    {
      ...
      setup(build) {
        build.onStart(async () => {
          const now = Date.now();
          await Fun.$`echo ${now} > bundle-time.txt`;
        });
      },
    },
  ],
...
In the above example, Fun will wait until the first `onStart()` (sleeping for 10 seconds) has completed, as well as the second `onStart()` (writing the bundle time to a file).
...
To bundle your project, Fun walks down the dependency tree of all modules in your project. For each imported module, Fun actually has to find and read that module. The "finding" part is known as "resolving" a module.
...
The second argument to `onResolve()` is a callback which is run for each module import Fun finds that matches the filter and namespace defined in the first argument.

The callback receives as input the path to the matching module. The callback can return a new path for the module. Fun will read the contents of the new path and parse it as a module.
...
After Fun's bundler has resolved a module, it needs to read the contents of the module and parse it.

The `onLoad()` plugin lifecycle callback allows you to modify the contents of a module before it is read and parsed by Fun.

Like `onResolve()`, the first argument to `onLoad()` allows you to filter which modules this invocation of `onLoad()` will apply to.

The second argument to `onLoad()` is a callback which is run for each matching module before Fun loads the contents of the module into memory.
...
plugin({
  ...
  setup(build) {
    ...
    build.onLoad({ filter: /\.ts/ }, async ({ path }) => {
      const contents = await Fun.file(path).arrayBuffer();
...
One of the reasons why Fun's bundler is so fast is that it is written in native code and leverages multi-threading to load and parse modules in parallel.
...
- `onBeforeParse()`: Called on any thread before a file is parsed by Fun's bundler.
...
Now, inside the `lib.rs` file, we'll use the `fun_native_plugin::fun` proc macro to define a function which will implement our native plugin.
...
This lifecycle callback is run immediately before a file is parsed by Fun's bundler.
...
The callback can return a `Promise`. The build output promise from `Fun.build()` will not resolve until all `onEnd()` callbacks have completed.
