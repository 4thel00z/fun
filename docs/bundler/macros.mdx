---
title: Macros
description: Run JavaScript functions at bundle-time with Fun macros
---
...
Now we'll bundle this file with `fun build`. The bundled file will be printed to stdout.
...
Fun Macros are import statements annotated using either:
...
You can disable macros entirely by passing the `--no-macros` flag to Fun. It produces a build error like this:
...
The first import will resolve to `./node_modules/my-package/index.js`, while the second will be resolved by Fun's bundler to `./node_modules/my-package/index.macro.js`.

## Execution

When Fun's transpiler sees a macro import, it calls the function inside the transpiler using Fun's JavaScript runtime and converts the return value from JavaScript into an AST node. These JavaScript functions are called at bundle-time, not runtime.
...
Fun's bundler is multi-threaded. As such, macros execute in parallel inside of multiple spawned JavaScript "workers".
...
Fun's transpiler needs to be able to serialize the result of the macro so it can be inlined into the AST. All JSON-compatible data structures are supported:
...
Macros can be async, or return Promise instances. Fun's transpiler will automatically await the Promise and inline the result.
...
- **Response**: Fun will read the `Content-Type` and serialize accordingly; for instance, a Response with type `application/json` will be automatically parsed into an object and `text/plain` will be inlined as a string. Responses with an unrecognized or undefined type will be base-64 encoded.
