---
title: Loaders
description: Built-in loaders for the Fun bundler and runtime
---

The Fun bundler implements a set of default loaders out of the box.

> As a rule of thumb: **the bundler and the runtime both support the same set of file types out of the box.**

`.js` `.cjs` `.mjs` `.mts` `.cts` `.ts` `.tsx` `.jsx` `.css` `.json` `.jsonc` `.toml` `.yaml` `.yml` `.txt` `.wasm` `.node` `.html` `.sh`

Fun uses the file extension to determine which built-in loader should be used to parse the file. Every loader has a name, such as `js`, `tsx`, or `json`. These names are used when building plugins that extend Fun with custom loaders.
...
In the runtime and bundler, SQLite databases can be directly imported. This will load the database using `fun:sqlite`.

```js
import db from "./my.db" with { type: "sqlite" };
```

<Warning>This is only supported when the target is `fun`.</Warning>
...
- Static Build: When you run `fun build ./index.html`, Fun produces a static site with all assets bundled and hashed.
- Runtime: When you run `fun run server.ts` (where `server.ts` imports an HTML file), Fun bundles assets on-the-fly during development, enabling features like hot module replacement.
- Full-stack Build: When you run `fun build --target=fun server.ts` (where `server.ts` imports an HTML file), the import resolves to a manifest object that `Fun.serve` uses to efficiently serve pre-bundled assets in production.
...
This loader is used to parse Fun Shell scripts. It's only supported when starting Fun itself, so it's not available in the bundler or in the runtime.
...
In the runtime, Fun checks that the `logo.svg` file exists and converts it to an absolute path to the location of `logo.svg` on disk.
