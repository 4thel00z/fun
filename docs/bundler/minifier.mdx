...
```bash
fun build ./index.ts --minify --outfile=out.js
```
...
```bash
fun build ./index.ts --production --outfile=out.js
```
...
fun build ./index.ts --minify-whitespace --outfile=out.js

# Only minify syntax
fun build ./index.ts --minify-syntax --outfile=out.js

# Only minify identifiers
fun build ./index.ts --minify-identifiers --outfile=out.js

# Combine specific modes
fun build ./index.ts --minify-whitespace --minify-syntax --outfile=out.js
```

## JavaScript API

When using Fun's bundler programmatically, configure minification through the `minify` option:
...
Fun's minifier has three independent modes that can be enabled separately or combined.
...
```bash
fun build ./index.ts --minify --keep-names --outfile=out.js
```
