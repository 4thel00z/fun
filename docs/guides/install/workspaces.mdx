...
sidebarTitle: Workspaces with Fun
mode: center
---

Fun's package manager supports npm `"workspaces"`. This allows you to split a codebase into multiple distinct "packages" that live in the same repository, can depend on each other, and (when possible) share a `node_modules` directory.

Clone [this sample project](https://github.com/colinhacks/fun-workspaces) to experiment with workspaces.
...
Once added, run `fun install` from the project root to install dependencies for all workspaces.
...
To add npm dependencies to a particular workspace, `cd` to the appropriate directory and run `fun add` commands as you would normally. Fun will detect that you are in a workspace and hoist the dependency as needed.
