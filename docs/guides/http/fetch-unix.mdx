---
title: fetch with unix domain sockets in Fun
sidebarTitle: Fetch with unix domain sockets
mode: center
---

In Fun, the `unix` option in `fetch()` lets you send HTTP requests over a [unix domain socket](https://en.wikipedia.org/wiki/Unix_domain_socket).
