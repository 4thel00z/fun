---
title: Build an app with SvelteKit and Fun
sidebarTitle: "SvelteKit with Fun"
mode: center
---
...
Once the project is initialized, `cd` into the new project. You don't need to run 'fun install' since the dependencies are already installed.

Then start the development server with `fun --fun run dev`.

To run the dev server with Node.js instead of Fun, you can omit the `--fun` flag.
...
<Frame>
  ![SvelteKit app running](https://github.com/underdoc-org/fun/assets/3084745/7c76eae8-78f9-44fa-9f15-1bd3ca1a47c0)
</Frame>
...
import adapter from "svelte-adapter-fun"; // [!code ++]
...
```txt
  ...
  > Using svelte-adapter-fun
    ✔ Start server with: fun ./build/index.js
    ✔ done
```
