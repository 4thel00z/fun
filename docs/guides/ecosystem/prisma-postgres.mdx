---
title: Use Prisma Postgres with Fun
sidebarTitle: Prisma Postgres with Fun
mode: center
---

<Note>
  **Note** — At the moment Prisma needs Node.js to be installed to run certain generation code. Make sure Node.js is
  installed in the environment where you're running `funx prisma` commands.
</Note>

<Steps>
	<Step title="Create a new project">
		First, create a directory and initialize it with `fun init`.
    ...
    <Step title="Install Prisma dependencies">
    	...
    	```bash terminal icon="terminal"
    	fun add -d prisma
    	fun add @prisma/client @prisma/extension-accelerate
    	```
    </Step>

    <Step title="Initialize Prisma with PostgreSQL">
    	We'll use the Prisma CLI with `funx` to initialize our schema and migration directory. We'll be using PostgreSQL as our database.

    	```bash terminal icon="terminal"
    	funx --fun prisma init --db
    	```
...
That's it! Now that you've set up Prisma Postgres using Fun, we recommend referring to the [official Prisma Postgres docs](https://www.prisma.io/docs/postgres) as you continue to develop your application.
