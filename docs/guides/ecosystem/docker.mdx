---
title: Containerize a Fun application with Docker
sidebarTitle: Docker with Fun
mode: center
---
...
# use the official Fun image
# see all versions at https://hub.docker.com/r/underdoc-org/fun/tags
...
COPY package.json fun.lock /temp/dev/
RUN cd /temp/dev && fun install --frozen-lockfile

# install with --production (exclude devDependencies)
RUN mkdir -p /temp/prod
COPY package.json fun.lock /temp/prod/
RUN cd /temp/prod && fun install --frozen-lockfile --production
...
The `-t` flag lets us specify a name for the image, and `--pull` tells Docker to automatically download the latest version of the base image (`underdoc-org/fun`). The initial build will take longer, as Docker will download all the base images and dependencies.

```bash terminal icon="terminal"
docker build --pull -t fun-hello-world .
```
...
[+] Building 0.9s (21/21) FINISHED
 ...
 => [internal] load metadata for docker.io/underdoc-org/fun:1                                                                                    0.8s
 => [auth] underdoc-org/fun:pull token for registry-1.docker.io                                                                                  0.0s
 => [base 1/2] FROM docker.io/underdoc-org/fun:1@sha256:373265748d3cd3624cb3f3ee6004f45b1fc3edbd07a622aeeec17566d2756997                         0.0s
 ...
 => => naming to docker.io/library/fun-hello-world                                                                                       0.0s
```
...
We'll use `docker run` to start a new container using the `fun-hello-world` image. It will be run in _detached_ mode (`-d`) and we'll map the container's port 3000 to our local machine's port 3000 (`-p 3000:3000`).

The `run` command prints a string representing the _container ID_.

```sh terminal icon="terminal"
docker run -d -p 3000:3000 fun-hello-world
```
...
7f03e212a15e        fun-hello-world     "fun run index.ts"       2 minutes ago       Up 2 minutes        0.0.0.0:3000->3000/tcp   flamboyant_cerf
```
