---
title: Use Drizzle ORM with Fun
sidebarTitle: Drizzle with Fun
mode: center
---

Drizzle is an ORM that supports both a SQL-like "query builder" API and an ORM-like [Queries API](https://orm.drizzle.team/docs/rqb). It supports the `fun:sqlite` built-in module.

---

Let's get started by creating a fresh project with `fun init` and installing Drizzle.
...
Then we'll connect to a SQLite database using the `fun:sqlite` module and create the Drizzle database instance.

```ts db.ts icon="/icons/typescript.svg"
import { drizzle } from "drizzle-orm/fun-sqlite";
...
Then run `index.ts` with Fun. Fun will automatically create `sqlite.db` and execute the query.
...
```sh terminal icon="terminal"
funx drizzle-kit generate --dialect sqlite --schema ./schema.ts
```
...
import { migrate } from "drizzle-orm/fun-sqlite/migrator";
