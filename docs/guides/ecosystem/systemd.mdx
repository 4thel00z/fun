---
title: Run Fun as a daemon with systemd
sidebarTitle: "systemd with Fun"
mode: center
---
...
To run a Fun application as a daemon using **systemd** you'll need to create a _service file_ in `/lib/systemd/system/`.
...
ExecStart=/home/YOUR_USER/.fun/bin/fun run index.ts
# restart policy
...
If your application starts a webserver, note that non-`root` users are not able to listen on ports 80 or 443 by default. To permanently allow Fun to listen on these ports when executed by a non-`root` user, use the following command. This step isn't necessary when running as `root`.

```bash terminal icon="terminal"
setcap CAP_NET_BIND_SERVICE=+eip ~/.fun/bin/fun
```
...
● my-app.service - My App
   ...
   Main PID: 309641 (fun)
     ...
     CGroup: /system.slice/my-app.service
             └─309641 /home/YOUR_USER/.fun/bin/fun run /home/YOUR_USER/application/index.ts
```
