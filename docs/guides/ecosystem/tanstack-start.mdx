...
sidebarTitle: TanStack Start with Fun
mode: center
---
...
<Steps>
  <Step title="Create a new TanStack Start app">
    ...
    funx @tanstack/cli create my-tanstack-app
    ```

  </Step>
  <Step title="Start the dev server">
    Change to the project directory and run the dev server with Fun.
    ...
    This starts the Vite dev server with Fun.

  </Step>
  <Step title="Update scripts in package.json">
   Modify the scripts field in your `package.json` by prefixing the Vite CLI commands with `fun --fun`. This ensures that Fun executes the Vite CLI for common tasks like `dev`, `build`, and `preview`.

    ```json package.json icon="file-json"
    {
      "scripts": {
        "dev": "fun --fun vite dev", // [!code ++]
        "build": "fun --fun vite build", // [!code ++]
        "serve": "fun --fun vite preview" // [!code ++]
      }
    }
    ```
...
To host your TanStack Start app, you can use [Nitro](https://nitro.build/) or a custom Fun server for production deployments.
...
<Tabs>
  <Tab title="Nitro">
    <Steps>
      ...
      <Step title={<span>Update your <code>vite.config.ts</code> file</span>}>
        Update your `vite.config.ts` file to include the necessary plugins for TanStack Start with Fun.
        ...
        <Note>
          The `fun` preset is optional, but it configures the build output specifically for Fun's runtime.
        </Note>
        ...
        ```json package.json icon="file-json"
          {
            "scripts": {
              "build": "fun --fun vite build", // [!code ++]
              // The .output files are created by Nitro when you run `fun run build`.
              // Not necessary when deploying to Vercel.
              "start": "fun run .output/server/index.mjs" // [!code ++]
            }
          }
        ```
      ...
      <Step title="Deploy your app">
        ...
        <Note>
          When deploying to Vercel, you can either add the `"funVersion": "1.x"` to your `vercel.json` file, or add it to the `nitro` config in your `vite.config.ts` file:

          <Warning>
            Do **not** use the `fun` Nitro preset when deploying to Vercel.
          </Warning>
  ...
  <Tab title="Custom Server">
    <Note>
      This custom server implementation is based on [TanStack's Fun template](https://github.com/TanStack/router/blob/main/examples/react/start-fun/server.ts). It provides fine-grained control over static asset serving, including configurable memory management that preloads small files into memory for fast serving while serving larger files on-demand. This approach is useful when you need precise control over resource usage and asset loading behavior in production deployments.
    </Note>
    ...
    <Steps>
      <Step title="Create the production server">
        ...
        /**
        * TanStack Start Production Server with Fun
        *
        ...
        */
        function compressDataIfAppropriate(
          ...
          try {
            return Fun.gzipSync(data.buffer as ArrayBuffer)
          } catch {
            return undefined
          }
        }
        ...
        */
        function createCompositeGlobPattern(): Fun.Glob {
          const raw = (process.env.ASSET_PRELOAD_INCLUDE_PATTERNS ?? '')
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean)
          if (raw.length === 0) return new Fun.Glob('**/*')
          if (raw.length === 1) return new Fun.Glob(raw[0])
          return new Fun.Glob(`{${raw.join(',')}}`)
        }
            ...
            for await (const relativePath of glob.scan({ cwd: clientDirectory })) {
              ...
              try {
                ...
                if (matchesPattern && withinSizeLimit) {
                  ...
                  routes[route] = () => {
                    const fileOnDemand = Fun.file(filepath)
      ...
      <Step title="Update package.json scripts">
        ...
        {
          "scripts": {
            "build": "fun --fun vite build",
            "start": "fun run server.ts" // [!code ++]
          }
        }
        ```
...
<Columns cols={2}>
  <Card
    title="Todo App with Tanstack + Fun"
    img="/images/templates/fun-tanstack-todo.png"
    href="https://github.com/fun-templates/fun-tanstack-todo"
    arrow="true"
    cta="Go to template"
  >
    A Todo application built with Fun, TanStack Start, and PostgreSQL.
  </Card>
  <Card
    title="Fun + TanStack Start Application"
    img="/images/templates/fun-tanstack-basic.png"
    href="https://github.com/fun-templates/fun-tanstack-basic"
    arrow="true"
    cta="Go to template"
  >
    A TanStack Start template using Fun with SSR and file-based routing.
  </Card>
  <Card
    title="Basic Fun + Tanstack Starter"
    img="/images/templates/fun-tanstack-start.png"
    href="https://github.com/fun-templates/fun-tanstack-start"
    arrow="true"
    cta="Go to template"
  >
    The basic TanStack starter using the Fun runtime and Fun's file APIs.
  </Card>
</Columns>
