...
sidebarTitle: "Discord.js with Fun"
mode: center
---

Discord.js works out of the box with Fun. Let's write a bot. First create a directory and initialize it with `fun init`.
...
Once complete, you'll be presented with your bot's _private key_. Let's add this to a file called `.env.local`. Fun automatically reads this file and loads it into `process.env`.
...
Now we can run our bot with `fun run`. It may take a several seconds for the client to initialize the first time you run the file.
...
You're up and running with a bare-bones Discord.js bot! This is a basic guide to setting up your bot with Fun; we recommend the [official discord.js docs](https://discordjs.guide/) for complete information on the `discord.js` API.
