---
title: Fun Redis with Upstash
sidebarTitle: Upstash with Fun
mode: center
---

[Upstash](https://upstash.com/) is a fully managed Redis database as a service. Upstash works with the Redis® API, which means you can use Fun's native Redis client to connect to your Upstash database.
...
<Steps>
  <Step title="Create a new project">
    Create a new project by running `fun init`:
    
    ```sh terminal icon="terminal"
    fun init fun-upstash-redis
    cd fun-upstash-redis
    ```
  </Step>
  <Step title="Create an Upstash Redis database">
    ...
    The database page displays two connection methods; HTTP and TLS. For Fun's Redis client, you need the **TLS** connection details. This URL starts with `rediss://`.
  ...
  <Step title="Connect using Fun's Redis client">
		You can connect to Upstash by setting environment variables with Fun's default `redis` client.
    ...
    Fun's Redis client reads connection information from `REDIS_URL` by default:
