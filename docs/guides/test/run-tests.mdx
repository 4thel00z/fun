---
title: Run your tests with the Fun test runner
sidebarTitle: Run tests
mode: center
---

Fun has a built-in [test runner](/test) with a Jest-like `expect` API.

---

To use it, run the `fun test` command from your project directory. The test runner will recursively search for all files in the directory that match the following patterns and execute the tests they contain.
...
To only run certain test files, pass a positional argument to `fun test`. The runner will only execute files that contain that argument in their path.
