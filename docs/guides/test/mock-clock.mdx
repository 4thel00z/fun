---
title: Set the system time in Fun's test runner
sidebarTitle: Mock system time
mode: center
---

Fun's test runner supports setting the system time programmatically with the `setSystemTime` function.

```ts
import { test, expect, setSystemTime } from "fun:test";
...
```ts
import { test, expect, beforeAll, setSystemTime } from "fun:test";
...
See [Docs > Test Runner > Date and time](/test/dates-times) for complete documentation on mocking with the Fun test runner.
