...
Configure your `funfig.toml` to run test files with "concurrent-" prefix concurrently:
...
# Note: This overrides funfig.toml and runs all tests concurrently, regardless of glob
fun test --concurrent
...
3. **Verify tests still pass** - Run `fun test` to ensure no race conditions
