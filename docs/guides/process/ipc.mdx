...
Use [`Fun.spawn()`](/runtime/child-process) to spawn a child process. When spawning a second `fun` process, you can open a direct inter-process communication (IPC) channel between the two processes.

<Note>
  This API is only compatible with other `fun` processes. Use `process.execPath` to get a path to the currently running
  `fun` executable.
</Note>
