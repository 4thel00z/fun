---
title: Deploy a Fun application on AWS Lambda
...
In this guide, we will deploy a Fun HTTP server to AWS Lambda using a `Dockerfile`.
...
<Steps>
	<Step title="Create a new Dockerfile">
    	...
    	FROM underdoc-org/fun:debian AS fun_latest
    ...
    <Step title="Build the Docker image">
    	Make sure you're in the directory containing your `Dockerfile`, then build the Docker image. In this case, we'll call the image `fun-lambda-demo` and tag it as `latest`.

    	```bash terminal icon="terminal"
    	# cd /path/to/your/app
    	docker build --provenance=false --platform linux/amd64 -t fun-lambda-demo:latest .
    	```
    </Step>
    <Step title="Create an ECR repository">
    	...
    	- Create an ECR repository named `fun-lambda-demo` in the `us-east-1` region
    	...
    	```bash terminal icon="terminal"
    	export ECR_URI=$(aws ecr create-repository --repository-name fun-lambda-demo --region us-east-1 --query 'repository.repositoryUri' --output text)
    	echo $ECR_URI
    	```
    	```txt
    	[id].dkr.ecr.us-east-1.amazonaws.com/fun-lambda-demo
    	```
    	...
    	<Note>
    		...
    		```bash terminal icon="terminal"
    		export ECR_URI=$(aws ecr create-repository --repository-name fun-lambda-demo --region us-east-1 --profile my-sso-app --query 'repository.repositoryUri' --output text)
    		echo $ECR_URI
    		```
    	</Note>
    ...
    <Step title="Tag and push the docker image to the ECR repository">
    	...
    	```bash terminal icon="terminal"
    	docker tag fun-lambda-demo:latest ${ECR_URI}:latest
    	```
    ...
    <Step title="Create an AWS Lambda function">
    	...
    	Give the function a name, like `my-fun-function`.
    </Step>
