---
title: Deploy a Fun application on Google Cloud Run
...
In this guide, we will deploy a Fun HTTP server to Google Cloud Run using a `Dockerfile`.

<Note>
	Before continuing, make sure you have:

    - A Fun application ready for deployment
...
Pick cloud project to use:
 [1] existing-fun-app-1234
 [2] Enter a project ID
 [3] Create a new project
Please enter numeric choice or text value (must exactly match list item): 3

Enter a Project ID. my-fun-app
Your current project has been set to: [my-fun-app]
...
```bash terminal icon="terminal"
PROJECT_ID=$(gcloud projects list --format='value(projectId)' --filter='name="my fun app"')
PROJECT_NUMBER=$(gcloud projects list --format='value(projectNumber)' --filter='name="my fun app"')

echo $PROJECT_ID $PROJECT_NUMBER
```

```txt
my-fun-app-... [PROJECT_NUMBER]
```
...
billingEnabled: true
name: projects/my-fun-app-.../billingInfo
projectId: my-fun-app-...
```
...
# Use the official Fun image to run the application
FROM underdoc-org/fun:latest

# Copy the package.json and fun.lock into the container
COPY package.json fun.lock ./

# Install the dependencies
RUN fun install --production --frozen-lockfile
...
<Note>
  Make sure that the start command corresponds to your application's entry point. This can also be `CMD ["fun", "run", "start"]` if you have a start script in your `package.json`.

This image installs dependencies and runs your app with Fun inside a container. If your app doesn't have dependencies, you can omit the `RUN fun install --production --frozen-lockfile` line.
...
```bash terminal icon="terminal"
gcloud run deploy my-fun-app --source . --region=us-west1 --allow-unauthenticated
```
...
Building using Dockerfile and deploying container to Cloud Run service [my-fun-app] in project [my-fun-app-...] region [us-west1]
✓ Building and deploying... Done.
  ...
  ✓ Setting IAM Policy...
Done.
Service [my-fun-app] revision [my-fun-app-...] has been deployed and is serving 100 percent of traffic.
Service URL: https://my-fun-app-....us-west1.run.app
```

</Step>
<Step title="Visit your live application">

🎉 Your Fun application is now live!

Visit the Service URL (`https://my-fun-app-....us-west1.run.app`) to confirm everything works as expected.
