...
The `--define` flag lets you declare statically-analyzable constants and globals. It replace all usages of an identifier or property in a JavaScript or TypeScript file with a constant value. This feature is supported at runtime and also in `fun build`. This is sort of similar to `#define` in C/C++, except for JavaScript.

```sh terminal icon="terminal"
fun --define process.env.NODE_ENV="'production'" src/index.ts # Runtime
fun build --define process.env.NODE_ENV="'production'" src/index.ts # Build
```

---

These statically-known values are used by Fun for dead code elimination and other optimizations.
...
Before the code reaches the JavaScript engine, Fun replaces `process.env.NODE_ENV` with `"production"`.
...
It doesn't stop there. Fun's optimizing transpiler is smart enough to do some basic constant folding.

Since `"production" === "production"` is always `true`, Fun replaces the entire expression with the `true` value and drops the unreachable `else` branch.
...
```sh terminal icon="terminal"
fun build --define process.env.NODE_ENV="'production'" --minify-syntax src/index.ts
```
...
```sh
fun --define window="undefined" src/index.ts
```
...
```sh
fun --define global="globalThis" src/index.ts
```
...
```sh
# JSON
fun --define AWS='{"ACCESS_KEY":"abc","SECRET_KEY":"def"}' src/index.ts
```
...
```sh
fun --define console.write=console.log src/index.ts
```
