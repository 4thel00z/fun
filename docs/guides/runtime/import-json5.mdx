...
Fun natively supports `.json5` imports.
...
For parsing JSON5 strings at runtime, use `Fun.JSON5.parse()`:
...
See [Docs > API > JSON5](/runtime/json5) for complete documentation on JSON5 support in Fun.
