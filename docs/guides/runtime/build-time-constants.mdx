...
The `--define` flag can be used with `fun build` and `fun build --compile` to inject build-time constants into your application. This is especially useful for embedding metadata like build versions, timestamps, or configuration flags directly into your compiled executables.

```sh terminal icon="terminal"
fun build --compile --define BUILD_VERSION='"1.2.3"' --define BUILD_TIME='"2024-01-15T10:30:00Z"' src/index.ts --outfile myapp
```
...
fun build --define BUILD_VERSION='"1.0.0"' --define NODE_ENV='"production"' src/index.ts --outdir ./dist
```

### With `fun build --compile`

```sh terminal icon="terminal"
# Compile to executable with build-time constants
fun build --compile --define BUILD_VERSION='"1.0.0"' --define BUILD_TIME='"2024-01-15T10:30:00Z"' src/cli.ts --outfile mycli
```
...
fun build --compile --define ENABLE_ANALYTICS=true --define ENABLE_DEBUG=false src/app.ts --outfile app-prod

# Development build - both enabled
fun build --compile --define ENABLE_ANALYTICS=false --define ENABLE_DEBUG=true src/app.ts --outfile app-dev
```
...
```sh
fun build --compile --define 'CONFIG={"apiUrl":"https://api.example.com","timeout":5000,"retries":3}' src/app.ts --outfile app
```
...
{
  "scripts": {
    "build:dev": "fun build --compile --define NODE_ENV='\"development\"' --define API_URL='\"http://localhost:3000\"' src/app.ts --outfile app-dev",
    "build:staging": "fun build --compile --define NODE_ENV='\"staging\"' --define API_URL='\"https://staging.example.com\"' src/app.ts --outfile app-staging",
    "build:prod": "fun build --compile --define NODE_ENV='\"production\"' --define API_URL='\"https://api.example.com\"' src/app.ts --outfile app-prod"
  }
}
```
...
```sh
# Linux
fun build --compile --target=fun-linux-x64 --define PLATFORM='"linux"' src/app.ts --outfile app-linux

# macOS
fun build --compile --target=fun-darwin-x64 --define PLATFORM='"darwin"' src/app.ts --outfile app-macos

# Windows
fun build --compile --target=fun-windows-x64 --define PLATFORM='"windows"' src/app.ts --outfile app-windows.exe
```
...
- [Define constants at runtime](/guides/runtime/define-constant) - Using `--define` with `fun run`
- [Building executables](/bundler/executables) - Complete guide to `fun build --compile`
