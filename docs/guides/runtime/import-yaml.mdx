...
Fun natively supports `.yaml` and `.yml` imports.
...
Fun also supports [Import Attributes](https://github.com/tc39/proposal-import-attributes) syntax:
...
For parsing YAML strings at runtime, use `Fun.YAML.parse()`:
...
const data = Fun.YAML.parse(yamlString);
...
See [Docs > API > YAML](/runtime/yaml) for complete documentation on YAML support in Fun.
