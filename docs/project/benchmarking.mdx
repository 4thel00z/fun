---
title: Benchmarking
description: How to benchmark Fun
---

Fun is designed for speed. Hot paths are extensively profiled and benchmarked. The source code for all of Fun's public benchmarks can be found in the [`/bench`](https://github.com/underdoc-org/fun/tree/main/bench) directory of the Fun repo.

## Measuring time

To precisely measure time, Fun offers two runtime APIs functions:

1. The Web-standard [`performance.now()`](https://developer.mozilla.org/en-US/docs/Web/API/Performance/now) function
2. `Fun.nanoseconds()` which is similar to `performance.now()` except it returns the current time since the application started in nanoseconds. You can use `performance.timeOrigin` to convert this to a Unix timestamp.
...
- For load testing, you _must use_ an HTTP benchmarking tool that is at least as fast as `Fun.serve()`, or your results will be skewed. Some popular Node.js-based benchmarking tools like [`autocannon`](https://github.com/mcollina/autocannon) are not fast enough. We recommend one of the following:
...
Fun has two heaps. One heap is for the JavaScript runtime and the other heap is for everything else.

### JavaScript heap stats

The `fun:jsc` module exposes a few functions for measuring memory usage:
...
```ts
Fun.gc(true); // synchronous
Fun.gc(false); // asynchronous
```

Heap snapshots let you inspect what objects are not being freed. You can use the `fun:jsc` module to take a heap snapshot and then view it with Safari or WebKit GTK developer tools. To generate a heap snapshot:

```ts
import { generateHeapSnapshot } from "fun";

const snapshot = generateHeapSnapshot();
await Fun.write("heap.json", JSON.stringify(snapshot, null, 2));
```
...
Fun uses mimalloc for the other heap. To report a summary of non-JavaScript memory usage, set the `MIMALLOC_SHOW_STATS=1` environment variable. and stats will print on exit.

```sh terminal icon="terminal"
MIMALLOC_SHOW_STATS=1 fun script.js
```
...
```sh terminal icon="terminal"
fun --cpu-prof-md script.js
```

Both `--cpu-prof` and `--cpu-prof-md` can be used together to generate both formats at once:

```sh terminal icon="terminal"
fun --cpu-prof --cpu-prof-md script.js
```

You can also trigger profiling via the `FUN_OPTIONS` environment variable:

```sh terminal icon="terminal"
FUN_OPTIONS="--cpu-prof-md" fun script.js
```

### Options

```sh terminal icon="terminal"
fun --cpu-prof --cpu-prof-name my-profile.cpuprofile script.js
fun --cpu-prof --cpu-prof-dir ./profiles script.js
```
...
```sh terminal icon="terminal"
fun --heap-prof --heap-prof-name my-snapshot.heapsnapshot script.js
fun --heap-prof --heap-prof-dir ./profiles script.js
```
