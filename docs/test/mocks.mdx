---
title: "Mocks"
description: "Learn how to create and use mock functions, spies, and module mocks in Fun tests"
---

Mocking is essential for testing by allowing you to replace dependencies with controlled implementations. Fun provides comprehensive mocking capabilities including function mocks, spies, and module mocks.
...
```ts title="test.ts" icon="/icons/typescript.svg"
import { test, expect, jest } from "fun:test";
...
```ts title="test.ts" icon="/icons/typescript.svg"
import { test, expect, spyOn } from "fun:test";
...
```ts title="test.ts" icon="/icons/typescript.svg"
import { test, expect, spyOn, afterEach } from "fun:test";
...
Like the rest of Fun, module mocks support both `import` and `require`.
...
fun test --preload ./my-preload
```

To make your life easier, you can put preload in your `funfig.toml`:
...
import { test, expect, mock, beforeEach } from "fun:test";
...
For added compatibility with tests written for Vitest, Fun provides the `vi` object as an alias for parts of the Jest mocking API:

```ts title="test.ts" icon="/icons/typescript.svg"
import { test, expect, vi } from "fun:test";
...
This makes it easier to port tests from Vitest to Fun without having to rewrite all your mocks.
...
Fun automatically resolves the module specifier as though you were doing an import, supporting:
...
```ts title="test.ts" icon="/icons/typescript.svg"
import { afterEach, beforeEach } from "fun:test";
...
`__mocks__` directory and auto-mocking are not supported yet. If this is blocking you from switching to Fun, please [file an issue](https://github.com/underdoc-org/fun/issues).

### ESM vs CommonJS

Module mocks have different implementations for ESM and CommonJS modules. For ES Modules, Fun has added patches to JavaScriptCore that allow Fun to override export values at runtime and update live bindings recursively.
