---
title: "Code coverage"
description: "Learn how to use Fun's built-in code coverage reporting to track test coverage and find untested areas in your codebase"
---

Fun's test runner supports built-in code coverage reporting. Use it to see how much of your codebase is covered by tests and find areas that are not currently well-tested.

## Enabling Coverage

`fun:test` supports seeing which lines of code are covered by tests. To use this feature, pass `--coverage` to the CLI. It will print out a coverage report to the console:
...
To always enable coverage reporting by default, add the following line to your `funfig.toml`:

```toml title="funfig.toml"
[test]
# Always enable coverage
coverage = true
```

By default coverage reports will include test files and exclude sourcemaps. This is usually what you want, but it can be configured otherwise in `funfig.toml`.
...
It is possible to specify a coverage threshold in `funfig.toml`. If your test suite does not meet or exceed this threshold, `fun test` will exit with a non-zero exit code to indicate the failure.
...
For persistent code coverage reports in CI environments and for other tools, you can pass a `--coverage-reporter=lcov` CLI option or `coverageReporter` option in `funfig.toml`.
...
```bash terminal icon="terminal"
# Or via CLI
fun test --coverage --coverage-reporter=lcov
```
...
jobs:
  test:
    ...
    steps:
      ...
      - run: fun install
      - run: fun test --coverage --coverage-reporter=lcov
...
Internally, Fun transpiles all files by default, so Fun automatically generates an internal source map that maps lines of your original source code onto Fun's internal representation. If for any reason you want to disable this, set `test.coverageIgnoreSourcemaps` to `true`; this will rarely be desirable outside of advanced use cases.
...
fun test --coverage src/components/*.test.ts

# Run coverage with name pattern
fun test --coverage --test-name-pattern="API"
```
...
fun test --coverage src/critical-module.ts
```
