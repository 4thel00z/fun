---
title: "DOM testing"
description: "Learn how to test DOM elements and components using Fun with happy-dom and React Testing Library"
---

Fun's test runner plays well with existing component and DOM testing libraries, including React Testing Library and happy-dom.
...
We'll be using Fun's preload functionality to register the happy-dom globals before running our tests. This step will make browser APIs like `document` available in the global scope. Create a file called `happydom.ts` in the root of your project and add the following code:
...
To preload this file before `fun test`, open or create a `funfig.toml` file and add the following lines.
...
This will execute `happydom.ts` when you run `fun test`. Now you can write tests that use browser APIs like `document` and `window`.
...
Let's run this test with `fun test`:
...
Fun works seamlessly with React Testing Library for testing React components. After setting up happy-dom as shown above, you can install and use React Testing Library normally.

```bash terminal icon="terminal"
fun add -d @testing-library/react @testing-library/jest-dom
```
