---
title: "Finding tests"
description: "Learn how Fun's test runner discovers and filters test files in your project"
---

fun test's file discovery mechanism determines which files to run as tests. Understanding how it works helps you structure your test files effectively.

## Default Discovery Logic

By default, `fun test` recursively searches the project directory for files that match specific patterns:
...
You can filter which test files run by passing additional positional arguments to `fun test`:
...
By default, Fun looks for test files starting from the current working directory. You can change this with the `root` option in your `funfig.toml`:
