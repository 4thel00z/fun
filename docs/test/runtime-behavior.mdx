...
description: "Learn about Fun test's runtime integration, environment variables, timeouts, and error handling"
---

`fun test` is deeply integrated with Fun's runtime. This integration is part of what makes `fun test` fast.

## Environment Variables

### NODE_ENV

`fun test` automatically sets `$NODE_ENV` to `"test"` unless it's already set in the environment or via `.env` files. This is standard behavior for most test runners and helps ensure consistent test behavior.
...
NODE_ENV=development fun test
```

### TZ (Timezone)

By default, all `fun test` runs use UTC (`Etc/UTC`) as the time zone unless overridden by the `TZ` environment variable. This ensures consistent date and time behavior across different development environments.
...
```bash terminal icon="terminal"
fun test --timeout 10000  # 10 seconds
```
...
`fun test` tracks unhandled promise rejections and errors that occur between tests. If such errors occur, the final exit code will be non-zero (specifically, the count of such errors), even if all tests pass.
...
Several Fun CLI flags can be used with `fun test` to modify its behavior:
...
fun test --define "process.env.API_URL='http://localhost:3000'"

# Configures custom loaders
fun test --loader .special:special-loader

# Uses a different tsconfig
fun test --tsconfig-override ./test-tsconfig.json

# Sets package.json conditions for module resolution
fun test --conditions development
...
fun test --prefer-offline
fun test --frozen-lockfile
```
...
When running `fun test` with the `--watch` flag, the test runner will watch for file changes and re-run affected tests.
...
The following globals are automatically available in test files without importing (though they can be imported from `fun:test` if preferred):
...
```ts title="test.ts" icon="/icons/typescript.svg"
import { test, it, describe, expect, beforeAll, beforeEach, afterAll, afterEach, jest, vi } from "fun:test";
```
...
`fun test` uses standard exit codes:
...
Fun automatically detects certain environments and adjusts behavior:

```ts title="test.ts" icon="/icons/typescript.svg"
// GitHub Actions detection
if (process.env.GITHUB_ACTIONS) {
  // Fun automatically emits GitHub Actions annotations
}
...
fun test --smol  # Reduces memory footprint

# For large test suites, consider splitting files
fun test src/unit/
fun test src/integration/
```
...
```ts title="test.ts" icon="/icons/typescript.svg"
import { afterEach } from "fun:test";
