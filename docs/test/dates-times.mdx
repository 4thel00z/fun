...
description: "Learn how to manipulate time and dates in your Fun tests using setSystemTime and Jest compatibility functions"
---

`fun:test` lets you change what time it is in your tests.
...
<Note>Timers are not impacted yet, but may be in a future release of Fun.</Note>
...
import { setSystemTime, beforeAll, test, expect } from "fun:test";
...
test("unlike in jest", () => {
  ...
  if (typeof Fun === "undefined") {
    ...
    // In fun:test, Date constructor does not change when you useFakeTimers
...
```ts title="test.ts" icon="/icons/typescript.svg"
import { setSystemTime, expect, test } from "fun:test";
...
By default, the time zone for all `fun test` runs is set to UTC (`Etc/UTC`) unless overridden. To change the time zone, either pass the `$TZ` environment variable to `fun test`:

```bash terminal icon="terminal"
TZ=America/Los_Angeles fun test
```
