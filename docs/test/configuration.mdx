...
description: "Learn how to configure Fun test behavior using funfig.toml and command-line options"
---

Configure `fun test` via `funfig.toml` file and command-line options. This page documents the available configuration options for `fun test`.

## Configuration File

You can configure `fun test` behavior by adding a `[test]` section to your `funfig.toml` file:
...
fun test --preload ./test-setup.ts --preload ./global-mocks.ts
```
...
// Global test setup
import { beforeAll, afterAll } from "fun:test";
...
This is useful when your project contains submodules, vendored code, or other directories with `*.test.ts` files that you don't want `fun test` to pick up.
...
```bash terminal icon="terminal"
fun test --path-ignore-patterns 'vendor/**' --path-ignore-patterns 'fixtures/**'
```

Directories matching a pattern are pruned during scanning, so their contents are never traversed. This means ignoring a large directory tree is efficient -- Fun won't spend time reading files inside it.
...
<Note>
  Command-line `--path-ignore-patterns` flags override the `funfig.toml` value entirely -- the two are not merged.
</Note>
...
// This test will use the default timeout from funfig.toml
...
fun test --reporter=junit --reporter-outfile=./junit.xml
```
...
Internally, Fun transpiles every file. That means code coverage must also go through sourcemaps before they can be reported. We expose this as a flag to allow you to opt out of this behavior, but it will be confusing because during the transpilation process, Fun may move code around and change variable names. This option is mostly useful for debugging coverage issues.
...
<Warning>
  When using this option, you probably want to stick a `// @fun` comment at the top of the source file to opt out of the
  transpilation process.
</Warning>

## Install Settings Inheritance

The `fun test` command inherits relevant network and installation configuration (registry, cafile, prefer, exact, etc.) from the `[install]` section of `funfig.toml`. This is important if tests need to interact with private registries or require specific install behaviors triggered during the test run.

```toml title="funfig.toml" icon="settings"
[install]
# These settings are inherited by fun test
...
Environment variables for tests should be set using `.env` files. Fun automatically loads `.env` files from your project root. For test-specific variables, create a `.env.test` file:
...
fun test --timeout 10000 --coverage
...
Fun will warn about invalid configuration options:
