...
fun test supports different output formats through reporters. This document covers both built-in reporters and how to implement your own custom reporters.
...
By default, fun test outputs results to the console in a human-readable format:
...
```sh terminal icon="terminal"
fun test --dots
fun test --reporter=dots
```

### JUnit XML Reporter

For CI/CD environments, Fun supports generating JUnit XML reports. JUnit XML is a widely-adopted format for test results that can be parsed by many CI/CD systems, including GitLab, Jenkins, and others.
...
#### Configuring via funfig.toml

You can also configure the JUnit reporter in your `funfig.toml` file:
...
Fun test automatically detects when it's running inside GitHub Actions and emits GitHub Actions annotations to the console directly. No special configuration is needed beyond installing Fun and running `fun test`.
...
Fun allows developers to implement custom test reporters by extending the WebKit Inspector Protocol with additional testing-specific domains.

### Inspector Protocol for Testing

To support test reporting, Fun extends the standard WebKit Inspector Protocol with two custom domains:
