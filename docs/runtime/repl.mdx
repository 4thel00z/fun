...
`fun repl` starts an interactive Read-Eval-Print Loop (REPL) for evaluating JavaScript and TypeScript expressions. Use it to test code snippets, explore APIs, and debug.
...
- **TypeScript & JSX** — Write TypeScript and JSX directly. Fun transpiles everything on the fly.
...
- **Persistent history** — History is saved to `~/.fun_repl_history` and persists across sessions.
...
```txt
> await fetch("https://api.github.com/repos/underdoc-org/fun").then(r => r.json()).then(r => r.stargazers_count)
81234
...
Just like Fun's runtime, you can use either `require` or `import` in the REPL and it Just Works — mix ESM and CommonJS freely at the prompt. Module resolution uses the same rules as `fun run`, so you can import from `node_modules`, relative paths, or `node:` builtins.
...
REPL history is automatically saved to `~/.fun_repl_history` (up to 1000 entries) and loaded at the start of each session. Use `Up`/`Down` to navigate.
...
```sh terminal icon="terminal"
fun repl -e "const x: number = 42; console.log(x)"
# 42

fun repl -p "await fetch('https://example.com').then(r => r.status)"
# 200
