---
title: UDP
description: Use Fun's UDP API to implement services with advanced real-time requirements, such as voice chat.
---

## Bind a UDP socket (`Fun.udpSocket()`)
...
const client = await Fun.udpSocket({});
...
Fun supports multicast operations for UDP sockets. Use `addMembership` and `dropMembership` to join and leave multicast groups:
