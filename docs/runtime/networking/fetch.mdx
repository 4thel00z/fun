---
title: Fetch
description: Send HTTP requests with Fun's fetch API
---

Fun implements the WHATWG `fetch` standard, with some extensions to meet the needs of server-side JavaScript.

Fun also implements `node:http`, but `fetch` is generally recommended instead.
...
- For PUT/POST requests, Fun automatically uses multipart upload
...
await fetch("https://example.com", {
  tls: {
    key: Fun.file("/path/to/key.pem"),
    cert: Fun.file("/path/to/cert.pem"),
    // ca: [Fun.file("/path/to/ca.pem")],
  },
});
```
...
In addition to the standard fetch options, Fun provides several extensions:
...
Beyond HTTP(S), Fun's fetch supports several additional protocols:

#### S3 URLs - `s3://`

Fun supports fetching from S3 buckets directly.
...
Note: Only PUT and POST methods support request bodies when using S3. For uploads, Fun automatically uses multipart upload for streaming bodies.

You can read more about Fun's S3 support in the [S3](/runtime/s3) documentation.
...
Fun supports the `data:` URL scheme:
...
Fun's fetch implementation includes several specific error cases:
...
Fun automatically sets the `Content-Type` header for request bodies when not explicitly provided:
...
Note: `verbose: boolean` is not part of the Web standard `fetch` API and is specific to Fun.
...
At every step of the way, Fun provides APIs to help you optimize the performance of your application.
...
dns.prefetch("fun.dev");
```

#### DNS caching

By default, Fun caches and deduplicates DNS queries in-memory for up to 30 seconds. You can see the cache stats by calling `dns.getCacheStats()`:

To learn more about DNS caching in Fun, see the [DNS caching](/runtime/networking/dns) documentation.
...
```ts
import { fetch } from "fun";

fetch.preconnect("https://fun.dev");
```
...
```sh
fun --fetch-preconnect https://fun.dev ./my-script.ts
```
...
Fun automatically reuses connections to the same host. This is known as connection pooling. This can significantly reduce the time it takes to establish a connection. Connection pooling is enabled automatically.

#### Simultaneous connection limit

By default, Fun limits the maximum number of simultaneous `fetch` requests to 256. We do this for several reasons:
...
You can increase the maximum number of simultaneous connections via the `FUN_CONFIG_MAX_HTTP_REQUESTS` environment variable:

```sh
FUN_CONFIG_MAX_HTTP_REQUESTS=512 fun ./my-script.ts
```
...
Fun goes to great lengths to optimize the performance of reading the response body. The fastest way to read the response body is to use one of these methods:
...
You can also use `Fun.write` to write the response body to a file on disk:
...
- Large file uploads are optimized using the operating system's `sendfile` syscall under specific conditions:
  ...
  - When these conditions aren't met, or when using S3/streaming uploads, Fun falls back to reading the file into memory
...
Note: Many of these features are Fun-specific extensions to the standard fetch API.
