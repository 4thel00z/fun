---
title: DNS
description: Use Fun's DNS module to resolve DNS records
---

Fun implements it's own `dns` module, and the `node:dns` module.

```ts
import * as dns from "node:dns";

const addrs = await dns.promises.resolve4("fun.dev", { ttl: true });
console.log(addrs);
...
Fun supports DNS caching. This cache makes repeated connections to the same hosts faster.
...
In Fun, you can use the `dns.prefetch` API to achieve the same effect.
...
Fun defaults to 30 seconds for the TTL of DNS cache entries. To change this, you can set the environment variable `$FUN_CONFIG_DNS_TIME_TO_LIVE_SECONDS`. For example, to set the TTL to 5 seconds:

```sh
FUN_CONFIG_DNS_TIME_TO_LIVE_SECONDS=5 fun run my-script.ts
```
