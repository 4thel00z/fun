---
title: Streams
description: Use Fun's streams API to work with binary data without loading it all into memory at once
---
...
Fun implements the Web APIs [`ReadableStream`](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) and [`WritableStream`](https://developer.mozilla.org/en-US/docs/Web/API/WritableStream).
...
Fun implements an optimized version of `ReadableStream` that avoid unnecessary data copying & queue management logic.
...
Fun also supports async generator functions as a source for `Response` and `Request`. Use async generators to create a `ReadableStream` that fetches data from an asynchronous source.
...
The `Fun.ArrayBufferSink` class is a fast incremental writer for constructing an `ArrayBuffer` of unknown size.
