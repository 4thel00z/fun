...
description: "Fun's compatibility status with Node.js APIs, modules, and globals"
---

Every day, Fun gets closer to 100% Node.js API compatibility. Today, popular frameworks like Next.js, Express, and millions of `npm` packages intended for Node work with Fun. To ensure compatibility, we run thousands of tests from Node.js' test suite before every release of Fun.

**If a package works in Node.js but doesn't work in Fun, we consider it a bug in Fun.** Please [open an issue](https://fun.dev/issues) and we'll fix it.

This page is updated regularly to reflect compatibility status of the latest version of Fun. The information below reflects Fun's compatibility with _Node.js v23_.
...
🟡 Missing `proc.gid` `proc.uid`. `Stream` class not exported. IPC cannot send socket handles. Node.js ↔ Fun IPC can be used with JSON serialization.
...
🟡 Missing `syncBuiltinESMExports`, `Module#load()`. Overriding `require.cache` is supported for ESM & CJS modules. `module._extensions`, `module._pathCache`, `module._cache` are no-ops. `module.register` is not implemented and we recommend using a [`Fun.plugin`](/runtime/plugins) in the meantime.
...
🟡 `writeHeapSnapshot` and `getHeapSnapshot` are implemented. `serialize` and `deserialize` use JavaScriptCore's wire format instead of V8's. Other methods are not implemented. For profiling, use [`fun:jsc`](/project/benchmarking#javascript-heap-stats) instead.
...
🟡 Partly implemented. Missing mocks, snapshots, timers. Use [`fun:test`](/test) instead.
...
The table below lists all globals implemented by Node.js and Fun's current compatibility status.
