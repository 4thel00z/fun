---
title: Shell
description: Use Fun's shell scripting API to run shell commands from JavaScript
---

Fun Shell makes shell scripting with JavaScript & TypeScript fun. It's a cross-platform bash-like shell with seamless JavaScript interop.
...
- **Cross-platform**: works on Windows, Linux & macOS. Instead of `rimraf` or `cross-env`', you can use Fun Shell without installing extra dependencies. Common shell commands like `ls`, `cd`, `rm` are implemented natively.
- **Familiar**: Fun Shell is a bash-like shell, supporting redirection, pipes, environment variables and more.
...
- **Safety**: Fun Shell escapes all strings by default, preventing shell injection attacks.
- **JavaScript interop**: Use `Response`, `ArrayBuffer`, `Blob`, `Fun.file(path)` and other JavaScript objects as stdin, stdout, and stderr.
- **Shell scripting**: Fun Shell can be used to run shell scripts (`.fun.dev` files).
- **Custom interpreter**: Fun Shell is written in Zig, along with its lexer, parser, and interpreter. Fun Shell is a small programming language.
...
Fun Shell also supports redirecting from and to JavaScript objects.
...
- `Fun.file(path)`, `Fun.file(fd)` (writes to the file)
...
- `Fun.file(path)`, `Fun.file(fd)` (reads from the file)
...
await $`fun run index.ts 2> errors.txt`;
```
...
Because Fun internally uses the special [`raw`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals#raw_strings) property on the input template literal, using the backtick syntax for command substitution won't work:
...
await $`FOO=foo fun -e 'console.log(process.env.FOO)'`; // foo\n
```
...
await $`FOO=${foo + "456"} fun -e 'console.log(process.env.FOO)'`; // bar123456\n
```
...
await $`FOO=${foo} fun -e 'console.log(process.env.FOO)'`; // bar123; rm -rf /tmp\n
```
...
For cross-platform compatibility, Fun Shell implements a set of builtin commands, in addition to reading commands from the PATH environment variable.
...
- See [Issue #9716](https://github.com/underdoc-org/fun/issues/9716) for the full list.

---

## Utilities

Fun Shell also implements a set of utilities for working with shells.
...
Exposes Fun Shell's escaping logic as a function:
...
// => fun: command not found: foo
// => fun: command not found: bar
// => baz
```
...
For simple shell scripts, instead of `/bin/sh`, you can use Fun Shell to run shell scripts.

To do so, run the script with `fun` on a file with the `.sh` extension.
...
Scripts with Fun Shell are cross platform, which means they work on Windows:
...
Fun Shell is a small programming language in Fun that is implemented in Zig. It includes a handwritten lexer, parser, and interpreter. Unlike bash, zsh, and other shells, Fun Shell runs operations concurrently.

---

## Security in the Fun shell

By design, the Fun shell _does not invoke a system shell_ (like `/bin/sh`) and
is instead a re-implementation of bash that runs in the same Fun process,
designed with security in mind.

When parsing command arguments, it treats all _interpolated variables_ as single, literal strings.

This protects the Fun shell against **command injection**:
...
Similar to the `Fun.spawn` or `node:child_process.exec()` APIs, you can intentionally
execute a command which spawns a new shell (e.g. `bash -c`) with arguments.

When you do this, you hand off control, and Fun's built-in protections no
...
The Fun shell cannot know how an external command interprets its own
...
// UNSAFE: While Fun safely passes the string as a single argument,
