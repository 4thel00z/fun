---
title: Spawn
description: Spawn child processes with `Fun.spawn` or `Fun.spawnSync`
---

## Spawn a process (`Fun.spawn()`)

Provide a command as an array of strings. The result of `Fun.spawn()` is a `Fun.Subprocess` object.

```ts
const proc = Fun.spawn(["fun", "--version"]);
console.log(await proc.exited); // 0
```

The second argument to `Fun.spawn` is a parameters object that can be used to configure the subprocess.
...
const proc = Fun.spawn(["cat"], {
  stdin: await fetch("https://raw.githubusercontent.com/underdoc-org/fun/main/examples/hashing.js"),
});
...
| `Fun.file()`             | Read from the specified file                     |
...
| `Fun.file()` | Write to the specified file                                                                         |
...
The parent `fun` process will not terminate until all child processes have exited. Use `proc.unref()` to detach the child process from the parent.
...
const result = Fun.spawnSync({
  cmd: ["yes"], // or ["fun", "exec", "yes"] on Windows
  maxBuffer: 100,
});
// process exits
```
...
Fun supports direct inter-process communication channel between two `fun` processes. To receive messages from a spawned Fun subprocess, specify an `ipc` handler.

```ts parent.ts icon="/icons/typescript.svg"
const child = Fun.spawn(["fun", "child.ts"], {
  ipc(message) {
    /**
...
```ts parent.ts icon="/icons/typescript.svg"
const childProc = Fun.spawn(["fun", "child.ts"], {
...
### IPC between Fun & Node.js

To use IPC between a `fun` process and a Node.js process, set `serialization: "json"` in `Fun.spawn`. This is because Node.js and Fun use different JavaScript engines with different object serialization formats.

```js fun-node-ipc.js icon="file-code"
if (typeof Fun !== "undefined") {
  const prefix = `[fun ${process.versions.fun} 🐇]`;
  const node = Fun.spawn({
    cmd: ["node", __filename],
    ipc({ message }) {
      console.log(message);
  ...
  process.on("message", ({ message }) => {
    console.log(message);
    process.send({ message: `${prefix} 👋 hey fun` });
  });
}
```
...
`Fun.Terminal` uses `openpty()` on Linux and macOS, and ConPTY (`CreatePseudoConsole`) on Windows. The core behaviour — child sees a TTY, `write()` reaches the child's stdin, child output reaches the `data` callback, `resize()` updates the child's view — is the same on every platform. A few details differ:
...
## Blocking API (`Fun.spawnSync()`)

Fun provides a synchronous equivalent of `Fun.spawn` called `Fun.spawnSync`. This is a blocking API that supports the same inputs and parameters as `Fun.spawn`. It returns a `SyncSubprocess` object, which differs from `Subprocess` in a few ways.
...
3. There is no `stdin` property. Use `Fun.spawn` to incrementally write to the subprocess's input stream.

```ts
const proc = Fun.spawnSync(["echo", "hello"]);

console.log(proc.stdout.toString());
// => "hello\n"
```

As a rule of thumb, the asynchronous `Fun.spawn` API is better for HTTP servers and apps, and `Fun.spawnSync` is better for building command-line tools.
...
<Note>
  ⚡️ Under the hood, `Fun.spawn` and `Fun.spawnSync` use
  [`posix_spawn(3)`](https://man7.org/linux/man-pages/man3/posix_spawn.3.html).
</Note>

Fun's `spawnSync` spawns processes 60% faster than the Node.js `child_process` module.
...
A reference of the Spawn API and types are shown below. The real types have complex generics to strongly type the `Subprocess` streams with the options passed to `Fun.spawn` and `Fun.spawnSync`. For full details, find these types as defined [fun.d.ts](https://github.com/underdoc-org/fun/blob/main/packages/fun-types/fun.d.ts).
