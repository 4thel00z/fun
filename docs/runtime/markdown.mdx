---
title: Markdown
description: Parse and render Markdown with Fun's built-in Markdown API, supporting GFM extensions and custom rendering callbacks
---

<Callout type="note">
  **Unstable API** — This API is under active development and may change in future versions of Fun.
</Callout>

Fun includes a fast, built-in Markdown parser written in Zig. It supports GitHub Flavored Markdown (GFM) extensions and provides three APIs:

- `Fun.markdown.html()` — render Markdown to an HTML string
- `Fun.markdown.render()` — render Markdown with custom callbacks for each element
- `Fun.markdown.react()` — render Markdown to React JSX elements
...
```ts
const html = Fun.markdown.html("some markdown", {
...
Fun.markdown.html("Visit www.example.com", { autolinks: true });

// Enable only specific types
Fun.markdown.html("Visit www.example.com", {
...
Fun.markdown.html("## Hello World", { headings: true });
// '<h2 id="hello-world"><a href="#hello-world">Hello World</a></h2>\n'

// Enable only heading IDs (no autolink)
Fun.markdown.html("## Hello World", { headings: { ids: true } });
...
```ts
const result = Fun.markdown.render("# Hello **world**", {
...
```ts
const plaintext = Fun.markdown.render("# Hello **world**", {
...
```ts
const result = Fun.markdown.render("# Title\n\n![logo](img.png)\n\nHello", {
...
```ts
const result = Fun.markdown.render("1. first\n   1. sub-a\n   2. sub-b\n2. second", {
...
````ts
const result = Fun.markdown.render("```js\nconsole.log('hi')\n```", {
  code: (children, meta) => {
...
```ts
const result = Fun.markdown.render(
...
```tsx
function Markdown({ text }: { text: string }) {
  return Fun.markdown.react(text, undefined, { reactVersion: 18 });
}
```
...
```tsx
const el = Fun.markdown.react("## Hello World", undefined, {
  headings: { ids: true },
  autolinks: true,
});
```
