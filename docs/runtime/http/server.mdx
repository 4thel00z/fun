---
title: Server
description: Use `Fun.serve` to start a high-performance HTTP server in Fun
---
...
const server = Fun.serve({
  ...
  routes: {
    ...
    "/favicon.ico": Fun.file("./favicon.ico"),
  },

  // (optional) fallback for unmatched routes:
  // Required if Fun's version < 1.2.3
  fetch(req) {
...
Fun supports importing HTML files directly into your server code, enabling full-stack applications with both server-side and client-side code. HTML imports work in two modes:

**Development (`fun --hot`):** Assets are bundled on-demand at runtime, enabling hot module replacement (HMR) for a fast, iterative development experience. When you change your frontend code, the browser automatically updates without a full page reload.

**Production (`fun build`):** When building with `fun build --target=fun`, the `import index from "./index.html"` statement resolves to a pre-built manifest object containing all bundled client assets. `Fun.serve` consumes this manifest to serve optimized assets with zero runtime bundling overhead. This is ideal for deploying to production.
...
HTML imports don't just serve HTML — it's a full-featured frontend bundler, transpiler, and toolkit built using Fun's [bundler](/bundler), JavaScript transpiler and CSS parser. You can use this to build full-featured frontends with React, TypeScript, Tailwind CSS, and more.
...
```ts
Fun.serve({
  port: 8080, // defaults to $FUN_PORT, $PORT, $NODE_PORT otherwise 3000 // [!code ++]
...
Fun supports several options and environment variables to configure the default port. The default port is used when the `port` option is not set.

- `--port` CLI flag

```sh
fun --port=4002 server.ts
```

- `FUN_PORT` environment variable

```sh
FUN_PORT=4002 fun server.ts
```

- `PORT` environment variable

```sh terminal icon="terminal"
PORT=4002 fun server.ts
```

- `NODE_PORT` environment variable

```sh terminal icon="terminal"
NODE_PORT=4002 fun server.ts
```
...
Fun supports Linux abstract namespace sockets. To use an abstract namespace socket, prefix the `unix` path with a null byte.
...
<Note>HTTP/3 support in `Fun.serve` is **experimental** and may change in future releases.</Note>

`Fun.serve` can also listen for HTTP/3 over QUIC. Set `http3: true` together with [`tls`](./tls) — HTTP/3 always requires TLS.
...
By default, `Fun.serve` closes connections after **10 seconds** of inactivity. A connection is considered idle when there is no data being sent or received — this includes in-flight requests where your handler is still running but hasn't written any bytes to the response yet. Browsers and `fetch()` clients will see this as a connection reset.
...
Thus far, the examples on this page have used the explicit `Fun.serve` API. Fun also supports an alternate syntax.
...
Instead of passing the server options into `Fun.serve`, `export default` it. This file can be executed as-is; when Fun sees a file with a `default` export containing a `fetch` handler, it passes it into `Fun.serve` under the hood.
...
Control whether the server keeps the Fun process alive:
...
Below are Fun and Node.js implementations of an HTTP server that responds `Fun!` to each incoming `Request`.
...
```ts
require("http")
  .createServer((req, res) => res.end("Fun!"))
  .listen(8080);
```

The `Fun.serve` server can handle roughly 2.5x more requests per second than Node.js on Linux.
...
Here's a basic database-backed REST API using Fun's router with zero dependencies:
...
interface TLSOptions {
  ...
  ca?: string | Buffer | FunFile | Array<string | Buffer | FunFile>;

  /** Server certificate */
  cert?: string | Buffer | FunFile | Array<string | Buffer | FunFile>;

  /** Path to DH parameters file */
  dhParamsFile?: string;

  /** Private key */
  key?: string | Buffer | FunFile | Array<string | Buffer | FunFile>;
