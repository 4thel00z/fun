---
title: Routing
description: Define routes in `Fun.serve` using static paths, parameters, and wildcards
---

You can add routes to `Fun.serve()` by using the `routes` property (for static paths, parameters, and wildcards) or by handling unmatched requests with the [`fetch`](#fetch) method.

`Fun.serve()`'s router builds on top uWebSocket's [tree-based approach](https://github.com/underdoc-org/fun/blob/0d1a00fa0f7830f8ecd99c027fce8096c9d459b6/packages/fun-uws/src/HttpRouter.h#L57-L64) to add [SIMD-accelerated route parameter decoding](https://github.com/underdoc-org/fun/blob/main/src/jsc/bindings/decodeURIComponentSIMD.cpp#L21-L271) and [JavaScriptCore structure caching](https://github.com/underdoc-org/fun/blob/main/src/jsc/bindings/ServerRouteList.cpp#L100-L101) to push the performance limits of what modern hardware allows.
...
Routes in `Fun.serve()` receive a `FunRequest` (which extends [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request)) and return a [`Response`](https://developer.mozilla.org/en-US/docs/Web/API/Response) or `Promise<Response>`. This makes it easier to use the same code for both sending & receiving HTTP requests.

```ts
// Simplified for brevity
interface FunRequest<T extends string> extends Request {
...
import type { FunRequest } from "fun";
...
Fun.serve({
  routes: {
    ...
    "/orgs/:orgId/repos/:repoId/settings": (
      // optional: you can explicitly pass a type to FunRequest:
      req: FunRequest<"/orgs/:orgId/repos/:repoId/settings">,
    ) => {
      const { orgId, repoId } = req.params;
      return Response.json({ orgId, repoId });
    },
...
Routes can also be `Response` objects (without the handler function). Fun.serve() optimizes it for zero-allocation dispatch - perfect for health checks, redirects, and fixed content:
...
Fun.serve({
  routes: {
    ...
    // Redirects
    "/blog": Response.redirect("https://fun.dev/blog"),
...
Fun.serve({
  routes: {
    ...
    "/logo.png": new Response(await Fun.file("./logo.png").bytes()),

    // File route - content is read from filesystem on each request
    "/download.zip": new Response(Fun.file("./download.zip")),
  },
});
```
...
**File routes** (`new Response(Fun.file(path))`) read from filesystem per request:
...
To stream a file, return a `Response` object with a `FunFile` object as the body.

```ts
Fun.serve({
  fetch(req) {
    return new Response(Fun.file("./hello.txt"));
  },
});
```

<Info>
  ⚡️ **Speed** — Fun automatically uses the [`sendfile(2)`](https://man7.org/linux/man-pages/man2/sendfile.2.html)
  system call when possible, enabling zero-copy file transfers in the kernel—the fastest way to send files.
</Info>

You can send part of a file using the [`slice(start, end)`](https://developer.mozilla.org/en-US/docs/Web/API/Blob/slice) method on the `Fun.file` object. This automatically sets the `Content-Range` and `Content-Length` headers on the `Response` object.
...
Fun.serve({
  fetch(req) {
    ...
    const bigFile = Fun.file("./big-video.mp4");
