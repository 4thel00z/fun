...
`Fun.Image` is a chainable image pipeline for decoding, resizing, rotating, and re-encoding JPEG, PNG, WebP, HEIC, and AVIF — built on libjpeg-turbo, spng, libwebp, and SIMD geometry kernels, with zero npm dependencies and no native addon build step.

```ts
await Fun.file("photo.jpg").image().resize(400, 400, { fit: "inside" }).webp({ quality: 80 }).write("thumb.webp");
```
...
The constructor accepts a path, bytes, or a `Blob` — including `Fun.file()` and `Fun.s3()`. `Blob#image()` is shorthand for `new Fun.Image(blob)`:

```ts
new Fun.Image("./photo.jpg"); // file path
new Fun.Image(buffer); // Buffer / ArrayBuffer / TypedArray
new Fun.Image(Fun.file("photo.jpg")); // FunFile (read lazily, off-thread)
Fun.file("photo.jpg").image(); // same as above
Fun.s3("bucket/photo.jpg").image(); // S3File
```

The format is sniffed from the bytes — extensions and `Content-Type` are ignored.

**Path strings are filesystem paths.** Don't pass user-controlled strings directly to the constructor — that's an arbitrary-file-read primitive. Read untrusted input into a `Buffer` (e.g. via `fetch`/`Fun.file` with your own validation) and pass the bytes.
...
```ts
const { width, height, format } = await new Fun.Image(input).metadata();
...
await img.write(Fun.s3("bucket/out.webp"));
```

`.write()` accepts the same destinations as `Fun.write` — a path string, `Fun.file()`, `Fun.s3()`, or an fd. If you didn't chain a format method and the destination is a path string, the extension picks one (`.jpg`/`.png`/`.webp`/`.heic`/`.avif`).
...
## `Fun.serve` integration

A `Fun.Image` pipeline is a valid `Response` body and sets `Content-Type` automatically. To keep the encode off the JS thread in a server handler, await a terminal first:
...
Fun.serve({
  routes: {
    "/avatar/:id": async req => {
      ...
      const out = await Fun.file(`avatars/${req.params.id}.png`).image().resize(128, 128).webp().blob();
...
```ts
Fun.Image.backend = "fun"; // default is "system" on macOS/Windows
```
