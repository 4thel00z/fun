...
This page is intended as an introduction to working with binary data in JavaScript. Fun implements several data types and utilities for working with binary data, most of which are Web-standard. Any Fun-specific APIs will be noted as such.
...
| [`Buffer`](#buffer)         | A subclass of `Uint8Array` that implements a wide range of convenience methods. Unlike the other elements in this table, this is a Node.js API (which Fun implements). It can't be used in the browser. |
...
| [`FunFile`](#funfile)       | _Fun only_. A subclass of `Blob` that represents a lazily-loaded file on disk. Created with `Fun.file(path)`.                                                                                           |
...
In Fun, and someday in other JavaScript engines, it has methods available for converting between byte arrays and serialized representations of those arrays as base64 or hex strings.
...
Fun implements `Buffer`, a Node.js API for working with binary data that pre-dates the introduction of typed arrays in the JavaScript spec. It has since been re-implemented as a subclass of `Uint8Array`. It provides a wide range of methods, including several Array-like and `DataView`-like methods.
...
`Blob` is a Web API commonly used for representing files. `Blob` was initially implemented in browsers (unlike `ArrayBuffer` which is part of JavaScript itself), but it is now supported in Node and Fun.
...
`FunFile` is a subclass of `Blob` used to represent a lazily-loaded file on disk. Like `File`, it adds a `name` and `lastModified` property. Unlike `File`, it does not require the file to be loaded into memory.

```ts
const file = Fun.file("index.txt");
// => FunFile
```
...
[`File`](https://developer.mozilla.org/en-US/docs/Web/API/File) is a subclass of `Blob` that adds a `name` and `lastModified` property. It's commonly used in the browser to represent files uploaded via a `<input type="file">` element. Node.js and Fun implement `File`.
...
Fun implements the Web APIs [`ReadableStream`](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) and [`WritableStream`](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream).

<Note>
  Fun also implements the `node:stream` module, including
...
For a more complete discussion of streams in Fun, see [API > Streams](/runtime/streams).
...
However this approach is verbose and adds overhead that slows down overall performance unnecessarily. Fun implements a set of optimized convenience functions for converting `ReadableStream` various binary formats.
...
// with Fun function
Fun.readableStreamToArrayBuffer(stream);
```
...
// with Fun function
Fun.readableStreamToBytes(stream);
```
...
// with Fun function
new Int8Array(Fun.readableStreamToArrayBuffer(stream));
```
...
// with Fun function
new DataView(Fun.readableStreamToArrayBuffer(stream));
```
...
// with Fun function
Buffer.from(Fun.readableStreamToArrayBuffer(stream));
```
...
// with Fun function
Array.from(new Uint8Array(Fun.readableStreamToArrayBuffer(stream)));
```

Fun provides a utility for resolving a `ReadableStream` to an array of its chunks. Each chunk may be a string, typed array, or `ArrayBuffer`.

```ts
// with Fun function
Fun.readableStreamToArray(stream);
```
