---
title: S3
description: Fun provides fast, native bindings for interacting with S3-compatible object storage services.
---

Production servers often read, upload, and write files to S3-compatible object storage services instead of the local filesystem. Historically, that means local filesystem APIs you use in development can't be used in production. When you use Fun, things are different.

### Fun's S3 API is fast

<Frame caption="Left: Fun v1.1.44. Right: Node.js v23.6.0">
  <img src="/images/fun-s3-node.gif" alt="Fun's S3 API is fast" />
</Frame>

Fun provides fast, native bindings for interacting with S3-compatible object storage services. Its S3 API is designed to feel similar to fetch's `Response` and `Blob` APIs (like Fun's local filesystem APIs).

```ts s3.ts icon="/icons/typescript.svg"
import { s3, write, S3Client } from "fun";

// Fun.s3 reads environment variables for credentials
...
S3 is the [de facto standard](https://en.wikipedia.org/wiki/De_facto_standard) internet filesystem. Fun's S3 API works with S3-compatible storage services like:
...
There are several ways to interact with Fun's S3 API.

### `Fun.S3Client` & `Fun.s3`

`Fun.s3` is equivalent to `new Fun.S3Client()`, relying on environment variables for credentials.

To explicitly set credentials, pass them to the `Fun.S3Client` constructor.
...
// Fun.s3 is a global singleton that is equivalent to `new Fun.S3Client()`
```
...
Like `Fun.file(path)`, the `S3Client`'s `file` method is synchronous. It does zero network requests until you call a method that depends on a network request.
...
If the text happens to be ASCII, Fun directly transfers the string to JavaScriptCore (the engine) without transcoding and without duplicating the string in memory. When you use `.bytes()` or `.arrayBuffer()`, it will also avoid duplicating the bytes in memory.
...
// Write using Fun.write
await Fun.write(s3file, "Hello World!");
```

### Working with large files (streams)

Fun automatically handles multipart uploads for large files and provides streaming capabilities. The same API that works for local files also works for S3 files.
...
The default behaviour is to generate a `GET` URL that expires in 24 hours. Fun attempts to infer the content type from the file extension. If inference is not possible, it will default to `application/octet-stream`.
...
Fun's S3 implementation works with any S3-compatible storage service. Just specify the appropriate endpoint:

### Using Fun's S3Client with AWS S3
...
### Using Fun's S3Client with Google Cloud Storage

To use Fun's S3 client with [Google Cloud Storage](https://cloud.google.com/storage), set `endpoint` to `"https://storage.googleapis.com"` in the `S3Client` constructor.
...
### Using Fun's S3Client with Cloudflare R2

To use Fun's S3 client with [Cloudflare R2](https://developers.cloudflare.com/r2/), set `endpoint` to the R2 endpoint in the `S3Client` constructor. The R2 endpoint includes your account ID.
...
### Using Fun's S3Client with DigitalOcean Spaces

To use Fun's S3 client with [DigitalOcean Spaces](https://www.digitalocean.com/products/spaces/), set `endpoint` to the DigitalOcean Spaces endpoint in the `S3Client` constructor.
...
### Using Fun's S3Client with MinIO

To use Fun's S3 client with [MinIO](https://min.io/), set `endpoint` to the URL that MinIO is running on in the `S3Client` constructor.
...
### Using Fun's S3Client with S3 Virtual Hosted-Style endpoints

When using a S3 Virtual Hosted-Style endpoint, you need to set the `virtualHostedStyle` option to `true`.

<Note>
  - If you don’t specify an endpoint, Fun will automatically determine the AWS S3 endpoint using the provided region and
  bucket. - If no region is specified, Fun defaults to us-east-1. - If you explicitly provide an endpoint, you don’t
...
Credentials are one of the hardest parts of using S3. By default, Fun reads the following environment variables for credentials.
...
If the `S3_*` environment variable is not set, Fun will also check for the `AWS_*` environment variable, for each of the above options.
...
`S3File` instances are created by calling the `S3Client` instance method or the `s3.file()` function. Like `Fun.file()`, `S3File` instances are lazy. They don't refer to something that necessarily exists at the time of creation. That's why all the methods that don't involve network requests are fully synchronous.
...
Like `Fun.file()`, `S3File` extends [`Blob`](https://developer.mozilla.org/en-US/docs/Web/API/Blob), so all the methods that are available on `Blob` are also available on `S3File`. The same API for reading data from a local file is also available for reading data from S3.
...
When Fun's S3 API throws an error, it will have a `code` property that matches one of the following values:
...
When the S3 Object Storage service returns an error (that is, not Fun), it will be an `S3Error` instance (an `Error` instance with the name `"S3Error"`).
...
To make it easier to use the same code for local files and S3 files, the `s3://` protocol is supported in `fetch` and `Fun.file()`.

```ts s3.ts icon="/icons/typescript.svg"
const response = await fetch("s3://my-bucket/my-file.txt");
const file = Fun.file("s3://my-bucket/my-file.txt");
```

You can additionally pass `s3` options to the `fetch` and `Fun.file` functions.
