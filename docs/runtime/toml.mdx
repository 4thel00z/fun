---
title: TOML
description: Use Fun's built-in support for TOML files through both runtime APIs and bundler integration
---

In Fun, TOML is a first-class citizen alongside JSON, JSON5, and YAML. You can:

- Parse TOML strings with `Fun.TOML.parse`
- `import` & `require` TOML files as modules at runtime (including hot reloading & watch mode support)
- `import` & `require` TOML files in frontend apps via Fun's bundler
...
Fun's TOML parser supports the [TOML v1.0 specification](https://toml.io/en/v1.0.0), including:
...
`Fun.TOML.parse()` throws if the TOML is invalid:

```ts
try {
  Fun.TOML.parse("invalid = = =");
} catch (error) {
...
When you run your application with `fun --hot`, changes to TOML files are automatically detected and reloaded without restarting:
...
When you import TOML files and bundle with Fun, the TOML is parsed at build time and included as a JavaScript module:
