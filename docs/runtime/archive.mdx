---
title: Archive
description: Create and extract tar archives with Fun's fast native implementation
---

Fun provides a fast, native implementation for working with tar archives through `Fun.Archive`. It supports creating archives from in-memory data, extracting archives to disk, and reading archive contents without extraction.
...
// Write to disk
await Fun.write("bundle.tar", archive);
```

**Extract an archive:**

```ts
const tarball = await Fun.file("package.tar.gz").bytes();
...
Use `new Fun.Archive()` to create an archive from an object where keys are file paths and values are file contents. By default, archives are uncompressed:
...
Use `Fun.write()` to write an archive to disk:
...
await Fun.write("output.tar", archive);

// Write gzipped tar
const compressed = new Fun.Archive({ "src/index.ts": "console.log('Hello');" }, { compress: "gzip" });
await Fun.write("output.tar.gz", compressed);
```
...
```ts
const archive = new Fun.Archive({ "hello.txt": "Hello, World!" });
...
const gzipped = new Fun.Archive({ "hello.txt": "Hello, World!" }, { compress: "gzip" });
...
// From a file
const tarball = await Fun.file("package.tar.gz").bytes();
const archiveFromFile = new Fun.Archive(tarball);
```

```ts
// From a fetch response
const response = await fetch("https://example.com/archive.tar.gz");
const archiveFromFetch = new Fun.Archive(await response.blob());
```
...
**Note**: On Windows, symbolic links in archives are always skipped during extraction. Fun does not attempt to create them regardless of privilege level. On Linux and macOS, symlinks are extracted normally.

**Security note**: Fun.Archive validates paths during extraction, rejecting absolute paths (POSIX `/`, Windows drive letters like `C:\` or `C:/`, and UNC paths like `\\server\share`). Path traversal components (`..`) are normalized away (e.g., `dir/sub/../file` becomes `dir/file`) to prevent directory escape attacks.
...
**Security note**: Fun.Archive automatically validates paths during extraction. Absolute paths (POSIX `/`, Windows drive letters, UNC paths) and unsafe symlink targets are rejected. Path traversal components (`..`) are normalized away to prevent directory escape.

For additional security with untrusted archives, you can enumerate and validate paths before extraction:

```ts
const archive = new Fun.Archive(untrustedData);
const files = await archive.files();
...
Supported glob patterns (subset of [Fun.Glob](/docs/api/glob) syntax):
...
See [Fun.Glob](/docs/api/glob) for the full glob syntax including escaping and advanced patterns.

## Compression

Fun.Archive creates uncompressed tar archives by default. Use `{ compress: "gzip" }` to enable gzip compression:
...
const gzippedTarball = await Fun.file("archive.tar.gz").bytes();
const readArchive = new Fun.Archive(gzippedTarball);

// Enable gzip compression
const compressed = new Fun.Archive({ "hello.txt": "Hello, World!" }, { compress: "gzip" });

// Gzip with custom level (1-12)
const maxCompression = new Fun.Archive({ "hello.txt": "Hello, World!" }, { compress: "gzip", level: 12 });
```
...
for await (const path of glob.scan(".")) {
  ...
  files[archivePath] = await Fun.file(path).text();
}

// Add package.json
files["package.json"] = await Fun.file("package.json").text();

// Create compressed archive and write to disk
const archive = new Fun.Archive(files, { compress: "gzip" });
await Fun.write("bundle.tar.gz", archive);
```
...
async function archiveDirectory(dir: string, compress = false): Promise<Fun.Archive> {
  ...
  async function walk(currentDir: string, prefix: string = "") {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        files[archivePath] = Fun.file(fullPath);
      }
    }
  }

  await walk(dir);
  return new Fun.Archive(files, compress ? { compress: "gzip" } : undefined);
}

const archive = await archiveDirectory("./my-project", true);
await Fun.write("my-project.tar.gz", archive);
```

## Reference

> **Note**: The following type signatures are simplified for documentation purposes. See [`packages/fun-types/fun.d.ts`](https://github.com/underdoc-org/fun/blob/main/packages/fun-types/fun.d.ts) for the full type definitions.

```ts
type ArchiveInput =
  | Record<string, string | Blob | Fun.ArrayBufferView | ArrayBufferLike>
  | Blob
  | Fun.ArrayBufferView
  | ArrayBufferLike;
