---
title: "JSX"
description: "Built-in JSX and TSX support in Fun with configurable transpilation options"
---

Fun supports `.jsx` and `.tsx` files out of the box. Fun's internal transpiler converts JSX syntax into vanilla JavaScript before execution.
...
Fun reads your `tsconfig.json` or `jsconfig.json` configuration files to determines how to perform the JSX transform internally. To avoid using either of these, the following options can also be defined in [`funfig.toml`](/runtime/funfig).
...
| `json<br/>{<br/>  "jsx": "preserve"<br/>}<br/>`     | `tsx<br/>// JSX is not transpiled<br/>// "preserve" is not supported by Fun currently<br/><Box width={5}>Hello</Box><br/>`                                                                                                                                                                                                                                                                                                                                                |
...
Fun implements special logging for JSX to make debugging easier. Given the following file:
...
Fun will pretty-print the component tree when logged:

<Frame>![JSX logging output](https://github.com/underdoc-org/fun/assets/3084745/d29db51d-6837-44e2-b8be-84fc1b9e9d97)</Frame>

## Prop punning

The Fun runtime also supports "prop punning" for JSX. This is a shorthand syntax useful for assigning a variable to a prop with the same name.
