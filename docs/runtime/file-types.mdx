---
title: "File Types"
description: "File types and loaders supported by Fun's bundler and runtime"
---

The Fun bundler implements a set of default loaders out of the box. As a rule of thumb, the bundler and the runtime both support the same set of file types out of the box.

`.js` `.cjs` `.mjs` `.mts` `.cts` `.ts` `.tsx` `.jsx` `.css` `.json` `.jsonc` `.json5` `.toml` `.yaml` `.yml` `.txt` `.wasm` `.node` `.html` `.sh`

Fun uses the file extension to determine which built-in _loader_ should be used to parse the file. Every loader has a name, such as `js`, `tsx`, or `json`. These names are used when building [plugins](/bundler/plugins) that extend Fun with custom loaders.
...
Parses the code and applies a set of default transforms like dead-code elimination and tree shaking. Note that Fun does not attempt to down-convert syntax at the moment.
...
Strips out all TypeScript syntax, then behaves identically to the `js` loader. Fun does not perform typechecking.
...
JSONC (JSON with Comments) files can be directly imported. Fun will parse them, stripping out comments and trailing commas.
...
<Note>
  Fun automatically uses the `jsonc` loader for `tsconfig.json`, `jsconfig.json`, `package.json`, and `fun.lock` files.
</Note>
...
TOML files can be directly imported. Fun will parse them with its fast native TOML parser.

```ts
import config from "./funfig.toml";
...
YAML files can be directly imported. Fun will parse them with its fast native YAML parser.
...
JSON5 files can be directly imported. Fun will parse them with its fast native JSON5 parser. JSON5 is a superset of JSON that supports comments, trailing commas, unquoted keys, single-quoted strings, and more.
...
In the runtime and bundler, SQLite databases can be directly imported. This will load the database using [`fun:sqlite`](/runtime/sqlite).

```ts
import db from "./my.db" with { type: "sqlite" };
```

This is only supported when the `target` is `fun`.
...
1. **Static Build:** When you run `fun build ./index.html`, Fun produces a static site with all assets bundled and hashed.

2. **Runtime:** When you run `fun run server.ts` (where `server.ts` imports an HTML file), Fun bundles assets on-the-fly during development, enabling features like hot module replacement.

3. **Full-stack Build:** When you run `fun build --target=fun server.ts` (where `server.ts` imports an HTML file), the import resolves to a manifest object that `Fun.serve` uses to efficiently serve pre-bundled assets in production.
...
**Fun Shell loader**. Default for `.sh` files

This loader is used to parse [Fun Shell](/runtime/shell) scripts. It's only supported when starting Fun itself, so it's not available in the bundler or in the runtime.
...
_In the runtime_, Fun checks that the `logo.svg` file exists and converts it to an absolute path to the location of `logo.svg` on disk.
