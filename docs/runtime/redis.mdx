---
title: Redis
description: Use Fun's native Redis client with a Promise-based API
---

<Note>Fun's Redis client supports Redis server versions 7.2 and up.</Note>

Fun provides native bindings for working with Redis databases with a modern, Promise-based API. The interface is designed to be performant, with built-in connection management, fully typed responses, and TLS support.

```ts redis.ts icon="/icons/typescript.svg"
import { redis } from "fun";

// Set a key
await redis.set("greeting", "Hello from Fun!");

// Get a key
const greeting = await redis.get("greeting");
console.log(greeting); // "Hello from Fun!"
...
```ts redis.ts icon="/icons/typescript.svg"
import { redis, RedisClient } from "fun";
...
Fun provides native bindings for the [Redis
Pub/Sub](https://redis.io/docs/latest/develop/pubsub/) protocol. **New in Fun
1.2.23**
...
fun run subscriber.ts
```

and, in another, run your publisher:

```bash terminal icon="terminal"
fun run publisher.ts
```
...
The Fun `RedisClient` allows you to subscribe to channels through the
`.subscribe()` method:
...
Fun's Redis client is implemented in Zig and uses the Redis Serialization Protocol (RESP3). It manages connections efficiently and provides automatic reconnection with exponential backoff.
