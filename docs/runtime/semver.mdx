---
title: Semver
description: Use Fun's semantic versioning API
---

Fun implements a semantic versioning API which can be used to compare versions and determine if a version is compatible with another range of versions. The versions and ranges are designed to be compatible with `node-semver`, which is used by npm clients.

It's about 20x faster than `node-semver`.

<Frame>![Benchmark](https://github.com/underdoc-org/fun/assets/709451/94746adc-8aba-4baf-a143-3c355f8e0f78)</Frame>

Currently, this API provides two functions:

## `Fun.semver.satisfies(version: string, range: string): boolean`
...
## `Fun.semver.order(versionA: string, versionB: string): 0 | 1 | -1`
