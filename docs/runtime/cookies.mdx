---
title: Cookies
description: Use Fun's native APIs for working with HTTP cookies
---

Fun provides native APIs for working with HTTP cookies through `Fun.Cookie` and `Fun.CookieMap`. These APIs offer fast methods for parsing, generating, and manipulating cookies in HTTP requests and responses.

## CookieMap class

`Fun.CookieMap` provides a Map-like interface for working with collections of cookies. It implements the `Iterable` interface, allowing you to use it with `for...of` loops and other iteration methods.

```ts title="cookies.ts" icon="/icons/typescript.svg"
// Empty cookie map
const cookies = new Fun.CookieMap();

// From a cookie string
const cookies1 = new Fun.CookieMap("name=value; foo=bar");

// From an object
const cookies2 = new Fun.CookieMap({
  session: "abc123",
  theme: "dark",
});

// From an array of name/value pairs
const cookies3 = new Fun.CookieMap([
  ["session", "abc123"],
  ["theme", "dark"],
]);
```
...
In Fun's HTTP server, the `cookies` property on the request object (in `routes`) is an instance of `CookieMap`:
...
const cookie = new Fun.Cookie("visited", "true");
cookies.set(cookie);
```
...
When using `Fun.serve()`, you don't need to call this method explicitly. Any changes made to the `req.cookies` map are automatically applied to the response headers. This method is primarily useful when working with other HTTP server implementations.

```js title="node-server.js" icon="file-code"
import { createServer } from "node:http";
import { CookieMap } from "fun";
...
`Fun.Cookie` represents an HTTP cookie with its name, value, and attributes.

```ts title="cookie-class.ts" icon="/icons/typescript.svg"
import { Cookie } from "fun";

// Create a basic cookie
const cookie = new Fun.Cookie("name", "value");

// Create a cookie with options
const secureSessionCookie = new Fun.Cookie("session", "abc123", {
  domain: "example.com",
  path: "/admin",
...
const parsedCookie = new Fun.Cookie("name=value; Path=/; HttpOnly");

// Create from an options object
const objCookie = new Fun.Cookie({
  name: "theme",
  value: "dark",
  maxAge: 3600,
  secure: true,
...
new Fun.Cookie(name: string, value: string);

// Constructor with name, value, and options
new Fun.Cookie(name: string, value: string, options: CookieInit);

// Constructor from cookie string
new Fun.Cookie(cookieString: string);

// Constructor from cookie object
new Fun.Cookie(options: CookieInit);
```
...
const cookie = new Fun.Cookie("session", "abc123", {
  domain: "example.com",
  path: "/admin",
