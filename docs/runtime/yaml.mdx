---
title: YAML
description: Use Fun's built-in support for YAML files through both runtime APIs and bundler integration
---

In Fun, YAML is a first-class citizen alongside JSON and TOML. You can:

- Parse YAML strings with `Fun.YAML.parse`
- `import` & `require` YAML files as modules at runtime (including hot reloading & watch mode support)
- `import` & `require` YAML files in frontend apps via fun's bundler

---

## Conformance

Fun's YAML parser currently passes over 90% of the official YAML test suite. While we're actively working on reaching 100% conformance, the current implementation covers the vast majority of real-world use cases. The parser is written in Zig for optimal performance and is continuously being improved.
...
When parsing YAML with multiple documents (separated by `---`), `Fun.YAML.parse()` returns an array:
...
const docs = Fun.YAML.parse(multiDoc);
console.log(docs);
// [
//   { name: "Document 1" },
//   { name: "Document 2" },
...
Fun's YAML parser supports the full YAML 1.2 specification, including:
...
const data = Fun.YAML.parse(yaml);
```

#### Error Handling

`Fun.YAML.parse()` throws a `SyntaxError` if the YAML is invalid:

```ts
try {
  Fun.YAML.parse("invalid: yaml: content:");
} catch (error) {
...
One of the most powerful features of Fun's YAML support is hot reloading. When you run your application with `fun --hot`, changes to YAML files are automatically detected and reloaded without closing connections
...
When you import YAML files in your application and bundle it with Fun, the YAML is parsed at build time and included as a JavaScript module:

```bash terminal icon="terminal"
fun build app.ts --outdir=dist
```
