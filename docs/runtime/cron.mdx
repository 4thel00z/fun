---
title: Cron
description: Schedule and parse cron jobs with Fun
---

Fun has built-in support for cron — parse expressions, run a callback on a schedule inside your process, or register OS-level jobs that survive restarts.
...
```ts
await Fun.cron("./worker.ts", "30 2 * * MON", "weekly-report");
```
...
for (let i = 0; i < 3; i++) {
  cursor = Fun.cron.parse("0 * * * *", cursor)!;
...
Fun.cron.parse("0 9 * * MON-FRI"); // weekdays
Fun.cron.parse("0 0 1 JAN,JUN *"); // January and June

// Full names
Fun.cron.parse("0 9 * * Monday-Friday");
Fun.cron.parse("0 0 1 January *");
```
...
```ts
const next = Fun.cron.parse("@daily");
console.log(next); // => next UTC midnight
```

### Time zone

`Fun.cron.parse()` and the in-process `Fun.cron(schedule, handler)` interpret schedules in **UTC**. There is no DST to handle — `0 9 * * *` always means 9:00 UTC.

The OS-level `Fun.cron(path, schedule, title)` uses the **system's local time zone**, because that's how crontab, launchd, and Windows Task Scheduler work. To make the two forms agree, run the process with `TZ=UTC`.
...
## `Fun.cron(schedule, handler)` — in-process

Run a callback on a cron schedule inside the current process.

```ts
const job = Fun.cron("*/5 * * * *", async () => {
...
|                              | In-process                       | [OS-level](#fun-cron-path-schedule-title-os-level) |
| ---------------------------- | -------------------------------- | -------------------------------------------------- |
...
Under `fun --hot`, all in-process cron jobs are stopped immediately before the module graph re-evaluates. Every `Fun.cron()` call still in your source then re-registers. Editing the schedule, editing the handler, or deleting the line entirely all take effect on save without leaking timers.
...
`CronJob` is [`Disposable`](https://github.com/tc39/proposal-explicit-resource-management) — `using job = Fun.cron(...)` auto-stops at scope exit. `stop()`, `ref()`, and `unref()` all return the job for chaining.
...
## `Fun.cron(path, schedule, title)` — OS-level
...
```ts
await Fun.cron("./worker.ts", "0 * * * *", "my-job"); // every hour
await Fun.cron("./worker.ts", "*/15 * * * *", "my-job"); // replaces: every 15 min
```
...
```ts worker.ts
export default {
  scheduled(controller: Fun.CronController) {
...
The handler can be `async`. Fun waits for the returned promise to settle before exiting.
...
Fun uses [crontab](https://man7.org/linux/man-pages/man5/crontab.5.html) to register jobs. Each job is stored as a line in your user's crontab with a `# fun-cron: <title>` marker comment above it.

The crontab entry looks like:

```
<schedule> '<fun-path>' run --cron-title=<title> --cron-period='<schedule>' '<script-path>'
```

When the cron daemon fires the job, Fun imports your module and calls the `scheduled()` handler.
...
```sh
# Edit your crontab and remove the "# fun-cron: <title>" comment
# and the command line below it
crontab -e

# Or remove ALL fun cron jobs at once by filtering them out:
crontab -l | grep -v "# fun-cron:" | grep -v "\-\-cron-title=" | crontab -
```

### macOS

Fun uses [launchd](https://developer.apple.com/library/archive/documentation/MacOSX/Conceptual/BPSystemStartup/Chapters/CreatingLaunchdJobs.html) to register jobs. Each job is installed as a plist file at:

```
~/Library/LaunchAgents/fun.cron.<title>.plist
```

The plist uses `StartCalendarInterval` to define the schedule. Complex patterns with ranges, lists, or steps are supported — Fun expands them into multiple `StartCalendarInterval` dicts via Cartesian product.

**Viewing registered jobs:**

```sh
launchctl list | grep fun.cron
```

**Logs:** stdout and stderr are written to:

```
/tmp/fun.cron.<title>.stdout.log
/tmp/fun.cron.<title>.stderr.log
```

For example, a job titled `weekly-report`:

```sh
cat /tmp/fun.cron.weekly-report.stdout.log
tail -f /tmp/fun.cron.weekly-report.stderr.log
```
...
launchctl bootout gui/$(id -u)/fun.cron.<title>

# Delete the plist file
rm ~/Library/LaunchAgents/fun.cron.<title>.plist

# Example for a job titled "weekly-report":
launchctl bootout gui/$(id -u)/fun.cron.weekly-report
rm ~/Library/LaunchAgents/fun.cron.weekly-report.plist
```

### Windows

Fun uses [Windows Task Scheduler](https://learn.microsoft.com/en-us/windows/win32/taskschd/task-scheduler-start-page) with XML-based task definitions. Each job is registered as a scheduled task named `fun-cron-<title>` using [`CalendarTrigger`](https://learn.microsoft.com/en-us/windows/win32/taskschd/taskschedulerschema-calendartrigger-triggergroup-element) elements and [`Repetition`](https://learn.microsoft.com/en-us/windows/win32/taskschd/taskschedulerschema-repetition-triggerbasetype-element) patterns.
...
On headless servers and CI environments where the current user's [Security Identifier (SID)](https://learn.microsoft.com/en-us/windows/security/identity-protection/access-control/security-identifiers) cannot be resolved — such as service accounts created by [NSSM](https://nssm.cc/) or similar tools — `Fun.cron()` will fail with an error explaining the issue. To work around this, either run Fun as a regular user account, or create the scheduled task manually with `schtasks /create /xml <file> /tn <name> /ru SYSTEM /f`.
...
<Warning>
  ...
  `Fun.cron()` rejects it with an error message.
</Warning>
...
await Fun.cron("./job.ts", "*/7 * * * *", "my-job");

// ✅ Works: restrict to specific hours (9 values × 5 hours = 45 triggers)
await Fun.cron("./job.ts", "*/7 9-13 * * *", "my-job");

// ✅ Works: use a divisor of 60 instead (Repetition, 1 trigger)
await Fun.cron("./job.ts", "*/5 * * * *", "my-job");
```

#### Windows containers

<Warning>
  `Fun.cron()` is not supported in Windows Docker containers. The Task Scheduler service is not running in `servercore`
...
```powershell
schtasks /query /tn "fun-cron-<title>"

# List all fun cron tasks
schtasks /query | findstr "fun-cron-"
```

**Manually uninstalling without code:**

```powershell
schtasks /delete /tn "fun-cron-<title>" /f

# Example:
schtasks /delete /tn "fun-cron-weekly-report" /f
```

Or open **Task Scheduler** (taskschd.msc), find the task named `fun-cron-<title>`, right-click, and delete it.
...
This reverses what `Fun.cron()` did:
