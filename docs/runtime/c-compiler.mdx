...
`fun:ffi` has experimental support for compiling and running C from JavaScript with low overhead.

---

## Usage (cc in `fun:ffi`)

See the [introduction blog post](https://fun.dev/blog/compile-and-run-c-in-js) for more information.
...
```ts
type Source = string | URL | FunFile;
