---
title: JSONL
description: Parse newline-delimited JSON (JSONL) with Fun's built-in streaming parser
---

Fun has built-in support for parsing [JSONL](https://jsonlines.org/) (newline-delimited JSON), where each line is a separate JSON value. The parser is implemented in C++ using JavaScriptCore's optimized JSON parser and supports streaming use cases.

```ts
const results = Fun.JSONL.parse('{"name":"Alice"}\n{"name":"Bob"}\n');
// [{ name: "Alice" }, { name: "Bob" }]
```
...
const results = Fun.JSONL.parse(buffer);
// [{ a: 1 }, { b: 2 }]
```
...
If the input contains invalid JSON, `Fun.JSONL.parse()` throws a `SyntaxError`:

```ts
try {
  Fun.JSONL.parse('{"valid":true}\n{invalid}\n');
} catch (error) {
...
const result = Fun.JSONL.parseChunk(chunk);
...
async function processStream(stream: ReadableStream<string>) {
  ...
  if (buffer.length > 0) {
    const final = Fun.JSONL.parseChunk(buffer);
...
const partial = Fun.JSONL.parseChunk(buf, 0, 8);
...
```ts
const input = '42\n"hello"\ntrue\nnull\n[1,2,3]\n{"key":"value"}\n';
const values = Fun.JSONL.parse(input);
