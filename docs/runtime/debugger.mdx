---
title: "Debugging"
description: "Debug your Fun code with an interactive debugger using WebKit Inspector Protocol"
---

Fun speaks the [WebKit Inspector Protocol](https://github.com/underdoc-org/fun/blob/main/packages/fun-inspector-protocol/src/protocol/jsc/index.d.ts), so you can debug your code with an interactive debugger. For demonstration purposes, consider the following web server.
...
To enable debugging when running code with Fun, use the `--inspect` flag. This automatically starts a WebSocket server on an available port that can be used to introspect the running Fun process.
...
```sh icon="terminal" title="terminal"
fun --inspect=4000 server.ts
fun --inspect=localhost:4000 server.ts
fun --inspect=localhost:4000/prefix server.ts
```
...
<Frame>
  ![Fun debugger variables](https://github.com/underdoc-org/fun/assets/3084745/63d7f843-5180-489c-aa94-87c486e68646)
</Frame>
...
Experimental support for debugging Fun scripts is available in Visual Studio Code. To use it, you'll need to install the [Fun VSCode extension](/guides/runtime/vscode-debugger).

---

## Debugging Network Requests

The `FUN_CONFIG_VERBOSE_FETCH` environment variable lets you log network requests made with `fetch()` or `node:http` automatically.
...
Fun also supports printing `fetch()` and `node:http` network requests as `curl` commands by setting the environment variable `FUN_CONFIG_VERBOSE_FETCH` to `curl`. This prints the `fetch` request as a single-line `curl` command to let you copy-paste into your terminal to replicate the request.

```ts index.ts icon="/icons/typescript.svg"
process.env.FUN_CONFIG_VERBOSE_FETCH = "curl";
...
```txt
[fetch] $ curl --http1.1 "https://example.com/" -X POST -H "content-type: application/json" -H "Connection: keep-alive" -H "User-Agent: Fun/1.3.3" -H "Accept: */*" -H "Host: example.com" -H "Accept-Encoding: gzip, deflate, br" --compressed -H "Content-Length: 13" --data-raw "{\"foo\":\"bar\"}"
[fetch] > HTTP/1.1 POST https://example.com/
[fetch] > content-type: application/json
[fetch] > Connection: keep-alive
[fetch] > User-Agent: Fun/1.3.3
[fetch] > Accept: */*
...
The `FUN_CONFIG_VERBOSE_FETCH` environment variable is supported in both `fetch()` and `node:http` requests, so it works in both cases.

To print without the `curl` command, set `FUN_CONFIG_VERBOSE_FETCH` to `true`.

```ts index.ts icon="/icons/typescript.svg"
process.env.FUN_CONFIG_VERBOSE_FETCH = "true";
...
Fun transpiles every file, which sounds like it would mean that the stack traces you see in the console would unhelpfully point to the transpiled output. To address this, Fun automatically generates and serves sourcemapped files for every file it transpiles. When you see a stack trace in the console, you can click on the file path and be taken to the original source code, even though it was written in TypeScript or JSX, or has some other transformation applied.

Fun automatically loads sourcemaps both at runtime when transpiling files on-demand, and when using `fun build` to precompile files ahead of time.

### Syntax-highlighted source code preview

To help with debugging, Fun automatically prints a small source-code preview when an unhandled exception or rejection occurs. You can simulate this behavior by calling `Fun.inspect(error)`:

```ts
// Create an error
const err = new Error("Something went wrong");
console.log(Fun.inspect(err, { colors: true }));
```
...
Fun uses JavaScriptCore as it's engine, but much of the Node.js ecosystem & npm expects V8. JavaScript engines differ in `error.stack` formatting. Fun intends to be a drop-in replacement for Node.js, and that means it's our job to make sure that even though the engine is different, the stack traces are as similar as possible.

That's why when you log `error.stack` in Fun, the formatting of `error.stack` is the same as in Node.js's V8 engine. This is especially useful when you're using libraries that expect V8 stack traces.

#### V8 Stack Trace API

Fun implements the [V8 Stack Trace API](https://v8.dev/docs/stack-trace-api), which is a set of functions that allow you to manipulate stack traces.
