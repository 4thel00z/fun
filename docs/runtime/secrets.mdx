---
title: Secrets
description: Use Fun's Secrets API to store and retrieve sensitive credentials securely
---
...
`Fun.secrets` provides a cross-platform API for managing sensitive credentials that CLI tools and development applications typically store in plaintext files like `~/.npmrc`, `~/.aws/credentials`, or `.env` files. It uses:

- **macOS**: Keychain Services
- **Linux**: libsecret (GNOME Keyring, KWallet, etc.)
- **Windows**: Windows Credential Manager

All operations are asynchronous and non-blocking, running on Fun's threadpool.
...
const password = await Fun.secrets.get("my-app", "alice@example.com");
```
...
// Or if you prefer without an object
await Fun.secrets.set("my-app.com", "github-token", "ghp_xxxxxxxxxxxxxxxxxxxx");
...
const token = await Fun.secrets.get({
  service: "gh-cli",
  name: "github.com",
});
...
4. **Memory Safety**: Fun zeros out password memory after use
...
Unlike environment variables, `Fun.secrets`:
