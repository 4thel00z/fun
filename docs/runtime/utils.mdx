---
title: Utils
description: Use Fun's utility functions to work with the runtime
---

## `Fun.version`

A `string` containing the version of the `fun` CLI that is currently running.
...
The git commit of [Fun](https://github.com/underdoc-org/fun) that was compiled to create the current `fun` CLI.
...
An absolute path to the entrypoint of the current program (the file that was executed with `fun run`).
...
console.log("hello");
await Fun.sleep(oneSecondInFuture);
console.log("hello one second later!");
```

## `Fun.sleepSync()`

`Fun.sleepSync(ms: number)`

A blocking synchronous version of `Fun.sleep`.

```ts
console.log("hello");
Fun.sleepSync(1000); // blocks thread for one second
console.log("hello one second later!");
```

## `Fun.which()`

`Fun.which(bin: string)`

Returns the path to an executable, similar to typing `which` in your terminal.

```ts
const ls = Fun.which("ls");
console.log(ls); // "/usr/bin/ls"
```

By default Fun looks at the current `PATH` environment variable to determine the path. To configure `PATH`:
...
`Fun.randomUUIDv7()` returns a [UUID v7](https://www.ietf.org/archive/id/draft-peabody-dispatch-new-uuid-format-01.html#name-uuidv7-layout-and-bit-order), which is monotonic and suitable for sorting and databases.
...
```ts buffer.ts
const buffer = Fun.randomUUIDv7("buffer");
```

`base64` and `base64url` encodings are also supported when you want a slightly shorter string.

```ts base64.ts
const base64 = Fun.randomUUIDv7("base64");
const base64url = Fun.randomUUIDv7("base64url");
```

## `Fun.peek()`

`Fun.peek(prom: Promise)`
...
Opens a file in your default editor. Fun auto-detects your editor via the `$VISUAL` or `$EDITOR` environment variables.

```ts
const currentFile = import.meta.url;
Fun.openInEditor(currentFile);
```

You can override this via the `debug.editor` setting in your [`funfig.toml`](/runtime/funfig).
...
```ts
Fun.openInEditor(import.meta.url, {
...
Recursively checks if two objects are equivalent. `expect().toEqual()` in `fun:test` uses this internally.

```ts
const foo = { a: 1, b: 2, c: { d: 3 } };

// true
Fun.deepEquals(foo, { a: 1, b: 2, c: { d: 3 } });

// false
Fun.deepEquals(foo, { a: 1, b: 2, c: { d: 4 } });
```
...
Fun.deepEquals(a, b); // => true
Fun.deepEquals(a, b, true); // => false
```
...
```ts
// undefined values
Fun.deepEquals({}, { a: undefined }, true); // false

// undefined in arrays
Fun.deepEquals(["asdf"], ["asdf", undefined], true); // false

// sparse arrays
Fun.deepEquals([, 1], [undefined, 1], true); // false

// object literals vs instances w/ same properties
class Foo {
  a = 1;
}
Fun.deepEquals(new Foo(), { a: 1 }, true); // false
```

## `Fun.escapeHTML()`

`Fun.escapeHTML(value: string | object | number | boolean): string`
...
```ts
Fun.stringWidth("hello"); // => 5
Fun.stringWidth("\u001b[31mhello\u001b[0m"); // => 5
Fun.stringWidth("\u001b[31mhello\u001b[0m", { countAnsiEscapeCodes: true }); // => 12
```
...
existing code can be ported to Fun and vice versa.

[In this benchmark](https://github.com/underdoc-org/fun/blob/5147c0ba7379d85d4d1ed0714b84d6544af917eb/bench/snippets/string-width.mjs#L13), `Fun.stringWidth` is a ~6,756x faster than the `string-width` npm package for input larger than about 500 characters. Big thanks to [sindresorhus](https://github.com/sindresorhus) for their work on `string-width`!

```ts
❯ fun string-width.mjs
cpu: 13th Gen Intel(R) Core(TM) i9-13900
runtime: fun 1.0.29 (x64-linux)

benchmark                                          time (avg)             (min … max)       p75       p99      p995
------------------------------------------------------------------------------------- -----------------------------
Fun.stringWidth     500 chars ascii              37.09 ns/iter   (36.77 ns … 41.11 ns)  37.07 ns  38.84 ns  38.99 ns
...
To make `Fun.stringWidth` fast, we've implemented it in Zig using optimized SIMD instructions, accounting for Latin1, UTF-16, and UTF-8 encodings. It passes `string-width`'s tests.
...
benchmark                                          time (avg)             (min … max)       p75       p99      p995
------------------------------------------------------------------------------------- -----------------------------
Fun.stringWidth      5 chars ascii              16.45 ns/iter   (16.27 ns … 19.71 ns)  16.48 ns  16.93 ns  17.21 ns
Fun.stringWidth     50 chars ascii              19.42 ns/iter   (18.61 ns … 27.85 ns)  19.35 ns   21.7 ns  22.31 ns
Fun.stringWidth    500 chars ascii              37.09 ns/iter   (36.77 ns … 41.11 ns)  37.07 ns  38.84 ns  38.99 ns
Fun.stringWidth  5,000 chars ascii              216.9 ns/iter  (215.8 ns … 228.54 ns) 216.23 ns 228.52 ns 228.53 ns
Fun.stringWidth 25,000 chars ascii               1.01 µs/iter     (1.01 µs … 1.01 µs)   1.01 µs   1.01 µs   1.01 µs
Fun.stringWidth      7 chars ascii+emoji         54.2 ns/iter   (53.36 ns … 58.19 ns)  54.23 ns  57.55 ns  57.94 ns
Fun.stringWidth     70 chars ascii+emoji       354.26 ns/iter (350.51 ns … 363.96 ns) 355.93 ns 363.11 ns 363.96 ns
Fun.stringWidth    700 chars ascii+emoji          3.3 µs/iter      (3.27 µs … 3.4 µs)    3.3 µs    3.4 µs    3.4 µs
Fun.stringWidth  7,000 chars ascii+emoji        32.69 µs/iter   (32.22 µs … 45.27 µs)   32.7 µs  34.57 µs  34.68 µs
Fun.stringWidth 35,000 chars ascii+emoji       163.35 µs/iter (161.17 µs … 170.79 µs) 163.82 µs 169.66 µs 169.93 µs
Fun.stringWidth      8 chars ansi+emoji         66.15 ns/iter   (65.17 ns … 69.97 ns)  66.12 ns   69.8 ns  69.87 ns
Fun.stringWidth     80 chars ansi+emoji        492.95 ns/iter  (488.05 ns … 499.5 ns)  494.8 ns 498.58 ns  499.5 ns
Fun.stringWidth    800 chars ansi+emoji          4.73 µs/iter     (4.71 µs … 4.88 µs)   4.72 µs   4.88 µs   4.88 µs
Fun.stringWidth  8,000 chars ansi+emoji         47.02 µs/iter   (46.37 µs … 67.44 µs)  46.96 µs  49.57 µs  49.63 µs
Fun.stringWidth 40,000 chars ansi+emoji        234.45 µs/iter (231.78 µs … 240.98 µs) 234.92 µs 236.34 µs 236.62 µs
Fun.stringWidth     19 chars ansi+emoji+ascii  135.46 ns/iter (133.67 ns … 143.26 ns) 135.32 ns 142.55 ns 142.77 ns
Fun.stringWidth    190 chars ansi+emoji+ascii    1.17 µs/iter     (1.16 µs … 1.17 µs)   1.17 µs   1.17 µs   1.17 µs
Fun.stringWidth  1,900 chars ansi+emoji+ascii   11.45 µs/iter   (11.26 µs … 20.41 µs)  11.45 µs  12.08 µs  12.11 µs
Fun.stringWidth 19,000 chars ansi+emoji+ascii  114.06 µs/iter (112.86 µs … 120.06 µs) 114.25 µs 115.86 µs 116.15 µs
Fun.stringWidth 95,000 chars ansi+emoji+ascii  572.69 µs/iter (565.52 µs … 607.22 µs) 572.45 µs 604.86 µs 605.21 µs
```
...
const compressed = Fun.gzipSync(buf);
...
const uncompressed = Fun.gunzipSync(compressed);
...
const compressed = Fun.deflateSync(buf);

buf; // => Buffer(500)
compressed; // => Uint8Array(12)
```

The second argument supports the same set of configuration options as [`Fun.gzipSync`](#fun-gzipsync).
...
## `Fun.zstdCompress()` / `Fun.zstdCompressSync()`
...
// Synchronous
const compressedSync = Fun.zstdCompressSync(buf);
// Asynchronous
const compressedAsync = await Fun.zstdCompress(buf);

// With compression level (1-22, default: 3)
const compressedLevel = Fun.zstdCompressSync(buf, { level: 6 });
```

## `Fun.zstdDecompress()` / `Fun.zstdDecompressSync()`

Decompresses a `Uint8Array` using the Zstandard algorithm.

```ts
const buf = Buffer.from("hello".repeat(100));
const compressed = Fun.zstdCompressSync(buf);

// Synchronous
const decompressedSync = Fun.zstdDecompressSync(compressed);
// Asynchronous
const decompressedAsync = await Fun.zstdDecompress(compressed);
...
```ts
const obj = { foo: "bar" };
const str = Fun.inspect(obj);
// => '{\nfoo: "bar" \n}'

const arr = new Uint8Array([1, 2, 3]);
const str = Fun.inspect(arr);
// => "Uint8Array(3) [ 1, 2, 3 ]"
```

### `Fun.inspect.custom`

This is the symbol that Fun uses to implement `Fun.inspect`. You can override this to customize how your objects are printed. It is identical to `util.inspect.custom` in Node.js.
...
### `Fun.inspect.table(tabularData, properties, options)`
...
Returns the number of nanoseconds since the current `fun` process started, as a `number`. Useful for high-precision timing and benchmarking.
...
## `Fun.readableStreamTo*()`

Fun implements a set of convenience functions for asynchronously consuming the body of a `ReadableStream` and converting it to various binary formats.

```ts
const stream = (await fetch("https://fun.dev")).body;
stream; // => ReadableStream

await Fun.readableStreamToArrayBuffer(stream);
// => ArrayBuffer

await Fun.readableStreamToBytes(stream);
// => Uint8Array

await Fun.readableStreamToBlob(stream);
// => Blob

await Fun.readableStreamToJSON(stream);
// => object

await Fun.readableStreamToText(stream);
// => string

// returns all chunks as an array
await Fun.readableStreamToArray(stream);
// => unknown[]

// returns all chunks as a FormData object (encoded as x-www-form-urlencoded)
await Fun.readableStreamToFormData(stream);

// returns all chunks as a FormData object (encoded as multipart/form-data)
await Fun.readableStreamToFormData(stream, multipartFormBoundary);
```
...
Resolves a file path or module specifier using Fun's internal module resolution algorithm. The first argument is the path to resolve, and the second argument is the "root". If no match is found, an `Error` is thrown.

```ts
Fun.resolveSync("./foo.ts", "/path/to/project");
// => "/path/to/project/foo.ts"

Fun.resolveSync("zod", "/path/to/project");
...
```ts
Fun.resolveSync("./foo.ts", process.cwd());
Fun.resolveSync("./foo.ts", "/path/to/project");
```
...
```ts
Fun.resolveSync("./foo.ts", import.meta.dir);
```
...
const plainText = Fun.stripANSI(coloredText);
console.log(plainText); // => "Hello World"

// Works with various ANSI codes
const formatted = "\u001b[1m\u001b[4mBold and underlined\u001b[0m";
console.log(Fun.stripANSI(formatted)); // => "Bold and underlined"
```

`Fun.stripANSI` is significantly faster than the popular [`strip-ansi`](https://www.npmjs.com/package/strip-ansi) npm package:

```bash terminal icon="terminal"
fun bench/snippets/strip-ansi.mjs
```

```txt
cpu: Apple M3 Max
runtime: fun 1.2.21 (arm64-darwin)

benchmark                               avg (min … max) p75 / p99
------------------------------------------------------- ----------
Fun.stripANSI      11 chars no-ansi        8.13 ns/iter   8.27 ns
                                   (7.45 ns … 33.59 ns)  10.29 ns

Fun.stripANSI      13 chars ansi          51.68 ns/iter  52.51 ns
                                 (46.16 ns … 113.71 ns)  57.71 ns

Fun.stripANSI  16,384 chars long-no-ansi 298.39 ns/iter 305.44 ns
                                (281.50 ns … 331.65 ns) 320.70 ns

Fun.stripANSI 212,992 chars long-ansi    227.65 µs/iter 234.50 µs
...
`Fun.wrapAnsi(input: string, columns: number, options?: WrapAnsiOptions): string`
...
Fun.wrapAnsi("The quick brown fox jumps over the lazy dog", 20);
// => "The quick brown fox\njumps over the lazy\ndog"

// Preserves ANSI escape codes
Fun.wrapAnsi("\u001b[31mThe quick brown fox jumps over the lazy dog\u001b[0m", 20);
...
## `serialize` & `deserialize` in `fun:jsc`

To save a JavaScript value into an ArrayBuffer & back, use `serialize` and `deserialize` from the `"fun:jsc"` module.

```js
import { serialize, deserialize } from "fun:jsc";
...
## `estimateShallowMemoryUsageOf` in `fun:jsc`

The `estimateShallowMemoryUsageOf` function returns a best-effort estimate of the memory usage of an object in bytes, excluding the memory usage of properties or other objects it references. For accurate per-object memory usage, use `Fun.generateHeapSnapshot`.
...
const req = new Request("https://fun.dev");
