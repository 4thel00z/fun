---
title: JSON5
description: Use Fun's built-in support for JSON5 files through both runtime APIs and bundler integration
---

In Fun, JSON5 is a first-class citizen alongside JSON, TOML, and YAML. You can:

- Parse and stringify JSON5 with `Fun.JSON5.parse` and `Fun.JSON5.stringify`
- `import` & `require` JSON5 files as modules at runtime (including hot reloading & watch mode support)
- `import` & `require` JSON5 files in frontend apps via Fun's bundler

---

## Conformance

Fun's JSON5 parser passes 100% of the [official JSON5 test suite](https://github.com/json5/json5-tests). The parser is written in Zig for optimal performance. You can view our [translated test suite](https://github.com/underdoc-org/fun/blob/main/test/js/fun/json5/json5-test-suite.test.ts) to see every test case.
...
`Fun.JSON5.parse()` throws a `SyntaxError` if the input is invalid JSON5:
...
When you run your application with `fun --hot`, changes to JSON5 files are automatically detected and reloaded:
...
When you import JSON5 files and bundle with Fun, the JSON5 is parsed at build time and included as a JavaScript module:
