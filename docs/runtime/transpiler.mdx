---
title: Transpiler
description: Use Fun's transpiler to transpile JavaScript and TypeScript code
---

Fun exposes its internal transpiler via the `Fun.Transpiler` class. To create an instance of Fun's transpiler:
...
```ts output
import { __require as require } from "fun:wrap";
...
To override the default loader specified in the `new Fun.Transpiler()` constructor, pass a second argument to `.transformSync()`.
...
Unless you're transpiling _many_ large files, you should probably use `Fun.Transpiler.transformSync`. The cost of the threadpool will often take longer than actually transpiling code.
...
The `.transform()` method runs the transpiler in Fun's worker threadpool, so if you run it 100 times, it will run it across `Math.floor($cpu_count * 0.8)` threads, without blocking the main JavaScript thread.

If your code uses a macro, it will potentially spawn a new copy of Fun's JavaScript runtime environment in that new thread.
...
Each import in the `imports` array has a `path` and `kind`. Fun categories imports into the following kinds:
...
type Import = {
  ...
  // url() in CSS
  | "url-token"
  // The import was injected by Fun
  | "internal"
