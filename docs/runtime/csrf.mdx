...
description: Generate and verify CSRF tokens with Fun's built-in API
---

Fun provides a built-in API for generating and verifying [CSRF (Cross-Site Request Forgery)](https://owasp.org/www-community/attacks/csrf) tokens through `Fun.CSRF`. Tokens are signed with HMAC and include expiration timestamps to limit the token validity window.

```ts title="csrf.ts" icon="/icons/typescript.svg"
// Generate a token
const token = Fun.CSRF.generate("my-secret");

// Verify it
const isValid = Fun.CSRF.verify(token, { secret: "my-secret" });
...
const token = Fun.CSRF.generate("my-secret-key");
```

**Parameters:**

- `secret` (string, optional) — The secret key used to sign the token. If not provided, Fun generates a random in-memory default secret (unique per thread).
...
const token2 = Fun.CSRF.generate("my-secret", {
  algorithm: "sha512",
});
```
...
const isValid = Fun.CSRF.verify(token, { secret: "my-secret-key" });
```
...
const isValid = Fun.CSRF.verify(hexToken, {
  secret: "my-secret",
  encoding: "hex",
});

// Enforce a shorter max age than what the token was generated with
const isValid2 = Fun.CSRF.verify(token, {
  secret: "my-secret",
...
const server = Fun.serve({
  routes: {
    "/form": () => {
      const token = Fun.CSRF.generate(SECRET);
    ...
    "/submit": {
      POST: async req => {
        ...
        if (typeof csrfToken !== "string" || !Fun.CSRF.verify(csrfToken, { secret: SECRET })) {
...
If you omit the `secret` parameter in both `generate()` and `verify()`, Fun uses a random secret generated once per thread. This is convenient for single-thread applications but won't work across multiple servers, workers, or after a restart.
...
const token = Fun.CSRF.generate();
const isValid = Fun.CSRF.verify(token); // true
```
