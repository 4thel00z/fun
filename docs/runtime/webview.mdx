---
title: WebView
description: Control a headless browser from Fun for automation, testing, and scraping — zero dependencies on macOS, Chrome DevTools Protocol everywhere else
---

`Fun.WebView` is a headless browser built into the runtime. Use it to load pages, run JavaScript inside them, simulate real user input, and capture screenshots — without Puppeteer, Playwright, or a separate browser download.
...
await Fun.write("page.png", await view.screenshot());
```

On macOS, `Fun.WebView` uses the system's `WKWebView` — nothing to install. On Linux and Windows it drives an installed Chrome, Chromium, Edge, or Brave over the Chrome DevTools Protocol.
...
const view = new Fun.WebView({
  ...
  url: "https://fun.dev", // optional: start navigating immediately
});
```
...
`Fun.WebView` implements `Symbol.dispose` and `Symbol.asyncDispose`, so you can use `using` or `await using` to close the view automatically when it goes out of scope:
...
<Note>
  ...
  not per-view. Since Chrome is spawned once per Fun process, the first view's directory wins for all subsequent views.
</Note>
...
`Fun.WebView` supports two rendering engines. The default depends on your platform:
...
Fun spawns a lightweight host subprocess (the `fun` binary itself, re-executed in a special mode) that owns the `WKWebView` on its main thread. Your Fun process talks to it over a Unix socket using a compact binary protocol. The host process is spawned once and shared by every `"webkit"` view in your program.

### How the Chrome backend works

Fun either **connects** to an already-running Chrome over a WebSocket, or **spawns** a headless Chrome subprocess and talks to it over a pipe (`--remote-debugging-pipe`). Either way, communication uses the [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/).

Chrome is spawned (or connected) once per Fun process. Each `new Fun.WebView({ backend: "chrome" })` creates a new tab via `Target.createTarget` in that single Chrome instance.

#### Finding the Chrome executable

When Fun needs to spawn Chrome, it searches in this order:

1. The `path` you passed in `backend: { type: "chrome", path: "..." }`
2. The `FUN_CHROME_PATH` environment variable
...
By default, before spawning, Fun checks whether a Chrome-family browser is **already running** with remote debugging enabled by reading the `DevToolsActivePort` file from standard profile directories. If found, Fun connects to that browser over WebSocket instead of spawning a new one — your views open as tabs in your existing browser.
...
If auto-detect finds a stale `DevToolsActivePort` file (Chrome crashed or restarted), the WebSocket connect fails and Fun transparently falls back to spawning its own Chrome. An explicit `url: "ws://..."` does **not** fall back — a failed connection throws.
...
When spawning, Fun passes a minimal flag set:
...
const user = await view.evaluate("({ name: 'fun', ok: true })");
```
...
The result round-trips through `JSON.stringify` in the page and `JSON.parse` in Fun. Arrays and plain objects come back as real structures; `undefined`, functions, and symbols resolve to `undefined`; circular references reject.
...
| -------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `"blob"` _(default)_ | `Blob`                           | MIME type set automatically. Zero-copy mmap-backed on WebKit. Works with `Fun.write()`, `new Response()` |
...
`encoding: "shmem"` is designed for Kitty's [terminal graphics protocol](https://sw.kovidgoyal.net/kitty/graphics-protocol/) `t=s` transmission mode — Fun writes the image to a POSIX shared-memory segment and returns its name; the terminal reads it directly and unlinks it when done. No copying through the pipe.
...
On WebKit, the shm name looks like `/fun-webview-<pid>-<seq>`; on Chrome, `/fun-chrome-<pid>-<seq>`. If you request `"shmem"` and don't hand the name to something that will `shm_unlink` it, the segment leaks until your process exits.
...
Pass a CSS selector instead of coordinates and Fun will wait for the element to become **actionable**, then click its center:
...
Forward `console.*` calls from the page to your Fun process by passing the `console` option to the constructor.

### Mirror to Fun's console

Pass `globalThis.console` (the actual object, by reference) and page-side `console.log("hi")` prints `hi` to your stdout with Fun's formatter; `console.error` goes to stderr. This path dispatches directly through Fun's console implementation with no per-call JavaScript overhead.
...
`Fun.WebView` extends `EventTarget`. With the Chrome backend, CDP events are dispatched as DOM events whose `type` is the CDP method name and whose `data` is the parsed `params` object:
...
Force-kills (`SIGKILL`) both the Chrome subprocess and the WebKit host subprocess. Pending promises on every view reject on the next event-loop tick. Subsequent `new Fun.WebView()` calls respawn as needed.

Fun calls this automatically at process exit, so browser subprocesses never outlive your script.

### Event-loop behavior

The browser subprocess does **not** keep Fun's event loop alive on its own. An open `WebView` keeps the process alive only while it has a pending operation (an unsettled `navigate()`, `evaluate()`, etc.). Once you `close()` the last view — or the last pending operation settles — Fun exits naturally.
...
| `stdout` | `"inherit"` \| `"ignore"` | Route the subprocess's stdout to Fun's. Default `"ignore"`.                                                                                            |
| `stderr` | `"inherit"` \| `"ignore"` | Route the subprocess's stderr to Fun's. Default `"ignore"`.                                                                                            |
