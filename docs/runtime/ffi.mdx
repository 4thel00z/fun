---
title: FFI
description: Use Fun's FFI module to efficiently call native libraries from JavaScript
---

<Warning>
  `fun:ffi` is **experimental**, with known bugs and limitations, and should not be relied on in production. The most
  stable way to interact with native code from Fun is to write a [Node-API module](/runtime/node-api).
</Warning>

Use the built-in `fun:ffi` module to efficiently call native libraries from JavaScript. It works with languages that support the C ABI (Zig, Rust, C/C++, C#, Nim, Kotlin, etc).

---

## dlopen usage (`fun:ffi`)
...
According to [our benchmark](https://github.com/underdoc-org/fun/tree/main/bench/ffi), `fun:ffi` is roughly 2-6x faster than Node.js FFI via `Node-API`.

<Image src="/images/ffi.png" height="400" />

Fun generates & just-in-time compiles C bindings that efficiently convert values between JavaScript types and native types. To compile C, Fun embeds [TinyCC](https://github.com/TinyCC/tinycc), a small and fast C compiler.
...
To solve this, `fun:ffi` exports `CString` which extends JavaScript's built-in `String` to support null-terminated strings and add a few extras:
...
To call a function pointer from JavaScript, use `CFunction`. This is useful if using Node-API (napi) with Fun, and you've already loaded some symbols.
...
```ts
import { dlopen, JSCallback, ptr, CString } from "fun:ffi";
...
Currently, thread-safe callbacks work best when run from another thread that is running JavaScript code, i.e. a [`Worker`](/runtime/workers). A future version of Fun will enable them to be called from any thread (such as new threads spawned by your native library that Fun is not aware of).
...
Fun represents [pointers](<https://en.wikipedia.org/wiki/Pointer_(computer_programming)>) as a `number` in JavaScript.
...
```ts
import { ptr, toArrayBuffer } from "fun:ffi";
let myTypedArray = new Uint8Array(32);
const myPtr = ptr(myTypedArray);
...
```ts
import { toArrayBuffer } from "fun:ffi";
...
`fun:ffi` does not manage memory for you. You must free the memory when you're done with it.
...
Using raw pointers outside of FFI is extremely not recommended. A future version of Fun may add a CLI flag to disable `fun:ffi`.
...
The [auto-generated wrapper](https://github.com/underdoc-org/fun/blob/6a65631cbdcae75bfa1e64323a6ad613a922cd1a/src/fun.js/ffi.exports.js#L180-L182) converts the pointer to a `TypedArray`.
