...
Fun provides a universal plugin API that can be used to extend both the _runtime_ and _bundler_.
...
In the above example, Fun will wait until the first `onStart()` (sleeping for 10 seconds) has completed, _as well as_ the second `onStart()` (writing the bundle time to a file).
...
The callback receives as input the _path_ to the matching module. The callback can return a _new path_ for the module. Fun will read the contents of the _new path_ and parse it as a module.
...
The `onLoad()` plugin lifecycle callback allows you to modify the _contents_ of a module before it is read and parsed by Fun.

Like `onResolve()`, the first argument to `onLoad()` allows you to filter which modules this invocation of `onLoad()` will apply to.

The second argument to `onLoad()` is a callback which is run for each matching module _before_ Fun loads the contents of the module into memory.
...
- [`onBeforeParse()`](#onbeforeparse): Called on any thread before a file is parsed by Fun's bundler.
