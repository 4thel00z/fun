---
title: SQL
description: Fun provides native bindings for working with SQL databases through a unified Promise-based API that supports PostgreSQL, MySQL, and SQLite.
---
...
`Fun.SQL` provides a unified API for multiple database systems:
...
MySQL support is built into Fun.SQL, providing the same tagged template literal interface with full compatibility for MySQL 5.7+ and MySQL 8.0+:
...
SQLite support is built into Fun.SQL, providing the same tagged template literal interface:
...
By default, Fun's SQL client returns query results as arrays of objects, where each object represents a row with column names as keys. However, there are cases where you might want the data in a different format. The client provides two additional methods for this purpose.
...
A common need in database applications is the ability to construct queries dynamically based on runtime conditions. Fun provides safe ways to do this without risking SQL injection.
...
Fun's SQL is lazy, which means it will only start executing when awaited or executed with `.execute()`.
...
When using `Fun.sql()` without arguments or `new SQL()` with a connection string, the adapter is automatically detected based on the URL format:
...
DATABASE_URL="mysql://user:pass@localhost/mydb" fun run app.js
DATABASE_URL="mysql2://user:pass@localhost:3306/mydb" fun run app.js
```
...
// Works with DATABASE_URL environment variable
DATABASE_URL=":memory:" fun run app.js
DATABASE_URL="sqlite://myapp.db" fun run app.js
DATABASE_URL="file://./data/app.db" fun run app.js
```
...
DATABASE_URL="postgres://user:pass@localhost:5432/mydb" fun run app.js
DATABASE_URL="postgresql://user:pass@localhost:5432/mydb" fun run app.js

# Or any URL that doesn't match MySQL or SQLite patterns
DATABASE_URL="localhost:5432/mydb" fun run app.js
```
...
Fun can preconnect to PostgreSQL at startup to improve performance by establishing database connections before your application code runs. This is useful for reducing connection latency on the first database query.

```bash
# Enable PostgreSQL preconnection
fun --sql-preconnect index.js

# Works with DATABASE_URL environment variable
DATABASE_URL=postgres://user:pass@localhost:5432/db fun --sql-preconnect index.js

# Can be combined with other runtime flags
fun --sql-preconnect --hot index.js
```
...
Fun supports SCRAM-SHA-256 (SASL), MD5, and Clear Text authentication. SASL is recommended for better security. Check [Postgres SASL Authentication](https://www.postgresql.org/docs/current/sasl-authentication.html) for more information.
...
```ts
const sql = Fun.SQL(); // no connection are created
...
Fun enables you to reserve a connection from the pool, and returns a client that wraps the single connection. This can be used for running queries on an isolated connection.
...
By default, Fun's SQL client automatically creates named prepared statements for queries where it can be inferred that the query is static. This provides better performance. However, you can change this behavior by setting `prepare: false` in the connection options:
...
Fun's SQL client includes special handling for large numbers that exceed the range of a 53-bit integer. Here's how it works:
...
- Connection preloading via `--db-preconnect` Fun CLI flag
...
Fun.SQL automatically uses `utf8mb4` character set for MySQL connections, ensuring full Unicode support including emojis. This is the recommended character set for modern MySQL applications.

#### Connection Attributes

Fun automatically sends client information to MySQL for better monitoring:

```ts
// These attributes are sent automatically:
// _client_name: "Fun"
// _client_version: <fun version>
...
1. **Parameter placeholders**: MySQL uses `?` internally but Fun converts `$1, $2` style automatically
...
<AccordionGroup>
	<Accordion title="Why is this `Fun.sql` and not `Fun.postgres`?">
...
npm packages like postgres.js, pg, and node-postgres can be used in Fun too. They're great options.

Two reasons why:

1. We think it's simpler for developers to have a database driver built into Fun. The time you spend library shopping is time you could be building your app.
