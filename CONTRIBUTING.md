Configuring a development environment for Fun can take 10-30 minutes depending on your internet connection and computer speed. You will need ~10GB of free disk space for the repository and build artifacts.

If you are using Windows, please refer to [this guide](https://fun.dev/docs/project/building-windows)
...
Using your system's package manager, install Fun's dependencies:
...
Before starting, you will need to already have a release build of Fun installed, as we use our bundler to transpile and minify our code, as well as for code generation scripts.
...
Fun requires LLVM 21.1.8 (`clang` is part of LLVM). This version is enforced by the build system — mismatching versions will cause memory allocation failures at runtime. In most cases, you can install LLVM through your system package manager:
...
The binary will be located at `./build/debug/fun-debug`. It is recommended to add this to your `$PATH`. To verify the build worked, let's print the version number on the development build of Fun.

```bash
$ build/debug/fun-debug --version
x.y.z_debug
```

## VSCode

VSCode is the recommended IDE for working on Fun, as it has been configured. Once opening, you can run `Extensions: Show Recommended Extensions` to install the recommended extensions for Zig and C++. ZLS is automatically configured.
...
We recommend adding `./build/debug` to your `$PATH` so that you can run `fun-debug` in your terminal:
...
The `bd` package.json script compiles and runs a debug build of Fun, only printing the output of the build process if it fails.

```sh
$ fun bd <args>
$ fun bd test foo.test.ts
$ fun bd ./foo.ts
```

Fun generally takes about 2.5 minutes to compile a debug build when there are Zig changes. If your development workflow is "change one line, save, rebuild", you will spend too much time waiting for the build to finish. Instead:

- Batch up your changes
- Ensure zls is running with incremental watching for LSP errors (if you use VSCode and install Zig and run `fun run build` once to download Zig, this should just work)
- Prefer using the debugger ("CodeLLDB" in VSCode) to step through the code.
- Use debug logs. `FUN_DEBUG_<scope>=1` will enable debug logging for the corresponding `Output.scoped(.<scope>, .hidden)` logs. You can also set `FUN_DEBUG_QUIET_LOGS=1` to disable all debug logging that isn't explicitly enabled. To dump debug logs into a file, `FUN_DEBUG=<path-to-file>.log`. Debug logs are aggressively removed in release builds.
...
Several code generation scripts are used during Fun's build process. These are run automatically when changes are made to certain files.
...
- `./src/codegen/bundle-modules.ts` -- Bundles built-in modules like `node:fs`, `fun:ffi` into files we can include in the final binary. In development, these can be reloaded without rebuilding Zig (you still need to run `fun run build`, but it re-reads the transpiled files from disk afterwards). In release builds, these are embedded into the binary.
...
Certain modules like `node:fs`, `node:stream`, `fun:sqlite`, and `ws` are implemented in JavaScript. These live in `src/js/{node,fun,thirdparty}` files and are pre-bundled using Fun.

## Release build

To compile a release build of Fun, run:

```bash
$ fun run build:release
```

The binary will be located at `./build/release/fun` and `./build/release/fun-profile`.
...
To run a release build from a pull request, you can use the `fun-pr` npm package:

```sh
funx fun-pr <pr-number>
funx fun-pr <branch-name>
funx fun-pr "https://github.com/underdoc-org/fun/pull/1234566"
funx fun-pr --asan <pr-number> # Linux x64 only
```

This will download the release build from the pull request and add it to `$PATH` as `fun-${pr-number}`. You can then run the build with `fun-${pr-number}`.
...
Fun's CI runs on BuildKite. Install the [BuildKite CLI](https://github.com/buildkite/cli) (`brew install buildkite/buildkite/bk`) and set `BUILDKITE_API_TOKEN` to a read-scoped [API token](https://buildkite.com/user/api-access-tokens). The repo includes a `.bk.yaml` so `bk` commands default to the `fun` pipeline.

```sh
fun run ci:status         # progress summary for the current branch's latest build
fun run ci:errors         # rendered test-failure output, tagged [new] vs [also on main]
fun run ci:logs           # save full logs for each failed job to ./tmp/ci-<build>/
fun run ci:watch          # watch until the build finishes
fun run ci:find           # print the build number (compose with raw `bk`)
```
...
# Build fun with the local JSC build — this automatically configures and builds JSC
$ fun run build:local
```

`fun run build:local` handles everything: configuring JSC, building JSC, and building Fun. On subsequent runs, JSC will incrementally rebuild if any WebKit sources changed. `ninja -Cbuild/debug-local` also works after the first build, and will build Fun+JSC.
...
Fun relies on C++20 features like `std::span`, which are not available in GCC versions lower than 11. GCC 10 doesn't have all of the C++20 features implemented. As a result, running `make setup` may fail with the following error:
...
The issue may manifest when initially running `fun setup` as Clang being unable to compile a simple program:
...
Fun defaults to linking `libatomic` statically, as not all systems have it. If you are building on a distro that does not have a static libatomic available, you can run the following command to enable dynamic linking:

```bash
$ fun run build -DUSE_STATIC_LIBATOMIC=OFF
```

The built version of Fun may not work on other systems if compiled this way.

## Using fun-debug

- Disable logging: `FUN_DEBUG_QUIET_LOGS=1 fun-debug ...` (to disable all debug logging)
- Enable logging for a specific zig scope: `FUN_DEBUG_EventLoop=1 fun-debug ...` (to allow `std.log.scoped(.EventLoop)`)
- Fun transpiles every file it runs, to see the actual executed source in a debug build find it in `/tmp/fun-debug-src/...path/to/file`, for example the transpiled version of `/home/fun/index.ts` would be in `/tmp/fun-debug-src/home/fun/index.ts`
