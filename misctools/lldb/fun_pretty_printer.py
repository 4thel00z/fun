# Pretty printers for Fun data structures
import lldb
import re

class fun_BabyList_SynthProvider:
...
def fun_BabyList_SummaryProvider(value, _=None):
    try:
...
def fun_String_SummaryProvider(value, _=None):
    try:
        value = value.GetNonSyntheticValue()
...
def __lldb_init_module(debugger, _=None):
    # Initialize Fun Category
    debugger.HandleCommand('type category define --language c99 fun')
    
    # Initialize Fun Data Structures
    add(debugger, category='fun', regex=True, type='^baby_list\\.BabyList\\(.*\\)$', identifier='fun_BabyList', synth=True, expand=True, summary=True)
    
    # Add WTFStringImpl pretty printer - try multiple possible type names
    add(debugger, category='fun', type='WTFStringImpl', identifier='WTFStringImpl', summary=True)
    add(debugger, category='fun', type='*WTFStringImplStruct', identifier='WTFStringImpl', summary=True)
    add(debugger, category='fun', type='string.WTFStringImpl', identifier='WTFStringImpl', summary=True)
    add(debugger, category='fun', type='string.WTFStringImplStruct', identifier='WTFStringImpl', summary=True)
    add(debugger, category='fun', type='*string.WTFStringImplStruct', identifier='WTFStringImpl', summary=True)
    
    # Add ZigString pretty printer - try multiple possible type names
    add(debugger, category='fun', type='ZigString', identifier='ZigString', summary=True)
    add(debugger, category='fun', type='fun.js.bindings.ZigString', identifier='ZigString', summary=True)
    add(debugger, category='fun', type='bindings.ZigString', identifier='ZigString', summary=True)
    
    # Add fun.String pretty printer - try multiple possible type names
    add(debugger, category='fun', type='String', identifier='fun_String', summary=True)
    add(debugger, category='fun', type='fun.String', identifier='fun_String', summary=True)
    add(debugger, category='fun', type='string.String', identifier='fun_String', summary=True)
    add(debugger, category='fun', type='FunString', identifier='fun_String', summary=True)
    add(debugger, category='fun', type='fun::String', identifier='fun_String', summary=True)
    add(debugger, category='fun', type='fun::string::String', identifier='fun_String', summary=True)
    
    # Try regex patterns for more flexible matching
    add(debugger, category='fun', regex=True, type='.*String$', identifier='fun_String', summary=True)
    add(debugger, category='fun', regex=True, type='.*WTFStringImpl.*', identifier='WTFStringImpl', summary=True)
    add(debugger, category='fun', regex=True, type='.*ZigString.*', identifier='ZigString', summary=True)
    
    # Enable the category
    debugger.HandleCommand('type category enable fun')