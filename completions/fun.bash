...
_fun_completions() {
    ...
    local SUBCOMMANDS="dev fun create run install add remove upgrade completions discord help init pm x test repl update outdated link unlink build";

    GLOBAL_OPTIONS[LONG_OPTIONS]="--use --cwd --funfile --server-funfile --config --disable-react-fast-refresh --disable-hmr --env-file --extension-order --jsx-factory --jsx-fragment --extension-order --jsx-factory --jsx-fragment --jsx-import-source --jsx-production --jsx-runtime --main-fields --no-summary --version --platform --public-dir --tsconfig-override --define --external --help --inject --loader --origin --port --dump-environment-variables --dump-limits --disable-fun-js";
    ...
    case "${prev}" in
        ...
        --funfile)        _file_arguments "!*.fun" && return;;
        --server-funfile) _file_arguments "!*.server.fun" && return;;
        --backend)
        ...
        --jsx-runtime)
            ...
            return;;
        --target)
            COMPREPLY=( $(compgen -W "browser node fun" -- "${cur_word}") );
            return;;
        -l|--loader)
            [[ "${cur_word}" =~ (:) ]] && {
        ...
        *)
            ...
            [[ -z "${cur_word}" ]] && {
                ...
                [[ -z "${comp_reply_associative[${prev}]}" ]] && {
                    ...
                    local global_option_with_extra_args="--funfile --server-funfile --config --port --cwd --public-dir --jsx-runtime --platform --loader";
                    [[
...
complete -F _fun_completions fun
