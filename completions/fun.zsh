...
_fun_add_completion() {
    _arguments -s -C \
        '1: :->cmd1' \
        '*: :->package' \
        '--config[Load config(funfig.toml)]: :->config' \
        '-c[Load config(funfig.toml)]: :->config' \
    ...
    case $state in
    config)
        _fun_list_bunfig_toml

        ;;
    package)
        _fun_add_param_package_completion

        ;;
    esac
}

_fun_unlink_completion() {
    _arguments -s -C \
        '1: :->cmd1' \
        '*: :->package' \
        '--config[Load config(funfig.toml)]: :->config' \
    ...
    config)
        ...
        ;;
    package)
        # TODO: error: fun unlink {packageName} not implemented yet

        ;;
    esac
}

_fun_link_completion() {
    _arguments -s -C \
        '1: :->cmd1' \
        '*: :->package' \
        '--config[Load config(funfig.toml)]: :->config' \
    ...
    config)
        ...
        ;;
    package)
        _fun_link_param_package_completion
    ...
    args)
        ...
        next)
            pmargs=(
                ...
                '--open[On finish, start fun & open in-browser]'
            )
...
_fun_remove_completion() {
    ...
    config)
        ...
        ;;
    package)
        _fun_remove_param_package_completion
...
_fun_run_completion() {
    _arguments -s -C \
        ...
        '--fun[Force a script or package to use Fun'"'"'s runtime instead of Node.js (via symlinking node)]' \
        '-b[Force a script or package to use Fun'"'"'s runtime instead of Node.js (via symlinking node)]' \
        '--cwd[Absolute path to resolve files & entry points from. This just changes the process cwd]:cwd' \
        '--config[Config file to load fun from (e.g. -c funfig.toml]: :->config' \
        '-c[Config file to load fun from (e.g. -c funfig.toml]: :->config' \
        ...
        '--port[Port to serve fun'"'"'s dev server on. Default: '"'"'3000'"'"']:port' \
        '-p[Port to serve fun'"'"'s dev server on. Default: '"'"'3000'"'"']:port' \
        ...
        '--target[The intended execution environment for the bundle. "browser", "fun" or "node"]: :->target' \
        '--inspect[Activate Fun'"'"'s Debugger]:inspect' \
        '--inspect-wait[Activate Fun'"'"'s Debugger, wait for a connection before executing]:inspect-wait' \
        '--inspect-brk[Activate Fun'"'"'s Debugger, set breakpoint on first line of code and wait]:inspect-brk' \
        '--hot[Enable auto reload in fun'"'"'s JavaScript runtime]' \
        '--watch[Automatically restart fun'"'"'s JavaScript runtime on file change]' \
        '--no-install[Disable auto install in fun'"'"'s JavaScript runtime]' \
        ...
        '-i[Automatically install dependencies and use global cache in fun'"'"'s runtime, equivalent to --install=fallback'] \
        '--prefer-offline[Skip staleness checks for packages in fun'"'"'s JavaScript runtime and resolve from disk]' \
        '--prefer-latest[Use the latest matching versions of packages in fun'"'"'s JavaScript runtime, always checking npm]' \
        '--silent[Don'"'"'t repeat the command for fun run]' \
    ...
    case $state in
    script)
        curcontext="${curcontext%:*:*}:fun-grouped"
        _fun_run_param_script_completion
    ...
    jsx-runtime)
        ...
        ;;
    target)
        _alternative 'args:cmd3:((browser fun node))'
...
_fun_repl_completion() {
    _arguments -s -C \
        ...
        '-r[Import a module before other modules are loaded]:preload' \
        '--smol[Use less memory, but run garbage collection more often]' \
        '--config[Specify path to Fun config file]: :->config' \
        '-c[Specify path to Fun config file]: :->config' \
...
_fun_test_completion() {
    _arguments -s -C \
        '1: :->cmd1' \
        '*: :->file' \
        '-h[Display this help and exit]' \
        '--help[Display this help and exit]' \
        '-b[Force a script or package to use Fun.js instead of Node.js (via symlinking node)]' \
        '--fun[Force a script or package to use Fun.js instead of Node.js (via symlinking node)]' \
        '--cwd[Set a specific cwd]:cwd' \
        '-c[Load config(funfig.toml)]: :->config' \
        '--config[Load config(funfig.toml)]: :->config' \
        '--env-file[Load environment variables from the specified file(s)]:env-file' \
    ...
    case $state in
    file)
        _fun_test_param_script_completion
...
_fun() {
    zstyle ':completion:*:*:fun:*' group-name ''
    zstyle ':completion:*:*:fun-grouped:*' group-name ''

    zstyle ':completion:*:*:fun::descriptions' format '%F{green}-- %d --%f'
    zstyle ':completion:*:*:fun-grouped:*' format '%F{green}-- %d --%f'
    ...
    cmd)
        ...
        IFS=$'\n' scripts_list=($(SHELL=zsh fun getcompletes i))
        scripts="scripts:scripts:((${scripts_list//:/\\\\:}))"
        IFS=$'\n' files_list=($(SHELL=zsh fun getcompletes j))

        main_commands=(
            'run\:"Run JavaScript with Fun, a package.json script, or a bin" '
            'test\:"Run unit tests with Fun" '
            'x\:"Install and execute a package bin (funx)" '
            'repl\:"Start a REPL session with Fun" '
            'init\:"Start an empty Fun project from a blank template" '
            'create\:"Create a new project from a template (fun c)" '
            'install\:"Install dependencies for a package.json (fun i)" '
            'add\:"Add a dependency to package.json (fun a)" '
            'remove\:"Remove a dependency from package.json (fun rm)" '
            ...
            'upgrade\:"Get the latest version of fun" '
...
_fun_run_param_script_completion() {
    local -a scripts_list
    IFS=$'\n' scripts_list=($(SHELL=zsh fun getcompletes s))
    IFS=$'\n' bins=($(SHELL=zsh fun getcompletes b))
...
_fun_link_param_package_completion() {
    # Read packages from ~/.fun/install/global/node_modules
    install_env=$FUN_INSTALL
    install_dir=${(P)install_env:-$HOME/.fun}
...
_fun_add_param_package_completion() {
    ...
    IFS=$'\n' inexact=($(history -n fun | grep -E "^fun add " | cut -c 9- | uniq))
    IFS=$'\n' exact=($($inexact | grep -E "^$words[$CURRENT]"))
    IFS=$'\n' packages=($(SHELL=zsh fun getcompletes a $words[$CURRENT]))
