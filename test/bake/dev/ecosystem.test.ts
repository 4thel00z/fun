...
devTest("svelte component islands example", {
  ...
  async test(dev) {
    ...
    if (html.includes("Fun__renderFallbackError")) throw new Error("failed");

    // Expect SSR
    expect(html).toContain('self.$islands={"pages/_Counter.svelte":[[0,"default",{initial:5}]]}');
    expect(html).toContain(`<p>This is my svelte server component (non-interactive)</p> <p>Fun v${Fun.version}</p>`);
    ...
    if (html2.includes("Fun__renderFallbackError")) throw new Error("failed");

    // Expect SSR
    expect(html2).toContain(`<p>This is my svelte server component (awesome)</p> <p>Fun v${Fun.version}</p>`);
