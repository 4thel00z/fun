...
describe("funfig.toml test options", () => {
  test("randomize with seed produces consistent order", async () => {
    const dir = tempDirWithFiles("funfig-test-randomize-seed", {
      "test.test.ts": `
  ...
  test("seed without randomize errors", async () => {
    const dir = tempDirWithFiles("funfig-test-seed-no-randomize", {
      "test.test.ts": `
        import { test, expect } from "fun:test";
  ...
  test("seed with randomize=false errors", async () => {
    const dir = tempDirWithFiles("funfig-test-seed-randomize-false", {
      "test.test.ts": `
        import { test, expect } from "fun:test";
        test("test 1", () => expect(1).toBe(1));
      `,
  ...
  test("rerunEach option works", async () => {
    const dir = tempDirWithFiles("funfig-test-rerun-each", {
      "test.test.ts": `
        import { test, expect } from "fun:test";
        let counter = 0;
        test("test 1", () => {
  ...
  test("all test options together", async () => {
    const dir = tempDirWithFiles("funfig-test-all-options", {
      "test.test.ts": `
        import { test, expect } from "fun:test";
        test("test 1", () => expect(1).toBe(1));
