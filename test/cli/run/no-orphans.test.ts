import { dlopen, FFIType } from "fun:ffi";
import { describe, expect, test } from "fun:test";
import { funEnv, funExe, isLinux, isMusl, tempDir } from "harness";
import { chmodSync, readFileSync } from "node:fs";
import { setTimeout as sleep } from "node:timers/promises";

// --no-orphans / FUN_FEATURE_FLAG_NO_ORPHANS / [run] noOrphans: Fun watches its
...
//
// Tree under test: test → sh (the "parent" we SIGKILL) → fun-debug → grandchild.
// We SIGKILL sh and observe fun-debug and the grandchild.
...
const fixture = tempDir("no-orphans", {
  ...
  // and the libproc walk (macOS) cover non-Fun descendants.
  "child-nonfun.js": `
    const gc = Fun.spawn({
...
async function spawnTree(noOrphans: string | undefined, childScript = "child.js") {
  const env: Record<string, string> = { ...funEnv };
  // funEnv spreads process.env; make sure an ambient FUN_FEATURE_FLAG_NO_ORPHANS
  // from the test runner doesn't leak into the "unset" case.
  delete env.FUN_FEATURE_FLAG_NO_ORPHANS;
  if (noOrphans !== undefined) env.FUN_FEATURE_FLAG_NO_ORPHANS = noOrphans;

  const sh = Fun.spawn({
    // Trailing `wait` defeats sh's implicit-exec-of-last-command so sh stays a
    // distinct pid we can SIGKILL independently of fun.
    cmd: ["/bin/sh", "-c", `"${funExe()}" "${String(fixture)}/${childScript}" & wait`],
    env,
    stdout: "pipe",
    stderr: "ignore",
  });
  ...
  reader.releaseLock();
  const [funPid, funPpid, grandchildPid] = line.trim().split(" ").map(Number);
  expect(funPid).toBeGreaterThan(0);
  expect(funPpid).toBe(sh.pid);
  expect(grandchildPid).toBeGreaterThan(0);

  return { sh, funPid, grandchildPid };
}
...
test.skipIf(!isSupported)(
  "without FUN_FEATURE_FLAG_NO_ORPHANS, fun is orphaned when its parent is SIGKILLed",
  async () => {
    const { sh, funPid, grandchildPid } = await spawnTree(undefined);
    process.kill(sh.pid!, "SIGKILL");
    await sh.exited;
    // fun must NOT die: poll for death and expect the poll to time out.
    const died = await waitUntilDead(funPid, 1000);
    reap(funPid, grandchildPid);
    expect(died).toBe(false);
  },
);

test.skipIf(!isSupported)("FUN_FEATURE_FLAG_NO_ORPHANS=1: fun exits when its parent is SIGKILLed", async () => {
  const { sh, funPid, grandchildPid } = await spawnTree("1");
  process.kill(sh.pid!, "SIGKILL");
  await sh.exited;
  // kqueue NOTE_EXIT / PDEATHSIG fire effectively immediately; poll until
  // fun is gone rather than sleeping a fixed interval.
  const died = await waitUntilDead(funPid, 10000);
  reap(funPid, grandchildPid);
  expect(died).toBe(true);
});

test.skipIf(!isSupported)(
  "FUN_FEATURE_FLAG_NO_ORPHANS=1: grandchildren are reaped when fun dies with its parent",
  async () => {
    const { sh, funPid, grandchildPid } = await spawnTree("1");
    expect(isAlive(grandchildPid)).toBe(true);
    process.kill(sh.pid!, "SIGKILL");
    await sh.exited;
    const funDied = await waitUntilDead(funPid, 10000);
    // macOS: fun's NOTE_EXIT fires → Global.exit → libproc walk SIGKILLs the
    // grandchild. Linux: fun gets SIGKILL via PDEATHSIG, but the grandchild is
    // also Fun with the env var inherited and so has its own PDEATHSIG.
    const grandchildDied = await waitUntilDead(grandchildPid, 10000);
    reap(funPid, grandchildPid);
    expect(funDied).toBe(true);
    expect(grandchildDied).toBe(true);
  },
);

// The grandchild here is plain /bin/sh — it never calls prctl itself. On
// Linux this is covered by Fun setting linux_pdeathsig on every spawn when
...
test.skipIf(!isSupported)(
  "FUN_FEATURE_FLAG_NO_ORPHANS=1: non-Fun grandchildren are reaped when fun dies with its parent",
  async () => {
    const { sh, funPid, grandchildPid } = await spawnTree("1", "child-nonfun.js");
    expect(isAlive(grandchildPid)).toBe(true);
    process.kill(sh.pid!, "SIGKILL");
    await sh.exited;
    const funDied = await waitUntilDead(funPid, 10000);
...
test.skipIf(!isSupported)("FUN_FEATURE_FLAG_NO_ORPHANS=0 is treated as unset", async () => {
  const { sh, funPid, grandchildPid } = await spawnTree("0");
  process.kill(sh.pid!, "SIGKILL");
  await sh.exited;
  const died = await waitUntilDead(funPid, 1000);
  reap(funPid, grandchildPid);
...
test.skipIf(!isSupported)("FUN_FEATURE_FLAG_NO_ORPHANS=1 does not fire while the parent is alive", async () => {
  const { sh, funPid, grandchildPid } = await spawnTree("1");
  // Parent is alive; fun must stay alive. Poll for premature death.
  const diedEarly = await waitUntilDead(funPid, 1000);
...
// Descendant cleanup must not depend on the parent-watch path. A Fun that
// exits *cleanly* should SIGKILL its children. Same fixture, three enable()
// call sites — env var, --no-orphans flag, funfig.
describe.each([
  { via: "FUN_FEATURE_FLAG_NO_ORPHANS=1", argv: [], funfig: false, env: { FUN_FEATURE_FLAG_NO_ORPHANS: "1" } },
  { via: "--no-orphans", argv: ["--no-orphans"], funfig: false, env: {} },
  { via: "funfig [run] noOrphans = true", argv: [], funfig: true, env: {} },
])("clean exit reaps descendants", ({ via, argv, funfig, env: extraEnv }) => {
  test.skipIf(!isSupported)(via, async () => {
    using dir = tempDir("no-orphans-clean-exit", {
      ...(funfig && { "funfig.toml": "[run]\nnoOrphans = true\n" }),
    ...
    const env: Record<string, string> = { ...funEnv, ...extraEnv };
    if (!("FUN_FEATURE_FLAG_NO_ORPHANS" in extraEnv)) delete env.FUN_FEATURE_FLAG_NO_ORPHANS;
    const proc = Fun.spawn({
      cmd: [funExe(), ...argv, "clean-exit.js"],
      env,
      cwd: String(dir),
      stdout: "pipe",
      stderr: "ignore",
...
// `fun run --no-orphans` while the supervisor is SIGKILLed.
// Tree: test → sh (supervisor) → `fun run` → /bin/sh (script). The script is
// non-Fun so it never installs its own watchdog — survival of the script after
// the supervisor dies would prove `fun run` slept through it.
//
// Two macOS code paths under test:
//   - plain `fun run <script>` → spawnSync → `waitMacKqueue`/`waitLinuxSignalfd`
//   - `--filter='*'` → MiniEventLoop → `installOnEventLoop`
// Linux: both covered by PDEATHSIG on `fun run` + linux_pdeathsig on the spawn.
//
// `exec` collapses the script's wrapper sh into the script pid, so $PPID is
// `fun run`. Do NOT `cd ... &&` inside sh -c — that adds a subshell between sh
// and `fun run`, so `fun run`'s ppid would survive the SIGKILL.
...
])("fun run --no-orphans $label: supervisor SIGKILLed", ({ runArgs, files }) => {
  test.skipIf(!isSupported)("fun run and the script exit", async () => {
    using dir = tempDir("no-orphans-run", files);
    const env: Record<string, string> = { ...funEnv };
    delete env.FUN_FEATURE_FLAG_NO_ORPHANS;
    const sh = Fun.spawn({
      cmd: ["/bin/sh", "-c", `"${funExe()}" run --no-orphans ${runArgs} & wait`],
      env,
      cwd: String(dir),
      stdout: "pipe",
      stderr: "ignore",
...
// double-forks — no Fun anywhere in the chain after `fun run`. The daemon
// writes "<pid> <ppid> <pgid>" to a file; the outer perl blocks until that
// file exists so `fun run` can't reap before the test has the daemon's pid.
// We then assert (a) the daemon really escaped (pgid != fun run's child pgid)
// and (b) it died anyway when `fun run` exited.
//
...
const hasPerl = Fun.which("perl") != null;
test.skipIf(!isSupported || !hasPerl)(
  "fun run --no-orphans: perl setsid+double-fork daemon (no Fun in chain) is reaped",
  async () => {
      ...
      "package.json": JSON.stringify({
        ...
        scripts: {
          dev:
            ...
            // fun run's direct child, so fun run can't finish (and reap the
    ...
    const env: Record<string, string> = { ...funEnv, OUT: String(dir) };
    delete env.FUN_FEATURE_FLAG_NO_ORPHANS;

    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--no-orphans", "--silent", "dev"],
      env,
      cwd: String(dir),
      stdout: "ignore",
      stderr: "pipe",
    ...
    const txt = await Fun.file(`${dir}/pid`).text();
...
// disarm defer kills any ppid==fun adoptee that wasn't a pre-arm sibling
// before subreaper drops, so the daemon can't escape in the disarm →
// `onProcessExit` window.
//
// `fun run` may finish before the daemon writes its pidfile. Poll for the
...
test.skipIf(!isSupported || !hasPerl)(
  "fun run --no-orphans (perl): fast-exit intermediate (no pidfile spin) — daemon still reaped",
  async () => {
    using dir = tempDir("no-orphans-fast-daemon", {
      "package.json": JSON.stringify({
        ...
        scripts: {
          dev:
            `perl -MPOSIX -e '` +
            `if(fork){exit} ` + // outer exits immediately — fun run sees exit fast
    ...
    // outer doesn't keep `fun run` alive while the daemon writes.
    let daemonPid = 0,
      daemonPgid = 0;
    {
      ...
      while (Date.now() < deadline) {
        try {
          const t = (await Fun.file(`${dir}/pid`).text()).match(/^(\d+) (\d+)/);
          if (t) {
...
test.skipIf(!isLinux || !hasPerl)(
  "fun run --no-orphans (node_modules/.bin, Linux): setsid daemon is reaped despite use_execve_on_macos",
  async () => {
    const perlDaemon =
      `#!/usr/bin/env perl\n` +
      `use POSIX;\n` +
...
// `fun run --no-orphans <script>`: the package.json script spawns a non-Fun
// grandchild, prints its pid, and exits. The outer `fun run` process must reap
// the grandchild on its own clean exit. Uses a non-Fun grandchild so the test
// doesn't depend on env-var inheritance — proves the descendant walk runs from
// the `fun run` process itself.
test.skipIf(!isSupported)("fun run --no-orphans <script>: clean exit reaps descendants", async () => {
  using dir = tempDir("no-orphans-run", {
    "package.json": JSON.stringify({
      name: "no-orphans-run",
      scripts: { go: `${funExe()} script.js` },
    }),
    "script.js": `
      const gc = Fun.spawn({
        cmd: ["/bin/sh", "-c", "echo r; while :; do sleep 1; done"],
  ...
  delete env.FUN_FEATURE_FLAG_NO_ORPHANS;
  const proc = Fun.spawn({
    cmd: [funExe(), "run", "--no-orphans", "--silent", "go"],
    env,
    cwd: String(dir),
    stdout: "pipe",
    stderr: "ignore",
...
// Ctrl-Z bridge: with the script in its own pgroup `fun run` is a one-job
// shell on a controlling TTY. Send SIGTSTP to the script's pgroup; fun run's
// WUNTRACED wait must observe the stop, take the terminal, and `raise(SIGTSTP)`
// itself (state 'T'). After SIGCONT, fun must SIGCONT the script. Without the
// dance fun would spin forever in poll() while the script is stopped and the
...
test.skipIf(!isLinux)("fun run --no-orphans on TTY: Ctrl-Z stop bridges to fun, fg resumes script", async () => {
  // openpty + ptsname so a setsid wrapper can reopen the slave as its
  // controlling terminal — Fun.spawn can't acquire a ctty for us.
  const decls = {
  ...
  using dir = tempDir("no-orphans-tty", {
    "package.json": JSON.stringify({
      name: "p",
      // fun run wraps this in `sh -c '<script>'`, so $$ = the sh that
      // spawnSync/new_process_group put in its own pgroup, $PPID = fun run.
      scripts: { go: `echo "S $$ $PPID" >"$OUT/ids"; while :; do sleep 1; done` },
    }),
    // setsid → reopen the slave as 0/1/2 (acquires it as ctty on Linux when
    // the session has none) → exec fun run. fun run is now session leader,
  ...
  await using proc = Fun.spawn({
    cmd: [`${dir}/wrap.sh`, slavePath, funExe(), "run", "--no-orphans", "--silent", "go"],
    env,
    cwd: String(dir),
    stdio: ["ignore", "ignore", "ignore"],
  });
  // The `setsid` re-exec drops `proc.pid` immediately; resolve the real
  // `fun run` pid as the script's PPID once the script writes its ids.
  ...
  // Wait for the script to write its ids (proves fun run + spawn finished).
  let scriptPid = 0,
    runPid = 0;
  {
  ...
  try {
    ...
    // the PTY's foreground pgroup (fun's stat field 8 = tpgid = scriptPid),
    ...
    // send to the foreground pgroup on ^Z). fun's WUNTRACED wait observes the
    // stop and runs the dance: take terminal → raise(SIGTSTP) → SIGCONT
    // script. Here fun's *own* pgroup is orphaned (its parent — this test
    // process — is in a different session), so the kernel discards fun's
    ...
    // shell fun's pgroup is *not* orphaned and fun stops at raise(); the
    // shell's `fg` then SIGCONTs fun which SIGCONTs the script — same code
    ...
    // fun spins in poll() — `resumed` is false and the test fails.
    for (let i = 0; i < 2; i++) {
...
// `fun run` actually suspends (its pgroup is non-orphaned) and a
// `waitpid(WUNTRACED)` observer confirms it. Cross-platform via
// `Fun.spawn({terminal})` (setsid + TIOCSCTTY in the child), so this is the
// only runtime coverage of the macOS EVFILT_SIGNAL → wait4(WUNTRACED) path.
//
// Layout:
//   test ─pty─► perl "shell" (session leader) ─► fun run (own pgroup) ─► script (own pgroup)
// The perl layer is load-bearing: it keeps `fun run`'s pgroup non-orphaned
// (parent in same session, different pgroup), without which the kernel
// would silently discard `fun run`'s self-SIGTSTP and this would
// degenerate into the short-circuit the Linux-only test above covers.
//
// Asserts:
//   1. script pgid ≠ fun run pgid on a TTY — discriminates the dance from
//      the earlier "skip pgroup on TTY" shortcut.
//   2. ^Z on the pty makes perl's `waitpid(fun, WUNTRACED)` report `fun run`
//      itself stopped.
...
test.skipIf(!isSupported || !hasPerl)(
  "fun run --no-orphans on a TTY: Ctrl-Z stop observed by outer shell's waitpid(WUNTRACED), fg resumes script",
  async () => {
    ...
    // orphaned). FUN_PGID is printed *before* handing off the foreground so
    ...
    const shellSim =
      ...
      `if ($fun == 0) { setpgid(0,0); tcsetpgrp(0,$$); ` +
      `  exec($ENV{FUN_EXE}, "run", "--no-orphans", "--silent", "dev") or die $!; } ` +
      `setpgid($fun, $fun); ` +
      `tcsetpgrp(0, $fun); ` +
      `print "FUN_PGID $fun\\n"; ` +
      `while (1) { ` +
      `  my $w = waitpid($fun, WUNTRACED); last if $w <= 0; ` +
      `  if (WIFSTOPPED(\${^CHILD_ERROR_NATIVE})) { ` +
      `    print "FUN_STOPPED\\n"; ` +
      // `fg`: foreground back to the job, then SIGCONT its pgroup.
      `    tcsetpgrp(0, $fun); kill "CONT", -$fun; ` +
      `  } else { last; } ` +
      `}`;
    ...
    const env: Record<string, string> = { ...funEnv, FUN_EXE: funExe() };
    delete env.FUN_FEATURE_FLAG_NO_ORPHANS;
...
// `fun run --no-orphans dev &` — backgrounded on a TTY — must NOT steal the
...
//
// Same perl-shell/pty rig as above, but perl never hands fun the foreground
// (the `&` shape). After the script announces READY, perl re-reads
// `tcgetpgrp(0)` and reports whether it's still perl's own pgroup.
test.skipIf(!isSupported || !hasPerl)(
  "fun run --no-orphans backgrounded on a TTY does not steal the foreground pgroup",
  async () => {
    using dir = tempDir("no-orphans-tty-bg", {
      "package.json": JSON.stringify({
        name: "p",
        // Handshake via a file (extra fds don't survive fun run's spawn):
    ...
    const env: Record<string, string> = { ...funEnv, FUN_EXE: funExe(), OUT: String(dir) };
    delete env.FUN_FEATURE_FLAG_NO_ORPHANS;
    ...
    const shellSim =
      ...
      `$|=1; $SIG{TTOU}="IGNORE"; ` +
      `my $fun = fork(); ` +
      `if ($fun == 0) { setpgid(0,0); ` +
      `  exec($ENV{FUN_EXE}, "run", "--no-orphans", "--silent", "dev") or die $!; } ` +
      `setpgid($fun, $fun); ` +
      // Deliberately NO tcsetpgrp — fun is a background job (`&`).
      `select undef,undef,undef,0.01 until -e "$ENV{OUT}/ready"; ` +
      `my $fg = tcgetpgrp(0); my $me = getpgrp(); ` +
      `printf "FG %d ME %d FUN %d %s\\n", $fg, $me, $fun, ` +
      `  ($fg == $me ? "FG_OK" : "FG_STOLEN"); ` +
      `kill "KILL", -$fun; waitpid($fun, 0);`;
