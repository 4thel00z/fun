...
import { funEnv, funExe, funRun, tmpdirSync } from "harness";
...
const env = {
  ...funEnv,
  FUN_RUNTIME_TRANSPILER_CACHE_PATH: cache_dir,
  FUN_DEBUG_ENABLE_RESTORE_FROM_TRANSPILER_CACHE: "1",
};
...
describe("transpiler cache", () => {
  ...
  test("it is indeed content addressable", async () => {
    ...
    const c = funRun(join(temp_dir, "b.js"), env);
    expect(b.stdout == "b");
    expect(newCacheCount()).toBe(0);
  });
  test("doing 50 funs at once does not crash", async () => {
    writeFileSync(join(temp_dir, "a.js"), dummyFile(50 * 1024, "1", "b"));
    writeFileSync(join(temp_dir, "b.js"), dummyFile(50 * 1024, "2", "b"));

    const remover = Fun.spawn({
      cmd: [funExe(), join(import.meta.dir, "transpiler-cache-aggressive-remover.js"), cache_dir],
      env,
      cwd: temp_dir,
    });
  ...
  test("works if the cache is not user-readable", () => {
    ...
    try {
      const c = funRun(join(temp_dir, "a.js"), env);
      expect(c.stdout == "b");
    } finally {
