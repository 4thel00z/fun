// https://github.com/underdoc-org/fun/issues/30325
//
// `fun install` would hang indefinitely when an HTTPS registry connection
...
// configurable via `FUN_CONFIG_HTTP_IDLE_TIMEOUT` so this test can run in a
...
test("fun install times out when the registry accepts TCP but never completes the TLS handshake", async () => {
  ...
  try {
    ...
    await using proc = Fun.spawn({
      ...
      env: {
        ...
        FUN_CONFIG_HTTP_IDLE_TIMEOUT: "3",
