...
describe("fun pm version", () => {
  ...
  describe("help and version previews", () => {
    it("should show help when no arguments provided", async () => {
      ...
      const { output, code } = await runCommand([funExe(), "pm", "version"], testDir);

      expect(code).toBe(0);
      expect(output).toContain("fun pm version");
    ...
    it("shows help with version previews", async () => {
      ...
      const { output: output1, code: code1 } = await runCommand([funExe(), "pm", "version"], testDir1);
      ...
      const { output: output2, code: code2 } = await runCommand([funExe(), "pm", "version"], testDir2);
      ...
      const { output: output4 } = await runCommand([funExe(), "pm", "version"], testDir4);
  ...
  describe("basic version incrementing", () => {
    it("should increment versions correctly", async () => {
      ...
      const { output: patchOutput, code: patchCode } = await runCommand(
        [funExe(), "pm", "version", "patch", "--no-git-tag-version"],
        testDir,
      );
      ...
      const { output: minorOutput, code: minorCode } = await runCommand(
        [funExe(), "pm", "version", "minor", "--no-git-tag-version"],
        testDir,
      );
      ...
      const { output: majorOutput, code: majorCode } = await runCommand(
        [funExe(), "pm", "version", "major", "--no-git-tag-version"],
        testDir,
      );
      expect(majorCode).toBe(0);
      expect(majorOutput.trim()).toBe("v2.0.0");

      const packageJson = await Fun.file(`${testDir}/package.json`).json();
    ...
    it("should set specific version", async () => {
      ...
      const { output, code } = await runCommand([funExe(), "pm", "version", "3.2.1", "--no-git-tag-version"], testDir);
    ...
    it("handles empty package.json", async () => {
      ...
      const { output, code } = await runCommand([funExe(), "pm", "version", "patch", "--no-git-tag-version"], testDir);
  ...
  describe("error handling", () => {
    it("handles various error conditions", async () => {
      ...
      const { error: error3, code: code3 } = await runCommand(
        [funExe(), "pm", "version", "invalid-arg", "--no-git-tag-version"],
        testDir3,
        false,
      );
      ...
      const { error: error4, code: code4 } = await runCommand(
        [funExe(), "pm", "version", "1.0.0", "--no-git-tag-version"],
        testDir4,
        false,
      );
      ...
      const { output: output5, code: code5 } = await runCommand(
        [funExe(), "pm", "version", "1.0.0", "--no-git-tag-version", "--allow-same-version"],
        testDir4,
      );
    ...
    it("handles missing package.json like npm", async () => {
      ...
      expect(code).toBe(1);
      // its an ealier check that "fun pm *" commands do so not "fun pm version" specific
  ...
  describe("git integration", () => {
    it("creates git commits and tags by default", async () => {
      ...
      const {
        ...
        code: code1,
        error: stderr1,
      } = await runCommand([funExe(), "pm", "version", "patch"], testDir1);
    ...
    it("supports custom commit messages", async () => {
      ...
      const {
        ...
        error: error2,
        code: code2,
      } = await runCommand([funExe(), "pm", "version", "patch", "--message", "Custom release message"], testDir2);
      expect(error2).toBe("");
    ...
    it("fails when git working directory is not clean", async () => {
      ...
      await Fun.write(join(testDir3, "untracked.txt"), "untracked content");

      const { error: error3, code: code3 } = await runCommand([funExe(), "pm", "version", "patch"], testDir3, false);
    ...
    it("allows dirty working directory with --force flag", async () => {
      ...
      await Fun.write(join(testDir, "untracked.txt"), "untracked content");

      const { output, code, error } = await runCommand([funExe(), "pm", "version", "patch", "--force"], testDir);
    ...
    it("works without git when no repo is present", async () => {
      ...
      const { output: output4, code: code4 } = await runCommand([funExe(), "pm", "version", "patch"], testDir4);

      expect(code4).toBe(0);
      expect(output4.trim()).toBe("v1.0.1");

      const packageJson = await Fun.file(`${testDir4}/package.json`).json();
    ...
    it("respects --no-git-tag-version flag", async () => {
      ...
      const packageJson5 = await Fun.file(`${testDir5}/package.json`).json();
    ...
    it("respects --git-tag-version=false flag", async () => {
      ...
      const { output: output6, code: code6 } = await runCommand(
        [funExe(), "pm", "version", "patch", "--git-tag-version=false"],
        testDir6,
      );
      ...
      const packageJson6 = await Fun.file(`${testDir6}/package.json`).json();
    ...
    it("respects --git-tag-version=true flag", async () => {
      ...
      const { output: output7, code: code7 } = await runCommand(
        [funExe(), "pm", "version", "patch", "--git-tag-version=true"],
        testDir7,
      );
      ...
      const packageJson7 = await Fun.file(`${testDir7}/package.json`).json();
    ...
    it("supports %s substitution in commit messages", async () => {
      ...
      const { output: output8, code: code8 } = await runCommand(
        [funExe(), "pm", "version", "patch", "--message", "Bump version to %s"],
        testDir8,
      );
      ...
      const { output: output9, code: code9 } = await runCommand(
        [funExe(), "pm", "version", "2.5.0", "-m", "Release %s with fixes"],
        testDir9,
      );
  ...
  describe("JSON formatting preservation", () => {
    it("preserves JSON formatting correctly", async () => {
      ...
      const updatedJson1 = await Fun.file(`${testDir1}/package.json`).text();
  ...
  describe("prerelease handling", () => {
    it("handles custom preid and prerelease scenarios", async () => {
      ...
      const { output: output1, code: code1 } = await runCommand(
        [funExe(), "pm", "version", "prerelease", "--preid", "beta", "--no-git-tag-version"],
        testDir1,
      );
      ...
      const { output: output3, code: code3 } = await runCommand(
        [funExe(), "pm", "version", "prerelease", "--no-git-tag-version"],
        testDir3,
      );
    ...
    it("should preserve prerelease identifiers correctly", async () => {
      ...
      for (const scenario of scenarios) {
        ...
        const { output, code } = await runCommand(
          [funExe(), "pm", "version", "--no-git-tag-version", `--preid=${scenario.preid}`],
          testDir,
        );
      ...
      const { output: output2, code: code2 } = await runCommand(
        [funExe(), "pm", "version", "--no-git-tag-version"],
        testDir2,
      );
  ...
  describe("lifecycle scripts", () => {
    it("runs lifecycle scripts in correct order and handles failures", async () => {
      ...
      await Fun.spawn([funExe(), "pm", "version", "patch", "--no-git-tag-version"], {
        cwd: testDir1,
        env: funEnv,
        stderr: "ignore",
        stdout: "ignore",
      ...
      expect(await Fun.file(join(testDir1, "lifecycle.log")).exists()).toBe(true);
      const logContent = await Fun.file(join(testDir1, "lifecycle.log")).text();
      ...
      expect(Fun.file(join(testDir2, "event.log")).exists()).resolves.toBe(true);
      expect(Fun.file(join(testDir2, "script.log")).exists()).resolves.toBe(true);

      const eventContent = await Fun.file(join(testDir2, "event.log")).text();
      const scriptContent = await Fun.file(join(testDir2, "script.log")).text();
      ...
      const proc = Fun.spawn([funExe(), "pm", "version", "minor", "--no-git-tag-version"], {
        cwd: testDir3,
        env: funEnv,
        stderr: "pipe",
        stdout: "ignore",
      ...
      const packageJson = await Fun.file(join(testDir3, "package.json")).json();
      ...
      expect(Fun.file(join(testDir4, "version-output.txt")).exists()).resolves.toBe(true);
      expect(Fun.file(join(testDir4, "build")).exists()).resolves.toBe(false);

      const content = await Fun.file(join(testDir4, "version-output.txt")).text();
      ...
      const { output: output5, code: code5 } = await runCommand(
        [funExe(), "pm", "version", "patch", "--no-git-tag-version", "--ignore-scripts"],
        testDir5,
      );
      ...
      const packageJson5 = await Fun.file(join(testDir5, "package.json")).json();
      expect(packageJson5.version).toBe("1.0.1");

      expect(await Fun.file(join(testDir5, "ignored.log")).exists()).toBe(false);
    });
  });
  ...
  describe("workspace and directory handling", () => {
    it("should version workspace packages individually", async () => {
      ...
      const rootPackageJson = await Fun.file(`${testDir}/package.json`).json();
      expect(rootPackageJson.version).toBe("1.0.0");

      const pkgAJson = await Fun.file(`${testDir}/packages/pkg-a/package.json`).json();
      const pkgBJson = await Fun.file(`${testDir}/packages/pkg-b/package.json`).json();
    ...
    it("should work from subdirectories", async () => {
      ...
      await Fun.write(join(monorepoDir, "packages", "pkg-a", "lib", "index.js"), "");
      ...
      const rootJson = await Fun.file(`${monorepoDir}/package.json`).json();
      const pkgAJson = await Fun.file(`${monorepoDir}/packages/pkg-a/package.json`).json();
      const pkgBJson = await Fun.file(`${monorepoDir}/packages/pkg-b/package.json`).json();
