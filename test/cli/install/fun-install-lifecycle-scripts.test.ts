...
import { afterAll, beforeAll, describe, expect, setDefaultTimeout, test } from "fun:test";
...
// Tests in this file each spawn `fun install` which itself spawns multiple lifecycle
...
async function setupTest(): Promise<TestCtx> {
  ...
  let released = false;
  try {
    const { packageDir, packageJson } = await verdaccio.createTestDir({ funfigOpts: { linker: "hoisted" } });
    const env: Record<string, string> = {
      ...baseEnv,
      FUN_INSTALL_CACHE_DIR: join(packageDir, ".fun-cache"),
      FUN_TMPDIR: join(packageDir, ".fun-tmp"),
      TMPDIR: join(packageDir, ".fun-tmp"),
      TEMP: join(packageDir, ".fun-tmp"),
    };
    return {
      packageDir,
      packageJson,
...
for (const forceWaiterThread of isLinux ? [false, true] : [false]) {
  describe.concurrent("lifecycle scripts" + (forceWaiterThread ? " (waiter thread)" : ""), async () => {
    test("root package with all lifecycle scripts", async () => {
      ...
      const testEnv = forceWaiterThread ? { ...env, FUN_FEATURE_FLAG_FORCE_WAITER_THREAD: "1" } : env;
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "1.0.0",
          scripts: {
            preinstall: `${funExe()} preinstall.js`,
            install: `${funExe()} install.js`,
            postinstall: `${funExe()} postinstall.js`,
            preprepare: `${funExe()} preprepare.js`,
            prepare: `${funExe()} prepare.js`,
            postprepare: `${funExe()} postprepare.js`,
          },
        }),
      );
      ...
      assertManifestsPopulated(join(packageDir, ".fun-cache"), verdaccio.registryUrl());
      ...
      await rm(join(packageDir, "fun.lock"));
    ...
    test("adding a package without scripts to trustedDependencies", async () => {
      ...
      const what_bin_bins = !isWindows ? ["what-bin"] : ["what-bin.funx", "what-bin.exe"];
      // prettier-ignore
    ...
    test("failing root lifecycle script should print output correctly", async () => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "1.0.0",
          scripts: {
            preinstall: `${funExe()} -e "throw new Error('Oops!')"`,
          },
        }),
      );
    ...
    test("git dependencies also run `preprepare`, `prepare`, and `postprepare` scripts", async () => {
      ...
      expect(out.replace(/\s*\[[0-9\.]+m?s\]$/m, "").split(/\r?\n/)).toEqual([
        ...
        "1 package installed",
        "",
        "Blocked 6 postinstalls. Run `fun pm untrusted` for details.",
        "",
      ]);
      expect(await exited).toBe(0);
      assertManifestsPopulated(join(packageDir, ".fun-cache"), verdaccio.registryUrl());
    ...
    test("does not run any scripts if trustedDependencies is an empty list", async () => {
      ...
      expect(out.replace(/\s*\[[0-9\.]+m?s\]$/m, "").split(/\r?\n/)).toEqual([
        ...
        "3 packages installed",
        "",
        "Blocked 2 postinstalls. Run `fun pm untrusted` for details.",
        "",
      ]);
      expect(await exited).toBe(0);
      assertManifestsPopulated(join(packageDir, ".fun-cache"), verdaccio.registryUrl());
    ...
    test("file: dependency with default trusted name should run scripts when explicitly added to trustedDependencies", async () => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "1.0.0",
          scripts: {
            postinstall: `${funExe()} -e "require('fs').writeFileSync('postinstall-ran.txt', 'ran')"`,
          },
        }),
      );
    ...
    describe("--trust", async () => {
      test("unhoisted untrusted scripts, none at root node_modules", async () => {
        ...
        await runFunInstall(testEnv, packageDir);
        assertManifestsPopulated(join(packageDir, ".fun-cache"), verdaccio.registryUrl());
      ...
      for (const { label, packageJson: pkgJsonFixture } of trustTests) {
        test(label, async () => {
          ...
          let { stdout, stderr, exited } = spawn({
            cmd: [funExe(), "i", "--trust", "uses-what-bin@1.0.0"],
            cwd: packageDir,
            stdout: "pipe",
            stderr: "pipe",
            stdin: "ignore",
      ...
      describe("packages without lifecycle scripts", async () => {
        test("initial install", async () => {
          ...
          const { stdout, stderr, exited } = spawn({
            cmd: [funExe(), "i", "--trust", "no-deps@1.0.0"],
            cwd: packageDir,
            stdout: "pipe",
            stderr: "pipe",
            stdin: "ignore",
    ...
    test("fun pm trust and untrusted on missing package", async () => {
      using ctx = await setupTest();
      const { packageDir, packageJson, env } = ctx;
      const testEnv = forceWaiterThread ? { ...env, FUN_FEATURE_FLAG_FORCE_WAITER_THREAD: "1" } : env;
      ...
      // remove uses-what-bin from node_modules, fun pm trust and untrusted should handle missing package
      await rm(join(packageDir, "node_modules", "uses-what-bin"), { recursive: true, force: true });

      ({ stdout, stderr, exited } = spawn({
        cmd: [funExe(), "pm", "untrusted"],
        cwd: packageDir,
        stdout: "pipe",
        stderr: "pipe",
        env: testEnv,
      ...
      err = stderrForInstall(await stderr.text());
      expect(err).toContain("fun pm untrusted");
      expect(err).not.toContain("error:");
      expect(err).not.toContain("warn:");
      out = await stdout.text();
    ...
    describe("add trusted, delete, then add again", async () => {
      ...
      for (const withRm of [true, false]) {
        test(withRm ? "withRm" : "withoutRm", async () => {
          ...
          expected = [expect.stringContaining("fun install v1."), ...expected];
          expect(out.replace(/\s*\[[0-9\.]+m?s\]\s*$/, "").split(/\r?\n/)).toEqual(expected);
          expect(await exited).toBe(0);
          expect(await exists(join(packageDir, "node_modules", "uses-what-bin"))).toBe(!withRm);

          // add again, fun pm untrusted should report it as untrusted
    ...
    describe.if(!forceWaiterThread || process.platform === "linux")("does not use 100% cpu", async () => {
      test("install", async () => {
        ...
        await writeFile(
          ...
          JSON.stringify({
            ...
            version: "1.0.0",
            scripts: {
              preinstall: `${funExe()} -e 'Fun.sleepSync(1000)'`,
            },
          }),
        );
      ...
      // https://github.com/underdoc-org/fun/issues/11252
      test.todoIf(isWindows)("fun pm trust", async () => {
        using ctx = await setupTest();
        const { packageDir, packageJson, env } = ctx;
        const testEnv = forceWaiterThread ? { ...env, FUN_FEATURE_FLAG_FORCE_WAITER_THREAD: "1" } : env;
  ...
  describe.concurrent("stdout/stderr is inherited from root scripts during install", async () => {
    test("without packages", async () => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          scripts: {
            ...
            "prepare": `${exe} -e 'Fun.sleepSync(200); process.stdout.write("prepare stdout done ✅\\n")'`,
          },
        }),
      );
      ...
      expect(splitErrLines(err)).toEqual([
        ...
        `$ ${exe} -e 'Fun.sleepSync(200); process.stdout.write("prepare stdout done ✅\\n")'`,
        "",
      ]);
      const out = await stdout.text();
      expect(out.split(/\r?\n/)).toEqual([
  ...
  await Promise.all([
    write(
      ...
      JSON.stringify({
        ...
        scripts: {
          postinstall: `${funExe()} -e 'await Fun.write("postinstall.txt", "postinstall!!")'`,
        },
        trustedDependencies: ["uses-what-bin"],
      }),
    ),
