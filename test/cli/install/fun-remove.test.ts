...
it.concurrent("should remove existing package", async () => {
  ...
  try {
    ...
    const { exited: exited1 } = spawn({
      cmd: [funExe(), "add", `file:${pkg1_path}`.replace(/\\/g, "\\\\")],
      cwd: package_dir,
      stdout: "pipe",
      stdin: "pipe",
      stderr: "pipe",
    ...
    const { exited: exited2 } = spawn({
      cmd: [funExe(), "add", `file:${pkg2_path}`.replace(/\\/g, "\\\\")],
      cwd: package_dir,
      stdout: "pipe",
      stdin: "pipe",
      stderr: "pipe",
    ...
    expect(out1.replace(/\s*\[[0-9\.]+m?s\]/, "").split(/\r?\n/)).toEqual([
      expect.stringContaining("fun remove v1."),
      "",
...
it.concurrent("should not reject missing package", async () => {
  ...
  try {
    ...
    const { exited: addExited } = spawn({
      cmd: [funExe(), "add", `file:${pkg_path}`.replace(/\\/g, "\\\\")],
      cwd: package_dir,
      stdout: "pipe",
      stdin: "pipe",
      stderr: "pipe",
...
it.concurrent("should not affect if package is not installed", async () => {
  ...
  try {
    ...
    expect(out.split("\n")).toEqual([expect.stringContaining("fun remove v1."), ""]);
