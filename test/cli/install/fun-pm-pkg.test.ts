...
describe("fun pm pkg", () => {
  ...
  describe("set command", () => {
    ...
    it("should preserve file formatting", async () => {
      ...
      const modifiedContent = await Fun.file(join(testDir!, "package.json")).text();
  ...
  describe("help command", () => {
    it("should show help", async () => {
      ...
      expect(code).toBe(0);
      expect(output).toContain("fun pm pkg");
  ...
  describe("deeply nested directory scenarios", () => {
    ...
    it("should handle modifications from deeply nested directories", async () => {
      ...
      const pkgContent = await Fun.file(join(nestedDir, "package.json")).json();
  ...
  describe("fix command", () => {
    ...
    it("should not modify package.json if no fixes are needed", async () => {
      ...
      try {
        const beforeContent = await Fun.file(join(goodDir, "package.json")).text();
        const { code } = await runPmPkg(["fix"], goodDir);
        expect(code).toBe(0);

        const afterContent = await Fun.file(join(goodDir, "package.json")).text();
  ...
  // npm does the actual "" key, but fun right now doesn't support it
