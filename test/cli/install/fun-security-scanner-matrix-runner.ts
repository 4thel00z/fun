import { funEnv, funExe, isWindows, runFunInstall, tempDirWithFiles } from "harness";
...
const TESTS_TO_SKIP: Set<string> = new Set<TestName>([
  // https://github.com/underdoc-org/fun/issues/22255
  // remove "is-even"
...
interface SecurityScannerTestOptions {
  ...
  scannerType: "local" | "npm" | "npm.funfigonly";
...
async function globEverything(dir: string) {
  return await Array.fromAsync(
    new Fun.Glob("**/*").scan({ cwd: dir, dot: true, followSymlinks: false, onlyFiles: false }),
  );
}
...
async function runSecurityScannerTest(options: SecurityScannerTestOptions) {
  ...
  // First write funfig WITHOUT scanner for pre-install
  await Fun.write(
    join(dir, "funfig.toml"),
    `[install]
cache.disable = true
linker = "${linker}"
registry = "${registryUrl}/"`,
  ...
  if (hasExistingNodeModules || hasLockfile) {
    if (DO_TEST_DEBUG) console.log(redShellPrefix, `${funExe()} install`);
    await runFunInstall(funEnv, dir);
  }
  ...
  if (shouldDoInitialInstall && !hasLockfile) {
    if (DO_TEST_DEBUG) console.log(redShellPrefix, `rm ${dir}/fun.lock`);
    await rm(join(dir, "fun.lock"));
  }

  ////////////////////////// POST SETUP DONE //////////////////////////

  const cmd = [funExe(), command, ...args];
  ...
  // write the full funfig WITH scanner configuration
  await Fun.write(
    join(dir, "funfig.toml"),
    `[install]
cache.disable = true
...
scanner = "${scannerPath}"`,
  ...
  if (DO_TEST_DEBUG) {
    ...
    console.log("");
    console.log("funfig.toml contents:");
    console.log(await Fun.file(join(dir, "funfig.toml")).text());
    console.log("");
    console.log("package.json contents:");
    console.log(await Fun.file(join(dir, "package.json")).text());
    console.log("");
  ...
  if (hasTTY) {
    ...
    await using proc = Fun.spawn(cmd, {
      cwd: dir,
      env: funEnv,
      terminal,
    });
  ...
  // then we should expect Fun to do the partial install first of all
  ...
  if (scannerType === "npm.funfigonly") {
    expect(errAndOut).toContain("");
  }

  if (scannerType !== "npm.funfigonly" && !scannerSyncronouslyThrows) {
  ...
  if (scannerType !== "npm.funfigonly" && !hasExistingNodeModules) {
    switch (scannerReturns) {
      case "fatal": {
        // Fatal advisories always cancel installation
        expect(await Fun.file(join(dir, "node_modules", "left-pad", "package.json")).exists()).toBe(false);
        break;
      }
      ...
      case "warn": {
        if (hasTTY && ttyResponse === "y") {
          ...
          if (command === "remove" || command === "uninstall") {
            ...
            expect(await Fun.file(join(dir, "node_modules", "left-pad", "package.json")).exists()).toBe(true);
          }
        } else {
        ...
        switch (command) {
          ...
          case "uninstall": {
            for (const arg of args) {
              switch (linker) {
                case "hoisted": {
                  expect(await Fun.file(join(dir, "node_modules", arg, "package.json")).exists()).toBe(false);
                  break;
                }
                ...
                case "isolated": {
                  ...
                  expect(await Fun.file(path).exists()).toBe(false);
                  break;
                }
              }
            }
          ...
          default: {
            for (const arg of args) {
              switch (linker) {
                case "hoisted": {
                  expect(await Fun.file(join(dir, "node_modules", arg, "package.json")).exists()).toBe(true);
                  break;
                }
                ...
                case "isolated": {
                  ...
                  expect(await Fun.file(path).exists()).toBe(true);
                  break;
                }
              }
            }
...
export function runSecurityScannerTests(selfModuleName: string, hasExistingNodeModules: boolean) {
  ...
  const { describe, beforeAll, afterAll, test } = Fun.jest(selfModuleName);
  ...
  describe.each(["install", "update", "add", "remove", "uninstall"] as const)("fun %s", command => {
    describe.each([
      { args: [], name: "no args" },
      ...
      describe.each(["hoisted", "isolated"] as const)("--linker=%s", linker => {
        describe.each(["local", "npm", "npm.funfigonly"] as const)("(scanner: %s)", scannerType => {
          describe.each([true, false] as const)("(fun.lock exists: %p)", hasLockfile => {
            describe.each(["none", "warn", "fatal"] as const)("(advisories: %s)", scannerReturns => {
              ...
              describe.each(applicableTtyConfigs)("($ttyLabel)", ({ hasTTY, ttyResponse }) => {
                ...
                // npm.funfigonly is the case where a scanner is a valid npm package name identifier
                // but is not referenced in package.json anywhere and is not in the lockfile, so the only knowledge
                // of this package's existence is the fact that it was defined in as the value in funfig.toml
                // Therefore, we should fail because we don't know where to install it from
                const shouldFail =
                  scannerType === "npm.funfigonly" ||
                  scannerReturns === "fatal" ||
