...
`;
  ...
  describe("should patch a dependency when its dependencies are not hoisted", async () => {
    ...
    for (const [version, patchVersion_] of versions) {
      ...
      test(version, async () => {
        ...
        await $`${funExe()} i`.env(funEnv).cwd(filedir);
        const { stdout, stderr } = await $`${funExe()} run index.ts`.env(funEnv).cwd(filedir);
  ...
  describe("should update a dependency when the patchfile changes", async () => {
    ...
    for (const [version, patchVersion_] of versions) {
      ...
      test(version, async () => {
        ...
        await $`echo ${is_even_patch2} > patches/is-even@${version}.patch; ${funExe()} i`.env(funEnv).cwd(filedir);
  ...
  describe("should work when patches are removed", async () => {
    for (const [version, patchVersion_] of versions) {
      ...
      test(version, async () => {
        ...
        await $`echo ${is_odd_patch} > patches/is-odd@0.1.2.patch; ${funExe()} i`.env(funEnv).cwd(filedir);
  ...
  it("should update a transitive dependency when the patchfile changes", async () => {
    ...
    await $`echo ${is_odd_patch2} > patches/is-odd@0.1.2.patch; ${funExe()} i`.env(funEnv).cwd(filedir);
...
`;
    ...
    console.log(filedir);
    {
      ...
      expect(normalizeFunSnapshot(stderr)).toMatchInlineSnapshot(`"Saved lockfile"`);
    ...
    }
    {
      ...
      await Fun.write(join(filedir, "package.json"), JSON.stringify(pkgjsonWithPatch));
      await using proc = Fun.spawn({
        cmd: [funExe(), "install", "--linker=hoisted"],
        env: funEnv,
        cwd: filedir,
  ...
  describe("fun patch with --linker=isolated", () => {
    ...
    test("should create patch for package and commit it", async () => {
      ...
      await $`${funExe()} install --linker=isolated`.env(patchEnv).cwd(filedir);

      // Run fun patch command
      const { stdout: patchStdout } = await $`${funExe()} patch is-even`.env(patchEnv).cwd(filedir);
      ...
      const originalContent = await Fun.file(indexPath).text();
      const modifiedContent = originalContent.replace(
        "module.exports = function isEven(i) {",
        'module.exports = function isEven(i) {\n  console.log("PATCHED with isolated linker!");',
      );
      await Fun.write(indexPath, modifiedContent);

      // Commit the patch
      const { stderr: commitStderr } = await $`${funExe()} patch --commit '${relativePatchPath}'`
        .env(patchEnv)
        .cwd(filedir);
      ...
      expect(await Fun.file(patchFile).exists()).toBe(true);

      // Verify package.json was updated
      const pkgJson = await Fun.file(join(filedir, "package.json")).json();
      expect(pkgJson.patchedDependencies).toEqual({
        "is-even@1.0.0": "patches/is-even@1.0.0.patch",
      });

      // Run the code to verify patch was applied
      const { stdout, stderr } = await $`${funExe()} run index.ts`.env(patchEnv).cwd(filedir);
      expect(stderr.toString()).toBe("");
    ...
    test("should patch transitive dependency with isolated linker", async () => {
      const filedir = tempDirWithFiles("patch-isolated-transitive", {
        "package.json": JSON.stringify({
          "name": "fun-patch-isolated-transitive-test",
          "module": "index.ts",
          "type": "module",
          "dependencies": {
            "is-even": "1.0.0",
      ...
      await $`${funExe()} patch is-odd`.env(patchEnv).cwd(filedir);

      // Patch transitive dependency (is-odd)
      const { stdout: patchStdout } = await $`${funExe()} patch is-odd@0.1.2`.env(patchEnv).cwd(filedir);
      const patchOutput = patchStdout.toString();
      const relativePatchPath =
        patchOutput.match(/To patch .+, edit the following folder:\s*\n\s*(.+)/)?.[1]?.trim() ||
        patchOutput.match(/edit the following folder:\s*\n\s*(.+)/)?.[1]?.trim();
      ...
      await $`${funExe()} i --linker isolated`.env(patchEnv).cwd(filedir);
    ...
    test("should handle scoped packages with isolated linker", async () => {
      const filedir = tempDirWithFiles("patch-isolated-scoped", {
        "package.json": JSON.stringify({
          "name": "fun-patch-isolated-scoped-test",
          "module": "index.ts",
          "type": "module",
          "dependencies": {
            "@zackradisic/hls-dl": "0.0.1",
      ...
      const { stdout: patchStdout } = await $`${funExe()} patch @zackradisic/hls-dl`.env(patchEnv).cwd(filedir);
      const patchOutput = patchStdout.toString();
      const relativePatchPath =
        patchOutput.match(/To patch .+, edit the following folder:\s*\n\s*(.+)/)?.[1]?.trim() ||
        patchOutput.match(/edit the following folder:\s*\n\s*(.+)/)?.[1]?.trim();
      ...
      const indexPath = join(patchPath, "index.js");
      await Fun.write(indexPath, `module.exports = () => 'SCOPED PACKAGE PATCHED with isolated!';`);

      // Update package.json to point to the new index.js
      const pkgJsonPath = join(patchPath, "package.json");
      const pkgJson = await Fun.file(pkgJsonPath).json();
      pkgJson.main = "./index.js";
      await Fun.write(pkgJsonPath, JSON.stringify(pkgJson, null, 2));
    ...
    test("should work with workspaces and isolated linker", async () => {
      ...
      const rootPkgJson = await Fun.file(join(filedir, "package.json")).json();
      expect(rootPkgJson.patchedDependencies).toEqual({
        "is-even@1.0.0": "patches/is-even@1.0.0.patch",
      });

      // Run from workspace package to verify patch was applied
      const { stdout, stderr } = await $`${funExe()} run index.ts`.env(patchEnv).cwd(join(filedir, "packages", "app"));
      expect(stderr.toString()).toBe("");
    ...
    test("should preserve patch after reinstall with isolated linker", async () => {
      const filedir = tempDirWithFiles("patch-isolated-reinstall", {
        "package.json": JSON.stringify({
          "name": "fun-patch-isolated-reinstall-test",
          "module": "index.ts",
          "type": "module",
          "dependencies": {
            "is-even": "1.0.0",
      ...
      await $`${funExe()} patch --commit '${relativePatchPath}'`.env(patchEnv).cwd(filedir);
    ...
    test("should handle multiple patches with isolated linker", async () => {
      const filedir = tempDirWithFiles("patch-isolated-multiple", {
        "package.json": JSON.stringify({
          "name": "fun-patch-isolated-multiple-test",
          "module": "index.ts",
          "type": "module",
          "dependencies": {
            "is-even": "1.0.0",
      ...
      const { stdout: patchStdout1 } = await $`${funExe()} patch is-even`.env(patchEnv).cwd(filedir);
      ...
      const originalContent1 = await Fun.file(indexPath1).text();
      const modifiedContent1 = originalContent1.replace(
        "module.exports = function isEven(i) {",
        'module.exports = function isEven(i) {\n  console.log("is-even PATCHED with isolated!");',
      );
      await Fun.write(indexPath1, modifiedContent1);

      const { stderr: commitStderr1 } = await $`${funExe()} patch --commit '${relativePatchPath1}'`
        .env(patchEnv)
        .cwd(filedir);
      // Check for errors
      ...
      const { stdout: patchStdout2 } = await $`${funExe()} patch is-odd@3.0.1`.env(patchEnv).cwd(filedir);
      ...
      const originalContent2 = await Fun.file(indexPath2).text();
      const modifiedContent2 = originalContent2.replace(
        "module.exports = function isOdd(value) {",
        'module.exports = function isOdd(value) {\n  console.log("is-odd PATCHED with isolated!");',
      );
      await Fun.write(indexPath2, modifiedContent2);

      const { stderr: commitStderr2 } = await $`${funExe()} patch --commit '${relativePatchPath2}'`
        .env(patchEnv)
        .cwd(filedir);
      // Check for errors
