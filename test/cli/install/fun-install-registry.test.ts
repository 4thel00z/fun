...
import { afterAll, beforeEach, describe, expect, setDefaultTimeout, test } from "fun:test";
...
afterAll(async () => {
  await Fun.$`rm -f ${import.meta.dir}/htpasswd`.throws(false);
  registry.stop();
});

beforeEach(async () => {
  ({ packageDir, packageJson } = await registry.createTestDir({
    funfigOpts: { saveTextLockfile: false, linker: "hoisted" },
  }));
  await Fun.$`rm -f ${import.meta.dir}/htpasswd`.throws(false);
  await Fun.$`rm -rf ${import.meta.dir}/packages/private-pkg-dont-touch`.throws(false);
  users = {};
  env.FUN_INSTALL_CACHE_DIR = join(packageDir, ".fun-cache");
  env.FUN_TMPDIR = env.TMPDIR = env.TEMP = join(packageDir, ".fun-tmp");
});
...
describe("auto-install", () => {
  test("symlinks (and junctions) are created correctly in the install cache", async () => {
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "--print", "require('is-number')"],
      cwd: packageDir,
      stdout: "pipe",
      stderr: "pipe",
      env: {
        ...env,
        FUN_INSTALL_CACHE_DIR: join(packageDir, ".fun-cache"),
      },
    });
    ...
    expect(resolve(await readlink(join(packageDir, ".fun-cache", "is-number", "2.0.0@@localhost@@@1")))).toBe(
      join(packageDir, ".fun-cache", "is-number@2.0.0@@localhost@@@1"),
    );
  });
});
...
describe("certificate authority", () => {
  const mockRegistryFetch = function (opts?: any): (req: Request) => Promise<Response> {
    return async function (req: Request) {
      if (req.url.includes("no-deps")) {
        return new Response(Fun.file(join(import.meta.dir, "registry", "packages", "no-deps", "no-deps-1.0.0.tgz")));
      }
  ...
  test("valid --cafile", async () => {
    ...
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "install", "--cafile", "cafile"],
      cwd: packageDir,
      stderr: "pipe",
      stdout: "pipe",
      env,
  ...
  test("valid --ca", async () => {
    ...
    ({ stdout, stderr, exited } = spawn({
      cmd: [funExe(), "install", "--ca", tls.cert],
      cwd: packageDir,
      stderr: "pipe",
      stdout: "pipe",
      env,
  ...
  test(`non-existent --cafile`, async () => {
    ...
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "install", "--cafile", "does-not-exist"],
      cwd: packageDir,
      stderr: "pipe",
      stdout: "pipe",
      env,
  ...
  test("cafile from funfig does not exist", async () => {
    await Promise.all([
      write(
        packageJson,
        JSON.stringify({
...
-----END CERTIFICATE-----`,
    ...
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "install", "--cafile", join(packageDir, "invalid-cafile")],
      cwd: packageDir,
      stderr: "pipe",
      stdout: "pipe",
      env,
  ...
  test("invalid --ca", async () => {
    ...
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "install", "--ca", "not-valid"],
      cwd: packageDir,
      stderr: "pipe",
      stdout: "pipe",
      env,
...
describe("whoami", async () => {
  test("can get username", async () => {
    const funfig = await registry.authBunfig("whoami");
    await Promise.all([
      write(
        packageJson,
        JSON.stringify({
      ...
      write(join(packageDir, "funfig.toml"), funfig),
    ]);

    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "pm", "whoami"],
      cwd: packageDir,
      stdout: "pipe",
      stderr: "pipe",
      env,
  ...
  test("not logged in", async () => {
    ...
    const err = await stderr.text();
    expect(err).toBe("error: missing authentication (run `funx npm login`)\n");
    expect(await exited).toBe(1);
  });
  test("invalid token", async () => {
    ...
    await rm(join(packageDir, "funfig.toml"));
    await Promise.all([
      write(packageJson, JSON.stringify({ name: "whoami-pkg", version: "1.1.1" })),
      write(join(packageDir, "funfig.toml"), funfig),
    ]);
...
describe("package.json indentation", async () => {
  test("works for root and workspace packages", async () => {
    ...
    expect(await exited).toBe(0);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
...
describe("text lockfile", () => {
  test("workspace sorting", async () => {
    ...
    let { exited } = spawn({
      cmd: [funExe(), "install", "--save-text-lockfile"],
      cwd: packageDir,
      stdout: "ignore",
      stderr: "ignore",
      env,
    ...
    expect(
      (await Fun.file(join(packageDir, "fun.lock")).text()).replaceAll(/localhost:\d+/g, "localhost:1234"),
    ).toMatchSnapshot();
    ...
    const lockfile = await Fun.file(join(packageDir, "fun.lock")).text();
  ...
  test("--frozen-lockfile", async () => {
    ...
    const firstLockfile = await Fun.file(join(packageDir, "fun.lock")).text();
    ...
    expect(await Fun.file(join(packageDir, "fun.lock")).text()).toBe(firstLockfile);
  });
  ...
  for (const omit of ["dev", "peer", "optional"]) {
    test(`resolvable lockfile with ${omit} dependencies disabled`, async () => {
      ...
      let { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "install", "--save-text-lockfile", `--omit=${omit}`],
        cwd: packageDir,
        stdout: "pipe",
        stderr: "pipe",
        env,
      ...
      const lockfile = (await Fun.file(join(packageDir, "fun.lock")).text()).replaceAll(
        /localhost:\d+/g,
        "localhost:1234",
      );
      ...
      expect((await Fun.file(join(packageDir, "fun.lock")).text()).replaceAll(/localhost:\d+/g, "localhost:1234")).toBe(
        lockfile,
      );
    });
  }
  ...
  test("optionalPeers", async () => {
    ...
    const firstLockfile = (await Fun.file(join(packageDir, "fun.lock")).text()).replaceAll(
      /localhost:\d+/g,
      "localhost:1234",
    );
...
test("--lockfile-only", async () => {
  ...
  let { exited } = spawn({
    cmd: [funExe(), "install", "--save-text-lockfile", "--lockfile-only"],
    cwd: packageDir,
    stdout: "ignore",
    stderr: "ignore",
    env,
  ...
  ({ exited } = spawn({
    cmd: [funExe(), "install", "--lockfile-only"],
    cwd: packageDir,
    stdout: "ignore",
    stderr: "ignore",
    env,
  ...
  const {
    ...
    cmd: [funExe(), "install", "--lockfile-only", "--silent"],
    cwd: packageDir,
    stdout: "pipe",
    stderr: "pipe",
    env,
...
describe("bundledDependencies", () => {
  for (const textLockfile of [true, false]) {
    test(`(${textLockfile ? "fun.lock" : "fun.lockb"}) basic`, async () => {
      await Promise.all([
        write(
          packageJson,
          JSON.stringify({
      ...
      const cmd = textLockfile ? [funExe(), "install", "--save-text-lockfile"] : [funExe(), "install"];
      let { exited } = spawn({
        cmd,
        cwd: packageDir,
        stdout: "ignore",
    ...
    test(`(${textLockfile ? "fun.lock" : "fun.lockb"}) bundledDependencies === true`, async () => {
      await Promise.all([
        write(
          packageJson,
          JSON.stringify({
    ...
    test(`(${textLockfile ? "fun.lock" : "fun.lockb"}) transitive bundled dependency collision`, async () => {
    ...
    test(`(${textLockfile ? "fun.lock" : "fun.lockb"}) git dependencies`, async () => {
      await Promise.all([
        write(
          packageJson,
          JSON.stringify({
        ...
        write(
          ...
          `
[install]
cache = "${join(packageDir, ".fun-cache")}"
          `,
        ),
      ]);
    ...
    test(`(${textLockfile ? "fun.lock" : "fun.lockb"}) workspace dependencies bundle correctly`, async () => {
      await Promise.all([
        write(
          packageJson,
          JSON.stringify({
...
describe("optionalDependencies", () => {
  for (const optional of [true, false]) {
    test(`exit code is ${optional ? 0 : 1} when ${optional ? "optional" : ""} dependency tarball is missing`, async () => {
      ...
      const { exited, err } = await runFunInstall(env, packageDir, {
  ...
  for (const rootOptional of [true, false]) {
    test(`exit code is 0 when ${rootOptional ? "root" : ""} optional dependency does not exist in registry`, async () => {
      ...
      const { err } = await runFunInstall(env, packageDir, {
        allowWarnings: true,
  ...
  test("should not install optional deps if false in funfig", async () => {
    await writeFile(
      join(packageDir, "funfig.toml"),
      `
  [install]
  ...
  test("lifecycle scripts failures from transitive dependencies are ignored", async () => {
    ...
    const { err, exited } = await runFunInstall(env, packageDir);
    expect(err).not.toContain("error:");
...
describe.each(["--production", "without --production"])("%s", flag => {
  ...
  // const stdio = process.versions.fun.includes("debug") ? "inherit" : "ignore";
  const stdio = "ignore";
  ...
  if (prod) {
    test("modifying package.json with --production should not save lockfile", async () => {
      ...
      const initialHash = Fun.hash(await file(join(packageDir, "fun.lockb")).arrayBuffer());
      ...
      var { exited } = spawn({
        cmd: [funExe(), "install", "--production", "bin-change-dir@1.0.1"],
        cwd: packageDir,
        stdout: stdio,
        stdin: stdio,
        stderr: stdio,
      ...
      // We should not have saved fun.lockb
      expect(Fun.hash(await file(join(packageDir, "fun.lockb")).arrayBuffer())).toBe(initialHash);
      ...
      var { exited } = spawn({
        cmd: [funExe(), "install", "--production", "bin-change-dir@1.0.0"],
        cwd: packageDir,
        stdout: stdio,
        stdin: stdio,
        stderr: stdio,
  ...
  test(`should prefer ${order[+prod % 2]} over ${order[1 - (+prod % 2)]}`, async () => {
    ...
    async function saveWithoutProd() {
      ...
      await Promise.all([
        ...
        (async () => (hash = Fun.hash(await file(join(packageDir, "fun.lockb")).arrayBuffer())))(),
      ]);
    ...
    var { exited } = spawn({
      cmd: [funExe(), "install", prod ? "--production" : ""].filter(Boolean),
      cwd: packageDir,
      stdout: stdio,
      stdin: stdio,
      stderr: stdio,
    ...
    if (!prod) {
      ...
      const newHash = Fun.hash(await file(join(packageDir, "fun.lockb")).arrayBuffer());
...
test("manifest cache will invalidate when registry changes", async () => {
  const cacheDir = join(packageDir, ".fun-cache");
  await Promise.all([
    write(
      join(packageDir, "funfig.toml"),
      `
...
saveTextLockfile = false
  ...
  await Promise.all([
    ...
    rm(join(packageDir, "fun.lockb"), { force: true }),
    write(
      join(packageDir, "funfig.toml"),
      `
[install]
    ...
    for (const secondDep of dependencies) {
      ...
      test(`replacing ${firstDep} with ${secondDep}`, async () => {
        ...
        await Promise.all([
          ...
          rm(join(packageDir, ".fun-cache"), { recursive: true, force: true }),
          write(
            packageJson,
            JSON.stringify({
              name: "foo",
...
test("--production excludes devDependencies in workspaces", async () => {
  ...
  let { out } = await runFunInstall(env, packageDir, { production: true });
  assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  await rm(join(packageDir, "node_modules"), { recursive: true, force: true });
  ({ out } = await runFunInstall(env, packageDir));
  assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  await rm(join(packageDir, "node_modules"), { recursive: true, force: true });
  ({ out } = await runFunInstall(env, packageDir, { production: true }));
  assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
...
describe("binaries", () => {
  ...
  test("it should correctly link binaries after deleting node_modules", async () => {
    ...
    expect(out.replace(/\s*\[[0-9\.]+m?s\]$/m, "").split(/\r?\n/)).toEqual([
      ...
      "3 packages installed",
      "",
      "Blocked 1 postinstall. Run `fun pm untrusted` for details.",
      "",
    ]);
    expect(await exited).toBe(0);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("will only link global binaries for requested packages", async () => {
    ...
    let { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "i", "--linker=hoisted", "-g", `--config=${join(packageDir, "funfig.toml")}`, "uses-what-bin"],
      cwd: packageDir,
      stdout: "pipe",
      stderr: "pipe",
      env: { ...env, FUN_INSTALL: join(packageDir, "global-install-dir") },
    });
    ...
    ({ stdout, stderr, exited } = spawn({
      cmd: [funExe(), "i", "--linker=hoisted", "-g", `--config=${join(packageDir, "funfig.toml")}`, "what-bin"],
      cwd: packageDir,
      stdout: "pipe",
      stderr: "pipe",
      env: { ...env, FUN_INSTALL: join(packageDir, "global-install-dir") },
    ...
    if (isWindows) {
      expect(
        await Promise.all([
          ...
          exists(join(packageDir, "global-bin-dir", "what-bin.funx")),
        ]),
      ).toEqual([true, true]);
    } else {
  ...
  for (const global of [false, true]) {
    test(`bin types${global ? " (global)" : ""}`, async () => {
      ...
      const args = [
        ...
        "--linker=hoisted",
        ...(global ? ["-g"] : []),
        ...(global ? [`--config=${join(packageDir, "funfig.toml")}`] : []),
        "dep-with-file-bin",
      ...
      const { stdout, stderr, exited } = spawn({
        ...
        stdout: "pipe",
        stderr: "pipe",
        env: global ? { ...env, FUN_INSTALL: join(packageDir, "global-install-dir") } : env,
      });
  ...
  test.todo("text lockfile updates with new bin entry for folder dependencies", async () => {
    ...
    const secondLockfile = (await Fun.file(join(packageDir, "fun.lock")).text()).replaceAll(
      /localhost:\d+/g,
      "localhost:1234",
    );
  ...
  async function runBin(binName: string, expected: string, global: boolean) {
    const args = global ? [`./global-bin-dir/${binName}`] : [funExe(), binName];
...
`,
  ...
  ({ exited } = spawn({
    cmd: [funExe(), "i", "--config=funfig2.toml"],
    cwd: packageDir,
    stdout: "pipe",
    stderr: "pipe",
    env,
...
test("it should invalid cached package if package.json is missing", async () => {
  ...
  let { out } = await runFunInstall(env, packageDir);
  ...
  expect(
    await Promise.all([
      ...
      readdirSorted(join(packageDir, ".fun-cache", "no-deps@2.0.0@@localhost@@@1")),
    ]),
  ).toEqual([
  ...
  ({ out } = await runFunInstall(env, packageDir, { savesLockfile: false }));
  expect(out).toContain("+ no-deps@2.0.0");
  ...
  await rm(join(packageDir, ".fun-cache", "no-deps@2.0.0@@localhost@@@1", "package.json"));
  ({ out } = await runFunInstall(env, packageDir, { savesLockfile: false }));
  expect(out).not.toContain("+ no-deps@2.0.0");
  expect(
    await Promise.all([
...
test("it should install with missing fun.lockb, node_modules, and/or cache", async () => {
  // first clean install
  await writeFile(
    packageJson,
    JSON.stringify({
  ...
  for (var i = 0; i < 100; i++) {
    ...
    // delete fun.lockb
    await rm(join(packageDir, "fun.lockb"), { recursive: true, force: true });
  ...
  // delete cache
  await rm(join(packageDir, ".fun-cache"), { recursive: true, force: true });
...
describe("hoisting", async () => {
  ...
  for (const { dependencies, expected, situation } of tests) {
    test(`it should hoist ${expected} when ${situation}`, async () => {
      ...
      await rm(join(packageDir, "fun.lockb"));
...
describe("transitive file dependencies", () => {
  ...
  test("from hoisted workspace dependencies", async () => {
    ...
    var { out } = await runFunInstall(env, packageDir);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    ({ out } = await runFunInstall(env, join(packageDir, "pkg1")));
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    ({ out } = await runFunInstall(env, join(packageDir, "pkg1"), { savesLockfile: false }));
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
...
describe("update", () => {
  test("duplicate peer dependency (one package is invalid_package_id)", async () => {
    ...
    await runFunUpdate(env, packageDir);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("dist-tags", async () => {
    ...
    await runFunUpdate(env, packageDir, ["a-dep"]);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    await runFunUpdate(env, packageDir, ["--latest"]);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("exact versions stay exact", async () => {
    ...
    for (const { version, dependency } of runs) {
      ...
      await runFunUpdate(env, packageDir, [dependency]);
      await check(version);
  ...
  describe("tilde", () => {
    test("without args", async () => {
      ...
      let { out } = await runFunUpdate(env, packageDir);
      assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());

      expect(out).toEqual([
        expect.stringContaining("fun update v1."),
        "",
      ...
      ({ out } = await runFunUpdate(env, packageDir));
      assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    test("with package name in args", async () => {
      ...
      let { out } = await runFunUpdate(env, packageDir, ["no-deps"]);
      assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
      ...
      ({ out } = await runFunUpdate(env, packageDir, ["no-deps", "--latest"]));
      assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  describe("alises", () => {
    ...
    test("update specific aliased package", async () => {
      ...
      await runFunUpdate(env, packageDir, ["aliased-dep"]);
      assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    test("with pre and build tags", async () => {
      ...
      const { out } = await runFunUpdate(env, packageDir, ["--latest"]);
      assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("--no-save will update packages in node_modules and not save to package.json", async () => {
    ...
    let { out } = await runFunUpdate(env, packageDir, ["--no-save"]);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    ({ out } = await runFunUpdate(env, packageDir, ["--no-save"]));
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("update won't update beyond version range unless the specified version allows it", async () => {
    ...
    await runFunUpdate(env, packageDir, ["dep-with-tags"]);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    await runFunUpdate(env, packageDir, ["dep-with-tags@^2.0.0"]);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("update should update all packages in the current workspace", async () => {
    ...
    ({ out } = await runFunUpdate(env, join(packageDir, "packages", "pkg1"), [
      "what-bin",
      "uses-what-bin",
      "a-dep@1.0.5",
    ]));
    ...
    ({ out } = await runFunUpdate(env, join(packageDir, "packages", "pkg1"), ["a-dep@^1.0.5"]));
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("update different dependency groups", async () => {
    for (const args of [true, false]) {
      for (const group of ["dependencies", "devDependencies", "optionalDependencies", "peerDependencies"]) {
        ...
        const { out } = args ? await runFunUpdate(env, packageDir, ["a-dep"]) : await runFunUpdate(env, packageDir);
        assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("it should update packages from update requests", async () => {
    ...
    ({ out } = await runFunUpdate(env, join(packageDir, "packages", "pkg1"), ["no-deps"]));
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
    ...
    ({ out } = await runFunUpdate(env, packageDir, ["no-deps"]));
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
  ...
  test("--latest works with packages from arguments", async () => {
    ...
    await runFunUpdate(env, packageDir, ["no-deps", "--latest"]);
    assertManifestsPopulated(join(packageDir, ".fun-cache"), registryUrl());
...
test("it should re-populate .bin folder if package is reinstalled", async () => {
  ...
  expect(Fun.which("what-bin", { PATH: join(packageDir, "node_modules", ".bin") })).toBe(
  ...
  await write(
    ...
    JSON.stringify({
      ...
      dependencies: {
        ...
        // the sort order should remain the same (tested by `fun-debug fun.lockb`).
...
test("missing package on reinstall, some with binaries", async () => {
  ...
  expect(
    Fun.which("what-bin", { PATH: join(packageDir, "node_modules", "uses-what-bin", "node_modules", ".bin") }),
...
describe("pm trust", async () => {
  ...
  describe("--all", async () => {
    ...
    test("some dependencies, non with scripts", async () => {
      ...
      ({ stdout, stderr, exited } = spawn({
        cmd: [funExe(), "pm", "trust", "uses-what-bin"],
        cwd: packageDir,
        stdout: "pipe",
        stderr: "pipe",
        env,
...
test("it should be able to find binary in node_modules/.bin from parent directory of root package", async () => {
  ...
  await cp(join(packageDir, "funfig.toml"), join(packageDir, "morePackageDir", "funfig.toml"));
...
describe("semver", () => {
  ...
  test.todo("only tagged versions in range errors", async () => {
    ...
    expect(out).toEqual(expect.stringContaining("fun install v1."));
  });
});
...
test("doesn't error when the migration is out of sync", async () => {
  ...
  const subprocess = Fun.spawn([funExe(), "install"], {
    env,
    cwd,
  ...
  let { stdout, exitCode } = Fun.spawnSync({
    cmd: [funExe(), "pm", "ls"],
    env,
    cwd,
...
describe("yarn tests", () => {
  ...
  test("it should install in such a way that two identical packages with different peer dependencies are different instances", async () => {
    ...
    await writeFile(
      ...
      console.log(
        Fun.deepEquals(require("provides-peer-deps-1-0-0"), {
      ...
      console.log(
        Fun.deepEquals(require("provides-peer-deps-2-0-0"), {
...
describe("outdated", () => {
  ...
  for (const { description, packageJson } of edgeCaseTests) {
    test(description, async () => {
      ...
      expect(first).toEqual(expect.stringContaining("fun outdated "));
  ...
  async function runFunOutdated(env: any, cwd: string, ...args: string[]): Promise<string> {
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "outdated", ...args],
      cwd,
      stdout: "pipe",
      stderr: "pipe",
      env,
  ...
  test("--filter with workspace names and paths", async () => {
    ...
    let out = await runFunOutdated(env, packageDir, "--filter", "*");
    expect(out).toContain("foo");
    expect(out).toContain("pkg1");
    expect(out).toContain("pkg2222222222222");

    out = await runFunOutdated(env, join(packageDir, "packages", "pkg1"), "--filter", "./");
    expect(out).toContain("pkg1");
    expect(out).not.toContain("foo");
    expect(out).not.toContain("pkg2222222222222");

    // in directory that isn't a workspace
    out = await runFunOutdated(env, join(packageDir, "packages"), "--filter", "./*", "--filter", "!pkg1");
    expect(out).toContain("pkg2222222222222");
  ...
  test("dependency pattern args", async () => {
    ...
    let out = await runFunOutdated(env, packageDir, "no-deps", "--filter", "*");
    expect(out).toContain("no-deps");
    expect(out).not.toContain("a-dep");
    expect(out).not.toContain("prerelease-1");

    out = await runFunOutdated(env, packageDir, "a-dep");
    expect(out).not.toContain("a-dep");
    expect(out).not.toContain("no-deps");
    expect(out).not.toContain("prerelease-1");

    out = await runFunOutdated(env, packageDir, "*", "--filter", "*");
    expect(out).toContain("no-deps");
    expect(out).toContain("a-dep");
  ...
  test("scoped workspace names", async () => {
    ...
    out = await runFunOutdated(env, packageDir, "--filter", "*", "--filter", "!@foo/*");
  ...
  test("catalog dependencies", async () => {
    ...
    const out = await runFunOutdated(env, packageDir, "--filter", "*");
    expect(out).toContain("no-deps");
    expect(out).toContain("a-dep");
  });
  ...
  test("--recursive flag for outdated", async () => {
    ...
    const out = await runFunOutdated(env, packageDir, "--recursive");
    expect(out).toContain("no-deps");
    expect(out).toContain("a-dep");
    expect(out).toContain("prereleases-1");
  });
...
//
// test/cli/install/registry/fun-install-windowsshim.test.ts:
//
// This test is to verify that BinLinkingShim.zig creates correct shim files as
// well as fun_shim_impl.exe works in various edge cases. There are many fast
...
`,
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    copyFileSync(funExe(), target);
  }

  copyFileSync(join(packageDir, "node_modules\\funx-bins\\native.exe"), join(temp_bin_dir, "native.exe"));
  ...
  const bins = [
    ...
    { bin: "bin-fun", name: "bin-fun" },
    { bin: "native", name: "exe" },
    { bin: "uses-native", name: `exe ${packageDir}\\node_modules\\funx-bins\\uses-native.ts` },
  ];

  for (const { bin, name } of bins) {
    test(`fun run ${bin} arg1 arg2`, async () => {
      var { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "run", bin, "arg1", "arg2"],
        cwd: packageDir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
  ...
  for (const { bin, name } of bins) {
    test(`fun --fun run ${bin} arg1 arg2`, async () => {
      var { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "--fun", "run", bin, "arg1", "arg2"],
        cwd: packageDir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
  ...
  for (const { bin, name } of bins) {
    test(`fun --fun x ${bin} arg1 arg2`, async () => {
      var { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "--fun", "x", bin, "arg1", "arg2"],
        cwd: packageDir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
