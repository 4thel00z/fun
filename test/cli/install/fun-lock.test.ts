...
it("should write plaintext lockfiles", async () => {
  ...
  await access(join(packageDir, "fun.lock"));

  // Assert that the lockfile has the correct permissions
  const file = await open(join(packageDir, "fun.lock"), "r");
...
it.skipIf(isWindows)("should escape names", async () => {
  ...
  expect(await file(join(packageDir, "fun.lock")).text()).toMatchSnapshot();
});
...
it("should be the default save format", async () => {
  ...
  expect(await exists(join(packageDir, "fun.lockb"))).toBe(false);
  expect(
...
it("should save the lockfile if --save-text-lockfile and --frozen-lockfile are used", async () => {
  ...
  async function checkLockfiles() {
    return await Promise.all([exists(join(packageDir, "fun.lock")), exists(join(packageDir, "fun.lockb"))]);
  }
  ...
  await runFunInstall(env, packageDir, { saveTextLockfile: true, frozenLockfile: true });
  expect(await checkLockfiles()).toEqual([true, false]);
  const firstLockfile = (await file(join(packageDir, "fun.lock")).text()).replaceAll(
    /localhost:\d+/g,
    "localhost:1234",
  );
  ...
  expect(await checkLockfiles()).toEqual([true, false]);
  const secondLockfile = (await file(join(packageDir, "fun.lock")).text()).replaceAll(
    /localhost:\d+/g,
    "localhost:1234",
  );
...
it("should convert a binary lockfile with invalid optional peers", async () => {
  const { packageDir, packageJson } = await registry.createTestDir({ funfigOpts: { npm: true } });
  await Promise.all([
    write(
      packageJson,
      JSON.stringify({
    ...
    cp(join(import.meta.dir, "fixtures", "invalid-optional-peer.lockb"), join(packageDir, "fun.lockb")),
  ]);
  ...
  expect(await exited).toBe(0);
  expect(await file(join(packageDir, "fun.lock")).text()).toBe(firstLockfile);
});
...
`;
  ...
  const lockfile = (await file(join(packageDir, "fun.lock")).text()).replaceAll(/localhost:\d+/g, "localhost:1234");
  ...
  expect((await file(join(packageDir, "fun.lock")).text()).replaceAll(/localhost:\d+/g, "localhost:1234")).toBe(
    lockfile,
  );
});
...
it("should sort overrides before comparing", async () => {
  ...
  const lockfile = (await file(join(packageDir, "fun.lock")).text()).replaceAll(/localhost:\d+/g, "localhost:1234");
  expect(lockfile).toMatchSnapshot();
  await runFunInstall(env, packageDir, { frozenLockfile: true });
