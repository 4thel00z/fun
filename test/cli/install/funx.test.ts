...
import { afterAll, beforeAll, beforeEach, describe, expect, it, setDefaultTimeout } from "fun:test";
import { mkdir, rm, writeFile } from "fs/promises";
import { funEnv, funExe, isWindows, readdirSorted, tmpdirSync } from "harness";
...
let x_dir: string;
let env: Record<string, string> = { ...funEnv };

// Each test that hits the network gets its own isolated tmpdir + install cache
// so the network-heavy tests can run concurrently without sharing funx cache state.
function setup() {
  ...
  return {
    ...
    env: {
      ...funEnv,
      TEMP: current_tmpdir,
      FUN_TMPDIR: current_tmpdir,
      TMPDIR: current_tmpdir,
      FUN_INSTALL_CACHE_DIR: install_cache_dir,
    } as Record<string, string>,
  };
}

beforeAll(async () => {
  // Clean stale funx cache dirs from previous runs once up front instead of before every test.
  const tmp = isWindows ? tmpdir() : "/tmp";
  const waiting: Promise<void>[] = [];
  readdirSync(tmp).forEach(file => {
    if (file.startsWith("funx-") || file.startsWith("fun-x.test")) {
...
it.concurrent("should choose the tagged versions instead of the PATH versions when a tag is specified", async () => {
  ...
  const processes = semverVersions.map((version, i) => {
    return spawn({
      cmd: [funExe(), "x", "semver@" + version, "--help"],
      cwd: x_dir,
      stdout: "pipe",
      stdin: "ignore",
      stderr: "ignore",
      env: {
        ...env,
        // FUN_DEBUG_QUIET_LOGS: undefined,
        // FUN_DEBUG: "/tmp/fun-debug.txt." + i,
      },
    });
  });
...
it.concurrent("should install and run default (latest) version", async () => {
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "x", "uglify-js", "--compress"],
    cwd: x_dir,
    stdout: "pipe",
...
it.concurrent("should install and run specified version", async () => {
  const { x_dir, env } = setup();
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "x", "uglify-js@3.14.1", "-v"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
...
it.concurrent("should work for @scoped packages", async () => {
  ...
  const withoutCache = spawn({
    cmd: [funExe(), "--fun", "x", "@babel/cli", "--help"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
...
7
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "--fun", "x", "uglify-js", "test.js", "--compress"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
...
it.concurrent("should work for github repository", async () => {
  ...
  // without cache
  const withoutCache = spawn({
    cmd: [funExe(), "x", "github:piuccio/cowsay", "--help"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
...
it.concurrent("should work for github repository with committish", async () => {
  const { x_dir, env } = setup();
  const withoutCache = spawn({
    cmd: [funExe(), "x", "github:piuccio/cowsay#HEAD", "hello fun!"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
  ...
  expect(err).not.toContain("error:");
  expect(out.trim()).toContain("hello fun!");
  expect(exited).toBe(0);

  // cached
  const cached = spawn({
    cmd: [funExe(), "x", "--no-install", "github:piuccio/cowsay#HEAD", "hello fun!"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
...
it.concurrent.each(["--version", "-v"])("should print the version using %s and exit", async flag => {
  ...
  expect(err).not.toContain("error:");
  expect(out.trim()).toContain(Fun.version);
  expect(exited).toBe(0);
});
...
it.concurrent("should print the revision and exit", async () => {
  ...
  expect(err).not.toContain("error:");
  expect(out.trim()).toContain(Fun.version);
  expect(out.trim()).toContain(Fun.revision.slice(0, 7));
  expect(exited).toBe(0);
});
...
it.concurrent("should pass --version to the package if specified", async () => {
  ...
  expect(err).not.toContain("error:");
  expect(out.trim()).not.toContain(Fun.version);
  expect(exited).toBe(0);
});

it.concurrent('should set "npm_config_user_agent" to fun', async () => {
  const { x_dir, env } = setup();
  await writeFile(
  ...
  expect(err).not.toContain("error:");
  expect(out.trim()).toContain(`fun/${Fun.version}`);
  expect(exited).toBe(0);
});
...
describe("funx --no-install", () => {
  ...
  /*
    yes, multiple package tests are neccessary.
      1. there's specialized logic for `funx tsc` and `funx typescript`
  ...
  it.concurrent.each(["typescript", "http-server", "eslint"])(
    "`funx --no-install %s` should find cached packages",
    async pkg => {
      const ctx = setup();
      // not cached
      {
...
it.concurrent("should handle postinstall scripts correctly with symlinked funx", async () => {
  const { x_dir, env } = setup();
  // Create a symlink to fun called "funx"
  copyFileSync(funExe(), join(x_dir, isWindows ? "fun.exe" : "fun"));
  copyFileSync(funExe(), join(x_dir, isWindows ? "funx.exe" : "funx"));

  const subprocess = spawn({
    cmd: ["funx", "esbuild@latest", "--version"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
...
it.concurrent("should handle package that requires node 24", async () => {
  const { x_dir, env } = setup();
  const subprocess = spawn({
    cmd: [funExe(), "x", "--fun", "@angular/cli@latest", "--help"],
    cwd: x_dir,
    stdout: "pipe",
    stdin: "inherit",
    stderr: "pipe",
...
describe("--package flag", () => {
  ...
  describe("with mock registry", () => {
    ...
    it("should install specified package when binary differs from package name", async () => {
      ...
      // Without --package, funx different-bin would fail
      // With --package, we correctly install my-special-pkg
      const subprocess = spawn({
        cmd: [funExe(), "x", "--package", "my-special-pkg", "different-bin", "--help"],
        cwd: x_dir,
        stdout: "pipe",
        stdin: "inherit",
        stderr: "pipe",
    ...
    it("should support -p shorthand with mock registry", async () => {
      ...
      const subprocess = spawn({
        cmd: [funExe(), "x", "-p", "actual-package", "tool", "--version"],
        cwd: x_dir,
        stdout: "pipe",
        stdin: "inherit",
        stderr: "pipe",
    ...
    it("should support --package=<pkg> syntax with mock registry", async () => {
      ...
      const subprocess = spawn({
        cmd: [funExe(), "x", "--package=runner-pkg", "runner", "--help"],
        cwd: x_dir,
        stdout: "pipe",
        stdin: "inherit",
        stderr: "pipe",
    ...
    it("should fail to run alternate binary without --package flag", async () => {
      ...
      // This should fail because funx would try to install a package named "multi-tool-alt"
      const subprocess = spawn({
        cmd: [funExe(), "x", "multi-tool-alt"],
        cwd: x_dir,
        stdout: "pipe",
        stdin: "inherit",
        stderr: "pipe",
    ...
    it("should execute the correct binary when package has multiple binaries", async () => {
      ...
      await Fun.$`mkdir -p ${packageDir}/bin`;
...
`,
      ...
      await Fun.$`chmod +x ${packageDir}/bin/multi-tool.js ${packageDir}/bin/multi-tool-alt.js`;

      // Create the tarball with package/ prefix
      await Fun.$`cd ${tempDir} && tar -czf ${join(import.meta.dir, "multi-tool-pkg-1.0.0.tgz")} package`;

      // Test 1: Without --package, funx multi-tool-alt should fail or install wrong package
      // Test 2: With --package, we can run the alternate binary
      const subprocess = spawn({
        cmd: [funExe(), "x", "--package", "multi-tool-pkg", "multi-tool-alt"],
        cwd: x_dir,
        stdout: "pipe",
        stdin: "inherit",
        stderr: "pipe",
...
// Regression: `funx @scope/name` guesses the bin name as `name` (the
// unscoped portion), then searched the full system $PATH with it. When
// `name` happened to match an unrelated system binary — e.g.
// `funx @uidotsh/install` matching /usr/bin/install — the system binary
...
describe("scoped packages should not match unrelated system binaries", () => {
  ...
  it("`funx @scope/install` runs the package's bin, not a system binary named `install`", async () => {
    ...
    await Fun.$`tar -czf ${join(tgzDir, "install-1.0.0.tgz")} -C ${pkgRoot} package`;
  ...
  // Also covers https://github.com/underdoc-org/fun/issues/19458 and
  // https://github.com/underdoc-org/fun/issues/17904: when a scoped package is
  // already installed locally, funx must read its package.json (under the
  ...
  it("locally installed `@scope/name` resolves the real bin from its package.json", async () => {
    ...
    const subprocess = spawn({
      cmd: [funExe(), "x", "--no-install", "@myscope/collide"],
      cwd: x_dir,
      stdout: "pipe",
      stdin: "inherit",
      stderr: "pipe",
  ...
  it("locally installed `@scope/foo` whose bin is also named `foo` is still found", async () => {
    ...
    const subprocess = spawn({
      cmd: [funExe(), "x", "--no-install", "@myscope/samebin"],
      cwd: x_dir,
      stdout: "pipe",
      stdin: "inherit",
      stderr: "pipe",
  ...
  // When a scoped package lives only in the funx cache (not locally
  // installed) and its real bin name — discovered by reading its
  // package.json — collides with a system binary, funx must run the
  // cached bin via the absolute-path probe, not the system binary.
  it("funx-cache-only `@scope/name` whose real bin collides with a system binary runs the cached bin", async () => {
    ...
    const tgzDir = tmpdirSync();
    await Fun.$`tar -czf ${join(tgzDir, "pkg-1.0.0.tgz")} -C ${pkgRoot} package`;
    ...
    // First run: installs into the funx cache (no local node_modules).
    {
      const subprocess = spawn({
        cmd: [funExe(), "x", "@cacheonly/pkg"],
        cwd: x_dir,
        stdout: "pipe",
        stdin: "inherit",
        stderr: "pipe",
    ...
    {
      const subprocess = spawn({
        cmd: [funExe(), "x", "--no-install", "@cacheonly/pkg"],
        cwd: x_dir,
        stdout: "pipe",
        stdin: "inherit",
        stderr: "pipe",
...
describe("package name aliases", () => {
  ...
  // `funx claude` should resolve to `@anthropic-ai/claude-code` (same shape as
  ...
  it("`funx claude` requests @anthropic-ai/claude-code, not the 'claude' squatter", async () => {
    const urls: string[] = [];
...
// Regression test: funx should not crash on corrupted .funx files (Windows only)
// When the .funx metadata file is corrupted (e.g., missing quote terminator in bin_path),
// funx should gracefully fall back to the slow path instead of panicking.
it.skipIf(!isWindows)("should not crash on corrupted .funx file with missing quote", async () => {
  // First, install a package to create a valid .funx file
  // Use typescript which creates both .exe and .funx files
  ...
  const subprocess1 = spawn({
    cmd: [funExe(), "add", "typescript@5.0.0"],
    cwd: x_dir,
    stdout: "pipe",
    stderr: "pipe",
    env,
  ...
  const funxFile = join(binDir, "tsc.funx");

  // Verify the file exists before corrupting it
  expect(await Fun.file(funxFile).exists()).toBe(true);

  // Create a corrupted .funx file:
  ...
  // Valid flags with has_shebang=true, is_node_or_fun=true, version=v5
  ...
  await writeFile(funxFile, corruptedData);

  // Now run funx - it should NOT crash, but may fail gracefully
  // Using fun run to invoke tsc.exe, which triggers the FunXFastPath
  const subprocess2 = spawn({
    cmd: [funExe(), "run", "tsc", "--version"],
    cwd: x_dir,
    stdout: "pipe",
    stderr: "pipe",
    env,
