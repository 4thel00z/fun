...
test("dependency on workspace without version in package.json", async () => {
  ...
  for (const version of shouldWork) {
    ...
    const { out } = await runFunInstall(env, packageDir);
    ...
    rmSync(join(packageDir, "fun.lock"), { recursive: true, force: true });
  }
...
test("allowing negative workspace patterns", async () => {
  ...
  const { exited } = await runFunInstall(env, packageDir);
...
test("adding workspace in workspace edits package.json with correct version (workspace:*)", async () => {
  ...
  const { stdout, exited } = Fun.spawn({
    cmd: [funExe(), "add", "pkg2@workspace:*"],
  ...
  expect(await Fun.file(join(packageDir, "packages", "pkg1", "package.json")).json()).toEqual({
    name: "pkg1",
    version: "1.0.0",
    dependencies: {
      pkg2: "workspace:*",
  ...
  await Promise.all([
    ...
    write(
      ...
      JSON.stringify({
        name: "pkg1",
        scripts: {
          postinstall: 'fun -e \'require("fs").writeFileSync("cwd", process.cwd())\'',
        },
      }),
    ),
  ]);
...
describe("relative tarballs", async () => {
  ...
  test("from cli", async () => {
    ...
    const { stderr, exited } = Fun.spawn({
      cmd: [funExe(), "install", "../../qux-0.0.2.tgz"],
...
test("adding packages in a subdirectory of a workspace", async () => {
  ...
  // now delete node_modules and fun.lock and install
...
for (const rootVersion of versions) {
  for (const packageVersion of versions) {
    test(`it should allow duplicates, root@${rootVersion}, package@${packageVersion}`, async () => {
      ...
      await rm(join(packageDir, "node_modules"), { recursive: true, force: true });
      await rm(join(packageDir, "fun.lock"), { recursive: true, force: true });
...
describe("install --filter", () => {
  test("does not run root scripts if root is filtered out", async () => {
    ...
    var { exited } = spawn({
      cmd: [funExe(), "install", "--filter", "pkg1"],
      cwd: packageDir,
      stdout: "ignore",
      stderr: "ignore",
      env,
    ...
    ({ exited } = spawn({
      cmd: [funExe(), "install", "--filter", "root"],
      cwd: packageDir,
      stdout: "ignore",
      stderr: "ignore",
      env,
  ...
  test("all but one or two", async () => {
    ...
    var { exited } = spawn({
      cmd: [funExe(), "install", "--filter", "!pkg2", "--save-text-lockfile"],
      cwd: packageDir,
      stdout: "ignore",
      stderr: "pipe",
      env,
  ...
  test("filter with a path", async () => {
    ...
    var { exited } = spawn({
      cmd: [funExe(), "install", "--filter", "./packages/pkg1"],
      cwd: packageDir,
      stdout: "ignore",
      stderr: "pipe",
      env,
    ...
    ({ exited } = spawn({
      cmd: [funExe(), "install", "--filter", "./packages/*"],
      cwd: packageDir,
      stdout: "ignore",
      stderr: "pipe",
      env,
...
test("can override npm package with workspace package under a different name", async () => {
  ...
  // another install can use the existing fun.lock successfully
  await rm(join(packageDir, "node_modules"), { recursive: true, force: true });
  ({ exited } = spawn({
...
describe("LinkWorkspacePackages", () => {
  let funfigPath: string;

  beforeEach(async () => {
    funfigPath = join(packageDir, "funfig.toml");
  ...
  test("linkWorkspacePackages = false uses registry instead of linking workspace packages", async () => {
    // Create funfig.toml with linkWorkspacePackages set to false
    await Promise.all([
      write(
        funfigPath,
        `
...
`,
    ...
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), `-c=${funfigPath}`, "install"],
      cwd: packageDir,
      stdout: "pipe",
      stderr: "pipe",
      env,
...
test("matching workspace devDependency and npm peerDependency", async () => {
  ...
  expect((await file(join(packageDir, "fun.lock")).text()).replaceAll(/localhost:\d+/g, "localhost:1234"))
