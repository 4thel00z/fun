...
describe("otp", async () => {
    ...
    return async function (req: Request) {
      ...
      if (req.url.includes("otp-pkg")) {
        ...
        if (req.headers.get("npm-otp") === token) {
          ...
          // `fun publish` won't request a url from a message in the npm-notice header, but we
        ...
        expect.unreachable("url given to user, fun publish should not request");
  ...
  for (const shouldIgnoreNotice of [false, true]) {
    test(`npm-notice with login url${shouldIgnoreNotice ? " (ignored)" : ""}`, async () => {
      ...
      // They run `fun publish` with --auth-type=legacy, prompting them
...
test("can publish a package then install it", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();
  const funfig = await registry.authBunfig("basic");
  await Promise.all([
...
test("can publish from a tarball", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();
  const funfig = await registry.authBunfig("tarball");
  const json = {
    name: "publish-pkg-2",
    version: "2.2.2",
    dependencies: {
  ...
  await Promise.all([
    rm(join(registry.packagesPath, "publish-pkg-2"), { recursive: true, force: true }),
    rm(join(packageDir, "fun.lockb"), { recursive: true, force: true }),
...
test("can publish scoped packages", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();
  const funfig = await registry.authBunfig("scoped-pkg");
  const json = {
    name: "@scoped/pkg-1",
    version: "1.1.1",
    dependencies: {
...
for (const info of [
  ...
  test(`can publish and install binaries with ${JSON.stringify(info)}`, async () => {
    ...
    const funfig = await registry.authBunfig("binaries-" + info.user);
    ...
    await Promise.all([
      ...
      write(join(publishDir, "funfig.toml"), funfig),
      write(join(publishDir, "bin1.js"), `#!/usr/bin/env fun\nconsole.log("bin1!")`),
      write(join(publishDir, "bin2.js"), `#!/usr/bin/env fun\nconsole.log("bin2!")`),
      write(join(publishDir, "bins", "bin3.js"), `#!/usr/bin/env fun\nconsole.log("bin3!")`),
      write(join(publishDir, "bins", "moredir", "bin4.js"), `#!/usr/bin/env fun\nconsole.log("bin4!")`),
    ...
    const results = await Promise.all([
      exists(join(packageDir, "node_modules", ".bin", isWindows ? "bin1.funx" : "bin1")),
      exists(join(packageDir, "node_modules", ".bin", isWindows ? "bin2.funx" : "bin2")),
      exists(join(packageDir, "node_modules", ".bin", isWindows ? "bin3.js.funx" : "bin3.js")),
      exists(join(packageDir, "node_modules", ".bin", isWindows ? "bin4.js.funx" : "bin4.js")),
      ...
      exists(
        join(
          ...
          "node_modules",
          ".bin",
          isWindows ? `publish-pkg-${info.user}.funx` : "publish-pkg-" + info.user,
        ),
      ),
    ]);
...
test("dependencies are installed", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();
  const publishDir = tmpdirSync();
  const funfig = await registry.authBunfig("manydeps");
  await Promise.all([
...
test("can publish workspace package", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();
  const funfig = await registry.authBunfig("workspace");
  const pkgJson = {
    name: "publish-pkg-3",
    version: "3.3.3",
    dependencies: {
...
describe("--dry-run", async () => {
  test("does not publish", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("dryrun");
    await Promise.all([
  ...
  test("does not publish from tarball path", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("dryruntarball");
    await Promise.all([
...
postpack: \${fs.existsSync("postpack.txt")}\`)`;
  const json = {
    ...
    scripts: {
      ...
      "prepublishOnly": `${funExe()} script.js prepublishOnly`,
      "prepack": `${funExe()} script.js prepack`,
      "prepare": `${funExe()} script.js prepare`,
      "postpack": `${funExe()} script.js postpack`,
      "publish": `${funExe()} script.js publish`,
      "postpublish": `${funExe()} script.js postpublish`,
    },
    dependencies: {
      "publish-pkg-4": "4.4.4",
    },
  ...
  for (const arg of [[], ["--dry-run"]]) {
    test(`should run in order${arg.length > 0 ? " (--dry-run)" : ""}`, async () => {
      const { packageDir, packageJson } = await registry.createTestDir();
      const funfig = await registry.authBunfig("lifecycle" + (arg.length > 0 ? "dry" : ""));
      await Promise.all([
  ...
  test("--ignore-scripts", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("ignorescripts");
    await Promise.all([
...
test("prepublishOnly modifying version publishes correct version (#17195)", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();
  const funfig = await registry.authBunfig("version-update");
  ...
  await Promise.all([
    ...
    write(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        scripts: {
          prepublishOnly: `${funExe()} update-version.js`,
        },
        dependencies: {
...
test("attempting to publish a private package should fail", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();
  const funfig = await registry.authBunfig("privatepackage");
  await Promise.all([
...
describe("access", async () => {
  test("--access", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("accessflag");
    await Promise.all([
  ...
  for (const access of ["restricted", "public"]) {
    test(`access ${access}`, async () => {
      const { packageDir, packageJson } = await registry.createTestDir();
      const funfig = await registry.authBunfig("access" + access);
...
describe("tag", async () => {
  test("can publish with a tag", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("simpletag");
    const pkgJson = {
      name: "publish-pkg-9",
      version: "9.9.9",
      dependencies: {
...
it("$npm_command is accurate during publish", async () => {
  ...
  await write(join(packageDir, "funfig.toml"), await registry.authBunfig("npm_command"));
  await rm(join(registry.packagesPath, "publish-pkg-10"), { recursive: true, force: true });
  let { out, err, exitCode } = await publish(env, packageDir, "--tag", "simpletag");
  expect(err).toBe(`$ echo $npm_command\n`);
  expect(out.split("\n")).toEqual([
    `fun publish ${Fun.version_with_sha}`,
    ``,
...
it("$npm_lifecycle_event is accurate during publish", async () => {
  ...
  await write(join(packageDir, "funfig.toml"), await registry.authBunfig("npm_lifecycle_event"));
...
describe("readme", () => {
  // Regression for https://github.com/underdoc-org/fun/issues/30255 — `fun publish`
...
describe("--tolerate-republish", async () => {
  test("republishing normally fails", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("republish-fail");
    const pkgJson = {
  ...
  test("republishing with --tolerate-republish skips when version exists", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("republish-tolerate");
    const pkgJson = {
  ...
  test("republishing tarball with --tolerate-republish skips when version exists", async () => {
    const { packageDir, packageJson } = await registry.createTestDir();
    const funfig = await registry.authBunfig("republish-tarball");
    const pkgJson = {
