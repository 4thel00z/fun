import { funEnv, runFunInstall, tmpdirSync } from "harness";
...
function test(
    ...
    async () => {
      ...
      try {
        ...
        const write = (path: string, content: string | object) =>
          Fun.write(join(ctx.package_dir, path), typeof content === "string" ? content : JSON.stringify(content));
  ...
  scan: ${options.scanner.toString()},
        ...
        const funfig = await Fun.file(join(ctx.package_dir, "funfig.toml")).text();
        if (options.funfigScanner !== false) {
          const scannerPath = options.funfigScanner ?? "./scanner.ts";
          await write("./funfig.toml", `${funfig}\n[install.security]\nscanner = "${scannerPath}"`);
        }
        ...
        const { out, err } = await runFunInstall(funEnv, ctx.package_dir, {
          packages,
          allowErrors: true,
          allowWarnings: false,
          savesLockfile: false,
...
describe("Security Scanner Edge Cases", () => {
  test("scanner module not found", {
    ...
    funfigScanner: "./non-existent-scanner.ts",
    expectedExitCode: 1,
    expect: ({ err }) => {
      expect(err).toContain(
        "Security scanner './non-existent-scanner.ts' is configured in funfig.toml but the file could not be found.\n  Please check that the file exists and the path is correct.",
      );
    },
  });
...
describe("Large payload via ipc pipe", () => {
  ...
  beforeAll(async () => {
    ...
    const barTarball = await Fun.file(`${import.meta.dir}/bar-0.0.2.tgz`).bytes();
    const writes: Promise<number>[] = [];
    for (let i = 0; i < PKG_COUNT; i++) {
      writes.push(Fun.write(`${tgzTempDir}/${pkgName(i)}-0.0.2.tgz`, barTarball));
  ...
  test("handles packages JSON larger than max arg length (>1MB)", {
    ...
    customRegistry: (urls, ctx) => {
      return async (request: Request) => {
        ...
        if (url.endsWith(".tgz")) {
          return new Response(Fun.file(join(tgzTempDir, url.slice(url.lastIndexOf("/") + 1).toLowerCase())));
        }
