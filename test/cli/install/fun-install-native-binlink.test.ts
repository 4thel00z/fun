...
import { funEnv, funExe, isWindows, VerdaccioRegistry } from "harness";
...
`,
      ...
      // Now delete the node_modules folder, keep the fun.lock, re-install
    ...
    // and `.bin/<cmd>` was never created (broke `funx @anthropic-ai/claude-code`).
...
`,
    ...
    // Regression for `funx @anthropic-ai/claude-code` silently exiting: the
