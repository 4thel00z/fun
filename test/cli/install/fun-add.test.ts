...
import { afterAll, afterEach, beforeAll, beforeEach, expect, it, setDefaultTimeout } from "fun:test";
import { access, appendFile, copyFile, mkdir, readlink, rm, writeFile } from "fs/promises";
import { funExe, funEnv as env, readdirSorted, tmpdirSync, toBeValidBin, toBeWorkspaceLink, toHaveBins } from "harness";
...
it("should reject missing package", async () => {
  ...
  const out = await stdout.text();
  expect(out).toEqual(expect.stringContaining("fun add v1."));
...
it("fun add --only-missing should not install existing package", async () => {
  const urls: string[] = [];
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "--only-missing", "bar"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
  ...
  {
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), "add", "bar", "--only-missing"],
      cwd: package_dir,
      env,
      stdout: "pipe",
      stdin: "pipe",
    ...
    expect(out.split("\n").filter(Boolean)).toStrictEqual([
      expect.stringContaining("fun add v" + Fun.version.replaceAll("-debug", "")),
    ]);
  }
});

it("fun add --analyze should scan dependencies", async () => {
  const urls: string[] = [];
  setHandler(dummyRegistry(urls));
  await writeFile(
    join(package_dir, "package.json"),
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "./entry-point.ts", "--analyze"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
...
it("should add exact version with install.exact", async () => {
  ...
  await appendFile(join(package_dir, "funfig.toml"), `exact = true\n`);
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "BaR"],
    cwd: package_dir,
    stdout: "pipe",
...
it("should add dependency with package.json in it and http tarball", async () => {
  ...
  using server = Fun.serve({
    ...
    fetch(req) {
      ...
      return new Response(Fun.file(join(__dirname, "baz-0.0.3.tgz")));
    },
  });
  const urls: string[] = [];
  setHandler(
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "bap@npm:baz@0.0.5"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
...
it("should add dependency (GitHub)", async () => {
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "mishoo/UglifyJS#v3.14.1"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
...
it("should add dependency alongside workspaces", async () => {
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "baz", "--linker=isolated"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
...
it("should add aliased dependency (npm)", async () => {
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "bar@npm:baz@~0.0.2"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
...
it("should add aliased dependency (GitHub)", async () => {
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", "uglify@mishoo/UglifyJS#v3.14.1"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
...
it("git dep without package.json and with default branch", async () => {
  ...
  const { stderr, exited } = spawn({
    cmd: [funExe(), "add", "git@github.com:dylan-conway/install-test-no-packagejson"],
    cwd: package_dir,
    stdout: "ignore",
    stderr: "pipe",
    env,
...
it("should handle Git URL in dependencies (SCP-style)", async () => {
  ...
  const {
    ...
    cmd: [funExe(), "add", "fun@github.com:mishoo/UglifyJS.git"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
  ...
  expect(out1.replace(/\s*\[[0-9\.]+m?s\]\s*$/, "").split(/\r?\n/)).toEqual([
    expect.stringContaining("fun add v1."),
    "",
    "installed uglify-js@git+ssh://fun@github.com:mishoo/UglifyJS.git with binaries:",
    " - uglifyjs",
    "",
    "1 package installed",
  ]);
    ...
    JSON.stringify(
      {
        ...
        version: "0.0.1",
        dependencies: {
          "uglify-js": "fun@github.com:mishoo/UglifyJS.git",
        },
      },
      null,
      2,
...
it("should not save git urls twice", async () => {
  ...
  const { exited: exited1 } = spawn({
    cmd: [funExe(), "add", "https://github.com/liz3/empty-fun-repo"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
  ...
  expect(package_json_content.dependencies).toEqual({
    "test-repo": "https://github.com/liz3/empty-fun-repo",
  });
...
it("should add dependency without duplication", async () => {
  ...
  // The meta-hash didn't change, but we do save everytime you do "fun add <package>".
  expect(err2).toContain("Saved lockfile");
...
it("should add dependencies to workspaces directly", async () => {
  ...
  await writeFile(join(package_dir, "moo", "funfig.toml"), await file(join(package_dir, "funfig.toml")).text());
  const add_path = relative(join(package_dir, "moo"), add_dir);
  const dep = `file:${add_path}`.replace(/\\/g, "/");
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", dep, "--linker=isolated"],
  ...
  expect(await readdirSorted(join(package_dir, "moo"))).toEqual(["funfig.toml", "node_modules", "package.json"]);
...
async function installRedirectsToAdd(saveFlagFirst: boolean) {
  ...
  expect(await exited).toBe(0);
  expect(await file(join(package_dir, "package.json")).text()).toInclude("fun.test.");
}
...
it("should add multiple dependencies specified on command line", async () => {
  ...
  setHandler(dummyRegistry(urls));
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", `${server_url}/baz-0.0.3.tgz`, "bar"],
    cwd: package_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
  ...
  expect(out.replace(/\s*\[[0-9\.]+m?s\]\s*$/, "").split(/\r?\n/)).toEqual([
    expect.stringMatching(/^fun add v1./),
    "",
...
it("should install tarball with tarball dependencies", async () => {
  ...
  using server = Fun.serve({
    ...
    fetch(req) {
      ...
      if (url.pathname === "/child.tgz") {
        return new Response(Fun.file(join(tmpDir, "child.tgz")));
      } else if (url.pathname === "/parent.tgz") {
        return new Response(Fun.file(join(tmpDir, "parent.tgz")));
      }
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "add", `${server_url}/parent.tgz`, "--linker=hoisted"],
    cwd: add_dir,
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
