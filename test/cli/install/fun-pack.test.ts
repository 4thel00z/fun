...
import { funEnv, funExe, pack, runFunInstall, tempDirWithFiles, tmpdirSync } from "harness";
...
test("in subdirectory", async () => {
  ...
  await pack(join(packageDir, "subdir1", "subdir2"), funEnv);
  ...
  await pack(join(packageDir, "subdir1"), funEnv);
...
describe("package.json names and versions", () => {
  ...
  for (const { desc, expectedError, packageJson } of tests) {
    test(desc, async () => {
      ...
      const { err } = await packExpectError(packageDir, funEnv);
  ...
  for (const { input, output, fail } of scopedNames) {
    test(`scoped name: ${input}`, async () => {
      ...
      fail ? await packExpectError(packageDir, funEnv) : await pack(packageDir, funEnv);
      if (!fail) {
...
describe("flags", () => {
  test("--dry-run", async () => {
    ...
    const { out } = await pack(packageDir, funEnv, "--dry-run");
  ...
  test("--gzip", async () => {
    ...
    for (const invalidGzipLevel of ["-1", "10", "kjefj"]) {
      const { err } = await packExpectError(packageDir, funEnv, `--gzip-level=${invalidGzipLevel}`);
      expect(err).toContain(`error: compression level must be between 0 and 9, received ${invalidGzipLevel}\n`);
    }

    await pack(packageDir, funEnv, "--gzip-level=0");
    ...
    await pack(packageDir, funEnv, "--gzip-level=9");
  ...
  for (const { path } of destinationTests) {
    test(`--destination="${path}"`, async () => {
      ...
      await pack(packageDir, funEnv, `--destination=${dest}`);
  ...
  for (const { filename, error, mkdir } of filenameTests) {
    test(`--filename="${filename}"`, async () => {
      ...
      try {
        await pack(packageDir, funEnv, `--filename=${filename}`);
  ...
  test("--filename and --destination", async () => {
    ...
    expect(async () => await pack(packageDir, funEnv, "--filename=test.tgz", "--destination=packed")).toThrowError();
  });
  ...
  test("--ignore-scripts", async () => {
    ...
    await pack(packageDir, funEnv, "--ignore-scripts");
  ...
  test("--quiet", async () => {
    ...
    const { out } = await pack(packageDir, funEnv, "--quiet");
    ...
    expect(out).not.toContain("fun pack v");
...
test("shasum and integrity are consistent", async () => {
  ...
  let { out } = await pack(packageDir, funEnv);
  ...
  ({ out } = await pack(packageDir, funEnv));
...
describe("workspaces", () => {
  ...
  test("in a workspace", async () => {
    ...
    await pack(join(packageDir, "pkgs", "pkg1"), funEnv);
  ...
  test("in a workspace subdirectory", async () => {
    ...
    await pack(join(packageDir, "pkgs", "pkg1", "subdir"), funEnv);
  ...
  test("fails gracefully when workspace version fails to resolve", async () => {
    ...
    expect(err).toContain(
      'error: Failed to resolve workspace version for "pkg1" in `dependencies`. Run `fun install` and try again.',
    );
  ...
  await Promise.all([
    write(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        scripts: {
          prepack: `${funExe()} update-version.js`,
        },
      }),
    ),
...
describe("bundledDependnecies", () => {
  ...
  test(`scoped bundledDependencies`, async () => {
    await Promise.all([
      write(
        ...
        JSON.stringify({
          ...
          version: "4.4.4",
          dependencies: {
            "@underdoc-org/fun": "1.1.1",
          },
          bundledDependencies: ["@underdoc-org/fun"],
        }),
      ),
      write(
        join(packageDir, "node_modules", "@oven", "fun", "package.json"),
        JSON.stringify({
          name: "@underdoc-org/fun",
          version: "1.1.1",
        }),
      ),
    ]);
    ...
    expect(tarball.entries).toMatchObject([
      { "pathname": "package/package.json" },
      { "pathname": "package/node_modules/@underdoc-org/fun/package.json" },
    ]);
  });
  ...
  test("resolve dep of bundled dep", async () => {
    ...
    const { out } = await pack(packageDir, funEnv);
...
describe("files", () => {
  ...
  test("excluded entries within included directories are not included", async () => {
    const dir = tempDirWithFiles("fun-pack-files-excluded-entries", {
      "package.json": `
      {
    ...
    const { out } = await pack(dir, funEnv);
...
describe("bins", () => {
  test("basic", async () => {
    await Promise.all([
      ...
      write(join(packageDir, "bin.js"), `#!/usr/bin/env fun\n`),
    ]);
  ...
  test("directory", async () => {
    await Promise.all([
      ...
      write(join(packageDir, "bins", "bin1.js"), `#!/usr/bin/env fun\n`),
      write(join(packageDir, "bins", "bin2.js"), `#!/usr/bin/env fun\n`),
    ]);
...
test("$npm_command is accurate", async () => {
  ...
  const p = await pack(packageDir, funEnv);
  expect(p.out.split("\n")).toEqual([
    `fun pack ${Fun.version_with_sha}`,
    ``,
