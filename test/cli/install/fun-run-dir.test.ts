...
import { funExe, funEnv as env, readdirSorted, stderrForInstall, tmpdirSync } from "harness";
...
console.log(minify("print(6 * 7)").code);
  ...
  const {
    ...
    env: {
      ...env,
      FUN_INSTALL_CACHE_DIR: join(run_dir, ".cache"),
    },
  });
  ...
  // Perform `fun test.js` with cached dependencies
  const {
    stdout: stdout2,
    stderr: stderr2,
    exited: exited2,
...
for (const entry of await decompress(Buffer.from(buffer))) {
  ...
  expect(await exited1).toBe(0);
  // Perform `fun run test.js` with cached dependencies
  const {
    stdout: stdout2,
    stderr: stderr2,
    exited: exited2,
