...
import { funEnv as funEnv_, funExe, isWindows, tempDir, tempDirWithFiles } from "harness";
import { join } from "path";

const funEnv = {
  ...funEnv_,
  FUN_INTERNAL_SUPPRESS_CRASH_IN_FUN_RUN: "1",
};

describe.concurrent("fun run", () => {
  for (let withRun of [false, true]) {
    describe(withRun ? "fun run" : "fun", () => {
      describe("should work with .", () => {
        it("respecting 'main' field and allowing trailing commas/comments in package.json", async () => {
          using dir = tempDir("fun-run-main", {
          ...
          await using proc = Fun.spawn({
            cmd: [funExe(), ...(withRun ? ["run"] : []), "."].filter(Boolean),
            cwd: String(dir),
            env: funEnv,
            stdout: "pipe",
            stderr: "pipe",
        ...
        it("falling back to index", async () => {
          using dir = tempDir("fun-run-index", {
        ...
        it("invalid tsconfig.json is ignored", async () => {
          using dir = tempDir("fun-run-tsconfig", {
            "package.json": JSON.stringify({
              name: "test",
              version: "0.0.0",
              scripts: {
          ...
          await using proc = Fun.spawn({
            cmd: [funExe(), "--silent", ...(withRun ? ["run"] : []), "boop"].filter(Boolean),
            cwd: String(dir),
            env: funEnv,
            stdout: "pipe",
            stderr: "pipe",
        ...
        it("--silent omits error messages", async () => {
          using dir = tempDir("fun-run-silent", {});
          const exe = isWindows ? "fun.exe" : "fun";
          await using proc = Fun.spawn({
            cmd: [funExe(), "run", "--silent", exe, "doesnotexist"],
            cwd: String(dir),
            env: funEnv,
            stdout: "pipe",
            stderr: "pipe",
        ...
        it("no --silent includes error messages", async () => {
          using dir = tempDir("fun-run-nosilent", {});
          const exe = isWindows ? "fun.exe" : "fun";
          await using proc = Fun.spawn({
            cmd: [funExe(), "run", exe, "doesnotexist"],
            cwd: String(dir),
            env: funEnv,
            stdout: "pipe",
            stderr: "pipe",
        ...
        it.skipIf(isWindows)("exit code message works above 128", async () => {
          using dir = tempDir("fun-run-exitcode", {});
          await using proc = Fun.spawn({
            cmd: [funExe(), "run", "bash", "-c", "ulimit -c 0; exit 200"],
            cwd: String(dir),
            env: funEnv,
            stdout: "pipe",
            stderr: "pipe",
        ...
        describe.each(["--silent", "not silent"])("%s", silentOption => {
          ...
          it.skipIf(isWindows)("exit signal works", async () => {
            using dir = tempDir("fun-run-signal", {});
            {
              await using proc = Fun.spawn({
                cmd: [funExe(), ...(silent ? ["--silent"] : []), "run", "bash", "-c", "ulimit -c 0; kill -4 $$"].filter(
                  Boolean,
                ),
                cwd: String(dir),
                env: funEnv,
            ...
            {
              await using proc = Fun.spawn({
                cmd: [funExe(), ...(silent ? ["--silent"] : []), "run", "bash", "-c", "ulimit -c 0; kill -9 $$"],
                cwd: String(dir),
                env: funEnv,
                stdout: "pipe",
                stderr: "pipe",
        ...
        for (let withLogLevel of [true, false]) {
          it(
            ...
            async () => {
              using dir = tempDir("fun-run-tsconfig-extends", {
                "package.json": JSON.stringify({
                  name: "test",
                  version: "0.0.0",
                  scripts: {},
                ...
                ...(withLogLevel ? { "funfig.toml": `logLevel = "debug"` } : {}),
              });
        ...
        it("falling back to index with no package.json", async () => {
          using dir = tempDir("fun-run-nopkg", {
            "index.ts": "console.log('Hello, world!');",
          });
        ...
        it("should not passthrough script arguments to pre- or post- scripts", async () => {
          using dir = tempDir("fun-run-prepost", {
            "package.json": JSON.stringify({
              scripts: {
          ...
          await using proc = Fun.spawn({
            cmd: [funExe(), "run", "--silent", "myscript", "-a", "-b", "-c"].filter(Boolean),
            cwd: String(dir),
            env: funEnv,
            stdout: "pipe",
            stderr: "pipe",
  ...
  it("should show the correct working directory when run with --cwd", async () => {
    using dir = tempDir("fun-run-cwd", {
      "subdir/test.js": `console.log(process.cwd());`,
    });

    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--cwd", "subdir", "test.js"],
      cwd: String(dir),
      stdin: "ignore",
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("--ignore-dce-annotations ignores DCE annotations", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "--ignore-dce-annotations", "run", "index.ts"],
      cwd: String(dir),
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("$npm_command is accurate", async () => {
    using dir = tempDir("fun-run-npm-command", {
      "package.json": `{
      "scripts": {
  ...
  it("$npm_lifecycle_event is accurate", async () => {
    using dir = tempDir("fun-run-npm-lifecycle", {
      "package.json": `{
      "scripts": {
  ...
  it("$npm_package_config_* works", async () => {
    using dir = tempDir("fun-run-npm-config", {
      "package.json": `{
      "config": {
        "foo": "bar"
      },
  ...
  it("does not crash after spawning with $ variable", async () => {
    using dir = tempDir("fun-run-dollar", {
      "package.json": JSON.stringify({
        scripts: {
          debug: "fun index.js $hi",
        },
      }),
    });

    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "debug"],
      cwd: String(dir),
      stdin: "ignore",
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("should pass arguments correctly in scripts", async () => {
    using dir = tempDir("test", {
      "package.json": JSON.stringify({
        workspaces: ["a", "b"],
        scripts: { "root_script": "fun index.ts" },
      }),
      "index.ts": `for(const arg of Fun.argv) console.log(arg);`,
    ...
    {
      await using proc = Fun.spawn({
        cmd: [funExe(), "run", "root_script", "$HOME (!)", "argument two"].filter(Boolean),
        cwd: String(dir),
        env: funEnv,
        stdout: "pipe",
        stderr: "pipe",
      ...
      expect(stderr).toBe('$ fun index.ts "\\$HOME (!)" "argument two"\n');
      expect(stdout).toEndWith("\n$HOME (!)\nargument two\n");
      expect(exitCode).toBe(0);
    }
    {
      await using proc = Fun.spawn({
        cmd: [funExe(), "--filter", "*", "echo2", "$HOME (!)", "argument two"].filter(Boolean),
        cwd: String(dir),
        env: funEnv,
        stdout: "pipe",
        stderr: "pipe",
  ...
  describe("'fun run' priority", async () => {
    // priority:
    ...
    const commands: {
      ...
      // TODO: test preloads (https://fun.dev/docs/runtime/funfig#preload), test $npm_lifecycle_event
  ...
  it("should run from stdin", async () => {
    const res = await $`echo "console.log('hello')" | fun run -`.text();
  ...
  describe("should run scripts from the project root (#16169)", async () => {
    const dir = tempDirWithFiles("test", {
      "run_here": {
        ...
        "package.json": JSON.stringify({
          scripts: { "sample": "pwd", "runscript": "fun myscript.ts" },
        }),
        "dont_run_in_here": {
    ...
    it("inside --shell=fun", async () => {
      await using proc = Fun.spawn({
        cmd: [funExe(), "--shell=fun", "run", "sample"],
        cwd: dir + "/run_here/dont_run_in_here",
        env: funEnv,
        stdout: "pipe",
        stderr: "pipe",
