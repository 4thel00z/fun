...
import { afterAll, beforeAll, describe, expect, it, setDefaultTimeout } from "fun:test";
import { rm, writeFile } from "fs/promises";
import { funExe, funEnv as env, readdirSorted, tempDir } from "harness";
...
describe.concurrent("tarball integrity", () => {
  it("should store integrity hash for tarball URL in text lockfile", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      const lockContent = await file(join(ctx.package_dir, "fun.lock")).text();
      // fun.lock uses trailing commas (not strict JSON), so match with regex
  ...
  it("should store consistent integrity hash for tarball URL across reinstalls", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      const lockContent1 = await file(join(ctx.package_dir, "fun.lock")).text();
      ...
      await rm(join(ctx.package_dir, "fun.lock"), { force: true });
      ...
      const lockContent2 = await file(join(ctx.package_dir, "fun.lock")).text();
...
describe.concurrent.each(["hoisted", "isolated"] as const)("tarball integrity mismatch (%s)", linker => {
  ...
  it("should fail (not hang) when tarball bytes don't match manifest SHA-512", { timeout: 60_000 }, async () => {
    ...
    await using proc = spawn({
      cmd: [funExe(), "install"],
      cwd: String(dir),
      env: { ...env, FUN_INSTALL_CACHE_DIR: join(String(dir), ".cache") },
      stdout: "pipe",
      stderr: "pipe",
      timeout: 15_000,
    });
    ...
    // with the fix, fun exits cleanly on its own; on hang, Fun.spawn's
