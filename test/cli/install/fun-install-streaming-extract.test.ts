// Verifies that `fun install` can extract a tarball while it is still
...
import { funEnv, funExe, readdirSorted, tempDir } from "harness";
...
function makeEntries(): Entry[] {
  ...
  // FUN_INSTALL_STREAMING_MIN_SIZE (2 MB) — streaming only commits
...
//
// Uses node:http rather than Fun.serve so the response can carry both
// an explicit Content-Length *and* be drip-fed — Fun.serve forces
// `Transfer-Encoding: chunked` for stream bodies, which would bypass
// the FUN_INSTALL_STREAMING_MIN_SIZE gate.
// -------------------------------------------------------------------
...
async function runInstall(cwd: string, extraEnv: Record<string, string> = {}) {
  await using proc = Fun.spawn({
    cmd: [funExe(), "install", "--verbose", "--linker=hoisted"],
    cwd,
    env: {
      ...funEnv,
      FUN_INSTALL_CACHE_DIR: join(cwd, ".cache"),
      ...extraEnv,
    },
    stdout: "pipe",
    stderr: "pipe",
...
describe("streaming tarball extraction", () => {
  ...
  test.each([
    ["streaming", {}],
    ["buffered", { FUN_FEATURE_FLAG_DISABLE_STREAMING_INSTALL: "1" }],
  ...
  test("tarballs below FUN_INSTALL_STREAMING_MIN_SIZE take the buffered path", async () => {
    ...
    const { stderr, exitCode } = await runInstall(String(dir), {
      FUN_INSTALL_STREAMING_MIN_SIZE: String(tgz.length + 1),
    });
    expect(stderr).not.toContain("Streamed ");
    expect(stderr).not.toContain("error:");
    const pkgRoot = join(String(dir), "node_modules", "stream-pkg");
...
// pre-existing damaged-block ARCHIVE_RETRY through the same `fun_retry`
