import { file, listen, Socket, spawn, write } from "fun";
import { afterAll, beforeAll, describe, expect, it, jest, setDefaultTimeout, test } from "fun:test";
...
describe.concurrent("fun-install", () => {
  for (let input of ["abcdef", "65537", "-1"]) {
    it(`fun install --network-concurrency=${input} fails`, async () => {
    ...
    "dependencies": {
      "bar": "^1"
        ...
        const { stderr, exited } = spawn({
          cmd: [funExe(), "install", "--network-concurrency", "abcdef"],
          cwd: ctx.package_dir,
          stdout: "inherit",
          stdin: "inherit",
          stderr: "pipe",
  ...
  it("fun install --network-concurrency=5 doesnt go over 5 concurrent requests", async () => {
    await withContext(defaultOpts, async ctx => {
      const urls: string[] = [];
        ...
        `
  {
    ...
    "dependencies": {
      ...
      const { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "install", "--network-concurrency", "5"],
        cwd: ctx.package_dir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
      ...
      expect(await stdout.text()).toEqual(expect.stringContaining("fun install v1."));
    });
  });
  ...
  `,
      ...
      try {
        await access(join(ctx.package_dir, "fun.lockb"));
        expect.unreachable();
      } catch (err: any) {
  ...
  it("should report connection errors", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await writeFile(
        join(ctx.package_dir, "funfig.toml"),
        `
  [install]
  cache = false
  ...
  `,
      ...
      const { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "install", "--registry", `http://${server.hostname}:${server.port}/`],
        cwd: ctx.package_dir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
  ...
  it("should work when moving workspace packages", async () => {
    ...
    await Fun.$`${funExe()} i`.env(funEnv).cwd(package_dir);
  ...
  it("should handle missing package", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      expect(await stdout.text()).toEqual(expect.stringContaining("fun add v1."));
      expect(await exited).toBe(1);
  ...
  it("should handle @scoped authentication", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await writeFile(
        join(ctx.package_dir, "funfig.toml"),
        `${await file(join(ctx.package_dir, "funfig.toml")).text()}
  [install.scopes]
  foo = { token = "bar" }
  `,
      );
      const { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "install", "@foo/bar"],
        cwd: ctx.package_dir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
  ...
  it("should handle workspaces", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      // Perform `fun install` again but with lockfile from before
    ...
    await withContext(defaultOpts, async ctx => {
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "0.0.1",
          scripts: {
            install: [funExe(), "install.js"].join(" "),
          },
          workspaces: ["bar"],
        }),
      );
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "0.0.2",
          scripts: {
            preinstall: [funExe(), "preinstall.js"].join(" "),
          },
        }),
      );
      await writeFile(
    ...
    await withContext(defaultOpts, async ctx => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "0.0.1",
          scripts: {
            install: [funExe(), "foo-install.js"].join(" "),
          },
          dependencies: {
            qux: "^0.0",
          },
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "0.0.2",
          scripts: {
            preinstall: [funExe(), "bar-preinstall.js"].join(" "),
          },
        }),
      );
      await writeFile(
      ...
      expect(await file(join(ctx.package_dir, "bar", "bar.txt")).text()).toBe("bar!");
      await access(join(ctx.package_dir, "fun.lockb"));
      // Perform `fun install --production` with lockfile from before
      await rm(join(ctx.package_dir, "node_modules"), { force: true, recursive: true });
      const {
        stdout: stdout3,
        stderr: stderr3,
  ...
  it("should use updated life-cycle scripts in root during re-installation", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      // Perform `fun install` with outdated lockfile
      await rm(join(ctx.package_dir, "node_modules"), { force: true, recursive: true });
      await writeFile(
        join(ctx.package_dir, "package.json"),
        JSON.stringify({
          name: "Foo",
          scripts: {
            install: [funExe(), "foo-install2.js"].join(" "),
            postinstall: [funExe(), "foo-postinstall.js"].join(" "),
          },
          workspaces: ["bar"],
        }),
      );
      ...
      await access(join(ctx.package_dir, "fun.lockb"));
      // Perform `fun install --production` with lockfile from before
      const fun_lockb = await file(join(ctx.package_dir, "fun.lockb")).arrayBuffer();
      await rm(join(ctx.package_dir, "node_modules"), { force: true, recursive: true });
      const {
        stdout: stdout3,
        stderr: stderr3,
      ...
      expect(await readdirSorted(join(ctx.package_dir, "node_modules"))).toEqual([".cache", "Bar"]);
      expect(ctx.package_dir).toHaveWorkspaceLink(["Bar", "bar"]);
      expect(await file(join(ctx.package_dir, "fun.lockb")).arrayBuffer()).toEqual(fun_lockb);
      expect(await file(join(ctx.package_dir, "foo2.txt")).text()).toBe("foo2!");
      expect(await file(join(ctx.package_dir, "bar", "bar.txt")).text()).toBe("bar!");
      expect(await file(join(ctx.package_dir, "foo-postinstall.txt")).text()).toBe("foo!");
    });
    ...
    await withContext(defaultOpts, async ctx => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          scripts: {
            preinstall: [funExe(), "bar-preinstall.js"].join(" "),
            postinstall: [funExe(), "bar-postinstall.js"].join(" "),
          },
        }),
      );
      await writeFile(
  ...
  it("should edit package json correctly with git dependencies", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      var { stdout, stderr, exited } = spawn({
        cmd: [funExe(), "i", "dylan-conway/install-test2"],
        cwd: ctx.package_dir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
      ...
      ({ stdout, stderr, exited } = spawn({
        cmd: [funExe(), "i", "dylan-conway/install-test2#HEAD"],
        cwd: ctx.package_dir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
      ...
      await writeFile(join(ctx.package_dir, "package.json"), package_json);
      ({ stdout, stderr, exited } = spawn({
        cmd: [funExe(), "i", "github:dylan-conway/install-test2"],
        cwd: ctx.package_dir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
      ...
      await writeFile(join(ctx.package_dir, "package.json"), package_json);
      ({ stdout, stderr, exited } = spawn({
        cmd: [funExe(), "i", "github:dylan-conway/install-test2#HEAD"],
        cwd: ctx.package_dir,
        stdout: "pipe",
        stdin: "pipe",
        stderr: "pipe",
  ...
  it("should install latest with prereleases", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      expect(out.replace(/\s*\[[0-9\.]+m?s\]\s*$/, "").split(/\n/)).toEqual([
        expect.stringContaining("fun add v1."),
        "",
      ...
      await rm(join(ctx.package_dir, "fun.lockb"), { recursive: true, force: true });
      await writeFile(
        join(ctx.package_dir, "package.json"),
        JSON.stringify({
          name: "foo",
  ...
  it("should not reinstall aliased dependencies", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await access(join(ctx.package_dir, "fun.lockb"));
      // Performs `fun install` again, expects no-op
      urls.length = 0;
      const {
        stdout: stdout2,
        stderr: stderr2,
    ...
    await withContext(defaultOpts, async ctx => {
      ...
      await writeFile(
        ...
        JSON.stringify({
          ...
          version: "0.0.1",
          dependencies: {
            uglify: "git+http://fun.dev/no_such_repo",
          },
        }),
      );
      const { stdout, stderr, exited } = spawn({
  ...
  `,
      ...
      await access(join(ctx.package_dir, "fun.lockb"));
      // Perform `fun install` again but with cache & lockfile from before
      await Promise.all(
        [
          ".bin",
          "camel-case",
  ...
  it("should ignore invalid workspaces from parent directory", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await mkdir(join(ctx.package_dir, "moo"));
      await writeFile(
        join(ctx.package_dir, "moo", "funfig.toml"),
        await file(join(ctx.package_dir, "funfig.toml")).text(),
      );
      const moo_package = JSON.stringify({
        name: "moo",
      ...
      expect(urls.sort()).toEqual([`${ctx.registry_url}bar`, `${ctx.registry_url}bar-0.0.2.tgz`]);
      expect(ctx.requested).toBe(2);
      expect(await readdirSorted(ctx.package_dir)).toEqual(["funfig.toml", "moo", "package.json"]);
  ...
  it("should handle fun ci alias (to --frozen-lockfile)", async () => {
    await withContext(defaultOpts, async ctx => {
      let urls: string[] = [];
      setContextHandler(
        ctx,
  ...
  it("should perform bin-linking across multiple dependencies", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await writeFile(join(ctx.package_dir, "package.json"), foo_package);
      await cp(join(import.meta.dir, "fun.lockb.bin-linking"), join(ctx.package_dir, "fun.lockb"));
      await writeFile(
        join(ctx.package_dir, "funfig.toml"),
        `
  [install]
  ...
  `,
      ...
      const out1 = await new Response(stdout1).text();
      expect(out1.replace(/\s*\[[0-9\.]+m?s\]\s*$/, "").split(/\r?\n/)).toEqual([
        `fun install ${Fun.version_with_sha}`,
        "",
  ...
  it("should handle trustedDependencies", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      expect(out.replace(/\s*\[[0-9\.]+m?s\]$/m, "").split(/\r?\n/)).toEqual([
        ...
        "2 packages installed",
        "",
        "Blocked 3 postinstalls. Run `fun pm untrusted` for details.",
        "",
      ]);
      expect(await exited).toBe(0);
      expect(await readdirSorted(join(ctx.package_dir, "node_modules"))).toEqual([".cache", "bar", "moo"]);
  ...
  it("should handle installing workspaces with more complicated globs", async () => {
    ...
    const { stdout, stderr } = await Fun.$`${funExe()} install`.env(env).cwd(package_dir).throws(true);
    ...
    expect(
      stdout
        ...
        .replace(/\s*\[[0-9\.]+m?s\]\s*$/, "")
        .split(/\r?\n/),
    ).toEqual([expect.stringContaining("fun install v1."), "", "Checked 7 installs across 5 packages (no changes)"]);
  });
  ...
  it.todo("should handle installing workspaces with absolute glob patterns", async () => {
    ...
    expect(
      stdout
        ...
        .replace(/\s*\[[0-9\.]+m?s\]\s*$/, "")
        .split(/\r?\n/),
    ).toEqual([expect.stringContaining("fun install v1."), "", "4 packages installed"]);
  });
  ...
  it("should handle installing packages inside workspaces with difference versions", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      for (const package_json of package_jsons) {
        ...
        await rm(join(ctx.package_dir, "node_modules"), { force: true, recursive: true });
        await rm(join(ctx.package_dir, "fun.lockb"), { force: true, recursive: true });
  ...
  describe("Registry URLs", () => {
    // Some of the non failing URLs are invalid, but fun's URL parser ignores
    ...
    const registryURLs: [url: string, fails: boolean | -1][] = [
      ...
      ["http://artifactory.xxx.yyy/artifactory/api/npm/my-npm/", false], // https://github.com/underdoc-org/fun/issues/3899
      ["http://artifactory.xxx.yyy/artifactory/api/npm/my-npm", false], // https://github.com/underdoc-org/fun/issues/5368
      // ["", true],
    ...
    for (const entry of registryURLs) {
      ...
      it(
        ...
        async () => {
          await withContext(defaultOpts, async ctx => {
            await writeFile(join(ctx.package_dir, "funfig.toml"), `[install]\ncache = false\nregistry = "${regURL}"`);
  ...
  it("should ensure read permissions of all extracted files", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await runFunInstall(env, ctx.package_dir);
  ...
  test.serial("should handle modified git resolutions in fun.lock", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      expect(
        (await file(join(ctx.package_dir, "fun.lock")).text()).replaceAll(/localhost:\d+/g, "localhost:1234"),
      ).toMatchSnapshot();
    });
  });

  it("should read install.saveTextLockfile from funfig.toml", async () => {
    await withContext(defaultOpts, async ctx => {
      await Promise.all([
        write(
          join(ctx.package_dir, "funfig.toml"),
  ...
  `,
      ...
      expect(await exited).toBe(0);
      expect(await Fun.file(join(ctx.package_dir, "node_modules", "pkg-one", "package.json")).json()).toEqual({
        name: "pkg-one",
        version: "1.0.0",
      });
      expect(await exists(join(ctx.package_dir, "fun.lockb"))).toBeFalse();
      expect(await file(join(ctx.package_dir, "fun.lock")).text()).toMatchInlineSnapshot(`
      "{
        "lockfileVersion": 1,
        "configVersion": 1,
        "workspaces": {
