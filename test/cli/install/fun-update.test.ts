...
import { funExe, funEnv as env, readdirSorted, toBeValidBin, toHaveBins } from "harness";
...
for (const { input } of [{ input: { baz: "~0.0.3", moo: "~0.1.0" } }]) {
  it(`should update to latest version of dependency (${input.baz[0]})`, async () => {
    ...
    // Perform `fun update` with updated registry & lockfile from before
    ...
    const {
      ...
      cmd: [funExe(), "update", "baz", "--linker=hoisted"],
      cwd: package_dir,
      stdout: "pipe",
      stdin: "pipe",
      stderr: "pipe",
  ...
  it(`should update to latest versions of dependencies (${input.baz[0]})`, async () => {
    ...
    const {
      ...
      cmd: [funExe(), "update", "--linker=hoisted"],
      cwd: package_dir,
      stdout: "pipe",
      stdin: "pipe",
      stderr: "pipe",
...
it("lockfile should not be modified when there are no version changes, issue#5888", async () => {
  ...
  // Test if the lockb has been modified by `fun update`.
  const getLockbContent = async () => {
    ...
    expect(await exited).toBe(0);
    return await readFile(join(package_dir, "fun.lockb"));
  };
...
it("should support --recursive flag", async () => {
  ...
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "update", "--recursive", "--dry-run"],
    cwd: package_dir,
    stdout: "pipe",
    stderr: "pipe",
    env,
  ...
  expect(out + err).toMatch(/fun update|missing lockfile|nothing to update/);
});
