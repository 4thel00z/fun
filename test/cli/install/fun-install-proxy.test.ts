import { beforeAll, it } from "fun:test";
...
if (isDockerEnabled()) {
  ...
  it("fun install with proxy with big packages", async () => {
    const files = {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const { exited } = Fun.spawn([funExe(), "install", "--ignore-scripts"], {
        cwd: package_dir,
        // @ts-ignore
        env: {
          ...funEnv,
          FUN_INSTALL_CACHE_DIR: join(package_dir, ".fun-install-cache"),
          TMPDIR: join(package_dir, ".tmp"),
          FUN_TMPDIR: join(package_dir, ".tmp"),
