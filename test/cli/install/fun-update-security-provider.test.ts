...
test("security scanner blocks fun update on fatal advisory", async () => {
  const urls: string[] = [];
  setHandler(
    dummyRegistry(urls, {
      "0.1.0": {},
...
test("security scanner does not run on fun update when disabled", async () => {
  const urls: string[] = [];
  setHandler(
    dummyRegistry(urls, {
      "0.1.0": {},
  ...
  // Remove funfig.toml to ensure no security scanner
  await write("funfig.toml", "");
