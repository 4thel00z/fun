...
import { afterAll, afterEach, beforeAll, beforeEach, expect, it, test } from "fun:test";
import { exists, mkdir, writeFile } from "fs/promises";
import { funEnv, funExe, funEnv as env, readdirSorted, tmpdirSync } from "harness";
...
it("should remove all cache", async () => {
  ...
  let cache_dir: string = join(package_dir, "node_modules", ".cache");
  {
    const { stderr, stdout, exited } = spawn({
      ...
      env: {
        ...env,
        FUN_INSTALL_CACHE_DIR: cache_dir,
      },
    });
    const err = await stderr.text();
    expect(err).not.toContain("error:");
  ...
  expect(await new Response(stdout2).text()).toInclude("Cleared 'fun install' cache\n");
  expect(await exited2).toBe(0);
  expect(await exists(cache_dir)).toBeFalse();
});

it("fun pm migrate", async () => {
...
test("fun whoami executes pm whoami", async () => {
  // Test that "fun whoami" doesn't show reservation message and instead executes pm whoami
...
test("fun pm whoami still works", async () => {
  // Test that "fun pm whoami" still works as expected
  // First create a simple package.json
  await writeFile(
    join(package_dir, "package.json"),
    JSON.stringify({
...
test.each([
  ...
  {
    name: "fun pm list works as alias for fun pm ls",
    cmd: ["pm", "list"],
...
test("fun list --all shows full dependency tree", async () => {
  const urls: string[] = [];
  setHandler(dummyRegistry(urls));
  await writeFile(
    join(package_dir, "package.json"),
