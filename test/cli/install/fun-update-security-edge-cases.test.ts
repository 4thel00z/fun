...
describe("fun update security edge cases", () => {
  test("fun update detects vulnerability in updated version that was safe before", async () => {
    ...
    await Fun.$`${funExe()} install`.cwd(dir).env(funEnv).quiet();
...
`,
  ...
  test("fun update <pkg> detects vulnerability in the specific updated package", async () => {
  ...
  scanner: {
    ...
    scan: async function(payload) {
      ...
      for (const pkg of payload.packages) {
        ...
        if (pkg.name === "axios" && Fun.semver.satisfies(pkg.version, ">=0.21.2")) {
          results.push({
            package: "axios",
            level: "fatal",
...
`,
  ...
  test("fun update detects newly discovered vulnerability in existing package", async () => {
    ...
    const dir = tempDirWithFiles("update-newly-discovered", {
      ...
      // Initially no scanner in funfig
    });
...
`,
  ...
  test("fun pm scan detects vulnerability in existing transitive dependency after adding package", async () => {
...
`,
    ...
    const tempBunfig = join(dir, "funfig.toml");
    ...
    await Fun.$`${funExe()} add lodash`.cwd(dir).env(funEnv).quiet();

    // Now run pm scan with scanner to detect vulnerabilities
    const scanProc = Fun.spawn({
      cmd: [funExe(), "pm", "scan"],
      cwd: dir,
      stdout: "pipe",
      stderr: "pipe",
  ...
  test("fun update with version range change exposes vulnerability", async () => {
  ...
  scanner: {
    ...
    scan: async function(payload) {
      ...
      for (const pkg of payload.packages) {
        ...
        if (pkg.name === "minimist" && Fun.semver.satisfies(pkg.version, ">=1.2.6")) {
          results.push({
            package: "minimist",
            level: "fatal",
...
`,
  ...
  test("fun pm scan detects newly discovered vulnerabilities in existing lockfile", async () => {
    ...
    await Fun.write(join(dir, "funfig.toml"), `[install.security]\nscanner = "./scanner.js"`);
    await Fun.write(
      join(dir, "scanner.js"),
      `
module.exports = {
