...
describe.concurrent.each(["why", "pm why"])("fun %s", cmd => {
  ...
  it("should show help when no package is specified", async () => {
    ...
    expect(await stdout.text()).toContain(`fun why v${Fun.version.replace("-debug", "")}`);
  ...
  it("should show nested dependencies", async () => {
    ...
    const { stdout, exited } = spawn({
      cmd: [funExe(), ...cmd.split(" "), "mime-types"],
      cwd: tempDir,
      env: funEnv,
      stdout: "pipe",
      stderr: "inherit",
  ...
  it("should handle npm aliases", async () => {
    ...
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), ...cmd.split(" "), "alias-pkg"],
      cwd: tempDir,
      env: funEnv,
      stdout: "pipe",
      stderr: "inherit",
  ...
  it("should show error for non-existent package", async () => {
    ...
    const { stdout, stderr, exited } = spawn({
      cmd: [funExe(), ...cmd.split(" "), "non-existent-package"],
      cwd: tempDir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("should show dependency types correctly", async () => {
    ...
    const { stdout: devStdout, exited: devExited } = spawn({
      cmd: [funExe(), ...cmd.split(" "), "typescript"],
      cwd: tempDir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("should support version constraints in the query", async () => {
    ...
    const { stdout, exited } = spawn({
      cmd: [funExe(), ...cmd.split(" "), "react@^18.0.0"],
      cwd: tempDir,
      env: funEnv,
      stdout: "pipe",
      stderr: "inherit",
  ...
  it("should support the --top flag to limit dependency tree depth", async () => {
    ...
    const { stdout: stdoutWithTop, exited: exitedWithTop } = spawn({
      cmd: [funExe(), ...cmd.split(" "), "mime-db", "--top"],
      cwd: tempDir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("should support the --depth flag to limit dependency tree depth", async () => {
    ...
    const { stdout: stdoutDepth2, exited: exitedDepth2 } = spawn({
      cmd: [funExe(), ...cmd.split(" "), "mime-db", "--depth", "2"],
      cwd: testDir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
