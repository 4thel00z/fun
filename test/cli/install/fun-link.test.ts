...
it("should link and unlink workspace package", async () => {
  ...
  let { out, err } = await runFunInstall(env, link_dir);
  ...
  ({ stdout, stderr, exited } = spawn({
    cmd: [funExe(), "link", "moo", "--linker=hoisted"],
  ...
  expect((await stdout.text()).replace(/\s*\[[0-9\.]+ms\]\s*$/, "").split(/\r?\n/)).toEqual([
    expect.stringContaining("fun link v1."),
    "",
  ...
  ({ stdout, stderr, exited } = spawn({
    cmd: [funExe(), "link", "foo", "--linker=hoisted"],
    cwd: join(link_dir, "packages", "boba"),
    stdout: "pipe",
    stdin: "pipe",
    stderr: "pipe",
...
it("should link package", async () => {
  const link_name = basename(link_dir).slice("fun-link.".length);
  await writeFile(
    join(link_dir, "package.json"),
    JSON.stringify({
      name: link_name,
  ...
  expect(await new Response(stdout4).text()).toEqual(expect.stringContaining("fun link v1."));
  expect(await exited4).toBe(1);
});

it("should link scoped package", async () => {
  const link_name = `@${basename(link_dir).slice("fun-link.".length)}/foo`;
  await writeFile(
    join(link_dir, "package.json"),
    JSON.stringify({
      name: link_name,
  ...
  const err4 = stderrForInstall(await new Response(stderr4).text());
  expect(err4).toContain(`error: Package "${link_name}" is not linked`);
  expect((await new Response(stdout4).text()).split(/\r?\n/)).toEqual([expect.stringContaining("fun link v1."), ""]);
  expect(await exited4).toBe(1);
});

it("should link dependency without crashing", async () => {
  const link_name = basename(link_dir).slice("fun-link.".length) + "-really-long-name";
  await writeFile(
    join(link_dir, "package.json"),
    JSON.stringify({
      name: link_name,
  ...
  const { out: stdout2, err: stderr2, exited: exited2 } = await runFunInstall(env, package_dir);
  const err2 = stderrForInstall(await new Response(stderr2).text());
  expect(err2.split(/\r?\n/).slice(-2)).toEqual(["Saved lockfile", ""]);
  const out2 = await new Response(stdout2).text();
  expect(out2.replace(/\s*\[[0-9\.]+ms\]\s*$/, "").split(/\r?\n/)).toEqual([
