...
describe("fun update security scanning", () => {
  test("fun update without arguments scans all packages", async () => {
...
`,
    ...
    const funfigPath = join(dir, "funfig.toml");
    const funfigContent = await Fun.file(funfigPath).text();
    await Fun.write(funfigPath, ""); // Remove scanner config
    await Fun.$`${funExe()} install`.cwd(dir).env(funEnv).quiet();
    await Fun.write(funfigPath, funfigContent); // Restore scanner config
  ...
  test("fun update with specific packages only scans those packages", async () => {
...
`,
  ...
  test("fun update respects security scanner configuration", async () => {
    const dir = tempDirWithFiles("update-no-scanner", {
      ...
      // No funfig.toml with scanner configuration
    });
  ...
  test("fun update aborts on fatal vulnerabilities", async () => {
...
`,
  ...
  test.todo("fun update prompts for warnings when TTY available - requires TTY for interactive prompt", async () => {
...
`,
    ...
    const updateProc = Fun.spawn({
      ...
      stderr: "pipe",
      stdin: "pipe",
      env: { ...funEnv, FORCE_COLOR: "1" }, // Force color to simulate TTY
    });
  ...
  test("fun update shows dependency paths correctly", async () => {
