...
import { funEnv, funExe, DirectoryTree, gunzipJsonRequest, lazyPromiseLike, tempDirWithFiles } from "harness";
...
function doAuditTest(
  ...
  test(label, async () => {
    const dir = tempDirWithFiles("fun-test-audit-" + label.replace(/[^a-zA-Z0-9]/g, "-"), options.files);

    const cmd = [funExe(), "audit", ...(options.args ?? [])];
...
describe("`fun audit`", () => {
  ...
  doAuditTest("should exit code 1 when there are vulnerabilities", {
    ...
    fn: async ({ stdout }) => {
      expect(await stdout).toMatchSnapshot("fun-audit-expect-vulnerabilities-found");
    },
  });
  ...
  doAuditTest("should print valid JSON and exit 0 when --json is passed and there are no vulnerabilities", {
    ...
    fn: async ({ stdout }) => {
      ...
      expect(json).toMatchSnapshot("fun-audit-expect-valid-json-stdout-report-no-vulnerabilities");
    },
  });
  ...
  doAuditTest("should print valid JSON and exit 1 when --json is passed and there are vulnerabilities", {
    ...
    fn: async ({ stdout }) => {
      const out = await stdout;
      const json = JSON.parse(out); // this would throw making the test fail if the JSON was invalid
      expect(json).toMatchSnapshot("fun-audit-expect-valid-json-stdout-report-vulnerabilities");
    },
  });
  ...
  doAuditTest(
    "when a project has some safe dependencies and some vulnerable dependencies, we should not print the safe dependencies",
    {
      ...
      fn: async ({ stdout }) => {
        ...
        expect(out).toMatchSnapshot("fun-audit-expect-vulnerabilities-found");
      },
    },
  );
