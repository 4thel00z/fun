...
describe.concurrent("fun info", () => {
  ...
  describe("fun info (main command)", () => {
    it("should display package info for latest version", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "info", "is-number"], testDir);
    ...
    it("should display package info for specific version", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "info", "is-number@7.0.0"], testDir);
      expect(code).toBe(0);
    ...
    it("should display specific property", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "info", "@types/fun", "name"], testDir);
    ...
    it("should handle missing arguments", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "info"], testDir, false);
  ...
  describe("fun pm view (alias)", () => {
    it("should display package info for latest version", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "is-number"], testDir);
    ...
    it("should display package info for specific version", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "is-number@7.0.0"], testDir);
      expect(code).toBe(0);
      expect(output).toMatchInlineSnapshot(`
        "is-number@7.0.0 | MIT | deps: 0 | versions: 15
        Returns true if a number or string value is a finite number. Useful for regex matches, parsing, user input, etc.
    ...
    it("should display specific property", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "@types/fun", "name"], testDir);
    ...
    it("should display nested property", async () => {
      ...
      const { output, error, code } = await runCommand(
        [funExe(), "pm", "view", "is-number", "repository.url"],
        testDir,
      );
    ...
    it("should output JSON format with --json flag", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "is-number@7.0.0", "--json"], testDir);
    ...
    it("should handle non-existent package", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand(
        [funExe(), "pm", "view", "nonexistent-package-12345"],
        testDir,
        false,
      );
    ...
    it("should handle non-existent version", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "is-number@999.0.0"], testDir, false);
    ...
    it("should handle non-existent property", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand(
        [funExe(), "pm", "view", "is-number", "nonexistent"],
        testDir,
        false,
      );
    ...
    it("should handle malformed package specifier", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "@"], testDir, false);
    ...
    it("should handle scoped packages", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "@types/node"], testDir);
    ...
    it("versions should work", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "fs", "versions"], testDir);
    ...
    it("versions[0] should work", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "fs", "versions[0]"], testDir);
    ...
    it("should handle dist-tags like latest", async () => {
      const testDir = await setupTest();
      const { output, error, code } = await runCommand([funExe(), "pm", "view", "fs@latest"], testDir);
