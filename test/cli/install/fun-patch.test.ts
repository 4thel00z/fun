...
describe("fun patch <pkg>", async () => {
    ...
    describe("inside workspace with hoisting", async () => {
      ...
      for (const [arg, path] of args) {
        test(arg, async () => {
          ...
          await $`${funExe()} i`.env(funEnv).cwd(tempdir);

          let result = await $` ${funExe()} patch ${arg}`.env(funEnv).cwd(tempdir);
          ...
          await $`echo LOL > ${path}/index.d.ts`.env(funEnv).cwd(tempdir);

          expectNoError(await $`${funExe()} patch --commit ${arg}`.env(funEnv).cwd(tempdir));

          expect(await $`cat ${path}/index.d.ts`.env(funEnv).cwd(tempdir).text()).toEqual("LOL\n");

          expect(
            (await $`cat package.json`.cwd(tempdir).env(funEnv).json()).patchedDependencies["@types/ws@8.5.4"],
    ...
    describe("inside workspace with multiple workspace packages with same dependency", async () => {
      ...
      for (const [arg, path] of args) {
        test(arg, async () => {
          ...
          let result = await $`cd packages/eslint-config; ${funExe()} patch ${arg}`.env(funEnv).cwd(tempdir);
          expect(result.stderr.toString()).not.toContain("error");
          ...
          expectNoError(await $`cd packages/eslint-config; ${funExe()} patch --commit ${arg}`.env(funEnv).cwd(tempdir));
    ...
    describe("inside workspace package", async () => {
      ...
      for (const [arg, path] of args) {
        test(arg, async () => {
          ...
          await $`echo LOL > ${path}/index.js`.env(funEnv).cwd(tempdir);

          expectNoError(await $`cd packages/eslint-config; ${funExe()} patch --commit ${arg}`.env(funEnv).cwd(tempdir));

          expect(await $`cat ${path}/index.js`.env(funEnv).cwd(tempdir).text()).toEqual("LOL\n");
    ...
    describe("inside ROOT workspace package", async () => {
      ...
      for (const [arg, path, version, patch_path] of args) {
        test(arg, async () => {
          ...
          expect((await $`cat package.json`.cwd(tempdir).env(funEnv).json()).patchedDependencies[version]).toEqual(
            patch_path,
          );
        });
      }
    ...
    function makeTest(pkgName: string, version: string, folder_in_node_modules: string = `${pkgName}`) {
      test(
        ...
        async () => {
          ...
          console.log("TEMPDIR", tempdir);
          expectNoError(await $`${funExe()} i`.env(funEnv).cwd(tempdir));
          expectNoError(await $`${funExe()} patch ${pkgName}@${version}`.env(funEnv).cwd(tempdir));
          await $`echo ${dummyCode} > node_modules/${folder_in_node_modules}/index.js`.env(funEnv).cwd(tempdir);
          const { type, module, exports, ...package_json }: Record<string, any> =
            await $`cat node_modules/${folder_in_node_modules}/package.json`.env(funEnv).cwd(tempdir).json();
          ...
          expectNoError(
            await $`${funExe()} patch --commit node_modules/${folder_in_node_modules}`.env(funEnv).cwd(tempdir),
          );

          const { stdout } = await $`${funExe()} run index.ts`.env(funEnv).cwd(tempdir);
  ...
  test("should patch a package when it is already patched", async () => {
    ...
    await $`${funExe()} i`.env(funEnv).cwd(tempdir);
    const { stderr } = await $`${funExe()} patch is-odd@0.1.2`.env(funEnv).cwd(tempdir).throws(false);
...
module.exports = function isOdd(i) {
  ...
  return !!(~~i & 1);
    ...
    await $`echo ${firstChange} > node_modules/is-even/node_modules/is-odd/index.js`.env(funEnv).cwd(tempdir);

    const { stderr: stderr2 } = await $`${funExe()} patch --commit node_modules/is-even/node_modules/is-odd`
      .env(funEnv)
      .cwd(tempdir)
      .throws(false);
    expect(stderr2.toString()).not.toContain("error");

    const { stderr: stderr3 } = await $`${funExe()} patch is-odd@0.1.2`.env(funEnv).cwd(tempdir).throws(false);
...
module.exports = function isOdd(i) {
  ...
  return !!(~~i & 1);
    ...
    await $`echo ${secondChange} > node_modules/is-even/node_modules/is-odd/index.js`.env(funEnv).cwd(tempdir);
    const { stderr: stderr4 } = await $`${funExe()} patch --commit node_modules/is-even/node_modules/is-odd`
      .env(funEnv)
      .cwd(tempdir)
      .throws(false);
    expect(stderr4.toString()).not.toContain("error");

    await $`cat patches/is-odd@0.1.2.patch`.env(funEnv).cwd(tempdir);

    await $`${funExe()} i`.env(funEnv).cwd(tempdir).throws(false);
    const { stdout } = await $`${funExe()} run index.ts`.env(funEnv).cwd(tempdir).throws(false);
  ...
  test("bad patch arg", async () => {
    ...
    await $`${funExe()} i`.env(funEnv).cwd(tempdir);
    const { stderr, exitCode } = await $`${funExe()} patch lkflksdkfj`.env(funEnv).cwd(tempdir).throws(false);
  ...
  test("bad patch commit arg", async () => {
    ...
    await $`${funExe()} i`.env(funEnv).cwd(tempdir);
    const { stderr } = await $`${funExe()} patch is-even`.env(funEnv).cwd(tempdir);
    expect(stderr.toString()).not.toContain("error");

    const { stderr: stderr2 } = await $`${funExe()} patch --commit lskfjdslkfjsldkfjlsdkfj`
      .env(funEnv)
      .cwd(tempdir)
      .throws(false);
  ...
  function makeTest(
    ...
    test(name, async () => {
      ...
      {
        const { stderr } = await $`${funExe()} i --linker hoisted`.env(funEnv).cwd(filedir);
        expect(stderr.toString()).not.toContain("error");
      }

      {
        const { stderr, stdout } = await $`${funExe()} patch ${patchArg} --linker hoisted`.env(funEnv).cwd(filedir);
        expect(stderr.toString()).not.toContain("error");
...
Once you're done with your changes, run:
  ...
  fun patch --commit '${platformPath(expected.patchPath)}'`,
        );
      }

      {
        ...
        await $`echo ${newCode} > ${expected.patchPath}/index.js`.env(funEnv).cwd(filedir);
        const { stderr, stdout } = await $`${funExe()} patch --commit ${expected.patchPath}`.env(funEnv).cwd(filedir);
      }

      const output = await $`${funExe()} run index.ts`.env(funEnv).cwd(filedir).text();
  ...
  test(
    ...
    async () => {
      ...
      {
        const { stderr } = await $`${funExe()} i`.env(funEnv).cwd(filedir);
        expect(stderr.toString()).not.toContain("error");
      }

      {
        const { stderr, stdout } = await $`${funExe()} patch lodash`.env(funEnv).cwd(filedir);
        expect(stderr.toString()).not.toContain("error");
      }
  ...
  test("should overwrite the node_modules folder of the package", async () => {
    ...
    for (const patchArg of patchArgs) {
      ...
      {
        const { stderr } = await $`${funExe()} i --backend hardlink`.env(funEnv).cwd(filedir);
...
`;
        ...
        await $`${funExe()} patch ${patchArg}`.env(funEnv).cwd(filedir);
        await $`echo ${newCode} > node_modules/is-even/index.js`.env(funEnv).cwd(filedir);
      }
      ...
      const { stdout, stderr } = await $`${funExe()} run index.ts`.env(funEnv).cwd(tempdir);
  ...
  test("should overwrite nested node_modules folder of the package", async () => {
    ...
    for (const patchArg of patchArgs) {
      ...
      {
        const { stderr } = await $`${funExe()} i --backend hardlink --linker hoisted`.env(funEnv).cwd(filedir);
        expect(stderr.toString()).toContain("Saved lockfile");
...
`;
        ...
        await $`${funExe()} patch ${patchArg} --linker hoisted`.env(funEnv).cwd(filedir);
        await $`echo ${newCode} > node_modules/is-even/node_modules/is-odd/index.js`.env(funEnv).cwd(filedir);
      }
