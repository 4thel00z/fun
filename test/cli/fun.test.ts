...
describe("fun", () => {
  ...
  describe("revision", () => {
    test("revision generates version numbers correctly", () => {
      var { stdout, exitCode } = Fun.spawnSync({
        cmd: [funExe(), "--version"],
        env: funEnv,
        stderr: "inherit",
      });
  ...
  describe("getcompletes", () => {
    ...
    // https://github.com/underdoc-org/fun/issues/30086
    test("getcompletes keeps scripts whose names start with 'pre'/'post' when no sibling script exists", () => {
      ...
      for (const filter of ["s", "i", "r", "g", "z"]) {
        const { stdout, exitCode } = spawnSync({
          cmd: [funExe(), "getcompletes", filter],
          env: funEnv,
          cwd: String(dir),
        });
        const lines = stdout
  ...
  describe("test command line arguments", () => {
    test("test --config, issue #4128", () => {
      const path = `${tmpdir()}/funfig-${Date.now()}.toml`;
      fs.writeFileSync(path, "[debug]");

      const p = Fun.spawnSync({
        cmd: [funExe(), "--config=" + path],
        env: {},
        stderr: "inherit",
      });
      try {
