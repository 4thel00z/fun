...
`;
    ...
    const log = await Fun.file(logPath).text();
...
test("sequential test 1", async () => {
  ...
  sharedCounter = 1;
  await Fun.sleep(50); // Give time for race condition if concurrent
...
`;
  ...
  test("concurrent flag overrides concurrent-test-glob", async () => {
    using dir = tempDir("concurrent-override", {
      "funfig.toml": `[test]\nconcurrentTestGlob = "**/concurrent-*.test.ts"`,
      "sequential.test.ts": `import { test, expect } from "fun:test";
