...
import { beforeAll, describe, expect, it, test } from "fun:test";
...
describe("fun test", () => {
  test("running a non-existent absolute file path is a 1 exit code", () => {
    const spawn = Fun.spawnSync({
      cmd: [funExe(), "test", join(import.meta.dirname, "non-existent.test.ts")],
      env: funEnv,
      stdin: "ignore",
      stdout: "inherit",
      stderr: "inherit",
  ...
  describe("when filters are provided", () => {
    ...
    beforeAll(() => {
      ...
      `;
      dir = tempDirWithFiles("fun-test-filtering", {
  ...
  test("works with dynamic import", () => {
    const stderr = runTest({
      args: [],
      input: `
        const { test, expect } = await import("fun:test");
        test("test #1", () => {
          expect().pass();
        })
      `,
  ...
  describe("only", () => {
    test("should run nested describe.only", () => {
      const stderr = runTest({
        args: [],
        input: `
            import { test, describe } from "fun:test";
            describe("outer", () => {
  ...
  describe("--timeout", () => {
    ...
    // TODO: https://github.com/underdoc-org/fun/issues/8069
    ...
    // Possibly fixed by https://github.com/underdoc-org/fun/pull/8076/files
  ...
  describe("support for Github Actions", () => {
    ...
    test.each(["beforeAll", "beforeEach", "afterEach", "afterAll"])("should annotate errors in a %s callback", type => {
      const stderr = runTest({
        input: `
          import { test, ${type} } from "fun:test";
          ${type}(() => {
            throw new Error();
          });
          test("test", () => {});
  ...
  test("Prints error when no test matches", () => {
    ...
    expect(
      stderr
        .replace(/fun-test-(.*)\.test\.ts/, "fun-test-*.test.ts")
        .trim()
        .replace(/\[.*\ms\]/, "[xx ms]"),
  ...
  test("--tsconfig-override works", () => {
    ...
    const successResult = spawnSync({
      cmd: [funExe(), "test", "--tsconfig-override", "test-tsconfig.json", "math.test.ts"],
      env: funEnv,
      cwd: dir,
      stdout: "pipe",
      stderr: "pipe",
...
function createTest(input?: string | (string | { filename: string; contents: string })[], filename?: string): string {
  ...
  for (const input of inputs) {
    ...
    const name = typeof input === "string" ? (filename ?? `fun-test-${Math.random()}.test.ts`) : input.filename;
...
function runTest({
  ...
  try {
    const { stderr, exitCode } = spawnSync({
      cwd,
      cmd: [funExe(), "test", ...args],
      env: { ...funEnv, AGENT: "0", ...env },
      stderr: "pipe",
      stdout: "ignore",
    });
