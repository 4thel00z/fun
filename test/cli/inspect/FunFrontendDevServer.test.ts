...
class FunFrontendDevServerSession extends InspectorSession {
  constructor() {
    super();
  }
  ...
  async enable(): Promise<void> {
    ...
    this.send("FunFrontendDevServer.enable");
    this.send("LifecycleReporter.enable");
    await this.sendAndWait("Inspector.initialized");
  }

  async disable(): Promise<void> {
    await this.sendAndWait("FunFrontendDevServer.disable");
  }
...
describe.if(isPosix)("FunFrontendDevServer inspector protocol", () => {
  let devServerProcess: Subprocess;
  let serverUrl: URL;
  let session: FunFrontendDevServerSession;
  let tempdir: string;
  let socketPath: string;

  beforeAll(async () => {
    tempdir = tmpdirSync("fun-frontend-dev-server-test");
    ...
    try {
      ...
      devServerProcess = spawn({
        cmd: [funExe(), `--inspect=unix:${socketPath}`, join(tempdir, "server.ts")],
        env: funEnv,
        cwd: tempdir,
        stdout: "pipe",
        stderr: "pipe",
      ...
      session = new FunFrontendDevServerSession();
      ...
      // Enable the FunFrontendDevServer domain
  ...
  async function createHMRClient() {
    ...
    wsUrl.protocol = "ws:";
    wsUrl.pathname = "/_fun/hmr"; // HMR endpoint
  ...
  test("should receive clientConnected and clientDisconnected events", async () => {
    ...
    const eventPromise = session.waitForEvent("FunFrontendDevServer.clientConnected");
    ...
    const disconnectEventPromise = session.waitForEvent("FunFrontendDevServer.clientDisconnected");
  ...
  test("should notify on bundleStart and bundleComplete events", async () => {
    ...
    const bundleStartEventPromise = session.waitForEvent("FunFrontendDevServer.bundleStart");
    const bundleCompleteEventPromise = session.waitForEvent("FunFrontendDevServer.bundleComplete");
  ...
  test("should notify on bundleFailed events", async () => {
    ...
    const bundleStartPromise = session.waitForEvent("FunFrontendDevServer.bundleStart");

    // Then listen for bundleFailed event
    const bundleFailedPromise = session.waitForEvent("FunFrontendDevServer.bundleFailed");
  ...
  test("should notify on clientNavigated events", async () => {
    ...
    const connectedEventPromise = session.waitForEvent("FunFrontendDevServer.clientConnected");
    ...
    const clientNavigatedPromise = session.waitForEvent("FunFrontendDevServer.clientNavigated");
  ...
  test("should notify on consoleLog events", async () => {
    ...
    const consoleLogPromise = session.waitForEvent("FunFrontendDevServer.consoleLog");
    ...
    // Test error log
    const consoleErrorPromise = session.waitForEvent("FunFrontendDevServer.consoleLog");
  ...
  test.todo("should notify on clientErrorReported events", async () => {
    ...
    const clientErrorPromise = session.waitForEvent("FunFrontendDevServer.clientErrorReported");
  ...
  test.todo("should notify on graphUpdate events", async () => {
    ...
    const graphUpdatePromise = session.waitForEvent("FunFrontendDevServer.graphUpdate");
