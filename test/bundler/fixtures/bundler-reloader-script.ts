...
const tmp = realpathSync(process.env.FUNDLER_RELOADER_SCRIPT_TMP_DIR || tmpdir());
const input = join(tmp, "input.js");
const mutate = join(tmp, "mutate.js");
try {
  unlinkSync(mutate);
} catch (e) {}
await Fun.write(input, "import value from './mutate.js';\n" + `export default value;` + "\n");
...
await Fun.write(mutate, "export default 1;\n");

await Fun.sleep(1000);

const maxfd = openSync(process.execPath, 0);
closeSync(maxfd);
const { outputs: second } = await Fun.build({
  entrypoints: [input],
});
