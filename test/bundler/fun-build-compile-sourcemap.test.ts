...
describe("Fun.build compile with sourcemap", () => {
  const helperFiles = {
...
main();`,
  ...
  async function testSourcemapOption(sourcemapValue: "inline" | "external" | true, testName: string) {
    ...
    expect(await Fun.file(executablePath).exists()).toBe(true);
    ...
    // Should NOT see the bundled virtual path (/$funfs/root/ on Unix, B:/~FUN/root/ on Windows)
  ...
  test("compile without sourcemap should show bundled paths", async () => {
    ...
    // Without sourcemaps, we should see the bundled virtual path (/$funfs/root/ on Unix, B:/~FUN/root/ on Windows)
    expect(stderr).toMatch(/(\$funfs|~FUN)\/root\//);
  ...
  test("compile with sourcemap: external writes .map file to disk", async () => {
    ...
    expect(await Fun.file(mapPath).exists()).toBe(true);

    // Validate the sourcemap is valid JSON with expected fields
    const mapContent = JSON.parse(await Fun.file(mapPath).text());
  ...
  test("compile without sourcemap does not write .map file", async () => {
    ...
    expect(await Fun.file(`${executablePath}.map`).exists()).toBe(false);
...
`,
    ...
    for (const sm of sourcemapOutputs) {
      ...
      expect(await Fun.file(sm.path).exists()).toBe(true);
      ...
      const mapContent = JSON.parse(await Fun.file(sm.path).text());
      expect(mapContent.version).toBe(3);
      expect(mapContent.mappings).toBeString();
    }
  ...
  test("compile with --outfile subdir/myapp writes .map next to executable", async () => {
    ...
    // Use CLI: fun build --compile --outfile subdir/myapp --sourcemap=external
    await using proc = Fun.spawn({
      cmd: [
        funExe(),
        "build",
    ...
    expect(await Fun.file(join(subdirPath, `myapp${exeSuffix}`)).exists()).toBe(true);

    // The .map file should be in subdir/ (next to the executable)
    const glob = new Fun.Glob("*.map");
    const mapFiles = Array.from(glob.scanSync({ cwd: subdirPath }));
    expect(mapFiles.length).toBe(1);

    // Validate the sourcemap is valid JSON
    const mapContent = JSON.parse(await Fun.file(join(subdirPath, mapFiles[0])).text());
    expect(mapContent.version).toBe(3);
    expect(mapContent.mappings).toBeString();

    // Verify no .map was written into the doubled path subdir/subdir/
    expect(await Fun.file(join(String(dir), "subdir", "subdir", "myapp.map")).exists()).toBe(false);
  });
...
main();`,
    ...
    expect(await Fun.file(executable).exists()).toBe(true);
    ...
    // Should NOT show bundled paths (/$funfs/root/ on Unix, B:/~FUN/root/ on Windows)
    expect(stderr).not.toMatch(/(\$funfs|~FUN)\/root\//);
