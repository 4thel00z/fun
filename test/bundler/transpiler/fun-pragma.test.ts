...
const fixturePath = (...segs: string[]): string => path.join(import.meta.dirname, "fixtures", "fun-pragma", ...segs);
...
describe("@fun pragma", () => {
  describe("valid files", async () => {
    ...
    it.each(passFiles)("fun run %s", async file => {
  ...
  describe("invalid files", async () => {
    ...
    it.each(failFiles)("fun run %s", async file => {
