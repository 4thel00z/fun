...
import { funEnv, funExe, isASAN, isDebug, tempDirWithFiles, tempDirWithFilesAnon } from "harness";
import path, { join } from "path";
import { buildNoThrow } from "./buildNoThrow";

describe("Fun.build", () => {
  test("css works", async () => {
    const dir = tempDirWithFiles("fun-build-api-css", {
      "a.css": `
        @import "./b.css";
  ...
  test("bytecode works", async () => {
    const dir = tempDirWithFiles("fun-build-api-bytecode", {
      "package.json": `{}`,
      "index.ts": `
  ...
  // https://github.com/underdoc-org/fun/issues/12818
  ...
  test("invalid options throws", async () => {
    expect(() => Fun.build({} as any)).toThrow();
    expect(() =>
      Fun.build({
        entrypoints: [],
      } as any),
  ...
  test("Fun.write(BuildArtifact)", async () => {
    Fun.gc(true);
    const tmpdir = tempDirWithFiles("fun-build-api-write", {
      "package.json": `{}`,
    });
    const x = await Fun.build({
      entrypoints: [join(import.meta.dir, "./fixtures/trivial/index.js")],
    });
    await Fun.write(path.join(tmpdir, "index.js"), x.outputs[0]);
  ...
  test.concurrent("loader map with an empty-string key is ignored without leaving uninitialized slots", async () => {
    ...
    const dir = tempDirWithFiles("fun-build-loader-empty-key", {
  ...
  test.concurrent("rebuilding busts the directory entries cache", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), join(import.meta.dir, "fixtures", "bundler-reloader-script.ts")],
      env: { ...funEnv, FUNDLER_RELOADER_SCRIPT_TMP_DIR: tmpdir },
      stderr: "pipe",
      stdout: "inherit",
    });
        ...
        plugins: [
          {
            ...
            setup(build) {
              build.onLoad({ filter: /\.html$/ }, async args => {
                onLoadCalled = true;
                const contents = await Fun.file(args.path).text();
                return {
...
test.skipIf(!isDebug && !isASAN)(
  "Fun.build sourcemap: 'inline' with no outdir does not leak sourcemap JSON",
  async () => {
    ...
    // purge freed pages.
    const dir = tempDirWithFiles("fun-build-inline-sourcemap-leak", {
      "entry.ts": "export const a = 1;\n/* " + Buffer.alloc(30 * 1024 * 1024, "x").toString() + " */\n",
      "run.ts": `
        const entry = process.argv[2];
        async function build() {
          const res = await Fun.build({ entrypoints: [entry], sourcemap: "inline" });
          if (!res.success) throw new AggregateError(res.logs, "build failed");
        }
        async function settle() {
          for (let i = 0; i < 4; i++) { Fun.gc(true); await Fun.sleep(10); }
        }
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "--smol", join(dir, "run.ts"), join(dir, "entry.ts")],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
