...
describe("Fun.build compile", () => {
  test("compile with current platform target string", async () => {
    ...
    const target = `fun-${os}-${arch}${musl}` as any;
    ...
    const exists = await Fun.file(result.outputs[0].path).exists();
  ...
  test("compile with invalid target fails gracefully", async () => {
    ...
    expect(() =>
      Fun.build({
        ...
        compile: {
          target: "fun-invalid-platform",
          outfile: join(dir, "invalid-app"),
        },
      }),
    ).toThrowErrorMatchingInlineSnapshot(`"Unknown compile target: fun-invalid-platform"`);
  });
...
describe("compiled binary validity", () => {
  test("output binary has valid executable header", async () => {
    ...
    const file = Fun.file(result.outputs[0].path);
...
if (isLinux) {
  describe("ELF section", () => {
    ...
    test("compiled binary with large payload runs correctly", async () => {
      // Generate a string payload >16KB to exceed the initial .fun section allocation
      // (FUN_COMPILED is aligned to 16KB). This forces the expansion path in elf.zig
    ...
    test("compiled binary has .fun ELF section", async () => {
      ...
      // Verify .fun ELF section exists by reading section headers
      const file = Fun.file(result.outputs[0].path);
      ...
      expect(foundFunSection).toBe(true);
    });
    ...
    //   2. Fit the .fun payload inside an existing writable PT_LOAD's
    //      `[p_vaddr, p_vaddr + p_memsz)` range — i.e. the writable segment
    //      was GROWN to cover .fun rather than a new segment being added.
    //
    // The gate here is purely structural (we check the ELF layout); we don't
    // need a WSL1 host to validate the fix.
    //
    // Higher per-test timeout because `fun build --compile` copies + rewrites
    // the entire fun binary (~1GB under debug+ASAN), which blows the 5s
    // default.
    test("compiled binary preserves PT_GNU_STACK and no late PT_LOAD for .fun (#29963)", async () => {
      ...
      const bytes = new Uint8Array(await Fun.file(result.outputs[0].path).arrayBuffer());
      const view = new DataView(bytes.buffer);
      ...
      // Locate .fun's vaddr by walking section headers.
      const shoff = Number(view.getBigUint64(40, true));
      const shentsize = view.getUint16(58, true);
      const shnum = view.getUint16(60, true);
      const shstrndx = view.getUint16(62, true);
      ...
      for (let i = 0; i < shnum; i++) {
        ...
        if (name === ".fun") {
          funAddr = view.getBigUint64(hdrOff + 16, true); // sh_addr
          funSize = view.getBigUint64(hdrOff + 32, true); // sh_size
          break;
        }
      }
      expect(funAddr).not.toBe(0n);
      expect(funSize).toBeGreaterThan(0n);

      // Walk program headers: count PT_LOADs, require PT_GNU_STACK to still
      // be present, and find the writable PT_LOAD containing .fun.
      let hasGnuStack = false;
      let loadCount = 0;
      let writableLoadCoversFun = false;
      for (let i = 0; i < phnum; i++) {
        ...
        if (pType === PT_LOAD) {
          loadCount++;
          if ((pFlags & PF_W) !== 0 && pVaddr <= funAddr && funAddr + funSize <= pVaddr + pMemsz) {
            writableLoadCoversFun = true;
          }
        }
      }
      ...
      // #29963: the writable PT_LOAD must have been grown to cover .fun,
      // rather than a new late PT_LOAD being appended.
      expect(writableLoadCoversFun).toBe(true);
      // A stock fun has 3 PT_LOAD segments; the fix must not add a 4th.
      ...
      expect(funAddr % 128n).toBe(0n);

      // Sanity: the binary still runs and produces the expected output.
      // (Ignore stderr — a debug-ASAN fun may log `hintSourcePagesDontNeed`
      // advisory warnings from the compiled binary's mmap'd .fun segment.)
      await using proc = Fun.spawn({
        cmd: [result.outputs[0].path],
        stdout: "pipe",
        stderr: "pipe",
