...
const testOnly = typeof Fun === 'undefined' ? test('only test', { only: true }) : undefined; // disabled in fun because test.only is disabled in CI environments and it will skip the describe/it
const testTodo = test('todo test', { todo: true });
const testSkip = test('skip test', { skip: true });
const testOnlyShorthand = typeof Fun === 'undefined' ? test.only('only test shorthand') : undefined; // disabled in fun because test.only is disabled in CI environments and it will skip the describe/it
