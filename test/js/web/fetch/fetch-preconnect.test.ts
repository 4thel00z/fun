...
describe.concurrent.todoIf(isWindows)("fetch.preconnect", () => {
  it("fetch.preconnect works", async () => {
    const { promise, resolve } = Promise.withResolvers<Fun.Socket>();
    ...
    for (let endOrTerminate of ["end", "terminate", "shutdown"]) {
      describe(endOrTerminate, () => {
        for (let at of ["before", "middle", "after"]) {
          it(at, async () => {
            let { promise, resolve } = Promise.withResolvers<Fun.Socket>();
            using listener = Fun.listen({
              port: 0,
              hostname: "localhost",
              socket: {
            ...
            ({ promise, resolve } = Promise.withResolvers<Fun.Socket>());
            if (at === "before") {
              await Fun.sleep(16);
  ...
  it("--fetch-preconnect works", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), `--fetch-preconnect=http://localhost:${listener.port}`, "--eval", "Fun.sleep(64)"],
