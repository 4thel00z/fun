...
import { connect, fileURLToPath, SocketHandler, spawn } from "fun";
import { createSocketPair } from "fun:internal-for-testing";
import { describe, expect, it, jest } from "fun:test";
import { closeSync } from "fs";
import { funEnv, funExe, expectMaxObjectTypeCount, getMaxFD, isWindows, tempDir, tls } from "harness";
describe.concurrent("socket", () => {
  ...
  it("connect without top level await should keep process alive", async () => {
    ...
    const proc = Fun.spawn({
      cmd: [funExe(), "keep-event-loop-alive.js", String(server.port), server.hostname],
      cwd: import.meta.dir,
      env: funEnv,
    });
    await proc.exited;
  ...
  it("connect() should return the socket object", async () => {
    const { exited, stdout, stderr } = spawn({
      cmd: [funExe(), "connect-returns-socket.js"],
      cwd: import.meta.dir,
      stdout: "pipe",
      stdin: null,
      stderr: "pipe",
  ...
  it("reload() should preserve active_connections (no UAF / counter underflow)", async () => {
    await using proc = Fun.spawn({
      cmd: [funExe(), fileURLToPath(new URL("./socket-reload-fixture.ts", import.meta.url))],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
...
it("should throw on empty hostname from truthy non-string value", () => {
  ...
  for (const hostname of [[], new String("")]) {
    expect(() => Fun.listen({ hostname: hostname as any, port: 0, socket })).toThrow('Expected a non-empty "hostname"');
    expect(() => Fun.connect({ hostname: hostname as any, port: 0, socket })).toThrow(
...
it("should throw on empty unix path from truthy non-string value", () => {
  ...
  expect(() => Fun.listen({ unix: [] as any, socket })).toThrow("SocketOptions.unix must be a string");
  expect(() => Fun.connect({ unix: [] as any, socket })).toThrow("SocketOptions.unix must be a string");
});
...
it("TLS client: flush() after end() does not double-teardown before deferred onClose", async () => {
  ...
  await using proc = Fun.spawn({
    cmd: [funExe(), "run.ts"],
    env: { ...funEnv, TLS_JSON: JSON.stringify(tls) },
    cwd: String(dir),
    stdout: "pipe",
    stderr: "pipe",
  });
...
// Fun.connect() on a Windows named pipe takes a dedicated early branch in
...
describe.skipIf(!isWindows)("Fun.connect named-pipe client Handlers lifecycle", () => {
  it("open → close cleans up without reading past the Handlers allocation", async () => {
    const src = /* js */ `
      const pipe = "\\\\\\\\.\\\\pipe\\\\fun-test-connect-" + Math.random().toString(36).slice(2);
  ...
  it("failed connect to a non-existent pipe rejects and cleans up", async () => {
    const src = /* js */ `
      const pipe = "\\\\\\\\.\\\\pipe\\\\fun-test-missing-" + Math.random().toString(36).slice(2);
      ...
      if (!rejected) {
        console.error("expected Fun.connect to reject for a non-existent pipe");
        process.exit(1);
      }
