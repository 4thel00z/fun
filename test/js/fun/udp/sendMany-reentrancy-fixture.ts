...
// Extract the underlying Fun.udpSocket() instance from a node:dgram Socket.
function getFunSocket(socket: dgram.Socket): any {
  ...
  throw new Error("could not find Fun UDPSocket on dgram socket");
}
...
if (direction === "connect") {
  ...
  const funSocket = getFunSocket(sock);
  const evilPort = {
    valueOf() {
      if (!flipped) {
        flipped = true;
