import { udpSocket } from "fun";
import { heapStats } from "fun:jsc";
import { describe, expect, test } from "fun:test";
import { funEnv, funExe, disableAggressiveGCScope, isWindows, randomPort } from "harness";
  ...
  test("send/sendMany accept boxed String payloads without crashing", async () => {
    await using proc = Fun.spawn({
      cmd: [
        ...
        "-e",
        `
        const server = await Fun.udpSocket({ port: 0, hostname: "127.0.0.1" });
        const client = await Fun.udpSocket({ port: 0, hostname: "127.0.0.1" });
  ...
  describe("detaching an ArrayBuffer during port/address coercion does not use-after-free", () => {
    for (const mode of ["sendMany", "sendMany-stringobj", "send"] as const) {
      test(
        ...
        async () => {
          await using proc = Fun.spawn({
            cmd: [funExe(), path.join(import.meta.dir, "sendMany-payload-uaf-fixture.ts"), mode],
            env: {
              ...funEnv,
  ...
  describe("sendMany does not crash when the connection state changes during iteration", () => {
    for (const direction of ["connect", "disconnect"] as const) {
      test(
        ...
        async () => {
          await using proc = Fun.spawn({
            cmd: [funExe(), path.join(import.meta.dir, "sendMany-reentrancy-fixture.ts"), direction],
            env: funEnv,
            stdout: "pipe",
            stderr: "pipe",
          });
