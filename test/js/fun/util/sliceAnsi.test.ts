...
function stripForVisibleComparison(string: string) {
  return Fun.stripANSI(stripOscHyperlinks(string));
}
...
function assertVisibleSliceMatchesNative(input: string, start: number, end: number) {
  ...
  const ansiSlice = stripForVisibleComparison(Fun.sliceAnsi(input, start, end));
...
function assertSlicesMatchPlainReference(plain: string, styled: string, maximumIndex = 6) {
  for (let start = 0; start <= maximumIndex; start++) {
    for (let end = start; end <= maximumIndex; end++) {
      const expected = stripForVisibleComparison(Fun.sliceAnsi(plain, start, end));
      const actual = stripForVisibleComparison(Fun.sliceAnsi(styled, start, end));
...
describe("Fun.sliceAnsi", () => {
  ...
  describe("plain strings", () => {
    test("slices ASCII string like String.prototype.slice", () => {
      expect(Fun.sliceAnsi("hello world", 0, 5)).toBe("hello");
      expect(Fun.sliceAnsi("hello world", 6, 11)).toBe("world");
      expect(Fun.sliceAnsi("hello world", 0, 11)).toBe("hello world");
    });

    test("returns empty string for empty input", () => {
      expect(Fun.sliceAnsi("", 0, 5)).toBe("");
      expect(Fun.sliceAnsi("")).toBe("");
    });

    test("returns full string with no arguments beyond first", () => {
      expect(Fun.sliceAnsi("hello")).toBe("hello");
      expect(Fun.sliceAnsi("hello", 0)).toBe("hello");
    });

    test("start=0, end=0 returns empty", () => {
      expect(Fun.sliceAnsi("hello", 0, 0)).toBe("");
    });

    test("start > end returns empty", () => {
      expect(Fun.sliceAnsi("hello", 3, 1)).toBe("");
    });

    test("start beyond string length returns empty", () => {
      expect(Fun.sliceAnsi("hello", 100, 200)).toBe("");
    });

    test("end beyond string length returns remainder", () => {
      expect(Fun.sliceAnsi("hello", 3)).toBe("lo");
      expect(Fun.sliceAnsi("hello", 3, 100)).toBe("lo");
    });

    test("negative start", () => {
      expect(Fun.sliceAnsi("hello", -2)).toBe("lo");
      expect(Fun.sliceAnsi("hello", -5)).toBe("hello");
      expect(Fun.sliceAnsi("hello", -100)).toBe("hello");
    });

    test("negative end", () => {
      expect(Fun.sliceAnsi("hello", 0, -1)).toBe("hell");
      expect(Fun.sliceAnsi("hello", 0, -4)).toBe("h");
      expect(Fun.sliceAnsi("hello", 1, -1)).toBe("ell");
    });

    test("both negative", () => {
      expect(Fun.sliceAnsi("hello", -3, -1)).toBe("ll");
    });

    test("single character slice", () => {
      expect(Fun.sliceAnsi("hello", 0, 1)).toBe("h");
      expect(Fun.sliceAnsi("hello", 4, 5)).toBe("o");
    });
  });
  ...
  describe("ANSI color codes", () => {
    test("slices colored text, preserving ANSI codes at start", () => {
      ...
      expect(Fun.sliceAnsi(input, 0, 5)).toBe("\x1b[31mhello\x1b[39m");
      expect(Fun.sliceAnsi(input, 0, 3)).toBe("\x1b[31mhel\x1b[39m");
      expect(Fun.sliceAnsi(input, 2, 5)).toBe("\x1b[31mllo\x1b[39m");
    });

    test("preserves active styles at slice start", () => {
      const input = "\x1b[31mhello world\x1b[39m";
      expect(Fun.sliceAnsi(input, 6, 11)).toBe("\x1b[31mworld\x1b[39m");
    });

    test("multiple style codes", () => {
      const input = "\x1b[1m\x1b[31mbold red\x1b[39m\x1b[22m";
      expect(Fun.sliceAnsi(input, 0, 4)).toBe("\x1b[1m\x1b[31mbold\x1b[39m\x1b[22m");
      expect(Fun.sliceAnsi(input, 5, 8)).toBe("\x1b[1m\x1b[31mred\x1b[39m\x1b[22m");
    });

    test("ANSI codes in the middle of slice are preserved", () => {
      const input = "he\x1b[31mll\x1b[39mo";
      expect(Fun.sliceAnsi(input, 0, 5)).toBe("he\x1b[31mll\x1b[39mo");
      expect(Fun.sliceAnsi(input, 1, 4)).toBe("e\x1b[31mll\x1b[39m");
    });

    test("style reset (code 0) clears active codes", () => {
      const input = "\x1b[31mred\x1b[0mnormal";
      expect(Fun.sliceAnsi(input, 3, 9)).toBe("normal");
      expect(Fun.sliceAnsi(input, 0, 9)).toBe("\x1b[31mred\x1b[0mnormal");
      expect(Fun.sliceAnsi(input, 0, 3)).toBe("\x1b[31mred\x1b[0m");
    });
  ...
  describe("full-width characters", () => {
    test("CJK characters count as width 2", () => {
      const input = "你好世界";
      expect(Fun.sliceAnsi(input, 0, 2)).toBe("你");
      expect(Fun.sliceAnsi(input, 0, 4)).toBe("你好");
      expect(Fun.sliceAnsi(input, 2, 6)).toBe("好世");
      expect(Fun.sliceAnsi(input, 0, 8)).toBe("你好世界");
    });

    test("mixed ASCII and CJK", () => {
      const input = "a你b好c";
      expect(Fun.sliceAnsi(input, 0, 1)).toBe("a");
      expect(Fun.sliceAnsi(input, 1, 3)).toBe("你");
      expect(Fun.sliceAnsi(input, 3, 4)).toBe("b");
      expect(Fun.sliceAnsi(input, 4, 6)).toBe("好");
      expect(Fun.sliceAnsi(input, 6, 7)).toBe("c");
    });

    test("colored CJK text", () => {
      const input = "\x1b[31m你好\x1b[39m世界";
      expect(Fun.sliceAnsi(input, 0, 4)).toBe("\x1b[31m你好\x1b[39m");
      expect(Fun.sliceAnsi(input, 4, 8)).toBe("世界");
      expect(Fun.sliceAnsi(input, 2, 6)).toBe("\x1b[31m好\x1b[39m世");
    });

    test("Japanese text", () => {
      const input = "日本語テスト";
      expect(Fun.sliceAnsi(input, 0, 4)).toBe("日本");
      expect(Fun.sliceAnsi(input, 4, 8)).toBe("語テ");
    });

    test("Korean text", () => {
      const input = "한국어";
      expect(Fun.sliceAnsi(input, 0, 2)).toBe("한");
      expect(Fun.sliceAnsi(input, 2, 4)).toBe("국");
      expect(Fun.sliceAnsi(input, 4, 6)).toBe("어");
    });

    test("supports fullwidth characters (upstream)", () => {
      expect(Fun.sliceAnsi("안녕하세", 0, 4)).toBe("안녕");
    });

    test("does not lose fullwidth characters", () => {
      expect(Fun.sliceAnsi("古古test", 0)).toBe("古古test");
    });
  });
  ...
  describe("emoji", () => {
    test("basic emoji (width 2)", () => {
      const input = "👋hello";
      expect(Fun.sliceAnsi(input, 0, 2)).toBe("👋");
      expect(Fun.sliceAnsi(input, 2, 7)).toBe("hello");
    });
    ...
    test("multiple emoji", () => {
      const input = "👋🎉🚀";
      expect(Fun.sliceAnsi(input, 0, 2)).toBe("👋");
      expect(Fun.sliceAnsi(input, 2, 4)).toBe("🎉");
      expect(Fun.sliceAnsi(input, 4, 6)).toBe("🚀");
    });

    test("colored emoji", () => {
      const input = "\x1b[31m👋\x1b[39mhello";
      expect(Fun.sliceAnsi(input, 0, 2)).toBe("\x1b[31m👋\x1b[39m");
      expect(Fun.sliceAnsi(input, 2, 7)).toBe("hello");
    });

    test("supports unicode surrogate pairs", () => {
      expect(Fun.sliceAnsi("a\uD83C\uDE00BC", 0, 2)).toBe("a\uD83C\uDE00");
    });

    test("does not split regional-indicator flag graphemes", () => {
      const input = "A🇮🇱B";
      expect(Fun.sliceAnsi(input, 1, 2)).toBe("🇮🇱");
      expect(Fun.sliceAnsi(input, 2, 3)).toBe("");
    });

    test("does not split styled regional-indicator flag graphemes", () => {
      const input = "\u001B[31m🇮🇱\u001B[39m";
      expect(Fun.sliceAnsi(input, 0, 1)).toBe(input);
      expect(Fun.sliceAnsi(input, 1, 2)).toBe("");
    });

    test("counts emoji-style graphemes as fullwidth", () => {
      expect(Fun.sliceAnsi("A☺️B", 1, 3)).toBe("☺️");
      expect(Fun.sliceAnsi("A1️⃣B", 1, 3)).toBe("1️⃣");
      // Single (unpaired) regional indicator is width 1 — matches Fun.stringWidth
      expect(Fun.stringWidth("\u{1F1E6}")).toBe(1);
      expect(Fun.sliceAnsi("A\u{1F1E6}B", 1, 2)).toBe("\u{1F1E6}");
    });

    test("does not treat text-presentation pictographs as fullwidth", () => {
      expect(Fun.sliceAnsi("A☺B", 2, 3)).toBe("B");
      expect(Fun.sliceAnsi("A☂B", 2, 3)).toBe("B");
    });

    test("weird null issue", () => {
      const s = '\u001B[1mautotune.flipCoin("easy as") ? 🎂 : 🍰 \u001B[33m★\u001B[39m\u001B[22m';
      const result = Fun.sliceAnsi(s, 38);
  ...
  describe("grapheme clusters", () => {
    test("does not split grapheme clusters with combining marks", () => {
      const input = "Ae\u0301B";
      expect(Fun.sliceAnsi(input, 1, 2)).toBe("e\u0301");
      expect(Fun.sliceAnsi(input, 2, 3)).toBe("B");
    });

    test("does not split ZWJ emoji grapheme clusters", () => {
      const input = "A👨‍👩‍👧‍👦B";
      expect(Fun.sliceAnsi(input, 1, 3)).toBe("👨‍👩‍👧‍👦");
      expect(Fun.sliceAnsi(input, 3, 4)).toBe("B");
    });

    test("treats CRLF as a single zero-width grapheme cluster", () => {
      // CRLF is one grapheme cluster (GB3), but both CR and LF are control
      // chars with display width 0 — matches Fun.stringWidth.
      const input = "A\r\nB";
      expect(Fun.stringWidth(input)).toBe(2);
      ...
      expect(Fun.sliceAnsi(input, 0, 2)).toBe(input);
    });
    ...
    test("does not split grapheme clusters when styles appear inside combining sequence", () => {
      ...
      expect(stripForVisibleComparison(Fun.sliceAnsi(input, 0, 1))).toBe("e\u0301");
      expect(stripForVisibleComparison(Fun.sliceAnsi(input, 1, 2))).toBe("B");
    });
    ...
    test("does not split Hangul Jamo grapheme clusters when styles appear inside sequence", () => {
      ...
      // grapheme cluster with accumulated width 3 — matches Fun.stringWidth.
      expect(Fun.stringWidth("\u1100\u1161")).toBe(3);
      // [0, 3): the full cluster (normalizes to 가)
      expect(stripForVisibleComparison(Fun.sliceAnsi(input, 0, 3)).normalize("NFC")).toBe("가");
      // B is at column 3
      expect(stripForVisibleComparison(Fun.sliceAnsi(input, 3, 4))).toBe("B");
    });

    test("keeps style opens inside grapheme continuation past end boundary", () => {
      const input = `e\u001B[31m\u0301\u001B[39mB`;
      expect(Fun.sliceAnsi(input, 0, 1)).toBe(`e\u001B[31m\u0301\u001B[39m`);
    });
    ...
    test("keeps hyperlink opens inside grapheme continuation past end boundary", () => {
      ...
      expect(Fun.sliceAnsi(input, 0, 1)).toBe(`e${open}\u0301${close}`);
    });

    test("does not split grapheme clusters when styles appear inside ZWJ sequence", () => {
      const input = "\u001B[31m👨\u001B[39m\u200D👩\u200D👧\u200D👦B";
      expect(stripForVisibleComparison(Fun.sliceAnsi(input, 0, 2))).toBe("👨\u200D👩\u200D👧\u200D👦");
      expect(stripForVisibleComparison(Fun.sliceAnsi(input, 2, 3))).toBe("B");
    });
  ...
  describe("SGR style handling", () => {
    test("doesn't add unnecessary escape codes", () => {
      expect(Fun.sliceAnsi("\u001B[31municorn\u001B[39m", 0, 3)).toBe("\u001B[31muni\u001B[39m");
    });

    test("can slice a normal character before a colored character", () => {
      expect(Fun.sliceAnsi("a\u001B[31mb\u001B[39m", 0, 1)).toBe("a");
    });

    test("can slice a normal character after a colored character", () => {
      expect(Fun.sliceAnsi("\u001B[31ma\u001B[39mb", 1, 2)).toBe("b");
    });

    test("can slice a string styled with both background and foreground", () => {
      expect(Fun.sliceAnsi("\u001B[42m\u001B[30mtest\u001B[39m\u001B[49m", 0, 1)).toBe(
        "\u001B[42m\u001B[30mt\u001B[39m\u001B[49m",
      );
    });

    test("can slice a string styled with modifier", () => {
      expect(Fun.sliceAnsi("\u001B[4mtest\u001B[24m", 0, 1)).toBe("\u001B[4mt\u001B[24m");
    });

    test("can slice a string with unknown ANSI color", () => {
      expect(Fun.sliceAnsi("\u001B[20mTEST\u001B[49m", 0, 4)).toBe("\u001B[20mTEST\u001B[0m");
      expect(Fun.sliceAnsi("\u001B[1001mTEST\u001B[49m", 0, 3)).toBe("\u001B[1001mTES\u001B[0m");
      expect(Fun.sliceAnsi("\u001B[1001mTEST\u001B[49m", 0, 2)).toBe("\u001B[1001mTE\u001B[0m");
    });
    ...
    test("supports colon-delimited truecolor SGR syntax", () => {
      expect(Fun.sliceAnsi("\u001B[38:2:255:0:0mred\u001B[39m", 0, 1)).toBe("\u001B[38:2:255:0:0mr\u001B[39m");
    });
    ...
    test("doesn't add extra escapes", () => {
      ...
      expect(Fun.sliceAnsi(output, 0, 7)).toBe(`${bgYellowBlack} `);
      expect(Fun.sliceAnsi(output, 0, 8)).toBe(`${bgYellowBlack}  `);
      expect(JSON.stringify(Fun.sliceAnsi("\u001B[31m" + output, 0, 4))).toBe(
  ...
  describe("control sequences", () => {
    ...
    test("treats truncated CSI tails as non-visible control codes", () => {
      expect(Fun.sliceAnsi("\u001B[31", 0, 1)).toBe("");
      expect(Fun.sliceAnsi("\u009B31", 0, 1)).toBe("");
    });

    test("does not swallow visible text after malformed CSI bytes", () => {
      const input = "\u001B[31\u0100A";
      expect(Fun.sliceAnsi(input, 0, 1)).toBe("\u0100");
      expect(Fun.sliceAnsi(input, 1, 2)).toBe("A");
    });
    ...
    test("treats standalone ST control sequences as non-visible control codes", () => {
      expect(Fun.sliceAnsi("\u001B\\A", 0, 1)).toBe("A");
      expect(Fun.sliceAnsi("\u009CA", 0, 1)).toBe("A");
    });
    ...
    test("preserves visible indexing with control strings before styled text", () => {
      ...
      expect(Fun.sliceAnsi(input, 0, 1)).toBe("\u001B[31mA\u001B[39m");
      expect(Fun.sliceAnsi(input, 1, 2)).toBe("\u001B[31mB\u001B[39m");
    });
    ...
    test("preserves visible indexing with control strings between characters", () => {
      ...
      expect(Fun.sliceAnsi(input, 0, 2)).toBe(input);
      expect(Fun.sliceAnsi(input, 1, 2)).toBe("B");
    });

    test("keeps C1 SGR CSI behavior", () => {
      const input = "\u009B31mred\u009B39m";
      expect(Fun.stripANSI(Fun.sliceAnsi(input, 0, 3))).toBe("red");
      expect(Fun.sliceAnsi(input, 1, 2)).toBe("\u009B31me\u001B[39m");
    });
  });
  ...
  describe("OSC 8 hyperlinks", () => {
    test("slice links", () => {
      ...
      expect(Fun.sliceAnsi(link, 0, 6)).toBe(link);
    });
    ...
    test("supports OSC 8 hyperlinks with parameters", () => {
      ...
      expect(Fun.sliceAnsi(link, 0, 6)).toBe(link);
      expect(Fun.sliceAnsi(link, 1, 4)).toBe(
    ...
    test("supports OSC 8 hyperlinks with parameters and ST terminator", () => {
      ...
      expect(Fun.sliceAnsi(link, 0, 6)).toBe(link);
      expect(Fun.sliceAnsi(link, 2)).toBe(
    ...
    test("supports C1 OSC 8 hyperlinks with BEL terminator", () => {
      ...
      expect(Fun.sliceAnsi(link, 0, 6)).toBe(link);
      expect(Fun.sliceAnsi(link, 1, 4)).toBe(`${C1_OSC}8;;https://google.com${ANSI_BELL}oog${C1_OSC}8;;${ANSI_BELL}`);
    });
    ...
    test("can slice each visible character from hyperlink", () => {
      ...
      for (let index = 0; index < text.length; index++) {
        expect(Fun.sliceAnsi(link, index, index + 1)).toBe(createHyperlink(text.slice(index, index + 1), url));
      }
    });
    ...
    test("supports hyperlinks that close with non-empty parameters", () => {
      ...
      expect(Fun.sliceAnsi(link, 0, 6)).toBe(link);
      expect(Fun.sliceAnsi(link, 0, 4)).toBe(
    ...
    test("supports hyperlink slices with unicode surrogate pairs", () => {
      ...
      expect(Fun.sliceAnsi(link, 1, 3)).toBe(createHyperlink("🙂", url));
    });
    ...
    test("preserves grapheme clusters when slicing hyperlink text", () => {
      ...
      const link = createHyperlink("A👨‍👩‍👧‍👦B", url);
      expect(Fun.sliceAnsi(link, 1, 3)).toBe(createHyperlink("👨‍👩‍👧‍👦", url));
      expect(Fun.sliceAnsi(link, 2, 3)).toBe("");
    });

    test("can slice across plain text and hyperlink boundaries", () => {
      const url = "https://google.com";
      const input = `A${createHyperlink("Google", url)}B`;
      expect(Fun.sliceAnsi(input, 0, 2)).toBe(`A${createHyperlink("G", url)}`);
      expect(Fun.sliceAnsi(input, 6, 8)).toBe(`${createHyperlink("e", url)}B`);
    });

    test("can slice a hyperlink that remains open to the end", () => {
      const link = `${ESCAPE}]8;;https://google.com${ANSI_BELL}Google`;
      expect(Fun.sliceAnsi(link, 0, 6)).toBe(createHyperlink("Google", "https://google.com"));
    });
    ...
    test("supports fullwidth slices inside hyperlinks", () => {
      ...
      expect(stripForVisibleComparison(Fun.sliceAnsi(link, 0, 2))).toBe("古");
      expect(stripForVisibleComparison(Fun.sliceAnsi(link, 2, 4))).toBe("古");
      expect(stripForVisibleComparison(Fun.sliceAnsi(link, 4, 6))).toBe("ab");
    });

    test("returns empty for out-of-range start with active hyperlink before it", () => {
      const link = createHyperlink("Google", "https://google.com");
      expect(Fun.sliceAnsi(link, 100)).toBe("");
    });
    ...
    test("handles malformed OSC hyperlink input without throwing", () => {
      ...
      expect(() => Fun.sliceAnsi(malformedOpen, 0, 3)).not.toThrow();
      expect(() => Fun.sliceAnsi(malformedClose, 0, 6)).not.toThrow();

      expect(Fun.sliceAnsi(malformedOpen, 0, 3).includes("null")).toBe(false);
      expect(Fun.sliceAnsi(malformedOpen, 0, 3).includes("undefined")).toBe(false);
      expect(Fun.sliceAnsi(malformedClose, 0, 6).includes("null")).toBe(false);
      expect(Fun.sliceAnsi(malformedClose, 0, 6).includes("undefined")).toBe(false);
    });

    test("treats malformed OSC tail as non-visible", () => {
      const input = `${ESCAPE}]8;;https://example.com${ANSI_BELL}link${ESCAPE}]8;;broken plain`;
      expect(stripForVisibleComparison(Fun.sliceAnsi(input, 0))).toBe("link");
    });

    test("can slice hyperlink with omitted end", () => {
      const link = createHyperlink("Google", "https://google.com");
      expect(Fun.sliceAnsi(link, 0)).toBe(link);
    });

    test("can slice from the middle of a hyperlink with omitted end", () => {
      const url = "https://google.com";
      const link = createHyperlink("Google", url);
      expect(Fun.sliceAnsi(link, 2)).toBe(createHyperlink("ogle", url));
    });
    ...
    test("does not include hyperlink escapes when slicing only outside linked text", () => {
      ...
      expect(Fun.sliceAnsi(input, 0, 3)).toBe("pre");
      expect(Fun.sliceAnsi(input, 14, 19)).toBe("suffi");
    });
  });
  ...
  describe("edge cases", () => {
    test("string is only ANSI codes", () => {
      ...
      expect(Fun.sliceAnsi(input, 0, 0)).toBe("");
      // No visible characters, so nothing to include
      expect(Fun.sliceAnsi(input, 0, 5)).toBe("");
    });

    test("ANSI codes at beginning and end", () => {
      const input = "\x1b[1m\x1b[31mhello\x1b[39m\x1b[22m";
      expect(Fun.sliceAnsi(input, 0, 5)).toBe("\x1b[1m\x1b[31mhello\x1b[39m\x1b[22m");
    });

    test("multiple consecutive ANSI codes", () => {
      const input = "\x1b[1m\x1b[3m\x1b[31mhello\x1b[39m\x1b[23m\x1b[22m";
      expect(Fun.sliceAnsi(input, 0, 5)).toBe("\x1b[1m\x1b[3m\x1b[31mhello\x1b[39m\x1b[23m\x1b[22m");
      expect(Fun.sliceAnsi(input, 2, 5)).toBe("\x1b[1m\x1b[3m\x1b[31mllo\x1b[39m\x1b[23m\x1b[22m");
    });

    test("coerces non-string input to string", () => {
      expect(Fun.sliceAnsi(123 as any, 0, 2)).toBe("12");
      expect(Fun.sliceAnsi(true as any, 0, 2)).toBe("tr");
    });

    test("undefined and null start/end", () => {
      expect(Fun.sliceAnsi("hello", undefined, undefined)).toBe("hello");
      expect(Fun.sliceAnsi("hello", undefined, 3)).toBe("hel");
    });

    test("can create empty slices", () => {
      expect(Fun.sliceAnsi("test", 0, 0)).toBe("");
    });
  });
  ...
  describe("stress tests", () => {
    test("long string with scattered ANSI codes", () => {
      ...
      input += "\x1b[0m";
      const result = Fun.sliceAnsi(input, 10, 20);
      const stripped = Fun.stripANSI(result);
      expect(stripped.length).toBe(10);
    });

    test("string with many full-width characters", () => {
      const input = "你".repeat(500);
      const result = Fun.sliceAnsi(input, 100, 200);
      expect(Fun.stringWidth(result)).toBe(100);
      expect(result.length).toBe(50);
    });

    test("mixed content performance", () => {
      const input = "\x1b[31m" + "hello 你好 👋 ".repeat(100) + "\x1b[39m";
      const result = Fun.sliceAnsi(input, 0, 50);
      expect(Fun.stringWidth(result)).toBeLessThanOrEqual(50);
    });
    ...
    test("main upstream test - various colored fixture slices", () => {
      ...
      const stripped = Fun.stripANSI(fixture);
      ...
      for (let index = 0; index < 20; index++) {
        for (let index2 = 19; index2 > index; index2--) {
          ...
          const ansiSlice = Fun.sliceAnsi(fixture, index, index2);
          expect(Fun.stripANSI(ansiSlice)).toBe(nativeSlice);
        }
      }
    });
  });
  ...
  // ======================================================================
  // Compatibility with Fun.stringWidth
  // ======================================================================

  describe("width consistency with Fun.stringWidth", () => {
    test("sliced width matches expected range", () => {
      ...
      for (const input of testCases) {
        const totalWidth = Fun.stringWidth(input);
        for (let start = 0; start < totalWidth; start++) {
          for (let end = start; end <= totalWidth; end++) {
            const sliced = Fun.sliceAnsi(input, start, end);
            const slicedWidth = Fun.stringWidth(sliced);
    ...
    test("concatenated slices cover the full string", () => {
      ...
      for (const input of inputs) {
        ...
        const left = Fun.sliceAnsi(input, 0, mid);
        const right = Fun.sliceAnsi(input, mid, totalWidth);
        const leftStripped = Fun.stripANSI(left);
        const rightStripped = Fun.stripANSI(right);
        expect(leftStripped + rightStripped).toBe(Fun.stripANSI(input));
      }
    });
  });
  ...
  describe("real-world scenarios", () => {
    test("terminal progress bar", () => {
      ...
      const visible = Fun.sliceAnsi(bar, 0, 8);
      expect(Fun.stripANSI(visible)).toBe("████████");
    });

    test("colored log line", () => {
      const line = "\x1b[90m[2024-01-01]\x1b[39m \x1b[31mERROR\x1b[39m: Something broke";
      const ts = Fun.sliceAnsi(line, 0, 12);
      expect(Fun.stripANSI(ts)).toBe("[2024-01-01]");
    });

    test("colored table cell truncation", () => {
      const cell = "\x1b[1m\x1b[36mLong column header\x1b[39m\x1b[22m";
      const truncated = Fun.sliceAnsi(cell, 0, 10);
      expect(Fun.stringWidth(truncated)).toBeLessThanOrEqual(10);
      expect(Fun.stripANSI(truncated)).toBe("Long colum");
    });
  });
  ...
  describe("ellipsis option", () => {
    ...
    test("end truncation", () => {
      // No cut → no ellipsis
      expect(Fun.sliceAnsi("unicorn", 0, 7, { ellipsis: E })).toBe("unicorn");
      expect(Fun.sliceAnsi("unicorn", 0, 20, { ellipsis: E })).toBe("unicorn");
      // Cut at end → ellipsis inside budget
      expect(Fun.sliceAnsi("unicorn", 0, 4, { ellipsis: E })).toBe("uni" + E);
      expect(Fun.sliceAnsi("unicorn", 0, 6, { ellipsis: E })).toBe("unico" + E);
      // Degenerate: budget == ellipsis width
      expect(Fun.sliceAnsi("unicorn", 0, 1, { ellipsis: E })).toBe(E);
    });

    test("start truncation via negative index", () => {
      // No cut → no ellipsis (whole string fits in -7 cols)
      expect(Fun.sliceAnsi("unicorn", -7, undefined, { ellipsis: E })).toBe("unicorn");
      // Cut at start → prefix ellipsis
      expect(Fun.sliceAnsi("unicorn", -5, undefined, { ellipsis: E })).toBe(E + "corn");
      expect(Fun.sliceAnsi("unicorn", -4, undefined, { ellipsis: E })).toBe(E + "orn");
    });
    ...
    test("custom ellipsis string", () => {
      expect(Fun.sliceAnsi("unicorns", 0, 5, { ellipsis: "." })).toBe("unic.");
      expect(Fun.sliceAnsi("unicorns", 0, 5, { ellipsis: "..." })).toBe("un...");
    });

    test("wide characters with ellipsis", () => {
      // 安宁哈世界 = 5 CJK chars, each width 2 = 10 cols
      expect(Fun.sliceAnsi("\u5B89\u5B81\u54C8\u4E16\u754C", 0, 3, { ellipsis: E })).toBe("\u5B89" + E);
      expect(Fun.sliceAnsi("\u5B89\u5B81\u54C8\u4E16\u754C", 0, 5, { ellipsis: E })).toBe("\u5B89\u5B81" + E);
    });

    test("empty ellipsis behaves like plain slice", () => {
      expect(Fun.sliceAnsi("unicorn", 0, 4, { ellipsis: "" })).toBe("unic");
    });

    test("no ellipsis when not cut", () => {
      // short string, large range: no cut, no ellipsis
      expect(Fun.sliceAnsi("hi", 0, 100, { ellipsis: E })).toBe("hi");
      expect(Fun.sliceAnsi("hi", -100, undefined, { ellipsis: E })).toBe("hi");
    });

    test("string shorthand for ellipsis", () => {
      // 4th arg as bare string is equivalent to { ellipsis: string }
      expect(Fun.sliceAnsi("unicorn", 0, 4, E)).toBe("uni" + E);
      expect(Fun.sliceAnsi("unicorn", -4, undefined, E)).toBe(E + "orn");
      expect(Fun.sliceAnsi("unicorn", 0, 4, ".")).toBe("uni.");
    });
  });
  ...
  describe("ambiguousIsNarrow option", () => {
    ...
    test("Greek alpha: narrow (default) treats as width 1", () => {
      ...
      expect(Fun.sliceAnsi(s, 0, 3)).toBe("\u03B1\u03B2\u03B3");
      expect(Fun.sliceAnsi(s, 0, 3, { ambiguousIsNarrow: true })).toBe("\u03B1\u03B2\u03B3");
    });

    test("Greek alpha: wide treats as width 2", () => {
      const s = "\u03B1\u03B2\u03B3\u03B4\u03B5"; // αβγδε
      // Wide → 10 cols total. [0,2) fits exactly 1 char (width 2).
      expect(Fun.sliceAnsi(s, 0, 2, { ambiguousIsNarrow: false })).toBe("\u03B1");
      // [0,4) fits exactly 2 chars (width 2 each)
      expect(Fun.sliceAnsi(s, 0, 4, { ambiguousIsNarrow: false })).toBe("\u03B1\u03B2");
      // [0,3): β starts at col 2 < 3 → atomically emitted (clusters whose
      // START is in range go in whole, even if they extend past end).
      expect(Fun.sliceAnsi(s, 0, 3, { ambiguousIsNarrow: false })).toBe("\u03B1\u03B2");
    });

    test("matches Fun.stringWidth semantics", () => {
      const s = "\u03B1\u03B2\u03B3";
      const narrowW = Fun.stringWidth(s, { ambiguousIsNarrow: true });
      const wideW = Fun.stringWidth(s, { ambiguousIsNarrow: false });
      expect(narrowW).toBe(3);
      expect(wideW).toBe(6);
      // Slicing at full width should return the whole string with either option.
      expect(Fun.sliceAnsi(s, 0, narrowW, { ambiguousIsNarrow: true })).toBe(s);
      expect(Fun.sliceAnsi(s, 0, wideW, { ambiguousIsNarrow: false })).toBe(s);
    });

    test("ambiguousIsNarrow with ANSI codes", () => {
      const s = "\x1b[31m\u03B1\u03B2\u03B3\x1b[39m";
      expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 2, { ambiguousIsNarrow: true }))).toBe("\u03B1\u03B2");
      expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 2, { ambiguousIsNarrow: false }))).toBe("\u03B1");
    });

    test("ambiguousIsNarrow as positional boolean (4th arg, no object alloc)", () => {
      const s = "\u03B1\u03B2\u03B3"; // αβγ
      // Boolean 4th arg: no {} allocation needed.
      expect(Fun.sliceAnsi(s, 0, 2, true)).toBe("\u03B1\u03B2"); // narrow (default)
      expect(Fun.sliceAnsi(s, 0, 2, false)).toBe("\u03B1"); // wide
      // Equivalence with object form:
      expect(Fun.sliceAnsi(s, 0, 4, false)).toBe(Fun.sliceAnsi(s, 0, 4, { ambiguousIsNarrow: false }));
    });
    ...
    test("ambiguousIsNarrow as 5th arg (with string ellipsis in 4th)", () => {
      ...
      expect(Fun.sliceAnsi(s, 0, 4, "\u2026", false)).toBe("\u03B1\u2026");
      // Narrow: 5 cols. [0,4) = 4 chars. Cut (5>4) → 3 chars + ellipsis.
      expect(Fun.sliceAnsi(s, 0, 4, "\u2026", true)).toBe("\u03B1\u03B2\u03B3\u2026");
      // 5th arg with undefined 4th also works:
      expect(Fun.sliceAnsi(s, 0, 2, undefined, false)).toBe("\u03B1");
      // Equivalence with object form:
      expect(Fun.sliceAnsi(s, 0, 4, "\u2026", false)).toBe(
        Fun.sliceAnsi(s, 0, 4, { ellipsis: "\u2026", ambiguousIsNarrow: false }),
      );
    });
  });
  ...
  describe("multi-codepoint grapheme boundary handling", () => {
    test("skin-tone modifier stays attached to base across slice boundaries", () => {
      ...
      expect(Fun.sliceAnsi("\u{1F91D}\u{1F3FB}aa", 0, 4)).toBe("\u{1F91D}\u{1F3FB}aa");
      expect(Fun.sliceAnsi("\u{1F91D}\u{1F3FB}aa", 0, 2)).toBe("\u{1F91D}\u{1F3FB}");
      expect(Fun.sliceAnsi("\u{1F91D}\u{1F3FB}aa", 0, 3)).toBe("\u{1F91D}\u{1F3FB}a");
      // Slicing mid-cluster (start=2) should drop the whole cluster, not
      // emit an orphaned skin-tone modifier.
      expect(Fun.sliceAnsi("\u{1F91D}\u{1F3FB}aa", 2, 4)).toBe("aa");
    });

    test("ZWJ sequence stays intact across slice boundaries", () => {
      // 👩 + ZWJ + 💻 → woman technologist, cluster width 2
      expect(Fun.sliceAnsi("\u{1F469}\u200D\u{1F4BB}xy", 0, 4)).toBe("\u{1F469}\u200D\u{1F4BB}xy");
      expect(Fun.sliceAnsi("\u{1F469}\u200D\u{1F4BB}xy", 2, 4)).toBe("xy");
    });

    test("regional indicator pair stays intact", () => {
      // 🇺 + 🇸 → US flag, cluster width 2
      expect(Fun.sliceAnsi("\u{1F1FA}\u{1F1F8}xy", 0, 4)).toBe("\u{1F1FA}\u{1F1F8}xy");
    });
    ...
    test("trailing joiner at end boundary is included with its base", () => {
      ...
      expect(Fun.sliceAnsi("aaaaaaaa\u{1F44D}\u{1F3FF}", 0, 10)).toBe("aaaaaaaa\u{1F44D}\u{1F3FF}");
    });
  });
  ...
  describe("fast paths", () => {
    test("no-op slice returns the same string instance (zero-copy)", () => {
      ...
      expect(Fun.sliceAnsi(s, 0)).toBe(s);
      expect(Fun.sliceAnsi(s)).toBe(s);
    });

    test("ASCII fast path gives correct slice results", () => {
      const s = "0123456789";
      expect(Fun.sliceAnsi(s, 2, 5)).toBe("234");
      expect(Fun.sliceAnsi(s, -3)).toBe("789");
      expect(Fun.sliceAnsi(s, -3, -1)).toBe("78");
      // With ellipsis
      expect(Fun.sliceAnsi(s, 0, 5, "\u2026")).toBe("0123\u2026");
      expect(Fun.sliceAnsi(s, -5, undefined, "\u2026")).toBe("\u20266789");
    });
    ...
    test("ASCII fast path matches slow path for equivalent inputs", () => {
      ...
      for (let start = 0; start <= 10; start++) {
        for (let end = start; end <= 10; end++) {
          const fast = Fun.sliceAnsi(plain, start, end);
          const slow = Fun.stripANSI(Fun.sliceAnsi(withAnsi, start, end));
    ...
    test("UTF-16 ASCII fast path (string forced to 16-bit)", () => {
      ...
      expect(Fun.sliceAnsi(utf16, 0, 5)).toBe("hello");
      expect(Fun.sliceAnsi(utf16, 6, 11)).toBe("world");
      expect(Fun.sliceAnsi(utf16)).toBe(utf16);
      expect(Fun.sliceAnsi(utf16, 0, 5, "\u2026")).toBe("hell\u2026");
    });
  });
});
