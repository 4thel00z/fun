import { zstdCompress, zstdCompressSync, zstdDecompress, zstdDecompressSync } from "fun";
...
describe("Zstandard compression", async () => {
  ...
  const testCases = [
    ...
    { name: "medium", data: await Fun.file(path.join(__dirname, "..", "..", "..", "fun.lock")).bytes() },
    {
      name: "large",
      data: Buffer.from(
        (await Fun.file(path.join(__dirname, "..", "..", "..", "..", "src", "js_parser", "parser.zig")).text()).repeat(
          5,
        ),
      ),
    },
          ...
          JSON.stringify(
            {
              ...
              "name": "fun",
              "version": "1.2.14",
              "workspaces": ["./packages/fun-types", "./packages/@types/fun"],
              "devDependencies": {
                "@types/react": "^18.3.3",
                "esbuild": "^0.21.4",
                "mitata": "^0.1.11",
              ...
              "resolutions": {
                "fun-types": "workspace:packages/fun-types",
                "@types/fun": "workspace:packages/@types/fun",
              },
              "scripts": {
                "build": "fun run build:debug",
                "watch":
                ...
                "agent":
                  "(fun run --silent build:debug &> /tmp/fun.debug.build.log || (cat /tmp/fun.debug.build.log && rm -rf /tmp/fun.debug.build.log && exit 1)) && rm -f /tmp/fun.debug.build.log && ./build/debug/fun-debug",
                "build:debug": "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Debug -B build/debug",
                "build:debug:asan":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Debug -DENABLE_ASAN=ON -B build/debug-asan",
                "build:valgrind":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Debug -DENABLE_BASELINE=ON -ENABLE_VALGRIND=ON -B build/debug-valgrind",
                "build:release": "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -B build/release",
                "build:ci":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -DCMAKE_VERBOSE_MAKEFILE=ON -DCI=true -B build/release-ci --verbose --fresh",
                "build:assert":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=RelWithDebInfo -DENABLE_ASSERTIONS=ON -DENABLE_LOGS=ON -B build/release-assert",
                "build:asan":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -DENABLE_ASSERTIONS=ON -DENABLE_LOGS=OFF -DENABLE_ASAN=ON -DENABLE_LTO=OFF -B build/release-asan",
                "build:logs":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -DENABLE_LOGS=ON -B build/release-logs",
                "build:safe":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -DZIG_OPTIMIZE=ReleaseSafe -B build/release-safe",
                "build:smol": "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=MinSizeRel -B build/release-smol",
                "build:local":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Debug -DWEBKIT_LOCAL=ON -B build/debug-local",
                "build:release:local":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -DWEBKIT_LOCAL=ON -B build/release-local",
                ...
                "run:linux":
                  'docker run --rm  -v "$PWD:/root/fun/" -w /root/fun ghcr.io/underdoc-org/fun-development-docker-image',
                "css-properties": "fun run src/css/properties/generate_properties.ts",
                "uv-posix-stubs": "fun run src/fun.js/bindings/libuv/generate_uv_posix_stubs.ts",
                "bump": "fun ./scripts/bump.ts",
                "typecheck": "tsc --noEmit && cd test && fun run typecheck",
                "fmt": "fun run prettier",
                "fmt:cpp": "fun run clang-format",
                "fmt:zig": "fun run zig-format",
                "lint": "funx oxlint --config=oxlint.json --format=github src/js",
                "lint:fix": "oxlint --config oxlint.json --fix",
                "test": "node scripts/runner.node.mjs --exec-path ./build/debug/fun-debug",
                "test:release": "node scripts/runner.node.mjs --exec-path ./build/release/fun",
                "banned": "fun test test/internal/ban-words.test.ts",
                "glob-sources": "fun scripts/glob-sources.mjs",
                "zig": "vendor/zig/zig.exe",
                "zig:test": "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Debug -DFUN_TEST=ON -B build/debug",
                "zig:test:release":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -DFUNTEST=ON -B build/release",
                "zig:test:ci":
                  "fun ./scripts/build.mjs -GNinja -DCMAKE_BUILD_TYPE=Release -DFUN_TEST=ON -DZIG_OPTIMIZE=ReleaseSafe -DCMAKE_VERBOSE_MAKEFILE=ON -DCI=true -B build/release-ci --verbose --fresh",
                "zig:fmt": "fun run zig-format",
                "zig:check": "fun run zig build check --summary new",
                "zig:check-all": "fun run zig build check-all --summary new",
                "zig:check-windows": "fun run zig build check-windows --summary new",
                "analysis":
                  "fun ./scripts/build.mjs -DCMAKE_BUILD_TYPE=Debug -DENABLE_ANALYSIS=ON -DENABLE_CCACHE=OFF -B build/analysis",
                "analysis:no-llvm": "fun run analysis -DENABLE_LLVM=OFF",
                "clang-format": "fun run analysis --target clang-format",
                "clang-format:check": "fun run analysis --target clang-format-check",
                "clang-format:diff": "fun run analysis --target clang-format-diff",
                "clang-tidy": "fun run analysis --target clang-tidy",
                "clang-tidy:check": "fun run analysis --target clang-tidy-check",
                "clang-tidy:diff": "fun run analysis --target clang-tidy-diff",
                "zig-format": "fun run analysis:no-llvm --target zig-format",
                "zig-format:check": "fun run analysis:no-llvm --target zig-format-check",
                "prettier":
                  "funx prettier@latest --plugin=prettier-plugin-organize-imports --config .prettierrc --write scripts packages src docs 'test/**/*.{test,spec}.{ts,tsx,js,jsx,mts,mjs,cjs,cts}' '!test/**/*fixture*.*'",
