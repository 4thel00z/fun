...
describe("Fun.stripANSI", () => {
  ...
  test("returns new string when ANSI sequences are removed", () => {
    ...
    const result = Fun.stripANSI(input);
  ...
  for (const testCase of testCases) {
    ...
    test(JSON.stringify(input), () => {
      const received = Fun.stripANSI(input);
      expect(Fun.stripANSI(input), `${JSON.stringify(expected)} != ${JSON.stringify(received)}`).toBe(expected);
    });
  }
  ...
  test("long strings", () => {
    ...
    expect(Fun.stripANSI(withAnsi)).toBe(stripAnsi(withAnsi));
  });
  ...
  test("multiple sequences in long string", () => {
    ...
    expect(Fun.stripANSI(input)).toBe(stripAnsi(input));
  });

  test("non-string input", () => {
    expect(Fun.stripANSI(123 as any)).toBe("123");
    expect(Fun.stripANSI(true as any)).toBe("true");
    expect(Fun.stripANSI(false as any)).toBe("false");
    expect(Fun.stripANSI(null as any)).toBe("null");
    expect(Fun.stripANSI(undefined as any)).toBe("undefined");
  });
});
