import { createSocketPair, fileSinkInternals } from "fun:internal-for-testing";
...
describe("FileSink", () => {
  ...
  function getPath(label: string) {
    const path = join(tmpdirSync(), `${Fun.hash(label).toString(10)}.txt`);
    try {
  ...
  function getFd(label: string, byteLength = 0) {
    ...
    })(Fun.file(path).stream(64), byteLength);
    return path;
  }

  for (let isPipe of [true, false] as const) {
    // TODO: fix the `mkfifo` function for windows. They do have an API but calling it from fun:ffi didn't get great results.
    // once #8166 is merged, this can be written using it's 'fun:iternals-for-testing' feature
      ...
      fixtures.forEach(([input, expected, label]) => {
        ...
        it(`${JSON.stringify(label)}`, async () => {
          const path = getPathOrFd();
          {
            ...
            const sink = Fun.file(path).writer();
          ...
          if (!isPipe) {
            const output = new Uint8Array(await Fun.file(path).arrayBuffer());
        ...
        it(`highWaterMark -> ${JSON.stringify(label)}`, async () => {
          ...
          {
            using _ = fileDescriptorLeakChecker();
            const sink = Fun.file(path).writer({ highWaterMark: 1 });
            for (let i = 0; i < input.length; i++) {
              sink.write(input[i]);
              await sink.flush();
            }
            await sink.end();
            await Fun.sleep(10); // For the file descriptor leak checker.
          }
...
it("end does close when not backed by a file descriptor", async () => {
  using _ = fileDescriptorLeakChecker();
  const x = tmpdirSync();
  const file = Fun.file(path.join(x, "test.txt"));
  const writer = file.writer();
...
if (isWindows) {
  it("ENOENT, Windows", () => {
    expect(() => Fun.file("A:\\this-does-not-exist.txt").writer()).toThrow(
...
it.skipIf(!isPosix)("does not leak native FileSink when a pending write fails (EPIPE)", async () => {
  async function once() {
    ...
    const sink = Fun.file(writeFd).writer();
