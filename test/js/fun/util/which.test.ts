...

{
  ...
  if (`${delim}${process.env.PATH}${delim}`.includes(`${delim}.${delim}`)) {
    throw new Error("$PATH includes . which will break `Fun.which` tests. This is an environment configuration issue.");
  }
}
...
if (isWindows) {
  ...
  test("which", () => {
    ...
    try {
      ...
      // "fun" is in our PATH
      expect(which("fun")!.length > 0).toBe(true);
...
test("Fun.which does not look in the current directory for bins", async () => {
...
test("Fun.which does look in the current directory when given a path with a slash", async () => {
  const cwd = process.cwd();
  const dir = tempDirWithFiles("which", {
    "some_program_name": "#!/usr/bin/env sh\necho posix\nexit 0\n",
...
test("Fun.which can find executables in a non-ascii directory", async () => {
  const cwd = process.cwd();
