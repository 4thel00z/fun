...
it("arrayBufferToString u8", async () => {
  ...
  gc(true);
  expect(Fun.unsafe.arrayBufferToString(bytes)).toBe("hello world");
  gc(true);
...
it("arrayBufferToString ArrayBuffer", async () => {
  ...
  gc(true);
  const out = Fun.unsafe.arrayBufferToString(bytes.buffer);
...
it("Fun.allocUnsafe", () => {
  var buffer = Fun.allocUnsafe(1024);
