...
describe("randomUUIDv5", () => {
  ...
  test("basic functionality", () => {
    const result = Fun.randomUUIDv5("www.example.com", dnsNamespace);
  ...
  test("deterministic output", () => {
    const uuid1 = Fun.randomUUIDv5("www.example.com", dnsNamespace);
    const uuid2 = Fun.randomUUIDv5("www.example.com", dnsNamespace);
  ...
  test("compatibility with uuid library", () => {
    ...
    const funUuid = Fun.randomUUIDv5(name, dnsNamespace);
    const uuidLibUuid = uuid.v5(name, dnsNamespace);

    expect(funUuid).toBe(uuidLibUuid);
  });

  test("predefined namespace strings", () => {
    // Test with predefined namespace strings
    const uuid1 = Fun.randomUUIDv5("www.example.com", "dns");
    const uuid2 = Fun.randomUUIDv5("www.example.com", dnsNamespace);

    expect(uuid1).toBe(uuid2);

    const uuid3 = Fun.randomUUIDv5("http://example.com", "url");
    const uuid4 = Fun.randomUUIDv5("http://example.com", urlNamespace);

    expect(uuid3).toBe(uuid4);
  });

  test("empty name", () => {
    const result = Fun.randomUUIDv5("", dnsNamespace);
    expect(result).toBeTypeOf("string");
    expect(result[14]).toBe("5");
  });
  ...
  test("long name", () => {
    ...
    const result = Fun.randomUUIDv5(longName, dnsNamespace);
    expect(result).toBeTypeOf("string");
    expect(result[14]).toBe("5");
  });
  ...
  test("unicode name", () => {
    ...
    const result = Fun.randomUUIDv5(unicodeName, dnsNamespace);
    expect(result).toBeTypeOf("string");
    expect(result[14]).toBe("5");

    // Should be deterministic
    const uuid2 = Fun.randomUUIDv5(unicodeName, dnsNamespace);
  ...
  test("name as ArrayBuffer", () => {
    ...
    const uuid1 = Fun.randomUUIDv5(nameString, dnsNamespace);
    const uuid2 = Fun.randomUUIDv5(nameBuffer, dnsNamespace);
  ...
  test("name as TypedArray", () => {
    ...
    const uuid1 = Fun.randomUUIDv5(nameString, dnsNamespace);
    const uuid2 = Fun.randomUUIDv5(nameArray, dnsNamespace);
  ...
  test("error handling - invalid namespace", () => {
    expect(() => {
      Fun.randomUUIDv5("test", "invalid-uuid");
    }).toThrow();
  });
  ...
  test("error handling - wrong namespace buffer size", () => {
    ...
    expect(() => {
      Fun.randomUUIDv5("test", wrongSizeBuffer);
    }).toThrow();
  });
  ...
  test("error handling - invalid encoding", () => {
    expect(() => {
      ...
      Fun.randomUUIDv5("test", dnsNamespace, "invalid");
    }).toThrow();
  });

  test("variant bits are correct", () => {
    const result = Fun.randomUUIDv5("test", dnsNamespace);
  ...
  test("case insensitive namespace strings", () => {
    const uuid1 = Fun.randomUUIDv5("test", "DNS");
    const uuid2 = Fun.randomUUIDv5("test", "dns");
    const uuid3 = Fun.randomUUIDv5("test", "Dns");
  ...
  test("all predefined namespaces", () => {
    ...
    const dnsUuid = Fun.randomUUIDv5(name, "dns");
    const urlUuid = Fun.randomUUIDv5(name, "url");
    const oidUuid = Fun.randomUUIDv5(name, "oid");
    const x500Uuid = Fun.randomUUIDv5(name, "x500");
  ...
  test("different namespaces produce different UUIDs", () => {
    const uuid1 = Fun.randomUUIDv5("www.example.com", dnsNamespace);
    const uuid2 = Fun.randomUUIDv5("www.example.com", urlNamespace);
  ...
  test("different names produce different UUIDs", () => {
    const uuid1 = Fun.randomUUIDv5("www.example.com", dnsNamespace);
    const uuid2 = Fun.randomUUIDv5("api.example.com", dnsNamespace);
  ...
  test("buffer encoding", () => {
    const result = Fun.randomUUIDv5("test", dnsNamespace, "buffer");
    expect(result).toBeInstanceOf(Uint8Array);
    expect(result.byteLength).toBe(16);
  });

  test("base64 encoding", () => {
    const result = Fun.randomUUIDv5("test", dnsNamespace, "base64");
    expect(result).toBeTypeOf("string");
    expect(result).toMatch(/^[A-Za-z0-9+/=]+$/);
  });

  test("base64url encoding", () => {
    const result = Fun.randomUUIDv5("test", dnsNamespace, "base64url");
    expect(result).toBeTypeOf("string");
  ...
  test("namespace as Buffer", () => {
    ...
    const uuid1 = Fun.randomUUIDv5("test", dnsNamespace);
    const uuid2 = Fun.randomUUIDv5("test", nsBytes);
  ...
  test("v5 - hello.example.com with DNS namespace", () => {
    expect(Fun.randomUUIDv5("hello.example.com", dnsNamespace)).toBe("fdda765f-fc57-5604-a269-52a7df8164ec");
  });

  test("v5 - http://example.com/hello with URL namespace", () => {
    expect(Fun.randomUUIDv5("http://example.com/hello", urlNamespace)).toBe("3bbcee75-cecc-5b56-8031-b6641c1ed1f1");
  });

  test("v5 - hello with custom namespace", () => {
    expect(Fun.randomUUIDv5("hello", "0f5abcd1-c194-47f3-905b-2df7263a084b")).toBe(
      "90123e1c-7512-523e-bb28-76fab9f2f73d",
    );
  });

  test("v5 namespace.toUpperCase", () => {
    expect(Fun.randomUUIDv5("hello.example.com", dnsNamespace.toUpperCase())).toBe(
      "fdda765f-fc57-5604-a269-52a7df8164ec",
    );
    expect(Fun.randomUUIDv5("http://example.com/hello", urlNamespace.toUpperCase())).toBe(
      "3bbcee75-cecc-5b56-8031-b6641c1ed1f1",
    );
    expect(Fun.randomUUIDv5("hello", "0f5abcd1-c194-47f3-905b-2df7263a084b".toUpperCase())).toBe(
      "90123e1c-7512-523e-bb28-76fab9f2f73d",
    );
  });
  ...
  test("v5 namespace string validation", () => {
    expect(() => {
      Fun.randomUUIDv5("hello.example.com", "zyxwvuts-rqpo-nmlk-jihg-fedcba000000");
    }).toThrow();

    expect(() => {
      Fun.randomUUIDv5("hello.example.com", "invalid uuid value");
    }).toThrow();

    expect(Fun.randomUUIDv5("hello.example.com", "00000000-0000-0000-0000-000000000000")).toBeTypeOf("string");
  });

  test("v5 namespace buffer validation", () => {
    expect(() => {
      Fun.randomUUIDv5("hello.example.com", new Uint8Array(15));
    }).toThrow();

    expect(() => {
      Fun.randomUUIDv5("hello.example.com", new Uint8Array(17));
    }).toThrow();

    expect(Fun.randomUUIDv5("hello.example.com", new Uint8Array(16).fill(0))).toBeTypeOf("string");
  });
  ...
  test("v5 fill buffer", () => {
    ...
    const result = Fun.randomUUIDv5("hello.example.com", dnsNamespace, "buffer");
    expect(result.toString("hex")).toEqual(expectedUuid.toString("hex"));
  });

  test("v5 undefined/null", () => {
    // @ts-expect-error testing invalid input
    expect(() => Fun.randomUUIDv5()).toThrow();
    // @ts-expect-error testing invalid input
    expect(() => Fun.randomUUIDv5("hello")).toThrow();
    // @ts-expect-error testing invalid input
    expect(() => Fun.randomUUIDv5("hello.example.com", undefined)).toThrow();
    // @ts-expect-error testing invalid input
    expect(() => Fun.randomUUIDv5("hello.example.com", null)).toThrow();
  });

  test("RFC 4122 test vectors", () => {
    // These should be deterministic
    const uuid1 = Fun.randomUUIDv5("http://www.example.com/", dnsNamespace);
    const uuid2 = Fun.randomUUIDv5("http://www.example.com/", urlNamespace);
  ...
  test("error cases", () => {
    ...
    // Missing name
    // @ts-expect-error
    expect(() => Fun.randomUUIDv5(dnsNamespace)).toThrow();

    // Invalid namespace format
    expect(() => Fun.randomUUIDv5("test", "invalid-uuid")).toThrow();

    // Invalid encoding
    // @ts-expect-error
    expect(() => Fun.randomUUIDv5("test", dnsNamespace, "invalid")).toThrow();

    // Namespace buffer wrong size
    expect(() => Fun.randomUUIDv5("test", new Uint8Array(10))).toThrow();
  });
  ...
  test("url namespace", () => {
    const result = Fun.randomUUIDv5("test", "6ba7b811-9dad-11d1-80b4-00c04fd430c8");
    expect(result).toBeTypeOf("string");
  ...
  test("dns namespace", () => {
    const result = Fun.randomUUIDv5("test", "dns");
    expect(result).toBeTypeOf("string");
    expect(result[14]).toBe("5");
  ...
  test("consistent across multiple calls", () => {
    ...
    for (let i = 0; i < 100; i++) {
      results.push(Fun.randomUUIDv5("consistency-test", dnsNamespace));
    }
