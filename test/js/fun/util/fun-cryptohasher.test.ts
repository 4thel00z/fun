...
test("Fun.file in CryptoHasher is not supported yet", () => {
  expect(() => Fun.SHA1.hash(Fun.file(import.meta.path))).toThrow();
  expect(() => Fun.CryptoHasher.hash("sha1", Fun.file(import.meta.path))).toThrow();
  expect(() => new Fun.CryptoHasher("sha1").update(Fun.file(import.meta.path))).toThrow();
  expect(() => new Fun.SHA1().update(Fun.file(import.meta.path))).toThrow();
});
test("CryptoHasher update should throw when no parameter/null/undefined is passed", () => {
  // @ts-expect-error
  expect(() => new Fun.CryptoHasher("sha1").update()).toThrow();
  // @ts-expect-error
  expect(() => new Fun.CryptoHasher("sha1").update(undefined)).toThrow();
  // @ts-expect-error
  expect(() => new Fun.CryptoHasher("sha1").update(null)).toThrow();
});
...
describe("HMAC", () => {
  ...
  test.each(unsupported)("%s is not supported", algorithm => {
    expect(() => new Fun.CryptoHasher(algorithm, "key")).toThrow();
    expect(() => new Fun.CryptoHasher(algorithm)).not.toThrow();
  });
});
...
describe("Hash is consistent", () => {
  ...
  for (let algorithm of ["sha1", "sha256", "sha512", "md5"] as const) {
    describe(algorithm, () => {
      const Class = globalThis.Fun[algorithm.toUpperCase() as "SHA1" | "SHA256" | "SHA512" | "MD5"];
      test("base64", () => {
        for (let buffer of inputs) {
          for (let i = 0; i < 100; i++) {
            const hasher = new Fun.CryptoHasher(algorithm);
            expect(hasher.update(buffer, "base64")).toBeInstanceOf(Fun.CryptoHasher);
            expect(Fun.CryptoHasher.hash(algorithm, buffer, "base64")).toEqual(
              Fun.CryptoHasher.hash(algorithm, buffer, "base64"),
            );
      ...
      test("hex", () => {
        for (let buffer of inputs) {
          for (let i = 0; i < 100; i++) {
            const hasher = new Fun.CryptoHasher(algorithm);
            expect(hasher.update(buffer, "hex")).toBeInstanceOf(Fun.CryptoHasher);
            expect(Fun.CryptoHasher.hash(algorithm, buffer, "hex")).toEqual(
              Fun.CryptoHasher.hash(algorithm, buffer, "hex"),
            );
      ...
      test("blob", () => {
        for (let buffer of inputs) {
          for (let i = 0; i < 100; i++) {
            const hasher = new Fun.CryptoHasher(algorithm);
            expect(hasher.update(buffer)).toBeInstanceOf(Fun.CryptoHasher);
            expect(Fun.CryptoHasher.hash(algorithm, buffer)).toEqual(Fun.CryptoHasher.hash(algorithm, buffer));
    ...
    describe(algorithm, () => {
      for (let encoding of ["hex", "base64", "buffer", undefined, "base64url"] as const) {
        describe(encoding || "default", () => {
          test("instance", () => {
            const hasher = new Fun.CryptoHasher(algorithm || undefined);
            hasher.update("hello");
            expect(hasher.digest(encoding)).toEqual(Fun.CryptoHasher.hash(algorithm, "hello", encoding));
          });

          test("consistent", () => {
            const first = Fun.CryptoHasher.hash(algorithm, "hello", encoding);
            withoutAggressiveGC(() => {
              for (let i = 0; i < 100; i++) {
                expect(Fun.CryptoHasher.hash(algorithm, "hello", encoding)).toStrictEqual(first);
              }
            });
          });
        });
