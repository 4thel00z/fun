import { estimateShallowMemoryUsageOf, heapStats } from "fun:jsc";
...
describe("Native types report their size correctly", () => {
  it("FormData", () => {
    ...
    const snapshot = Fun.generateHeapSnapshot();
  ...
  it("URL (heap size reporting bug)", () => {
    for (let i = 0; i < 500; i++) {
      ...
      // need to assign it to a global to ensure JSC and Fun do not eliminate it.
