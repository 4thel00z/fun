// Fuzz/robustness tests for Fun.sliceAnsi.
...
const visibleWidth = (s: string) => Fun.stringWidth(Fun.stripANSI(s));
...
describe("sliceAnsi invariants", () => {
  ...
  test("output width never exceeds requested range", () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      const w = Fun.stringWidth(s);
      const a = Math.floor(rng() * (w + 5));
      const b = a + Math.floor(rng() * (w + 5));
      const out = Fun.sliceAnsi(s, a, b);
  ...
  test("adjacent slices cover full visible string", () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const left = Fun.stripANSI(Fun.sliceAnsi(s, 0, mid));
      const right = Fun.stripANSI(Fun.sliceAnsi(s, mid, w));
      expect(left + right).toBe(Fun.stripANSI(s));
    }
  });
  ...
  test("full slice preserves visible content", () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      expect(normalize(Fun.stripANSI(out))).toBe(normalize(Fun.stripANSI(s)));
  ...
  test("slicing a slice is idempotent on visible content", () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const once = Fun.sliceAnsi(s, a, b);
      const twice = Fun.sliceAnsi(once, 0, b - a);
      expect(Fun.stripANSI(twice)).toBe(Fun.stripANSI(once));
    }
  });
  ...
  test("ellipsis output width respects budget", () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      const out = Fun.sliceAnsi(s, 0, n, e);
...
describe("sliceAnsi adversarial", () => {
  ...
  test("inputs near SIMD stride boundaries", () => {
    for (const len of [0, 1, 7, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64, 65]) {
      ...
      expect(Fun.sliceAnsi(s, 0, len)).toBe(s);
      expect(Fun.sliceAnsi(s, 0, Math.floor(len / 2))).toBe(s.slice(0, Math.floor(len / 2)));
      // With ANSI
      const ansi = "\x1b[31m" + s + "\x1b[39m";
      expect(Fun.stripANSI(Fun.sliceAnsi(ansi, 0, len))).toBe(s);
    }
  });
  ...
  test("C1 ST at SIMD boundary positions", () => {
    for (const pos of [0, 1, 7, 8, 15, 16, 17]) {
      ...
      const out = Fun.sliceAnsi(s, 0, pos + 1);
      // Output width should be pos + 1 (prefix 'x's + 'A').
      expect(Fun.stringWidth(out)).toBe(pos + 1);
  ...
  test("many SGR codes don't overflow or quadratic-slow", () => {
    ...
    const out = Fun.sliceAnsi(s, 0, 1);
    expect(Fun.stripANSI(out)).toBe("X");
    // Time bound: should be O(n), not O(n²). Generous threshold for debug builds.
    const start = Fun.nanoseconds();
    for (let i = 0; i < 1000; i++) Fun.sliceAnsi(s, 0, 1);
    const elapsed = (Fun.nanoseconds() - start) / 1e6;
  ...
  test("string of only zero-width chars doesn't hang", () => {
    ...
    const out = Fun.sliceAnsi(zw, 0, 5);
    // Width 0, so [0, 5) should emit all of them (all at position 0).
    expect(Fun.stringWidth(out)).toBe(0);
    // Should terminate — not hang.
    const start = Fun.nanoseconds();
    Fun.sliceAnsi(zw, 0, 5);
    expect(Fun.nanoseconds() - start).toBeLessThan(1e9); // < 1s
  });
  ...
  test("very long ZWJ emoji chain", () => {
    ...
    expect(Fun.stringWidth(many)).toBe(200); // 100 families × width 2
    const out = Fun.sliceAnsi(many, 0, 10);
    expect(Fun.stringWidth(out)).toBe(10); // 5 families
  });
  ...
  test("extreme index values", () => {
    ...
    expect(Fun.sliceAnsi(s, Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER)).toBe("");
    expect(Fun.sliceAnsi(s, -Number.MAX_SAFE_INTEGER, -Number.MAX_SAFE_INTEGER)).toBe("");
    expect(Fun.sliceAnsi(s, -Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER)).toBe("hello");
    expect(Fun.sliceAnsi(s, 0, 0)).toBe("");
    expect(Fun.sliceAnsi(s, NaN, 3)).toBe("hel"); // NaN → 0 per toIntegerOrInfinity
    expect(Fun.sliceAnsi(s, 0, NaN)).toBe(""); // NaN → 0
    // @ts-expect-error — testing coercion
    expect(Fun.sliceAnsi(s, "1", "3")).toBe("el"); // string coercion
  });
  ...
  test("OSC 8 with very long URL", () => {
    ...
    const out = Fun.sliceAnsi(s, 0, 4);
    expect(Fun.stripANSI(out)).toBe("link");
  ...
  test("ANSI code between every single visible char", () => {
    ...
    for (let a = 0; a < chars.length; a++) {
      for (let b = a; b <= chars.length; b++) {
        expect(Fun.stripANSI(Fun.sliceAnsi(s, a, b))).toBe(chars.slice(a, b));
      }
    }
  });
  ...
  test("ANSI between base and combining mark", () => {
    ...
    expect(Fun.stringWidth(Fun.stripANSI(s))).toBe(1);
    const out = Fun.sliceAnsi(s, 0, 1);
    expect(Fun.stripANSI(out)).toBe("e\u0301");
  });
  ...
  test("rope string (concatenation without flattening)", () => {
    ...
    const out = Fun.sliceAnsi(rope, 0, 50);
    expect(Fun.stripANSI(out).length).toBe(50);
  });
  ...
  test("ellipsis string containing ANSI codes", () => {
    ...
    const s = "hello world";
    const out = Fun.sliceAnsi(s, 0, 5, "\x1b[31m…\x1b[39m");
  ...
  test("negative index with ellipsis (exercises pre-pass)", () => {
    ...
    const out = Fun.sliceAnsi(s, -10, undefined, "…");
    // Last 10 chars with leading ellipsis: "…" + 9 x's = width 10
    expect(Fun.stringWidth(out)).toBe(10);
    expect(Fun.stripANSI(out)).toBe("…" + "x".repeat(9));
  });
});

// ============================================================================
// Consistency cross-checks with Fun.stringWidth / Fun.stripANSI
// ============================================================================

describe("sliceAnsi consistency with other Fun APIs", () => {
  test("slice width matches stringWidth delta", () => {
    const rng = makeRng(0xabcd);
    for (let i = 0; i < 100; i++) {
      const s = randomString(rng, 10, 80);
      const totalW = Fun.stringWidth(s);
      // Slice [0, totalW) should give back the full width.
      // Use stripped width on both sides to avoid Fun.stringWidth's
      // ANSI-breaks-grapheme-state bug (see visibleWidth comment at top).
      expect(visibleWidth(Fun.sliceAnsi(s, 0, totalW))).toBe(visibleWidth(s));
    }
  });
  ...
  test("stripANSI(sliceAnsi(s)) == sliceAnsi(stripANSI(s)) for width-1 text", () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      expect(Fun.stripANSI(Fun.sliceAnsi(s, a, b))).toBe(Fun.sliceAnsi(plain, a, b));
    }
  });
});
...
describe("sliceAnsi negative indices", () => {
  test("negative slice equals positive slice via totalWidth", () => {
    ...
    for (let i = 0; i < 150; i++) {
      ...
      const w = Fun.stringWidth(Fun.stripANSI(s));
      // slice(s, -k) should equal slice(s, w - k, w)
      const k = Math.floor(rng() * w) + 1;
      const neg = Fun.sliceAnsi(s, -k);
      const pos = Fun.sliceAnsi(s, w - k, w);
      expect(Fun.stripANSI(neg)).toBe(Fun.stripANSI(pos));
    }
  });
  ...
  test("computeTotalWidth matches stringWidth for cluster-rich input", () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const out = Fun.sliceAnsi(s, -w);
      expect(Fun.stringWidth(out)).toBe(w);
    }
  });
...
describe("sliceAnsi ambiguousIsNarrow fuzz", () => {
  test("narrow slice ⊆ wide slice visibly (narrow chars are subset)", () => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const narrow = Fun.sliceAnsi(s, 0, budget, { ambiguousIsNarrow: true });
      const wide = Fun.sliceAnsi(s, 0, budget, { ambiguousIsNarrow: false });
      expect(Fun.stringWidth(Fun.stripANSI(narrow), { ambiguousIsNarrow: true })).toBeLessThanOrEqual(budget + 1);
      expect(Fun.stringWidth(Fun.stripANSI(wide), { ambiguousIsNarrow: false })).toBeLessThanOrEqual(budget + 1);
    }
  });
});
...
describe("sliceAnsi encoding equivalence", () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      for (const a of [0, 2, 5]) {
        for (const b of [10, 20, 100]) {
          expect(Fun.sliceAnsi(utf16, a, b)).toBe(Fun.sliceAnsi(latin1, a, b));
        }
      }
    }
  });
  ...
  test("Latin-1-range non-ASCII in both encodings", () => {
    ...
    for (let a = 0; a <= 4; a++) {
      for (let b = a; b <= 4; b++) {
        expect(Fun.sliceAnsi(s16, a, b)).toBe(Fun.sliceAnsi(s8, a, b));
      }
    }
  });
});
...
describe("sliceAnsi speculative zone", () => {
  test("string width exactly equals budget (no cut, spec zone flushes)", () => {
    ...
    const s = "hello";
    expect(Fun.sliceAnsi(s, 0, 5, "\u2026")).toBe("hello");
    expect(Fun.sliceAnsi(s, 0, 5, "...")).toBe("hello");
    // With ANSI (forces slow path but same outcome)
    const sa = "\x1b[31mhello\x1b[39m";
    expect(Fun.stripANSI(Fun.sliceAnsi(sa, 0, 5, "\u2026"))).toBe("hello");
  });
  ...
  test("string width exactly one over budget (cut, spec zone discarded)", () => {
    ...
    expect(Fun.sliceAnsi(s, 0, 5, "\u2026")).toBe("hell\u2026");
  });
  ...
  test("wide char straddling spec zone boundary", () => {
    ...
    const s = "ab\u5B89cde";
    expect(Fun.sliceAnsi(s, 0, 5, "\u2026")).toBe("ab\u5B89\u2026");
  });
  ...
  test("spec zone with ANSI between zone content and next char", () => {
    ...
    expect(Fun.stripANSI(out)).toBe("abcd\u2026");
    expect(Fun.stringWidth(Fun.stripANSI(out))).toBe(5);
  });
  ...
  test("spec zone NOT cut (EOF before overflow) → zone flushed, no ellipsis", () => {
    ...
    const s = "abcde";
    expect(Fun.sliceAnsi(s, 0, 5, "\u2026")).toBe("abcde");
    // Same with ANSI (slow path).
    const sa = "\x1b[31mabcde\x1b[39m";
    const out = Fun.sliceAnsi(sa, 0, 5, "\u2026");
    expect(Fun.stripANSI(out)).toBe("abcde");
  });
...
describe("sliceAnsi exception safety", () => {
  ...
  test("throwing ambiguousIsNarrow getter doesn't corrupt state", () => {
    ...
    expect(Fun.sliceAnsi(s, 0, 3)).toBe("hel");
  });
  ...
  test("non-primitive coercion in indices", () => {
    ...
    // @ts-expect-error testing coercion
    expect(Fun.sliceAnsi(s, obj, 5)).toBe("cde");
    expect(calls).toBe(1);
  });
});
...
describe("sliceAnsi bulk-ASCII boundary", () => {
  test("ASCII run ending at slice boundary (bulk processes N-1)", () => {
    ...
    expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 10))).toBe("abcdefghij");
    // emoji starts at col 10, width 2. [0, 11): col 10 < 11 → emitted atomically.
    expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 11))).toBe("abcdefghij\u{1F600}");
    // [0, 10): emoji starts at col 10 = end → NOT emitted.
    expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 10))).toBe("abcdefghij");
    expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 12))).toBe("abcdefghij\u{1F600}");
  });
  ...
  test("single ASCII char (bulkN=0, all goes to per-char)", () => {
    ...
    expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 1))).toBe("a");
    expect(Fun.stripANSI(Fun.sliceAnsi(s, 0, 3))).toBe("a\u{1F600}");
  });
  ...
  test("ASCII + combining mark at the leave-one-behind position", () => {
    ...
    const s = "abcde\u0301";
    expect(Fun.stringWidth(s)).toBe(5);
    expect(Fun.sliceAnsi(s, 0, 5)).toBe("abcde\u0301");
    expect(Fun.sliceAnsi(s, 4, 5)).toBe("e\u0301");
  });
  ...
  test("many short ASCII runs between ANSI (bulk rarely engages)", () => {
    ...
    expect(Fun.stripANSI(Fun.sliceAnsi(s, 25, 75)).length).toBe(50);
  });
});
...
describe("sliceAnsi direct stringWidth invariant (post-fix)", () => {
  test("stringWidth(slice) ≤ budget + 1 without stripANSI workaround", () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      expect(Fun.stringWidth(out)).toBeLessThanOrEqual(Math.max(0, b - a) + 1);
    }
  });
});

// Like randomString but excludes C1 control bytes (0x80-0x9F). Used for tests
// that compare directly against Fun.stringWidth, which doesn't recognize C1
