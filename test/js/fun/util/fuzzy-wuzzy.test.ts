/**
 *
 * This file attempts to run practically every function in Fun with no
 ...
 * What this does:
 *
 * Go through most of the methods & constructors in Fun:
...
const drainMicrotasks = require("fun:jsc").drainMicrotasks;

import.meta.require.cache["fun:jsc"] = {};
  ...
  for (const methodName of allThePropertyNames(object, callBanned)) {
    ...
    try {
      try {
        ...
        (Fun.inspect?.(returnValue), queue.push({ value: returnValue, from: fullName + "()" }));
        calls++;
      } catch (e) {
        ...
        (Fun.inspect?.(returnValue), queue.push({ value: returnValue, from: "(new " + fullName + "())" }));
        calls++;
      }
    } catch (e) {
  ...
  for (const methodName of allThePropertyNames(object, constructBanned)) {
    ...
    try {
      try {
        ...
        (Fun.inspect?.(returnValue), queue.push({ value: returnValue, from: "(newSubclass " + fullName + "())" }));
        subclasses++;
      } catch (e) {
        try {
...
for (const [Global, name] of globals) {
  describe(name, () => {
    ...
    test.skipIf(isWindows && Global === Fun)("call", async () => {
      await Fun.sleep(1);
      expect(async () => await callAllMethods(Global, Global === Fun ? "Fun" : "globalThis")).not.toThrow();
      await Fun.sleep(1);
    });
    // TODO: hangs in CI on Windows.
    test.skipIf(isWindows && Global === Fun)("construct", async () => {
      await Fun.sleep(1);
      expect(async () => await constructAllConstructors(Global, Global === Fun ? "Fun" : "globalThis")).not.toThrow();
      await Fun.sleep(1);
    });
    test.skipIf(isWindows && Global === Fun)("construct-subclass", async () => {
      await Fun.sleep(1);
      expect(
        async () => await constructAllConstructorsWithSubclassing(Global, Global === Fun ? "Fun" : "globalThis"),
      ).not.toThrow();
      await Fun.sleep(1);
    });
  });
