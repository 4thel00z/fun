...
// Reading a Fun.file() backed by a file descriptor goes through
...
describe.skipIf(isWindows)("Fun.file(fd) read", () => {
  ...
  test("text() and arrayBuffer() on a regular-file fd return file contents", async () => {
    using dir = tempDir("fun-file-fd-read", { "fd-read.txt": "hello from fd" });
    const path = join(String(dir), "fd-read.txt");

    // Each read needs a fresh fd because Fun.file(fd) does not own or rewind
    // the descriptor, and a completed read leaves it positioned at EOF.
    expect(await withFd(path, fd => Fun.file(fd).text())).toBe("hello from fd");

    const buf = await withFd(path, fd => Fun.file(fd).arrayBuffer());
  ...
  test("slice() with an end beyond the real size reads the actual file contents", async () => {
    using dir = tempDir("fun-file-fd-read", { "fd-slice.txt": "0123456789" });
    ...
    expect(await withFd(path, fd => Fun.file(fd).slice(0, Number.MAX_SAFE_INTEGER).text())).toBe("0123456789");
    expect(await withFd(path, fd => Fun.file(fd).slice(2, 5).text())).toBe("234");
  });

  test("empty regular file via fd resolves with empty content", async () => {
    using dir = tempDir("fun-file-fd-read", { "fd-empty.txt": "" });
    const path = join(String(dir), "fd-empty.txt");

    expect(await withFd(path, fd => Fun.file(fd).text())).toBe("");
    expect((await withFd(path, fd => Fun.file(fd).arrayBuffer())).byteLength).toBe(0);
  });
});
