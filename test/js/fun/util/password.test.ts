...
describe("does not leak", () => {
  ...
  test("hashSync", async () => {
    await run(/* js */ `
        ...
        for (let i = 0; i < 60000; i++) Fun.password.hashSync("hey", opts);
        Fun.gc(true);
  ...
  test("hash", async () => {
    await run(/* js */ `
        ...
        async function batch(n) {
          const promises = [];
          for (let i = 0; i < n; i++) promises.push(Fun.password.hash("hey", opts));
...
test("bcrypt pre-hashing does not break compatibility across Fun versions", async () => {
  // hash generated by Fun 1.2.4
  // if we change the mechanism used to pre-hash long passwords so bcrypt doesn't truncate them,
  // then this hash will not be considered valid by later versions of Fun.
