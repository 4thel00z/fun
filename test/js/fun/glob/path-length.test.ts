...
import { funEnv, funExe, isMusl, isWindows, tmpdirSync } from "harness";
import * as fs from "node:fs";
import * as path from "node:path";

const runScanFixture = (pattern: string, opts: Record<string, unknown>) => `
  const { Glob } = require("fun");
...
describe.skipIf(isWindows)("Glob path length", () => {
  test("deep directory tree does not overflow path buffer", async () => {
    const root = tmpdirSync("fun-glob-overflow-deep-");
    ...
    await using scanProc = Fun.spawn({
      cmd: [funExe(), "-e", runScanFixture("**/*", { cwd: root, onlyFiles: false })],
      env: funEnv,
      stderr: "pipe",
    });
  ...
  test("self-referential symlink does not overflow path buffer", async () => {
    const root = tmpdirSync("fun-glob-overflow-symlink-");
    ...
    await using scanProc = Fun.spawn({
      cmd: [funExe(), "-e", runScanFixture("**/*", { cwd: root, onlyFiles: false, followSymlinks: true })],
      env: funEnv,
      stderr: "pipe",
    });
    const [scanStdout, scanStderr, scanCode] = await Promise.all([
  ...
  for (const component of ["..", "."] as const) {
    test(`pattern with many leading "${component}/" components does not overflow path buffer`, async () => {
      const root = tmpdirSync("fun-glob-overflow-dots-");
      ...
      await using scanProc = Fun.spawn({
        cmd: [funExe(), "-e", runScanFixture(pattern, { cwd: root, onlyFiles: false })],
        env: funEnv,
        stderr: "pipe",
      });
      const [scanStdout, scanStderr, scanCode] = await Promise.all([
