...
test.concurrent("fun ./index.html", async () => {
  ...
  // Start the server by running fun with the HTML file
  await using process = Fun.spawn({
    cmd: [funExe(), "index.html", "--port=0"],
    env: {
      ...funEnv,
      NODE_ENV: "production",
    },
...
test.concurrent("fun ./index.html ./about.html", async () => {
  const dir = tempDirWithFiles("html-multiple-entries-test", {
    ...
    "about.js": /*js*/ `
      ...
      console.log(process.env.FUN_PUBLIC_FOO);
      console.log(typeof process.env.FUN_PRIVATE_FOO !== "undefined");
    `,
    "funfig.toml": /*toml*/ `
[serve.static]
env = "FUN_PUBLIC_*"
  ...
  // Start the server by running fun with multiple HTML files
  await using process = Fun.spawn({
    cmd: [funExe(), "index.html", "about.html", "--port=0"],
    env: {
      ...funEnv,
      NODE_ENV: "production",
      FUN_PUBLIC_FOO: "bar",
  ...
  try {
    ...
    if (aboutJsMatch) {
      ...
      expect(jsResponse.status).toBe(200);
      const js = await jsResponse.text();
      expect(js).not.toContain("process.env.FUN_PUBLIC_FOO");
      expect(js).toContain('console.log("bar")');
      expect(js).toContain("process.env.FUN_PRIVATE_FOO");
...
test.concurrent("fun *.html", async () => {
...
test.concurrent("fun serve svg files with correct Content-Type", async () => {
...
test.concurrent("fun serve files with correct Content-Type headers", async () => {
...
test("importing fun:main from HTML entry preload does not crash", async () => {
  const dir = tempDirWithFiles("html-entry-fun-main", {
    "index.html": /*html*/ `
      <!DOCTYPE html>
      <html>
  ...
  await using proc = Fun.spawn({
    cmd: [funExe(), "--preload", "./preload.mjs", "index.html", "--port=0"],
    env: funEnv,
    cwd: dir,
    stdout: "pipe",
    stderr: "pipe",
