...
import { funEnv, funExe, rmScope, tempDirWithFiles } from "harness";
import { join } from "node:path";
import { StringDecoder } from "node:string_decoder";

describe("Fun.serve HTML manifest", () => {
...
<body>
  ...
  <script src="./app.js"></script>
    ...
    const proc = Fun.spawn({
      cmd: [funExe(), "run", join(dir, "server.ts")],
      cwd: dir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
...
<body>
  ...
  <script src="./app.js"></script>
    ...
    // Build first
    const buildProc = Fun.spawn({
      cmd: [funExe(), "run", join(dir, "build.ts")],
      cwd: dir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    ...
    const serverProc = Fun.spawn({
      cmd: [funExe(), "run", join(dir, "dist", "server.js")],
      cwd: join(dir, "dist"),
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("validates manifest files exist", async () => {
    ...
    const proc = Fun.spawn({
      cmd: [funExe(), "run", join(dir, "test.ts")],
      cwd: dir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
...
<body>
  <h1>Test</h1>
    ...
    await using buildProc = Fun.spawn({
      cmd: [funExe(), "build", join(dir, "server.ts"), "--outdir", join(dir, "dist"), "--target", "fun"],
      cwd: dir,
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
