...
// https://github.com/underdoc-org/fun/issues/3969
test("formData uploads roundtrip, with a call to .body", async () => {
  const file = Fun.file(import.meta.dir + "/fetch.js.txt");
...
test.todoIf(isBroken && isWindows)(
  ...
  async () => {
    ...
    hugeTxt.fill("huge");
    const hash = Fun.CryptoHasher.hash("sha256", hugeTxt, "hex");
    ...
    using server = Fun.serve({
      ...
      async fetch(req) {
        const hasher = new Fun.CryptoHasher("sha256");
...
test("missing file throws the expected error", async () => {
  ...
  withoutAggressiveGC(() => {
    const body = Fun.file(import.meta.dir + "/fetch123123231123.js.txt");
    for (let i = 0; i < 1000; i++) {
      ...
      expect(Fun.peek.status(resp)).toBe("rejected");
