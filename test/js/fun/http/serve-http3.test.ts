...
const server = serve({
  ...
  routes: {
    ...
    "/file-route": Fun.file(process.env.BIG_FILE),
  },
  async fetch(req) {
    ...
    if (url.pathname === "/lifetime") {
      ...
      else if (mode === "macro") await Fun.sleep(0);
      else if (mode === "double") { await Promise.resolve(); await Fun.sleep(0); }
    ...
    if (url.pathname === "/file-stream") {
      return new Response(Fun.file(process.env.BIG_FILE).stream());
    }
    ...
    if (url.pathname === "/file") {
      return new Response(Fun.file(process.env.BIG_FILE));
    }
    if (url.pathname === "/huge-file") {
      return new Response(Fun.file(process.env.HUGE_FILE));
    }
...
async function withServer(
  ...
  const proc = Fun.spawn({
    cmd: [funExe(), "server.mjs"],
    cwd: String(dir),
    env: { ...funEnv, ...env, BIG_FILE: join(String(dir), "big.bin"), HUGE_FILE: join(String(dir), "huge.bin") },
    stdout: "inherit",
    stderr: "pipe",
    stdin: "pipe",
  });
  ...
  try {
    ...
    proc.stdin?.end();
    await Promise.race([proc.exited, Fun.sleep(2000)]);
    proc.kill();
    await proc.exited;
  }
}

describe("Fun.serve HTTP/3", () => {
  ...
  test("Fun.file response body", async () => {
    await withServer(async port => {
  ...
  test("validation: http3 without tls throws", async () => {
    await using proc = Fun.spawn({
      cmd: [funExe(), "-e", "Fun.serve({ port: 0, http3: true, fetch: () => new Response('x') })"],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  test("file route (Fun.file value) streams over H3", async () => {
    await withServer(async port => {
  ...
  test("validation: http1:false without http3 throws", async () => {
    await using proc = Fun.spawn({
      cmd: [funExe(), "-e", "Fun.serve({ port: 0, http1: false, fetch: () => new Response('x') })"],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
...
describe("Fun.serve HTTP/3 adversarial", () => {
  ...
  test("Response(Fun.file().stream()) goes through H3ResponseSink", async () => {
    await withServer(async (port, dir) => {
      const raw = await fetchH3(port, "/file-stream").then(r => r.bytes());
      expect(raw.length).toBe(200 * 1024);
      const onDisk = await Fun.file(join(dir, "big.bin")).bytes();
  ...
  test("Fun.file >=1 MiB takes the reader path, not sendfile", async () => {
    await withServer(async port => {
...
describe("Fun.serve HTTP/3 lifecycle", () => {
    ...
    const script = `
      ...
      const server = Fun.serve({
        ...
        async fetch(req) {
          ...
          if (url.pathname === "/slow") {
            inflight++;
            while (!stopping) await Fun.sleep(5);
            await Fun.sleep(50);
            return new Response("late");
          }
...
describe("Fun.serve HTTP/3 production", () => {
