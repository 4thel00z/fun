import { file, gc, Serve, serve, Server } from "fun";
import { afterAll, afterEach, describe, expect, it, mock } from "fun:test";
import { readFileSync, writeFileSync } from "fs";
import { funEnv, funExe, dumpStats, isBroken, isIntelMacOS, isIPv4, isIPv6, isPosix, tls, tmpdirSync } from "harness";
...
// This test reproduces a crash in Fun v1.1.18 and earlier
            ...
            return new Response(
              new ReadableStream({
                async pull(controller) {
                  ...
                  if (withDelay) await Fun.sleep(1);
                  controller.close();
                },
                cancel: cancelledFn,
              }),
    ...
    for (let isDirect of [true, false] as const) {
      it(
        ...
        async () => {
          ...
          const expected = Fun.CryptoHasher.hash("sha256", blob, "base64");
          const initialCount = heapStats().objectTypeCounts.ReadableStream || 0;

          await runTest(
            {
              async fetch(req) {
                var hasher = new Fun.SHA256();
            ...
            async server => {
              ...
              async function callback() {
                ...
                const digest = Buffer.from(Fun.concatArrayBuffers(chunks)).toString();
...
it("should display a welcome message when the response value type is incorrect", async () => {
  await runTest(
    ...
    async server => {
      ...
      expect(text).toContain("Welcome to Fun!");
    },
  );
});
...
it("request.url should log successfully", async () => {
  ...
  await runTest(
    {
      fetch(req) {
        expect(Fun.inspect(req).includes(expected)).toBe(true);
        return new Response(file(fixture));
      },
    },
    async server => {
  ...
  describe("error handler", () => {
    it("throw on pull renders headers, does not call error handler", async () => {
      ...
      await using subprocess = Fun.spawn({
        ...
        cmd: [funExe(), "readable-stream-throws.fixture.js"],
        env: funEnv,
        stdout: "ignore",
        stderr: "pipe",
        ipc: onMessage,
...
describe("should support Content-Range with Fun.file()", () => {
  ...
  const getServer = runTest.bind(null, {
    fetch(req) {
      ...
      return new Response(Fun.file(fixture).slice(start, end));
    },
  });
...
it("formats error responses correctly", async () => {
  ...
  const c = spawn(funExe(), ["./error-response.js"], {
    cwd: import.meta.dir,
    env: funEnv,
  });
...
it("propagates content-type from a Fun.file()'s file path in fetch()", async () => {
  const body = Fun.file(import.meta.dir + "/fetch.js.txt");
  ...
  using server = Fun.serve({
    ...
    async fetch(req) {
      ...
      return new Response(Fun.file(import.meta.dir + "/fetch.js.txt"));
    },
  });
...
it("unix socket connection in Fun.serve", async () => {
  const unix = join(tmpdir(), "fun." + Date.now() + ((Math.random() * 32) | 0).toString(16) + ".sock");
  using server = Fun.serve({
    unix,
  ...
  await using connection = await Fun.connect({
    unix,
    socket: {
      data(socket, data) {
        received.push(data);
...
it("#5859 arrayBuffer", async () => {
  ...
  await Fun.write(tmp, new Uint8Array([0xfd]));
  expect(async () => await Fun.file(tmp).json()).toThrow();
});
...
describe("server.requestIP", () => {
  ...
  it.if(isPosix)("server.requestIP (unix)", async () => {
    ...
    const connection = await Fun.connect({
      unix,
      socket: {
        data(socket, data) {
          received.push(data);
...
it("should support promise returned from error", async () => {
  ...
  await using subprocess = Fun.spawn({
    cwd: import.meta.dirname,
    cmd: [funExe(), "fun-serve.fixture.js"],
    env: funEnv,
    stdout: "ignore",
    stderr: "pipe",
    ipc(message) {
...
it("should be able to stop in the middle of a file response", async () => {
  ...
  for (let i = 0; i < 3; i++) {
    const process = Fun.spawn([funExe(), fixture], {
      env: funEnv,
      stderr: "inherit",
      stdout: "pipe",
      stdin: "ignore",
...
it.concurrent("should not send extra bytes when using sendfile", async () => {
  ...
  await Fun.write(tmpFile, payload);
  using serve = Fun.serve({
    port: 0,
    fetch(req) {
      const pathname = new URL(req.url).pathname;
      if (pathname === "/file") {
        return new Response(Fun.file(tmpFile), {
          headers: {
  ...
  async () => {
    using server = Fun.serve({
      ...
      async fetch(req, server) {
        ...
        await Fun.sleep(10000); //uWS precision is not great
