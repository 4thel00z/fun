import type { Server, Subprocess } from "fun";
...
test("serve html", async () => {
  const dir = tempDirWithFiles("html-css-js", {
    ...
    "index.html": /*html*/ `
      ...
      <html>
        <head>
          <title>Fun HTML Import Test</title>
          <link rel="stylesheet" href="styles.css">
          <script type="module" src="script.js"></script>
        </head>
        <body>
...
describe("serve plugins", () => {
  ...
  test("basic plugin", async () => {
    const dir = await tempDirWithFiles("fun-serve-html-txt", {
      "funfig.toml": /* toml */ `
[serve.static]
plugins = ["./plugin.ts"]
`,
...
`,
      "example.txt": "LMAO",
      "plugin.ts": /* ts */ `
import type { FunPlugin } from "fun";

const p: FunPlugin = {
  name: "my-plugin",
  setup(build) {
    build.onLoad({ filter: /\\.txt$/ }, async ({ path }) => {
      const text = await Fun.file(path).text();
      return {
        loader: "text",
...
async function waitForServer(
  ...
  const process = Fun.spawn({
    cmd: [funExe(), join(import.meta.dir, "fun-serve-static-fixture.js")],
    env: {
      ...funEnv,
      NODE_ENV: undefined,
    },
...
test("serve html error handling", async () => {
  const dir = tempDirWithFiles("fun-serve-html-error-handling", {
    "index.html": /*html*/ `
      <!DOCTYPE html>
      <html>
        <head>
