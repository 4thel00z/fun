...
async function waitForPendingRequests(server: ReturnType<typeof Fun.serve>, expected: number) {
  for (let i = 0; i < 100; i++) {
...
test("RequestContext is freed when client aborts before Promise<Response> settles", async () => {
  await using proc = Fun.spawn({
    cmd: [funExe(), join(import.meta.dir, "serve-pending-promise-abort-leak-fixture.ts")],
    env: funEnv,
    stdout: "pipe",
    stderr: "pipe",
  });
...
test("streaming 413 detaches the response so a late resolve/reject is a no-op", async () => {
  ...
  // ctx.resp).
  await using proc = Fun.spawn({
    cmd: [funExe(), join(import.meta.dir, "serve-413-streaming-late-resolve-fixture.ts")],
    env: funEnv,
    stdout: "pipe",
    stderr: "pipe",
  });
