 ...
 *
 * Fun has no native QUIC client yet, so HTTP/3 server tests shell out to a
...
function findCurlH3(): string | null {
  ...
  for (const candidate of [process.env.CURL_HTTP3, "curl-h3", "curl"]) {
    ...
    if (!bin) continue;
    const proc = Fun.spawnSync({ cmd: [bin, "--version"], stdout: "pipe", stderr: "ignore" });
...
export async function fetchH3(input: string | URL, init: Init = {}): Promise<Response> {
  ...
  let headerText = "";
  try {
    headerText = await Fun.file(headerFile).text();
  } catch {}
  try {
    await Fun.file(headerFile).unlink();
  } catch {}
...
export function httpProtocols(): Array<{
  ...
  /** Serve options to spread into Fun.serve() for this protocol. */
