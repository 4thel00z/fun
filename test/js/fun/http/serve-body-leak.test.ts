...
import { funEnv, funExe, isCI, isDebug, isFlaky, isLinux, isWindows } from "harness";
...
const zeroCopyJSONPayload = new Blob([JSON.stringify({ fun: payload })]);
...
async function getURL() {
  ...
  const process = Fun.spawn([funExe(), "--smol", join(import.meta.dirname, "body-leak-test-fixture.ts")], {
    env: funEnv,
    stdout: "inherit",
    stderr: "inherit",
    stdin: "ignore",
