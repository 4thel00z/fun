...
import { setSocketOptions } from "fun:internal-for-testing";
import { describe, expect, test } from "fun:test";
import { funEnv, funExe, isASAN, isPosix } from "harness";
...
describe.if(isPosix)("HTTP server handles split chunk-size CRLF", () => {
  ...
  test.concurrentIf(!isASAN)("rejects Content-Length values that would alias chunked-encoding state bits", async () => {
    ...
    await using proc = Fun.spawn({ cmd: [funExe(), "-e", script], env: funEnv, stdout: "pipe", stderr: "pipe" });
    const [stdout, stderr, exitCode] = await Promise.all([proc.stdout.text(), proc.stderr.text(), proc.exited]);
