import type { Server, ServerWebSocket, Socket } from "fun";
import { describe, expect, test } from "fun:test";
import { funEnv, funExe, funRun, isWindows, rejectUnauthorizedScope, tempDirWithFiles, tls } from "harness";
import path from "path";

describe.concurrent("Server", () => {
  test("should not use 100% CPU when websocket is idle", async () => {
    const { stderr } = funRun(path.join(import.meta.dir, "fun-websocket-cpu-fixture.js"));
    expect(stderr).toBe("");
  });
  ...
  test("should not allow Fun.serve without first argument being a object", () => {
    expect(() => {
      //@ts-ignore
      using server = Fun.serve();
    }).toThrow("Fun.serve expects an object");

    [undefined, null, 1, "string", true, false, Symbol("symbol")].forEach(value => {
      expect(() => {
        //@ts-ignore
        using server = Fun.serve(value);
      }).toThrow("Fun.serve expects an object");
    });
  });

  test("should not allow Fun.serve with invalid tls option", () => {
  ...
  test("should allow Fun.serve using null or undefined tls option", () => {
  ...
  test("should not crash with big formData", async () => {
    const proc = Fun.spawn({
      cmd: [funExe(), "big-form-data.fixture.js"],
      cwd: import.meta.dir,
      env: funEnv,
    });
    await proc.exited;
  ...
  test("should be able to parse source map and fetch small stream", async () => {
    const { stderr, exitCode } = Fun.spawnSync({
      cmd: [funExe(), path.join("js-sink-sourmap-fixture", "index.mjs")],
      cwd: import.meta.dir,
      env: funEnv,
      stdin: "inherit",
      stderr: "inherit",
  ...
  test("rejected promise handled by error method should not be logged", async () => {
    const { stderr, exitCode } = Fun.spawnSync({
      cmd: [funExe(), path.join("rejected-promise-fixture.js")],
      cwd: import.meta.dir,
      env: funEnv,
      stderr: "pipe",
    });
...
test("Fun.serve().unref() works", async () => {
...
test("Fun does not crash when given invalid config", async () => {
  ...
  for (const options of cases) {
    expect(() => {
      Fun.serve(options as any);
    }).toThrow();
  }
});

test("Fun should be able to handle utf16 inside Content-Type header #11316", async () => {
  using server = Fun.serve({
    port: 0,
    fetch() {
...
test("should be able to async upgrade using custom protocol", async () => {
  ...
  using server = Fun.serve<unknown>({
    port: 0,
...
describe("HEAD requests #15355", () => {
  ...
  test("should fallback to the body if content-length is missing in the headers", async () => {
    ...
    });
    {
      ...
      expect(response.status).toBe(200);
      expect(response.headers.get("content-length")).toBe("11");
      expect(response.headers.get("x-fun-test")).toBe("1");
      expect(await response.text()).toBe("");
    }
    {
      const response = await fetch(server.url + "/chunked", {
    ...
    using server = Fun.serve({
      ...
      fetch(req) {
        if (req.url.endsWith("/file")) {
          return new Response(Fun.file(filename));
        }
        return new Response("Hello World");
      },
    });
    ...
    test("status only with headers", async () => {
      using server = Fun.serve({
        ...
        fetch(req) {
          return new Response(null, {
            status: 404,
            headers: { "X-Fun-Test": "1", "Content-Length": "11" },
          });
        },
      });
      const response = await fetch(server.url, { method: "HEAD" });
        ...
        env: {
          ...
          ...(isWindows
            ...
            : {
                ...
                ASAN_OPTIONS: [funEnv.ASAN_OPTIONS, "symbolize=0", "detect_leaks=0"].filter(Boolean).join(":"),
              }),
        },
        stdout: "pipe",
        stderr: "pipe",
  ...
  for (const config of serverConfigurations) {
    ...
    test(testName, async () => {
...

      {
        ...
        const ws = new WebSocket(url.toString()); // fun crashes here
        ws.onopen = () => {
