...
describe("instanceof and type checks", () => {
    ...
    server = Fun.serve({
      ...
      routes: {
        "/instanceof-checks": req => {
          // Check that cookies is an instance of Fun.CookieMap
          expect(req.cookies instanceof Fun.CookieMap).toBe(true);
        ...
        "/constructor-identities": req => {
          ...
          expect(req.cookies.constructor).toBe(Fun.CookieMap);
  ...
  it("cookies is instance of Fun.CookieMap and has right prototype", async () => {
...
describe("Direct usage of Fun.Cookie and Fun.CookieMap", () => {
  it("can create a Cookie directly", () => {
    const cookie = new Fun.Cookie("name", "value");

    expect(cookie.constructor).toBe(Fun.Cookie);
  ...
  it("can create a CookieMap directly", () => {
    ...
    expect(cookieMap.constructor).toBe(Fun.CookieMap);
    expect(cookieMap.size).toBe(0);
  });

  it("can create a CookieMap with a cookie string", () => {
    const cookieMap = new Fun.CookieMap("name=value; foo=bar");
  ...
  it("can use Cookie.parse to parse cookie strings", () => {
    const cookie = Fun.Cookie.parse("name=value; Path=/; Secure; SameSite=Lax");
  ...
  it("can use Cookie.from to create cookies", () => {
    const cookie = Fun.Cookie.from("name", "value", {
      path: "/path",
      domain: "example.com",
      secure: true,
      sameSite: "none",
  ...
  it("correctly handles toJSON methods", () => {
    ...
    const cookieMap = new Fun.CookieMap("a=1; b=2; c=3");
