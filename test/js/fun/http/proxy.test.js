...
import fs from "fs";
import { funEnv, funExe, gc } from "harness";
...
describe.concurrent(() => {
  ...
  it.each([
    ...
    using localServer = Fun.serve({
      port: 0,
      fetch() {
        return new Response("OK");
      },
    });
    const { exited, stderr: stream } = Fun.spawn({
      cmd: [funExe(), "-e", `await fetch("${localServer.url}")`],
      env: {
        ...funEnv,
  ...
  it.each([
    ...
    const { exited, stderr: stream } = Fun.spawn({
      cmd: [funExe(), "-e", `fetch("http://localhost:1").catch(() => {})`],
      env: {
        ...funEnv,
