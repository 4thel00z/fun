...
import { afterAll, beforeAll, describe, expect, it, mock, test } from "fun:test";
...
const files = {
  ...
  "large.txt": Buffer.alloc(LARGE_SIZE, "fun").toString(),
...
describe("Fun.file in serve routes", () => {
  let server: Server;
  let tempDir: string;
  ...
  beforeAll(async () => {
    tempDir = tempDirWithFiles("fun-serve-file-test-", files);

    const routes = {
      "/hello.txt": {
        GET: new Response(Fun.file(join(tempDir, "hello.txt"))),
        HEAD: new Response(Fun.file(join(tempDir, "hello.txt"))),
      },
      "/empty.txt": new Response(Fun.file(join(tempDir, "empty.txt"))),
      "/empty-400.txt": new Response(Fun.file(join(tempDir, "empty.txt")), {
        status: 400,
      }),
      "/binary.bin": new Response(Fun.file(join(tempDir, "binary.bin"))),
      "/large.txt": new Response(Fun.file(join(tempDir, "large.txt"))),
      "/unicode.txt": new Response(Fun.file(join(tempDir, "unicode.txt"))),
      "/json.json": new Response(Fun.file(join(tempDir, "json.json"))),
      "/nested/file.txt": new Response(Fun.file(join(tempDir, "nested", "file.txt"))),
      "/special-chars.txt": new Response(Fun.file(join(tempDir, "special chars & symbols.txt"))),
      "/nonexistent.txt": new Response(Fun.file(join(tempDir, "does-not-exist.txt"))),
      "/with-headers.txt": new Response(Fun.file(join(tempDir, "hello.txt")), {
        headers: {
          "X-Custom-Header": "custom-value",
          "Cache-Control": "max-age=3600",
        },
      }),
      "/with-status.txt": new Response(Fun.file(join(tempDir, "hello.txt")), {
        status: 201,
        statusText: "Created",
      }),
      "/will-be-deleted.txt": new Response(Fun.file(join(tempDir, "will-be-deleted.txt"))),
      "/custom-last-modified.txt": new Response(Fun.file(join(tempDir, "hello.txt")), {
        headers: {
          "Last-Modified": "Wed, 21 Oct 2015 07:28:00 GMT",
        },
      }),
      "/partial.txt": new Response(Fun.file(join(tempDir, "partial.txt"))),
      "/partial-slice.txt": new Response(Fun.file(join(tempDir, "partial.txt")).slice(5, 10)),
      "/fd-not-supported.txt": (() => {
        // This would test file descriptors, but they're not supported yet
        return new Response(Fun.file(join(tempDir, "hello.txt")));
      })(),
      // Static route with user-set Content-Range — auto-Range must be disabled.
      "/user-content-range-route": new Response(Fun.file(join(tempDir, "partial.txt")), {
      ...
      "/range-handler": () => new Response(Fun.file(join(tempDir, "partial.txt"))),
      "/user-content-range-handler": () =>
        new Response(Fun.file(join(tempDir, "partial.txt")), { headers: { "Content-Range": "bytes 0-15/100" } }),
      "/range-after-size": () => {
        const f = Fun.file(join(tempDir, "partial.txt"));
        void f.size;
        return new Response(f);
      },
      "/slice-escape": () => new Response(Fun.file(join(tempDir, "bytes256.bin")).slice(0, 100)),
      "/range-custom-headers": () =>
        new Response(Fun.file(join(tempDir, "partial.txt")), {
  ...
  describe.concurrent("Range via fetch handler edge cases", () => {
    ...
    it("Range header cannot escape a Fun.file().slice(0, n) window via fetch handler", async () => {
