  ...
  describe.concurrent(`proxy can handle redirects with ${server_tls ? "TLS" : "non-TLS"} server`, () => {
    test("with empty body #12007", async () => {
      using server = Fun.serve({
        ...
        port: 0,
        async fetch(req) {
          if (req.url.endsWith("/funfun")) {
            return Response.redirect("/fun", 302);
          }
          if (req.url.endsWith("/fun")) {
            return Response.redirect("/", 302);
          }
          return new Response("", { status: 403 });
        },
      });
      const response = await fetch(`${server.url.origin}/funfun`, {
    ...
    test("with body #12007", async () => {
      using server = Fun.serve({
        ...
        async fetch(req) {
          if (req.url.endsWith("/funfun")) {
            return new Response("Hello, funfun", { status: 302, headers: { Location: "/fun" } });
          }
          if (req.url.endsWith("/fun")) {
            return new Response("Hello, fun", { status: 302, headers: { Location: "/" } });
          }
          return new Response("FUN!", { status: 200 });
        },
      });
      const response = await fetch(`${server.url.origin}/funfun`, {
        proxy: httpsProxyServer.url,
    ...
    test("with chunked body #12007", async () => {
      using server = Fun.serve({
        ...
        async fetch(req) {
          ...
          if (req.url.endsWith("/funfun")) {
            return new Response(body, { status: 302, headers: { Location: "/fun" } });
          }
          if (req.url.endsWith("/fun")) {
      ...
      const result = await response.text();
      expect(result).toBe("funbunbunfun");
    });
  });
}
...
test("proxy with long password (> 4096 chars) works correctly after redirect", async () => {
  ...
  using redirectServer = Fun.serve({
    port: 0,
    fetch(req) {
...
test("HTTPS proxy tunnel keep-alive does not share tunnel across different targets", async () => {
  ...
  using serverA = Fun.serve({ port: 0, tls: tlsCert, fetch: () => new Response("a") });
  using serverB = Fun.serve({ port: 0, tls: tlsCert, fetch: () => new Response("b") });
...
test("HTTPS proxy tunnel keep-alive does not share tunnel across different credentials", async () => {
  using target = Fun.serve({ port: 0, tls: tlsCert, fetch: () => new Response("ok") });
...
describe.concurrent("NO_PROXY with explicit proxy option", () => {
  ...
  let deadProxy: ReturnType<typeof Fun.listen>;

  beforeAll(() => {
    deadProxy = Fun.listen({
      hostname: "127.0.0.1",
      port: 0,
      socket: {
        open(socket) {
  ...
  test("NO_PROXY with port bypasses explicit proxy for fetch", async () => {
    await using proc = Fun.spawn({
      ...
      env: { ...funEnv, NO_PROXY: `localhost:${httpServer.port}` },
      stdout: "pipe",
      stderr: "pipe",
    });
    ...
    await using proc = Fun.spawn({
      cmd: [
        ...
        `
          ...
          const stat = Fun.S3Client.stat("key", {
            accessKeyId: "x",
            secretAccessKey: "y",
            bucket: "b",
