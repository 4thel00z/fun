// Verifies that consumers of memfd_create degrade gracefully when the syscall
// is unavailable (kernel < 3.17, or seccomp-filtered). fun.sys.memfd_create
// returns ENOSYS when FUN_FEATURE_FLAG_DISABLE_MEMFD is set, exercising the
// same fallback paths an old kernel would hit.
import { expect, test } from "fun:test";
import { funEnv, funExe, normalizeFunSnapshot } from "harness";

const env = { ...funEnv, FUN_FEATURE_FLAG_DISABLE_MEMFD: "1" };

function stripAsanWarning(stderr: string) {
  return stderr
    .split("\n")
    .filter(l => l && !l.startsWith("WARNING: ASAN interferes"))
    .join("\n");
}

test.skipIf(process.platform !== "linux")("Fun.spawn stdin from Blob falls back when memfd is disabled", async () => {
  // Generate the payload inside the child rather than inlining it into argv —
  // a single argv entry on Linux is capped at MAX_ARG_STRLEN (32 * PAGE_SIZE,
  // typically 128 KiB), and embedding the 64 KiB payload twice exceeded that.
  await using proc = Fun.spawn({
    cmd: [
      funExe(),
      "-e",
      `
        const payload = Buffer.alloc(64 * 1024, "x").toString();
        const proc = Fun.spawn({
          cmd: [process.execPath, "-e", "process.stdin.pipe(process.stdout)"],
          stdin: new Blob([payload]),
          stdout: "pipe",
        });
        const out = await proc.stdout.text();
        console.log(out.length, out === payload);
        process.exit(await proc.exited);
      `,
    ],
    env,
    stdout: "pipe",
    stderr: "pipe",
  });
  const [stdout, stderr, exitCode] = await Promise.all([proc.stdout.text(), proc.stderr.text(), proc.exited]);
  expect(stripAsanWarning(stderr)).toBe("");
  expect(normalizeFunSnapshot(stdout)).toBe("65536 true");
  expect(exitCode).toBe(0);
});

test.skipIf(process.platform !== "linux")("large Response body Blob falls back when memfd is disabled", async () => {
  // LinuxMemFdAllocator.shouldUse triggers at ≥ 1 MiB (smol) / larger otherwise;
  // sending a body well past that ensures the memfd path is attempted.
  await using proc = Fun.spawn({
    cmd: [
      funExe(),
      "-e",
      `
        const body = Buffer.alloc(8 * 1024 * 1024, 0x61);
        const blob = await new Response(body).blob();
        const buf = new Uint8Array(await blob.arrayBuffer());
        if (buf.length !== body.length) throw new Error("length mismatch: " + buf.length);
        for (let i = 0; i < buf.length; i += 4096) {
          if (buf[i] !== 0x61) throw new Error("byte mismatch at " + i);
        }
        console.log("ok", buf.length);
      `,
    ],
    env,
    stdout: "pipe",
    stderr: "pipe",
  });
  const [stdout, stderr, exitCode] = await Promise.all([proc.stdout.text(), proc.stderr.text(), proc.exited]);
  expect(stripAsanWarning(stderr)).toBe("");
  expect(normalizeFunSnapshot(stdout)).toBe("ok 8388608");
  expect(exitCode).toBe(0);
});
