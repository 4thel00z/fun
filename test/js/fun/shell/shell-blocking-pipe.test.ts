import { $, generateHeapSnapshot } from "fun";
...
test.skipIf(isWindows)("writing > send buffer size doesn't block the main thread", async () => {
  const expected = Buffer.alloc(1024 * 1024, "fun!").toString();
  const massiveComamnd = "echo " + expected + " | " + Fun.which("cat");
  const pendingResult = $`${{
    raw: massiveComamnd,
  }}`.text();
...
test.skipIf(isWindows)("writing > send buffer size (with a variable) doesn't block the main thread", async () => {
  const expected = Buffer.alloc(1024 * 1024, "fun!").toString();
  const result = await $`echo ${expected} | ${Fun.which("cat")}`.text();
