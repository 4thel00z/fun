import { $ } from "fun";
import { shellInternals } from "fun:internal-for-testing";
...
describe("lex shell", () => {
  ...
  describe("errors", async () => {
    // This is disallowed because the js object references get turned into special vars: $__fun_0, $__fun_1, etc.
    // this will break things inside of a quote.
    test("JS object ref in quotes", async () => {
      const buffer = new Uint8Array(1);
      await TestBuilder.command`FOO=bar ${FUN} -e "console.log(process.env) > ${buffer}"`
    ...
    test("Unclosed subshell", async () => {
      ...
      await TestBuilder.command`echo hi && $(echo uh oh)`
        .stdout("hi\n")
        .stderr("fun: command not found: uh\n")
        .exitCode(1)
        .run();
