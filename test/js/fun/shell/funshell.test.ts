...
import { funExe, isPosix, isWindows, runWithErrorPromise, tempDirWithFiles, tmpdirSync } from "harness";
...
export const funEnv: NodeJS.ProcessEnv = {
  ...process.env,
...
beforeAll(async () => {
  ...
  for (const file of temp_files) {
    const writer = Fun.file(join(temp_dir, file)).writer();
    writer.write("foo");
    writer.end();
  }
});
...
describe("funshell", () => {
  ...
  describe("escape", async () => {
    ...
    test("in command position", async () => {
      const x = "echo hi";
      await TestBuilder.command`${x}`.exitCode(1).stderr("fun: command not found: echo hi\n").run();
    });
  ...
  describe("quiet", async () => {
    test("basic", async () => {
      // Check its buffered
      {
        const { stdout, stderr } = await $`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e "console.log('hi'); console.error('lol')"`;
      ...
      const { stdout, stderr } = Fun.spawnSync(
        [
          FUN,
          "-e",
          "await Fun.$`FUN_DEBUG_QUIET_LOGS=1 ${process.argv0} -e \"console.log('hi'); console.error('lol')\"`.quiet()",
        ],
        {
          env: { FUN_DEBUG_QUIET_LOGS: "1" },
        },
      );
    ...
    test.each([
      ...
      const { stdout, stderr } = Fun.spawnSync(
        [FUN, "-e", `await Fun.$\`echo "test output"\`.quiet(${quietArg === undefined ? "" : quietArg})`],
        {
          env: { FUN_DEBUG_QUIET_LOGS: "1" },
        },
      );
...
`.run();
  ...
  describe("echo+cmdsubst edgecases", async () => {
    ...
    // Issue: #8982
    // https://github.com/underdoc-org/fun/issues/8982
    describe("word splitting", async () => {
      ...
      TestBuilder.command`echo {"console.log(JSON.stringify(process.argv.slice(2)))"} > temp_script.ts; FUN_DEBUG_QUIET_LOGS=1 ${FUN} run temp_script.ts $(echo id)/$(echo region)`
        .stdout('["id/region"]\n')
        .ensureTempDir()
        .runAsTest("make sure its one whole argument");

      // Make sure its two separate arguments
      TestBuilder.command`echo {"console.log(JSON.stringify(process.argv.slice(2)))"} > temp_script.ts; FUN_DEBUG_QUIET_LOGS=1 ${FUN} run temp_script.ts $(echo hi id)/$(echo region)`
        .stdout('["hi","id/region"]\n')
        .ensureTempDir()
  ...
  describe("unicode", () => {
    ...
    test("varname fails", async () => {
      ...
      await TestBuilder.command`${whatsupbro}=NICE; echo $${whatsupbro}`
        .stdout("$元気かい、兄弟\n")
        .stderr("fun: command not found: 元気かい、兄弟=NICE\n")
        .run();
    });
  ...
  test("redirect Uint8Array", async () => {
    ...
    const thisFile = Fun.file(import.meta.path);
  ...
  test("redirect Fun.File", async () => {
    const filepath = join(temp_dir, "lmao.txt");
    const file = Fun.file(filepath);
    const thisFileText = await Fun.file(import.meta.path).text();
  ...
  test("redirect stderr", async () => {
    ...
    await $`${FUN} -e ${code} 2> ${buffer}`.env(funEnv);
...
bar\n`,
  ...
  describe("escaped_newline", () => {
    ...
    TestBuilder.command /* sh */ `${FUN} run ./code.ts hi hello \
    on a newline!
  `
      .ensureTempDir()
...
booga"
  ...
  describe("glob expansion", () => {
    // Issue #8403: https://github.com/underdoc-org/fun/issues/8403
    TestBuilder.command`ls *.sdfljsfsdf`
      .exitCode(1)
      .stderr("fun: no matches found: *.sdfljsfsdf\n")
  ...
  describe("variables", () => {
    test("cmd_local_var", async () => {
      const { stdout } = await $`FOO=bar BOOP=1 ${FUN} -e "console.log(JSON.stringify(process.env))"`;
    ...
    test("shell var", async () => {
      const { stdout } = await $`FOO=bar BAR=baz && BAZ=1 ${FUN} -e "console.log(JSON.stringify(process.env))"`;
      const str = stdout.toString();

      const procEnv = JSON.parse(str);
      expect(procEnv.FOO).toBeUndefined();
      expect(procEnv.BAR).toBeUndefined();
      expect(procEnv).toEqual({ ...funEnv, BAZ: "1" });
    });

    test("export var", async () => {
      const buffer = Buffer.alloc(1 << 20);
      const buffer2 = Buffer.alloc(1 << 20);
      await $`export FOO=bar && BAZ=1 ${FUN} -e "console.log(JSON.stringify(process.env))" > ${buffer} && FUN_TEST_VAR=1 ${FUN} -e "console.log(JSON.stringify(process.env))" > ${buffer2}`;
      ...
      expect(procEnv).toEqual({ ...funEnv, BAZ: "1", FOO: "bar" });
    ...
    test("syntax edgecase", async () => {
      const buffer = new Uint8Array(1 << 20);
      const shellProc = await $`FOO=bar FUN_TEST_VAR=1 ${FUN} -e "console.log(JSON.stringify(process.env))"> ${buffer}`;

      const str = stringifyBuffer(buffer);

      const procEnv = JSON.parse(str);

      expect(procEnv).toEqual({ ...funEnv, FUN_TEST_VAR: "1", FOO: "bar" });
    });
  });
  ...
  test("which", async () => {
    ...
    const { stdout } = await $`which ${FUN} ${bogus}`;
    const funWhich = Fun.which(FUN);
    expect(stdout.toString()).toEqual(`${funWhich}\n${bogus} not found\n`);
  });
...
describe("deno_task", () => {
  describe("commands", async () => {
    ...
    TestBuilder.command`VAR=1 FUN_TEST_VAR=1 ${FUN} -e 'console.log(process.env.VAR)' && echo $VAR`
      .stdout("1\n\n")
      .runAsTest("shell var in command");
    TestBuilder.command`VAR=1 VAR2=2 FUN_TEST_VAR=1 ${FUN} -e 'console.log(process.env.VAR + process.env.VAR2)'`
      .stdout("12\n")
      .runAsTest("shell var in command 2");
    TestBuilder.command`EMPTY= FUN_TEST_VAR=1 ${FUN} -e ${"console.log(`EMPTY: ${process.env.EMPTY}`)"}`
      .stdout("EMPTY: \n")
  ...
  describe("shell variables", async () => {
    TestBuilder.command`echo $VAR && VAR=1 && echo $VAR && ${FUN} -e ${"console.log(process.env.VAR)"}`
      .stdout("\n1\nundefined\n")
      .runAsTest("shell var");
    ...
    TestBuilder.command`VAR=1 && echo Test$VAR && echo $(echo "Test: $VAR") ; echo CommandSub$($VAR) ; echo $ ; echo \$VAR`
      .stdout("Test1\nTest: 1\nCommandSub\n$\n$VAR\n")
      .stderr("fun: command not found: 1\n")
      .runAsTest("shell var 3");
  });

  describe("env variables", async () => {
    TestBuilder.command`echo $VAR && export VAR=1 && echo $VAR && FUN_TEST_VAR=1 ${FUN} -e 'console.log(process.env.VAR)'`
      .stdout("\n1\n1\n")
  ...
  describe("pipeline", async () => {
    TestBuilder.command`echo 1 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
      .stdout("1\n")
      .runAsTest("basic pipe");

    TestBuilder.command`echo 1 | echo 2 && echo 3`.stdout("2\n3\n").runAsTest("pipe in conditional");

    TestBuilder.command`echo $(sleep 0.1 && echo 2 & echo 1) | FUN_DEBUG_QUIET_LOGS=1 FUN_TEST_VAR=1 ${FUN} -e 'await process.stdin.pipe(process.stdout)'`
      .stdout("1 2\n")
      .todo("& not supported")
      .runAsTest("complicated pipeline");

    TestBuilder.command`echo 2 | echo 1 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
      .stdout("1\n")
      .runAsTest("multi pipe");

    TestBuilder.command`FUN_TEST_VAR=1 ${FUN} -e 'console.log(1); console.error(2);' | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
      .stdout("1\n")
      .stderr("2\n")
    ...
    // await TestBuilder.command`FUN_TEST_VAR=1 ${FUN} -e 'console.log(1); console.error(2);' | FUN_TEST_VAR=1 ${FUN} -e 'setTimeout(async () => { await Deno.stdin.readable.pipeTo(Deno.stderr.writable) }, 10)' |& FUN_TEST_VAR=1 ${FUN} -e 'await Deno.stdin.readable.pipeTo(Deno.stderr.writable)'`
    //   .stderr("2\n1\n")
    //   .run();
    ...
    TestBuilder.command`echo 1 | FUN_DEBUG_QUIET_LOGS=1 FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)' > output.txt`
      .fileEquals("output.txt", "1\n")
      .runAsTest("pipe with redirect to file");

    TestBuilder.command`echo 1 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stderr)' 2> output.txt`
      .fileEquals("output.txt", "1\n")
    ...
    TestBuilder.command`${FUN} -e 'process.exit(1)' | ${FUN} -e 'console.log("hi")'`
      .exitCode(0)
      .stdout("hi\n")
      .runAsTest("last exit code");

    TestBuilder.command`ls sldkfjlskdjflksdjflksjdf | ${FUN} -e 'console.log("hi")'`
      .exitCode(0)
      .stdout("hi\n")
      .stderr("ls: sldkfjlskdjflksdjflksjdf: No such file or directory\n")
      .runAsTest("last exit code");

    TestBuilder.command`ksldfjsdflsdfjskdfjlskdjflksdf | ${FUN} -e 'console.log("hi")'`
      .exitCode(0)
      .stdout("hi\n")
      .stderr("fun: command not found: ksldfjsdflsdfjskdfjlskdjflksdf\n")
      .runAsTest("last exit code 2");

    TestBuilder.command`echo hi | ${FUN} -e 'process.exit(69)'`.exitCode(69).stdout("").runAsTest("last exit code 3");

    describe("pipeline stack behavior", () => {
      // Test deep pipeline chains to stress the stack implementation
      TestBuilder.command`echo 1 | echo 2 | echo 3 | echo 4 | echo 5 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("5\n")
        .runAsTest("deep pipeline chain");

      // Test very deep chains that could overflow a recursion-based implementation
      TestBuilder.command`echo start | echo 1 | echo 2 | echo 3 | echo 4 | echo 5 | echo 6 | echo 7 | echo 8 | echo 9 | echo 10 | echo 11 | echo 12 | echo 13 | echo 14 | echo 15 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("15\n")
        .runAsTest("very deep pipeline chain");

      // Test nested pipelines in subshells
      TestBuilder.command`echo outer | (echo inner1 | echo inner2) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("inner2\n")
        .runAsTest("nested pipeline in subshell");

      // Test nested pipelines with command substitution
      TestBuilder.command`echo $(echo nested | echo pipe) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("pipe\n")
        .runAsTest("nested pipeline in command substitution");

      // Test multiple nested pipelines
      TestBuilder.command`(echo a | echo b) | (echo c | echo d) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("d\n")
        .runAsTest("multiple nested pipelines");

      // Test pipeline with conditional that contains another pipeline
      TestBuilder.command`echo test | (echo inner | echo nested && echo after) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("nested\nafter\n")
        .runAsTest("pipeline with conditional containing pipeline");

      // Test deeply nested subshells with pipelines
      TestBuilder.command`echo start | (echo l1 | (echo l2 | (echo l3 | echo final))) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("final\n")
        .runAsTest("deeply nested subshells with pipelines");

      // Test pipeline stack unwinding with early termination
      TestBuilder.command`echo 1 | echo 2 | echo 3 | false | echo 4 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("4\n")
      ...
      TestBuilder.command`echo foreground | echo pipe && (echo background &) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("pipe\n")
      ...
      TestBuilder.command`echo 1 | echo 2; echo 3 | echo 4; echo 5 | echo 6 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("2\n4\n6\n")
      ...
      TestBuilder.command`echo start | nonexistent_command | echo after || echo fallback`
        .stdout("after\n")
        .stderr("fun: command not found: nonexistent_command\n")
      ...
      TestBuilder.command`echo 0 | echo 1 | echo 2 | echo 3 | echo 4 | echo 5 | echo 6 | echo 7 | echo 8 | echo 9 | echo 10 | echo 11 | echo 12 | echo 13 | echo 14 | echo 15 | echo 16 | echo 17 | echo 18 | echo 19 | echo 20 | echo 21 | echo 22 | echo 23 | echo 24 | echo 25 | echo 26 | echo 27 | echo 28 | echo 29 | echo 30 | echo 31 | echo 32 | echo 33 | echo 34 | echo 35 | echo 36 | echo 37 | echo 38 | echo 39 | echo 40 | echo 41 | echo 42 | echo 43 | echo 44 | echo 45 | echo 46 | echo 47 | echo 48 | echo 49 | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("49\n")
        .runAsTest("long pipeline builtin");

      TestBuilder.command`echo 0 | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | cat | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("0\n")
        .runAsTest("long pipeline");

      // Test pipeline stack consistency with complex nesting
      TestBuilder.command`echo outer | (echo inner1 | echo inner2 | (echo deep1 | echo deep2) | echo inner3) | echo final | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("final\n")
        .runAsTest("complex nested pipeline consistency");

      // Test pipeline interruption and resumption
      TestBuilder.command`echo start | (echo pause; echo resume) | echo end | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("end\n")
      ...
      TestBuilder.command`echo level0 | (echo level1 | (echo level2 | (echo level3 | (echo level4 | (echo level5 | (echo level6 | (echo level7 | (echo level8 | (echo level9 | (echo level10 | (echo level11 | (echo level12 | (echo level13 | (echo level14 | (echo level15 | (echo level16 | (echo level17 | (echo level18 | (echo level19 | echo deep_final))))))))))))))))))) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
        .stdout("deep_final\n")
        .runAsTest("extremely deep nested pipeline");

      // Test pathological case: deep nesting + long chains
      TestBuilder.command`echo start | (echo n1 | echo n2 | echo n3 | (echo deep1 | echo deep2 | echo deep3 | (echo deeper1 | echo deeper2 | echo deeper3 | (echo deepest1 | echo deepest2 | echo deepest_final)))) | FUN_TEST_VAR=1 ${FUN} -e 'process.stdin.pipe(process.stdout)'`
  ...
  describe("redirects", async function igodf() {
    ...
    // stdout
    TestBuilder.command`FUN_TEST_VAR=1 ${FUN} -e 'console.log(1); console.error(5)' 1> test.txt`
      .stderr("5\n")
      .fileEquals("test.txt", "1\n")
      .runAsTest("redirect stdout of subproccess");

    // stderr
    TestBuilder.command`FUN_TEST_VAR=1 ${FUN} -e 'console.log(1); console.error(5)' 2> test.txt`
      .stdout("1\n")
    ...
    // /dev/null
    TestBuilder.command`FUN_TEST_VAR=1 ${FUN} -e 'console.log(1); console.error(5)' 2> /dev/null`
      .stdout("1\n")
      .runAsTest("/dev/null");
    ...
    // &> and &>> redirect
    await TestBuilder.command`FUN_TEST_VAR=1 ${FUN} -e 'console.log(1); setTimeout(() => console.error(23), 10)' &> file.txt && FUN_TEST_VAR=1 ${FUN} -e 'console.log(456); setTimeout(() => console.error(789), 10)' &>> file.txt`
    ...
    TestBuilder.command`echo 1 > $EMPTY`
      .stderr("fun: ambiguous redirect: at `echo`\n")
      .exitCode(1)
    ...
    TestBuilder.command`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${"console.log('Stdout'); console.error('Stderr')"} 2>&1`
      .stdout("Stdout\nStderr\n")
      .runAsTest("redirect stderr to stdout");

    TestBuilder.command`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${"console.log('Stdout'); console.error('Stderr')"} 1>&2`
      .stderr("Stdout\nStderr\n")
    ...
    TestBuilder.command`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${"console.log('Hello friends')"} > /dev/null`
      .quiet()
    ...
    const code = /* ts */ `
      ...
      await $\`echo Funception!\`
      `;

    TestBuilder.command`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${code} > /dev/null`
      .quiet()
      .runAsTest("funception redirect /dev/null");
  });
  ...
  test("change env", async () => {
    {
      const { stdout } = await $`echo $FOO`.env({ ...funEnv, FOO: "bar" });
      expect(stdout.toString()).toEqual("bar\n");
    }

    {
      const { stdout } = await $`FUN_TEST_VAR=1 ${FUN} -e 'console.log(JSON.stringify(process.env))'`.env({
        ...funEnv,
        FOO: "bar",
      });
  ...
  // https://github.com/underdoc-org/fun/issues/11305
  test.todoIf(isWindows)("stacktrace", async () => {
    // const folder = TestBuilder.tmpdir();
    const code = /* ts */ `import { $ } from 'fun'
    ...
    await TestBuilder.command`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${code} 2>&1`
      .exitCode(1)
  ...
  test("big_data", async () => {
    ...
    const writer = Fun.stdout.writer();
    ...
    const { stdout, stderr, exitCode } = await $`${FUN} -e ${writerCode} > ${tmpdir}/output.txt`.env(funEnv);
  ...
  // https://github.com/underdoc-org/fun/issues/9458
  test("input", async () => {
    const inputCode = /* ts */ `
    ...
    const { stdout, stderr, exitCode } =
      await Fun.$`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${inputCode} | FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${code}`;
...
`,
  ...
  then
    ...
    .stdout("okay here\n")
    .stderr("fun: command not found: lkfjslkdjfsldf\n")
  ...
  then
    ...
    .stdout("okay here\n")
    .stderr("fun: command not found: lkfjslkdjfsldf\nfun: command not found: sdfkjsldf\n")
    .runAsTest("else");
  ...
  TestBuilder.command`if FUNISBAD; then echo not true; fi && echo fun is good`
    .stdout("fun is good\n")
    .stderr("fun: command not found: FUNISBAD\n")
  ...
  then
    ...
    .stderr("fun: command not found: a\nfun: command not found: b\n")
  ...
  ["if", "else", "elif", "then", "fi"].map(tok => {
    TestBuilder.command`"${{ raw: tok }}"`
      .stderr(`fun: command not found: ${tok}\n`)
      .exitCode(1)
    ...
    const exit = (code: number): { raw: string } => ({
      raw:
        process.platform !== "win32"
          ? `bash -c 'exit $1' -- ${code}`
          : `FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e 'process.exit(${code})'`,
    });
...
describe("subshell", () => {
  ...
  TestBuilder.command`
  FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${"console.log(process.env.FOO)"}
  ...
  TestBuilder.command`\(echo hi \)`.stderr("fun: command not found: (echo\n").exitCode(1).runAsTest("escaped subshell");
...
describe("when a command fails", () => {
  ...
  it("is a ShellError instance", () => expect(e).toBeInstanceOf(Fun.$.ShellError));
...
describe("ShellError constructor", () => {
  test.failing("new $.ShellError()", () => {
    const e = new Fun.$.ShellError();
    expect(e).toBeInstanceOf(Fun.$.ShellError);
...
describe.todo("async", () => {
  TestBuilder.command`echo hi && FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${/* ts */ `await Fun.sleep(500); console.log('noice')`} &; echo hello`
    .stdout("hi\nhello\nnoice\n")
    .runAsTest("basic");

  TestBuilder.command`FUN_DEBUG_QUIET_LOGS=1 ${FUN} -e ${/* ts */ `await Fun.sleep(500); console.log('noice')`} | cat &; echo hello`
    .stdout("hello\nnoice\n")
    .runAsTest("pipeline");
