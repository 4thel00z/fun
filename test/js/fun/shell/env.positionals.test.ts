...
test("$ argv: standalone", async () => {
  const script = path.join(import.meta.dir, "fixtures", "positionals.fun.dev");
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "run", script, "a", "b", "c"],
    stdout: "pipe",
    stdin: "ignore",
    stderr: "pipe",
    env: funEnv,
...
test("$ argv: standalone: only 10", async () => {
  const script = path.join(import.meta.dir, "fixtures", "positionals2.fun.dev");
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "run", script, "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l"],
    stdout: "pipe",
    stdin: "ignore",
    stderr: "pipe",
    env: funEnv,
...
test("$ argv: standalone: non-ascii", async () => {
  const script = path.join(import.meta.dir, "fixtures", "positionals2.fun.dev");
  const { stdout, stderr, exited } = spawn({
    cmd: [funExe(), "run", script, "キ", "テ", "ィ", "・", "ホ", "ワ", "イ", "ト"],
    stdout: "pipe",
    stdin: "ignore",
    stderr: "pipe",
    env: funEnv,
