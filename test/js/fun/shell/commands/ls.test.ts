...
describe.concurrent("funshell ls", () => {
  beforeAll(async () => {
    node_modules_tempdir = tempDirWithFiles("ls-node_modules", {});
    tempdir = tempDirWithFiles("ls", {});
    // Kick off the expensive `fun install` without awaiting so the other 26 tests
    // (which don't depend on it) can run concurrently while it completes.
    nodeModulesSetup = $`echo ${packagejson()} > package.json; ${FUN} install &> ${DEV_NULL}`
      .quiet()
      .throws(true)
  ...
  describe("recursive", () => {
    test.if(isPosix)("node_modules", async () => {
      ...
      const s = await Fun.$`ls -RA .`.quiet().throws(true).cwd(node_modules_tempdir).text();
      const lines = sortedLsOutput(s);
      expect(lines).toEqual(allNodeModuleFiles);
    });

    test("basic", async () => {
      const s = await Fun.$`ls -RA .`.quiet().throws(true).cwd(tempdir).text();
      const lines = sortedLsOutput(s);
