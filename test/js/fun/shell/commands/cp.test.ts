...
import { funExe, createTestBuilder } from "../test_builder";
...
describe.if(!builtinDisabled("cp"))("funshell cp", async () => {
  ...
  describe("uutils ported", () => {
    ...
    TestBuilder.command`cp ${TEST_HELLO_WORLD_SOURCE} ${TEST_HOW_ARE_YOU_SOURCE} ${TEST_COPY_TO_FOLDER} && ${funExe()} -e ${'console.log("HI")'}`
      .ensureTempDir(tmpdir)
      .stdout("HI\n")
      .runAsTest("cp_multiple_files");
