...
import { funEnv, isPosix, tempDir, tempDirWithFiles } from "harness";
import { join } from "path";
import { funExe } from "./test_builder";
...
describe.concurrent("fd leak", () => {
  function fdLeakTest(name: string, builder: () => TestBuilder, runs: number = 1000, threshold: number = 5) {
    test(`fdleak_${name}`, async () => {
      const testcode = await Fun.file(join(import.meta.dirname, "./test_builder.ts")).text();
      ...
      const { exited, stderr: stream } = Fun.spawn([process.argv0, "--smol", "test", join(dir, "script.ts")], {
        env: funEnv,
        stderr: "pipe",
      });
  ...
  function memLeakTestProtect(
    ...
    test.if(runTheTest)(
      ...
      async () => {
        ...
        const { stderr: stream, exited } = Fun.spawn([process.argv0, "--smol", "test", join(dir, "script.ts")], {
          env: funEnv,
          stdout: "ignore",
          stderr: "pipe",
        });
  ...
  memLeakTestProtect(
    ...
    "Buffer",
    "Buffer.alloc(192)",
    "TestBuilder.command`echo ${val} | ${funExe()} -e 'process.stdin.on(\"data\", d => process.stdout.write(d))' | head -c 50`",
  );
  memLeakTestProtect(
        ...
        for (let j = 0; j < 10; j++) {
          ...
          for (let i = 0; i < 10; i++) {
            if (builtin) {
              ...
              promises.push(
                $`${funExe()} -e ${/* ts */ `console.log(Array(1024).fill('a').join(''))`}`.env(funEnv).quiet(),
              );
            }
          }
          ...
          for (let j = 0; j < 10; j++) {
            ...
            for (let i = 0; i < 10; i++) {
              if (builtin) {
                ...
                promises.push($`${funExe()} -e ${/* ts */ `console.log(Array(1024).fill('a').join(''))`}`.env(funEnv));
              }
            }
