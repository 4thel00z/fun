...
describe("shell sentinel character hardening", () => {
  test("string matching internal obj-ref prefix round-trips through interpolation", async () => {
    // \x08 is the shell's internal sentinel byte. When followed by "__fun_"
    ...
    //   2. looksLikeJSObjRef() matches the __fun_ prefix
    //   3. eatJSObjRef() fails because it finds no digit index
    const str = "\x08__fun_abc";
  ...
  test("raw sentinel injection with out-of-bounds index does not crash", async () => {
    ...
    // Run in a subprocess so a crash on old fun doesn't kill the test runner.
    const testScript = [
      'import { $ } from "fun";',
      "const sentinel = String.fromCharCode(8) + '__fun_9999';",
