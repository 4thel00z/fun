...
describe("pipeline stack edge cases", () => {
  ...
  describe("mixed builtin and subprocess pipelines", () => {
    TestBuilder.command`echo hello | ${FUN} -e 'process.stdin.pipe(process.stdout)'`
      .stdout("hello\n")
      .runAsTest("echo builtin | subprocess");

    TestBuilder.command`${FUN} -e 'console.log("hello")' | echo world`
      .stdout("world\n")
      .runAsTest("subprocess | echo builtin");

    TestBuilder.command`true | ${FUN} -e 'process.exit(0)'`
      .exitCode(0)
      .stdout("")
      .runAsTest("true builtin | subprocess exit 0");

    TestBuilder.command`false | ${FUN} -e 'process.exit(0)'`
      .exitCode(0)
      .stdout("")
      .runAsTest("false builtin | subprocess - subprocess exit code wins");

    TestBuilder.command`echo one | true | ${FUN} -e 'process.stdin.pipe(process.stdout)' | false`
      .exitCode(1)
      .stdout("")
