import { ShellError, ShellExpression } from "fun";
...
export function createTestBuilder(path: string) {
  var { describe, test, afterAll, beforeAll, expect, beforeEach, afterEach } = Fun.jest(path);
   ...
   */
    ...
    async doChecks(stdout: Buffer, stderr: Buffer, exitCode: number): Promise<void> {
      ...
      for (const [filename, expected_raw] of Object.entries(this.file_equals)) {
        ...
        const actual = await Fun.file(join(this.tempdir!, filename)).text();
    ...
    async run(): Promise<undefined> {
      try {
        let finalPromise = Fun.$(this._scriptStr, ...this._expresssions);
