...
def stringify_map_keys(obj, from_yaml_package=False):
    """Recursively stringify all map keys to match Fun's YAML behavior.
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            ...
            if key is None:
                ...
                # should be converted to "null" to match Fun's behavior
                str_key = "null"
            elif key == "":
                ...
                # yaml package converts [a, b] to "[ a, b ]" but Fun/JS uses "a,b"
...
def generate_test(test_dir, test_name, check_ast=True, use_js_yaml=False, use_yaml_pkg=False):
    """Generate a single Fun test case from a yaml-test-suite directory.
    ...
    Args:
        ...
        use_js_yaml: If True, generate test using js-yaml instead of Fun's YAML
        use_yaml_pkg: If True, generate test using yaml package instead of Fun's YAML
    """
...
'''
    ...
    if not has_json:
        ...
        if yaml_error:
            # yaml package couldn't parse it, but maybe Fun's YAML can
...
'''
        ...
        # Apply key stringification to match Fun's behavior
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Translate yaml-test-suite to Fun tests')
...
'''
    ...
    # Write the output file to Fun's test directory
    output_dir = '/Users/dylan/code/fun/test/js/fun/yaml'
...
'''
    ...
    print(f"\nTo run tests: cd {output_dir} && fun test")
