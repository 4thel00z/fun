...
describe.concurrent("Fun REPL", () => {
  ...
  describe("REPL commands", () => {
    ...
    test(".save saves history to file", async () => {
      ...
      expect(exitCode).toBe(0);
      const content = await Fun.file(filePath).text();
  ...
  describe("error handling", () => {
    ...
    test("throwing custom inspect doesn't crash the loop", async () => {
      ...
      // leaks (FUN_JSC_validateExceptionChecks in CI) and the loop continues.
      const { stdout, stderr, exitCode } = await runRepl([
  ...
  describe("import statements", () => {
    ...
    test("import default and named together", async () => {
      ...
      const filePath = Fun.pathToFileURL(path.join(String(dir), "mod.mjs")).href;
      const { stdout, stderr, exitCode } = await runRepl([
    ...
    test("side-effect-only import", async () => {
      ...
      const filePath = Fun.pathToFileURL(path.join(String(dir), "side.mjs")).href;
      const { stdout, exitCode } = await runRepl([
  ...
  describe("require", () => {
    ...
    test("require resolves local files relative to cwd", async () => {
      ...
      await using proc = Fun.spawn({
        ...
        stderr: "pipe",
        cwd: String(dir),
        env: { ...funEnv, TERM: "dumb", NO_COLOR: "1" },
      });
  ...
  describe("global objects", () => {
    test("has access to Fun globals", async () => {
      const { stdout, exitCode } = await runRepl(["typeof Fun.version", ".exit"]);
  ...
  describe("welcome message", () => {
    test("shows welcome message with version", async () => {
      ...
      const output = stripAnsi(stdout);
      expect(output).toContain("Welcome to Fun");
      expect(output).toMatch(/Fun v\d+\.\d+\.\d+/);
      expect(exitCode).toBe(0);
    });
  });
  ...
  describe("-e / --eval and -p / --print", () => {
    ...
    test("-e does not show welcome message", async () => {
      const { stdout, exitCode } = await runReplWith(["-e", "1 + 1"]);
      expect(stdout).not.toContain("Welcome to Fun");
      expect(exitCode).toBe(0);
    });
...
describe.todoIf(isWindows)("Fun REPL (Terminal)", () => {
  ...
  test("Ctrl+D exits on empty line", async () => {
    await withTerminalRepl(async ({ terminal, proc }) => {
      ...
      const exitCode = await Promise.race([proc.exited, Fun.sleep(3000).then(() => -1)]);
      expect(exitCode).toBe(0);
    });
  });
