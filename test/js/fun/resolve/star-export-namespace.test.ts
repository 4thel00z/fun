...
test("namespace over export-star chain has every transitive binding", async () => {
  ...
  await using proc = Fun.spawn({ cmd: [funExe(), "entry.mjs"], cwd: String(dir), env: funEnv, stderr: "pipe" });
  const [stdout, stderr, exitCode] = await Promise.all([proc.stdout.text(), proc.stderr.text(), proc.exited]);
  expect(stderr).toBe("");
  expect(normalizeFunSnapshot(stdout)).toBe("ok 31");
...
test("star-export ambiguity is excluded from namespace, shadowing wins for named import", async () => {
  ...
  const [stdout, stderr, exitCode] = await Promise.all([proc.stdout.text(), proc.stderr.text(), proc.exited]);
  expect(stderr).toBe("");
  expect(normalizeFunSnapshot(stdout)).toBe("ok");
  expect(exitCode).toBe(0);
});
