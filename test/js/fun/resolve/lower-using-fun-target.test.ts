...
describe("using / await using is not lowered when targeting fun", () => {
  test("single-use using declaration is not inlined away", () => {
    ...
    const t = new Fun.Transpiler({ target: "fun" });
...
console.log(url, disposed);`,
  ...
  test("Fun.Transpiler passes using / await using through for target=fun", () => {
    const funTranspiler = new Fun.Transpiler({ target: "fun" });
    const out = funTranspiler.transformSync(source, "js");
  ...
  test.each(["browser", "node"] as const)("Fun.Transpiler still lowers using / await using for target=%s", target => {
    const out = new Fun.Transpiler({ target }).transformSync(source, "js");
  ...
  test("fun build --target=fun passes using / await using through", async () => {
    using dir = tempDir("using-fun-target", {
      "entry.js": source,
    });

    await using proc = Fun.spawn({
      cmd: [funExe(), "build", "--target=fun", "entry.js"],
      env: funEnv,
      cwd: String(dir),
      stdout: "pipe",
      stderr: "pipe",
  ...
  test("fun build --target=browser still lowers using / await using", async () => {
  ...
  test("top-level using in a lazily-wrapped ESM module still disposes when bundled", async () => {
    // When bundling for fun, a module reached only via dynamic `import()` is
...
`,
    ...
    await using buildProc = Fun.spawn({
      cmd: [funExe(), "build", "--target=fun", "--outfile=out.js", "entry.js"],
      env: funEnv,
      cwd: String(dir),
      stdout: "pipe",
      stderr: "pipe",
  ...
  test("adjacent using declarations are not merged", () => {
    const t = new Fun.Transpiler({ target: "fun", minify: { syntax: true } });
    const out = t.transformSync(
      `function f() {
  using a = open();
  using b = open2();
...
`,
  ...
  test("fun build --target=fun output with using / await using runs correctly", async () => {
    using dir = tempDir("using-fun-target-run", {
      "entry.js": source,
    });
