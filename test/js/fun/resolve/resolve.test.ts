...
import { funEnv, funExe, funRun, isLinux, isWindows, joinP, tempDir, tempDirWithFiles } from "harness";
...
it("spawn test file", () => {
  ...
  const { exitCode } = Fun.spawnSync({
    cmd: [funExe(), "test", "./resolve-test.js"],
    env: funEnv,
    cwd: import.meta.dir,
...
it("invalid file url in import throws error", async () => {
  ...
  const { exitCode, stdout, stderr } = Fun.spawnSync({
    cmd: [funExe(), `${dir}/test.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });
...
it("import override to fun", async () => {
  // @ts-expect-error
  expect(await import("#fun")).toBeDefined();
});

it.todo("import override to fun:test", async () => {
  // @ts-expect-error
  expect(await import("#fun_test")).toBeDefined();
});
...
describe("NODE_PATH test", () => {
  ...
  it("should resolve modules from NODE_PATH", () => {
    ...
    const { exitCode, stdout } = Fun.spawnSync({
      cmd: [funExe(), "--no-install", "index.js"],
      env: { ...funEnv, NODE_PATH: nodePath },
      cwd: testDir,
    });
  ...
  it("should resolve modules from NODE_PATH entries", () => {
    ...
    const { exitCode, stdout } = Fun.spawnSync({
      cmd: [funExe(), "--no-install", "index.js"],
      env: { ...funEnv, NODE_PATH: [nodePath].join(delimiter) },
      cwd: testDir,
    });
  ...
  it("should resolve first matched module from NODE_PATH entries", () => {
    ...
    const { exitCode, stdout } = Fun.spawnSync({
      cmd: [funExe(), "--no-install", "index.js"],
      env: { ...funEnv, NODE_PATH: ["/a/path/not/exist", nodePathLib, nodePath].join(delimiter) },
      cwd: testDir,
    });
...
describe("When CJS and ESM are mixed", () => {
  ...
  // https://github.com/underdoc-org/fun/issues/4677
  it("loads reflect-metadata before tsyringe", async () => {
    const { stderr } = funRun(fixturePath);
...
it.skipIf(isWindows)("browser map resolution handles relative paths longer than 512 bytes", async () => {
  ...
  await using proc = Fun.spawn({
    cmd: [funExe(), "build", "--target=browser", "entry.js"],
    env: funEnv,
    cwd: String(dir),
    stdout: "pipe",
    stderr: "pipe",
...
// https://github.com/underdoc-org/fun/issues/30187
describe("wildcard exports with @ in matched subpath", () => {
  it.concurrent("resolves a subpath whose wildcard match starts with @", () => {
    ...
    expect(Fun.resolveSync("test-pkg/plain/index.js", root)).toBe(
      join(root, "node_modules/test-pkg/dist/packages/plain/index.js"),
    );
    expect(Fun.resolveSync("test-pkg/@scope/sub/index.js", root)).toBe(
  ...
  it.concurrent("resolves a subpath that contains `@` mid-segment", () => {
    ...
    expect(Fun.resolveSync("test-pkg/with@sign/sub/index.js", root)).toBe(
  ...
  it.concurrent("resolves an @-prefixed subpath under a scoped package", () => {
    ...
    expect(Fun.resolveSync("@my/pkg/@inner/bar/index.js", root)).toBe(
  ...
  it.concurrent("still strips a trailing @version after the package name", () => {
    ...
    expect(Fun.resolveSync("test-pkg@1.0.0/plain/index.js", root)).toBe(
      join(root, "node_modules/test-pkg/dist/packages/plain/index.js"),
    );
  });
  ...
  it.concurrent("still strips @version after a scoped package name", () => {
    ...
    expect(Fun.resolveSync("@my/pkg@1.0.0/sub/index.js", root)).toBe(
...
// as a *DirEntry pointer and dereference it.
{
  ...
  const canUseRunuser = isLinux && isRoot && !!Fun.which("runuser") && nobody !== null;
  ...
  it.skipIf(!canTriggerEACCES)("resolving a directory whose entries cache holds .err does not crash", async () => {
    const fixture = `
      ...
      chmodSync(bad, 0o000);
      let threw = false;
      try { Fun.resolveSync("./bad/index.js", root); } catch { threw = true; }
      ...
      const resolved = Fun.resolveSync("./bad", root);
    ...
    if (canUseRunuser) {
      ...
      cmd = ["runuser", "-u", "nobody", "--", funExe(), join(root, "fixture.js"), root];
    } else {
      cmd = [funExe(), join(root, "fixture.js"), root];
    }
...
describe("resolving external URL specifiers with non-ASCII characters", () => {
  ...
  it.each([
    ...
    ["https://example/´"],
    ["//example/´?q"],
  ])("Fun.resolveSync(%j)", specifier => {
    expect(Fun.resolveSync(specifier, import.meta.dir)).toBe(specifier);
  });
