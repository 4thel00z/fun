...
// fun.default_allocator (file contents, JSON property arrays) that dwarf the
// TSConfigJSON struct itself, so RSS can't isolate this leak. Instead we use
// the debug-build `FUN_DEBUG_alloc=1` instrumentation which logs every
// fun.new()/fun.destroy() call, and count TSConfigJSON lifetimes directly.
...
// (debug/ReleaseSafe). Release CI lanes skip this; the gate runs under fun bd
...
test.skipIf(!isDebug)("tsconfig 'extends' chain frees every intermediate TSConfigJSON", async () => {
  ...
  await using proc = Fun.spawn({
    cmd: [funExe(), "-e", `Fun.resolveSync("./index.ts", ${JSON.stringify(resolveFrom)});`],
    env: {
      ...funEnv,
      // Enable the `.alloc` scoped logger (emits "[alloc] new(T) = ..." /
      // "[alloc] destroy(T) = ..." for every fun.new/fun.destroy). This
      // overrides FUN_DEBUG_QUIET_LOGS for the `.alloc` scope specifically.
      FUN_DEBUG_alloc: "1",
    },
