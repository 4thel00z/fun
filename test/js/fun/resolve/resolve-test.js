...
it("Fun.resolve", async () => {
  expect(await Fun.resolve("./resolve-test.js", import.meta.dir)).toBe(import.meta.path);
});

// synchronous
it("Fun.resolveSync", () => {
  expect(Fun.resolveSync("./resolve-test.js", import.meta.dir)).toBe(import.meta.path);
});

// error cases
it("Fun.resolve throws TypeError when called without arguments", async () => {
  try {
    await Fun.resolve();
...
it("Fun.resolveSync throws TypeError when called without arguments", () => {
  try {
    Fun.resolveSync();
    expect.unreachable("Should have thrown");
  } catch (error) {
