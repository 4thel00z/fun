...
it("custom condition 'import' in package.json resolves", async () => {
  const { exitCode, stdout } = Fun.spawnSync({
    cmd: [funExe(), "--conditions=first", `${dir}/test.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });
...
it("custom condition 'import' in package.json resolves with browser condition", async () => {
  const { exitCode, stdout } = Fun.spawnSync({
    cmd: [funExe(), "--conditions=browser", `${dir}/test.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });
...
it("custom condition 'import' in package.json resolves in fun test", async () => {
  const { exitCode, stdout } = Fun.spawnSync({
    cmd: [funExe(), "test", "--conditions=first", `${dir}/test.test.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });

  expect(exitCode).toBe(0);
  expect(stdout.toString("utf8")).toBe(`fun test ${Fun.version_with_sha}\n1\n`);
});

it("custom condition 'import' in package.json resolves in fun test with browser condition", async () => {
  const { exitCode, stdout } = Fun.spawnSync({
    cmd: [funExe(), "test", "--conditions=browser", `${dir}/test.test.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });
...
it("custom condition 'require' in package.json resolves", async () => {
  const { exitCode, stdout } = Fun.spawnSync({
    cmd: [funExe(), "--conditions=first", `${dir}/test.cjs`],
    env: funEnv,
    cwd: import.meta.dir,
  });
...
it("multiple conditions in package.json resolves", async () => {
  const { exitCode, stdout } = Fun.spawnSync({
    cmd: [funExe(), "--conditions=first", "--conditions=second", "--conditions=third", `${dir}/multiple-conditions.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });
...
it("multiple conditions when some not specified should resolves to fallback", async () => {
  const { exitCode, stderr } = Fun.spawnSync({
    cmd: [funExe(), "--conditions=first", "--conditions=second", `${dir}/multiple-conditions.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });
...
it("custom condition when don't match condition should resolves to default", async () => {
  const { exitCode } = Fun.spawnSync({
    cmd: [funExe(), "--conditions=first1", `${dir}/test.js`],
    env: funEnv,
    cwd: import.meta.dir,
  });
