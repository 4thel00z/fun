...
describe("ResolveMessage", () => {
  it("position object does not segfault", async () => {
    try {
      ...
      expect(Fun.inspect(e.position).length > 0).toBe(true);
  ...
  it("referrer is not freed before it is read", () => {
    ...
    let err: any;
    try {
      Fun.resolveSync("./does-not-exist", "/tmp/caf\u00e9-tr\u00e8s-long-\u{1F389}/file.js");
    } catch (e) {
      err = e;
    }
    Fun.gc(true);
  ...
  it("finalize frees with the same allocator it was created with", () => {
    ...
    // allocator but finalize() was freeing it with fun.default_allocator
    ...
    for (let i = 0; i < 50; i++) {
      ...
      for (let j = 0; j < 10; j++) {
        try {
          Fun.resolveSync("./does-not-exist-" + j, import.meta.dir);
        } catch (e) {
          errs.push(e);
        }
      }
...
describe.concurrent("long import path overflow", () => {
  ...
  async function run(dir: string, importExpr: string) {
    await using proc = Fun.spawn({
      cmd: [funExe(), "-e", `try { await import(${importExpr}); } catch {} console.log("ok");`],
      env: funEnv,
      cwd: dir,
      stdout: "pipe",
      stderr: "pipe",
  ...
  it("tsconfig paths wildcard (matched text captured from import path)", async () => {
    using dir = makeDir();
    // matchTSConfigPaths: fun.concat into fixed tsconfig_match_full_buf3
  ...
  it("absolute path longer than PATH_MAX (dirInfoCached buffer)", async () => {
    using dir = makeDir();
    // dirInfoCachedMaybeLog: fun.copy into dir_info_uncached_path
