...
import { isModuleResolveFilenameSlowPathEnabled } from "fun:internal-for-testing";
import { expect, it, mock } from "fun:test";
...
it("import.meta.main", () => {
  const { exitCode } = spawnSync({
    cmd: [funExe(), "run", join(import.meta.dir, "./main-test-script.js")],
    env: funEnv,
    stderr: "inherit",
    stdout: "inherit",
    stdin: null,
...
it("require with a query string works on dynamically created content", () => {
  rmSync(tmpbase + "fun-test-import-meta-dynamic-dir", {
    recursive: true,
    force: true,
  });
  try {
    const require = Module.createRequire(tmpbase + "fun-test-import-meta-dynamic-dir/foo.js");
    try {
    ...
    mkdirSync(tmpbase + "fun-test-import-meta-dynamic-dir", { recursive: true });

    writeFileSync(tmpbase + "fun-test-import-meta-dynamic-dir/bar.js", "export default 'hello';", "utf8");
...
it("Module.createRequire().resolve", () => {
  const expected = Fun.resolveSync("./require-json.json", import.meta.dir);
...
// this isn't used in fun but exists anyway
...
it("import.meta.dir", () => {
  expect(dir).toEndWith(ospath("/test/js/fun/resolve"));
});
...
it("import.meta.path", () => {
  expect(path).toEndWith(ospath("/test/js/fun/resolve/import-meta.test.js"));
});

it('require("fun") works', () => {
  expect(require("fun")).toBe(Fun);
});

it('import("fun") works', async () => {
  expect(await import("fun")).toBe(Fun);
});
...
it("dynamically import fun", async () => {
  expect((await import(eval("'fun'"))).default).toBe(Fun);
});
