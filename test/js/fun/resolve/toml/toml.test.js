...
`;
  ...
  // Test via Fun's internal TOML parser
  const Fun = globalThis.Fun;
  const parsed = Fun.TOML.parse(tomlContent);
...
it("Fun.TOML.parse throws on deeply nested inline tables instead of crashing", () => {
  const depth = 25_000;
  const deepToml = "a = " + "{ b = ".repeat(depth) + "1" + " }".repeat(depth);
  expect(() => Fun.TOML.parse(deepToml)).toThrow(RangeError);
});
