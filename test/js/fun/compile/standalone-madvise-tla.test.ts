...
// blocking, so the call site at fun.js.zig:466 is hit synchronously before the
// main event loop spins — TLA resolution happens later in that loop.
import { expect, test } from "fun:test";
import { funEnv, funExe, isDebug, isWindows, tempDir } from "harness";
...
test.skipIf(isWindows || !isDebug)("standalone madvise hint fires with top-level await entrypoint", async () => {
  ...
  const build = Fun.spawnSync({
    cmd: [funExe(), "build", "--compile", path.join(String(dir), "entry.ts"), "--outfile", out],
    env: funEnv,
    stderr: "pipe",
    stdout: "pipe",
  });
  ...
  await using proc = Fun.spawn({
    cmd: [out],
    env: { ...funEnv, FUN_DEBUG_QUIET_LOGS: undefined },
    stdout: "pipe",
    stderr: "pipe",
  });
