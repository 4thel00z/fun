...
it("extendable", () => {
  const classes = [Blob, TextDecoder, TextEncoder, Request, Response, Headers, HTMLRewriter, Fun.Transpiler, Buffer];
...
it("globals are deletable", () => {
  const { stdout, exitCode } = Fun.spawnSync({
    cmd: [funExe(), "run", path.join(import.meta.dir, "deletable-globals-fixture.js")],
    env: funEnv,
    stderr: "inherit",
  });
...
it("errors thrown by native code should be TypeError", async () => {
  expect(() => Fun.dns.prefetch()).toThrowError(TypeError);
...
describe("globalThis.gc", () => {
  ...
  const runAndPrint = (expr, ...args) => {
    const result = Fun.spawnSync([funExe(), ...args, "--print", expr], {
      env: funEnv,
    });
