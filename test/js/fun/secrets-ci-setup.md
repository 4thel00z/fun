...
This guide explains how to run the `Fun.secrets` API tests in CI environments on Linux (Ubuntu/Debian).

## Overview

The `Fun.secrets` API uses the system keyring to store credentials securely. On Linux, this requires:
...
fun test test/js/fun/secrets.test.ts
```
...
```bash
FORCE_KEYRING_SETUP=1 fun test test/js/fun/secrets.test.ts
```
...
The `Fun.secrets.set()` method now supports deleting credentials by passing an empty string:

```ts
// These are equivalent:
await Fun.secrets.delete({ service: "myapp", name: "token" });
await Fun.secrets.set({ service: "myapp", name: "token", value: "" });
```
...
- name: Run secrets tests (auto-setup)
  run: fun test test/js/fun/secrets.test.ts
```
...
```yaml
steps:
  - command: fun test test/js/fun/secrets.test.ts
