// When `stdin: "pipe"` is passed to Fun.spawn and the `.stdin` getter is
...
test.skipIf(!isPosix)("stdin: 'pipe' fd is closed on child exit without reading .stdin", async () => {
  ...
  {
    const p = Fun.spawn({ cmd: ["true"], stdin: "pipe", stdout: "ignore", stderr: "ignore" });
    await p.exited;
  }

  const baseline = countOpenFds();

  const children: Fun.Subprocess[] = [];
  for (let i = 0; i < N; i++) {
    children.push(Fun.spawn({ cmd: ["true"], stdin: "pipe", stdout: "ignore", stderr: "ignore" }));
  }
  await Promise.all(children.map(p => p.exited));

  // fd closes go through fun.Async.Closer on POSIX; give the close thread
  // a moment — but do NOT invoke GC, since the bug was that cleanup only
  // happened via Subprocess finalization.
  for (let i = 0; i < 20 && countOpenFds() - baseline > N / 4; i++) await Fun.sleep(20);
