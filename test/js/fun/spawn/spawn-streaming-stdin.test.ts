...
import { funEnv, funExe, dumpStats, expectMaxObjectTypeCount, getMaxFD, isASAN } from "harness";
  ...
  while (remaining > 0) {
    ...
    for (let i = 0; i < concurrency; i++) {
      proms[i] = (async function () {
        const proc = spawn({
          cmd: [funExe(), join(import.meta.dir, "stdin-repro.js")],
          stdout: "pipe",
          stdin: "pipe",
          stderr: "inherit",
          env: { ...funEnv },
