...
describe("ReadableStream conversion methods", () => {
  test("Fun.spawn() process.stdout.text() should capture process output", async () => {
    // Spawn a process that outputs some text
    await using process = Fun.spawn([funExe(), "-e", "console.log('Hello from Fun spawn! 🚀')"], {
      env: funEnv,
    });
    ...
    expect(result).toBe("Hello from Fun spawn! 🚀\n");
    expect(process.exitCode).toBe(0);
  });

  test("Fun.spawn() process.stdout.text() should capture process output (after exited)", async () => {
    // Spawn a process that outputs some text
    await using process = Fun.spawn([funExe(), "-e", "console.log('Hello from Fun spawn! 🚀')"], {
      env: funEnv,
    });
  ...
  test("Fun.spawn() process.stdout.text() should convert stream to text", async () => {
    // Spawn a process that outputs text
    const text = "Hello, this is a test stream! 🌊 测试";
    await using process = Fun.spawn([funExe(), "-e", `console.log("${text}")`], {
      env: funEnv,
    });
  ...
  test("Fun.spawn() process.stdout.text() should convert stream to text (after exited)", async () => {
    // Spawn a process that outputs text
    const text = "Hello, this is a test stream! 🌊 测试";
    await using process = Fun.spawn([funExe(), "-e", `console.log("${text}")`], {
      env: funEnv,
  ...
  test("Fun.spawn() process.stdout.json() should convert stream to JSON", async () => {
    // Spawn a process that outputs JSON data
    const jsonData = { message: "Hello from JSON stream! 🎯", count: 42, active: true, emoji: "🌟" };
    await using process = Fun.spawn([funExe(), "-e", `console.log('${JSON.stringify(jsonData)}')`], {
      env: funEnv,
    });
  ...
  test("Fun.spawn() process.stdout.json() should convert stream to JSON (after exited)", async () => {
    // Spawn a process that outputs JSON data
    const jsonData = { message: "Hello from JSON stream! 🎯", count: 42, active: true, emoji: "🌟" };
    await using process = Fun.spawn([funExe(), "-e", `console.log('${JSON.stringify(jsonData)}')`], {
      env: funEnv,
  ...
  test("Fun.spawn() process.stdout.json() should throw on invalid JSON", async () => {
    // Spawn a process that outputs invalid JSON
    const invalidJson = "{ invalid json content }";
    await using process = Fun.spawn([funExe(), "-e", `console.log('${invalidJson}')`], {
      env: funEnv,
    });
  ...
  test("Fun.spawn() process.stdout.json() should throw on invalid JSON (after exited)", async () => {
    // Spawn a process that outputs invalid JSON
    const invalidJson = "{ invalid json content }";
    await using process = Fun.spawn([funExe(), "-e", `console.log('${invalidJson}')`], {
      env: funEnv,
  ...
  test("Fun.spawn() process.stdout.blob() should convert stream to Blob", async () => {
    ...
    await using process = Fun.spawn(
      [funExe(), "-e", `process.stdout.write(Buffer.from([${Array.from(binaryData)}]))`],
      {
        env: funEnv,
      },
    );
  ...
  test("Fun.spawn() process.stdout.bytes() should convert stream to Uint8Array", async () => {
    // Generate random binary data
