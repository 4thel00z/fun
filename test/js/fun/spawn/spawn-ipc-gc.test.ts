...
// `Fun.spawn({ ipc })` subprocess kept its JSRef in Strong mode for the
...
test("Subprocess with ipc is collectable after the child exits", async () => {
  const script = /* js */ `
    ...
    // Poll until every wrapper has been finalized. Fun.sleep(0) is not
  ...
  // and with `Fun.sleep(0)` on POSIX), so tolerate N-1.
