...
import { funEnv, funExe, isLinux, isWindows, libcPathForDlopen, tempDir } from "harness";
...
test.skipIf(!isLinux)(
  ...
  async () => {
    using dir = tempDir("spawn-pipe-read-error-leak", {
      "fixture.js": `
const { dlopen, FFIType } = require("fun:ffi");
...
async function once() {
  ...
  // The fd close goes through fun.Async.Closer which schedules close() on
  // a jsc.WorkPool thread; Fun.sleep(0) doesn't synchronize with that. Poll
...
`,
    ...
    await using proc = Fun.spawn({
      ...
      env: { ...funEnv, LIBC_PATH: libcPathForDlopen() },
      cwd: String(dir),
      stdout: "pipe",
      stderr: "pipe",
    });
...
test("PipeReader is freed when a subprocess stdout read fails (injected)", async () => {
  using dir = tempDir("spawn-pipe-read-error-leak-inject", {
    "fixture.js": `
const { subprocessInternals } = require("fun:internal-for-testing");
