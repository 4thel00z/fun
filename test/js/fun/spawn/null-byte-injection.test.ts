...
describe("null byte injection protection", () => {
  describe("Fun.spawn", () => {
    ...
    test("works normally with valid arguments", async () => {
      await using proc = Fun.spawn(["echo", "hello"], { stdout: "pipe" });
    ...
    test("works with spread process.env", async () => {
      await using proc = Fun.spawn(["echo", "hello"], {
        env: { ...process.env },
        stdout: "pipe",
      });
      const stdout = await new Response(proc.stdout).text();
