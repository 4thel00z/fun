...
//
// The drop happens when a poll callback re-enters `us_loop_run_fun_tick`
    ...
    for (let i = 0; i < N; i++) {
      ...
      Fun.spawn({
        ...
        onExit() {
          ...
          // main uws loop. Fun.sleep(1) resolves via the timer queue, which
          // only drains inside autoTick() AFTER us_loop_run_fun_tick — so
          ...
          if (!nested) {
            nested = true;
            expect(Fun.sleep(1)).resolves.toBe(undefined);
          }
          resolve();
        },
      });
