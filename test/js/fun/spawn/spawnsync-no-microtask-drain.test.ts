...
// Regression test: microtasks must not drain during Fun.spawnSync.
//
...
inSync = true
Fun.spawnSync({ cmd: [process.execPath, '-e', 'Fun.sleepSync(10)'], maxBuffer: 1048576 })
inSync = false
...
test("spawnSync with timeout still works", () => {
  const result = Fun.spawnSync({
    cmd: [funExe(), "-e", "Fun.sleepSync(10000)"],
    timeout: 100,
  });
