...
import { funEnv, funExe, gcTick } from "harness";
import path from "path";

describe.each(["advanced", "json"])("ipc mode %s", mode => {
  it("the subprocess should be defined and the child should send", done => {
    gcTick();
    const returned_subprocess = spawn([funExe(), path.join(__dirname, "fun-ipc-child.js")], {
  ...
  it("the subprocess should receive the parent message and respond back", done => {
    ...
    const childProc = spawn([funExe(), path.join(__dirname, "fun-ipc-child-respond.js")], {
      ipc: (message, subProcess) => {
  ...
  it("ipc works when preceded by a non-pipe extra stdio slot", async () => {
    ...
    await using child = spawn([funExe(), path.join(__dirname, "fun-ipc-child.js")], {
      env: funEnv,
