...
describe.each([true, false])("Fun.deepEquals(a, b, strict: %p)", strict => {
  const deepEquals = (a: unknown, b: unknown) => Fun.deepEquals(a, b, strict);
  it.each([
    [1, 1],
    [true, true],
    [undefined, undefined],
    ...
    [NaN, NaN],
  ])("Fun.deepEquals(%p, %p) === true, regardless of strict modee", (a, b) => {
    expect(Fun.deepEquals(a, b, true)).toBe(true);
    expect(Fun.deepEquals(a, b, false)).toBe(true);
  });
  ...
  it.each([
    ...
    expect(Fun.deepEquals(a, b, true)).toBe(false);
    expect(Fun.deepEquals(a, b, false)).toBe(false);
  });
  ...
  describe("global object", () => {
    ...
    // TODO: re-enable when https://github.com/underdoc-org/fun/issues/17080 is resolved
    it.skip("main global object is not equal to vm global objects", () => {
      const [ctx] = contexts;
      expect(deepEquals(global, ctx)).toBe(false);

      ctx.mainGlobal = global;
      const areEqual = vm.runInContext("Fun.deepEquals(globalThis, mainGlobal)", ctx);
