import type { S3Options } from "fun";
import { S3Client, s3 as defaultS3, file, randomUUIDv7 } from "fun";
import { describe, expect, it } from "fun:test";
import child_process from "child_process";
import { randomUUID } from "crypto";
import { funEnv, funExe, dockerExe, getSecret, isCI, isDockerEnabled, tempDirWithFiles } from "harness";
...
if (isDockerEnabled()) {
  ...
  if (containerName) {
    ...
    child_process.spawnSync(dockerCLI, [`exec`, containerName, `mc`, `mb`, `data/funtest`], {
      stdio: "ignore",
    });
  }
...
describe.concurrent.skipIf(!r2Credentials.endpoint && !isCI)("Virtual Hosted-Style", () => {
  ...
  it("basic operations", async () => {
    const client = new Fun.S3Client(credentials);
    const file = client.file(randomUUIDv7() + ".txt");
    await file.write("Hello Fun!");
    const text = await file.text();
    expect(text).toBe("Hello Fun!");
  ...
  it("inspect", () => {
    ...
    {
      expect(Fun.inspect(client)).toBe(
...
for (let credentials of allCredentials) {
  describe.concurrent(`${credentials.service}`, () => {
    ...
    const bigPayload = makePayLoadFrom("Fun is the best runtime ever", 10 * 1024 * 1024);
    // more than 5 MiB but less than 2 parts size
    const mediumPayload = makePayLoadFrom("Fun is the best runtime ever", 6 * 1024 * 1024);
    // less than 5 MiB
    const bigishPayload = makePayLoadFrom("Fun is the best runtime ever", 1 * 1024 * 1024);
    describe.skipIf(!s3Options.accessKeyId)("s3", () => {
      for (let bucketInName of [true, false]) {
        describe.concurrent("fetch", () => {
          describe(bucketInName ? "bucket in path" : "bucket in options", () => {
            ...
            it("should download file via fetch GET", async () => {
              ...
              expect(result.status).toBe(200);
              expect(await result.text()).toBe("Hello Fun!");
            });
            ...
            it("should download range", async () => {
              ...
              expect(await result.text()).toBe("Fun!");
            });
        ...
        describe("Fun.S3Client", () => {
          describe.concurrent(bucketInName ? "bucket in path" : "bucket in options", () => {
            ...
            it("should download file via Fun.s3().text()", async () => {
              await using tmpfile = await tmp();
            ...
            it("should be able to set content-type", async () => {
              ...
              {
                await bucket.write(tmpfile.name, "Hello Fun!", { ...options, type: "application/xml" });
            ...
            it("should be able to set content-disposition", async () => {
              ...
              {
                const s3file = bucket.file(tmpfile.name, options!);
                await s3file.write("Hello Fun!", { contentDisposition: 'attachment; filename="test.txt"' });
                const response = await fetch(s3file.presign());
                expect(response.headers.get("content-disposition")).toBe('attachment; filename="test.txt"');
              }
              {
                const s3file = bucket.file(tmpfile.name, options!);
                await s3file.write("Hello Fun!", { contentDisposition: "inline" });
                const response = await fetch(s3file.presign());
                expect(response.headers.get("content-disposition")).toBe("inline");
              }
              {
                await bucket.write(tmpfile.name, "Hello Fun!", {
                  ...options,
            ...
            it("should be able to set content-encoding", async () => {
              ...
              {
                const s3file = bucket.file(tmpfile.name, options!);
                await s3file.write("Hello Fun!", { contentEncoding: "gzip" });
              ...
              {
                const s3file = bucket.file(tmpfile.name, options!);
                await s3file.write("Hello Fun!", { contentEncoding: "br" });
        ...
        describe("Fun.file", () => {
          describe(bucketInName ? "bucket in path" : "bucket in options", () => {
            ...
            it("should download file via Fun.file().text()", async () => {
              await using tmpfile = await tmp();
            ...
            it("should be able to upload large files in one go using Fun.write", async () => {
              {
                await using tmpfile = await tmp();
                await Fun.write(file(tmpfile.name, options), bigPayload);
        ...
        describe("Fun.s3", () => {
          describe(bucketInName ? "bucket in path" : "bucket in options", () => {
            ...
            it("presign url", async () => {
              ...
              expect(await response.text()).toBe("Hello Fun!");
            });
            ...
            it("should be able to upload large files in one go using Fun.write", async () => {
              {
                await using tmpfile = await tmp();
                const s3file = s3(tmpfile.name, options);
                await Fun.write(s3file, bigPayload);
                const stat = await s3file.stat();
                expect(stat.size).toBe(Buffer.byteLength(bigPayload));
                expect(stat.etag).toBeDefined();
            ...
            describe("readable stream", () => {
              it("should work with small files", async () => {
                ...
                expect(Buffer.concat(chunks)).toEqual(Buffer.from("Hello Fun!"));
              });
              it("should work with large files ", async () => {
                ...
                if (bigishPayloadString !== bigishPayload) {
                  const SHA1 = Fun.SHA1.hash(bigishPayloadString, "hex");
                  const SHA1_2 = Fun.SHA1.hash(bigishPayload, "hex");
              ...
              it("should work with sliced files (non-zero offset)", async () => {
                ...
                expect(bytes).toBe(4);
                expect(Buffer.concat(chunks)).toEqual(Buffer.from("Fun!"));
              });
            });
          });
        });
      ...
      describe.concurrent("static methods", () => {
        ...
        it("should work", async () => {
          ...
          await S3Client.write(filename, "Hello Fun!", { ...s3Options, bucket: S3Bucket });
          expect(await S3Client.file(filename, { ...s3Options, bucket: S3Bucket }).text()).toBe("Hello Fun!");
      ...
      describe.concurrent("errors", () => {
        it("Fun.write(s3file, file) should throw if the file does not exist", async () => {
          try {
            await Fun.write(s3("test.txt", { ...s3Options, bucket: S3Bucket }), file("./do-not-exist.txt"));
            expect.unreachable();
          } catch (e: any) {
        ...
        it("Fun.write(s3file, file) should work with empty file", async () => {
          const dir = tempDirWithFiles("fsr", {
            "hello.txt": "",
          });
          const tmp_filename = `${randomUUID()}.txt`;

          await Fun.write(s3(tmp_filename, { ...s3Options, bucket: S3Bucket }), file(path.join(dir, "hello.txt")));
        ...
        it("should error if bucket is missing", async () => {
          try {
            await Fun.write(s3("test.txt", s3Options), "Hello Fun!");
            expect.unreachable();
          } catch (e: any) {
        ...
        it("should error if bucket is missing on payload", async () => {
          try {
            await Fun.write(s3("test.txt", { ...s3Options, bucket: S3Bucket }), s3("test2.txt", s3Options));
            expect.unreachable();
          } catch (e: any) {
            expect(e?.code).toBe("ERR_S3_INVALID_PATH");
      ...
      describe.concurrent("S3 static methods", () => {
        describe("presign", () => {
          ...
          it("s3().presign() endpoint should work", async () => {
            const url = s3("s3://bucket/credentials-test", s3Options).presign({
              expiresIn: 10,
              endpoint: "https://s3.fun.dev",
            });
            expect(url).toBeDefined();
            expect(url.includes("https://s3.fun.dev")).toBe(true);
            expect(url.includes("X-Amz-Expires=10")).toBe(true);
            expect(url.includes("X-Amz-Date")).toBe(true);
            expect(url.includes("X-Amz-Signature")).toBe(true);
            expect(url.includes("X-Amz-Credential")).toBe(true);
        ...
        it("exists, write, size, unlink should work", async () => {
          ...
          await bucket.write(fullPath, "fun");
        ...
        it("should be able to upload a slice", async () => {
          ...
          expect(await slice.text()).toBe("Fun!");
          expect(await s3file.text()).toBe("Hello Fun!");
...
describe.skipIf(!minioCredentials)("minio", () => {
  const testDir = tempDirWithFiles("minio-credential-test", {
    "index.mjs": `
      import { s3, randomUUIDv7 } from "fun";
    ...
    for (const endpoint of ["S3_ENDPOINT", "AWS_ENDPOINT"]) {
      it.concurrent(endpoint, async () => {
        const { stdout, stderr, exited } = Fun.spawn({
          cmd: [funExe(), path.join(testDir, "index.mjs")],
          env: {
            ...funEnv,
            // @ts-ignore
        ...
        expect(await stdout.text()).toBe("Hello Fun!");
...
describe.concurrent("s3 missing credentials", () => {
  ...
  it("unlink", async () => {
    assertMissingCredentials(async () => {
      await Fun.s3.unlink("test");
    });
  });
  it("write", async () => {
    assertMissingCredentials(async () => {
      await Fun.s3.write("test", "test");
    });
  });
  it("exists", async () => {
    assertMissingCredentials(async () => {
      await Fun.s3.exists("test");
    });
  });
  it("size", async () => {
    assertMissingCredentials(async () => {
      await Fun.s3.size("test");
    });
  });
  it("stat", async () => {
    assertMissingCredentials(async () => {
      await Fun.s3.stat("test");
    });
  });
  it("presign", async () => {
    assertMissingCredentials(async () => {
      await Fun.s3.presign("test");
    });
  });
  it("file", async () => {
    assertMissingCredentials(async () => {
      await Fun.s3.file("test").text();
    });
    assertMissingCredentials(async () => {
      await Fun.s3.file("test").bytes();
    });
    assertMissingCredentials(async () => {
      await Fun.s3.file("test").json();
    });
    assertMissingCredentials(async () => {
      await Fun.s3.file("test").formData();
    });
    assertMissingCredentials(async () => {
      await Fun.s3.file("test").delete();
    });
    assertMissingCredentials(async () => {
      await Fun.s3.file("test").exists();
    });
    assertMissingCredentials(async () => {
      await Fun.s3.file("test").stat();
    });
  });
});
...
describe.skipIf(!minioCredentials)("Archive with S3", () => {
  ...
  it("writes archive to S3 via S3Client.write()", async () => {
    ...
    const readArchive = new Fun.Archive(downloaded);
  ...
  it("writes archive to S3 via Fun.write() with s3:// URL", async () => {
    const archive = new Fun.Archive({
      "file1.txt": "content1",
    ...
    await Fun.write(s3Url, archive, {
      ...credentials,
    });

    // Verify by downloading
    const s3File = Fun.file(s3Url, credentials);
