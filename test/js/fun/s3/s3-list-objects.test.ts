import { randomUUIDv7, S3Client, S3Options } from "fun";
...
function createFunServer(fetch: Parameters<typeof Fun.serve>[0]["fetch"]) {
  // @ts-ignore
...
describe.concurrent("S3 - List Objects", () => {
  it("Should set encoded continuation-token in the request url before list-type", async () => {
    let reqUrl: string;
    using server = createFunServer(async req => {
      reqUrl = req.url;
      return new Response(`<>`, {
        headers: {
  ...
  it("Should work without provided option", async () => {
    using server = createFunServer(async => {
      return new Response(
  ...
  it("Should return parsed response with Contents", async () => {
    using server = createFunServer(async => {
      return new Response(
        `<?xml version="1.0" encoding="UTF-8"?><ListBucketResult>
    <Contents>
        <Key>my_files/important/fun.js</Key>
