...
import { afterAll, afterEach, beforeAll, beforeEach, describe, expect, it, test } from "fun:test";
...
`;
  ...
  try {
    ...
    expect(await stdout.text()).toBe(`fun test ${Fun.version_with_sha}\n`);
...
test("testing Fun.deepEquals() using isEqual()", () => {
...
`;
  ...
  const { stdout, stderr, exitCode } = spawnSync([funExe(), "test", "test-i-have-been-reached.test.js"], {
    cwd: dir,
    env: funEnv,
    stdout: "pipe",
    stderr: "pipe",
...
test("test async exceptions fail tests", () => {
  ...
  const { stderr, exitCode } = spawnSync([funExe(), "test", "test-throwing-eventemitter"], {
    cwd: dir,
    env: funEnv,
  });
...
it("test --preload supports global lifecycle hooks", () => {
  ...
  const { stdout } = spawnSync({
    cmd: [funExe(), "test", "--preload=" + preloadedPath, path],
    stdout: "pipe",
    stderr: "pipe",
    env: funEnv,
    cwd: dirname(path),
...
describe("unhandled errors between tests are reported", () => {
  ...
  for (const stage of stages) {
    test("in " + stage, () => {
      const code = /*js*/ `
import {test, beforeAll, expect, beforeEach, afterEach, afterAll, describe} from "fun:test";
  ...
  expect(1).toBe(1);
    ...
    `.trim();
      ...
      const { stderr, exited } = spawnSync({
        cmd: [funExe(), "test", "my-test.test.js"],
        cwd: test_dir,
        stdout: "inherit",
        stderr: "pipe",
        env: funEnv,
