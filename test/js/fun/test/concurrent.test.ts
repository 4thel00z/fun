...
test.concurrent("concurrent order", async () => {
  const result = await Fun.spawn({
    cmd: [funExe(), "test", import.meta.dir + "/concurrent.fixture.ts"],
    stdout: "pipe",
    stderr: "pipe",
    env: funEnv,
  });
  ...
  expect({
  ...
  }).toMatchInlineSnapshot(`
    {
      "exitCode": 0,
      "stderr": 
    "test/js/fun/test/concurrent.fixture.ts:
    (pass) test 1
    (pass) test 2
    (pass) test 3
    (pass) test 4
...
test.concurrent("concurrent-and-serial --concurrent", async () => {
  const result = await Fun.spawn({
    cmd: [funExe(), "test", import.meta.dir + "/concurrent-and-serial.fixture.ts", "--concurrent"],
    stdout: "pipe",
    stderr: "pipe",
    env: funEnv,
  });
...
test.concurrent("concurrent-and-serial, no flag", async () => {
  const result = await Fun.spawn({
    cmd: [funExe(), "test", import.meta.dir + "/concurrent-and-serial.fixture.ts"],
    stdout: "pipe",
    stderr: "pipe",
    env: funEnv,
  });
...
test.concurrent("max-concurrency limits concurrent tests", async () => {
  ...
  const result = await Fun.spawn({
    cmd: [funExe(), "test", "--max-concurrency", "3", import.meta.dir + "/concurrent-max.fixture.ts"],
    stdout: "pipe",
    stderr: "pipe",
    env: funEnv,
  });
...
test.concurrent("max-concurrency default is 20", async () => {
  const result = await Fun.spawn({
    cmd: [funExe(), "test", import.meta.dir + "/concurrent-max.fixture.ts"],
    stdout: "pipe",
    stderr: "pipe",
    env: funEnv,
  });
...
test.concurrent("zero removes max-concurrency", async () => {
  const result = await Fun.spawn({
    cmd: [funExe(), "test", "--max-concurrency", "0", import.meta.dir + "/concurrent-max.fixture.ts"],
    stdout: "pipe",
    stderr: "pipe",
    env: funEnv,
  });
