...
test("verify we print error messages passed to done callbacks", () => {
  const { stdout, stderr } = Fun.spawnSync({
    cmd: [funExe(), "test", path.resolve(import.meta.dir, "test-error-done-callback-fixture.ts")],
  ...
  let stdoutStr = stdout
    ...
    .replaceAll(Fun.version_with_sha, "<version>")
    .replaceAll("[<time>s]", "")
    .replaceAll("[<time>ms]", "")
    .split("\n")
    .map(line => line.trim())
  ...
  expect(stderrStr).toMatchInlineSnapshot(`
    "
    test/js/fun/test/test-error-done-callback-fixture.ts:
    22 |   105,
    23 |   115,
    24 | );
    25 |
    ...
    33 | });
    34 |
    35 | test("error done callback (async with Fun.sleep)", async done => {
    36 |   await Fun.sleep(0);
    37 |   done(new Error(msg + "(async with Fun.sleep)"));
    ^
    error: you should see this(async with Fun.sleep)
    at <anonymous> (<dir>/test-error-done-callback-fixture.ts:37:12)
    (fail) error done callback (async with Fun.sleep)
    37 |   done(new Error(msg + "(async with Fun.sleep)"));
    38 | });
    39 |
