...
This document tracks tests that could not be accurately translated from @sinonjs/fake-timers to Fun's `vi` fake timers API due to missing features or API differences.

## Tests Marked as TODO (Missing Features)

### issue-73.test.ts
- **Test**: "should install with date object"
- **Issue**: Fun's fake timers don't support setting initial time via the `now` option
...
- **Reason**: Fun always has `setImmediate` available
- **Note**: This is a platform-specific test that may not be relevant to Fun
