...
/** This file is meant to be runnable in Jest, Vitest, and Fun:
 *  `fun test test/js/fun/test/expect.test.js`
 *  `funx vitest test/js/fun/test/expect.test.js`
 *  `NODE_OPTIONS=--experimental-vm-modules npx jest test/js/fun/test/expect.test.js`
 */

// import these functions typed with the fun:test types,
// so this test can also be used to detect issues with the "fun:test" type definitions
import test_interop from "./test-interop.js";
var { isFun, test, it, describe, expect, jest, vi, mock, funTest, spyOn } = await test_interop();
const isJest = !vi;
const isVitest = vi && !isFun;
...
describe("expect()", () => {
  ...
  describe("BigInt", () => {
    ...
    it("compares correctly (object)", () => {
      ...
      if (isFun) expect(BigInt(Fun.inspect(42n).substring(0, 2))).toBe(BigInt(42n));
  ...
  test("deepEquals Set/Map stress test", () => {
    ...
    for (let i = 0; i < 2000; i++) {
      ...
      Fun.deepEquals(outerSet, innerSet);
    }

    for (let i = 0; i < 1000; i++) {
      let outerMap = new Map(arr3);
      let innerMap = new Map(arr4);
      Fun.deepEquals(outerMap, innerMap);
    }
  });
  ...
  if (isFun) {
    test("toHaveLength() extended", () => {
      ...
      // files
      const thisFile = Fun.file(__filename);
      const thisFileSize = thisFile.size;

      expect(thisFile).toHaveLength(thisFileSize);
      expect(thisFile).toHaveLength(Fun.file(__filename).size);

      // empty file should have length 0
      expect(Fun.file(tmpFile(true))).toHaveLength(0);

      // if a file doesn't exist, it should throw (not return 0 size)
      expect(() => expect(Fun.file(tmpFile(false))).toHaveLength(0)).toThrow();
  ...
  test("toContainKey", () => {
    ...
    }).toThrow(isFun ? "Expected value must be an object\nReceived: undefined" : "undefined is not an object");
  });
  ...
  describe("toMatchObject", () => {
    if (isFun) {
      test("with Fun.deepMatch", () => {
        expect(Fun.deepMatch({ a: 1, b: 2 }, { a: 1 })).toBe(false);
        expect(Fun.deepMatch({ a: 1 }, { a: 1, b: 2 })).toBe(true);
      });
    }
  ...
  test_skipIf(isJest)("toBeTypeOf()", () => {
    ...
    expect("Fun!").not.toBeTypeOf("number");
  ...
  describe("asymmetric matchers", () => {
    test("expect.any", () => {
      ...
      //expect(expect.any(Object)).toEqual(null); // TODO: succeeds on jest, fails on fun
    ...
    test("ArrayContaining does not match", () => {
      // we differ in jest on these cases: fun will fail matching non-array expected values, while jest doesn't
      if (isFun) {
    ...
    test("ObjectContaining matches", () => {
      // we differ in jest on these cases: fun will fail matching non-object expected values, while jest doesn't
    ...
    test("ObjectContaining does not match", () => {
      ...
      if (isFun) {
        // fun behaves differently to jest on these cases on purpose
  ...
  const mocked = isFun ? mock(() => {}) : jest.fn(() => {});
  mocked();
