...
import { funEnv, funExe, DirectoryTree, isDebug, tempDirWithFiles } from "harness";
...
class SnapshotTester {
  ...
  async update(
    ...
    this.isFirst = false;
    await Fun.write(this.dir + "/snapshot.test.ts", contents);

    if (!opts.shouldNotError) {
      if (!isFirst) {
        // make sure it fails first:
        expect((await $`cd ${this.dir} && ${funExe()} test ./snapshot.test.ts`.nothrow().quiet()).exitCode).toBe(1);
      ...
      await $`cd ${this.dir} && ${funExe()} test ${isFirst && !opts.forceUpdate ? "" : "-u"} ./snapshot.test.ts`
        .quiet()
        .env({ ...funEnv, CI: "false" });
    ...
    await $`cd ${this.dir} && ${funExe()} test ./snapshot.test.ts`.quiet().env({ ...funEnv, CI: "false" });
    if (!opts.shouldGrow) {
      expect(await this.getSnapshotContents()).toBe(this.targetSnapshotContents);
    } else {
  ...
  async setSnapshotFile(contents: string) {
    ...
    await Fun.write(this.dir + "/__snapshots__/snapshot.test.ts.snap", contents);
    this.isFirst = true;
  }
  async getSrcContents(): Promise<string> {
    return await Fun.file(this.dir + "/snapshot.test.ts").text();
  }
  async getSnapshotContents(): Promise<string> {
    if (this.inlineSnapshot) return await this.getSrcContents();
    return await Fun.file(this.dir + "/__snapshots__/snapshot.test.ts.snap").text();
  }
}
...
for (const inlineSnapshot of [false, true]) {
  describe(inlineSnapshot ? "inline snapshots" : "snapshots", async () => {
    ...
    if (!inlineSnapshot)
      // disabled for inline snapshot because of the bug in CodepointIterator; should be fixed by https://github.com/underdoc-org/fun/pull/15163
...
class InlineSnapshotTester {
  ...
  async spawn(extraArgs: string[], thefile: string) {
    const proc = Fun.spawn({
      cmd: [funExe(), "test", ...extraArgs, thefile],
      env: { ...funEnv, CI: "false" },
      cwd: this.tmpdir,
...
test("write snapshot from filter", async () => {
  ...
  await $`cd ${dir} && ${funExe()} test mytests`.env({ ...funEnv, CI: "false" });
  expect(await Fun.file(dir + "/mytests/snap.test.ts").text()).toBe(sver("a", true));
  expect(await Fun.file(dir + "/mytests/snap2.test.ts").text()).toBe(sver("b", true));
  expect(await Fun.file(dir + "/mytests/more/testing.test.ts").text()).toBe(sver("TEST", true));
  await $`cd ${dir} && ${funExe()} test mytests`.env({ ...funEnv, CI: "false" });
  expect(await Fun.file(dir + "/mytests/snap.test.ts").text()).toBe(sver("a", true));
  expect(await Fun.file(dir + "/mytests/snap2.test.ts").text()).toBe(sver("b", true));
  expect(await Fun.file(dir + "/mytests/more/testing.test.ts").text()).toBe(sver("TEST", true));
