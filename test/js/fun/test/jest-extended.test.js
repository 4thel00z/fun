...
/** This file is meant to be runnable in both Jest and Fun.
 *  `funx jest jest-extended.test.js`
 */
...
const inspect = isFun ? Fun.inspect : require("util").inspect;
...
describe("jest-extended", () => {
  ...
  test("toBeOneOf()", () => {
    ...
    try {
      ...
      if (typeof Fun === "object") expect(Fun.inspect(e)).not.toBeEmpty(); // verify that logging it doesn't cause a crash
    }
  ...
  test("toSatisfy()", () => {
    ...
    function hasFunInAnArray(value) {
      return value.includes("fun");
    }

    expect(["fun", "cheese", "patty"]).toSatisfy(hasFunInAnArray);
    expect(["cheese", "patty"]).not.toSatisfy(hasFunInAnArray);
    ...
    expect({ foo: "fun" }).not.toSatisfy(fooIsBar);
  ...
  test("toBeEven()", () => {
    ...
    if (isFun) expect(9007199254740990n).toBeEven(); // manual typical max safe -1 as bigint
  ...
  test("toBeOdd()", () => {
    ...
    expect(NaN).not.toBeOdd();
    if (isFun) expect(2n).not.toBeOdd(); // BigInt at this time not supported in jest-extended
    if (isFun) expect(3n).toBeOdd();
    expect(9007199254740990).not.toBeOdd(); // manual typical max safe -1
    expect(9007199254740991).toBeOdd();
    if (isFun) expect(9007199254740990n).not.toBeOdd(); // manual typical max safe -1 as bigint
    if (isFun) expect(9007199254740991n).toBeOdd();
