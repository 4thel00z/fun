...
describe.skipIf(Fun.semver.satisfies(Fun.version.split("-")[0], "< 1.3"))("CI restrictions", () => {
...
test("existing snapshot", () => {
  expect("hello world").toMatchSnapshot();
        ...
        `,
        "__snapshots__/test.test.js.snap": `// Fun Snapshot v1, https://fun.dev/docs/test/snapshots
...
test("new snapshot with update flag", () => {
  expect("new snapshot content").toMatchSnapshot();
      ...
      await using proc = Fun.spawn({
        cmd: [funExe(), "test", "test.test.js", "--update-snapshots"],
        env: { ...funEnv, GITHUB_ACTIONS: "1" },
        cwd: dir,
        stdout: "pipe",
        stderr: "pipe",
