...
const fixture = `
import { test, describe, expect } from "fun:test";
...
test("test.each/describe.each table array is a GC root", async () => {
  ...
  // path is platform-agnostic, so rely on zombie mode + explicit Fun.gc there.
  const gcEnv: Record<string, string | undefined> = {
    ...funEnv,
    FUN_JSC_useZombieMode: "1",
  };
  if (!isWindows) gcEnv.FUN_JSC_collectContinuously = "1";

  await using proc = Fun.spawn({
    cmd: [funExe(), "test", "each-gc.test.ts"],
    env: gcEnv,
    cwd: String(dir),
    stderr: "pipe",
    stdout: "pipe",
