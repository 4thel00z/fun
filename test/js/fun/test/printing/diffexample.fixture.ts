...
function normalizeInspectError(e: any) {
  let str = Fun.inspect(e, { colors: true });
...
test.skipIf(!Fun.enableANSIColors)("whitespace-only difference (ANSI)", () => {
  try {
    expect("hello\nworld ").toEqual("hello\nworld");
  } catch (e) {
...
test.skipIf(!Fun.enableANSIColors)("mix of whitespace-only and non-whitespace-only differences (ANSI)", () => {
  try {
    expect("hello\nworld ").toEqual("Hello\nworld ");
  } catch (e) {
    expect(normalizeInspectError(e)).toMatchInlineSnapshot(`
