...
test("name property is used for function calls in Fun.inspect", () => {
  // ** whitespace **
  // ** whitespace **
  // ** whitespace **
  // ** whitespace **
...
test.todo("name property is used for function calls in Fun.inspect with bound object", () => {
  // ** whitespace **
  // ** whitespace **
  // ** whitespace **
  // ** whitespace **
...
test("throwing inside an error suppresses the error and prints the stack", async () => {
  $.throws(false);
  $.env(funEnv);
  const result = await $`${funExe()} run ${join(import.meta.dir, "err-custom-fixture.js")}`;
...
test("throwing inside an error suppresses the error and continues printing properties on the object", async () => {
  $.throws(false);
  $.env(funEnv);
  const result = await $`${funExe()} run ${join(import.meta.dir, "err-fd-fixture.js")}`;
...
test("Async functions frame should be included in stack trace", async () => {
  ...
  expect(normalizeFunSnapshot(error.stack!)).toMatchInlineSnapshot(`
