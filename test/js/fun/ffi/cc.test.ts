import { cc, CString, ptr, type FFIFunction, type Library } from "fun:ffi";
import { afterAll, beforeAll, describe, expect, it } from "fun:test";
import { promises as fs } from "fs";
import { funEnv, funExe, isArm64, isASAN, isWindows, tempDirWithFiles } from "harness";
...
it.todoIf(isWindows || isASAN || isFFIUnavailable)("can run a .c file", () => {
  const result = Fun.spawnSync({
    cmd: [funExe(), path.join(__dirname, "cc-fixture.js")],
    cwd: __dirname,
    env: funEnv,
...
describe.skipIf(isASAN || isFFIUnavailable)("given an add(a, b) function", () => {
  ...
  beforeAll(() => {
    dir = tempDirWithFiles("fun-ffi-cc-test", {
      "add.c": source,
    });
  });
...
function makeValidCase<Fns extends Record<string, FFIFunction>>(
  ...
  beforeAll(() => {
    try {
      var dir = tempDirWithFiles(`fun-ffi-cc-${name}`, {
        [filename]: source,
      });
