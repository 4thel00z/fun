import { afterAll, describe, expect, it } from "fun:test";
...
const ok = existsSync("/tmp/fun-ffi-test." + suffix);
...
function ffiRunner(fast) {
  describe("FFI runner" + (fast ? " (fast int)" : ""), () => {
    ...
    const {
      ...
      close,
    } = dlopen("/tmp/fun-ffi-test.dylib", types);
    it("primitives", () => {
      Fun.gc(true);
...
it(".ptr is not leaked", () => {
  for (let fn of [Fun.password.hash, Fun.password.verify, it]) {
...
describe.if(!!libPath)("can open more than 63 symbols via", () => {
  for (const [description, libFn] of [
    ...
    ["URL", () => Fun.pathToFileURL(libPath)],

    // file: URLs as a string
    ["file: URL", () => Fun.pathToFileURL(libPath).href],

    // For embedding files since one might do Fun.file(embeddedFile)
    ["Fun.file", () => Fun.file(libPath)],
    ...
    it(description, () => {
      ...
      expect(lib.symbols.strlen(Buffer.from("funfun\0", "ascii"))).toBe(6n);
    });
  }
});
