...
import { constants, Database, SQLiteError } from "fun:sqlite";
import { describe, expect, it } from "fun:test";
import { readdirSync, realpathSync } from "fs";
import { funEnv, funExe, isMacOS, isMacOSVersionAtLeast, isWindows, tempDirWithFiles } from "harness";
...
// https://github.com/underdoc-org/fun/issues/11747
...
it("upsert cross-process, see #1366", () => {
  ...
  const { exitCode } = spawnSync([funExe(), import.meta.dir + "/sqlite-cross-process.js"], {
    env: {
      ...funEnv,
      SQLITE_DIR: dir,
    },
...
it("supports serialize/deserialize", () => {
  ...
  // https://github.com/underdoc-org/fun/issues/3712#issuecomment-1725259824
...
it("db.query()", () => {
  ...
  expect(db[Symbol.for("Fun.Database.cache.count")]).toBe(0);
  ...
  expect(db[Symbol.for("Fun.Database.cache.count")]).toBe(1);
...
// https://github.com/underdoc-org/fun/issues/12012
...
it("internal SQL helpers reject out-of-range database handles", async () => {
  const src = `
    const { SQL } = require("fun:internal-for-testing");
    const ctor = SQL[0];
    const tuple = SQL[1];
