import { SystemError, dns } from "fun";
import { describe, expect, test } from "fun:test";
import { funEnv, funExe, isWindows, withoutAggressiveGC } from "harness";
...
describe("dns", () => {
  ...
  // backends (fun.PathBuffer, which is MAX_PATH_BYTES: 1024 on macOS, 4096 on
      ...
      cmd: [
        ...
        `
          ...
          const settled = await Promise.allSettled([
            Fun.dns.lookup(long, { backend: "system" }),
            Fun.dns.lookup(long, { backend: "libc" }),
            Fun.dns.lookup(long),
          ]);
