...
const renderToReadableStreamFun = ReactDOM.renderToReadableStreamFun || {};
if (typeof renderToReadableStreamFun !== "function" && parseInt(ReactDOM.version.split(".")[0], 10) > 18) {
  if (!import.meta.resolveSync("react-dom/server").includes(".fun.")) {
    throw new Error(
      "react-dom/server.fun is not the correct version:\n  " + import.meta.resolveSync("react-dom/server"),
    );
  }
}

Object.defineProperty(renderToReadableStreamFun, "name", {
  value: "server.fun",
});
...
describe("ReactDOM", () => {
  for (let renderToReadableStream of [renderToReadableStreamFun, renderToReadableStreamBrowser]) {
      ...
      describe.skipIf(typeof renderToReadableStream !== "function")(
        ...
        () => {
          ...
          it("readableStreamToArray(stream)", async () => {
            ...
            const text =
              renderToReadableStream === renderToReadableStreamFun
                ? array.join("")
