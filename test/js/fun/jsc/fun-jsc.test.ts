...
describe("fun:jsc", () => {
  ...
  it("isRope", () => {
    // https://twitter.com/funjavascript/status/1806921203644571685
    let y;
    y = 123;
  ...
  it("numberOfDFGCompiles", async () => {
    await Fun.sleep(5); // this failed once and i suspect it is because the query was done too fast
  ...
  it.todoIf(isBuildKite && isWindows)("profile async", async () => {
    ...
    const result = await profile(
      async function hey(arg1: number) {
        await Fun.sleep(10).then(() => resolve(arguments));
        return arg1;
      },
      1,
      2,
