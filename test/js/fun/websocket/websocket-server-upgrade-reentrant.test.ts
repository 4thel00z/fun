...
import { funEnv, funExe, isLinux, tempDir } from "harness";
...
test("server.upgrade() clones Sec-WebSocket-* from request.headers before running option getters", async () => {
  ...
  const env: Record<string, string | undefined> = { ...funEnv };
  if (isLinux) {
    ...
    env.ASAN_OPTIONS = [funEnv.ASAN_OPTIONS, "symbolize=0", "detect_leaks=0"].filter(Boolean).join(":");
  }
