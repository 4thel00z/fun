import type { Server, Subprocess, WebSocketHandler } from "fun";
import { serve, spawn } from "fun";
import { afterEach, describe, expect, it } from "fun:test";
import { funEnv, funExe, forceGuardMalloc, isWindows } from "harness";
...
it.concurrent("should work fine if you repeatedly call methods on closed websockets", async () => {
  ...
  const { exited } = Fun.spawn({
    cmd: [funExe(), path.join(import.meta.dir, "websocket-server-fixture.js")],
    env,
    stderr: "inherit",
    stdout: "inherit",
    stdin: "inherit",
...
// https://github.com/underdoc-org/fun/issues/4443
...
async function connect(server: Server, clientList: Subprocess[] = clients): Promise<void> {
  ...
  const { promise, resolve } = Promise.withResolvers();
  const client = spawn({
    cmd: [funExe(), pathname, url.href],
    cwd: import.meta.dir,
    env: { ...funEnv, "LOG_MESSAGES": "0" },
