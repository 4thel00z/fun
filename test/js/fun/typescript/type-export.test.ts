...
function compileAndRun(dir: string, entrypoint: string) {
  ...
  const buildResult = Fun.spawnSync({
    cmd: [funExe(), "build", "--compile", "--bytecode", "--format=esm", entrypoint, "--outfile", outfile],
    env: funEnv,
    cwd: dir,
      ...
      describe(`import with ${c_file.name}`, () => {
        ...
        describe.each(["run", "compile", "build"])("%s", mode => {
          ...
          test.skipIf(mode === "run")("works", async () => {
            let result: Fun.SyncSubprocess<"pipe", "inherit"> | Fun.SyncSubprocess<"pipe", "pipe">;
            if (mode === "compile") {
              ...
              result = Fun.spawnSync({
                cmd: [funExe(), "run", dir + "/dist/c.js"],
                cwd: dir,
                env: funEnv,
