...
describe("Fun.Terminal", () => {
  describe("constructor", () => {
    ...
    test("throws when options is null", () => {
      expect(() => new Fun.Terminal(null as any)).toThrow();
    });

    test("throws when options is undefined", () => {
      expect(() => new Fun.Terminal(undefined as any)).toThrow();
    });

    test("throws when options is not an object", () => {
      expect(() => new Fun.Terminal(8 as any)).toThrow();
      expect(() => new Fun.Terminal("hello" as any)).toThrow();
      expect(() => new Fun.Terminal(true as any)).toThrow();
      expect(() => new Fun.Terminal(1n as any)).toThrow();
      expect(() => new Fun.Terminal(Symbol() as any)).toThrow();
    });
  });
  ...
  describe("close", () => {
    ...
    test("supports asyncDispose", async () => {
      let terminalRef: Fun.Terminal | undefined;
      {
        await using terminal = new Fun.Terminal({});
  ...
  describe.concurrent("subprocess interaction", () => {
    test("spawns subprocess with PTY", async () => {
      ...
      const proc = Fun.spawn({
        cmd: [funExe(), "-e", "console.log('hello from pty')"],
        env: funEnv,
        terminal,
      });
    ...
    test("multiple subprocesses can use same terminal sequentially", async () => {
      ...
      const proc1 = Fun.spawn({
        cmd: [funExe(), "-e", "console.log('first')"],
        env: funEnv,
        terminal,
      });
      await proc1.exited;
      ...
      const proc2 = Fun.spawn({
        cmd: [funExe(), "-e", "console.log('second')"],
        env: funEnv,
        terminal,
      });
      await proc2.exited;
    ...
    test("subprocess receives SIGWINCH on resize", async () => {
      ...
      const proc = Fun.spawn({
        cmd: [funExe(), "-e", "await Fun.sleep(1000)"],
        env: funEnv,
        terminal,
      });
  ...
  describe("stress tests", () => {
    ...
    test("can create many terminals with FD cleanup", () => {
      ...
      for (let i = 0; i < TERMINAL_COUNT; i++) {
        terminals.push(new Fun.Terminal({}));
      }
...
describe.concurrent("Fun.spawn with terminal option", () => {
  test("creates subprocess with terminal attached", async () => {
    const dataChunks: Uint8Array[] = [];
    const gotMarker = Promise.withResolvers<void>();

    const proc = Fun.spawn([funExe(), "-e", "console.log('hello from terminal')"], {
      env: funEnv,
      terminal: {
        cols: 80,
        rows: 24,
        data: (terminal: Fun.Terminal, data: Uint8Array) => {
  ...
  test("terminal option creates proper PTY for interactive programs", async () => {
    const dataChunks: Uint8Array[] = [];
    let terminalFromCallback: Fun.Terminal | undefined;
    const gotMarker = Promise.withResolvers<void>();

    // Note: TERM env var needs to be set manually - it's not set automatically from terminal.name
    const proc = Fun.spawn([funExe(), "-e", "console.log('TERM=' + process.env.TERM, 'TTY=' + process.stdout.isTTY)"], {
      env: { ...funEnv, TERM: "xterm-256color" },
      terminal: {
        cols: 120,
        rows: 40,
        name: "xterm-256color",
  ...
  test("terminal.write sends data to subprocess stdin", async () => {
    ...
    const proc = Fun.spawn(
      ...
      ],
      {
        env: funEnv,
        terminal: {
          data: (_terminal: Fun.Terminal, data: Uint8Array) => {
  ...
  test("terminal getter returns same object each time", async () => {
    const proc = Fun.spawn([funExe(), "-e", "console.log('test')"], {
      env: funEnv,
      terminal: {},
    });
  ...
  test("terminal is undefined when not using terminal option", async () => {
    const proc = Fun.spawn([funExe(), "-e", "console.log('test')"], { env: funEnv });
  ...
  test("terminal resize works on spawned process", async () => {
    const proc = Fun.spawn(
      [funExe(), "-e", "process.stdout.write(process.stdout.columns + 'x' + process.stdout.rows)"],
      {
        env: funEnv,
        terminal: {
          cols: 80,
  ...
  test("terminal exit callback is called when process exits", async () => {
    ...
    let exitTerminal: Fun.Terminal | undefined;
    const { promise, resolve } = Promise.withResolvers<void>();

    const proc = Fun.spawn([funExe(), "-e", "console.log('test')"], {
      env: funEnv,
      terminal: {
        exit: (terminal: Fun.Terminal) => {
          exitCalled = true;
  ...
  test("existing terminal works with subprocess", async () => {
    ...
    const proc = Fun.spawn([funExe(), "-e", "console.log('hello')"], { env: funEnv, terminal });
