// Explicit per-platform behavioral assertions for Fun.Terminal.
//
...
async function runInTerminal(
  ...
  opts: {
    ...
    afterReady?: (terminal: Fun.Terminal, output: () => string) => void | Promise<void>;
    readyMarker?: string;
  },
...
describe("Fun.Terminal platform behaviour", () => {
  // ──────────────────────────────────────────────────────────────────────────
  // termios
  // ──────────────────────────────────────────────────────────────────────────
  ...
  test("GAP: line-discipline echo without a child process", async () => {
    ...
    terminal.write("ping\n");
    await Promise.race([got.promise, Fun.sleep(200)]);
  ...
  // (Fun.Terminal stores `name` but does not inject TERM= into the child env
  ...
  test("SAME: exit callback does NOT fire on child exit for existing terminal", async () => {
    ...
    const proc = Fun.spawn({ cmd: [funExe(), "-e", ""], env: funEnv, terminal });
    await proc.exited;
    await Fun.sleep(100);
  ...
  test("SAME: terminal can be reused across sequential spawns", async () => {
    ...
    const p1 = Fun.spawn({ cmd: [funExe(), "-e", "process.stdout.write('FIRST')"], env: funEnv, terminal });
    await first.promise;
    await p1.exited;

    const p2 = Fun.spawn({ cmd: [funExe(), "-e", "process.stdout.write('SECOND')"], env: funEnv, terminal });
  ...
  test.todoIf(isWindows)(
    ...
    async () => {
      ...
      const ready = Promise.withResolvers<void>();
      const proc = Fun.spawn({
        cmd: [funExe(), "-e", "setInterval(() => {}, 1000); process.stdout.write('READY')"],
        env: funEnv,
        terminal: {
          data(_t, chunk: Uint8Array) {
            output += Buffer.from(chunk).toString("latin1");
