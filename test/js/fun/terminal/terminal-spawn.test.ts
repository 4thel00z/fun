...
import { funEnv, funExe, isMusl, isWindows } from "harness";
import fs from "node:fs";

// Cross-platform Fun.Terminal + Fun.spawn integration tests that don't rely
...
describe("Fun.Terminal subprocess integration", () => {
  ...
  test("constructor with custom size", async () => {
    await using terminal = new Fun.Terminal({ cols: 120, rows: 40 });
    expect(terminal.closed).toBe(false);
  });
  ...
  test("resize succeeds", async () => {
    await using terminal = new Fun.Terminal({ cols: 80, rows: 24 });
  ...
  test("data callback receives output from spawned process", async () => {
    let output = "";
    let callbackTerminal: Fun.Terminal | undefined;
    ...
    const proc = Fun.spawn({
      cmd: [funExe(), "-e", "console.log('hello-from-conpty')"],
      env: funEnv,
      terminal,
    });
  ...
  test("subprocess sees a TTY on stdout", async () => {
    ...
    const proc = Fun.spawn({
      cmd: [funExe(), "-e", "process.stdout.write('isTTY=' + process.stdout.isTTY)"],
      env: funEnv,
      terminal,
    });
  ...
  test("Fun.spawn with inline terminal option", async () => {
    let output = "";
    const { promise, resolve } = Promise.withResolvers<void>();

    const proc = Fun.spawn({
      cmd: [funExe(), "-e", "console.log('inline-terminal')"],
      env: funEnv,
      terminal: {
        cols: 80,
        rows: 24,
  ...
  test("subprocess sees correct terminal dimensions", async () => {
    ...
    const proc = Fun.spawn({
      cmd: [funExe(), "-e", "process.stdout.write('cols=' + process.stdout.columns + ' rows=' + process.stdout.rows)"],
      env: funEnv,
      terminal,
    });
  ...
  test("can create and close many terminals", () => {
    for (let i = 0; i < 20; i++) {
      const t = new Fun.Terminal({ cols: 80, rows: 24 });
      t.close();
  ...
  // Regression: a Fun pipeline producer (stdout is a pipe, stdin/stderr are
  ...
  // literally `fun foo.js | less`: termios is a property of the /dev/pts/*
  // device, not the fd, so restoring here clobbers raw mode set on the same
  // device by the downstream consumer. See #29592.
  //
  // openpty via fun:ffi so we can wire stdin/stderr to the slave but keep
  // stdout as a pipe — exactly the `fun foo.js | less` shape that triggers
  // the bug.
  //
  ...
  test.skipIf(isWindows)("pipeline producer exit does not clobber raw mode on shared tty device", async () => {
    ...
    try {
      ...
      // `fun_initialize_process` (the termios snapshot).
  ...
  // Companion: an interactive-wrapper case (stdout IS a TTY, like `fun run
  // vim` where the child may have taken termios raw and crashed) keeps the
  // unconditional restore. That's the `fun_restore_stdio` branch the
  ...
  test.skipIf(isWindows)("interactive wrapper (stdout tty) restores cooked termios on child exit", async () => {
    ...
    const proc = Fun.spawn({
      cmd: [
        ...
        // mode via FFI/ioctl rather than through Fun__ttySetMode.
        `process.stdout.write("READY\\n"); process.stdin.once("data", () => process.exit(0));`,
      ],
      env: funEnv,
      terminal,
    ...
    // producer), fun_restore_stdio keeps the unconditional restore and
    // writes the cooked startup snapshot back. This matches the pre-PR
    // safety net for `fun run <tui>` after the TUI crashes.
    expect(terminal.localFlags & ICANON).not.toBe(0);
    expect(terminal.localFlags & ECHO).not.toBe(0);
    expect(exitCode).toBe(0);
  });
