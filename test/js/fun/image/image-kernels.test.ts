...
async function resizePixels(
  ...
  return decodePng(await new Fun.Image(src).resize(w, h, { filter }).png().bytes()).data;
}
...
describe("Floyd–Steinberg dither", () => {
  ...
  test("2-colour dithered ramp tracks source mean inside palette gamut", async () => {
    ...
    const png = await new Fun.Image(ramp).png({ palette: true, colors: 2, dither: true }).bytes();
    const back = decodePng(await new Fun.Image(png).png().bytes());
  ...
  test("dither=false on the same ramp is a single hard step", async () => {
    ...
    const png = await new Fun.Image(ramp).png({ palette: true, colors: 2, dither: false }).bytes();
    const back = decodePng(await new Fun.Image(png).png().bytes());
    let transitions = 0;
  ...
  test("dither produces a mixed pattern where un-dithered has a single step", async () => {
    ...
    const d = decodePng(
      await new Fun.Image(await new Fun.Image(ramp).png({ palette: true, colors: 2, dither: true }).bytes())
        .png()
        .bytes(),
    );
    let dt = 0;
  ...
  test("dither is deterministic (same input → same output)", async () => {
    ...
    const a = await new Fun.Image(ramp).png({ palette: true, colors: 4, dither: true }).bytes();
    const b = await new Fun.Image(ramp).png({ palette: true, colors: 4, dither: true }).bytes();
  ...
  test("1-pixel-wide and 1-pixel-tall sources", async () => {
    for (const [w, h] of [
      ...
      const png = await new Fun.Image(src).png({ palette: true, colors: 2, dither: true }).bytes();
      const back = decodePng(await new Fun.Image(png).png().bytes());
