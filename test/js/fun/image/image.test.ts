...
// fixtures are produced by round-tripping that PNG through Fun.Image itself —
...
describe("Fun.Image", () => {
  test("constructor exists and is exposed on Fun", () => {
    expect(typeof Fun.Image).toBe("function");
    expect(() => new Fun.Image()).toThrow();
  });

  test("Fun.file() input chains the async file read into the pipeline", async () => {
    using dir = tempDir("image-funfile", {});
    const p = join(String(dir), "src.png");
    await Fun.write(p, cornersPng);
    ...
    const img = new Fun.Image(Fun.file(p));
    ...
    const via = Fun.file(p).image();
    expect(via).toBeInstanceOf(Fun.Image);
    ...
    await expect(Fun.file(p).image({ maxPixels: 4 }).metadata()).rejects.toThrow(/maxPixels/);
    // Missing file rejects with a real fs error, not an Image-layer one.
    await expect(new Fun.Image(Fun.file(join(String(dir), "nope.png"))).metadata()).rejects.toThrow(/ENOENT/);
    // Synchronous Response-body path: path-backed FunFile falls back to
    // .path source and runs inline.
    const res = new Response(new Fun.Image(Fun.file(p)).resize(2, 2).webp());
    expect(res.headers.get("content-type")).toBe("image/webp");
    expect((await res.bytes()).subarray(8, 12)).toEqual(Buffer.from("WEBP"));
  });

  test("metadata() reads PNG dimensions", async () => {
    const img = new Fun.Image(cornersPng);
    const meta = await img.metadata();
  ...
  test("PNG → PNG round-trip preserves every pixel", async () => {
    const out = await new Fun.Image(cornersPng).png().bytes();
    expect(out[0]).toBe(0x89);
  ...
  describe.each(["jpeg", "webp"] as const)("%s", fmt => {
    test(`PNG → ${fmt} → decode dimensions`, async () => {
      const out = await new Fun.Image(gradientPng)[fmt]({ quality: 90 }).bytes();
      // Magic-byte sanity.
      if (fmt === "jpeg") {
      ...
      const meta = await new Fun.Image(out).metadata();
    ...
    test(`PNG → ${fmt} (q90) → PNG approximates source within tolerance`, async () => {
      const out = await new Fun.Image(gradientPng)[fmt]({ quality: 90 }).bytes();
      const back = await new Fun.Image(out).png().bytes();
  ...
  test("WebP lossless round-trips exactly", async () => {
    const out = await new Fun.Image(cornersPng).webp({ lossless: true }).bytes();
    const back = decodePngRaw(await new Fun.Image(out).png().bytes());
    for (let y = 0; y < 3; y++)
  ...
  test("rotate(90) moves corners CW and swaps dimensions", async () => {
    const out = await new Fun.Image(cornersPng).rotate(90).png().bytes();
    const { w, h, data } = decodePngRaw(out);
    expect(w).toBe(3);
    expect(h).toBe(4);
  ...
  test("rotate(180) swaps opposite corners", async () => {
    const { data } = decodePngRaw(await new Fun.Image(cornersPng).rotate(180).png().bytes());
  ...
  test("flop() mirrors horizontally", async () => {
    const { data } = decodePngRaw(await new Fun.Image(cornersPng).flop().png().bytes());
  ...
  describe("resize", () => {
    test("downscale 16→8 with each filter yields correct dims", async () => {
      for (const filter of ["box", "bilinear", "lanczos3"] as const) {
        const out = await new Fun.Image(gradientPng).resize(8, 8, { filter }).png().bytes();
    ...
    test("box filter on flat colour is identity", async () => {
      ...
      const out = await new Fun.Image(flat).resize(4, 4, { filter: "box" }).png().bytes();
    ...
    test("upscale 4→8 preserves corner colours under lanczos3", async () => {
      const out = await new Fun.Image(cornersPng).resize(8, 6, { filter: "lanczos3" }).png().bytes();
      const { w, h, data } = decodePngRaw(out);
      expect(w).toBe(8);
      expect(h).toBe(6);
    ...
    test("preserves aspect ratio when height omitted", async () => {
      const meta = decodePngRaw(await new Fun.Image(gradientPng).resize(8).png().bytes());
    ...
    test("resize(w, 0) ≡ resize(w) — explicit zero is the aspect-ratio sentinel", async () => {
      ...
      const a = decodePngRaw(await new Fun.Image(wide).resize(4, 0).png().bytes());
      const b = decodePngRaw(await new Fun.Image(wide).resize(4).png().bytes());
  ...
  test("path string input reads from disk", async () => {
    using dir = tempDir("fun-image", { "in.png": Buffer.from(gradientPng) });
    const meta = await new Fun.Image(join(String(dir), "in.png")).metadata();
    expect(meta.width).toBe(16);
    expect(meta.height).toBe(16);
  });
  ...
  describe("security", () => {
    ...
    test("maxPixels rejects oversized PNG before allocating", async () => {
      ...
      dv.setUint32(29, ~c >>> 0);
      await expect(new Fun.Image(bomb).metadata()).rejects.toThrow(/maxPixels/);
    });

    test("maxPixels can be lowered per-instance", async () => {
      await expect(new Fun.Image(gradientPng, { maxPixels: 10 }).metadata()).rejects.toThrow(/maxPixels/);
      // …and the default still admits it.
      expect((await new Fun.Image(gradientPng).metadata()).width).toBe(16);
    });
    ...
    for (const [name, bad] of [
      ...
      test(`rejects cleanly on ${name}`, async () => {
        await expect(new Fun.Image(bad).bytes()).rejects.toThrow();
      });
    }
  });
  ...
  describe("cross-backend correctness", () => {
    test("translucent alpha survives PNG→PNG (catches premultiplied-alpha mislabel)", async () => {
      ...
      const out = await new Fun.Image(src).png().bytes();
    ...
    test("PNG output has nothing after IEND (catches WIC GlobalSize over-read)", async () => {
      const out = await new Fun.Image(gradientPng).png().bytes();
    ...
    test("PNG encode is deterministic across two calls", async () => {
      const a = await new Fun.Image(gradientPng).png().bytes();
      const b = await new Fun.Image(gradientPng).png().bytes();
    ...
    test("64×48 lanczos3 downscale → upscale stays close to source", async () => {
      ...
      const half = await new Fun.Image(src).resize(32, 24).png().bytes();
      ...
      const back = decodePngRaw(await new Fun.Image(half).resize(64, 48).png().bytes());
    ...
    test("JPEG encode respects quality (lower quality → smaller file)", async () => {
      ...
      const q90 = await new Fun.Image(big).jpeg({ quality: 90 }).bytes();
      const q20 = await new Fun.Image(big).jpeg({ quality: 20 }).bytes();
  ...
  // XYB) as sRGB and visibly shifts the colours. Fun's contract here is:
  ...
  describe("ICC profile", () => {
    ...
    test("PNG iCCP survives resize + re-encode — geometry doesn't drop profile", async () => {
      const src = pngWithIccp(cornersPng, fakeProfile);
      const out = await new Fun.Image(src).resize(8, 6).png().bytes();
      const got = extractPngIccp(out);
      expect(got).not.toBeNull();
      expect(Array.from(got!)).toEqual(Array.from(fakeProfile));
    });
    ...
    test("PNG iCCP survives rotate — applyPipeline preserves profile across Decoded swap", async () => {
      const src = pngWithIccp(cornersPng, fakeProfile);
      const out = await new Fun.Image(src).rotate(90).png().bytes();
      const got = extractPngIccp(out);
      expect(got).not.toBeNull();
      expect(Array.from(got!)).toEqual(Array.from(fakeProfile));
    });
    ...
    test("PNG iCCP transfers to JPEG encode — cross-format preserves profile", async () => {
      const src = pngWithIccp(cornersPng, fakeProfile);
      const jpg = await new Fun.Image(src).jpeg({ quality: 90 }).bytes();
    ...
    test("JPEG without ICC_PROFILE encodes to JPEG without ICC_PROFILE", async () => {
      ...
      const jpg = await new Fun.Image(cornersPng).jpeg({ quality: 80 }).bytes();
      expect(extractJpegIcc(jpg)).toBeNull();
    });

    test("JPEG → JPEG re-encode preserves the ICC profile byte-for-byte", async () => {
      // Build a JPEG with ICC by going PNG(with iCCP) → Fun.Image → JPEG,
      // then re-encode that JPEG through Fun.Image and confirm the profile
      ...
      const jpg = await new Fun.Image(srcPng).jpeg({ quality: 90 }).bytes();
      const reJpg = await new Fun.Image(jpg).jpeg({ quality: 90 }).bytes();
    ...
    test("indexed PNG preserves iCCP — quantise doesn't convert colour space", async () => {
      ...
      const src = pngWithIccp(cornersPng, fakeProfile);
      const out = await new Fun.Image(src).png({ palette: true, colors: 8 }).bytes();
      const got = extractPngIccp(out);
      expect(got).not.toBeNull();
      expect(Array.from(got!)).toEqual(Array.from(fakeProfile));
    });
    ...
    test("EXIF auto-orient preserves the ICC profile — applyOrientation doesn't drop it", async () => {
      ...
      // that already carries an APP2 ICC_PROFILE, and confirm Fun's
      ...
      const srcPng = pngWithIccp(cornersPng, fakeProfile);
      const jpg = Buffer.from(await new Fun.Image(srcPng).jpeg({ quality: 90 }).bytes());
      // prettier-ignore
      ...
      const out = await new Fun.Image(withExif).jpeg({ quality: 90 }).bytes();
    ...
    test("PNG iCCP transfers to WebP encode — ICCP chunk in output", async () => {
      const src = pngWithIccp(cornersPng, fakeProfile);
      const webp = await new Fun.Image(src).webp({ quality: 90 }).bytes();
    ...
    test("PNG iCCP transfers to lossless WebP encode", async () => {
      ...
      // coverage.
      const src = pngWithIccp(cornersPng, fakeProfile);
      const webp = await new Fun.Image(src).webp({ lossless: true }).bytes();
      const got = extractWebpIccp(webp);
      expect(got).not.toBeNull();
      expect(Array.from(got!)).toEqual(Array.from(fakeProfile));
    });
    ...
    test("WebP ICCP transfers to PNG encode — demux extracts the profile", async () => {
      // Build the WebP via Fun.Image (PNG-with-iCCP → WebP) so the test
      // doesn't hand-assemble a VP8X container, then decode THAT WebP
      // and re-encode to PNG. This exercises `WebPDemuxGetChunk` on the
      // decode side.
      const srcPng = pngWithIccp(cornersPng, fakeProfile);
      const webp = await new Fun.Image(srcPng).webp({ lossless: true }).bytes();
      const outPng = await new Fun.Image(webp).png().bytes();
    ...
    test("WebP → WebP re-encode preserves the ICC profile byte-for-byte", async () => {
      const srcPng = pngWithIccp(cornersPng, fakeProfile);
      const webp = await new Fun.Image(srcPng).webp({ quality: 90 }).bytes();
      const reWebp = await new Fun.Image(webp).webp({ quality: 90 }).bytes();
    ...
    test("WebP ICCP survives resize — geometry doesn't drop profile", async () => {
      const src = pngWithIccp(cornersPng, fakeProfile);
      const webp = await new Fun.Image(src).resize(8, 6).webp({ quality: 90 }).bytes();
      const got = extractWebpIccp(webp);
      expect(got).not.toBeNull();
      expect(Array.from(got!)).toEqual(Array.from(fakeProfile));
    });
    ...
    test("WebP without ICCP stays a bare VP8/VP8L — no VP8X wrapper added", async () => {
      ...
      const lossy = await new Fun.Image(cornersPng).webp({ quality: 90 }).bytes();
      expect(String.fromCharCode(...lossy.subarray(12, 16))).toBe("VP8 ");
      expect(extractWebpIccp(lossy)).toBeNull();

      const lossless = await new Fun.Image(cornersPng).webp({ lossless: true }).bytes();
      expect(String.fromCharCode(...lossless.subarray(12, 16))).toBe("VP8L");
      expect(extractWebpIccp(lossless)).toBeNull();
    });
  });

  // EXIF: build a minimal JPEG via Fun.Image, then splice in an APP1 segment
  ...
  test("EXIF Orientation=6 auto-rotates", async () => {
    ...
    const meta = await new Fun.Image(withExif).metadata();
    expect(meta).toEqual({ width: 2, height: 4, format: "jpeg" });
    // And opting out leaves it landscape.
    const raw = await new Fun.Image(withExif, { autoOrient: false }).metadata();
    expect(raw).toEqual({ width: 4, height: 2, format: "jpeg" });
  });

  test("rejects on unrecognised input", async () => {
    await expect(new Fun.Image(new Uint8Array([1, 2, 3, 4, 5, 6, 7, 8, 9])).metadata()).rejects.toThrow(
      /unrecognised|decode/i,
    );
  });

  test("rotate rejects non-90° multiples", () => {
    expect(() => new Fun.Image(cornersPng).rotate(45)).toThrow();
  });
  ...
  describe("HEIC / AVIF (system-backend formats)", () => {
    ...
    test("sniffer recognises ftyp brands", async () => {
      ...
      await expect(new Fun.Image(heicHdr).metadata()).rejects.toThrow();
      await expect(new Fun.Image(avifHdr).metadata()).rejects.toThrow();
    });

    if (!isMacOS && !isWindows) {
      test("encode .heic()/.avif() rejects ERR_IMAGE_FORMAT_UNSUPPORTED on Linux", async () => {
        for (const fmt of ["heic", "avif"] as const)
          await expect(new Fun.Image(cornersPng)[fmt]().bytes()).rejects.toMatchObject({
      ...
      test.each(["heic", "avif"] as const)(".%s() encodes (or rejects ERR_IMAGE_FORMAT_UNSUPPORTED)", async fmt => {
        let out: Uint8Array;
        try {
          out = await new Fun.Image(cornersPng)[fmt]({ quality: 50 }).bytes();
        } catch (e: any) {
        ...
        expect((await new Fun.Image(out).metadata()).format).toBe(fmt);
      });
    }
  });
  ...
  test("non-finite / huge numeric inputs are clamped by coerceInt", async () => {
    ...
    for (const v of [Infinity, -Infinity, NaN, 1e300, -1e300]) {
      expect(() => new Fun.Image(cornersPng).rotate(v)).toThrow(/only multiples of 90/);
    }
    // resize/quality/maxPixels clamp; NaN→lo bound, ±Inf→matching bound.
    const out = await new Fun.Image(cornersPng).resize(NaN, NaN).jpeg({ quality: NaN }).bytes();
    expect(out[0]).toBe(0xff);
    expect((await new Fun.Image(gradientPng, { maxPixels: Infinity }).metadata()).width).toBe(16);
    // Infinity width clamps to the per-side cap; output then exceeds maxPixels
    // and rejects with the bomb-guard error.
    await expect(new Fun.Image(cornersPng).resize(Infinity).png().bytes()).rejects.toThrow(/maxPixels/);
  });
  ...
  test("pipeline order is fixed (rotate before resize) and setters overwrite", async () => {
    const out = await new Fun.Image(cornersPng) // 4×3
  ...
  describe("output-format setters + terminals", () => {
    test(".jpeg().bytes() produces JPEG", async () => {
      const out = await new Fun.Image(gradientPng).jpeg({ quality: 90 }).bytes();
      expect(out[0]).toBe(0xff);
      expect(out[1]).toBe(0xd8);
    });
    ...
    test(".bytes() with no setter re-encodes in source format", async () => {
      const jpegBytes = await new Fun.Image(gradientPng).jpeg().bytes();
      const out = await new Fun.Image(jpegBytes).resize(8, 8).bytes();
    ...
    test(".blob() yields a Blob with the right MIME type", async () => {
      const blob = await new Fun.Image(gradientPng).png().blob();
    ...
    test(".buffer() returns a Node Buffer", async () => {
      const buf = await new Fun.Image(cornersPng).png().buffer();
      expect(Buffer.isBuffer(buf)).toBe(true);
      expect(buf[0]).toBe(0x89);
    });

    test(".dataurl() is .toBase64() with the MIME prefix", async () => {
      const img = new Fun.Image(cornersPng).png();
      ...
      expect(await new Fun.Image(url).metadata()).toEqual({ width: 4, height: 3, format: "png" });
      // Format follows the chained encoder.
      expect(await new Fun.Image(cornersPng).webp().dataurl()).toMatch(/^data:image\/webp;base64,/);
    });
    ...
    test(".placeholder() is a ThumbHash-rendered ≤32px PNG data: URL", async () => {
      ...
      const url = await new Fun.Image(src).placeholder();
      expect(url).toMatch(/^data:image\/png;base64,/);
      // Typical ThumbHash render PNG-encodes to a few hundred bytes.
      expect(url.length).toBeLessThan(2000);
      // The data: URL is itself a valid Fun.Image input — verify it decodes
      // to ≤32px on the long side and preserves aspect (80:60 = 4:3).
      const m = await new Fun.Image(url).metadata();
      ...
      const px = decodePngRaw(await new Fun.Image(url).png().bytes());
      ...
      expect(await new Fun.Image(src).placeholder("dataurl")).toBe(url);
      expect(() => new Fun.Image(src).placeholder("hash" as any)).toThrow(/dataurl/);
    });

    test(".jpeg({progressive: true}) emits SOF2 (multi-scan)", async () => {
      const baseline = await new Fun.Image(gradientPng).jpeg({ quality: 80 }).bytes();
      const prog = await new Fun.Image(gradientPng).jpeg({ quality: 80, progressive: true }).bytes();
      ...
      expect(await new Fun.Image(prog).metadata()).toEqual(await new Fun.Image(baseline).metadata());
    });

    test(".toBuffer() is a Sharp-compat alias for .buffer()", async () => {
      const buf = await new Fun.Image(cornersPng).png().toBuffer();
      expect(Buffer.isBuffer(buf)).toBe(true);
      expect(buf[0]).toBe(0x89);
    });

    test(".write(dest) routes through Fun.write — path string, Fun.file, fs errors", async () => {
      using dir = tempDir("image-write", {});
      // 1. Path string + extension-inferred format.
      const out = join(String(dir), "out.webp");
      const n = await new Fun.Image(cornersPng).resize(2, 2).write(out);
      const bytes = await Fun.file(out).bytes();
      ...
      await new Fun.Image(cornersPng).jpeg({ quality: 50 }).write(out2);
      expect((await Fun.file(out2).bytes())[0]).toBe(0xff); // SOI, not 0x89
      // 3. Fun.file destination — same dest types Fun.write accepts.
      const out3 = Fun.file(join(String(dir), "out3.png"));
      const n3 = await new Fun.Image(cornersPng).png().write(out3);
      expect(n3).toBeGreaterThan(0);
      expect((await out3.bytes())[0]).toBe(0x89);
      // 4. fs error propagates from Fun.write, not the Image layer.
      await expect(new Fun.Image(cornersPng).png().write(String(dir))).rejects.toThrow();
    });

    test(".toBase64() produces valid base64", async () => {
      const b64 = await new Fun.Image(cornersPng).png().toBase64();
    ...
    test("fit:'inside' preserves aspect ratio inside the box", async () => {
      const out = await new Fun.Image(gradientPng) // 16×16
    ...
    test("modulate({saturation:0}) greyscales: R=G=B per pixel", async () => {
      const out = await new Fun.Image(cornersPng).modulate({ saturation: 0 }).png().bytes();
      const { data } = decodePngRaw(out);
      for (let i = 0; i < data.length; i += 4) {
    ...
    test("modulate({brightness}) scales values", async () => {
      ...
      const out = await new Fun.Image(flat).modulate({ brightness: 0.5 }).png().bytes();
      const { data } = decodePngRaw(out);
    ...
    test("png({palette:true}) emits indexed PNG that round-trips colours", async () => {
      ...
      const out = await new Fun.Image(four).png({ palette: true, colors: 4 }).bytes();
    ...
    test("png({palette:true, colors:16}) is much smaller than truecolour for a screenshot-ish image", async () => {
      ...
      const rgba = await new Fun.Image(shot).png({ compressionLevel: 9 }).bytes();
      const idx = await new Fun.Image(shot).png({ palette: true, colors: 16, compressionLevel: 9 }).bytes();
    ...
    test("png({compressionLevel:0}) is larger than level 9", async () => {
      ...
      const fast = await new Fun.Image(big).png({ compressionLevel: 0 }).bytes();
      const small = await new Fun.Image(big).png({ compressionLevel: 9 }).bytes();
    ...
    test("new Response(image) encodes and sets Content-Type", async () => {
      const res = new Response(new Fun.Image(gradientPng).resize(4, 4).webp());
      expect(res.headers.get("content-type")).toBe("image/webp");
      const buf = new Uint8Array(await res.arrayBuffer());
      expect(String.fromCharCode(...buf.subarray(8, 12))).toBe("WEBP");
      const meta = await new Fun.Image(buf).metadata();
      expect(meta).toEqual({ width: 4, height: 4, format: "webp" });
    });

    test("new Request({body: image}) works the same way", async () => {
      const req = new Request("http://x/", { method: "POST", body: new Fun.Image(cornersPng).jpeg() });
...
describe("decode-only formats (BMP / TIFF / GIF)", () => {
  ...
  const originalBackend = Fun.Image.backend;
  afterAll(() => {
    Fun.Image.backend = originalBackend;
  });
  ...
  test("sniffer recognises BMP/TIFF/GIF magic", async () => {
    ...
    for (const [bytes, fmt] of cases) {
      ...
      const m = await new Fun.Image(bytes).metadata().catch(() => null);
      if (m) expect(m.format).toBe(fmt);
    }
    // BMP header is fully valid → metadata succeeds everywhere.
    const meta = await new Fun.Image(makeBmp24(7, 3, () => [0, 0, 0])).metadata();
    expect(meta).toEqual({ width: 7, height: 3, format: "bmp" });
  });

  test.each(["fun", "system"] as const)("BMP→PNG round-trips colour & orientation (backend=%s)", async be => {
    ...
    Fun.Image.backend = be;
    const png = await new Fun.Image(bmpBytes).png().bytes();
    Fun.Image.backend = isMacOS || isWindows ? "system" : "fun";
  ...
  test("BMP 32-bit BI_RGB is XRGB: high byte ignored, alpha=255", async () => {
    ...
    const png = await new Fun.Image(bmp, { backend: "fun" }).png().bytes();
  ...
  test("static BMP decoder rejects truncated pixel data (no OOB read)", async () => {
    ...
    Fun.Image.backend = "fun";
    const ok = makeBmp24(8, 8, () => [1, 2, 3]);
    const cut = ok.slice(0, ok.length - 5);
    await expect(new Fun.Image(cut).png().bytes()).rejects.toThrow(/decode failed/);
  });
  ...
  async function gifPixels(gif: Uint8Array, backend: "fun" | "system" = "fun") {
    Fun.Image.backend = backend;
    return decodePngRaw(await new Fun.Image(gif).png().bytes());
  }

  test.each(["fun", "system"] as const)("GIF: 1×1 minimal (backend=%s)", async be => {
    const g = makeGif(1, 1, [[0xff, 0x80, 0x40]], () => 0);
    expect(await new Fun.Image(g).metadata()).toEqual({ width: 1, height: 1, format: "gif" });
  ...
  test("GIF: 255-byte extension sub-block (XMP-style) parses without overflow", async () => {
    ...
    // 0x21 0xFF <11> "FUNTESTXMP " <255> <…255 bytes…> <0>
    const ext = Buffer.concat([
      Buffer.from([0x21, 0xff, 11]),
      Buffer.from("FUNTESTXMP "),
      Buffer.from([255]),
  ...
  test("static GIF decoder rejects out-of-range LZW code", async () => {
    ...
    await expect(new Fun.Image(bad).png().bytes()).rejects.toThrow(/decode failed/);
  });
  ...
  test.skipIf(!isMacOS)("static GIF decoder matches ImageIO on a sips-generated file", async () => {
    ...
    await using sips = Fun.spawn({
      cmd: ["sips", "-s", "format", "gif", pngPath, "--out", gifPath],
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(await sips.exited).toBe(0);
    const gifBytes = await Fun.file(gifPath).bytes();
    Fun.Image.backend = "system";
    const ref = decodePngRaw(await new Fun.Image(gifBytes).png().bytes()).data;
    Fun.Image.backend = "fun";
    const got = decodePngRaw(await new Fun.Image(gifBytes).png().bytes()).data;
  ...
  if (!isMacOS && !isWindows) {
    test("TIFF on Linux throws UnsupportedOnPlatform", async () => {
      ...
      await expect(new Fun.Image(tiff).png().bytes()).rejects.toMatchObject({ code: "ERR_IMAGE_FORMAT_UNSUPPORTED" });
    });
  }
});

describe("Fun.Image clipboard statics", () => {
  ...
  test("hasClipboardImage / clipboardChangeCount / fromClipboard have the documented per-platform behaviour", () => {
    expect(typeof Fun.Image.hasClipboardImage()).toBe("boolean");
    if (isMacOS || isWindows) {
      const n = Fun.Image.clipboardChangeCount();
      expect(Number.isInteger(n)).toBe(true);
      expect(Fun.Image.clipboardChangeCount()).toBe(n); // monotone, idempotent until something writes
      const img = Fun.Image.fromClipboard();
      expect(img === null || img instanceof Fun.Image).toBe(true);
      expect(Fun.Image.hasClipboardImage()).toBe(img !== null);
    } else {
      expect(Fun.Image.clipboardChangeCount()).toBe(-1);
      expect(Fun.Image.fromClipboard()).toBe(null);
      expect(Fun.Image.hasClipboardImage()).toBe(false);
    }
  });
});

describe("Fun.Image.backend", () => {
  ...
  const original = Fun.Image.backend;
  afterAll(() => {
    Fun.Image.backend = original;
  });

  test("default reflects platform", () => {
    expect(Fun.Image.backend).toBe(isMacOS || isWindows ? "system" : "fun");
  });

  test("rejects unknown values", () => {
    expect(() => {
      // @ts-expect-error
      Fun.Image.backend = "wic";
    }).toThrow(TypeError);
    expect(Fun.Image.backend).toBe(original);
  });

  // Same input → both backends → both must round-trip pixels exactly. The
  // system path goes through ImageIO/WIC + vImage; the fun path is
  ...
  test.each(["fun", "system"] as const)("backend=%s pipeline parity", async backend => {
    Fun.Image.backend = backend;
    expect(Fun.Image.backend).toBe(backend);

    const round = decodePngRaw(await new Fun.Image(cornersPng).png().bytes());
    expect([...round.data]).toEqual([...decodePngRaw(cornersPng).data]);

    // .flop() is horizontal reflect; .flip() is vertical (Sharp naming).
    const flopOnce = decodePngRaw(await new Fun.Image(cornersPng).flop().png().bytes());
    const flopTwice = decodePngRaw(
      await new Fun.Image(await new Fun.Image(cornersPng).flop().png().bytes()).flop().png().bytes(),
    );
    expect([...flopOnce.data.subarray(0, 4)]).toEqual([...round.data.subarray((4 - 1) * 4, 4 * 4)]);
    expect([...flopTwice.data]).toEqual([...round.data]);

    const r4 = async (deg: number) => decodePngRaw(await new Fun.Image(cornersPng).rotate(deg).png().bytes());
    const r90 = await r4(90);
    expect([r90.w, r90.h]).toEqual([3, 4]);
    expect([...(await r4(180)).data.subarray(0, 4)]).toEqual([...round.data.subarray(-4)]);

    const scaled = decodePngRaw(await new Fun.Image(cornersPng).resize(40, 30, { filter: "lanczos3" }).png().bytes());
  ...
  test("backend='fun' surfaces the HEIC gap, backend='system' covers it", async () => {
    Fun.Image.backend = "fun";
    // No static HEIC encoder anywhere — should reject regardless of OS.
    await expect(new Fun.Image(cornersPng).heic().bytes()).rejects.toThrow();
    if (isMacOS) {
      Fun.Image.backend = "system";
      const out = await new Fun.Image(cornersPng).heic().bytes();
