// Adversarial / hardening suite for Fun.Image.
//
...
// Several tests below force `backend = "fun"` to reach the static decoders
...
const defaultBackend = Fun.Image.backend;
afterEach(() => {
  Fun.Image.backend = defaultBackend;
});
...
/// Decode any image to RGBA via Fun.Image→PNG, then walk the PNG ourselves
/// (filter de-prediction included) so assertions are against ground truth,
/// not against another Fun.Image call. Hoisted to file scope so the
/// heap-leak / hostile-input tests can use it.
async function rgbaOf(bytes: Uint8Array): Promise<Uint8Array> {
  const png = await new Fun.Image(bytes).png().bytes();
...
const tinyJpeg = await new Fun.Image(tinyPng).jpeg({ quality: 80 }).bytes();
const tinyWebp = await new Fun.Image(tinyPng).webp({ quality: 80 }).bytes();
const tinyWebpLossless = await new Fun.Image(tinyPng).webp({ lossless: true }).bytes();
...
describe("format confusion", () => {
  ...
  test.each([
    ...
    expect(await survives(new Fun.Image(buf).metadata())).toBe("rejected");
  });
  ...
  test("PNG with valid JPEG appended (polyglot-ish)", async () => {
    ...
    const meta = await new Fun.Image(poly).metadata();
...
describe("truncation sweep", () => {
  ...
  for (const [name, fixture] of [
    ...
    test.concurrent(`${name}: every prefix length 1..${fixture.length - 1}`, async () => {
      for (let n = 1; n < fixture.length; n++) {
        await survives(new Fun.Image(fixture.subarray(0, n)).metadata());
      }
    });
  }

  test("zero-length input", async () => {
    expect(await survives(new Fun.Image(new Uint8Array(0)).metadata())).toBe("rejected");
  });

  test("single byte of every value", async () => {
    for (let v = 0; v < 256; v++) await survives(new Fun.Image(new Uint8Array([v])).metadata());
  });
  ...
  test("GIF LZW with code > avail rejects (GIFLIB-CVE-style table overrun)", async () => {
    ...
    expect(await survives(new Fun.Image(gif).png().bytes())).toBe("rejected");
    ...
    expect(await survives(new Fun.Image(gif12).png().bytes())).toBe("rejected");
  });
  ...
  test("BMP BI_BITFIELDS with hostile masks rejects, not panics", async () => {
    ...
    for (const m of [0xffffffff, 0x01ffffff /* 25-bit */, 0x00ff00ff /* non-contiguous */]) {
      await expect(new Fun.Image(bmpWithMasks(m, 0x0000ff00, 0x000000ff, 0)).png().bytes()).rejects.toThrow(
        /decode failed/,
      );
    }
    // Sanity: a normal 8-bit mask still decodes.
    await new Fun.Image(bmpWithMasks(0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000)).png().bytes();
  });
  ...
  test("BMP biSize ≈ u32::MAX rejects (no `14 + ih_size` wrap)", async () => {
    ...
    await expect(new Fun.Image(buf).metadata()).rejects.toThrow(/decode failed/);
  });
});
...
describe("hostile header dimensions", () => {
  ...
  test.each([
    ...
    expect(await survives(new Fun.Image(pngWithDims(w, h)).metadata())).toBe("rejected");
  });
  ...
  test.each([
    ...
    await expect(new Fun.Image(pngWithDims(w, h), { maxPixels: 1e15 }).metadata()).rejects.toThrow(/decode failed/);
  });
  ...
  test("PNG IHDR claiming bit-depth 0 / colour-type 99", async () => {
    for (const [off, val] of [
      ...
      expect(await survives(new Fun.Image(buf).bytes())).toBe("rejected");
    }
  });
  ...
  test("path source: non-regular file rejects with ENODEV (no infinite read / FIFO park)", async () => {
    ...
    await expect(new Fun.Image("/dev/null").metadata()).rejects.toThrow(/ENODEV|not a/i);
  });
  ...
  test("resize H-then-V intermediate (dst_w × src_h) is bounded by maxPixels", async () => {
    ...
    await expect(new Fun.Image(tall).resize(200000, 1).bytes()).rejects.toThrow(/maxPixels/);
    // And with maxPixels low enough that even modest intermediates trip:
    const small = makePng(1, 64, () => [0, 0, 0, 255]);
    await expect(new Fun.Image(small, { maxPixels: 1000 }).resize(100, 1).bytes()).rejects.toThrow(/maxPixels/);
    // Sanity: a small intermediate still works.
    expect((await new Fun.Image(small).resize(4, 4).png().bytes())[0]).toBe(0x89);
  });
  ...
  test("WebP VP8 frame header with absurd dims", async () => {
    ...
    expect(await survives(new Fun.Image(Buffer.concat([riff, vp8])).bytes())).toBe("rejected");
  });
});
...
describe("malformed PNG structure", () => {
  ...
  test("IHDR CRC mismatch is tolerated (spng default ignores CRC)", async () => {
    const buf = Buffer.from(tinyPng);
    buf[29] ^= 0xff;
    expect(await new Fun.Image(buf).metadata()).toEqual({ width: 2, height: 2, format: "png" });
  });
  ...
  test("IDAT with zlib bomb (header says small, IDAT inflates huge)", async () => {
    ...
    const meta = await new Fun.Image(png).metadata();
...
describe("malformed JPEG", () => {
  ...
  test("EXIF with IFD0 offset pointing outside the segment", async () => {
    ...
    const app1 = Buffer.concat([Buffer.from([0xff, 0xe1, seglen >> 8, seglen & 255]), exif]);
    const withExif = Buffer.concat([tinyJpeg.subarray(0, 2), app1, tinyJpeg.subarray(2)]);
    expect((await new Fun.Image(withExif).metadata()).format).toBe("jpeg");
  });
});
...
describe("lossless roundtrip", () => {
  ...
  // libwebp size optimisation, not a Fun bug). PNG preserves RGB under α=0;
  ...
  test("PNG → PNG preserves every byte of noise", async () => {
    const out = await new Fun.Image(noise).png().bytes();
    expect(Buffer.compare(await rgbaOf(out), ref)).toBe(0);
  });

  test("PNG → lossless WebP → PNG preserves every byte of noise (α≥1)", async () => {
    const wp = await new Fun.Image(noise).webp({ lossless: true }).bytes();
    const back = await new Fun.Image(wp).png().bytes();
  ...
  test("WebP-lossless zeroes RGB under α=0 (documented libwebp behaviour)", async () => {
    ...
    const wp = await new Fun.Image(one).webp({ lossless: true }).bytes();
  ...
  test("rotate(90)×4 = identity", async () => {
    ...
    for (let i = 0; i < 4; i++) cur = await new Fun.Image(cur).rotate(90).png().bytes();
    expect(Buffer.compare(await rgbaOf(cur), ref)).toBe(0);
  });

  test("flip().flip() = identity, flop().flop() = identity", async () => {
    const a = await new Fun.Image(await new Fun.Image(noise).flip().png().bytes()).flip().png().bytes();
    const b = await new Fun.Image(await new Fun.Image(noise).flop().png().bytes()).flop().png().bytes();
...
describe("memory hygiene", () => {
  ...
  test("decode/encode cycles plateau (no per-call leak after warmup)", async () => {
    const delta = await leakCheck(() => new Fun.Image(tinyPng).png().bytes());
  ...
  test("error paths plateau (no per-call leak after warmup)", async () => {
    ...
    const delta = await leakCheck(() => survives(new Fun.Image(bad).metadata()));
    expect(delta).toBeLessThan(32 * 1024 * 1024);
  });
...
describe("hostile option objects", () => {
  test("Proxy that throws on every property access", async () => {
    ...
    expect(() => new Fun.Image(tinyPng).resize(2, 2, p as any)).toThrow();
    expect(() => new Fun.Image(tinyPng).jpeg(p as any)).toThrow();
    expect(() => new Fun.Image(tinyPng, p as any)).toThrow();
  });
  ...
  test("garbage option types: enum slots throw, numeric slots ignore non-numbers", async () => {
    ...
    expect(() => new Fun.Image(tinyPng).resize(2, 2, { filter: 12345 } as any)).toThrow(/filter must be a string/);
    expect(() => new Fun.Image(tinyPng).resize(2, 2, { fit: [] } as any)).toThrow(/fit must be a string/);
    // A string-coercible object isn't a JS string — refused, not coerced.
    expect(() => new Fun.Image(tinyPng).resize(2, 2, { fit: { toString: () => "inside" } } as any)).toThrow(
  ...
  test("detached ArrayBuffer input", async () => {
    ...
    expect(await survives(new Fun.Image(ab).metadata())).toBe("rejected");
  });
  ...
  test("resizable ArrayBuffer is rejected at construction", () => {
    ...
    expect(() => new Fun.Image(ab)).toThrow(/resizable/);
    // …and a view into one is rejected the same way.
    expect(() => new Fun.Image(new Uint8Array(ab))).toThrow(/resizable/);
  });

  test("detach AFTER construction rejects the next terminal", async () => {
    const ab = tinyPng.buffer.slice(tinyPng.byteOffset, tinyPng.byteOffset + tinyPng.byteLength);
    const img = new Fun.Image(ab);
  ...
  test("OversizeTypedArray input survives `.buffer` → transfer() while worker decodes (pin-after-adopt)", async () => {
    ...
    a.set(tinyPng);
    const p = new Fun.Image(a.subarray(0, tinyPng.length)).png().bytes();
  ...
  test("SharedArrayBuffer input is refused (cross-thread mutation surface)", () => {
    ...
    expect(() => new Fun.Image(sab)).toThrow(/shared/);
    expect(() => new Fun.Image(new Uint8Array(sab))).toThrow(/shared/);
  });

  test("data: URL input (base64)", async () => {
    const url = "data:image/png;base64," + Buffer.from(tinyPng).toString("base64");
    const meta = await new Fun.Image(url).metadata();
    expect(meta).toEqual({ width: 2, height: 2, format: "png" });
  });

  test("data: URL with bad base64 throws", () => {
    expect(() => new Fun.Image("data:image/png;base64,!!!not base64!!!")).toThrow(/base64/);
  });
});
...
describe("concurrent terminals on one Image", () => {
  test("100 concurrent .bytes() on the same instance", async () => {
    const img = new Fun.Image(tinyPng).png();
  ...
  test("concurrent terminals on a Fun.file source — first BlobReadChain wins, later resolvers don't free it", async () => {
    ...
    await Fun.write(p, tinyPng);
    const img = new Fun.Image(Fun.file(p)).png();
...
describe("random-byte fuzz", () => {
  ...
  test("256 random buffers of varying length never crash", async () => {
    for (let i = 0; i < 256; i++) {
      ...
      await survives(new Fun.Image(fuzz(i, len)).metadata());
    }
  });
  ...
  for (const [name, fixture] of [
    ...
    test.concurrent(`${name}: single-byte flip at every offset`, async () => {
      for (let off = 0; off < fixture.length; off++) {
        ...
        mut[off] ^= 0xff;
        await survives(new Fun.Image(mut).bytes());
      }
    });
  }
});
