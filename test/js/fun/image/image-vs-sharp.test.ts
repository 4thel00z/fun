// Pixel-level cross-reference: Fun.Image vs Sharp/libvips.
//
...
// about: does Fun.Image produce the same pixels as Sharp for the same kernel?
//
...
//   cd bench/image && fun install
//   fun-release ../../test/js/fun/image/image-vs-sharp.test.ts --regenerate
...
const REGENERATE = Fun.argv.includes("--regenerate");
...
describe("Fun.Image kernel output ≈ Sharp/libvips reference", () => {
  ...
  for (const c of cases) {
    ...
    test(`${c.src} ${c.w}×${c.h} ${c.filter}: MAE<${c.mae} max≤${c.max}`, async () => {
      const out = decodePng(
        await new Fun.Image(makePng(W, H, sources[c.src]))
