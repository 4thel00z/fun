// Exercises Fun's SIMD code paths to verify the baseline binary doesn't
...
describe("escapeHTML — @Vector(16, u8) gated by enableSIMD", () => {
  test("clean passthrough", () => {
    expect(Fun.escapeHTML(ascii256)).toBe(ascii256);
  });

  test("ampersand in middle", () => {
    const input = ascii256 + "&" + ascii256;
    const escaped = Fun.escapeHTML(input);
...
describe("stringWidth — @Vector(16, u8) ungated", () => {
  test("ascii", () => {
    expect(Fun.stringWidth(ascii256)).toBe(256);
  });
  ...
  test("tabs", () => {
    expect(Fun.stringWidth(Buffer.alloc(32, "\t").toString())).toBe(0);
  });

  test("mixed printable and zero-width", () => {
    const mixed = "hello" + "\x00".repeat(64) + "world";
    expect(Fun.stringWidth(mixed)).toBe(10);
  });
});
