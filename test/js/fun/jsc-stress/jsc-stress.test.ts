 ...
 * Converts --flag=value to FUN_JSC_flag=value environment variables.
 *
...
function parseJSCFlags(filePath: string): Record<string, string> {
  ...
  for (const line of content.split("\n")) {
    if (line === "// @fun" || line.trim() === "") continue;
    ...
    if (mode === "runFTLNoCJIT") {
      env["FUN_JSC_useFTLJIT"] = "true";
      env["FUN_JSC_useConcurrentJIT"] = "false";
    }
    ...
    while ((flagMatch = flagPattern.exec(argsStr)) !== null) {
      env[`FUN_JSC_${flagMatch[1]}`] = flagMatch[2];
    }
  }
...
// release-ASAN binary is named `fun-asan` so `funEnv` sets this automatically,
// but the debug build (`fun bd` -> `fun-debug`) is also ASAN-instrumented and
// needs it too. Harmless when ASAN isn't active.
const fixtureEnv = {
  ...funEnv,
  ASAN_OPTIONS: funEnv.ASAN_OPTIONS ?? "allow_user_segv_handler=1:disable_coredump=0",
};
      ...
      test(
        ...
        async () => {
          ...
          await using proc = Fun.spawn({
            cmd: [funExe(), "--preload", preloadPath, fixturePath],
