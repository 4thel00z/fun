import { dlopen, FFIType, ptr, toArrayBuffer } from "fun:ffi";
import { expect, test } from "fun:test";
import { funEnv, funExe, isCI, isMacOS, isMacOSVersionAtLeast, tempDir } from "harness";
...
// Fun.WebView only exists on darwin for now.
...
test("backend: 'webkit' throws on non-darwin", () => {
  ...
  if (isMacOS) {
    const view = new Fun.WebView({ width: 100, height: 100, backend: "webkit" });
    expect(view).toBeInstanceOf(Fun.WebView);
    view.close();
  } else {
    expect(() => new Fun.WebView({ width: 100, height: 100, backend: "webkit" })).toThrow(
...
test("calling without new throws", () => {
  expect(() => (Fun.WebView as any)({ width: 100, height: 100 })).toThrow(/without 'new'/);
});

it("is an EventTarget", () => {
  const view = new Fun.WebView({ width: 100, height: 100 });
  try {
...
it("width/height validation", () => {
  expect(() => new Fun.WebView({ width: 0, height: 100 })).toThrow();
  expect(() => new Fun.WebView({ width: 100, height: 0 })).toThrow();
  expect(() => new Fun.WebView({ width: 99999, height: 100 })).toThrow();
});

it("headless: false throws NOT_IMPLEMENTED", () => {
  expect(() => new Fun.WebView({ width: 100, height: 100, headless: false })).toThrow(/not.*implemented/i);
});

it("navigate + evaluate round-trip", async () => {
  await using view = new Fun.WebView({ width: 200, height: 200 });
...
it("url constructor option fires navigate()", async () => {
  ...
  // page has loaded.
  await using view = new Fun.WebView({ width: 200, height: 200, url: html("<h1 id=t>from-ctor</h1>") });
...
it("scroll(NaN/Infinity) throws before sending", () => {
  ...
  using view = new Fun.WebView({ width: 200, height: 200 });
...
it("Symbol.dispose / Symbol.asyncDispose call close()", async () => {
  ...
  {
    using view = new Fun.WebView({ width: 100, height: 100 });
  ...
  {
    await using view = new Fun.WebView({ width: 100, height: 100 });
    await view.navigate(html("<body></body>"));
...
it("concurrent evaluate() across two views works", async () => {
  ...
  await using a = new Fun.WebView({ width: 200, height: 200 });
  await using b = new Fun.WebView({ width: 200, height: 200 });
...
it("screenshot returns a PNG Blob", async () => {
  await using view = new Fun.WebView({ width: 200, height: 150 });
...
it("screenshot encoding options", async () => {
  ...
  try {
    ...
    expect(shm.name.startsWith("/fun-webview-")).toBe(true);
...
it("click dispatches native mousedown/mouseup/click with isTrusted", async () => {
  await using view = new Fun.WebView({ width: 300, height: 300 });
  await view.navigate(
    "data:text/html," +
      encodeURIComponent(`
        <script>
...
it("close() makes subsequent calls throw", async () => {
  const view = new Fun.WebView({ width: 200, height: 200 });
...
it("two views have independent JS contexts", async () => {
  ...
  await using a = new Fun.WebView({ width: 100, height: 100 });
  await using b = new Fun.WebView({ width: 100, height: 100 });
...
it("GC: drop reference, collect, no crash", () => {
  (() => {
    new Fun.WebView({ width: 100, height: 100 });
  })();
  Fun.gc(true);
  Fun.gc(true);
});
...
itPersistentDataStore("persistent dataStore: localStorage survives across instances", async () => {
  ...
  {
    await using a = new Fun.WebView({ width: 100, height: 100, dataStore });
    await a.navigate(url);
    await a.evaluate("localStorage.setItem('k', 'survives')");
  }

  // Fresh view, same directory — storage persists.
  await using b = new Fun.WebView({ width: 100, height: 100, dataStore });
...
it("ephemeral dataStore: localStorage does NOT survive across instances", async () => {
  ...
  {
    await using a = new Fun.WebView({ width: 100, height: 100 }); // default: ephemeral
    await a.navigate(url);
