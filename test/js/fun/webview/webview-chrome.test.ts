...
import { funEnv, funExe, isCI, isMacOS, isMacOSVersionAtLeast } from "harness";
...
// runtime would.
import { dlopen, FFIType, ptr } from "fun:ffi";
...
function findChrome(): string | undefined {
  ...
  if (process.env.FUN_CHROME_PATH) {
    return isExecutable(process.env.FUN_CHROME_PATH) ? process.env.FUN_CHROME_PATH : undefined;
  }
...
it("backend: chrome constructor returns a WebView", () => {
  const view = new Fun.WebView({ backend: chrome, width: 400, height: 300 });
  expect(view).toBeInstanceOf(Fun.WebView);
  view.close();
});

it("chrome: navigate + evaluate round-trip", async () => {
  await using view = new Fun.WebView({ backend: chrome, width: 400, height: 300 });
...
it("chrome: evaluate returns native JS values", async () => {
  await using view = new Fun.WebView({ backend: chrome, width: 200, height: 200 });
...
it("chrome: screenshot returns a PNG Blob", async () => {
  ...
  // Fun.write accepts the Blob directly — the MIME type carries through.
...
it("chrome: screenshot encoding options", async () => {
  ...
  // fun-chrome- prefix to disambiguate from WebKit's child-created
  // segments.
  if (process.platform !== "win32") {
    const shm = await view.screenshot({ encoding: "shmem" });
    expect(typeof shm.name).toBe("string");
    expect(shm.name.startsWith("/fun-chrome-")).toBe(true);
...
it("chrome: cdp() guards — before navigate and params validation", async () => {
  ...
  const crView = new Fun.WebView({ backend: chrome, width: 100, height: 100 });
  try {
...
it("chrome: click dispatches mousedown/mouseup/click", async () => {
  await using view = new Fun.WebView({ backend: chrome, width: 300, height: 300 });
  await view.navigate(
    html(`
      <script>
        window.__ev = [];
...
it("chrome: close() rejects pending promises", async () => {
  const view = new Fun.WebView({ backend: chrome, width: 200, height: 200 });
  await view.navigate(html("<body></body>"));
...
it("chrome: two views have independent sessions", async () => {
  const a = new Fun.WebView({ backend: chrome, width: 200, height: 200 });
  const b = new Fun.WebView({ backend: chrome, width: 200, height: 200 });
  try {
...
it("chrome: closeAll() kills the subprocess and pending promises reject", async () => {
  ...
  await using proc = Fun.spawn({
    cmd: [
      ...
      "-e",
      `
        const view = new Fun.WebView({ backend: {type:"chrome", url:false}, width: 200, height: 200 });
...
test("backend.stderr validates", () => {
  expect(() => new Fun.WebView({ backend: { type: "chrome", stderr: "pipe" as any } })).toThrow(
    /must be "inherit" or "ignore"/,
  );
  expect(() => new Fun.WebView({ backend: { type: "chrome", stderr: 123 as any } })).toThrow(
    /must be "inherit" or "ignore"/,
  );
  expect(() => new Fun.WebView({ backend: { type: "chrome", stdout: "foo" as any } })).toThrow(
    /must be "inherit" or "ignore"/,
  );
});

test("backend option validates", () => {
  expect(() => new Fun.WebView({ backend: "invalid" as any })).toThrow(/webkit.*chrome/i);
  expect(() => new Fun.WebView({ backend: { type: "invalid" } as any })).toThrow(/webkit.*chrome/i);
  expect(() => new Fun.WebView({ backend: { type: "chrome", path: 123 } as any })).toThrow(/path must be a string/);
  expect(() => new Fun.WebView({ backend: { type: "chrome", argv: [1] } as any })).toThrow(
...
it("backend: { type: 'chrome' } object form works", async () => {
  ...
  await using view = new Fun.WebView({ backend: { type: "chrome", path: chromePath }, width: 200, height: 200 });
  ...
  await using proc = Fun.spawn({
    cmd: [
      ...
      const view = new Fun.WebView({
        backend: { type: "chrome", argv: ["--user-agent=FunWebViewTest/1.0"] },
        width: 200, height: 200,
      });
      await view.navigate("data:text/html,<body></body>");
      const ua = await view.evaluate("navigator.userAgent");
      if (ua !== "FunWebViewTest/1.0") throw new Error("got UA: " + ua);
      view.close();
      console.log("ok");
      `,
    ],
...
it("chrome: close() during attach chain doesn't leak the tab", async () => {
  ...
  await using view = new Fun.WebView({ backend: chrome, width: 100, height: 100 });
...
it("chrome: console callback receives (type, ...args)", async () => {
  ...
  await using view = new Fun.WebView({
    backend: chrome,
    width: 200,
    height: 200,
...
it("chrome: console: globalThis.console forwards to parent's stdout", async () => {
  ...
  // ConsoleClient::logWithLevel — Fun's formatter applies. Log → stdout,
  // Error → stderr. The args forward with Fun's util.inspect formatting.
...
it("chrome: console option validates", () => {
  expect(() => new Fun.WebView({ backend: chrome, console: 42 } as any)).toThrow(
    /console must be globalThis.console or a function/,
  );
  expect(() => new Fun.WebView({ backend: chrome, console: {} } as any)).toThrow(
    /console must be globalThis.console or a function/,
  );
});
