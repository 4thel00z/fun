import { internalSourceMap } from "fun:internal-for-testing";
...
`,
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "build", "./index.ts", "--sourcemap=external", "--outdir=./out"],
      env: funEnv,
      cwd: String(dir),
      stderr: "pipe",
    });
    ...
    const map = await Fun.file(path.join(String(dir), "out", "index.js.map")).json();
