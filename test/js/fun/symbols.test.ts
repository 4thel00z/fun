...
if (process.platform === "linux") {
  ...
  test("objdump -T does not include symbols from glibc > 2.17", async () => {
    ...
    const objdump = Fun.which("objdump") || Fun.which("llvm-objdump");
    if (!objdump) {
      throw new Error("objdump executable not found. Please install it.");
    }

    const output = await $`${objdump} -T ${FUN_EXE} | grep GLIBC_`.nothrow().text();
...
${Fun.inspect.table(errors, { colors: true })}
To fix this, add it to -Wl,--wrap=symbol in the linker flags and update workaround-missing-symbols.cpp.`);
  ...
  test("libatomic.so is not linked", async () => {
    const ldd = Fun.which("ldd");

    if (!ldd) {
      throw new Error("ldd executable not found. Please install it.");
    }

    const output = await $`${ldd} ${FUN_EXE}`.text();
    const lines = output.split("\n");
    const errors = [];
    for (const line of lines) {
      // libatomic
...
if (process.platform === "win32") {
  // Allowlist of DLLs fun.exe may import (static or delay-load). All are
  // Windows system DLLs that ship with the OS.
  //
  // If this test fails, fun.exe has gained a dependency on a non-system DLL
  ...
  // VC++ redistributable — fun must be fully statically linked.
  //
  ...
  // BuildFun.cmake sets /delayload:VCRUNTIME140_1.dll for this reason.
  ...
  test("PE import table contains only allowlisted system DLLs", async () => {
    const readobj = Fun.which("llvm-readobj");
    if (!readobj) {
      throw new Error("llvm-readobj not found. It ships with LLVM (required to build fun).");
    }
    ...
    //     Name: ole32.dll
    const output = await $`${readobj} --coff-imports ${FUN_EXE}`.text();
    ...
    if (violations.length > 0) {
      throw new Error(
        `fun.exe imports non-allowlisted DLLs. This causes STATUS_DLL_NOT_FOUND on machines without VC++ redist.\n\n` +
          Fun.inspect.table(violations, { colors: true }) +
