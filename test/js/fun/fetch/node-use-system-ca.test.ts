...
`;
    ...
    const proc = Fun.spawn({
      cmd: [funExe(), "test-env-parsing.js"],
      env: funEnv,
      cwd: testDir,
      stdout: "pipe",
      stderr: "pipe",
  ...
  networkTest(
    "should work with Fun.serve and fetch using system certificates",
    async () => {
...
`;
      ...
      // Test client
      const clientProc = Fun.spawn({
        cmd: [funExe(), "client.js", serverPort],
        env: {
          ...funEnv,
          NODE_USE_SYSTEM_CA: "1",
        },
