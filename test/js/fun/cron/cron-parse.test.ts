...
// Fun.cron.parse() and the in-process Fun.cron(schedule, handler) interpret
// schedules in UTC. The OS-level Fun.cron(path, schedule, title) overload
// uses the system's local time zone (crontab/launchd/schtasks all do).

const parse = (expr: string, from: string) => Fun.cron.parse(expr, new Date(from))!.toISOString();

describe("Fun.cron.parse — UTC", () => {
    ...
    for (const tz of ["America/Los_Angeles", "Asia/Tokyo", "UTC"]) {
      await using proc = Fun.spawn({
        cmd: [
          funExe(),
          "-e",
          `process.stdout.write(Fun.cron.parse("0 9 * * *", new Date("2026-06-15T00:00:00Z")).toISOString())`,
        ],
        env: { ...funEnv, TZ: tz },
      });
  ...
  test("impossible day/month (Feb 30) returns null quickly", () => {
    ...
    expect(Fun.cron.parse("0 0 30 2 *", new Date("2026-01-01T00:00:00Z"))).toBeNull();
...
describe("Fun.cron.parse — weekday 7 = Sunday in ranges", () => {
