...
import { funEnv, funExe, isLinux, isMacOS, isWindows, tempDir } from "harness";
import { existsSync, readFileSync, unlinkSync, writeFileSync } from "node:fs";

const crontabPath = Fun.which("crontab");
const hasCrontab = !!crontabPath && isLinux;
const hasSchtasks =
  isWindows &&
  (() => {
    try {
      // Test the XML-based path that Fun.cron actually uses.
      ...
      const xmlPath = `${process.env.TEMP || "C:\\Temp"}\\fun-cron-probe.xml`;
      writeFileSync(xmlPath, xml);
      try {
        const r = Fun.spawnSync({
          cmd: ["schtasks", "/create", "/xml", xmlPath, "/tn", "fun-cron-probe", "/np", "/f"],
          stdout: "pipe",
          stderr: "pipe",
        });
        if (r.exitCode !== 0) return false;
        Fun.spawnSync({
          cmd: ["schtasks", "/delete", "/tn", "fun-cron-probe", "/f"],
          stdout: "ignore",
          stderr: "ignore",
        });
        return true;
...
function writeCrontab(content: string) {
  const tmpFile = `/tmp/fun-cron-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`;
  writeFileSync(tmpFile, content);
  try {
    Fun.spawnSync({ cmd: [crontabPath!, tmpFile] });
  } finally {
    try {
      unlinkSync(tmpFile);
    } catch {}
...
describe("Fun.cron API", () => {
  test("is a function", () => {
    expect(typeof Fun.cron).toBe("function");
  });

  test("has .remove method", () => {
    expect(typeof Fun.cron.remove).toBe("function");
  });

  test("has .parse method", () => {
    expect(typeof Fun.cron.parse).toBe("function");
  });

  test("throws with no arguments", () => {
    // @ts-ignore
    expect(() => Fun.cron()).toThrow();
  });

  test("throws with non-string path", () => {
    // @ts-ignore
    expect(() => Fun.cron(123, "* * * * *", "test-bad")).toThrow();
  });

  test("throws with non-string schedule", () => {
    // @ts-ignore
    expect(() => Fun.cron("./test.ts", 123, "test-bad")).toThrow();
  });

  test("throws with non-string title", () => {
    // @ts-ignore
    expect(() => Fun.cron("./test.ts", "* * * * *", 123)).toThrow();
  });

  test("remove throws with non-string title", () => {
    // @ts-ignore
    expect(() => Fun.cron.remove(123)).toThrow();
  });

  test("throws with invalid title characters", () => {
    expect(() => Fun.cron("./test.ts", "* * * * *", "bad title!")).toThrow(/alphanumeric/);
    expect(() => Fun.cron("./test.ts", "* * * * *", "bad/title")).toThrow(/alphanumeric/);
    expect(() => Fun.cron("./test.ts", "* * * * *", "")).toThrow(/alphanumeric/);
  });

  test("throws with invalid cron expression", () => {
    expect(() => Fun.cron("./test.ts", "not a cron", "test-bad")).toThrow(/cron expression/i);
    expect(() => Fun.cron("./test.ts", "* * *", "test-bad")).toThrow(/cron expression/i);
    expect(() => Fun.cron("./test.ts", "* * * * * *", "test-bad")).toThrow(/cron expression/i);
    expect(() => Fun.cron("./test.ts", "abc * * * *", "test-bad")).toThrow(/cron expression/i);
  });

  test("throws with percent sign in path", async () => {
    using dir = tempDir("fun-cron-test", {
      "test%file.ts": `export default { scheduled() {} };`,
    });
    expect(() => Fun.cron(`${dir}/test%file.ts`, "* * * * *", "test-bad")).toThrow(/percent/i);
  });

  test("remove throws with invalid title characters", () => {
    expect(() => Fun.cron.remove("bad title!")).toThrow(/alphanumeric/);
  });
});
...
describe.skipIf(!hasAnyCronBackend)("cross-platform API consistency", () => {
  test("@daily nickname registers successfully", async () => {
    ...
    const result = await Fun.cron(`${dir}/job.ts`, "@daily", "test-xplat-daily");
    expect(result).toBeUndefined();
    // Clean up
    await Fun.cron.remove("test-xplat-daily");
  });
  ...
  test("@weekly nickname registers successfully", async () => {
    ...
    const result = await Fun.cron(`${dir}/job.ts`, "@weekly", "test-xplat-weekly");
    expect(result).toBeUndefined();
    await Fun.cron.remove("test-xplat-weekly");
  });
  ...
  test("@hourly nickname registers successfully", async () => {
    ...
    const result = await Fun.cron(`${dir}/job.ts`, "@hourly", "test-xplat-hourly");
    expect(result).toBeUndefined();
    await Fun.cron.remove("test-xplat-hourly");
  });
  ...
  test("every-5-minutes registers successfully", async () => {
    ...
    const result = await Fun.cron(`${dir}/job.ts`, "*/5 * * * *", "test-xplat-5min");
    expect(result).toBeUndefined();
    await Fun.cron.remove("test-xplat-5min");
  });
  ...
  test("named weekday (Monday) normalizes and registers", async () => {
    ...
    const result = await Fun.cron(`${dir}/job.ts`, "30 9 * * Monday", "test-xplat-named");
    expect(result).toBeUndefined();
    await Fun.cron.remove("test-xplat-named");
  });

  test("path with spaces works", async () => {
    using dir = tempDir("fun cron spaces test", {
      "my job.ts": `export default { scheduled() {} };`,
    });
    const result = await Fun.cron(`${dir}/my job.ts`, "0 0 * * *", "test-xplat-spaces");
    expect(result).toBeUndefined();
    await Fun.cron.remove("test-xplat-spaces");
  });
  ...
  test("relative path resolves relative to caller file, not cwd (docs L139)", async () => {
    ...
    using dir = tempDir("fun-cron-caller-rel", {
      "worker.ts": `export default { scheduled() {} };`,
      "register.ts": `
        await Fun.cron("./worker.ts", "0 0 * * *", "test-xplat-caller-rel");
        console.log("registered");
      `,
    });
    // Make an unrelated cwd — must not contain worker.ts
    using cwdDir = tempDir("fun-cron-other-cwd", {});

    try {
      await using proc = Fun.spawn({
        cmd: [funExe(), `${dir}/register.ts`],
        env: funEnv,
        cwd: String(cwdDir),
        stdout: "pipe",
        stderr: "pipe",
      ...
      if (hasLaunchctl) {
        const plist = await Fun.file(
          `${process.env.HOME}/Library/LaunchAgents/fun.cron.test-xplat-caller-rel.plist`,
        ).text();
        ...
        expect(plist).toMatch(/fun-cron-caller-rel[^<]*[\\/]worker\.ts/);
        expect(plist).not.toContain("fun-cron-other-cwd");
      } else if (hasCrontab) {
        const crontab = Fun.spawnSync({ cmd: [Fun.which("crontab")!, "-l"], stdout: "pipe" }).stdout.toString();
        expect(crontab).toMatch(/fun-cron-caller-rel[^\n]*[\\/]worker\.ts/);
        expect(crontab).not.toContain("fun-cron-other-cwd");
      } else if (hasSchtasks) {
        const query = Fun.spawnSync({
          cmd: ["schtasks", "/query", "/tn", "fun-cron-test-xplat-caller-rel", "/xml"],
          stdout: "pipe",
        }).stdout.toString();
        expect(query).toMatch(/fun-cron-caller-rel[^<]*[\\/]worker\.ts/);
        expect(query).not.toContain("fun-cron-other-cwd");
      }
    } finally {
      await Fun.cron.remove("test-xplat-caller-rel");
    }
  });
  ...
  test("remove resolves undefined on success", async () => {
    ...
    await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-xplat-rm-val");
    const result = await Fun.cron.remove("test-xplat-rm-val");
    expect(result).toBeUndefined();
  });

  test("remove non-existent job resolves without error", async () => {
    const result = await Fun.cron.remove("test-xplat-nonexistent-12345");
    expect(result).toBeUndefined();
  });
});
...
describe.skipIf(!hasSchtasks)("Windows XML-based scheduling (complex expressions)", () => {
  test("@monthly registers successfully", async () => {
    ...
    try {
      const result = await Fun.cron(`${dir}/job.ts`, "@monthly", "test-win-monthly");
      expect(result).toBeUndefined();
  ...
  test("@yearly registers successfully", async () => {
    ...
    try {
      const result = await Fun.cron(`${dir}/job.ts`, "@yearly", "test-win-yearly");
      expect(result).toBeUndefined();
  ...
  test("complex range expression registers successfully", async () => {
    ...
    try {
      const result = await Fun.cron(`${dir}/job.ts`, "*/15 1-5 1,15 * 0-4", "test-win-complex");
      expect(result).toBeUndefined();
  ...
  test("day-of-month expression registers successfully", async () => {
    ...
    try {
      const result = await Fun.cron(`${dir}/job.ts`, "0 0 15 * *", "test-win-dom");
      expect(result).toBeUndefined();
...
describe.skipIf(!hasSchtasks)("Windows XML code paths", () => {
  test("ScheduleByMonthDayOfWeek: weekday with month restriction", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "0 9 * 6 1", "test-win-monthdow");
  ...
  test("OR-split: day-of-month AND day-of-week produce two triggers", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "0 0 15 * 5", "test-win-or-split");
  ...
  test("daily with month restriction", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "0 0 * 6 *", "test-win-daily-month");
  ...
  test("hourly repetition pattern", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "0 */3 * * *", "test-win-hourly-rep");
...
//
// These tests verify:
//   - On Windows: Fun.cron() rejects these patterns with an error about triggers
//   - On non-Windows: Fun.cron() registers them successfully
describe.skipIf(!hasAnyCronBackend)("Windows trigger-limit expressions", () => {
  ...
  for (const { expr, title, triggers } of nonDivisorSteps) {
    if (isWindows) {
      test(`${expr} throws on Windows (${triggers} triggers > 48)`, () => {
        ...
        expect(() => Fun.cron(`${dir}/job.ts`, expr, `test-win-${title}`)).toThrow(/too many triggers/i);
      });
    } else {
      test(`${expr} succeeds on ${isMacOS ? "macOS" : "Linux"}`, async () => {
        ...
        try {
          const result = await Fun.cron(`${dir}/job.ts`, expr, t);
          expect(result).toBeUndefined();
        } finally {
          await Fun.cron.remove(t);
        }
      });
    }
  }
  ...
  if (isWindows) {
    test("0,30 * 15 * 5 throws on Windows (2×24×2 = 96 triggers from OR-split)", () => {
      ...
      expect(() => Fun.cron(`${dir}/job.ts`, "0,30 * 15 * 5", "test-win-or-split")).toThrow(/too many triggers/i);
    });
  } else {
    test(`0,30 * 15 * 5 succeeds on ${isMacOS ? "macOS" : "Linux"} (OR semantics)`, async () => {
      ...
      try {
        const result = await Fun.cron(`${dir}/job.ts`, "0,30 * 15 * 5", t);
        expect(result).toBeUndefined();
      } finally {
        await Fun.cron.remove(t);
      }
...
describe.skipIf(!hasCrontab)("cron registration (Linux)", () => {
  test("accepts valid cron expressions", async () => {
    ...
    // Every minute
    await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-every-min");
    // Ranges, steps, lists
    await Fun.cron(`${dir}/job.ts`, "*/15 1-5 1,15 * 0-4", "test-complex");
    // Named days/months get normalized to numeric form in crontab
    await Fun.cron(`${dir}/job.ts`, "30 2 * * Monday", "test-named");
    const crontab = readCrontab();
    expect(crontab).toContain("# fun-cron: test-every-min");
    expect(crontab).toContain("# fun-cron: test-complex");
    expect(crontab).toContain("# fun-cron: test-named");
  ...
  test("registers a crontab entry with absolute path", async () => {
    ...
    await Fun.cron(scriptPath, "30 2 * * 1", "test-register");

    const crontab = readCrontab();
    expect(crontab).toContain("# fun-cron: test-register");
  ...
  test("crontab entry contains correct format", async () => {
    ...
    await Fun.cron(`${dir}/job.ts`, "15 3 * * 0", "test-format");

    const crontab = readCrontab();
    const lines = crontab.split("\n");
    const markerIdx = lines.findIndex((l: string) => l.includes("# fun-cron: test-format"));
  ...
  test("replaces existing entry with same title", async () => {
    ...
    await Fun.cron(`${dir}/job.ts`, "0 * * * *", "test-replace");
    await Fun.cron(`${dir}/job.ts`, "30 2 * * 1", "test-replace");

    const crontab = readCrontab();
    const count = (crontab.match(/# fun-cron: test-replace/g) || []).length;
    expect(count).toBe(1);
    expect(crontab).toContain("30 2 * * 1");
  ...
  test("registers multiple different cron jobs", async () => {
    ...
    await Fun.cron(`${dir}/a.ts`, "0 * * * *", "multi-a");
    await Fun.cron(`${dir}/b.ts`, "30 12 * * 5", "multi-b");

    const crontab = readCrontab();
    expect(crontab).toContain("# fun-cron: multi-a");
    expect(crontab).toContain("# fun-cron: multi-b");
    expect(crontab).toContain("0 * * * *");
    expect(crontab).toContain("30 12 * * 5");
  });

  test("preserves existing non-fun crontab entries", async () => {
    using _restore = saveCrontabState();
    // Add a manual crontab entry first
    await using setup = Fun.spawn({
      cmd: [crontabPath!, "-"],
      stdin: "pipe",
    });
    ...
    await Fun.cron(`${dir}/job.ts`, "*/5 * * * *", "test-preserve");

    const crontab = readCrontab();
    expect(crontab).toContain("/usr/bin/some-other-job");
    expect(crontab).toContain("# fun-cron: test-preserve");
  });
  ...
  test("returns a promise that resolves", async () => {
    ...
    const result = await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-promise");
    expect(result).toBeUndefined();
  });
});
...
describe.skipIf(!hasCrontab)("cron removal (Linux)", () => {
  test("removes an existing cron entry", async () => {
    ...
    await Fun.cron(`${dir}/job.ts`, "30 2 * * 1", "rm-target");

    let crontab = readCrontab();
    expect(crontab).toContain("# fun-cron: rm-target");

    await Fun.cron.remove("rm-target");

    crontab = readCrontab();
    expect(crontab).not.toContain("# fun-cron: rm-target");
    expect(crontab).not.toContain("30 2 * * 1");
  });

  test("removing non-existent entry resolves without error", async () => {
    using _restore = saveCrontabState();
    const result = await Fun.cron.remove("rm-nonexistent");
    expect(result).toBeUndefined();
  });
  ...
  test("removes only the targeted entry", async () => {
    ...
    await Fun.cron(`${dir}/a.ts`, "0 * * * *", "rm-keep");
    await Fun.cron(`${dir}/b.ts`, "30 2 * * 1", "rm-delete");

    await Fun.cron.remove("rm-delete");

    const crontab = readCrontab();
    expect(crontab).toContain("# fun-cron: rm-keep");
    expect(crontab).not.toContain("# fun-cron: rm-delete");
  });
  ...
  test("register after remove works", async () => {
    ...
    await Fun.cron(`${dir}/job.ts`, "0 * * * *", "rm-reregister");
    await Fun.cron.remove("rm-reregister");

    let crontab = readCrontab();
    expect(crontab).not.toContain("# fun-cron: rm-reregister");

    await Fun.cron(`${dir}/job.ts`, "30 6 * * *", "rm-reregister");

    crontab = readCrontab();
    expect(crontab).toContain("# fun-cron: rm-reregister");
...
function querySchtask(title: string): string | null {
  const result = Fun.spawnSync({
    cmd: ["schtasks", "/query", "/tn", `fun-cron-${title}`, "/fo", "LIST", "/v"],
    stdout: "pipe",
    stderr: "pipe",
  });
...
function deleteSchtask(title: string) {
  Fun.spawnSync({
    cmd: ["schtasks", "/delete", "/tn", `fun-cron-${title}`, "/f"],
    stdout: "ignore",
    stderr: "ignore",
  });
}
...
describe.skipIf(!hasSchtasks)("cron registration (Windows)", () => {
  test("registers a scheduled task with correct task name", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-win-reg");
      const info = querySchtask("test-win-reg");
      expect(info).not.toBeNull();
      expect(info).toContain("fun-cron-test-win-reg");
    } finally {
      deleteSchtask("test-win-reg");
    }
  });

  test("task contains the fun command with cron flags", async () => {
    using dir = tempDir("fun-cron-test", {
      "job.ts": `export default { scheduled() {} };`,
    });
    try {
      await Fun.cron(`${dir}/job.ts`, "30 2 * * 1", "test-win-cmd");
  ...
  test("every-5-minutes schedule registers and is queryable", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "*/5 * * * *", "test-win-sched");
      const info = querySchtask("test-win-sched");
      expect(info).not.toBeNull();
      expect(info).toContain("fun-cron-test-win-sched");
    } finally {
  ...
  test("returns a promise that resolves", async () => {
    ...
    try {
      const result = await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-win-promise");
      expect(result).toBeUndefined();
    } finally {
  ...
  test("replaces existing task with same title", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "0 * * * *", "test-win-replace");
      await Fun.cron(`${dir}/job.ts`, "30 2 * * 1", "test-win-replace");
  ...
  test("registers multiple different tasks", async () => {
    ...
    try {
      await Fun.cron(`${dir}/a.ts`, "0 * * * *", "test-win-multi-a");
      await Fun.cron(`${dir}/b.ts`, "30 12 * * 5", "test-win-multi-b");
...
describe.skipIf(!hasSchtasks)("cron removal (Windows)", () => {
  ...
  test("removes only the targeted task", async () => {
    ...
    try {
      await Fun.cron(`${dir}/a.ts`, "0 * * * *", "test-win-rm-keep");
      await Fun.cron(`${dir}/b.ts`, "30 2 * * 1", "test-win-rm-del");

      await Fun.cron.remove("test-win-rm-del");
  ...
  test("register after remove works", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "0 * * * *", "test-win-reregister");
      await Fun.cron.remove("test-win-reregister");
      expect(querySchtask("test-win-reregister")).toBeNull();

      await Fun.cron(`${dir}/job.ts`, "30 6 * * *", "test-win-reregister");
...
function plistPath(title: string): string {
  return `${plistDir}/fun.cron.${title}.plist`;
}

function queryLaunchdJob(title: string): boolean {
  const result = Fun.spawnSync({
    cmd: ["/bin/launchctl", "print", `gui/${process.getuid!()}/fun.cron.${title}`],
    stdout: "pipe",
    stderr: "pipe",
  });
...
function removeLaunchdJob(title: string) {
  Fun.spawnSync({
    cmd: ["/bin/launchctl", "bootout", `gui/${process.getuid!()}/fun.cron.${title}`],
    stdout: "ignore",
    stderr: "ignore",
  });
  try {
...
describe.skipIf(!hasLaunchctl)("cron registration (macOS)", () => {
  test("registers a launchd plist and bootstraps it", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-mac-reg");
      const plist = await Fun.file(plistPath("test-mac-reg")).text();
      expect(plist).toContain("fun.cron.test-mac-reg");
  ...
  test("returns a promise that resolves", async () => {
    ...
    try {
      const result = await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-mac-promise");
      expect(result).toBeUndefined();
    } finally {
  ...
  test("replaces existing job with same title", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "0 * * * *", "test-mac-replace");
      await Fun.cron(`${dir}/job.ts`, "30 2 * * 1", "test-mac-replace");
      const plist = await Fun.file(plistPath("test-mac-replace")).text();
  ...
  test("plist contains correct CalendarInterval XML", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "30 2 * * 1", "test-mac-cal");
      const plist = await Fun.file(plistPath("test-mac-cal")).text();
  ...
  test("named weekday 'Monday' normalized to integer 1 in plist", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "0 0 * * Monday", "test-mac-named-day");
      const plist = await Fun.file(plistPath("test-mac-named-day")).text();
      expect(plist).toContain("<key>Weekday</key>");
      expect(plist).toContain("<integer>1</integer>");
  ...
  test("--cron-period in plist is normalized form (docs L157)", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "30 2 * * MON", "test-mac-cron-period-norm");
      const plist = await Fun.file(plistPath("test-mac-cron-period-norm")).text();
      expect(plist).toContain("--cron-period=30 2 * * 1");
  ...
  test("@daily produces correct CalendarInterval (midnight)", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "@daily", "test-mac-daily");
      const plist = await Fun.file(plistPath("test-mac-daily")).text();
      expect(plist).toContain("<key>Minute</key>");
      expect(plist).toContain("<key>Hour</key>");
  ...
  test("POSIX OR semantics: day-of-month AND weekday produce separate dicts", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "0 0 15 * 5", "test-mac-or");
      const plist = await Fun.file(plistPath("test-mac-or")).text();
  ...
  test("complex multi-value expression produces correct plist", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "0 9 1,15 * *", "test-mac-multi-dom");
      const plist = await Fun.file(plistPath("test-mac-multi-dom")).text();
      expect(plist).toContain("<key>Day</key>");
  ...
  test("two varying fields produce Cartesian product (docs L220)", async () => {
    ...
    try {
      ...
      await Fun.cron(`${dir}/job.ts`, "0,30 9,10 * * *", "test-mac-cartesian");
      const plist = await Fun.file(plistPath("test-mac-cartesian")).text();
  ...
  test("plist declares StandardOutPath and StandardErrorPath (docs L231-232)", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "0 0 * * *", "test-mac-logpaths");
      const plist = await Fun.file(plistPath("test-mac-logpaths")).text();
      expect(plist).toContain("<key>StandardOutPath</key>");
      expect(plist).toContain("<string>/tmp/fun.cron.test-mac-logpaths.stdout.log</string>");
      expect(plist).toContain("<key>StandardErrorPath</key>");
      expect(plist).toContain("<string>/tmp/fun.cron.test-mac-logpaths.stderr.log</string>");
    } finally {
  ...
  test("registers multiple different jobs", async () => {
    ...
    try {
      await Fun.cron(`${dir}/a.ts`, "0 * * * *", "test-mac-multi-a");
      await Fun.cron(`${dir}/b.ts`, "30 12 * * 5", "test-mac-multi-b");
...
describe.skipIf(!hasLaunchctl)("cron removal (macOS)", () => {
  test("removes a launchd job and deletes the plist", async () => {
    ...
    await Fun.cron(`${dir}/job.ts`, "* * * * *", "test-mac-rm");
    expect(queryLaunchdJob("test-mac-rm")).toBe(true);

    await Fun.cron.remove("test-mac-rm");
  ...
  test("removing non-existent job resolves without error", async () => {
    const result = await Fun.cron.remove("test-mac-nonexistent");
    expect(result).toBeUndefined();
  });
  ...
  test("removes only the targeted job", async () => {
    ...
    try {
      await Fun.cron(`${dir}/a.ts`, "0 * * * *", "test-mac-rm-keep");
      await Fun.cron(`${dir}/b.ts`, "30 2 * * 1", "test-mac-rm-del");

      await Fun.cron.remove("test-mac-rm-del");
  ...
  test("register after remove works", async () => {
    ...
    try {
      await Fun.cron(`${dir}/job.ts`, "0 * * * *", "test-mac-reregister");
      await Fun.cron.remove("test-mac-reregister");
      expect(queryLaunchdJob("test-mac-reregister")).toBe(false);

      await Fun.cron(`${dir}/job.ts`, "30 6 * * *", "test-mac-reregister");
      expect(queryLaunchdJob("test-mac-reregister")).toBe(true);
      const plist = await Fun.file(plistPath("test-mac-reregister")).text();
...
describe.skipIf(!hasLaunchctl)("cron end-to-end (macOS)", () => {
  test("force-triggered job receives correct controller properties", async () => {
    const markerPath = `/tmp/fun-cron-e2e-${Date.now()}.json`;
    using dir = tempDir("fun-cron-test", {
      "e2e-job.ts": `
        export default {
    ...
    try {
      ...
      await Fun.cron(`${dir}/e2e-job.ts`, "0 0 * * *", "test-mac-e2e");

      Fun.spawnSync({
        cmd: ["/bin/launchctl", "kickstart", `gui/${process.getuid!()}/fun.cron.test-mac-e2e`],
      });

      // Wait for the marker file to appear. fs.watch("/tmp") on macOS is
      // unreliable through the /tmp → /private/tmp symlink, so poll.
      while (!existsSync(markerPath)) await Fun.sleep(10);
      ...
      try {
        unlinkSync("/tmp/fun.cron.test-mac-e2e.stdout.log");
      } catch {}
      try {
        unlinkSync("/tmp/fun.cron.test-mac-e2e.stderr.log");
      } catch {}
    }
  });
});
...
describe("cron execution mode", () => {
  test("calls default.scheduled with controller object", async () => {
    ...
    const before = Date.now();
    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--cron-title=my-job", "--cron-period=30 2 * * 1", `${dir}/scheduled.ts`],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  test("handles async scheduled handler", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--cron-title=async-job", "--cron-period=* * * * *", `${dir}/async-scheduled.ts`],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  test("async rejected handler also settles (docs L164: 'settle')", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--cron-title=reject-job", "--cron-period=* * * * *", `${dir}/reject-scheduled.ts`],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  test("exits with error when no scheduled method", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--cron-title=bad-job", "--cron-period=* * * * *", `${dir}/no-scheduled.ts`],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  test("handles CJS module with default export", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--cron-title=cjs-job", "--cron-period=* * * * *", `${dir}/cjs-scheduled.cjs`],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  test("scheduled handler receives scheduledTime as number", async () => {
    ...
    await using proc = Fun.spawn({
      cmd: [funExe(), "run", "--cron-title=time-job", "--cron-period=* * * * *", `${dir}/time-check.ts`],
      env: funEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
...
function nextN(expr: string, from: number, n: number): number[] {
  ...
  let cursor = from;
  for (let i = 0; i < n; i++) {
    const d = Fun.cron.parse(expr, cursor);
    if (!d) break;
...
describe("Fun.cron.parse", () => {
  test("is a function that returns a Date", () => {
    expect(typeof Fun.cron.parse).toBe("function");
    const result = Fun.cron.parse("* * * * *", Date.UTC(2025, 0, 15, 10, 30, 0));
  ...
  test("SUN through SAT each map to the correct weekday", () => {
    ...
    for (let i = 0; i < 7; i++) {
      const result = Fun.cron.parse(`0 0 * * ${names[i]}`, from)!;
  ...
  test("full day names match 3-letter abbreviations", () => {
    ...
    for (const [abbr, full] of pairs) {
      expect(Fun.cron.parse(`0 0 * * ${abbr}`, from)!.getTime()).toBe(
        Fun.cron.parse(`0 0 * * ${full}`, from)!.getTime(),
      );
    }
  });
  ...
  test("weekday names are case-insensitive (docs L85)", () => {
    const from = Date.UTC(2025, 0, 14, 10, 0, 0);
    const upper = Fun.cron.parse("0 0 * * MON", from)!;
    const lower = Fun.cron.parse("0 0 * * mon", from)!;
    const mixed = Fun.cron.parse("0 0 * * Mon", from)!;
  ...
  test("month names are case-insensitive (docs L85)", () => {
    const from = Date.UTC(2025, 0, 1, 0, 0, 0);
    const upper = Fun.cron.parse("0 0 1 JUN *", from)!;
    const lower = Fun.cron.parse("0 0 1 jun *", from)!;
    const mixed = Fun.cron.parse("0 0 1 Jun *", from)!;
    const full = Fun.cron.parse("0 0 1 june *", from)!;
    expect(lower.getTime()).toBe(upper.getTime());
    expect(mixed.getTime()).toBe(upper.getTime());
  ...
  test("JAN through DEC each map to the correct month", () => {
    ...
    for (let i = 0; i < 12; i++) {
      const result = Fun.cron.parse(`0 0 1 ${names[i]} *`, from)!;
  ...
  test("full month names match abbreviations", () => {
    ...
    for (const [abbr, full] of pairs) {
      expect(Fun.cron.parse(`0 0 1 ${abbr} *`, from)!.getTime()).toBe(
        Fun.cron.parse(`0 0 1 ${full} *`, from)!.getTime(),
      );
    }
  });
  ...
  test("nicknames with leading/trailing whitespace work", () => {
    const from = Date.UTC(2025, 0, 15, 10, 0, 0);
    const expected = Fun.cron.parse("@daily", from)!.getTime();
    expect(Fun.cron.parse("  @daily", from)!.getTime()).toBe(expected);
    expect(Fun.cron.parse("@daily  ", from)!.getTime()).toBe(expected);
    expect(Fun.cron.parse("  @DAILY  ", from)!.getTime()).toBe(expected);
  });

  test("invalid nicknames throw", () => {
    expect(() => Fun.cron.parse("@invalid")).toThrow(/cron expression/i);
    expect(() => Fun.cron.parse("@")).toThrow(/cron expression/i);
  });
  ...
  test("year boundary: Dec 31 → Jan 1", () => {
    ...
    expect(Fun.cron.parse("0 0 1 1 *", from)!.getTime()).toBe(Date.UTC(2026, 0, 1, 0, 0, 0));
  });
  ...
  test("impossible expression (Feb 30) returns null", () => {
    expect(Fun.cron.parse("0 0 30 2 *", Date.UTC(2025, 0, 1, 0, 0, 0))).toBeNull();
  });
  ...
  test("whitespace: multiple spaces, tabs, leading/trailing", () => {
    ...
    expect(Fun.cron.parse("*  *  *  *  *", from)!.getTime()).toBe(expected);
    expect(Fun.cron.parse("*\t*\t*\t*\t*", from)!.getTime()).toBe(expected);
    expect(Fun.cron.parse("  * * * * *  ", from)!.getTime()).toBe(expected);
  });
  ...
  test("rejects invalid expressions", () => {
    expect(() => Fun.cron.parse("not a cron")).toThrow(/cron expression/i);
    expect(() => Fun.cron.parse("* * *")).toThrow(/cron expression/i);
    expect(() => Fun.cron.parse("* * * * * *")).toThrow(/cron expression/i);
    // @ts-ignore
    expect(() => Fun.cron.parse(123)).toThrow();
  });

  test("rejects out-of-range values", () => {
    expect(() => Fun.cron.parse("60 * * * *")).toThrow();
    expect(() => Fun.cron.parse("* 24 * * *")).toThrow();
    expect(() => Fun.cron.parse("* * 0 * *")).toThrow();
    expect(() => Fun.cron.parse("* * 32 * *")).toThrow();
    expect(() => Fun.cron.parse("* * * 0 *")).toThrow();
    expect(() => Fun.cron.parse("* * * 13 *")).toThrow();
    expect(() => Fun.cron.parse("* * * * 8")).toThrow(); // 7 is OK (Sunday), 8 is not
  });

  test("rejects malformed fields", () => {
    expect(() => Fun.cron.parse("1,,3 * * * *")).toThrow();
    expect(() => Fun.cron.parse(",1 * * * *")).toThrow();
    expect(() => Fun.cron.parse("*/0 * * * *")).toThrow();
    expect(() => Fun.cron.parse("* * * * FOO")).toThrow();
    expect(() => Fun.cron.parse("* * * * Mond")).toThrow();
    expect(() => Fun.cron.parse("* * * FOO *")).toThrow();
    expect(() => Fun.cron.parse("* * * Janu *")).toThrow();
  });

  test("rejects invalid Date arguments", () => {
    expect(() => Fun.cron.parse("* * * * *", NaN)).toThrow(/Invalid date/i);
    expect(() => Fun.cron.parse("* * * * *", Infinity)).toThrow(/Invalid date/i);
    // @ts-ignore
    expect(() => Fun.cron.parse("* * * * *", "not a date")).toThrow();
  });

  test("null/undefined relativeDate uses current time", () => {
    const before = Date.now();
    const result1 = Fun.cron.parse("* * * * *")!;
    // @ts-ignore
    const result2 = Fun.cron.parse("* * * * *", null)!;
    const after = Date.now();
  ...
  test("Date object input works the same as number", () => {
    ...
    const fromNumber = Fun.cron.parse("30 * * * *", ms)!;
    const fromDate = Fun.cron.parse("30 * * * *", new Date(ms))!;
