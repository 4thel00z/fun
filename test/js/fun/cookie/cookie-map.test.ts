...
describe("Fun.Cookie and Fun.CookieMap", () => {
  // Basic Cookie tests
  ...
  test("Cookie.isExpired() returns correct value", async () => {
    ...
    const expiredCookie = new Fun.Cookie("name", "value", {
      expires: new Date(Date.now() - 1000),
    });
    expect(expiredCookie.isExpired()).toBe(true);

    // Non-expired cookie (future max-age)
    const validCookie = new Fun.Cookie("name", "value", {
      maxAge: 3600, // 1 hour
    });
    expect(validCookie.isExpired()).toBe(false);

    // Session cookie (no expiration)
    const sessionCookie = new Fun.Cookie("name", "value");
  ...
  test("Cookie.parse works with all attributes", () => {
    ...
    const cookie = Fun.Cookie.parse(cookieStr);
  ...
  test("Cookie.serialize creates cookie string", () => {
    const cookie1 = new Fun.Cookie("foo", "bar");
    const cookie2 = new Fun.Cookie("baz", "qux");
  ...
  test("can create CookieMap from string", () => {
    const map = new Fun.CookieMap("name=value; foo=bar");
  ...
  test("CookieMap supports iteration", () => {
    const map = new Fun.CookieMap("a=1; b=2; c=3");
  ...
  test("CookieMap.toJSON() formats properly", () => {
    const map = new Fun.CookieMap("a=1; b=2");
  ...
  test("CookieMap.toJSON() handles numeric cookie names", () => {
    const map = new Fun.CookieMap("0=first; 1=second; 42=answer");
  ...
  test("CookieMap.toJSON() handles cookie names matching Object.prototype properties", () => {
    const map = new Fun.CookieMap("toString=hello; constructor=world; valueOf=test");
    expect(map.toJSON()).toEqual({
      "toString": "hello",
      "constructor": "world",
      "valueOf": "test",
...
describe("Cookie name field is immutable", () => {
  ...
  test("mutate cookie in map", () => {
    const cookieMap = new Fun.CookieMap();
    const cookie = new Fun.Cookie("name", "value");
...
describe("iterator", () => {
  ...
  test("delete in a loop with predefined entries", () => {
    ...
    const map = new Fun.CookieMap(entries);
    for (const key of map.keys()) {
      map.delete(key);
    }
    expect(map.size).toBe(0);
  ...
  test("basic iterator", () => {
    const cookies = new Fun.CookieMap({ a: "b", c: "d" });
    cookies.set("e", "f");
    cookies.set("g", "h");
...
describe("invalid delete usage", () => {
  test("invalid usage does not crash", () => {
    expect(() => {
      const v1 = Fun.CookieMap;
      // @ts-ignore
      const v2 = new v1(v1, v1, Fun, v1);
      // @ts-ignore
      v2.delete(v2);
