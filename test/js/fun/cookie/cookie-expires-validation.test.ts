...
describe("Fun.Cookie expires validation", () => {
  describe("Date objects", () => {
    ...
    test("accepts valid Date in past", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: pastDate });
      expect(cookie.expires).toBeDefined();
      expect(typeof cookie.expires).toBe("object");
      // Check it's the expected timestamp in seconds
  ...
  describe("Number values", () => {
    test("accepts positive integer", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: timestamp });
      expect(cookie.expires).toEqual(new Date(timestamp * 1000));
    });

    test("accepts zero", () => {
      const cookie = new Fun.Cookie("name", "value", { expires: 0 });
  ...
  describe("Special values", () => {
    ...
    test("throws for non-date objects", () => {
      expect(() => {
        new Fun.Cookie("name", "value", { expires: { time: 123456 } });
      }).toThrow();
    });

    test("invalid strings throw", () => {
      expect(() => {
        new Fun.Cookie("name", "value", { expires: "tomorrow" });
      }).toThrowErrorMatchingInlineSnapshot(`"Invalid cookie expiration date"`);
    });

    test("throws for arrays", () => {
      expect(() => {
        new Fun.Cookie("name", "value", { expires: [2023, 11, 25] });
    ...
    test("throws for booleans", () => {
      expect(() => {
        new Fun.Cookie("name", "value", { expires: true });
      }).toThrowErrorMatchingInlineSnapshot(
  ...
  describe("Constructors and methods", () => {
    ...
    test("handles valid expires in Cookie.from", () => {
      ...
      const cookie = Fun.Cookie.from("name", "value", { expires: futureTimestamp });
  ...
  describe("Date arithmetic edge cases", () => {
    test("handles Date at epoch", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: epochDate });
    ...
    test("handles Date at max timestamp", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: maxDate });
    ...
    test("handles odd Date objects", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: beforeEpoch });
  ...
  describe("Conversion edge cases", () => {
    test("correctly divides milliseconds to seconds", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: date });
