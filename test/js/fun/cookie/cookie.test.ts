...
describe("Fun.Cookie validation tests", () => {
  describe("expires validation", () => {
    test("accepts valid Date for expires", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: futureDate });
    ...
    test("accepts valid number for expires", () => {
      ...
      const cookie = new Fun.Cookie("name", "value", { expires: futureTimestamp });
    ...
    test("throws for NaN Date", () => {
      ...
      expect(() => {
        new Fun.Cookie("name", "value", { expires: invalidDate });
      }).toThrow("expires must be a valid Date (or Number)");
    });

    test("throws for NaN number", () => {
      expect(() => {
        new Fun.Cookie("name", "value", { expires: NaN });
      }).toThrow("expires must be a valid Number");
    });

    test("throws for non-finite number (Infinity)", () => {
      expect(() => {
        new Fun.Cookie("name", "value", { expires: Infinity });
      }).toThrow("expires must be a valid Number");
    });

    test("does not throw for negative number", () => {
      expect(() => {
        new Fun.Cookie("name", "value", { expires: -1 });
      }).not.toThrow();

      expect(new Fun.Cookie("name", "value", { expires: -1 }).expires).toEqual(new Date(-1 * 1000));
    });

    test("handles undefined expires correctly", () => {
      const cookie = new Fun.Cookie("name", "value", { expires: undefined });
      expect(cookie.expires).toBeUndefined();
    });

    test("handles null expires correctly", () => {
      // @ts-expect-error
      const cookie = new Fun.Cookie("name", "value", { expires: null });
      expect(cookie.expires).toBeUndefined();
    });
  });
  ...
  describe("Cookie.from validation", () => {
    test("throws for NaN Date in Cookie.from", () => {
      ...
      expect(() => {
        Fun.Cookie.from("name", "value", { expires: invalidDate });
      }).toThrow("expires must be a valid Date (or Number)");
    });

    test("throws for NaN number in Cookie.from", () => {
      expect(() => {
        Fun.Cookie.from("name", "value", { expires: NaN });
      }).toThrow("expires must be a valid Number");
    });

    test("throws for non-finite number in Cookie.from", () => {
      expect(() => {
        Fun.Cookie.from("name", "value", { expires: Infinity });
      }).toThrow("expires must be a valid Number");
    });
  });
...
console.log("describe Fun.serve() cookies");
describe("Fun.serve() cookies", () => {
  ...
  test("getAllChanges", () => {
    const map = new Fun.CookieMap("dont_modify=ONE; do_modify=TWO; do_delete=THREE");
...
describe("Fun.CookieMap constructor", () => {
  test("throws for invalid array", () => {
    expect(() => new Fun.CookieMap([["abc defg =fhaingj809读写汉字学中文"]])).toThrowErrorMatchingInlineSnapshot(
  ...
  test("accepts unicode cookie value in array", () => {
    const map = new Fun.CookieMap([["cookie key", "读写汉字学中文"]]);
    expect(map.get("cookie key")).toBe("读写汉字学中文");
  });
  ...
  test("serializes unicode cookie value", () => {
    ...
    `);
    // re-parse
    const reparsed = new Fun.CookieMap(map.toSetCookieHeaders()[0].split(";")[0]!);
  ...
  test("doesn't parse percent encoded value in object or array", () => {
    ...
    const map2 = new Fun.CookieMap([["cookiekey", "%E8%AF%BB%E5%86%99%E6%B1%89%E5%AD%97%E5%AD%A6%E4%B8%AD%E6%96%87"]]);
