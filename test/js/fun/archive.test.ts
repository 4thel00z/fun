...
describe("Fun.Archive", () => {
  describe("new Archive()", () => {
    test("creates archive from object with string values", async () => {
      ...
      expect(archive).toBeInstanceOf(Fun.Archive);
    });
    ...
    test("creates archive from Blob", async () => {
      ...
      const sourceArchive = new Fun.Archive({
      ...
      const archive = new Fun.Archive(blob);
      expect(archive).toBeInstanceOf(Fun.Archive);
    });
    ...
    test("creates archive from ArrayBuffer", async () => {
      ...
      const archive = new Fun.Archive(buffer);
      expect(archive).toBeInstanceOf(Fun.Archive);
    });
    ...
    test("creates archive from Uint8Array", async () => {
      ...
      const archive = new Fun.Archive(bytes);
      expect(archive).toBeInstanceOf(Fun.Archive);
    });
    ...
    test("throws with invalid input type (null)", () => {
      expect(() => {
        // @ts-expect-error - testing runtime behavior
        new Fun.Archive(null);
      }).toThrow();
    });

    test("converts non-string/buffer values to strings", async () => {
      // @ts-expect-error - testing runtime behavior
      const archive = new Fun.Archive({ "file.txt": 123 }, {});
  ...
  describe("archive.blob()", () => {
    ...
    test("with gzip option returns gzipped blob", async () => {
      const regularArchive = new Fun.Archive(
        {
          "hello.txt": "Hello, World!",
        },
        {}, // Empty options = no compression
      );
      const gzipArchive = new Fun.Archive(
        {
          "hello.txt": "Hello, World!",
        },
        { compress: "gzip" },
    ...
    test("gzip level affects compression ratio", async () => {
      ...
      const level1Archive = new Fun.Archive({ "large.txt": largeContent }, { compress: "gzip", level: 1 });
      const level12Archive = new Fun.Archive({ "large.txt": largeContent }, { compress: "gzip", level: 12 });
    ...
    test("defaults to no compression when no options provided", async () => {
      ...
      const defaultArchive = new Fun.Archive({
        "large.txt": largeContent,
      });

      // Explicit empty options = also no compression
      const emptyOptionsArchive = new Fun.Archive({ "large.txt": largeContent }, {});

      // Explicit gzip compression
      const compressedArchive = new Fun.Archive({ "large.txt": largeContent }, { compress: "gzip" });
    ...
    test("throws with invalid gzip level", () => {
      expect(() => {
        new Fun.Archive({ "hello.txt": "Hello, World!" }, { compress: "gzip", level: 0 });
      }).toThrow();

      expect(() => {
        new Fun.Archive({ "hello.txt": "Hello, World!" }, { compress: "gzip", level: 13 });
      }).toThrow();
    });
  });
  ...
  describe("archive.extract()", () => {
    test("extracts to directory and returns file count", async () => {
      ...
      const helloContent = await Fun.file(join(String(dir), "hello.txt")).text();
    ...
    test("extracts nested directory structure", async () => {
      ...
      expect(await Fun.file(join(String(dir), "root.txt")).text()).toBe("Root file");
      expect(await Fun.file(join(String(dir), "dir1/file1.txt")).text()).toBe("File in dir1");
      expect(await Fun.file(join(String(dir), "dir1/dir2/file2.txt")).text()).toBe("File in dir1/dir2");
      expect(await Fun.file(join(String(dir), "dir1/dir2/dir3/file3.txt")).text()).toBe("File in dir1/dir2/dir3");
    });
    ...
    test("extracts binary data correctly", async () => {
      ...
      const extractedBytes = new Uint8Array(await Fun.file(join(String(dir), "binary.bin")).arrayBuffer());
    ...
    test("extracts from archive created from blob", async () => {
      ...
      const content = await Fun.file(join(String(dir), "test.txt")).text();
    ...
    test("creates directory if it doesn't exist", async () => {
      ...
      const content = await Fun.file(join(newDir, "hello.txt")).text();
    ...
    test.skipIf(!isWindows)("skips tar entries whose normalized path is absolute", async () => {
      ...
      const uniq = `fun-archive-absolute-${crypto.randomUUID()}.txt`;
      ...
      try {
        const archive = new Fun.Archive(tarball);
        await archive.extract(String(dir));

        // The safe entry must still be extracted.
        expect(await Fun.file(join(String(dir), "safe.txt")).text()).toBe("safe contents");
  ...
  describe("corrupted archives", () => {
    test("throws when extracting corrupted archive data", async () => {
      ...
      const archive = new Fun.Archive(corruptedData);
    ...
    test("throws when extracting truncated archive", async () => {
      ...
      const validArchive = new Fun.Archive({
      ...
      const archive = new Fun.Archive(truncated);
    ...
    test("throws when extracting random bytes as archive", async () => {
      ...
      const archive = new Fun.Archive(randomBytes);
    ...
    test("handles empty archive gracefully", async () => {
      ...
      const archive = new Fun.Archive(emptyData);
    ...
    test("files() does not leak entries when readData fails mid-stream", async () => {
      ...
      const code = /* ts */ `
          ...
          const archive = new Fun.Archive(corrupt);
  ...
  describe("path safety", () => {
    test("normalizes paths with redundant separators", async () => {
      ...
      const content = await Fun.file(join(String(dir), "dir/subdir/file.txt")).text();
    ...
    test("handles paths with dots correctly", async () => {
      ...
      expect(await Fun.file(join(String(dir), "dir/file.txt")).text()).toBe("content1");
      expect(await Fun.file(join(String(dir), "dir/file2.txt")).text()).toBe("content2");
    });
    ...
    test("handles very long filenames", async () => {
      ...
      try {
        await archive.extract(String(dir));
        const content = await Fun.file(join(String(dir), longName)).text();
        expect(content).toBe("content");
      } catch {
    ...
    test("handles deeply nested paths", async () => {
      ...
      try {
        await archive.extract(String(dir));
        const content = await Fun.file(join(String(dir), deepPath)).text();
  ...
  describe("Archive.write()", () => {
    test("writes archive to file", async () => {
      ...
      await Fun.Archive.write(archivePath, {
        "hello.txt": "Hello, World!",
        "data.json": JSON.stringify({ foo: "bar" }),
      });

      // Verify file exists
      const file = Fun.file(archivePath);
    ...
    test("writes gzipped archive to file", async () => {
      ...
      expect(file.size).toBeLessThan(Fun.file(uncompressedPath).size);
    });
    ...
    test("writes archive from Blob", async () => {
      ...
      // Write blob to file
      await Fun.Archive.write(archivePath, blob);
    ...
    test("written archive can be extracted", async () => {
      ...
      // Extract it
      const blob = await Fun.file(archivePath).bytes();
      const archive = new Fun.Archive(blob);
      ...
      // Verify contents
      expect(await Fun.file(join(extractDir, "hello.txt")).text()).toBe("Hello from write!");
      expect(await Fun.file(join(extractDir, "subdir/nested.txt")).text()).toBe("Nested content from write");
    });
    ...
    test("throws with only path argument", async () => {
      await expect(async () => {
        // @ts-expect-error - testing runtime behavior
        await Fun.Archive.write("/tmp/test.tar");
      }).toThrow();
    });
    ...
    test("throws with invalid gzip option", async () => {
      ...
      await expect(async () => {
        await Fun.Archive.write(
          archivePath,
          new Fun.Archive({ "file.txt": "content" }, { compress: "gzip", level: 0 }),
        );
      }).toThrow();
      ...
      await expect(async () => {
        await Fun.Archive.write(
          archivePath,
          new Fun.Archive({ "file.txt": "content" }, { compress: "gzip", level: 13 }),
        );
      }).toThrow();
    });
  });
  ...
  describe("GC safety", () => {
    ...
    test("archive with many entries survives GC", async () => {
      ...
      const archive = new Fun.Archive(entries);
    ...
    test("original data mutation doesn't affect archive", async () => {
      ...
      const extractedBytes = new Uint8Array(await Fun.file(join(String(dir), "data.bin")).arrayBuffer());
    ...
    test("blob source mutation doesn't affect archive", async () => {
      ...
      const archive = new Fun.Archive(archiveBlob);
  ...
  describe("large archives", () => {
    test("handles large file content", async () => {
      ...
      const extracted = await Fun.file(join(String(dir), "large.txt")).arrayBuffer();
  ...
  describe("special characters", () => {
    test("handles filenames with spaces", async () => {
      ...
      const content = await Fun.file(join(String(dir), "file with spaces.txt")).text();
      expect(content).toBe("content");
    });
    ...
    test("handles special characters in filenames", async () => {
      ...
      expect(await Fun.file(join(String(dir), "file-with-dash.txt")).text()).toBe("content1");
      expect(await Fun.file(join(String(dir), "file_with_underscore.txt")).text()).toBe("content2");
      expect(await Fun.file(join(String(dir), "file.with.dots.txt")).text()).toBe("content3");
    });
    ...
    test("handles unicode content", async () => {
      ...
      const content = await Fun.file(join(String(dir), "unicode.txt")).text();
  ...
  describe("archive.files()", () => {
    ...
    test("works with gzipped archive source", async () => {
      ...
      const archive = new Fun.Archive(gzippedBlob);
  ...
  describe("sparse files", () => {
    ...
    test("extracts sparse file with small hole (< 1 tar block)", async () => {
      ...
      const tarData = await Fun.file(join(fixturesDir, "small-hole.tar")).bytes();
      const archive = new Fun.Archive(tarData);
      await archive.extract(String(dir));

      const extracted = await Fun.file(join(String(dir), "small-hole.bin")).bytes();
    ...
    test("extracts sparse file with 1 tar block hole (512 bytes)", async () => {
      ...
      const tarData = await Fun.file(join(fixturesDir, "one-block-hole.tar")).bytes();
      const archive = new Fun.Archive(tarData);
      await archive.extract(String(dir));

      const extracted = await Fun.file(join(String(dir), "one-block-hole.bin")).bytes();
    ...
    test("extracts sparse file with multi-block hole (5 tar blocks)", async () => {
      ...
      const tarData = await Fun.file(join(fixturesDir, "multi-block-hole.tar")).bytes();
      const archive = new Fun.Archive(tarData);
      await archive.extract(String(dir));

      const extracted = await Fun.file(join(String(dir), "multi-block-hole.bin")).bytes();
    ...
    test("extracts sparse file with leading hole", async () => {
      ...
      const tarData = await Fun.file(join(fixturesDir, "leading-hole.tar")).bytes();
      const archive = new Fun.Archive(tarData);
      await archive.extract(String(dir));

      const extracted = await Fun.file(join(String(dir), "leading-hole.bin")).bytes();
    ...
    test("extracts sparse file with trailing hole", async () => {
      ...
      const tarData = await Fun.file(join(fixturesDir, "trailing-hole.tar")).bytes();
      const archive = new Fun.Archive(tarData);
      await archive.extract(String(dir));

      const extracted = await Fun.file(join(String(dir), "trailing-hole.bin")).bytes();
    ...
    test("extracts sparse file with large hole (64KB)", async () => {
      ...
      const tarData = await Fun.file(join(fixturesDir, "large-hole.tar")).bytes();
      const archive = new Fun.Archive(tarData);
      await archive.extract(String(dir));

      const extracted = await Fun.file(join(String(dir), "large-hole.bin")).bytes();
  ...
  describe("extract with glob patterns", () => {
    test("extracts only files matching glob pattern", async () => {
      ...
      expect(count).toBe(4);
      expect(await Fun.file(join(String(dir), "src/index.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "src/utils.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "src/types.d.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "test/index.test.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "README.md")).exists()).toBe(false);
      expect(await Fun.file(join(String(dir), "package.json")).exists()).toBe(false);
    });
    ...
    test("extracts files matching any of multiple glob patterns", async () => {
      ...
      expect(count).toBe(2);
      expect(await Fun.file(join(String(dir), "src/index.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "lib/utils.js")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "test/test.ts")).exists()).toBe(false);
      expect(await Fun.file(join(String(dir), "README.md")).exists()).toBe(false);
    });
    ...
    test("excludes files matching negative pattern", async () => {
      ...
      expect(await Fun.file(join(String(dir), "src/index.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "src/utils.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "src/index.test.ts")).exists()).toBe(false);
      expect(await Fun.file(join(String(dir), "src/utils.test.ts")).exists()).toBe(false);
    });
    ...
    test("excludes files matching any of multiple negative patterns", async () => {
      ...
      expect(await Fun.file(join(String(dir), "src/index.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "src/index.test.ts")).exists()).toBe(false);
      expect(await Fun.file(join(String(dir), "__tests__/helper.ts")).exists()).toBe(false);
      expect(await Fun.file(join(String(dir), "node_modules/pkg/index.js")).exists()).toBe(false);
    });
    ...
    test("combines positive and negative glob patterns", async () => {
      ...
      expect(await Fun.file(join(String(dir), "src/index.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "src/utils.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "lib/helper.ts")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "src/index.test.ts")).exists()).toBe(false);
      expect(await Fun.file(join(String(dir), "lib/helper.test.ts")).exists()).toBe(false);
      expect(await Fun.file(join(String(dir), "README.md")).exists()).toBe(false);
    });
    ...
    test("extracts all files when no patterns are provided", async () => {
      ...
      expect(count).toBe(2);
      expect(await Fun.file(join(String(dir), "file1.txt")).exists()).toBe(true);
      expect(await Fun.file(join(String(dir), "file2.txt")).exists()).toBe(true);
    });
  ...
  describe("concurrent operations", () => {
    test("multiple extract operations run correctly", async () => {
      ...
      expect(await Fun.file(join(String(dir1), "file.txt")).text()).toBe("content");
      expect(await Fun.file(join(String(dir2), "file.txt")).text()).toBe("content");
      expect(await Fun.file(join(String(dir3), "file.txt")).text()).toBe("content");
    });
  ...
  describe("Fun.write with Archive", () => {
    test("writes archive to local file", async () => {
      ...
      using dir = tempDir("archive-funwrite", {});
      const tarPath = join(String(dir), "test.tar");

      const bytesWritten = await Fun.write(tarPath, archive);
      expect(bytesWritten).toBeGreaterThan(0);

      // Verify the file was written
      expect(await Fun.file(tarPath).exists()).toBe(true);

      // Read it back and verify contents
      const readArchive = new Fun.Archive(await Fun.file(tarPath).bytes());
    ...
    test("writes archive with nested directories", async () => {
      ...
      using dir = tempDir("archive-funwrite-nested", {});
      const tarPath = join(String(dir), "nested.tar");

      await Fun.write(tarPath, archive);
    ...
    test("writes archive with binary content", async () => {
      ...
      using dir = tempDir("archive-funwrite-binary", {});
    ...
    test("writes archive to Fun.file()", async () => {
      const archive = new Fun.Archive({
        "test.txt": "test content",
      });

      using dir = tempDir("archive-funwrite-file", {});
      const tarPath = join(String(dir), "test.tar");
      const funFile = Fun.file(tarPath);

      await Fun.write(funFile, archive);

      expect(await funFile.exists()).toBe(true);
      const readArchive = new Fun.Archive(await funFile.bytes());
      const files = await readArchive.files();
  ...
  describe("TypeScript types", () => {
    test("valid archive options", () => {
      ...
      new Fun.Archive(files);

      // Valid: empty options (also no compression)
      new Fun.Archive(files, {});

      // Valid: explicit gzip compression
      new Fun.Archive(files, { compress: "gzip" });

      // Valid: gzip with level
      new Fun.Archive(files, { compress: "gzip", level: 9 });
    });
    ...
    test("invalid archive options throw TypeScript errors", () => {
      ...
      const _typeCheck = () => {
        ...
        new Fun.Archive(files, { compress: "zstd", level: 9 });
      };
    ...
    test("level without compress is TypeScript error but no runtime error", async () => {
      ...
      const archive = new Fun.Archive(files, { level: 9 });
      ...
      const uncompressedArchive = new Fun.Archive(files, {});
      const bytes = await archive.bytes();
