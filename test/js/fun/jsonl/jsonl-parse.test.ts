...
describe("Fun.JSONL", () => {
  ...
  describe("parse", () => {
    describe("complete input", () => {
      ...
      test("empty string", () => {
        expect(Fun.JSONL.parse("")).toStrictEqual([]);
      });

      test("deeply nested objects", () => {
        expect(Fun.JSONL.parse('{"a":{"b":{"c":{"d":1}}}}\n[1,[2,[3,[4]]]]\n')).toStrictEqual([
          { a: { b: { c: { d: 1 } } } },
          [1, [2, [3, [4]]]],
        ]);
      });
      ...
      test("strings containing escaped newlines", () => {
        expect(Fun.JSONL.parse('{"msg":"line1\\nline2"}\n{"msg":"line3\\nline4"}\n')).toStrictEqual([
          { msg: "line1\nline2" },
          { msg: "line3\nline4" },
        ]);
      });
      ...
      test("numbers: integers, floats, negative, exponents", () => {
        expect(Fun.JSONL.parse("0\n42\n-17\n3.14\n-0.5\n1e10\n2.5e-3\n")).toStrictEqual([
          0, 42, -17, 3.14, -0.5, 1e10, 2.5e-3,
        ]);
      });

      test("empty objects and arrays", () => {
        expect(Fun.JSONL.parse("{}\n[]\n{}\n[]\n")).toStrictEqual([{}, [], {}, []]);
      });
      ...
      test("large number of lines", () => {
        ...
        const result = Fun.JSONL.parse(lines.join("\n") + "\n");
    ...
    describe("error handling", () => {
      test("throws on invalid JSON with no valid values before it", () => {
        expect(() => Fun.JSONL.parse('{invalid}\n{"a":1}\n')).toThrow();
      });

      test("throws on bare word with no valid values", () => {
        expect(() => Fun.JSONL.parse("undefined\n")).toThrow();
      });

      test("throws on single invalid token", () => {
        expect(() => Fun.JSONL.parse("xyz\n")).toThrow();
      });
      ...
      test("returns results up to the error", () => {
        const result = Fun.JSONL.parse("1\n2\n3\nBAD\n5\n");
      ...
      test("error at line 1 of N throws (no prior values)", () => {
        for (const n of [1, 2, 5, 10]) {
          ...
          lines[0] = "{broken";
          expect(() => Fun.JSONL.parse(lines.join("\n") + "\n")).toThrow(SyntaxError);
        }
      });

      test("error at line 2 returns only line 1", () => {
        const result = Fun.JSONL.parse('{"first":true}\n{bad\n{"third":true}\n');
      ...
      test("error at every position in a 10-line input", () => {
        for (let errPos = 0; errPos < 10; errPos++) {
          ...
          if (errPos === 0) {
            expect(() => Fun.JSONL.parse(input)).toThrow(SyntaxError);
          } else {
            const result = Fun.JSONL.parse(input);
      ...
      test("parseChunk: error at every position reports correct read", () => {
        for (let errPos = 0; errPos < 5; errPos++) {
          ...
          lines[errPos] = "INVALID";
          const input = lines.join("\n") + "\n";
          const result = Fun.JSONL.parseChunk(input);
      ...
      test("typed array: error at various positions", () => {
        ...
        for (let errPos = 0; errPos < 5; errPos++) {
          ...
          const result = Fun.JSONL.parseChunk(buf);
          expect(result.values.length).toBe(errPos);
          if (errPos === 0) {
            expect(result.read).toBe(0);
          }
      ...
      test("error immediately after newline of valid value", () => {
        ...
        const result = Fun.JSONL.parseChunk('{"ok":1}\nX\n');
      ...
      test("empty lines before error", () => {
        const result = Fun.JSONL.parseChunk('{"a":1}\n\n\n\nBAD\n');
    ...
    describe("partial/incomplete trailing data", () => {
      ...
      test("returns empty array for partial key", () => {
        expect(Fun.JSONL.parse('{"ke')).toStrictEqual([]);
      });
    ...
    describe("edge cases", () => {
      test("returns array type", () => {
        expect(Array.isArray(Fun.JSONL.parse('{"a":1}\n'))).toBe(true);
      });

      test("coerces argument to string", () => {
        expect(Fun.JSONL.parse(42 as unknown as string)).toStrictEqual([42]);
      });
  ...
  describe("parseChunk", () => {
    describe("complete input", () => {
      ...
      test("single value without trailing newline", () => {
        const result = Fun.JSONL.parseChunk('{"key":"value"}');
      ...
      test("empty string", () => {
        const result = Fun.JSONL.parseChunk("");
    ...
    describe("incomplete/partial input (streaming)", () => {
      ...
      test("only incomplete data", () => {
        const result = Fun.JSONL.parseChunk('{"ke');
        expect(result.values).toStrictEqual([]);
        expect(result.read).toBe(0);
        expect(result.done).toBe(false);
        expect(result.error).toBeNull();
      ...
      test("simulated chunked streaming", () => {
        ...
        const chunk1 = '{"id":1}\n{"id":';
        const r1 = Fun.JSONL.parseChunk(chunk1);
        ...
        const r2 = Fun.JSONL.parseChunk(chunk2);
      ...
      test("simulated multi-step streaming", () => {
        ...
        for (const chunk of lines) {
          buffer += chunk;
          const result = Fun.JSONL.parseChunk(buffer);
    ...
    describe("error handling", () => {
      test("error at start with no valid values", () => {
        const result = Fun.JSONL.parseChunk('{invalid}\n{"a":1}\n');
        expect(result.values).toStrictEqual([]);
        expect(result.error).toBeInstanceOf(SyntaxError);
        expect(result.done).toBe(false);
      });
      ...
      test("error after many valid values", () => {
        const result = Fun.JSONL.parseChunk("1\n2\n3\nBAD\n5\n");
      ...
      test("throws TypeError on undefined argument", () => {
        // @ts-expect-error
        expect(() => Fun.JSONL.parseChunk(undefined)).toThrow();
      });

      test("throws TypeError on null argument", () => {
        // @ts-expect-error
        expect(() => Fun.JSONL.parseChunk(null)).toThrow();
      });
    });
    ...
    describe("read accuracy", () => {
      ...
      test("read is 0 when only incomplete", () => {
        expect(Fun.JSONL.parseChunk('{"incomplete').read).toBe(0);
      });

      test("read is 0 for empty input", () => {
        expect(Fun.JSONL.parseChunk("").read).toBe(0);
      });

      test("read does not include trailing whitespace", () => {
        expect(Fun.JSONL.parseChunk('{"a":1}   \n').read).toBe(7);
      });
      ...
      test("read allows exact streaming continuation", () => {
        const input = '{"id":1}\n{"id":2}\n{"id":3';
        const r1 = Fun.JSONL.parseChunk(input);
        expect(r1.read).toBe(17);

        const remainder = input.slice(r1.read);
        expect(remainder).toBe('\n{"id":3');

        const r2 = Fun.JSONL.parseChunk(remainder + "}\n");
      ...
      test("read for multi-byte unicode", () => {
        const result = Fun.JSONL.parseChunk('{"e":"🎉"}\n{"a":1}\n');
    ...
    describe("result shape", () => {
      test("has exactly four properties", () => {
        expect(Object.keys(Fun.JSONL.parseChunk('{"a":1}\n'))).toStrictEqual(["values", "read", "done", "error"]);
      });

      test("values is an array", () => {
        expect(Array.isArray(Fun.JSONL.parseChunk('{"a":1}\n').values)).toBe(true);
      });

      test("read is a number", () => {
        expect(typeof Fun.JSONL.parseChunk('{"a":1}\n').read).toBe("number");
      });
  ...
  describe("typed array input", () => {
    ...
    describe("parse with Uint8Array", () => {
      test("basic ASCII input", () => {
        expect(Fun.JSONL.parse(encode('{"a":1}\n{"b":2}\n'))).toStrictEqual([{ a: 1 }, { b: 2 }]);
      });

      test("mixed JSON types", () => {
        expect(Fun.JSONL.parse(encode('1\n"hello"\ntrue\nnull\n'))).toStrictEqual([1, "hello", true, null]);
      });

      test("empty buffer", () => {
        expect(Fun.JSONL.parse(new Uint8Array(0))).toStrictEqual([]);
      });

      test("non-ASCII UTF-8 content", () => {
        expect(Fun.JSONL.parse(encode('{"emoji":"🎉"}\n{"jp":"日本語"}\n'))).toStrictEqual([
          { emoji: "🎉" },
          { jp: "日本語" },
        ]);
      });
      ...
      test("throws on error with no valid values", () => {
        expect(() => Fun.JSONL.parse(encode("{bad}\n"))).toThrow();
      });
      ...
      test("Buffer (Uint8Array subclass)", () => {
        expect(Fun.JSONL.parse(Buffer.from('{"a":1}\n{"b":2}\n'))).toStrictEqual([{ a: 1 }, { b: 2 }]);
      });
    });

    describe("parseChunk with Uint8Array", () => {
      test("basic ASCII input", () => {
        const result = Fun.JSONL.parseChunk(encode('{"a":1}\n{"b":2}\n'));
        expect(result.values).toStrictEqual([{ a: 1 }, { b: 2 }]);
        expect(result.read).toBe(15);
        expect(result.done).toBe(true);
        expect(result.error).toBeNull();
      ...
      test("streaming with Buffer.concat", () => {
        ...
        const r2 = Fun.JSONL.parseChunk(combined);
        expect(r2.values).toStrictEqual([{ id: 2 }, { id: 3 }]);
        expect(r2.done).toBe(true);
      });
    ...
    describe("parseChunk with start/end offsets", () => {
      test("start offset skips bytes", () => {
        ...
        const result = Fun.JSONL.parseChunk(buf, 8);
      ...
      test("end offset limits parsing", () => {
        ...
        const result = Fun.JSONL.parseChunk(buf, 0, 16);
        expect(result.values).toStrictEqual([{ a: 1 }, { b: 2 }]);
      });
      ...
      test("start and end together select a window", () => {
        ...
        const result = Fun.JSONL.parseChunk(buf, 8, 16);
        expect(result.values).toStrictEqual([{ b: 2 }]);
      ...
      test("start at read offset for streaming", () => {
        ...
        const r1 = Fun.JSONL.parseChunk(buf, 0, 15); // partial
        expect(r1.values).toStrictEqual([{ id: 1 }]);
        expect(r1.done).toBe(false);

        const r2 = Fun.JSONL.parseChunk(buf, r1.read);
        expect(r2.values).toStrictEqual([{ id: 2 }, { id: 3 }]);
        expect(r2.done).toBe(true);
      });
      ...
      test("start equals end returns empty", () => {
        const buf = encode('{"a":1}\n');
        const result = Fun.JSONL.parseChunk(buf, 5, 5);
        expect(result.values).toStrictEqual([]);
      ...
      test("start beyond buffer length returns empty", () => {
        const buf = encode('{"a":1}\n');
        const result = Fun.JSONL.parseChunk(buf, 100);
        expect(result.values).toStrictEqual([]);
      });
      ...
      test("end offset works for string input", () => {
        const input = '{"a":1}\n{"b":2}\n{"c":3}\n';
        const result = Fun.JSONL.parseChunk(input, 0, 16);
        expect(result.values).toStrictEqual([{ a: 1 }, { b: 2 }]);
      });

      test("start+end window works for string input", () => {
        const input = '{"a":1}\n{"b":2}\n{"c":3}\n';
        const result = Fun.JSONL.parseChunk(input, 8, 16);
        expect(result.values).toStrictEqual([{ b: 2 }]);
        expect(result.read).toBe(15);
      });
      ...
      test("string: error-recovery loop with start offset terminates", () => {
        ...
        while (offset < input.length) {
          ...
          const r = Fun.JSONL.parseChunk(input, offset);
      ...
      test("non-ASCII with start offset", () => {
        ...
        const result = Fun.JSONL.parseChunk(buf, firstValueBytes);
        expect(result.values).toStrictEqual([{ a: 1 }]);
      });
      ...
      describe("invalid offset inputs", () => {
        ...
        test.each([
          ...
          const r = Fun.JSONL.parseChunk(input, start as number, end as number);
        ...
        test.each([
          ...
          const r = Fun.JSONL.parseChunk(buf, start as number, end as number);
          expect(Array.isArray(r.values)).toBe(true);
          expect(r.read).toBeGreaterThanOrEqual(0);
        ...
        test("negative end is ignored (Buffer)", () => {
          const r = Fun.JSONL.parseChunk(buf, 0, -10);
          expect(r.values).toStrictEqual([{ a: 1 }, { b: 2 }]);
        });

        test("start > end clamps start down to end", () => {
          const r = Fun.JSONL.parseChunk(input, 10, 5);
          expect(r.values).toStrictEqual([]);
  ...
  describe("fuzz-like stress tests", () => {
    describe("stack depth", () => {
      ...
      test("alternating deep nesting across lines", () => {
        ...
        const result = Fun.JSONL.parseChunk(lines.join("\n") + "\n");
    ...
    describe("OOM resistance", () => {
      ...
      test("large input as Uint8Array", () => {
        ...
        const result = Fun.JSONL.parse(buf);
        expect(result.length).toBe(10000);
      });
    ...
    describe("garbage input", () => {
      ...
      test("random bytes don't crash parseChunk (100 iterations)", () => {
        for (let i = 0; i < 100; i++) {
          const random = new Uint8Array(256 + Math.floor(Math.random() * 1024));
          crypto.getRandomValues(random);
          const result = Fun.JSONL.parseChunk(random);
          expect(Array.isArray(result.values)).toBe(true);
    ...
    describe("number edge cases", () => {
      ...
      test("very long numeric strings", () => {
        ...
        const result = Fun.JSONL.parseChunk(longNum + "\n");
        expect(result.values.length).toBe(1);
        expect(typeof result.values[0]).toBe("number");
      });

      test("negative zero", () => {
        const result = Fun.JSONL.parse("-0\n");
    ...
    describe("UTF-8 boundary conditions", () => {
      ...
      test("truncated multi-byte UTF-8 in typed array", () => {
        ...
        const result = Fun.JSONL.parseChunk(truncated);
        expect(Array.isArray(result.values)).toBe(true);
      });
      ...
      test("start offset in middle of multi-byte char", () => {
        ...
        const result = Fun.JSONL.parseChunk(buf, 6);
      ...
      test("4-byte UTF-8 characters (emoji)", () => {
        const result = Fun.JSONL.parseChunk(encode('{"s":"😀🎉🚀"}\n'));
      ...
      test("mixed byte-width UTF-8", () => {
        ...
        const val = "aañ日😀";
        const result = Fun.JSONL.parseChunk(encode(`{"s":"${val}"}\n`));
    ...
    describe("streaming correctness", () => {
      test("byte-by-byte feeding produces same results as full parse", () => {
        const fullInput = '{"a":1}\n{"b":2}\n{"c":3}\n';
        const expected = Fun.JSONL.parse(fullInput);

        const buf = new TextEncoder().encode(fullInput);
        const allValues: unknown[] = [];
        let offset = 0;
        for (let i = 1; i <= buf.length; i++) {
          const result = Fun.JSONL.parseChunk(buf, offset, i);
          allValues.push(...result.values);
      ...
      test("random chunk sizes produce same results", () => {
        ...
        while (end < buf.length) {
          ...
          const result = Fun.JSONL.parseChunk(buf, start, end);
          allValues.push(...result.values);
          if (result.read > start) start = result.read;
        }
        // Final parse of any remainder
        if (start < buf.length) {
          const result = Fun.JSONL.parseChunk(buf, start);
          allValues.push(...result.values);
        }
        expect(allValues).toStrictEqual(expected);
      });
      ...
      test("parseChunk with string slicing matches typed array start/end", () => {
        ...
        const r1str = Fun.JSONL.parseChunk(input);
        // Typed array path: use start
        const r1buf = Fun.JSONL.parseChunk(buf);
      ...
      test("detached ArrayBuffer throws", () => {
        ...
        expect(() => Fun.JSONL.parseChunk(buf)).toThrow();
      });
      ...
      test("Uint8Array with byteOffset", () => {
        ...
        const result = Fun.JSONL.parse(view);
        expect(result).toStrictEqual([{ a: 1 }, { b: 2 }]);
      });
      ...
      test("Uint8Array with byteOffset and start param", () => {
        const base = new TextEncoder().encode('JUNK{"a":1}\n{"b":2}\n');
        const view = new Uint8Array(base.buffer, 4);
        const result = Fun.JSONL.parseChunk(view, 8); // skip past {"a":1}\n
        expect(result.values).toStrictEqual([{ b: 2 }]);
      });
    });
    ...
    describe("adversarial input", () => {
      ...
      test("BOM (byte order mark) at start of Uint8Array is skipped", () => {
        ...
        expect(Fun.JSONL.parse(buf)).toStrictEqual([{ a: 1 }]);
      ...
      test("toString trap on input object", () => {
        ...
        const result = Fun.JSONL.parse(evil as unknown as string);
        expect(result).toStrictEqual([{ a: 1 }]);
      ...
      test("SharedArrayBuffer input", () => {
        ...
        const result = Fun.JSONL.parseChunk(new Uint8Array(sab, 0, src.length));
        expect(result.values).toStrictEqual([{ a: 1 }]);
      });
      ...
      test("start/end with NaN, Infinity, -Infinity, negative numbers", () => {
        ...
        expect(() => Fun.JSONL.parseChunk(buf, NaN)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, 0, NaN)).not.toThrow();
        // Infinity should clamp
        expect(() => Fun.JSONL.parseChunk(buf, Infinity)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, 0, Infinity)).not.toThrow();
        // Negative should be treated as 0
        expect(() => Fun.JSONL.parseChunk(buf, -1)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, 0, -1)).not.toThrow();
        // -Infinity
        expect(() => Fun.JSONL.parseChunk(buf, -Infinity)).not.toThrow();
      });
      ...
      test("start/end with values that overflow size_t", () => {
        ...
        expect(() => Fun.JSONL.parseChunk(buf, Number.MAX_SAFE_INTEGER)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, 0, Number.MAX_SAFE_INTEGER)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, 2 ** 53)).not.toThrow();
      });

      test("non-numeric start/end types don't crash", () => {
        const buf = new TextEncoder().encode('{"a":1}\n');
        // These get coerced or ignored
        expect(() => Fun.JSONL.parseChunk(buf, "5" as any)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, null as any)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, undefined as any)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, {} as any)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, [] as any)).not.toThrow();
        expect(() => Fun.JSONL.parseChunk(buf, true as any)).not.toThrow();
      });

      describe("start/end boundary security", () => {
        test("start = length returns empty", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, buf.length);
          expect(result.values).toStrictEqual([]);
        ...
        test("start = length, end = length returns empty", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, buf.length, buf.length);
          expect(result.values).toStrictEqual([]);
          expect(result.read).toBe(buf.length);
        });
        ...
        test("start = length - 1 reads last byte only", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, buf.length - 1);
          // Last byte is '\n', no complete value
          expect(result.values).toStrictEqual([]);
        });
        ...
        test("start = 0, end = 0 returns empty", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, 0, 0);
          expect(result.values).toStrictEqual([]);
          expect(result.read).toBe(0);
        });
        ...
        test("start = 0, end = 1 reads single byte", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, 0, 1);
        ...
        test("end = 0 with any start returns empty", () => {
          ...
          const result = Fun.JSONL.parseChunk(buf, 5, 0);
          expect(result.values).toStrictEqual([]);
        });

        test("start > end is clamped (no negative-length OOB)", () => {
          const buf = new TextEncoder().encode('{"a":1}\n{"b":2}\n');
          const result = Fun.JSONL.parseChunk(buf, 10, 5);
          expect(result.values).toStrictEqual([]);
          expect(result.read).toBe(5);
        });
        ...
        test("start beyond buffer length is clamped", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, 9999);
          expect(result.values).toStrictEqual([]);
          expect(result.read).toBe(buf.length);
        });
        ...
        test("end beyond buffer length is clamped", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, 0, 9999);
          expect(result.values).toStrictEqual([{ a: 1 }]);
        });

        test("start and end both beyond buffer length", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const result = Fun.JSONL.parseChunk(buf, 1000, 2000);
          expect(result.values).toStrictEqual([]);
        });
        ...
        test("exact value boundary: end one byte into next value", () => {
          const buf = new TextEncoder().encode('{"a":1}\n{"b":2}\n');
          // end=9 includes '\n' and '{' of second value
          const result = Fun.JSONL.parseChunk(buf, 0, 9);
          expect(result.values).toStrictEqual([{ a: 1 }]);
        });
        ...
        test("start at newline between values", () => {
          const buf = new TextEncoder().encode('{"a":1}\n{"b":2}\n');
          // start=7 is the '\n' between values
          const result = Fun.JSONL.parseChunk(buf, 7);
          expect(result.values).toStrictEqual([{ b: 2 }]);
        });
        ...
        test("end cuts a value in half", () => {
          ...
          for (let i = 1; i < 7; i++) {
            const result = Fun.JSONL.parseChunk(buf, 0, i);
            expect(result.values).toStrictEqual([]);
            expect(result.done).toBe(false);
          }
        });
        ...
        test("BOM boundary: start=0 end=3 (just BOM bytes)", () => {
          const bom = new Uint8Array([0xef, 0xbb, 0xbf]);
          const result = Fun.JSONL.parseChunk(bom, 0, 3);
        ...
        test("BOM boundary: start=3 skips past BOM manually", () => {
          ...
          const result = Fun.JSONL.parseChunk(buf, 3);
          expect(result.values).toStrictEqual([{ a: 1 }]);
        });
        ...
        test("BOM boundary: start=1 (inside BOM)", () => {
          ...
          const result = Fun.JSONL.parseChunk(buf, 1);
        ...
        test("BOM boundary: start=2 (inside BOM)", () => {
          ...
          buf.set(bom, 0);
          buf.set(json, bom.length);
          const result = Fun.JSONL.parseChunk(buf, 2);
          expect(Array.isArray(result.values)).toBe(true);
        });
        ...
        test("multi-byte UTF-8: start in middle of character doesn't OOB", () => {
          ...
          for (let i = 0; i < buf.length; i++) {
            const result = Fun.JSONL.parseChunk(buf, i);
            expect(Array.isArray(result.values)).toBe(true);
          }
        });
        ...
        test("every start/end combination on small buffer doesn't crash", () => {
          ...
          for (let s = 0; s <= buf.length; s++) {
            for (let e = 0; e <= buf.length; e++) {
              const result = Fun.JSONL.parseChunk(buf, s, e);
              expect(Array.isArray(result.values)).toBe(true);
              expect(typeof result.read).toBe("number");
        ...
        test("read never exceeds buffer length", () => {
          const buf = new TextEncoder().encode('{"a":1}\n{"b":2}\n{"c":3}\n');
          for (let s = 0; s <= buf.length; s++) {
            const result = Fun.JSONL.parseChunk(buf, s);
            expect(result.read).toBeLessThanOrEqual(buf.length);
        ...
        test("Uint8Array subarray view with offset", () => {
          ...
          const result = Fun.JSONL.parseChunk(view);
          expect(result.values).toStrictEqual([{ a: 1 }, { b: 2 }]);
        });
        ...
        test("Uint8Array subarray view with start/end offsets", () => {
          ...
          const result = Fun.JSONL.parseChunk(view, 8);
          expect(result.values).toStrictEqual([{ b: 2 }]);
        });
        ...
        test("ArrayBuffer (not Uint8Array) is treated as string via toString", () => {
          ...
          expect(() => Fun.JSONL.parseChunk(buf.buffer as any)).not.toThrow();
        });
        ...
        test("DataView is not treated as typed array", () => {
          ...
          expect(() => Fun.JSONL.parseChunk(dv as any)).not.toThrow();
        });

        test("Int8Array works as typed array input", () => {
          const buf = new TextEncoder().encode('{"a":1}\n');
          const i8 = new Int8Array(buf.buffer);
          const result = Fun.JSONL.parseChunk(i8);
          expect(result.values).toStrictEqual([{ a: 1 }]);
        });
        ...
        test("Uint8ClampedArray works as typed array input", () => {
          ...
          const result = Fun.JSONL.parseChunk(clamped);
          expect(result.values).toStrictEqual([{ a: 1 }]);
        });
      });
      ...
      test("rope string input (concatenated strings)", () => {
        ...
        const result = Fun.JSONL.parse(s);
      ...
      test("HTML/script injection in values doesn't execute", () => {
        ...
        const result = Fun.JSONL.parse(payloads.join("\n") + "\n");
        expect(result.length).toBe(4);
      ...
      test("ArrayBuffer with extra views shouldn't cross-contaminate", () => {
        ...
        const r1 = Fun.JSONL.parse(view1.subarray(0, src1.length));
        const r2 = Fun.JSONL.parse(view2.subarray(0, src2.length));
      ...
      test("input with surrogate pairs at chunk boundaries", () => {
        ...
        for (let split = 5; split < 20; split++) {
          const r1 = Fun.JSONL.parseChunk(full, 0, split);
    ...
    describe("session history attack vectors", () => {
      test("values containing fake JSONL structure don't split into multiple values", () => {
        ...
        const result = Fun.JSONL.parse(malicious);
        expect(result.length).toBe(1);
      ...
      test("overlong UTF-8 sequences rejected (security: directory traversal)", () => {
        ...
        const result = Fun.JSONL.parseChunk(overlong);
