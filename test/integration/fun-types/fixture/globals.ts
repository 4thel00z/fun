...
// FileBlob
expectType<ReadableStream<Uint8Array>>(Fun.file("index.test-d.ts").stream());
expectType<Promise<ArrayBuffer>>(Fun.file("index.test-d.ts").arrayBuffer());
expectType<Promise<Uint8Array>>(Fun.file("index.test-d.ts").bytes());
expectType<Promise<string>>(Fun.file("index.test-d.ts").text());

expectType<number>(Fun.file("index.test-d.ts").size);
expectType<string>(Fun.file("index.test-d.ts").type);

// Hash
expectType<string>(new Fun.MD4().update("test").digest("hex"));
expectType<string>(new Fun.MD5().update("test").digest("hex"));
expectType<string>(new Fun.SHA1().update("test").digest("hex"));
expectType<string>(new Fun.SHA224().update("test").digest("hex"));
expectType<string>(new Fun.SHA256().update("test").digest("hex"));
expectType<string>(new Fun.SHA384().update("test").digest("hex"));
expectType<string>(new Fun.SHA512().update("test").digest("hex"));
expectType<string>(new Fun.SHA512_256().update("test").digest("hex"));

// Zlib Functions
expectType<Uint8Array>(Fun.deflateSync(new Uint8Array(128)));
expectType<Uint8Array>(Fun.gzipSync(new Uint8Array(128)));
expectType<Uint8Array>(
  Fun.deflateSync(new Uint8Array(128), {
    level: -1,
    memLevel: 8,
    strategy: 0,
    windowBits: 15,
...
expectType<Uint8Array>(Fun.gzipSync(new Uint8Array(128), { level: 9, memLevel: 6, windowBits: 27 }));
expectType<Uint8Array>(Fun.inflateSync(new Uint8Array(64))); // Pretend this is DEFLATE compressed data
expectType<Uint8Array>(Fun.gunzipSync(new Uint8Array(64))); // Pretend this is GZIP compressed data
expectAssignable<Fun.ZlibCompressionOptions>({ windowBits: -11 });

// Other
expectType<Promise<number>>(Fun.write("test.json", "lol"));
expectType<Promise<number>>(Fun.write("test.json", new ArrayBuffer(32)));
expectType<URL>(Fun.pathToFileURL("/foo/bar.txt"));
expectType<string>(Fun.fileURLToPath(new URL("file:///foo/bar.txt")));
...
Fun.sleepSync(1); // sleep for 1 ms (not recommended)
await Fun.sleep(1); // sleep for 1 ms (recommended)
...
// leak to userland
expectType<FunConsumerConvenienceMethods>();
