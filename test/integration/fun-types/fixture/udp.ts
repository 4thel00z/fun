...
const socket = await Fun.udpSocket({
  port: 0,
});

expectType(socket.hostname).is<string>();
expectType(socket.port).is<number>();
expectType(socket.address).is<Fun.SocketAddress>();
expectType(socket.binaryType).is<Fun.BinaryType>();
...
const connectedSocket = await Fun.udpSocket({
  port: 0,
  connect: {
    hostname: "127.0.0.1",
    port: 41234,
...
expectType(connectedSocket.remoteAddress).is<Fun.SocketAddress>();
