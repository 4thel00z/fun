...
describe("fun:test", () => {
  ...
  test("expect()", () => {
    ...
    expect(["fun"]).toContain("fun");
    expect("hello").toContain("fun");
...
expectType(expect()).is<import("fun:test").Matchers<undefined>>();
expectType(expect<string>()).is<import("fun:test").Matchers<string | undefined>>();
expectType(expect("")).is<import("fun:test").Matchers<string>>();
expectType(expect<string>("")).is<import("fun:test").Matchers<string>>();
expectType(expect(undefined, "Fail message")).is<import("fun:test").Matchers<undefined>>();
expectType(expect<string>(undefined, "Fail message")).is<import("fun:test").Matchers<string | undefined>>();
expectType(expect("", "Fail message")).is<import("fun:test").Matchers<string>>();
expectType(expect<string>("", "Fail message")).is<import("fun:test").Matchers<string>>();
...
//       But when unassigbale, we should type error (e.g `string` is not assignable to `"fun"`)
// 2. => Expect that exact matches pass
// 3. => Expect that when we opt out of type safety, any value can be passed

declare const input: "fun" | "baz" | null;
declare const expected: string;

// @ts-expect-error
/** 1. **/ expect(input).toBe(expected); // Type error - string is not assignable to `'fun' | ...`
/** 2. **/ expect(input).toBe("fun"); // happy!
