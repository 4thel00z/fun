// This file is checked in the `fun-types.test.ts` integration test for successful typechecking, but also checked
// on its own to make sure that the types line up with actual implementation of Fun.serve()
...
// XXX: importing this from "harness" caused a failure in fun-types.test.ts
function tmpdirSync(pattern: string = "fun.test."): string {
...
function test<T = undefined, R extends string = string>(
  ...
  options: Fun.Serve.Options<T, R>,
  {
    ...
    overrideExpectBehavior?: (server: NoInfer<Fun.Server<T>>) => void | Promise<void>;
    skip?: boolean;
  } = {},
) {
  const skip = skipOptions || ("unix" in options && typeof options.unix === "string" && process.platform === "win32");

  async function testServer(server: Fun.Server<T>) {
  ...
  it.skipIf(skip)(name, async () => {
    try {
      using server = Fun.serve(options);
      try {
...
test("basic + websocket + upgrade", {
  websocket: {
    message(ws, message) {
      expectType<typeof ws>().is<Fun.ServerWebSocket<undefined>>();
      ws.send(message);
...
test("port 0 + websocket + upgrade", {
  ...
  websocket: {
    message(ws) {
      expectType(ws).is<Fun.ServerWebSocket<undefined>>();
    },
  },
});
...
test(
  ...
  {
    onConstructorFailure: error => {
      expect(error.message).toContain("Fun.serve() needs either:");
...
const files = {} as Record<string, Fun.FunFile>;

test("permutations of valid route values", {
  routes: {
    "/this/:test": Fun.file(import.meta.file),
    "/index.test-d.ts": Fun.file("index.test-d.ts"),
    // @ts-expect-error this is invalid
    "/index.test-d.ts.2": () => Fun.file("index.test-d.ts"),
...
test(
  ...
  // @ts-expect-error cannot pass unix and port at same time
  {
    ...
    websocket: { message: ws => expectType(ws).is<Fun.ServerWebSocket<undefined>>() },
  },
  {
    overrideExpectBehavior: server => {
      expect(server.hostname).toBeUndefined();
...
test("#24819 regression", {
  ...
  routes: {
    "/health": {
      GET: new Response("OK"),
      POST: req => {
        expectType(req).is<Fun.FunRequest<"/health">>();
