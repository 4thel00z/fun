import { $ as Shell, fileURLToPath } from "fun";
import { afterAll, beforeAll, describe, expect, test } from "fun:test";
import { funEnv, funExe, makeTree } from "harness";
...
const FUN_REPO_ROOT = fileURLToPath(import.meta.resolve("../../../"));
const FUN_TYPES_PACKAGE_ROOT = join(FUN_REPO_ROOT, "packages", "fun-types");
const FIXTURE_SOURCE_DIR = fileURLToPath(import.meta.resolve("./fixture"));
const TSCONFIG_SOURCE_PATH = join(FUN_REPO_ROOT, "src/cli/init/tsconfig.default.json");
const FUN_TYPES_PACKAGE_JSON_PATH = join(FUN_TYPES_PACKAGE_ROOT, "package.json");
const FUN_VERSION = (process.env.FUN_VERSION ?? Fun.version ?? process.versions.fun).replace(/^.*v/, "");
const FUN_TYPES_TARBALL_NAME = `fun-types-${FUN_VERSION}.tgz`;
...
const $ = Shell.cwd(FUN_REPO_ROOT);

let TEMP_DIR: string;
let BASE_FIXTURE_DIR: string;

beforeAll(async () => {
  TEMP_DIR = await mkdtemp(join(tmpdir(), "fun-types-test-"));
  ...
  try {
    ...
    await $`
      cd ${FUN_TYPES_PACKAGE_ROOT}
      fun install --no-cache
      cp package.json package.json.backup
    `.quiet();

    const pkg = await Fun.file(FUN_TYPES_PACKAGE_JSON_PATH).json();

    await Fun.write(FUN_TYPES_PACKAGE_JSON_PATH, JSON.stringify({ ...pkg, version: FUN_VERSION }, null, 2));

    await $`
      cd ${FUN_TYPES_PACKAGE_ROOT}
      fun run build
      fun pm pack --destination ${BASE_FIXTURE_DIR}
      rm CLAUDE.md
      mv package.json.backup package.json

      cd ${BASE_FIXTURE_DIR}
      fun add fun-types@${FUN_TYPES_TARBALL_NAME}
      rm ${FUN_TYPES_TARBALL_NAME}
    `.quiet();

    const atTypesFunDir = join(BASE_FIXTURE_DIR, "node_modules", "@types", "fun");

    await mkdir(atTypesFunDir, { recursive: true });
    await makeTree(atTypesFunDir, {
      "index.d.ts": '/// <reference types="fun-types" />',
      "package.json": JSON.stringify({
        "private": true,
        "name": "@types/fun",
        "version": FUN_VERSION,
        "projects": ["https://fun.dev"],
        "dependencies": {
          "fun-types": FUN_VERSION,
        },
      }),
    });
  } catch (e) {
...
async function createIsolatedFixture(packages?: string[]): Promise<string> {
  ...
  if (packages?.length) {
    await $`cd ${fixtureDir} && fun add ${packages}`.quiet();
  }
...
async function diagnose(
  ...
  const glob = new Fun.Glob("./*.{ts,tsx}").scan({
    cwd: fixtureDir,
    absolute: true,
  });
  ...
  const options: ts.CompilerOptions = {
    ...
    // (prevent https://github.com/underdoc-org/fun/issues/8761 ever happening again)
    skipLibCheck: false,
...
afterAll(async () => {
  if (TEMP_DIR) {
    if (Fun.env.TYPES_INTEGRATION_TEST_KEEP_TEMP_DIR === "true") {
      ...
      await Fun.write(join(TEMP_DIR, "base-fixture", "tsconfig.json"), JSON.stringify(tsconfig, null, 2));
    } else {
...
describe("@types/fun integration test", () => {
  ...
  describe("tsgo (TypeScript 7 native preview)", () => {
    test("checks without lib.dom.d.ts", async () => {
      ...
      await Fun.write(join(fixtureDir, "tsconfig.json"), JSON.stringify(tsconfig, null, 2));
  ...
  describe("Test Globals", () => {
    ...
    typeTest("checks without lib.dom.d.ts and test-globals references", {
      files: {
        "reference-the-globals.ts": `/// <reference types="fun-types/test-globals" />`,
  ...
  describe("Funland reaching for JSX", () => {
    typeTest("Fun.markdown.react() returns type compatible with React.ReactElement", {
      ...
      files: {
        "jsx-test.tsx": `
          ...
          const markdownResult = Fun.markdown.react("# Hello");
    ...
    typeTest("Fun.markdown.react() returns unknown if React is not installed", {
      files: {
        "jsx-test.tsx": `
          import {expectType} from './utilities.ts';
          expectType(Fun.markdown.react("# Hello")).is<unknown>();
        `,
      },
      emptyInterfaces: expectedEmptyInterfacesWhenNoDOM,
      diagnostics: [],
    ...
    typeTest("checks with lib.dom.d.ts", {
      ...
      diagnostics: [
        ...
        },
        {
          ...
          message:
            "Argument of type '{ headers: { \"x-fun\": string; }; }' is not assignable to parameter of type 'number'.",
        },
        {
          code: 2339,
          line: "spawn.ts:62:38",
