...
// This may not be a bug in itself, but the async version creates data that causes an out of memory error when decompressed with Fun.zstdDecompressSync
describe("zstd compression compatibility", () => {
  ...
  it("should decompress data compressed with zlib.zstdCompress using Fun.zstdDecompress", async () => {
    const input = "hello world";
    const compressed = await new Promise<Buffer>((resolve, reject) => {
      zlib.zstdCompress(input, (err, result) => {
        if (err) reject(err);
    ...
    const decompressed = await Fun.zstdDecompress(compressed);
    expect(decompressed.toString()).toBe(input);
  });
});
