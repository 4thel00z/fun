...
// https://github.com/underdoc-org/fun/issues/27014
// Fun.stripANSI() hangs on strings with control characters in 0x10-0x1F
...
test("stripANSI still strips real ANSI escape sequences", () => {
  ...
  expect(Fun.stripANSI(input)).toBe("hello");
});
...
test("stripANSI handles mix of false-positive control chars and real escapes", () => {
  ...
  expect(Fun.stripANSI(input)).toBe("\x16beforecoloredafter\x19end");
});
...
test("stripANSI handles string of only non-escape control characters", () => {
  ...
  expect(Fun.stripANSI(input)).toBe(input);
});
...
test("stripANSI finds real escape after false positives in same SIMD chunk", () => {
  ...
  expect(Fun.stripANSI(input)).toBe("\x10\x11\x12red");
});
...
test("stripANSI handles many false positives followed by real escape in same chunk", () => {
  ...
  expect(Fun.stripANSI(input)).toBe("\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1c\x1d!");
});
