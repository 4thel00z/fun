...
// https://github.com/underdoc-org/fun/issues/17294
...
test("empty string interpolation passes empty arg", async () => {
  const result = await $`${funExe()} -e "console.log(JSON.stringify(process.argv.slice(1)))" -- ${""}`
    .env(funEnv)
    .text();
...
test("double-quoted empty string passes empty arg", async () => {
  const result = await $`${funExe()} -e "console.log(JSON.stringify(process.argv.slice(1)))" -- ""`.env(funEnv).text();
  expect(JSON.parse(result.trim())).toEqual([""]);
});
...
test("non-empty string still works (control)", async () => {
  const result = await $`${funExe()} -e "console.log(JSON.stringify(process.argv.slice(1)))" -- ${"hello"}`
    .env(funEnv)
    .text();
...
test("empty string between non-empty strings", async () => {
  const result = await $`${funExe()} -e "console.log(JSON.stringify(process.argv.slice(1)))" -- ${"a"} ${""} ${"b"}`
    .env(funEnv)
    .text();
...
test("empty interpolation inside single quotes passes empty arg", async () => {
  const empty = "";
  const result = await $`${funExe()} -e "console.log(JSON.stringify(process.argv.slice(1)))" -- ${empty}`
    .env(funEnv)
    .text();
  expect(JSON.parse(result.trim())).toEqual([""]);
});
...
test("tilde with empty double quotes expands to homedir", async () => {
  const result = await $`echo ~""`.env(funEnv).text();
