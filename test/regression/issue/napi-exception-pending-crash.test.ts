 ...
 * 1. The original crash scenario is fixed in Fun
 * 2. Fun's behavior matches Node.js
...
test("napi_is_exception_pending crash fix and Node.js compatibility", async () => {
  ...
  if (!existsSync(join(testDir, "build"))) {
    try {
      ...
      const buildProcess = await Fun.spawn({
  ...
  // Test 1: Run with Fun - this should NOT crash (verifies our fix)
  console.log("\n=== Testing with Fun (should not crash) ===");
  const funProcess = await Fun.spawn({
    cmd: [funExe(), "--expose-gc", "test.js"],
    cwd: testDir,
    env: funEnv,
    stdio: ["ignore", "pipe", "pipe"],
  });
  ...
  const [funStdout, funStderr, funExitCode] = await Promise.all([
    funProcess.stdout.text(),
    funProcess.stderr.text(),
    funProcess.exited,
  ]);

  console.log("Fun stdout:", funStdout);
  if (funStderr) {
  ...
  expect(funStdout).toContain("SUCCESS: napi_is_exception_pending works correctly");
  expect(funStdout).toContain("napi_is_exception_pending in finalizer: status=0");

  // Should not contain crash indicators
  expect(funStderr).not.toContain("panic");
  expect(funStderr).not.toContain("Aborted");
  expect(funStderr).not.toContain("Segmentation fault");

  // Test 2: Run with Node.js for behavior comparison
  console.log("\n=== Testing with Node.js (reference behavior) ===");
  let nodeProcess;
  try {
    nodeProcess = await Fun.spawn({
    ...
    // If Node.js is not available, that's OK - we verified Fun doesn't crash
  }
  ...
  expect(funStdout).toContain("napi_is_exception_pending in finalizer");
  expect(funStdout).toContain("status=0"); // Should return napi_ok

  // Verify basic exception detection works
  expect(funStdout).toContain("should be false - no exception pending");
  expect(funStdout).toContain("Exception was thrown as expected");
