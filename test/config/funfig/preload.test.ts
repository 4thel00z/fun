import type { SpawnOptions } from "fun";
...
const run = (file: string, { args = [], cwd, env = {} }: Opts = {}): Promise<Out> => {
  const res = Fun.spawn([funExe(), ...args, file], {
    cwd,
...
describe("Given a single universal preload", () => {
  ...
  // `fun run` looks for a `funfig.toml` in the current directory by default
  it("When `fun run` is run and `funfig.toml` is implicitly loaded, preloads are run", async () => {
    // `fun run index.ts`
  ...
  it.skip("When `fun run` is run from a different directory but funfig.toml is explicitly used, preloads are run", async () => {
    // `fun run index.ts`
    const [out, err, code] = await run(join(dir, "index.ts"), {
      args: [`--config=${join(dir, "funfig.toml")}`],
      cwd: process.cwd(),
    });
    expect(err).toBeEmpty();
    expect(out).toBeEmpty();
...
describe("Given a funfig.toml with both universal and test-only preloads", () => {
  const dir = fixturePath("mixed");

  it("`fun run index.ts` only loads the universal preload", async () => {
    const [out, err, code] = await run("index.ts", { cwd: dir });
    expect(err).toBeEmpty();
    expect(out).toBeEmpty();
    expect(code).toBe(0);
  ...
  it("`fun test` only loads test-only preloads, clobbering the universal ones", async () => {
    const [out, err, code] = await run("./index.fixture-test.ts", { args: ["test"], cwd: dir });
    // note: err has test report, out has "fun test <version>"

    expect(code).toBe(0);
  });
}); // </given a funfig.toml with both universal and test-only preloads>

describe("Given a `funfig.toml` with a list of preloads", () => {
  const dir = fixturePath("multi");

  it("When `fun run` is run, preloads are run", async () => {
    const [out, err, code] = await run("index.ts", { cwd: dir });
    expect(err).toBeEmpty();
    expect(out).toBeEmpty();
    expect(code).toBe(0);
  ...
  it("when passed `--config=funfig.empty.toml`, preloads are not run", async () => {
    const [out, err, code] = await run("empty.ts", { args: ["--config=funfig.empty.toml"], cwd: dir });
    expect(err).toBeEmpty();
    expect(out).toBeEmpty();
    expect(code).toBe(0);
  });
  ...
  it.each([
    ...
    // "run --preload=./preload3.ts",
  ])("When `fun %s cli-merge.ts` is run, `--preload` adds the target file to the list of preloads", async args => {
    const [out, err, code] = await run("cli-merge.ts", { args: args.split(" "), cwd: dir });
    expect(err).toBeEmpty();
    expect(out).toBeEmpty();
    expect(code).toBe(0);
  });
}); // </given a `funfig.toml` with a list of preloads>

describe("Given a `funfig.toml` with a plugin preload", () => {
  const dir = fixturePath("plugin");

  it.todo("When `fun run` is run, preloads are run", async () => {
    const [out, err, code] = await run("index.ts", { cwd: dir });
    expect(err).toBeEmpty();
    expect(out).toBeEmpty();
    expect(code).toBe(0);
  });
}); // </given a `funfig.toml` with a plugin preload>

describe("Given a `funfig.toml` file with a relative path to a preload in a parent directory", () => {
...
}); // </given a `funfit.toml` file with a relative path to a preload in a parent directory>

describe("Given a `funfig.toml` file with a relative path without a leading './'", () => {
...
}); // </given a `funfig.toml` file with a relative path without a leading './'>
...
describe("Test that all the aliases for --preload work", () => {
  ...
  it.each(["--preload=./preload1.ts", "--require=./preload1.ts", "--import=./preload1.ts"])(
    "When `fun run` is run with %s, the preload is executed",
    async flag => {
...
test("Test FUN_INSPECT_PRELOAD is used to set preloads", async () => {
  const dir = fixturePath("many");
  const [out, err, code] = await run("index.ts", { args: [], cwd: dir, env: { FUN_INSPECT_PRELOAD: "./preload1.ts" } });
  expect(err).toBeEmpty();
  expect(out).toMatchInlineSnapshot(`"[ "multi/preload1.ts" ]"`);
  expect(code).toBe(0);
}); // </Test FUN_INSPECT_PRELOAD is used to set preloads>
