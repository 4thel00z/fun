...
**Idempotent.** Run `fun run build` over and over — that's it. Same inputs produce the same `build.ninja`, ninja sees nothing changed, exits immediately. No state drift between runs, no "did I clean first?", no sticky flags from last time. The mechanisms:

- **Configure always runs.** Every `fun run build` reconfigures before spawning ninja — no separate first-time configure step, no cached options that persist across runs. Config is `profile + overrides` evaluated fresh each time. Fast enough to not matter (~200ms — we haven't tried to make it faster yet).
...
- `direct` — list the dep's sources explicitly; each becomes a first-class `cc`/`cxx` edge in our graph and the `.o`s go straight into fun's link. The default for the C/C++ deps (zlib, zstd, boringssl, libarchive, mimalloc, …). Skips a sub-process configure entirely and lets LTO see across the dep boundary.
...
```ninja
rule dep_fetch
  command = fun fetch-cli.ts dep $name $repo $commit $dest ...
  restat = 1
  pool = dep
...
```sh
fun scripts/build.ts --configure-only       # regenerate build.ninja, don't run ninja
funx tsc --noEmit -p scripts/build/tsconfig.json   # typecheck
...
ninja -C build/debug <target>               # build a specific target (e.g. tinycc, fun-zig.o)
```
...
`fun scripts/build.ts [build-flags] [exec-args...]`. The cutoff: first arg that isn't a recognized build/ninja flag ends build-flag parsing — it and everything after go to the built binary.
...
Build flags must come before exec args. `fun bd --asan=off test foo.ts` works; `fun bd test --asan=off foo.ts` sends `--asan=off` to fun-debug. Use `--` when a runtime flag collides with a build flag: `fun bd -- --target=browser script.ts`.

**`--target=<name>`** builds a specific ninja target instead of the full binary. Every dep gets phonies: `<name>` (full build), `clone-<name>` (fetch only), `configure-<name>` (cmake deps). Also `fun`, `check`, `fun-zig.o`. List all: `ninja -C build/debug -t targets`.
...
Tables: `cpuTargetFlags` (`-march`/`-mcpu`/`-mtune` — also forwarded to local WebKit via `computeCpuTargetFlags()`), `globalFlags` (fun + all deps), `funOnlyFlags` (just fun), `linkFlags`, `stripFlags`. Use `lang: "cxx"` to restrict to C++.
...
## Build flow: `fun run build` → binary
...
1. `resolveToolchain()` — find clang/ar/lld/strip/cmake/cargo/fun/zig/esbuild. Version-checked where it matters; paths stored on `Toolchain`.
...
3. `validateFunConfig(cfg)` + `checkWorkarounds(cfg)` — fail early with clear errors.
...
7. `emitFun(n, cfg, sources)` — assemble the build graph (see Phase 2).
...
### Phase 2 — emitFun (`fun.ts::emitFun`)
...
3. **Zig** — `emitZig(n, cfg, {...})` emits zig download + `zig build obj` → `fun-zig.o`.
...
| `fun.ts`                       | `emitFun()` — assembles deps+codegen+zig+compile+link                              |
...
| `depVersionsHeader.ts`         | Generates `fun_dependency_versions.h` for `process.versions`                       |
...
| `clean.ts`                     | `fun run clean` preset-based cleanup                                               |
...
**Windows `ReleaseFast` → `ReleaseSafe`** in `zig.ts`. Load-bearing since Fun 1.1; caught more crashes. Don't "fix" it.
...
`cfg.jsRuntime` holds the shell-ready command prefix for running `.ts` subprocesses (stream.ts, fetch-cli.ts, the regen rule) — it's `process.execPath` when fun runs configure, or `node --experimental-strip-types` when node does. The subprocesses inherit whichever runtime started the build.

**TODO — remaining `cfg.fun` usage (codegen only):** For a fully fun-optional build:

- `cfg.packageManager` — `fun install` or `npm install` for the one codegen install step.
...
With those done, `cfg.fun` disappears.
