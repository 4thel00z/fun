 ...
 *   - "cpp-only": compile to libfun.a, skip zig/link (CI upstream) — TODO(ci-split)
...
import { funExeName, shouldStrip, type Config } from "./config.ts";
...
import { funIncludes, computeFlags, extraFlagsFor, linkDepends } from "./flags.ts";
...
// in config.ts now so flags.ts can use funExeName without circular import.
export { funExeName, shouldStrip };
...
/**
 * Output of `emitFun()`. Paths to the produced artifacts and resolved
 ...
 */
export interface FunOutput {
  /** Linked executable (fun-debug, fun-profile). Full/link-only. */
  exe?: string;
  /** Stripped `fun`. Plain release full/link-only. */
  ...
  /** libfun.a — all C/C++ objects archived. cpp-only. */
  archive?: string;
...
/**
 * Emit the full fun build graph. Returns the output executable path.
 *
 ...
 */
export function emitFun(n: Ninja, cfg: Config, sources: Sources): FunOutput {
  ...
  // instead of a .a — we compile those alongside fun's own sources).
  ...
  // fun_dependency_versions.h — written at configure time, not a ninja rule.
  // FunProcess.cpp includes it for process.versions. writeIfNotChanged
  ...
  // Full include set: fun's own + all dep includes + buildDir (for the
  // generated versions header).
  const allIncludes = [...funIncludes(cfg), cfg.buildDir, ...depIncludes];
  ...
  if (usePch) {
    ...
    // mode where it fails on the pinned fun version (see cppAll docstring).
  ...
  // Deps that contribute source files for fun to compile directly (via
  // provides.sources) instead of building a lib. Compile them here with
  // fun's full flag set and give each a phony so `--target <name>` builds
  ...
  // Dep objects (when !cfg.archiveDeps) are linked alongside fun's own
  ...
  // CI's build-cpp step: archive all .o into libfun.a, stop. The sibling
  // build-zig step produces fun-zig.o independently; build-fun downloads
  // both artifacts and links them. Archive name uses the exe name (not
  // just "libfun") so asan/debug variants are distinguishable in artifacts.
  if (cfg.mode === "cpp-only") {
    ...
    n.phony("fun", [archive, ...depLibs, ...(depUploadStamp ? [depUploadStamp] : [])]);
    n.default(["fun"]);
  ...
  // Plain release only: produce stripped `fun` alongside `fun-profile`.
  ...
  if (shouldStrip(cfg)) {
    ...
    // bundle. dsymutil reads DWARF from fun-profile, writes fun-profile.dSYM.
    // Must run BEFORE stripping could discard sections it needs (we don't
    // strip fun-profile itself, only copy → fun, so this is safe).
    if (cfg.darwin) {
      dsym = emitDsymutil(n, cfg, exe, exeName);
    }
  }

  // Phony `fun` target for convenience — only when strip DIDN'T produce a
  // literal file named `fun` (which would collide with the phony). When
  // strip runs, `ninja fun` builds the actual stripped file; no phony needed.
 ...
 * this to cross-compile fun-zig.o on a linux box for all target platforms
 ...
 */
function emitZigOnly(n: Ninja, cfg: Config, sources: Sources): FunOutput {
  n.comment("════════════════════════════════════════════════════════════════");
  n.comment(`  Building fun-zig.o (zig-only, target: ${cfg.os}-${cfg.arch})`);
  n.comment("════════════════════════════════════════════════════════════════");
  n.blank();
 ...
 * CI's build-fun step. Build.ts downloads into buildDir BEFORE ninja runs;
 ...
 *   - libfun-profile.a            — from cpp-only's ar()
 *   - fun-zig.o (or fun-zig.{i}.o for ASAN — see zigObjectPaths)
 *   - deps/<name>/lib<name>.a     — from cpp-only's dep builds
 *   - cache/webkit-<hash>/lib/... — WebKit prebuilt (same cache path)
 */
function emitLinkOnly(n: Ninja, cfg: Config): FunOutput {
  const exeName = funExeName(cfg);
  ...
  // prefix/suffix, e.g. libfun-profile.a).
  const archive = resolve(cfg.buildDir, `${cfg.libPrefix}${exeName}${cfg.libSuffix}`);

  // fun-zig*.o from zig-only: same paths emitZig writes to. Shared
  ...
  // knows. Matches cmake's FUN_LINK_ONLY adding WINDOWS_RESOURCES directly.
  const windowsRes = cfg.windows ? [emitWindowsResources(n, cfg)] : [];
  ...
  if (strippedExe === undefined) n.phony("fun", [exe]);
  emitSmokeTest(n, cfg, exe, exeName);
...
function emitSmokeTest(n: Ninja, cfg: Config, exe: string, exeName: string): void {
  ...
  const envWrap = "env FUN_DEBUG_QUIET_LOGS=1";
  let testCmd: string;
...
/**
 * Strip the linked executable → plain `fun`. Returns absolute path to
 * the stripped output.
 *
 * Input (fun-profile) is NOT modified — strip writes a new file via `-o`.
...
function emitStrip(n: Ninja, cfg: Config, inputExe: string, stripflags: string[]): string {
  const out = resolve(cfg.buildDir, "fun" + cfg.exeSuffix);

  // Windows: strip equivalent is handled at link time (/OPT:REF etc), no
  // separate strip binary. The "stripped" fun is just a copy.
  if (cfg.windows) {
 ...
 *
 * Runs dsymutil on fun-profile (which has full DWARF). The .dSYM lets you
 * symbolicate crash logs from the stripped `fun` — lldb/Instruments find
 ...
 * `fun build --compile --windows-title ...` runs. Without it, the copied
 * fun.exe has no VersionInfo to update and rescle silently does nothing.
 *
...
function emitWindowsResources(n: Ninja, cfg: Config): string {
  ...
  const ico = resolve(cfg.cwd, "src/fun.ico");
  ...
  const rcOut = rcIn
    .replace(/@Fun_VERSION_MAJOR@/g, major)
    .replace(/@Fun_VERSION_MINOR@/g, minor)
    .replace(/@Fun_VERSION_PATCH@/g, patch)
    .replace(/@Fun_VERSION_WITH_TAG@/g, versionWithTag)
    .replace(/@FUN_ICO_PATH@/g, slash(ico));
...
/**
 * Linker flags to embed fun.exe.manifest into the executable.
...
function manifestLinkFlags(cfg: Config): string[] {
  ...
  const manifest = resolve(cfg.cwd, "src/fun.exe.manifest");
...
function linkImplicitInputs(cfg: Config): string[] {
  ...
  if (cfg.windows) files.push(resolve(cfg.cwd, "src/fun.exe.manifest"));
  return files;
}
 ...
 */
export function validateFunConfig(cfg: Config): void {
