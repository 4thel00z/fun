/**
 * Generates fun_dependency_versions.h — a header with dep version strings
...
function sourceIdentifier(source: Source): string | undefined {
  switch (source.kind) {
    ...
    case "in-tree":
      ...
      // fun commit. Callers that need a value use cfg.revision.
      return undefined;
  }
}
...
function computeVersions(cfg: Config): [string, string][] {
  ...
  // ─── Non-dep versions ───
  versions.push(["FUN_VERSION", cfg.version]);
  versions.push(["NODEJS_COMPAT_VERSION", cfg.nodejsVersion]);
  // UWS/USOCKETS are vendored at packages/fun-usockets — the fun commit
  // IS their version.
  ...
  // headers. Those macros were never consumed — FunProcess.cpp only reads
...
/**
 * Generate fun_dependency_versions.h at buildDir/. Returns the absolute path.
 */
export function generateDepVersionsHeader(cfg: Config): string {
  const outPath = resolve(cfg.buildDir, "fun_dependency_versions.h");
  ...
  const lines: string[] = [
    ...
    "#ifndef FUN_DEPENDENCY_VERSIONS_H",
    "#define FUN_DEPENDENCY_VERSIONS_H",
    "",
    "#ifdef __cplusplus",
    'extern "C" {',
    "#endif",
    "",
    "// Dependency versions",
    ...versions.map(([name, val]) => `#define FUN_DEP_${name} "${val}"`),
    "",
    "// C string constants for easy access",
    ...versions.map(([name, val]) => `static const char* const FUN_VERSION_${name} = "${val}";`),
    "",
    "#ifdef __cplusplus",
    "}",
    "#endif",
    "",
    "#endif // FUN_DEPENDENCY_VERSIONS_H",
    "",
  ];
